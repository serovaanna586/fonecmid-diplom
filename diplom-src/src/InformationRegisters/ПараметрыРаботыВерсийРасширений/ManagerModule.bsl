...
Функция ПараметрРаботыРасширения(ИмяПараметра, БезУчетаВерсииРасширений = Ложь) Экспорт
	...
	ВерсияРасширений = ?(БезУчетаВерсииРасширений, Справочники.ВерсииРасширений.ПустаяСсылка(),
...
Процедура ЗаполнитьВсеПараметрыРаботыРасширенийФоновоеЗадание(Параметры) Экспорт
	...
	Если Параметры.ИмяКонфигурации <> Метаданные.Имя Или Параметры.ВерсияКонфигурации <> Метаданные.Версия Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось обновить все параметры работы расширений, так как
	...
	Если ТипЗнч(Параметры.ПроверяемыеРасширения) = Тип("Соответствие") Тогда
		...
		Для Каждого ПроверяемоеРасширение Из Параметры.ПроверяемыеРасширения Цикл
			Если ПодключенныеРасширения[ПроверяемоеРасширение.Ключ] = Неопределено Тогда
				НеподключенныеРасширения = НеподключенныеРасширения + ?(НеподключенныеРасширения = "", "", ", ")
					+ ПроверяемоеРасширение.Значение;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "",
	ТекстДляАсинхронногоВызова = "") Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Общая форма Расширения: Заполнение параметров работы расширений'",
		ОбщегоНазначения.КодОсновногоЯзыка()) + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'из сеанса %1 от %2'", ОбщегоНазначения.КодОсновногоЯзыка()), Формат(ТекущийСеанс.НомерСеанса,
		"ЧГ="), Формат(ТекущийСеанс.НачалоСеанса, "ДЛФ=DT")) + ")";
...
Процедура ПриПервомСерверномВызове() Экспорт
	...
	Если ТекущийРежимЗапуска() = Неопределено Или Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()
		Или Не ТребуетсяВключитьЗаполнениеПараметровРаботыРасширений()
		Или РегистрыСведений.ПараметрыРаботыПрограммы.НеобходимоОбновление() Или Не ОбщегоНазначения.РазделениеВключено()
		И ПланыОбмена.ГлавныйУзел() = Неопределено И ЗначениеЗаполнено(Константы.ГлавныйУзел.Получить()) Тогда
		Возврат;
	КонецЕсли;
	...
	ВключитьЗаполнениеПараметровРаботыРасширений(СтрНайти(ПараметрыЗапуска, "РежимОтладки") > 0
		Или Не ОбщегоНазначения.ИнформационнаяБазаФайловая() И ОбщегоНазначения.РазделениеВключено());
...
Процедура ПриУспешномНачалеВыполненияРегламентногоЗадания() Экспорт
	...
	Запустить = ТекущееФоновоеЗадание = Неопределено Или ТекущееФоновоеЗадание.ИмяМетода <> МетаданныеЗадания.ИмяМетода;
...
Процедура ЗаполнитьВсеПараметрыРаботыПоследнейВерсииРасширений() Экспорт
	...
	Попытка
		...
		ЗаполнитьАктивноеФоновоеЗадание(ИсполняющееФоновоеЗадание, ИдентификаторЗадания);
		Если ИсполняющееФоновоеЗадание = Неопределено Тогда
			СтандартныеПодсистемыСервер.УстановитьПараметрРаботыРасширения(ИмяПараметра, ИдентификаторТекущегоЗадания,
				Истина);
		КонецЕсли;
		ЗафиксироватьТранзакцию();
	Исключение
	...
	Если ВерсияПриЗапуске.ВерсияРасширений <> ВерсияРасширений Или КонфигурацияБазыДанныхИзмененаДинамически()
	...
	Пока Истина Цикл
		...
		Исключение
			...
			Если Не ТребуетсяПерезапускСеанса Или Не СтандартныеПодсистемыСервер.ЭтоОшибкаТребованияПерезапускаСеанса(
				...
				Если ПредыдущаяОшибка = ТекущаяОшибка Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						ОбщегоНазначения.КодОсновногоЯзыка()), Формат(НомерПопытки, "ЧГ="), Формат(КоличествоПопыток,
						"ЧГ="));
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При попытке заполнения %1 из %2 возникла ошибка:
							 |%3'", ОбщегоНазначения.КодОсновногоЯзыка()), Формат(НомерПопытки, "ЧГ="), Формат(
						КоличествоПопыток, "ЧГ="), ТекущаяОшибка);
					ДобавитьДополнительныеСведения(ТекстОшибки);
				КонецЕсли;
				ПредыдущаяОшибка = ТекущаяОшибка;
				ЗаписьЖурналаРегистрации(ИмяСобытияЗаполненияПараметров(), УровеньЖурналаРегистрации.Ошибка, , ,
					ТекстОшибки);
			КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПредыдущаяОшибка) Тогда
		Комментарий = НСтр("ru = 'Заполнение выполнено успешно.'", ОбщегоНазначения.КодОсновногоЯзыка());
		ДобавитьДополнительныеСведения(Комментарий);
		ЗаписьЖурналаРегистрации(ИмяСобытияЗаполненияПараметров(), УровеньЖурналаРегистрации.Информация, , ,
			Комментарий);
	КонецЕсли;
...
Функция НаименованиеЗаданияЗаполненияПараметров() Экспорт
	...
	Возврат НСтр("ru = 'Заполнение параметров работы расширений'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбработчикДлительнойОперацииВыполнитьОбновлениеРазделенныхДанных(Параметры, АдресРезультата) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось обновить параметры работы расширений по причине:
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		Если Параметры.ПодключаемыеКоманды.ПодключаемыеКомандыРасширений.Обновлять Тогда
			Параметры.ПодключаемыеКоманды.ПодключаемыеКомандыРасширений.ЕстьИзменения = МодульПодключаемыеКоманды.ПриЗаполненииВсехПараметровРаботыРасширений().ЕстьИзменения;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Если Параметры.ВариантыОтчетов.ОтчетыКонфигурации.Обновлять Тогда
			...
			Параметры.ВариантыОтчетов.ОтчетыКонфигурации.ЕстьИзменения = МодульВариантыОтчетов.Обновить(
		...
		Если Параметры.ВариантыОтчетов.ИндексПоискаОтчетовКонфигурации.Обновлять Тогда
			...
			Параметры.ВариантыОтчетов.ИндексПоискаОтчетовКонфигурации.ЕстьИзменения = МодульВариантыОтчетов.Обновить(
				Настройки).ЕстьИзменения;
		КонецЕсли;
		...
		Если Параметры.ВариантыОтчетов.ОтчетыРасширений.Обновлять Тогда
			Настройки.Оперативное = Истина;
			Настройки.Отложенное = Ложь;
			Параметры.ВариантыОтчетов.ОтчетыРасширений.ЕстьИзменения = МодульВариантыОтчетов.Обновить(
				Настройки).ЕстьИзменения;
		КонецЕсли;
		Если Параметры.ВариантыОтчетов.ИндексПоискаОтчетовРасширений.Обновлять Тогда
			...
			Настройки.Отложенное = Истина;
			Настройки.ИндексироватьСхему = Истина;
			Параметры.ВариантыОтчетов.ИндексПоискаОтчетовРасширений.ЕстьИзменения = МодульВариантыОтчетов.Обновить(
				Настройки).ЕстьИзменения;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Если ВключитьБезусловно Или ИдентификаторОбновления <> ИдентификаторЗаполнения Тогда
			Если ИдентификаторОбновления <> ИдентификаторЗаполнения Тогда
				СтандартныеПодсистемыСервер.УстановитьПараметрРаботыРасширения(ИмяПараметра, ИдентификаторОбновления,
					Истина);
			КонецЕсли;
...
Процедура ЗаполнитьАктивноеФоновоеЗадание(ИсполняющееФоновоеЗадание, ИдентификаторЗадания)
	...
	Если ТипЗнч(ИдентификаторЗадания) = Тип("УникальныйИдентификатор") Тогда
		...
		Если ИсполняющееФоновоеЗадание <> Неопределено И ИсполняющееФоновоеЗадание.Состояние
...
Процедура ОтключитьЗаполнениеПараметровРаботыРасширений(ВерсияПриЗапуске, Перезапустить = Ложь,
	...
	Попытка
		...
		Если ВерсияПриЗапуске.ВерсияРасширений = ТекущаяВерсия.ВерсияРасширений И ВерсияПриЗапуске.ДатаОбновления
			= ТекущаяВерсия.ДатаОбновления И (ОбновитьИдентификаторЗаполнения
			Или Не ТребуетсяВключитьЗаполнениеПараметровРаботыРасширений())
...
Процедура ЗапуститьЗаполнениеПараметровРаботыРасширений(ОжидатьЗавершения = Ложь) Экспорт
	...
	Если ТранзакцияАктивна() Или МонопольныйРежим() И Не ОжидатьЗавершения
		Или КонфигурацияБазыДанныхИзмененаДинамически() И ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
	...
	НаименованиеЗадания = НСтр("ru = 'Автозапуск'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": "
		+ МетаданныеЗадания.Синоним + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'из сеанса %1 от %2'", ОбщегоНазначения.КодОсновногоЯзыка()), Формат(ТекущийСеанс.НомерСеанса,
		"ЧГ="), Формат(ТекущийСеанс.НачалоСеанса, "ДЛФ=DT")) + ")";
	...
	Если ОжидатьЗавершения И ФоновоеЗадание <> Неопределено И (ТекущийРежимЗапуска() <> Неопределено
...
Процедура ДобавитьДополнительныеСведения(Комментарий)
	...
	ДополнительныеСведения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '*** Дополнительные сведения ***
			 ...
			 |%5'", ОбщегоНазначения.КодОсновногоЯзыка()), ОписаниеРасширений.Подключенные,
		ОписаниеРасширений.Отключенные, ОписаниеРасширений.Все, Строка(ПараметрыСеанса.ВерсияРасширений),
