...
&НаКлиенте
Процедура ГиперссылкаВыборОбработчиковОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
...
Процедура ГиперссылкаВыборОбработчиковНажатие(Элемент)
	...
	ОткрытьФорму("РегистрСведений.ОбработчикиОбновления.Форма.ФормаВыбора", ПараметрыВыбора, , , , ,
		ОбработкаЗавершения);
КонецПроцедуры
...
Процедура ПослеВыбораОбработчиков(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Элементы.ГиперссылкаВыборОбработчиков.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбрано обработчиков: %1'"), ВыбранныеОбработчики.Количество());
	КонецЕсли;
...
Функция ДлительнаяОперация()
	...
	Результат = ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения,
		"ОбновлениеИнформационнойБазы.ПерезапуститьОтложенноеОбновление", ВыбранныеОбработчики.ВыгрузитьЗначения());
