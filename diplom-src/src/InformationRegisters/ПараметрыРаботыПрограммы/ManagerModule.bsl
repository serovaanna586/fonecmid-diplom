...
Функция ИзмененияПараметраРаботыПрограммы(ИмяПараметра) Экспорт
	...
	Пока Индекс >= 0 Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияИБ, ВерсияИзменения) >= 0 И Не (ОбновлениеВнеОбновленияИБ
...
Процедура ДобавитьИзмененияПараметраРаботыПрограммы(ИмяПараметра, Знач Изменения) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() И ОбновлениеИнформационнойБазыСлужебный.РежимОбновленияДанных()
		= "ПереходСДругойПрограммы" Тогда
	...
	Попытка
		...
		Если ОбновитьСоставИзменений Тогда
			...
			УстановитьХранимыеДанныеПараметраРаботыПрограммы(ИмяПараметраХраненияИзменений, ПоследниеИзменения);
		КонецЕсли;
...
Функция ОбновитьПараметрыРаботыПрограммыВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	Если ОбщегоНазначения.РежимОтладки() И Не ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) Тогда
		СообщитьПрогресс = Ложь;
	КонецЕсли;

	Если ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) И Не ДоступноВыполнениеФоновыхЗаданий() Тогда

		ТекстОшибки = НСтр("ru = 'Обновление параметров работы программы, когда подключены расширения конфигурации,
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(ИмяПроцедурыФоновогоЗадания(), СообщитьПрогресс, ПараметрыОперации);
...
Функция ОбновитьПараметрыРаботыВерсийРасширенийВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр(
		"ru = 'Фоновое обновление параметров работы версий расширений'");
	ПараметрыОперации.ОжидатьЗавершение = ОжидатьЗавершение;

	Если ОбщегоНазначения.РежимОтладки() И Не СтандартныеПодсистемыСервер.ЭтоРазделенныйРежимСеансаБезРазделителей() Тогда
...
Функция ОбработанныйРезультатДлительнойОперации(Результат, Операция) Экспорт
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		Если Операция = "ЗагрузкаПараметровРаботыПрограммы" Тогда
			КраткоеПредставлениеОшибки = НСтр("ru = 'Не удалось загрузить параметры работы программы по причине:
											  |Фоновое задание, выполняющее загрузку отменено.'");

		ИначеЕсли Операция = "ОбновлениеПараметровРаботыПрограммы" Тогда
			КраткоеПредставлениеОшибки = НСтр("ru = 'Не удалось обновить параметры работы программы по причине:
											  |Фоновое задание, выполняющее обновление отменено.'");

		Иначе // ОбновлениеПараметровРаботыВерсийРасширений.
			КраткоеПредставлениеОшибки = НСтр("ru = 'Не удалось обновить параметры работы версий расширений по причине:
											  |Фоновое задание, выполняющее обновление отменено.'");
		КонецЕсли;
	...
	ИначеЕсли Результат.Статус <> "ЗагрузкаПараметровРаботыПрограммыНеТребуется" И Результат.Статус <> "ЗагрузкаИОбновлениеПараметровРаботыПрограммыНеТребуются"
...
Функция ПараметрыОбновления(Обновлять = Ложь) Экспорт
	...
	ПараметрыПодсистемы.Вставить("ОписаниеПоставляемыхПрофилейГруппДоступа", НовыеСвойстваПараметраОбновления(
		Обновлять));
	ПараметрыПодсистемы.Вставить("ОписаниеВозможныхПравДляНастройкиПравОбъектов", НовыеСвойстваПараметраОбновления(
		Обновлять));
	...
	ПараметрыПодсистемы.Вставить("ПараметрыИндексаПоискаОтчетовКонфигурации", НовыеСвойстваПараметраОбновления(
		Обновлять));
...
Процедура ОбработчикДлительнойОперацииВыполнитьОбновлениеНеразделенныхДанных(Параметры, АдресРезультата) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось обновить параметры работы программы по причине:
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		Если Параметры.ПодключаемыеКоманды.ПодключаемыеКомандыКонфигурации.Обновлять Тогда
			Параметры.ПодключаемыеКоманды.ПодключаемыеКомандыКонфигурации.ЕстьИзменения = МодульПодключаемыеКоманды.ОперативноеОбновлениеОбщихДанныхКонфигурации().ЕстьИзменения;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		...
		Если Параметры.УправлениеДоступом.ЗависимостиПрав.Обновлять Тогда
			...
			МодульЗависимостиПравДоступа.ОбновитьДанныеРегистра(
				Параметры.УправлениеДоступом.ЗависимостиПрав.ЕстьИзменения);
		КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Если Параметры.ВариантыОтчетов.ПараметрыОтчетовКонфигурации.Обновлять Тогда
			...
			Параметры.ВариантыОтчетов.ПараметрыОтчетовКонфигурации.ЕстьИзменения = МодульВариантыОтчетов.Обновить(
				Настройки).ЕстьИзменения;
		КонецЕсли;
		Если Параметры.ВариантыОтчетов.ПараметрыИндексаПоискаОтчетовКонфигурации.Обновлять
			...
			Параметры.ВариантыОтчетов.ПараметрыИндексаПоискаОтчетовКонфигурации.ЕстьИзменения = МодульВариантыОтчетов.Обновить(
				Настройки).ЕстьИзменения;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ИнформацияПриЗапуске") Тогда
		...
		Если Параметры.ИнформацияПриЗапуске.ПакетыИнформацииПриЗапуске.Обновлять Тогда
			Параметры.ИнформацияПриЗапуске.ПакетыИнформацииПриЗапуске.ЕстьИзменения = МодульИнформацияПриЗапуске.Обновить().ЕстьИзменения;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыРаботыПрограммыCУчетомРежимаВыполнения(СообщитьПрогресс)
	...
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось загрузить параметры работы программы по причине:
						   |Загрузку невозможно выполнить в области данных.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;

	НастройкаПодчиненногоУзлаРИБ = Ложь;
	Если Не НеобходимоОбновление(НастройкаПодчиненногоУзлаРИБ) Или Не ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не НастройкаПодчиненногоУзлаРИБ Тогда
		...
		Если СтандартнаяОбработка = Истина И ОбщегоНазначения.ПодсистемаСуществует(
	...
	Попытка
		Справочники.ИдентификаторыОбъектовМетаданных.ВыполнитьОбновлениеДанных(Ложь, Ложь, Истина, ,
			СписокКритичныхИзменений);
	Исключение
		// Переход в режим открытия формы повторной синхронизации данных перед запуском
		// с одним вариантом "Синхронизировать и продолжить".
		Если Не НастройкаПодчиненногоУзлаРИБ И ОбщегоНазначения.ПодсистемаСуществует(
			"СтандартныеПодсистемы.ОбменДанными") Тогда
			МодульОбменДаннымиСервер.ВключитьПовторениеЗагрузкиСообщенияОбменаДаннымиПередЗапуском();
		КонецЕсли;
		ВызватьИсключение;
	...
	Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Информационная база не может быть обновлена из-за проблемы в главном узле:
		...
		Если НастройкаПодчиненногоУзлаРИБ Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, "/C" + " "
				+ "ЗапуститьОбновлениеИнформационнойБазы", НСтр("ru = '- затем повторите создание подчиненного узла.'"));
		Иначе
			// Обновление подчиненного узла РИБ.
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, "/C" + " "
				+ "ЗапуститьОбновлениеИнформационнойБазы", НСтр("ru = '- затем повторите синхронизацию данных с этой информационной базой
...
Процедура ОбновитьПараметрыРаботыПрограммы(СообщитьПрогресс = Ложь)
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() И ОбщегоНазначения.ПодсистемаСуществует(
...
Функция ОписаниеЗначенияПараметраРаботыПрограммы(ИмяПараметра, ПроверитьВозможностьОбновленияВМоделиСервиса = Истина)
	...
	Если ТипЗнч(ОписаниеЗначения) <> Тип("Структура") Или ОписаниеЗначения.Количество() <> 2
		Или Не ОписаниеЗначения.Свойство("Версия") Или Не ОписаниеЗначения.Свойство("Значение") Тогда
...
Процедура ПроверитьВозможностьОбновленияВМоделиСервиса(ИмяПараметра, НовоеЗначение, Операция)
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '1. Перешлите сообщение в техническую поддержку.
			 ...
			 |
			 |Сведения об проблемном параметре:'"), "/С" + " " + "ЗапуститьОбновлениеИнформационнойБазы");

	Комментарий = Комментарий + Символы.ПС + "ВерсияМетаданных = " + Метаданные.Версия + "
																						 |ИмяПараметра = "
		+ ИмяПараметра + "
						 |Операция = " + Операция + "
													|ОписаниеЗначения =
	...
	ТекстОшибки = НСтр("ru = 'Параметры работы программы не обновлены в неразделенном режиме.
...
Функция ЭтоИзмененияПараметраРаботыПрограммы(ПоследниеИзменения)
	...
	Если ТипЗнч(ПоследниеИзменения) <> Тип("ТаблицаЗначений") Или ПоследниеИзменения.Колонки.Количество() <> 2
		Или ПоследниеИзменения.Колонки[0].Имя <> "ВерсияКонфигурации" Или ПоследниеИзменения.Колонки[1].Имя
		<> "Изменения" Тогда
