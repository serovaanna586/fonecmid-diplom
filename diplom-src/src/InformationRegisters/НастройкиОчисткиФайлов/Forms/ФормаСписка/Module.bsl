...
Функция МожноОчищатьВерсии()
	Если Элементы.ДеревоОбъектовМетаданных.ВыделенныеСтроки.Количество() = 1 Тогда
		...
		Если Не ТекущиеДанные.ЭтоФайл И ТекущиеДанные.ВладелецФайла <> Неопределено Тогда
...
Процедура ОтобразитьИтоги(ОбъемУдаляемыхФайлов, ОбъемНенужныхФайлов)
	Элементы.ОбъемУдаленныхФайлов.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удаляемые файлы: %1 Мб'"), Формат(ОбъемУдаляемыхФайлов, "ЧДЦ=2; ЧН=0;"));
	Элементы.ОбъемНенужныхФайлов.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ненужные файлы: %1 Мб'"), Формат(ОбъемНенужныхФайлов, "ЧДЦ=2; ЧН=0;"));
КонецПроцедуры
...
Процедура ЗаполнитьТипыОбъектовВДеревеЗначений()
	...
	Для Каждого Справочник Из МетаданныеСправочники Цикл
		...
		Для Каждого ТипВладельца Из ТипыВладельцевФайлов Цикл
			...
			Если Метаданные.Справочники.Содержит(МетаданныеВладельца) И МетаданныеВладельца.Иерархический Тогда
	...
	Для Каждого Тип Из ТаблицаТипов Цикл
		...
		Если СтрНачинаетсяС(ТипВладельцаФайла.Имя, "Удалить") Или ВладельцыФайлов.Найти(Тип.ИмяВладельцаФайла)
			<> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ВсеСправочники.СодержитТип(Тип.ВладелецФайла) Тогда
			...
			ДетализированныеНастройки = НастройкиОчистки.НайтиСтроки(Новый Структура("ИдентификаторВладельца, ЭтоФайл",
		...
		НайденныеНастройки = НастройкиОчистки.НайтиСтроки(Новый Структура("ВладелецФайла, ЭтоФайл",
			НоваяСтрокаТаблицы.ВладелецФайла, Тип.ЭтоФайл));
...
Процедура УстановитьПериодОчисткиДляВыбранногоОбъекта(ВыбранныйОбъект, ПериодОчистки)
	...
	ЗаписатьТекущиеНастройкиПоОбъекту(
		ВыбранныйОбъект.ВладелецФайла, ВыбранныйОбъект.ТипВладельцаФайла, ВыбранныйОбъект.Действие, ПериодОчистки,
...
Процедура УстановитьДействиеВыбранногоОбъекта(ВыбранныйОбъект, Знач Действие)
	...
	ЗаписатьТекущиеНастройкиПоОбъекту(
		ВыбранныйОбъект.ВладелецФайла, ВыбранныйОбъект.ТипВладельцаФайла, Действие, ВыбранныйОбъект.ПериодОчистки,
		ВыбранныйОбъект.ЭтоФайл);
...
Процедура ЗаписатьТекущиеНастройки()
	...
	ТекущиеДанные = Элементы.ДеревоОбъектовМетаданных.ТекущиеДанные;
	ЗаписатьТекущиеНастройкиПоОбъекту(
		ТекущиеДанные.ВладелецФайла, ТекущиеДанные.ТипВладельцаФайла, ТекущиеДанные.Действие,
		ТекущиеДанные.ПериодОчистки, ТекущиеДанные.ЭтоФайл);
...
Процедура ОткрытьФормуНастроек()
	...
	Отбор = Новый Структура("ВладелецФайла, ТипВладельцаФайла", ТекущиеДанные.ВладелецФайла,
...
Процедура НастроитьРежимыОчисткиФайлов()
	...
	Элементы.АвтоматическиОчищатьНенужныеФайлы.Заголовок = ?(ИспользоватьТома, НСтр("ru = 'Автоматически очищать:'"),
