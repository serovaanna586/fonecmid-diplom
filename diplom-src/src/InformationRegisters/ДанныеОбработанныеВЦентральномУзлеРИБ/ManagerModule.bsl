...
Процедура ОтметитьВыполнениеОбработки(Очередь, Данные, ДополнительныеПараметры) Экспорт
	...
	Если ТипДанных = Тип("Массив") И Данные.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ЭтоДвижения Или ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений Тогда
	...
	ИначеЕсли ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений Тогда
		...
		Для Каждого ЭлементДанных Из Данные Цикл
			...
			Для Каждого УзелРИБ Из УзлыРИБ Цикл
				...
				НоваяСтрока.ЗначенияОтборовНезависимогоРегистра = Новый ХранилищеЗначения(СтруктураЗначенийИзмерений,
	...
	Иначе
		Если ТипЗнч(Данные) <> Тип("Массив") Тогда
			...
			Если ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных) И ОбъектМетаданных.РежимЗаписи
...
Процедура ДобавитьДанныеВНабор(УзлыРИБ, НаборДанных, Данные, ОбъектМетаданных = Неопределено, Очередь = Неопределено)
	...
	Если ТипЗнч(Данные) = Тип("Массив") Тогда
		...
		Для Каждого ЭлементДанных Из Данные Цикл
			...
			НаборДанных.Отбор.Очередь.Установить(Очередь);
			НаборДанных.Отбор.ОбъектМетаданных.Установить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(
				ЭлементДанных)));
	...
	Иначе
		...
		Если Не НаборДанных.Отбор.ОбъектМетаданных.Использование Или Не ЗначениеЗаполнено(
			НаборДанных.Отбор.ОбъектМетаданных.Значение) Тогда
			НаборДанных.Отбор.ОбъектМетаданных.Установить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(
				Значение)));
		КонецЕсли;
