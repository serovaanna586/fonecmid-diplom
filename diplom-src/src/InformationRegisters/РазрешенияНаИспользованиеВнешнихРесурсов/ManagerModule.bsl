...
Функция ДополнениеРазрешения(Знач Разрешение) Экспорт
	...
	Если Разрешение.Тип() = ФабрикаXDTO.Тип(РаботаВБезопасномРежимеСлужебный.Пакет(), "AttachAddin") Тогда
		Возврат РаботаВБезопасномРежимеСлужебный.КонтрольныеСуммыФайловКомплектаВнешнейКомпоненты(
	...
	Если Менеджер.Выбран() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Добавляемое разрешение уже существует:
				 |%1, %2, %3, %4, %5.'"), Строка(ТипПрограммногоМодуля), Строка(ИдентификаторПрограммногоМодуля),
			Строка(ТипВладельца), Строка(ИдентификаторВладельца), КлючРазрешения);
