...
Процедура ОбновитьВозможныеПраваДляНастройкиПравОбъектов(ЕстьИзменения = Неопределено) Экспорт
	...
	Попытка
		...
		СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы(
			"СтандартныеПодсистемы.УправлениеДоступом.ВозможныеПраваДляНастройкиПравОбъектов", НовоеЗначение,
			ЕстьТекущиеИзменения);

		СтандартныеПодсистемыСервер.ДобавитьИзмененияПараметраРаботыПрограммы(
			"СтандартныеПодсистемы.УправлениеДоступом.ВозможныеПраваДляНастройкиПравОбъектов", ?(ЕстьТекущиеИзменения,
...
Функция Прочитать(Знач СсылкаНаОбъект) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			"РегистрыСведений.НастройкиПравОбъектов.Прочитать", "СсылкаНаОбъект", Строка(СсылкаНаОбъект),
	...
	НастройкиПрав.Вставить("Наследовать", РегистрыСведений.НаследованиеНастроекПравОбъектов.НаследованиеНастроек(
		СсылкаНаОбъект));
	...
	Для Каждого Строка Из Таблица Цикл
		Если ТекущийВладелецНастройки <> Строка.ВладелецНастройки Или ТекущийПользователь <> Строка.Пользователь Тогда
		...
		Если Настройки.Колонки.Найти(Строка.Право) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в процедуре %1
					 ...
					 |Требуется исправить данные регистра.'"), "РегистрыСведений.НастройкиПравОбъектов.Прочитать",
				СсылкаНаОбъект.Метаданные().ПолноеИмя(), Строка.Право, "НастройкиПравОбъектов", Строка(СсылкаНаОбъект));
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	...
	Попытка
		...
		Для Каждого Настройка Из Настройки.НайтиСтроки(Отбор) Цикл
			Для Каждого ОписаниеПрава Из ОписаниеПрав Цикл
				...
				// Кэш-реквизиты
				НастройкаПрав.УровеньРазрешенияПрава = ?(НастройкаПрав.ПравоЗапрещено, 0, ?(
					НастройкаПрав.НаследованиеРазрешено, 2, 1));
				НастройкаПрав.УровеньЗапрещенияПрава = ?(НастройкаПрав.ПравоЗапрещено, ?(
					НастройкаПрав.НаследованиеРазрешено, 2, 1), 0);
		...
		Если ПустыеСсылкиВладелецПрав.Получить(ТипВладельцаПрав) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в процедуре %1
					 ...
					 |
					 |Тип владельцев прав ""%3"" не указан в измерении %4.'"), "ОбновитьВспомогательныеДанныеРегистра",
				"НастройкиПравОбъектов", ТипВладельцаПрав, "Объект");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура ДобавитьОбъектыИерархии(Ссылка, МассивОбъектов)
	...
	|	И НЕ ТаблицаСИерархией.Ссылка В (&МассивОбъектов)", "ТаблицаОбъектов", Ссылка.Метаданные().ПолноеИмя());
...
Функция ПроверенныеВозможныеПраваСеанса(СвойстваВидовДоступа = Неопределено) Экспорт
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1
			 |общего модуля %2.'"), "ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов",
	...
	ОпределяемыйТипВладельцевПрав  = УправлениеДоступомСлужебныйПовтИсп.ТипыПоляТаблицы(
		"ОпределяемыйТип.ВладелецНастроекПрав");
	ОпределяемыйТипЗначенийДоступа = УправлениеДоступомСлужебныйПовтИсп.ТипыПоляТаблицы(
	...
	Для Каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найден владелец прав ""%1"".'"), ВозможноеПраво.ВладелецПрав);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;

		ДополнительныеПараметры.ВладелецПрав = ВозможноеПраво.ВладелецПрав;

		ЗаполнитьИдентификаторы("ЧтениеВТаблицах", ВозможноеПраво, ЗаголовокОшибки, ОтдельныеТаблицы,
			ДополнительныеПараметры);
		ЗаполнитьИдентификаторы("ИзменениеВТаблицах", ВозможноеПраво, ЗаголовокОшибки, ОтдельныеТаблицы,
			ДополнительныеПараметры);
		...
		Если ПраваВладельца = Неопределено Тогда
			...
			Если ОпределяемыйТипВладельцевПрав.Получить(ТипСсылки) = Неопределено Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
						 |не указан в определяемом типе %2.'"), Строка(ТипСсылки), "ВладелецНастроекПрав");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
				...
				Если ТипыПодпискиЗаписатьЗависимыеНаборыЗначенийДоступа.Получить(ТипОбъекта) <> Неопределено
					...
					ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Тип владельца прав ""%1""
							 ...
							 |для корректного заполнения регистра %6.'"), Строка(ТипСсылки), "ЗначениеДоступа",
						"ЗаписатьЗависимыеНаборыЗначенийДоступа" + "*", "ЗаписатьНаборыЗначенийДоступа" + "*",
						"ЗначениеДоступа", "НаборыЗначенийДоступа");
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
			КонецЕсли;
			...
			Если СвойстваВидаДоступа <> Неопределено Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
						 ...
						 |но обнаружен в описании вида доступа ""%2"".'"), Строка(ТипСсылки), СвойстваВидаДоступа.Имя);
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
			...
			Если ТипыПодпискиОбновитьГруппыВладельцевНастроекПрав.Получить(ТипОбъекта) = Неопределено Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
						 |не указан в определяемом типе %2.'"), Строка(ТипОбъекта), "ВладелецНастроекПравОбъект");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		...
		Если ПраваВладельца.Получить(ВозможноеПраво.Имя) <> Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для владельца прав ""%1""
					 |повторно определено право ""%2"".'"), ВозможноеПраво.ВладелецПрав, ВозможноеПраво.Имя);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		Для Каждого ТребуемоеПраво Из ВозможноеПраво.ТребуемыеПрава Цикл
			...
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для права ""%1"" владельца прав ""%2""
					 |указано некорректное имя требуемого права ""%3"".'"), ВозможноеПраво.Имя,
				ВозможноеПраво.ВладелецПрав, ТребуемоеПраво);
			ВызватьИсключение ТекстОшибки;
		КонецЦикла;
...
Функция СвойстваВозможногоПрава(ВозможноеПраво) Экспорт
	...
	СвойстваВозможногоПрава = Новый Структура("ВладелецПрав,
											  |Имя,
											  |НачальноеЗначение,
											  |ТребуемыеПрава,
											  |ЧтениеВТаблицах,
	...
	СвойстваВозможногоПрава.ТребуемыеПрава = Новый ФиксированныйМассив(СвойстваВозможногоПрава.ТребуемыеПрава);

	СвойстваВозможногоПрава.ЧтениеВТаблицах = Новый ФиксированныйМассив(СвойстваВозможногоПрава.ЧтениеВТаблицах);

	СвойстваВозможногоПрава.ИзменениеВТаблицах = Новый ФиксированныйМассив(СвойстваВозможногоПрава.ИзменениеВТаблицах);
...
Функция ПредставлениеВозможныхПрав() Экспорт
	...
	Для Каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		ПредставлениеВозможныхПрав.Вставить(ПолноеИмяПрава,
			Новый ФиксированнаяСтруктура(Новый Структура("Имя, Заголовок, Подсказка", ВозможноеПраво.Имя,
			ВозможноеПраво.Заголовок, ВозможноеПраво.Подсказка)));
	КонецЦикла;
		...
		ИначеЕсли Свойство = "ЧтениеВТаблицах" Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для владельца прав ""%1""
					 ...
					 |Имеет смысл использовать только символ ""*"".'"), ДополнительныеПараметры.ВладелецПрав,
				ВозможноеПраво.Имя, Значение, "Чтение");
			ВызватьИсключение ТекстОшибки;
...
Функция ИерархическийОбъектМетаданных(ОписаниеОбъектаМетаданных)
	...
	Если Не Метаданные.Справочники.Содержит(ОбъектМетаданных) И Не Метаданные.ПланыВидовХарактеристик.Содержит(
