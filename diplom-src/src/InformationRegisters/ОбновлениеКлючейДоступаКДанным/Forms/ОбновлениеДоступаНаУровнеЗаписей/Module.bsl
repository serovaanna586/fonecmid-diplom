...
&НаКлиенте
Перем АдресРезультата, АдресХранимыхДанных, ИдентификаторЗаданияОбновленияПрогресса, ТекстОшибкиОбновленияДоступа, ТекстОшибкиОбновленияПрогресса;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Элементы.ГруппаКоличествоПотоковОбновленияДоступаПодсказка1.Подсказка = Метаданные.Константы.КоличествоПотоковОбновленияДоступа.Подсказка;
	Элементы.ГруппаКоличествоПотоковОбновленияДоступаПодсказка2.Подсказка = Метаданные.Константы.КоличествоПотоковОбновленияДоступа.Подсказка;

	Элементы.ГруппаБалансировкаНагрузкиНаДискПодсказка1.Подсказка = НСтр("ru = 'Если при обновлении доступа скорость передачи данных
	...
	Элементы.ГруппаБалансировкаНагрузкиНаДискПодсказка2.Подсказка = Элементы.ГруппаБалансировкаНагрузкиНаДискПодсказка1.Подсказка;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ОбновлениеКлючейДоступаКДанным" Или ИмяСобытия
		= "Запись_ОбновлениеКлючейДоступаПользователей" Тогда
...
&НаКлиенте
Процедура ПоследнееЗавершениеОбновленияДоступаОбработкаНавигационнойСсылки(Элемент,
...
Процедура ОтменитьОбновлениеПрогресса(Команда)
	...
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОбновленияПрогресса) Тогда
		...
		Иначе
			Пояснение = "";
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ru = 'Обновление прогресса отменено'"), , Пояснение);
	КонецЕсли;
...
Процедура ОбновитьЗаголовокГруппыКоличествоПотоковОбновленияДоступа()
	...
	Элементы.ГруппаКоличествоПотоковОбновленияДоступа1.Заголовок = Формат(КоличествоПотоковОбновленияДоступа, "ЧГ=") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПотоковОбновленияДоступа, "", НСтр(
		"ru = 'поток,потока,потоков,,,,,,0'"));

	Элементы.ГруппаКоличествоПотоковОбновленияДоступа2.Заголовок = Элементы.ГруппаКоличествоПотоковОбновленияДоступа1.Заголовок;
...
Процедура ОбновитьСостояниеЗаданияОбновленияДоступа(Состояние = Неопределено)
	...
	Элементы.ГруппаПредупреждениеПервоеОбновлениеДоступаЕщеНеЗавершилось.Видимость = Универсально
	...
	Если ЗначениеЗаполнено(Состояние.ПоследнееЗавершениеОбновленияДоступа) Тогда
		...
		Если Состояние.ПоследнееЗавершениеСегодня Тогда
			...
			ПоследнееЗавершение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, Формат(
				Состояние.ПоследнееЗавершениеОбновленияДоступа, "ДЛФ=T"),
				Состояние.ПоследнееВыполнениеПродолжительность);
		Иначе
			ПоследнееЗавершение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, Формат(
				Состояние.ПоследнееЗавершениеОбновленияДоступа, "ДЛФ=DT"),
				Состояние.ПоследнееВыполнениеПродолжительность);
		КонецЕсли;
		ПоследнееЗавершение = СтрокаСФорматированнымиЧастями("(" + ПоследнееЗавершение + ")", ФорматЧастей, 3);
	Иначе
		ПоследнееЗавершение = "(" + ?(Состояние.ОбновлениеДоступаВыполняется, НСтр("ru = 'не завершалось'"), НСтр(
			"ru = 'не запускалось'")) + ")";
	КонецЕсли;
	Элементы.ПоследнееЗавершениеОбновленияДоступа.Заголовок = ПоследнееЗавершение;

	ВыполнениеЗаданияЗавершено = ЗначениеЗаполнено(ПоследнееЗавершениеОбновленияДоступа) И ЗначениеЗаполнено(
		Состояние.ПоследнееЗавершениеОбновленияДоступа) И ПоследнееЗавершениеОбновленияДоступа
		<> Состояние.ПоследнееЗавершениеОбновленияДоступа;
...
Функция ВремяВыполненияСтрокой(ВремяВСекундах)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, Формат(Секунд, "ЧН=0; ЧГ="), Формат(Минут,
		"ЧН=0; ЧГ="), Формат(ВсегоЧасов, "ЧГ="));
...
Функция СостояниеЗаданияОбновленияДоступа()
	...
	Состояние.Вставить("ПоследнееЗавершениеОбновленияДоступа", ?(ЗначениеЗаполнено(
		ПоследнееОбновлениеДоступа.ДатаПолногоЗавершения), ПоследнееОбновлениеДоступа.ДатаПолногоЗавершения,
		ПоследнееОбновлениеДоступа.ДатаЗавершенияНаСервере));

	Состояние.Вставить("ПоследнееВыполнениеПродолжительность", ВремяВыполненияСтрокой(
		ПоследнееОбновлениеДоступа.ПоследнееВыполнениеСекунд));

	Состояние.Вставить("ПоследнееЗавершениеСегодня", ЭтоСегодняшняяДата(ТекущаяДатаНаСервере,
		Состояние.ПоследнееЗавершениеОбновленияДоступа));

	Состояние.Вставить("ОбновлениеОтменено", ПоследнееОбновлениеДоступа.ОбновлениеОтменено);

	Состояние.Вставить("ТекстОшибкиОбновленияДоступа", ПоследнееОбновлениеДоступа.ТекстОшибкиЗавершения);

	Состояние.Вставить("ОбновлениеДоступаЗапрещено", ПоследнееОбновлениеДоступа.ОбновлениеДоступаЗапрещено);
	...
	ИначеЕсли ТипЗнч(Исполнитель) = Тип("ФоновоеЗадание") И (Исполнитель.УникальныйИдентификатор
		<> ПоследнееОбновлениеДоступа.ИдентификаторФоновогоЗадания Или ОбщегоНазначения.ИнформационнаяБазаФайловая()
...
Процедура НачатьОбновлениеПрогресса(ЗапускВручную = Ложь)
	...
	ИначеЕсли Не АвтообновлениеПрогресса И Не ЗапускВручную Или Элементы.ОбновлениеПрогресса.ТекущаяСтраница
		= Элементы.ОбновлениеПрогрессаВыполняется Тогда
...
Процедура ОбновитьПрогрессПослеПолученияДанных(Контекст)
	...
	Если Контекст.ДобавленныеСтроки.Количество() > 0 Или СортировкаСПереходомКНачалу()
...
Процедура ЗавершитьОбновлениеПрогрессаНаКлиенте(Контекст)
	...
	Если АвтообновлениеПрогресса Тогда
		ПодключитьОбработчикОжидания("НачатьОбновлениеПрогрессаОбработчикОжидания", ПериодОбновленияПрогресса, Истина);
	КонецЕсли;
...
Процедура ОбновитьЗадержкуОбновленияСписка(Строка, ТекущийМомент)
	...
	Если ЗначениеЗаполнено(Строка.ПоследнееОбновление) Тогда
		Строка.ЗадержкаОбработкиСписка = Цел((ТекущийМомент - Строка.ПоследнееОбновление) / Делитель) * Делитель;

	ИначеЕсли Строка.ПервоеПланированиеОбновления < МаксимальнаяДата Тогда
		Строка.ЗадержкаОбработкиСписка = Цел((ТекущийМомент - Строка.ПервоеПланированиеОбновления) / Делитель)
			* Делитель;
	Иначе
...
Функция НачатьОбновлениеПрогрессаНаСервере(Контекст, АдресРезультата, АдресХранимыхДанных, ИдентификаторФормы,
	...
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Или ОбщегоНазначения.РазделениеВключено()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр(
		"ru = 'Управление доступом: Получение прогресса обновления доступа'");
	...
	Если РезультатЗапуска.Статус = "Выполнено" Тогда
		ЗавершитьОбновлениеПрогрессаНаСервере(Контекст, АдресРезультата, АдресХранимыхДанных, Неопределено);
...
Функция ЗавершитьОбновлениеПрогрессаНаСервере(Контекст, Знач АдресРезультата, Знач АдресХранимыхДанных,
	...
	Если ИдентификаторЗаданияОбновленияПрогресса <> Неопределено Тогда
		...
		Если Не ЗаданиеВыполнено Тогда
			Контекст = Новый Структура("СостояниеЗаданияОбновленияДоступа", СостояниеЗаданияОбновленияДоступа());
			Контекст.Вставить("ОбновлениеПрогрессаВыполняется", ДлительныеОперации.ПрочитатьПрогресс(
				ИдентификаторЗаданияОбновленияПрогресса) <> Неопределено);
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура СортироватьСписок(ПоУбыванию = Ложь)
	...
	Если ТекущаяКолонка = Неопределено Или Не СтрНачинаетсяС(ТекущаяКолонка.Имя, "Списки") Тогда

		ПоказатьПредупреждение( , НСтр("ru = 'Выберите колонку для сортировки'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(
		?(ПоУбыванию, НСтр("ru = 'Сортировка по убыванию'"), НСтр("ru = 'Сортировка по возрастанию'")), ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Колонка ""%1""'"), СтрЗаменить(ТекущаяКолонка.Заголовок, Символы.ПС, " ")));
...
Процедура УстановитьКартинкуСортировки(Элемент, ПоУбыванию)
	...
	КонецЕсли;
	Элемент.КартинкаШапки = ?(ПоУбыванию, БиблиотекаКартинок.СортироватьСписокПоУбыванию,
...
Процедура СортироватьСписокПоПолям(ИндексПоляСортировки = 0, СтрокиСписка = Неопределено)
	...
	Если СтрокиСписка = Неопределено Тогда
		...
		Для Каждого Строка Из Списки Цикл
			СтрокиСписка.Добавить(Строка, ПредставлениеДляСортировки(Строка[ПолеСортировки]));
		КонецЦикла;
	ИначеЕсли СтрокиСписка.Количество() < 2 Тогда
		Возврат;
	Иначе
		Для Каждого ЭлементСписка Из СтрокиСписка Цикл
			ЭлементСписка.Представление = ПредставлениеДляСортировки(ЭлементСписка.Значение[ПолеСортировки]);
		КонецЦикла;
	КонецЕсли;
	...
	Если ИндексПоляСортировки = 0 И СортировкаСПереходомКНачалу() И Списки.Количество() > 0 Тогда
...
Функция СтрокаСФорматированнымиЧастями(Шаблон, ФорматЧастей, КоличествоФорматируемыхЧастей)
	...
	Для НомерФорматируемойЧасти = 1 По КоличествоФорматируемыхЧастей Цикл
		...
		Если СтрЧислоВхождений(Шаблон, РазделительНачало) <> 1 Или СтрЧислоВхождений(Шаблон, РазделительКонец) <> 1 Тогда
		...
		Если ФорматЧастей.Получить(НомерФорматируемойЧасти) <> Неопределено Тогда
			...
			ФорматируемаяСтрока = Новый ФорматированнаяСтрока(ФорматируемаяСтрока, ПараметрыФормата.Шрифт,
				ПараметрыФормата.ЦветТекста, ПараметрыФормата.ЦветФона, ПараметрыФормата.Ссылка);
		КонецЕсли;
