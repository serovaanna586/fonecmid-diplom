...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ОписаниеИдентификатора Из Идентификаторы Цикл
		СпискиДляОбновления.Добавить(ОписаниеИдентификатора.Значение, Строка(ОписаниеИдентификатора.Значение), Истина);
	КонецЦикла;
...
Процедура ОбъектОбновленияДоступаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТипыОбъектовОбновленияДоступа.ПоказатьВыборЭлемента(
		Новый ОписаниеОповещения("ОбъектОбновленияДоступаНачалоВыбораПродолжение", ЭтотОбъект), НСтр(
		"ru = 'Выбор типа данных'"), ТекущийЭлементТипа);
...
Процедура ЗапланироватьОбновлениеДоступаСписков(Команда)
	...
	Если Не ОбновитьКлючиДоступаКЭлементамДанных И Не ОбновитьПраваНаКлючиДоступа И Не УдалитьУстаревшиеСлужебныеДанные Тогда
...
Функция ОбновитьДоступКОбъектуНаСервере()
	...
	ОбновитьДоступКОбъектуДляВидаПользователей(ОбъектОбновленияДоступа, ПолноеИмя, ИдентификаторТранзакции, Ложь, Текст);

	ОбновитьДоступКОбъектуДляВидаПользователей(ОбъектОбновленияДоступа, ПолноеИмя, ИдентификаторТранзакции, Истина,
		Текст);
...
&НаСервере
Процедура ОбновитьДоступКОбъектуДляВидаПользователей(СсылкаНаОбъект, ПолноеИмя, ИдентификаторТранзакции,
	ДляВнешнихПользователей, Текст)

	ПараметрыОграничения = УправлениеДоступомСлужебный.ПараметрыОграничения(ПолноеИмя, ИдентификаторТранзакции,
	...
	УправлениеДоступомСлужебный.ОбновитьКлючиДоступаЭлементовДанныхПриЗаписи(СсылкаНаОбъект, ПараметрыОграничения,
		ИдентификаторТранзакции, Истина, ЕстьИзмененияПрав);

	Если ПараметрыОграничения.БезЗаписиКлючейДоступа Тогда
		Если ПараметрыОграничения.ДоступЗапрещен Или ПараметрыОграничения.ОграничениеОтключено Тогда
			Текст = Текст + Символы.ПС + НСтр(
				"ru = 'Обновление не требуется. Ключи доступа к объектам этого типа не нужны.'");
		Иначе
			Текст = Текст + Символы.ПС + НСтр(
				"ru = 'Обновление не требуется. Объекты этого типа используют ключи доступа к их владельцам.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ЕстьИзмененияПрав Тогда
		Если ПараметрыОграничения.ЕстьОграничениеПоПользователям Тогда
			Если ДляВнешнихПользователей Тогда
				Текст = Текст + Символы.ПС + НСтр(
					"ru = '2. Обновлены права на ключ доступа у внешних пользователей или групп доступа.'");
			Иначе
				Текст = Текст + Символы.ПС + НСтр(
					"ru = '2. Обновлены права на ключ доступа у пользователей или групп доступа.'");
			КонецЕсли;
		Иначе
	...
	Иначе
		Если ПараметрыОграничения.ЕстьОграничениеПоПользователям Тогда
			Если ДляВнешнихПользователей Тогда
				Текст = Текст + Символы.ПС + НСтр(
					"ru = '2. Обновление не требуется. Права на ключ доступа у внешних пользователей и групп доступа не устарели.'");
			Иначе
				Текст = Текст + Символы.ПС + НСтр(
					"ru = '2. Обновление не требуется. Права на ключ доступа у пользователей и групп доступа не устарели.'");
			КонецЕсли;
		Иначе
			Текст = Текст + Символы.ПС + НСтр(
				"ru = '2. Обновление не требуется. Права на ключ доступа у групп доступа не устарели.'");
		КонецЕсли;
	КонецЕсли;
