...
Процедура ПодготовитьДанныеДляЗаписи() Экспорт
	...
	Для Каждого Строка Из ДанныеДляЗаписи Цикл
		...
		Данные = РегистрыСведений.КэшПрограммныхИнтерфейсов.ПодготовитьДанныеКэшаВерсий(Строка.ТипДанных,
			ПараметрыПолучения);
...
Процедура ВызватьИсключениеКонтроля()
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Недопустимое изменение ресурса %1 записи регистра сведений %2
			 |внутри транзакции записи из сеанса с включенным разделением.'"), "Данные", "КэшПрограммныхИнтерфейсов");
