...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВладелецФайлаНеИдентификаторОбъектовМетаданных = ТипЗнч(Запись.ВладелецФайла) <> Тип(
	...
	Иначе
		...
		СписокСуществующихНастроек.ЗагрузитьЗначения(СуществующиеОбъектыСинхронизации(ТипЗнч(
			Запись.ВладелецФайла.ЗначениеПустойСсылки)));
	...
	Если ЕстьНастройкаПравиламСинхронизации(Запись.ВладелецФайла) И (НоваяНастройка Или ТипЗнч(Запись.ВладелецФайла)
	...
	Элементы.ПравилоНастройкиОтбор.РасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ПравилоНастройкиОтбор.РасширеннаяПодсказка.Заголовок, ПредставлениеТипВФ);
	Элементы.ОбъектСинхронизацииВсеФайлы.СписокВыбора[0].Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ОбъектСинхронизацииВсеФайлы.СписокВыбора[0].Представление, ПредставлениеТипВФ);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОбъектСинхронизации = "ТолькоФайлыЭлемента" И Не ЗначениеЗаполнено(ЭлементСправочника) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не заполнен объект с присоединенными файлами.'"), , "ЭлементСправочника");
	КонецЕсли;

	Запись.ВладелецФайла = ?(ОбъектСинхронизации = "ВсеФайлы", ИдентификаторВладельцаФайлов, ЭлементСправочника);
	...
	ИначеЕсли ВсеДокументы.СодержитТип(ТипЗнч(ТипОбъекта.ЗначениеПустойСсылки)) Тогда
		...
		Для Каждого Реквизит Из Документ.Реквизиты Цикл
			...
			Если Реквизит.Тип.СодержитТип(Тип("Дата")) Тогда
				...
				ПоляВладельцаФайла = ПоляВладельцаФайла + "," + Символы.ПС + СтрЗаменить(
					"РАЗНОСТЬДАТ(&ИмяРеквизита, &ТекущаяДата, ДЕНЬ) Как ДнейДоУдаленияОт&ИмяРеквизита",
...
Процедура ДобавитьУсловиеПоДате(Команда)
	...
	ОткрытьФорму("РегистрСведений.НастройкиСинхронизацииФайлов.Форма.ДобавленияУсловияПоДате", ПараметрыФормы,
		ЭтотОбъект);
...
Процедура ДобавитьВОтборИнтервалИсключение(ВыбранноеЗначение)
	...
	ОтборПоИнтервалу.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДнейДоУдаленияОт"
		+ ВыбранноеЗначение.РеквизитСТипомДата);
	...
	ПредставлениеРеквизитаСТипомДата = МассивРеквизитовСТипомДата.НайтиПоЗначению(
		ВыбранноеЗначение.РеквизитСТипомДата).Представление;
	ТекстПредставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Очищать спустя %1 дней относительно даты (%2)'"), ВыбранноеЗначение.ИнтервалИсключение,
		ПредставлениеРеквизитаСТипомДата);
