...
Процедура ЗаполнитьТипыОбъектовВДеревеЗначений()
	...
	Для Каждого СведенияОВладельце Из ТаблицаВладельцевФайлов Цикл
		...
		Если СтрНачинаетсяС(ТипВладельцаФайла.Имя, "Удалить") Или ВладельцыФайлов.Найти(
			СведенияОВладельце.ИмяВладельцаФайла) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ДетализированныеНастройки.Количество() > 0 Тогда
			Для Каждого Настройка Из ДетализированныеНастройки Цикл
				...
				ДетализированнаяНастройка.ПравилоОтбора    = ?(ЕстьПравилоОтбора, НСтр("ru = 'Выбранные файлы'"), НСтр(
					"ru = 'Все файлы'"));
		...
		НоваяСтрокаТаблицы.ВладелецФайла = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
			СведенияОВладельце.ВладелецФайла);
		НоваяСтрокаТаблицы.ТипВладельцаФайла = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
			СведенияОВладельце.ТипВладельцаФайла);
		...
		Если НайденныеНастройки.Количество() > 0 Тогда
			...
			Если ПравилоОтбора <> Неопределено И ПравилоОтбора.Отбор.Элементы.Количество() > 0 Тогда
				НоваяСтрокаТаблицы.ПравилоОтбора = ?(ПустаяСтрока(НайденныеНастройки[0].Наименование), НСтр(
					"ru = 'Выбранные файлы'"), НайденныеНастройки[0].Наименование);
			Иначе
...
Процедура ЗаписатьТекущиеНастройки()
	...
	ЗаписатьТекущиеНастройкиПоОбъекту(
		ТекущиеДанные.ВладелецФайла, ТекущиеДанные.ТипВладельцаФайла, ТекущиеДанные.Синхронизировать,
		ТекущиеДанные.УчетнаяЗапись, ТекущиеДанные.ЭтоФайл);
...
Процедура УстановитьНастройкиОтбора(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если СтрокаВладелец.ВладелецФайла <> ВыбранноеЗначение.ВладелецФайла Тогда
		...
		ЭтоНоваяНастройка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
								ДополнительныеПараметры, "НоваяНастройка", Ложь);
	...
	Если ВыбранноеЗначение.ЕстьПравилоОтбора Тогда
		ОбновляемаяСтрока.ПравилоОтбора = ?( ЗначениеЗаполнено(ВыбранноеЗначение.Наименование),
			ВыбранноеЗначение.Наименование, НСтр("ru = 'Выбранные файлы'"));
	Иначе
...
Функция СтрокаВладельцаВКоллекции(ЭлементВладелец, ВыбранноеЗначение)
	...
	Для Каждого СтрокаНастройки Из ЭлементВладелец Цикл
		Если СтрокаНастройки.ВладелецФайла = ВыбранноеЗначение.ВладелецФайла И СтрокаНастройки.УчетнаяЗапись
			= ВыбранноеЗначение.УчетнаяЗапись Тогда
...
Процедура УстановитьЗначениеСинхронизацииОбъекта(ВыбранныйОбъект, Знач Синхронизировать)
	...
	ЗаписатьТекущиеНастройкиПоОбъекту(
		ВыбранныйОбъект.ВладелецФайла, ВыбранныйОбъект.ТипВладельцаФайла, Синхронизировать,
		ВыбранныйОбъект.УчетнаяЗапись, ВыбранныйОбъект.ЭтоФайл);
...
Процедура ОткрытьФормуНастроек()
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ВладелецФайла) Или Не ЗначениеЗаполнено(ТекущиеДанные.ТипВладельцаФайла) Тогда
		Возврат;
	КонецЕсли;
	...
	ОткрытьФорму("РегистрСведений.НастройкиСинхронизацииФайлов.ФормаЗаписи", ПараметрыФормы, ЭтотОбъект, , , ,
		Оповещение);
...
Функция ОчиститьДанныеНастройки()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр(
		"ru = 'Подсистема ""Работа с файлами"": Отключение синхронизации файлов с облачным сервисом'");
...
Процедура УдалитьНастройкуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		НастройкаДляУдаления = ДеревоОбъектовМетаданных.НайтиПоИдентификатору(
			Элементы.ДеревоОбъектовМетаданных.ТекущаяСтрока);
		...
		Обработчик = Новый ОписаниеОповещения("ОчиститьДанныеНастройкиЗавершение", ЭтотОбъект,
			ДополнительныеПараметрыВызова);
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Запуск вручную: %1'"), РегламентноеЗаданиеМетаданные.Синоним);
		РезультатЗадания = ДлительныеОперации.ВыполнитьВФоне(РегламентноеЗаданиеМетаданные.ИмяМетода, Новый Структура,
			ПараметрыЗадания);
		Если ЗначениеЗаполнено(ИдентификаторФоновогоЗадания) Тогда
