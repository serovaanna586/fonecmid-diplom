...
Процедура ОбновитьГруппировкиПользователей(Параметры = Неопределено, ЕстьИзменения = Неопределено) Экспорт
	...
	ИначеЕсли Параметры.Количество() = 2 И Параметры.Свойство("Пользователи") И Параметры.Свойство(
		"ГруппыПользователей") Тогда

		ВидОбновления = "ПользователиИГруппыПользователей";

	ИначеЕсли Параметры.Количество() = 1 И Параметры.Свойство("Пользователи") Тогда

		ВидОбновления = "ПользователиИГруппыПользователей";

	ИначеЕсли Параметры.Количество() = 1 И Параметры.Свойство("ГруппыПользователей") Тогда

		ВидОбновления = "ПользователиИГруппыПользователей";

	ИначеЕсли Параметры.Количество() = 1 И Параметры.Свойство("ГруппыИсполнителей") Тогда

		ВидОбновления = "ГруппыИсполнителей";

	ИначеЕсли Параметры.Количество() = 1 И Параметры.Свойство("ОбъектыАвторизации") Тогда
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре %1
				 ...
				 |
				 |Указаны неверные параметры.'"), "ОбновитьГруппировкиПользователей", "ГруппыЗначенийДоступа");
...
//                    устанавливается Истина, иначе не изменяется.
//
Процедура ОбновитьГруппыЗначенийДоступа(ЗначенияДоступа = Неопределено, ЕстьИзменения = Неопределено) Экспорт
...
Процедура ОбновитьГруппыЗначенияДоступа(ЗначениеДоступа, ЕстьИзменения, ЗначенияСИзменениямиПоТипам)
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при обновлении групп значений доступа.'") + Символы.ПС + Символы.ПС;
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для типа ""%1""
				 |не настроено использование групп значений доступа.'"), Строка(ТипЗначенияДоступа));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Исключение
		...
		Если СвойстваВидаДоступа.ТипГруппЗначений = Тип("Неопределено") Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У значения доступа ""%1""
					 ...
					 |%4'"), Строка(ЗначениеДоступа), Строка(ТипЗначенияДоступа), "Ссылка", КраткоеПредставлениеОшибки(
				Ошибка));
			ВызватьИсключение ТекстОшибки;
		...
		ИначеЕсли СвойстваВидаДоступа.НесколькоГруппЗначений Тогда
			...
			Если ТабличнаяЧастьМетаданные = Неопределено Или ТабличнаяЧастьМетаданные.Реквизиты.Найти("ГруппаДоступа")
				= Неопределено Тогда

				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У типа значений доступа ""%1""
						 |не создана специальная табличная часть %2
						 |со специальным реквизитом %3.'"), Строка(ТипЗначенияДоступа), "ГруппыДоступа",
					"ГруппаДоступа");
				ВызватьИсключение ТекстОшибки;
			Иначе
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У значения доступа ""%1""
						 ...
						 |%5'"), Строка(ЗначениеДоступа), Строка(ТипЗначенияДоступа), "ГруппыДоступа", "ГруппаДоступа",
		...
		Иначе
			Если МетаданныеТипа.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У типа значений доступа ""%1""
						 |не создан специальный реквизит %2.'"), Строка(ТипЗначенияДоступа), "ГруппаДоступа");
				ВызватьИсключение ТекстОшибки;
			Иначе
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У значения доступа ""%1""
						 |типа ""%2""
						 |не удалось прочитать реквизит %3 по причине:
						 |%4'"), Строка(ЗначениеДоступа), Строка(ТипЗначенияДоступа), "ГруппаДоступа",
					КраткоеПредставлениеОшибки(Ошибка));
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		КонецЕсли;
	...
	Если СвойстваВидаДоступа.ТипГруппЗначений <> Тип("Неопределено") И (Объект = Неопределено Или Не Объект.ЭтоНовый()) Тогда
...
//
Процедура ОбновитьПользователей(Пользователи1 = Неопределено, ЕстьИзменения = Неопределено)
	...
	УправлениеДоступомСлужебный.УстановитьУсловиеОтбораВЗапросе(Запрос, Пользователи1, "Пользователи", "&УсловиеОтбораПользователей1:СоставыГруппПользователей.Пользователь
...
//
//
Процедура ОбновитьГруппыПользователей(ГруппыПользователей = Неопределено, ЕстьИзменения = Неопределено)
	...
	УправлениеДоступомСлужебный.УстановитьУсловиеОтбораВЗапросе(Запрос, ГруппыПользователей, "ГруппыПользователей", "&УсловиеОтбораГруппПользователей1:СоставыГруппПользователей.ГруппаПользователей
...
//                                   той же группы исполнителей.
//
Процедура ОбновитьГруппыИсполнителей(ГруппыИсполнителей = Неопределено, Исполнители = Неопределено,
	...
	Если ГруппыИсполнителей = Неопределено И Исполнители = Неопределено Тогда
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре %1
				 |модуля менеджера регистра сведений %2.
				 |
				 |Указаны неверные параметры.'"), "ОбновитьГруппыИсполнителей", "ГруппыЗначенийДоступа");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;

	НетГруппИсполнителей = Истина;
	ИнтеграцияПодсистемБСП.ПриОпределенииГруппИсполнителей(Запрос.МенеджерВременныхТаблиц, СодержаниеПараметра,
	...
	Запрос.УстановитьПараметр("ПустыеСсылкиГруппЗначений", УправлениеДоступомСлужебныйПовтИсп.ТаблицаПустыхСсылокУказанныхТипов(
...
Процедура ОбновитьОбъектыАвторизации(ОбъектыАвторизации = Неопределено, ЕстьИзменения = Неопределено)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПустыеСсылкиЗначений", УправлениеДоступомСлужебныйПовтИсп.ТаблицаПустыхСсылокУказанныхТипов(
	...
	УправлениеДоступомСлужебный.УстановитьУсловиеОтбораВЗапросе(Запрос, ОбъектыАвторизации, "ОбъектыАвторизации", "&УсловиеОтбораОбъектовАвторизации1:ВнешниеПользователи.ОбъектАвторизации
