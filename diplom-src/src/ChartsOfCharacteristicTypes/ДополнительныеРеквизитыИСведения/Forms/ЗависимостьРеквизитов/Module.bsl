...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолноеИмяОбъектаМетаданных = Лев(ИмяПредопределенныхДанных, ПозицияЗаменяемогоСимвола - 1) + "." + Сред(
...
Процедура ЗависимостиРеквизитовВидСравненияПриИзменении(Элемент)
	...
	Если ТаблицаФормы.ТекущиеДанные.Условие = "ВСписке" Или ТаблицаФормы.ТекущиеДанные.Условие = "НеВСписке" Тогда
		Если ТипЗнч(ТекущаяСтрока.Значение) <> Тип("СписокЗначений") Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ТаблицаФормы.ТекущиеДанные.Условие = "Равно" Или ТаблицаФормы.ТекущиеДанные.Условие = "НеРавно" Тогда
		Если ТипЗнч(ТекущаяСтрока.Значение) = Тип("СписокЗначений") И ТекущаяСтрока.Значение.Количество() > 0 Тогда
...
Функция СписокРеквизитовДляОтбора(ПолноеИмяОбъектаМетаданных, НаборДополнительныхРеквизитов)
	...
	Для Каждого ДополнительныйРеквизит Из НаборДополнительныхРеквизитов Цикл
		СвойстваОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДополнительныйРеквизит.Свойство,
			"Заголовок, ТипЗначения", , ТекущийЯзык().КодЯзыка);
