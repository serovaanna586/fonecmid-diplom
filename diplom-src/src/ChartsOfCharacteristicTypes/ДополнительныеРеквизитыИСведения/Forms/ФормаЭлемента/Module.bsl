...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПереданныеПараметрыФормы.ВыборОбщегоСвойства Или ПереданныеПараметрыФормы.ВыборВладельцаДополнительныхЗначений
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			ТекстВопроса = НаименованиеУжеИспользуется(
				Объект.Заголовок, Объект.Ссылка, ТекущийНаборСвойств, Объект.Наименование, Объект.ЗаголовокЯзык1,
				Объект.ЗаголовокЯзык2);
		Иначе
			ТекстВопроса = НаименованиеУжеИспользуется(
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПослеОтветаНаВопросКогдаНаименованиеУжеИспользуется", ЭтотОбъект,
				ПараметрыЗаписи), ТекстВопроса, Кнопки, , "ВернутьсяКВводуНаименования");
	...
	Если Не ПараметрыЗаписи.Свойство("КогдаИмяУжеИспользуется") И ЗначениеЗаполнено(Объект.Имя) Тогда
		// Заполнение наименования по набору свойств
		// и проверка есть ли свойство с тем же наименованием.
	...
	Если Не ПараметрыЗаписи.Свойство("КогдаИдентификаторДляФормулУжеИспользуется") И ЗначениеЗаполнено(
		Объект.ИдентификаторДляФормул) Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПослеОтветаНаВопросКогдаИдентификаторДляФормулУжеИспользуется", ЭтотОбъект,
				ПараметрыЗаписи), ТекстВопроса, Кнопки, , "ПродолжитьЗапись");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоДополнительноеСведение Или Не (Объект.ТипЗначения.СодержитТип(Тип("Число"))
		Или Объект.ТипЗначения.СодержитТип(Тип("Дата")) Или Объект.ТипЗначения.СодержитТип(Тип("Булево"))) Тогда
	...
	Если Не Объект.ЭтоДополнительноеСведение И Объект.ТипЗначения.Типы().Количество() = 1
	...
	Если Не ЗначениеЗаполнено(ТекущийОбъект.Имя) Или ПараметрыЗаписи.Свойство("КогдаИмяУжеИспользуется") Тогда
		ТекущийОбъект.Имя = "";
	...
	Если Не ЗначениеЗаполнено(ТекущийОбъект.ИдентификаторДляФормул) Или ПараметрыЗаписи.Свойство(
		"КогдаИдентификаторДляФормулУжеИспользуется") Тогда

		ЗаголовокОбъекта = ТекущийОбъект.Заголовок;
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			СуффиксТекущегоЯзыка = МодульМультиязычностьСервер.СуффиксТекущегоЯзыка();
			Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) И ЗначениеЗаполнено(ТекущийОбъект["Заголовок"
				+ СуффиксТекущегоЯзыка]) Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ДополнительныеРеквизитыИСведения", Новый Структура("Ссылка", Объект.Ссылка), Объект.Ссылка);

	Если ПараметрыЗаписи.Свойство("ИзмененныйНабор") Тогда

		Оповестить("Запись_НаборыДополнительныхРеквизитовИСведений", Новый Структура("Ссылка",
			ПараметрыЗаписи.ИзмененныйНабор), ПараметрыЗаписи.ИзмененныйНабор);
	КонецЕсли;
	...
	Если ПараметрыЗаписи.Свойство("ОбработкаПродолжения") Тогда
		...
		ВыполнитьОбработкуОповещения(
			Новый ОписаниеОповещения(ПараметрыЗаписи.ОбработкаПродолжения.ИмяПроцедуры, ЭтотОбъект,
			ПараметрыЗаписи.ОбработкаПродолжения.Параметры), Ложь);
	КонецЕсли;
...
Процедура ДополнительныеЗначенияСВесомПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И Не Объект.ДополнительныеЗначенияСВесом Тогда

		ТекстВопроса = НСтр("ru = 'Очистить введенные весовые коэффициенты?
							|
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ПослеПодтвержденияОчисткиВесовыхКоэффициентов", ЭтотОбъект), ТекстВопроса,
			Кнопки, , "ОчиститьИЗаписать");
	Иначе
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ПослеПодтвержденияВключенияВесовыхКоэффициентов", ЭтотОбъект), ТекстВопроса,
			Кнопки, , "Записать");
	КонецЕсли;
...
Процедура ЗначенияПриИзменении(Элемент)
	...
	Оповестить(ИмяСобытия, Новый Структура("Ссылка", Элемент.ТекущиеДанные.Ссылка), Элемент.ТекущиеДанные.Ссылка);
...
#КонецЕсли
	...
	Если ИндексСтраницы = 0 И Элементы.Свойства.ТекущиеДанные = Неопределено Тогда
...
Процедура Изменить(Команда)
	...
	Если Элементы.Свойства.ТекущиеДанные <> Неопределено Тогда
		...
		ОткрытьФорму("ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.ФормаОбъекта", ПараметрыФормы,
			Элементы.Свойства, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура ОбновитьПредлагаемоеЗначениеИдентификатора()
	...
	Если Не Элементы.ИдентификаторДляФормул.ТолькоПросмотр Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			СуффиксТекущегоЯзыка = МодульМультиязычностьСервер.СуффиксТекущегоЯзыка();
			Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) И ЗначениеЗаполнено(Объект["Заголовок"
				+ СуффиксТекущегоЯзыка]) Тогда
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Для Каждого Ссылка Из Наборы Цикл
		...
		Если ЭтоДополнительноеСведение = 1 И ВидыСвойствНабора.ДополнительныеСведения Или ЭтоДополнительноеСведение = 0
	...
	Если ЗначениеЗаполнено(РодительТекущегоНабора) Тогда
		...
		Если СвойстваНабора = Неопределено Тогда
			ИмяПредопределенныхДанных = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РодительТекущегоНабора,
				"ИмяПредопределенныхДанных");
		Иначе
			ИмяПредопределенныхДанных = СвойстваНабора.Имя;
		КонецЕсли;
		ПозицияЗаменяемогоСимвола = СтрНайти(ИмяПредопределенныхДанных, "_");
		ПолноеИмяОбъекта = Лев(ИмяПредопределенныхДанных, ПозицияЗаменяемогоСимвола - 1) + "." + Сред(
			ИмяПредопределенныхДанных, ПозицияЗаменяемогоСимвола + 1);
...
Процедура УстановитьУсловноеОформлениеСписка(ДоступныеНаборыСписок)
	...
	ГруппаЭлементовОтбораДанных = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип(
...
Процедура ЗаполнитьКарточкуРеквизитаИлиСведения()
	...
	Если СозданиеРеквизитаКопированием Тогда
		...
		СвойстваВладельца = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ПереданныеПараметрыФормы.ВладелецДополнительныхЗначений,
			"ТипЗначения, ДополнительныеЗначенияСВесом, ФорматСвойства");
...
Процедура ПослеОтветаНаВопросКогдаНаименованиеУжеИспользуется(Ответ, ПараметрыЗаписи) Экспорт
	...
	Если Ответ <> "ПродолжитьЗапись" Тогда
		...
		Если ПараметрыЗаписи.Свойство("ОбработкаПродолжения") Тогда
			ВыполнитьОбработкуОповещения(
				Новый ОписаниеОповещения(ПараметрыЗаписи.ОбработкаПродолжения.ИмяПроцедуры, ЭтотОбъект,
				ПараметрыЗаписи.ОбработкаПродолжения.Параметры), Истина);
		КонецЕсли;
	Иначе
...
Процедура ДополнительныеЗначенияСВесомПриИзмененииЗавершение(Отказ, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Оповестить(
			"Изменение_ЗначениеХарактеризуетсяВесовымКоэффициентом", Объект.ДополнительныеЗначенияСВесом, Объект.Ссылка);
	КонецЕсли;
...
Процедура УточнениеСпискаЗначенийКомментарийНажатиеЗавершение(Отказ, Контекст) Экспорт
	...
	ОткрытьФорму("ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.ФормаОбъекта", ПараметрыФормы, ВладелецФормы);
...
Процедура УточнениеНаборовКомментарийНажатиеПродолжение(Отказ, Контекст) Экспорт
	...
	Если СписокНаборов.Количество() > 1 Тогда
		ПоказатьВыборИзСписка(
			Новый ОписаниеОповещения("УточнениеНаборовКомментарийНажатиеЗавершение", ЭтотОбъект), СписокНаборов,
			Элементы.УточнениеНаборовКомментарий);
	Иначе
...
&НаКлиенте
Процедура ЗаписатьОбъект(ВариантТекстаВопроса, ОбработчикПродолжения, ДополнительныеПараметры = Неопределено,
	...
	Если ВариантТекстаВопроса = "ИзменениеПометкиУдаления" Тогда
		Если Модифицированность Тогда
			...
			Иначе
				ТекстВопроса = НСтр(
					"ru = 'Для установки пометки удаления запишите внесенные изменения. Записать данные?'");
			КонецЕсли;
		Иначе
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения(ОбработчикПродолжения.ИмяПроцедуры, ОбработчикПродолжения.Модуль, ПараметрыЗаписи),
			ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		Возврат;
	КонецЕсли;

	Если ЗначениеЗаполнено(Объект.Ссылка) И Не Модифицированность Тогда
		ВыполнитьОбработкуОповещения(Новый ОписаниеОповещения(ОбработчикПродолжения.ИмяПроцедуры,
			ОбработчикПродолжения.Модуль, ДополнительныеПараметры), Ложь);
		Возврат;
	КонецЕсли;
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ЗаписатьОбъектПродолжение", ЭтотОбъект, ПараметрыЗаписи), ТекстВопроса, Кнопки, ,
		"Записать");
...
Процедура ЗаписатьОбъектПродолжение(Ответ, ПараметрыЗаписи) Экспорт
	...
	Если Ответ = "Записать" Или Ответ = КодВозвратаДиалога.Да Тогда
		Записать(ПараметрыЗаписи);
	КонецЕсли;
...
Процедура ПослеОшибкиЗаписи()
	...
	Если ОбработкаПродолженияПриОшибкеЗаписи <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(
			Новый ОписаниеОповещения(ОбработкаПродолженияПриОшибкеЗаписи.ИмяПроцедуры, ЭтотОбъект,
			ОбработкаПродолженияПриОшибкеЗаписи.Параметры), Истина);
	КонецЕсли;
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Контекст) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр(
			"ru = 'Следующие настройки формата автоматически не применяются в большинстве мест:'");
		...
		Для Каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 Или СтрНайти(Подстрока, "DE=") > 0 Тогда // @Non-NLS
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС + " - " + НСтр(
					"ru = 'представление пустой даты'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 Или СтрНайти(Подстрока, "NZ=") > 0 Тогда // @Non-NLS
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС + " - " + НСтр(
					"ru = 'представление пустого числа'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 Или СтрНайти(Подстрока, "DF=") > 0 Тогда // @Non-NLS
				Если СтрНайти(Подстрока, "ддд") > 0 Или СтрНайти(Подстрока, "ddd") > 0 Тогда // @Non-NLS
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС + " - " + НСтр(
						"ru = 'кратное название дня недели'");
				КонецЕсли;
			...
			Если СтрНайти(Подстрока, "ДЛФ=") > 0 Или СтрНайти(Подстрока, "DLF=") > 0 Тогда // @Non-NLS
				Если СтрНайти(Подстрока, "ДД") > 0 Или СтрНайти(Подстрока, "DD") > 0 Тогда // @Non-NLS
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС + " - " + НСтр(
						"ru = 'длинная дата (месяц прописью)'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		...
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаСписка, ?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр(
			"ru = 'дополнительное сведение'"), НСтр("ru = 'дополнительный реквизит'")), Строка(
			ПереданныеПараметрыФормы.ТекущийНаборСвойств));
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаПереключателя, ?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр(
			"ru = 'сведения'"), НСтр("ru = 'реквизита'")), Строка(ВладелецДополнительныхЗначений), Строка(
			ПереданныеПараметрыФормы.ТекущийНаборСвойств));
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
	...
	Если Не Объект.ТипЗначения.СодержитТип(Тип("Число")) И Не Объект.ТипЗначения.СодержитТип(Тип("Дата"))
	...
	Если Объект.ЭтоДополнительноеСведение И Объект.ТипЗначения.СодержитТип(Тип("Строка"))
	...
	Если ЗначениеЗаполнено(Объект.ВладелецДополнительныхЗначений) Тогда
		...
		Если СвойстваВладельца.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
			Объект.ТипЗначения = Новый ОписаниеТипов(Объект.ТипЗначения,
				"СправочникСсылка.ЗначенияСвойствОбъектовИерархия", "СправочникСсылка.ЗначенияСвойствОбъектов");
		Иначе
			Объект.ТипЗначения = Новый ОписаниеТипов(Объект.ТипЗначения, "СправочникСсылка.ЗначенияСвойствОбъектов",
				...
				Если СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия"))
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Недопустимо удалять тип ""%1"",
							 ...
							 |
							 |Удаление отменено.'"), Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")));
				...
				ИначеЕсли СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектов"))
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Недопустимо удалять тип ""%1"",
							 ...
							 |Сначала нужно удалить введенные дополнительные значения.
							 |
							 |Удаление отменено.'"), Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")));

					Объект.ТипЗначения = Новый ОписаниеТипов(Объект.ТипЗначения,
						"СправочникСсылка.ЗначенияСвойствОбъектов", "СправочникСсылка.ЗначенияСвойствОбъектовИерархия");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
			...
			Если Не СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимо одновременно использовать типы значения
						 ...
						 |""%2"".
						 |
						 |Второй тип удален.'"), Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")), Строка(Тип(
					"СправочникСсылка.ЗначенияСвойствОбъектовИерархия")));
			...
			Иначе
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимо одновременно использовать типы значения
						 ...
						 |""%2"".
						 |
						 |Первый тип удален.'"), Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")), Строка(Тип(
					"СправочникСсылка.ЗначенияСвойствОбъектовИерархия")));
	...
	Если Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СуффиксТекущегоЯзыка) И ОбщегоНазначения.ПодсистемаСуществует(
			"СтандартныеПодсистемы.Мультиязычность") Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			МодульМультиязычностьСервер.ИзменитьПолеЗапросаПодТекущийЯзык(СвойстваСписка.ТекстЗапроса,
	...
	Иначе
		...
		Элементы.УточнениеСпискаЗначенийКомментарий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонУточнения, СвойстваВладельца.Заголовок);
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли СписокНаборов.Количество() < 2 Тогда
			...
			КонецЕсли;
			ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонУточнения, СокрЛП(
		...
		Иначе
			...
			СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(), "", НСтр(
				"ru = 'набор,набора,наборов,,,,,,0'"));

			ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонУточнения, Формат(
				СписокНаборов.Количество(), "ЧГ="), СтрокаНаборы);
		КонецЕсли;
...
Процедура ПереключитьНастройкиВыводаРеквизита(ТипЗначения)
	...
	РазрешитьВыборМногострочногоПоля = (Объект.ТипЗначения.Типы().Количество() = 1) И (Объект.ТипЗначения.СодержитТип(
		Тип("Строка")));
	РазрешитьВыводВВидеГиперссылки   = РазрешитьВыборМногострочногоПоля Или (Не Объект.ТипЗначения.СодержитТип(Тип(
		"Строка")) И Не Объект.ТипЗначения.СодержитТип(Тип("Дата")) И Не Объект.ТипЗначения.СодержитТип(Тип("Булево"))
...
Процедура ЗаписатьЗначенияДополнительногоРеквизитаПриКопированииРекурсивно(Владелец, СтрокаДерева, Родитель)
	...
	Для Каждого ЭлементДерева Из СтрокаДерева Цикл
		...
		ЗаписатьЗначенияДополнительногоРеквизитаПриКопированииРекурсивно(Владелец, ДочерниеЭлементы,
			КопияОбъекта.Ссылка);
	КонецЦикла
	;
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	Элементы.ЗадатьУсловиеДоступности.Заголовок = ?(ЗаданаЗависимостьДоступности, ШаблонЗависимостьЗадана,
		ШаблонЗависимостьНеЗадана);

	Элементы.ЗадатьУсловиеЗаполнения.Заголовок = ?(ЗаданаЗависимостьОбязательностиЗаполнения, ШаблонЗависимостьЗадана,
		ШаблонЗависимостьНеЗадана);

	Элементы.ЗадатьУсловиеВидимости.Заголовок = ?(ЗаданаЗависимостьВидимости, ШаблонЗависимостьЗадана,
		ШаблонЗависимостьНеЗадана);
...
Процедура ПриИзмененииТекущегоНабораНаСервере(ТекущийНаборЭтоГруппа = Неопределено)
	...
	Если ЗначениеЗаполнено(ВыбранныйНаборСвойств) И Не ТекущийНаборЭтоГруппа Тогда
		ОбновитьСписокСвойствТекущегоНабора();
	Иначе
		Свойства.Очистить();
	КонецЕсли;
...
Процедура ОбновитьСписокСвойствТекущегоНабора()
	...
	Если ЭтоДополнительноеСведение Тогда
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст, "Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты",
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипЗначения <> Null И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(
			Выборка.ТипЗначения) Тогда

			НоваяСтрока.ТипЗначения = Строка(Новый ОписаниеТипов(Выборка.ТипЗначения, , "СправочникСсылка.ЗначенияСвойствОбъектовИерархия,
																						|СправочникСсылка.ЗначенияСвойствОбъектов"));
...
Функция НаименованиеУжеИспользуется(Знач Заголовок, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование,
	Знач ЗаголовокЯзык1, Знач ЗаголовокЯзык2)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		ИначеЕсли МодульМультиязычностьСервер.КодВторогоДополнительногоЯзыкаИнформационнойБазы()
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ru = 'Рекомендуется использовать другое наименование,
...
Функция ИмяУжеИспользуется(Знач Имя, Знач ТекущееСвойство, НовоеНаименование)
	...
	Если НовоеИмя <> Имя Или ИмяНачинаетсяСЦифры(Имя) Или СтрРазделить(НовоеИмя, " ", Истина).Количество() > 1 Тогда

		ТекстВопроса = НСтр(
			"ru = 'Имя (группа Для разработчиков) должно состоять из одного слова, начинаться с буквы и не содержать специальных символов, кроме ""_"".'");
		Возврат ТекстВопроса;
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ru = 'Рекомендуется использовать другое имя,
													  |иначе программа может работать некорректно.
													  |
													  |Создать новое имя и продолжить запись?'"), Имя);
...
Функция ИдентификаторДляФормулУжеИспользуется(Знач ИдентификаторДляФормул, Знач ТекущееСвойство)
	...
	ПроверочныйИдентификатор = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ИдентификаторДляФормул(
	...
	Если ВРег(ИдентификаторДляФормул) <> ВРег(ПроверочныйИдентификатор) Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстВопроса, ИдентификаторДляФормул);
		Возврат ТекстВопроса;
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ru = 'Рекомендуется использовать другой идентификатор для формул,
													  |иначе программа может работать некорректно.
													  |
													  |Создать новый идентификатор для формул и продолжить запись?'"),
		ИдентификаторДляФормул);
