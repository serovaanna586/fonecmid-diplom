...
#Иначе
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			...
			МодульМультиязычностьКлиентСервер.ОбработкаПолученияПредставления(Данные, Представление,
				СтандартнаяОбработка, "Заголовок");
		КонецЕсли;
#КонецЕсли
...
Процедура ИзменитьНастройкуСвойства(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Если МетаданныеВладельца = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при изменении настройки свойства %1.
					 |Набор свойств %2 не связан ни с одним владельцем значений свойств.'"), Свойство,
				ТекущийНаборСвойств);
		КонецЕсли;
		...
		ИначеЕсли Свойство = СвойствоОбъект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при изменении настройки свойства %1.
					 |Тип значения не содержит дополнительных значений.'"), Свойство);
		КонецЕсли;

		Если Свойство <> СвойствоОбъект.Ссылка Или СоответствиеСсылок.Количество() > 0 Тогда
			...
			Если Свойство = СвойствоОбъект.Ссылка Тогда
				...
				ТаблицаЗначений.Колонки.Добавить("Значение", Новый ОписаниеТипов("СправочникСсылка." + ИмяСправочника));
			...
			Пока Истина Цикл
				...
				Пока Выборка.Следующий() Цикл
					Заменить = Истина;
					Если ТребуетсяАнализНаборовКаждогоОбъектаВладельца Тогда
						ЗапросАнализа.УстановитьПараметр("ВсеНаборыОбъекта", УправлениеСвойствамиСлужебный.ПолучитьНаборыСвойствОбъекта(
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
		...
		Попытка
			...
			Для Каждого Зависимость Из Объект.ЗависимостиДополнительныхРеквизитов Цикл
				...
				Если ЗначениеЗаполнено(Зависимость.НаборСвойств) Тогда
					ИмяПредопределенныхДанных = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Зависимость.НаборСвойств,
						"ИмяПредопределенныхДанных");
				...
				Иначе
					...
					ИмяНабора = Сред(ИмяПредопределенныхДанных, ДлинаПрефикса + 1, СтрДлина(ИмяПредопределенныхДанных)
						- ДлинаПрефикса);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация, Метаданные.ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения, ,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Обработана очередная порция дополнительных реквизитов (сведений): %1'"), ОбъектовОбработано));
	КонецЕсли;
