...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	...
	Если ПланыОбмена.ГлавныйУзел() = Неопределено И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
		"Константа.ГлавныйУзел").Получить()) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не может использоваться
				 ...
				 |нажмите кнопку Отключить или программно очистите константу Главный узел.'"), НазваниеСправочника(
			ОбъектыРасширений));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Попытка
		...
		Для Каждого Объект Из Объекты Цикл
			...
			Если Не ЭтоКоллекция(СвойстваЗагружаемого.Ссылка) Тогда
				...
				СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени = МетаданныеНайтиПоПолномуИмени(Объект.ПолноеИмя);

				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу = Неопределено
					И СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени = Неопределено И Объект.ПометкаУдаления
					<> Истина Тогда
			...
			Если Свойства <> Неопределено И Свойства.Наименование = Объект.Наименование
				И Свойства.Родитель = Объект.Родитель И Свойства.ПорядокКоллекции = Объект.ПорядокКоллекции
				И Свойства.Имя = Объект.Имя И Свойства.Синоним = Объект.Синоним И Свойства.ПолноеИмя = Объект.ПолноеИмя
				И Свойства.ПолныйСиноним = Объект.ПолныйСиноним И Свойства.БезДанных = Объект.БезДанных
				И Свойства.ЗначениеПустойСсылки = Объект.ЗначениеПустойСсылки И Свойства.ИмяПредопределенныхДанных
				= Объект.ИмяПредопределенныхДанных И Свойства.ПометкаУдаления = Объект.ПометкаУдаления
		...
		Для Каждого СвойстваЗагружаемого Из ТаблицаЗагружаемых Цикл
			...
			Если ТипЗнч(КлючОбъектаМетаданных) = Тип("Тип") И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
				...
				Если КлючиЗагружаемых.Получить(КлючОбъектаМетаданных) <> Неопределено Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
							 ...
							 |""%1"".'"), Строка(КлючОбъектаМетаданных));
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
				КлючиЗагружаемых.Вставить(КлючОбъектаМетаданных);

				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу
					<> СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
							 |Невозможно загрузить элемент у которого ключ объекта метаданных
							 |""%1"" не соответствует полному имени ""%2"".'"), Строка(КлючОбъектаМетаданных),
						Объект.ПолноеИмя);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
...
Процедура ЗарегистрироватьПолноеИзменениеДляПодчиненныхУзловРИБ() Экспорт
	...
	Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Или Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		Возврат;
	КонецЕсли;
...
//  ОбъектыРасширений - Булево
//
Процедура ВыполнитьОбновлениеДанных(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения = Ложь,
	СписокКритичныхИзменений = "", ОбъектыРасширений = Ложь) Экспорт

	Если ОбъектыРасширений И ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения)
	...
	ОбновитьДанные(ЕстьТекущиеИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения,
		СписокКритичныхИзменений, ОбъектыРасширений);

	Если ЕстьТекущиеИзменения Тогда
		ЕстьИзменения = Истина;
	КонецЕсли;

	Если Не ОбъектыРасширений И Не СтандартныеПодсистемыСервер.ВерсияПрограммыОбновленаДинамически() Тогда
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректное значение параметра %1 в функции %2.
					 |Указан несуществующий объект метаданных: ""%3"".'"), "ОписаниеОбъектаМетаданных",
				"ОбщегоНазначения.ИдентификаторОбъектаМетаданных", ОписаниеОбъектаМетаданных);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный тип параметра %1 в функции %2:
				 |""%3"".'"), "ОписаниеОбъектаМетаданных", "ОбщегоНазначения.ИдентификаторОбъектаМетаданных",
...
Функция ИдентификаторыОбъектовМетаданных(ОписаниеОбъектовМетаданных, ВызыватьИсключение = Истина, ОдинЭлемент = Ложь) Экспорт
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректное значение параметра %1 функции %2:
						 |Указан несуществующий объект метаданных: ""%3"".'"), "ОписаниеОбъектовМетаданных",
					"ОбщегоНазначения.ИдентификаторыОбъектовМетаданных", ОписаниеОбъектаМетаданных);
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректный тип параметра %1 функции %2:
					 |""%3"".'"), "ОписаниеОбъектовМетаданных", "ОбщегоНазначения.ИдентификаторыОбъектовМетаданных",
				ТипОписанияОбъектаМетаданных);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	...
	Идентификаторы = ИдентификаторыОбъектовМетаданныхСПопыткойПовтора(ПолныеИменаБезКэша, ВызыватьИсключение,
		ОдинЭлемент);
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1 общего модуля %2.'"), "ПриДобавленииПереименованийОбъектовМетаданных",
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не совпадают имена типов переименованного объекта метаданных.
				 |Прежний тип: ""%1"",
				 |новый тип: ""%2"".'"), СтароеПолноеИмя, НовоеПолноеИмя);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если Итог.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		...
		СписокДопустимыхТипов = ?(ЗначениеЗаполнено(СписокДопустимыхТипов), Лев(СписокДопустимыхТипов, СтрДлина(
			СписокДопустимыхТипов) - 1), "");
		...
		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для типа объекта метаданных ""%1"" не требуется описывать переименование,
				 ...
				 |%2.'"), СтароеПолноеИмя, СписокДопустимыхТипов);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ОбновитьСвойстваИдентификатора(Объект) Экспорт
	...
	Иначе
		...
		СвойстваОбъектовМетаданных = СвойстваОбъектовМетаданных(ОбъектыРасширений, СвойстваКоллекций.Скопировать(
			Строки));
		...
		Если ТипЗнч(Объект) <> Тип("ДанныеФормыСтруктура") Тогда
			КлючОбъектаМетаданных = Объект.КлючОбъектаМетаданных.Получить();
			Если КлючОбъектаМетаданных = Неопределено Или СвойстваОбъекта.БезКлючаОбъектаМетаданных
...
Функция ТаблицаПереименованияДляТекущейВерсии() Экспорт
	...
	КоллекцииБезКлючаОбъектаМетаданных = СтандартныеПодсистемыПовтИсп.СвойстваКоллекцийОбъектовМетаданных().НайтиСтроки(
		Отбор);
...
Функция СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений = Ложь) Экспорт
	...
	СвойстваКоллекцийОбъектовМетаданных.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(50)));
	СвойстваКоллекцийОбъектовМетаданных.Колонки.Добавить("ИмяВЕдЧисле", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(50)));
	СвойстваКоллекцийОбъектовМетаданных.Колонки.Добавить("Синоним", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(255)));
	СвойстваКоллекцийОбъектовМетаданных.Колонки.Добавить("СинонимВЕдЧисле", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(255)));
	...
	СвойстваКоллекцийОбъектовМетаданных.Колонки.Добавить("Идентификатор",
...
Процедура ПередУдалениемОбъекта(Объект) Экспорт
	...
	Если Не Объект.ПометкаУдаления Тогда
		ВызватьИсключениеПоОшибке(ОбъектыРасширений, НСтр("ru = 'Удаление идентификаторов объектов, у которых значение
...
Процедура ФормаЭлементаПриСозданииНаСервере(Форма) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь( , Не ОбъектыРасширений) Или ЗапрещеноИзменятьПолноеИмя(
		Форма.Объект) Или Не ОбъектыРасширений И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Или ОбъектыРасширений
		И Справочники.ИдентификаторыОбъектовРасширений.ОбъектРасширенияОтключен(Форма.Объект.Ссылка) Тогда
...
//
Процедура ОпределитьДублиПриЗагрузкеВПодчиненныйУзел(Выгрузка, Отбор, ЗагружаемыйОбъект, СсылкаЗагружаемого,
	ТаблицаЗагружаемых)

	Строки = Выгрузка.НайтиСтроки(Отбор);
	Для Каждого Строка Из Строки Цикл

		Если Строка.Ссылка <> СсылкаЗагружаемого И ТаблицаЗагружаемых.Найти(Строка.Ссылка, "Ссылка") = Неопределено Тогда
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения,
	СписокКритичныхИзменений, ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		...
		СвойстваРасширений.Вставить("ИменаПодключенныхРасширений", ИменаРасширений(
			ИсточникРасширенийКонфигурации.СеансАктивные, ИдентификаторыКлючейРасширений));
		СвойстваРасширений.Вставить("ИменаНеподключенныхРасширений", ИменаРасширений(
			ИсточникРасширенийКонфигурации.СеансОтключенные, ИдентификаторыКлючейРасширений));
	...
	Попытка
		...
		Если Не ОбъектыРасширений И Не ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Переименование идентификаторов объектов метаданных %1 -> %2:'"), "СтароеПолноеИмя",
				"НовоеПолноеИмя");
			СписокКритичныхИзменений = СписокКритичныхИзменений + Символы.ПС + СписокПереименованийОбъектовМетаданных
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений + НСтр(
				"ru = 'Добавление новых идентификаторов объектов метаданных:'") + Символы.ПС
				+ СписокНовыхОбъектовМетаданных + Символы.ПС;
		КонецЕсли;

		Если Не ТолькоПроверка И Не ОбъектыРасширений И ЗначениеЗаполнено(СписокКритичныхИзменений)
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, НСтр("ru = 'Критичные изменения могут быть выполнены только
		...
		Если Не ТолькоПроверка Тогда
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(?(ОбъектыРасширений, НСтр(
					"ru = 'Идентификаторы объектов расширений.Выполнены критичные изменения'",
					ОбщегоНазначения.КодОсновногоЯзыка()), НСтр(
					"ru = 'Идентификаторы объектов метаданных.Выполнены критичные изменения'",
...
Функция ВыгрузкаВсехИдентификаторов(ОбъектыРасширений = Ложь)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяРасширения", ?(ОбъектыРасширений, "Идентификаторы.ИмяРасширения",
		""""""));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИдентификаторРасширения", ?(ОбъектыРасширений,
		"Идентификаторы.ИдентификаторРасширения", """"""));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ХешСуммаРасширения", ?(ОбъектыРасширений,
		"Идентификаторы.ХешСуммаРасширения", """"""));
	...
	Для Каждого Строка Из Выгрузка Цикл
		Если Не ЗначениеЗаполнено(Строка.Ссылка) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Данные справочника %1 повреждены.
					 ...
					 |выполните ""Тестирование и исправление..."".'"), ?(ОбъектыРасширений,
				"ИдентификаторыОбъектовРасширений", "ИдентификаторыОбъектовМетаданных"));
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		КонецЕсли;
		Строка.БезКлючаОбъектаМетаданных = Строка.КлючОбъектаМетаданных = Неопределено Или Строка.КлючОбъектаМетаданных
			= Тип("Неопределено");
	КонецЦикла;
...
Процедура ПереименоватьПолныеИмена(Выгрузка, СписокПереименованийОбъектовМетаданных = "", ЕстьКритичныеИзменения = Ложь)
	...
	Для Каждого ОписаниеПереименования Из ТаблицаПереименования Цикл
		...
		ЭтоПодсистема = ВРег(Лев(ОписаниеПереименования.СтароеПолноеИмя, СтрДлина("Подсистема."))) = ВРег("Подсистема.");
		...
		Для Каждого Строка Из Выгрузка Цикл
			...
			Если ЭтоПодсистема Тогда
				Если ВРег(Лев(Строка.ПолноеИмя, ДлинаСтарогоПолногоИмени)) = ВРег(
					ОписаниеПереименования.СтароеПолноеИмя) Тогда

					НовоеПолноеИмя = ОписаниеПереименования.НовоеПолноеИмя + Сред(Строка.ПолноеИмя,
						ДлинаСтарогоПолногоИмени + 1);
				КонецЕсли;
			Иначе
	...
	Для Каждого Строка Из Выгрузка Цикл
		...
		СписокПереименованийОбъектовМетаданных = СписокПереименованийОбъектовМетаданных + ?(ЗначениеЗаполнено(
			СписокПереименованийОбъектовМетаданных), "," + Символы.ПС, "") + Переименование.СтароеПолноеИмя + " -> "
			+ Переименование.НовоеПолноеИмя;
	КонецЦикла;
...
Процедура ОбработатьИдентификаторыОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений,
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если ОбъектыРасширений И (СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИмяРасширения)] <> Неопределено
			Или СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИдентификаторРасширения)]
			<> Неопределено) Тогда
		...
		Если ОбъектыРасширений И СвойстваРасширений.ИменаПодключенныхРасширений[НРег(Свойства.ИмяРасширения)] = Неопределено
		...
		ОбъектМетаданных = ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, Свойства.ИмяРасширения,
			Свойства.ИдентификаторРасширения, Свойства.ХешСуммаРасширения);
		...
		Иначе
			...
			Если ВРег(Лев(ОбъектМетаданных.Имя, СтрДлина("Удалить"))) = ВРег("Удалить") И ВРег(Лев(
				Свойства.Имя, СтрДлина("Удалить"))) <> ВРег("Удалить") Тогда
		...
		Иначе
			...
			Если Свойства.Наименование <> СвойстваОбъекта.Наименование Или Свойства.ПорядокКоллекции <> СвойстваОбъекта.ПорядокКоллекции
				Или Свойства.Имя <> СвойстваОбъекта.Имя Или Свойства.Синоним <> СвойстваОбъекта.Синоним
				Или Свойства.ПолноеИмя <> СвойстваОбъекта.ПолноеИмя Или Свойства.ПолныйСиноним <> СвойстваОбъекта.ПолныйСиноним
				Или Свойства.БезДанных <> СвойстваОбъекта.БезДанных Или Свойства.ЗначениеПустойСсылки
				<> СвойстваОбъекта.ЗначениеПустойСсылки Или Свойства.ИмяРасширения <> СвойстваОбъекта.ИмяРасширения
				...
				Или Свойства.ИмяПредопределенныхДанных <> СвойстваОбъекта.ИмяПредопределенныхДанных Или Свойства.ПометкаУдаления
				Или КлючОбъектаМетаданных = Неопределено Или СвойстваОбъекта.БезКлючаОбъектаМетаданных
				<> (КлючОбъектаМетаданных = Тип("Неопределено")) Тогда
...
Функция МетаданныеРасширенияНайтиПоПолномуИмени(Свойства)
	...
	Если НРег(РасширениеОбъекта.УникальныйИдентификатор) = НРег(Сред(Свойства.ИдентификаторРасширения, 3)) Или НРег(
		РасширениеОбъекта.Имя) = НРег(Сред(Свойства.ИмяРасширения, 3)) Тогда
...
Процедура ДобавитьИдентификаторыНовыхОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений,
	...
	Для Каждого СвойстваОбъекта Из СвойстваОбъектов Цикл
		...
		ЕстьКритичныеИзменения = Истина;
		СписокНовыхОбъектовМетаданных = СписокНовыхОбъектовМетаданных + ?(ЗначениеЗаполнено(
			СписокНовыхОбъектовМетаданных), "," + Символы.ПС, "") + СвойстваОбъекта.ПолноеИмя;
	КонецЦикла;
...
Процедура ОбновитьИдентификаторыОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений,
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если Не Свойства.ЭтоКоллекция Тогда
			...
			Если ОбъектыРасширений И Свойства.ПометкаУдаления = Ложь
				И СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИмяРасширения)] = Неопределено
				И СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИдентификаторРасширения)]
				= Неопределено Тогда
	...
	Если ОбъектыРасширений Тогда
		...
		Если Строки.Количество() > 0 И (Строки.Количество() <> 1 Или ЗначениеЗаполнено(Строки[0].ПолноеИмяОбъекта)
...
Процедура ОбновитьСвойстваПомеченногоНаУдаление(Свойства, СвойстваОбновлены = Ложь, ЕстьУдаленные = Ложь,
	...
	Если Не Свойства.ПометкаУдаления Или ЗначениеЗаполнено(Свойства.Родитель) Или Лев(Свойства.Наименование, 1) <> "?"
		Или Лев(Свойства.Имя, 1) <> "?" Или Лев(Свойства.Синоним, 1) <> "?" Или Лев(Свойства.ПолноеИмя, 1) <> "?"
		Или Лев(Свойства.ПолныйСиноним, 1) <> "?" Или ОбъектыРасширений И Лев(Свойства.ИмяРасширения, 1) <> "?"
		Или ОбъектыРасширений И Лев(Свойства.ИдентификаторРасширения, 1) <> "?" Или ОбъектыРасширений И Лев(
		Свойства.ХешСуммаРасширения, 1) <> "?" Или СтрНайти(Свойства.ПолноеИмя, "(") = 0
		Или Свойства.ЗначениеПустойСсылки <> Неопределено Или Свойства.ИмяПредопределенныхДанных <> "" Или ЭтоДубль
...
Процедура ПроверитьОбновитьСвойстваКоллекции(Знач ТекущиеСвойства, ОбъектыРасширений)
	...
	Если ТекущиеСвойства.Наименование <> НаименованиеКоллекции Или ТекущиеСвойства.ПорядокКоллекции <> НовыеСвойства.ПорядокКоллекции
		Или ТекущиеСвойства.Имя <> НовыеСвойства.Имя Или ТекущиеСвойства.Синоним <> НовыеСвойства.Синоним
		Или ТекущиеСвойства.ПолноеИмя <> НовыеСвойства.Имя Или ТекущиеСвойства.ПолныйСиноним <> НовыеСвойства.Синоним
		Или ТекущиеСвойства.БезДанных <> Ложь Или ТекущиеСвойства.ЗначениеПустойСсылки <> Неопределено
		Или ТекущиеСвойства.ИмяПредопределенныхДанных <> "" Или ТекущиеСвойства.ПометкаУдаления <> Ложь
...
Функция КлючОбъектаМетаданныхСоответствуетПолномуИмени(СвойстваИдентификатора)
	...
	Если КлючОбъектаМетаданных <> Неопределено И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
		...
		ОбъектМетаданных = ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, СвойстваИдентификатора.ИмяРасширения,
			СвойстваИдентификатора.ИдентификаторРасширения, СвойстваИдентификатора.ХешСуммаРасширения);
		Если ОбъектМетаданных <> Неопределено Тогда
	...
	Если ОбъектМетаданных = Неопределено И СвойстваИдентификатора.ПометкаУдаления И СтрНачинаетсяС(
		СвойстваИдентификатора.ПолноеИмяКоллекции, "? ") Тогда
...
Функция ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, ИмяРасширения, ИдентификаторРасширения, ХешСуммаРасширения)
	...
	Если НРег(РасширениеОбъекта.УникальныйИдентификатор) = НРег(ИдентификаторРасширения) Или НРег(
		РасширениеОбъекта.УникальныйИдентификатор) = НРег(Сред(ИдентификаторРасширения, 3)) Или НРег(
		РасширениеОбъекта.Имя) = НРег(ИмяРасширения) Или НРег(РасширениеОбъекта.Имя) = НРег(Сред(ИмяРасширения, 3)) Тогда
...
Функция СвойстваОбъектовМетаданных(ОбъектыРасширений, СвойстваКоллекций = Неопределено,
	ИдентификаторыКлючейРасширений = Неопределено)
	...
	СвойстваОбъектовМетаданных.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(150)));
	СвойстваОбъектовМетаданных.Колонки.Добавить("ПолноеИмя", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(510)));
	СвойстваОбъектовМетаданных.Колонки.Добавить("ПолноеИмяРодителя", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(510)));
	...
	СвойстваОбъектовМетаданных.Колонки.Добавить("ИмяПредопределенныхДанных", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(255)));
	СвойстваОбъектовМетаданных.Колонки.Добавить("Синоним", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(255)));
	СвойстваОбъектовМетаданных.Колонки.Добавить("ПолныйСиноним", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(510)));
	...
	СвойстваОбъектовМетаданных.Колонки.Добавить("ИмяРасширения", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(128)));
	СвойстваОбъектовМетаданных.Колонки.Добавить("ИдентификаторРасширения", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(38)));
	СвойстваОбъектовМетаданных.Колонки.Добавить("ХешСуммаРасширения", Новый ОписаниеТипов("Строка", ,
	...
	ИменаПредопределенныхДанных = ?(ОбъектыРасширений, Метаданные.Справочники.ИдентификаторыОбъектовРасширений,
...
Процедура ДобавитьСвойстваОбъектовМетаданных(Знач КоллекцияОбъектовМетаданных, Знач СвойстваКоллекции,
	Знач СвойстваОбъектовМетаданных, Знач ИдентификаторыКлючейРасширений, Знач ИменаПредопределенных,
	Знач ПолноеИмяРодителя = "", Знач ПолныйСинонимРодителя = "")
	...
	Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
		...
		Если ОбъектыРасширений И Расширение <> Неопределено Тогда
			...
			Если НовыйИдентификаторРасширения <> Неопределено И НовыйИдентификаторРасширения <> ИдентификаторРасширения Тогда
		...
		Если Не СвойстваКоллекции.БезДанных И Не ОбъектыРасширений И Не СтандартныеПодсистемыСервер.ЭтоТаблицаРегистра(
			СвойстваКоллекции.ИмяВЕдЧисле) И СтрНайти(СвойстваКоллекции.ИмяВЕдЧисле, "Константа") = 0 Тогда
		...
		НоваяСтрока.ПолныйСиноним = ПолныйСинонимРодителя + СвойстваКоллекции.СинонимВЕдЧисле + ". "
			+ НоваяСтрока.Синоним;
		...
		Если СвойстваКоллекции.Имя = "Подсистемы" Тогда
			ДобавитьСвойстваОбъектовМетаданных(
				ОбъектМетаданных.Подсистемы, СвойстваКоллекции, СвойстваОбъектовМетаданных,
				ИдентификаторыКлючейРасширений, ИменаПредопределенных, ПолноеИмя, НоваяСтрока.ПолныйСиноним + ". ");
		КонецЕсли;
...
Процедура ПодготовитьСписокНовыхПодсистемВПодчиненномУзле(ОбъектыДляЗаписи)
	...
	Для Каждого Объект Из ОбъектыДляЗаписи Цикл
		Если Не Объект.ЭтоНовый() Или Объект.ДополнительныеСвойства.Свойство("ЭтоЗаменаДубля") Тогда
			Продолжить;
		КонецЕсли;
...
Функция ЭтоОбъектРасширений(ОбъектИлиСсылка)
	...
	Возврат ТипЗнч(ОбъектИлиСсылка) = Тип("СправочникОбъект.ИдентификаторыОбъектовРасширений") Или ТипЗнч(
...
Процедура УточнитьИмяСправочникаВТекстеЗапроса(ТекстЗапроса, ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Справочник.ИдентификаторыОбъектовМетаданных", ИмяСправочника(
			ОбъектыРасширений));
	КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при работе со справочником ""%1"".'"), НазваниеСправочника(ОбъектыРасширений));
...
Процедура ПроверитьОбъектПередЗаписью(Объект, АвтоматическоеОбновление = Ложь)
	...
	Если Не АвтоматическоеОбновление Тогда
		...
		Если Объект.ЭтоНовый() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, НСтр("ru = 'Создание нового идентификатора объекта
		...
		ИначеЕсли ЗапрещеноИзменятьПолноеИмя(Объект) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При изменении идентификатора объекта указано
					 ...
					 |установлено только автоматически при обновлении данных справочника.'"), Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При изменении идентификатора объекта указано
						 |полное имя ""%1"",
						 |которое уже используется в справочнике ""%2"".'"), Объект.ПолноеИмя, НазваниеСправочника));
			КонецЕсли;
		КонецЕсли;
	...
	Если Не ОбъектыРасширений И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		Если Объект.ЭтоНовый() И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового(), ЭтоОбъектРасширений(Объект)) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, НСтр("ru = 'Добавление новых элементов может быть выполнено
		...
		Если Не Объект.ПометкаУдаления И Не ЭтоКоллекция(Объект.Ссылка, ЭтоОбъектРасширений(Объект)) Тогда
			...
			Если ВРег(Объект.ПолноеИмя) <> ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, НСтр("ru = 'Изменение реквизита ""Полное имя"" может быть выполнено
																  |только в главном узле распределенной информационной базы.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеОшибкиИдентификаторыОбъектовРасширенийНедоступныВНеразделенномРежиме()
	...
	Возврат НСтр("ru = 'Справочник ""Идентификаторы объектов расширений""
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение,
	ОдинЭлемент, ПропускатьНеподдерживаемыеОбъекты)
	...
	Для Каждого ПолноеИмяОбъектаМетаданных Из ПолныеИменаОбъектовМетаданных Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Объект метаданных с именем ""%1"" не существует.'"), ПолноеИмяОбъектаМетаданных);
				Ошибки.Добавить(ОписаниеОшибки);
				Продолжить;
			КонецЕсли;

			Если Не Метаданные.Роли.Содержит(ОбъектМетаданных) И Не Метаданные.ПланыОбмена.Содержит(ОбъектМетаданных)
				И Не Метаданные.Константы.Содержит(ОбъектМетаданных) И Не Метаданные.Справочники.Содержит(
				ОбъектМетаданных) И Не Метаданные.Документы.Содержит(ОбъектМетаданных)
				И Не Метаданные.ЖурналыДокументов.Содержит(ОбъектМетаданных) И Не Метаданные.Отчеты.Содержит(
				ОбъектМетаданных) И Не Метаданные.Обработки.Содержит(ОбъектМетаданных)
				И Не Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных) И Не Метаданные.ПланыСчетов.Содержит(
				ОбъектМетаданных) И Не Метаданные.ПланыВидовРасчета.Содержит(ОбъектМетаданных)
				И Не Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) И Не Метаданные.РегистрыНакопления.Содержит(
				ОбъектМетаданных) И Не Метаданные.РегистрыБухгалтерии.Содержит(ОбъектМетаданных)
				И Не Метаданные.РегистрыРасчета.Содержит(ОбъектМетаданных) И Не Метаданные.БизнесПроцессы.Содержит(
				ОбъектМетаданных) И Не Метаданные.Задачи.Содержит(ОбъектМетаданных) И Не ЭтоПодсистема(
			...
			Расширение = ОбъектМетаданных.РасширениеКонфигурации();
			Если Расширение <> Неопределено И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификаторы объектов метаданных расширений не поддерживаются в неразделенном режиме.
						 ...
						 |расширения ""%2"" версии ""%3"".'"), ПолноеИмяОбъектаМетаданных, Расширение.Имя,
					Расширение.Версия);
				Ошибки.Добавить(ОписаниеОшибки);
				Продолжить;
			КонецЕсли;
			...
			ШаблонОшибки = ?(Расширение <> Неопределено, НСтр("ru = 'Для объекта метаданных ""%1""
		...
		ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
			...
			ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений, НСтр("ru = 'Для объекта метаданных ""%1""
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для объекта метаданных ""%1""
						 ...
						 |которому соответствует удаленный объект метаданных.'"), ПолноеИмяОбъектаМетаданных,
					НазваниеСправочника);
			Иначе
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для объекта метаданных ""%1""
						 |найден идентификатор в справочнике ""%2"",
						 |который соответствует другому объекту метаданных ""%3"".'"), ПолноеИмяОбъектаМетаданных,
					НазваниеСправочника, РезультатПроверки.ОбъектМетаданных);
			КонецЕсли;
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если ОдинЭлемент Тогда
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при выполнении функции %1.'"), "ОбщегоНазначения.ИдентификаторОбъектаМетаданных");

		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при выполнении функции %1.'"), "ОбщегоНазначения.ИдентификаторыОбъектовМетаданных");
		Иначе
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибки при выполнении функции %1.'"), "ОбщегоНазначения.ИдентификаторыОбъектовМетаданных");
		КонецЕсли;
		...
		Для Каждого ОписаниеОшибки Из Ошибки Цикл
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				...
				ОписаниеОшибки = "... " + СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = ';И еще %1 ошибка;;И еще %1 ошибки;И еще %1 ошибок;И еще %1 ошибки'"), (КоличествоОшибок
					- НомерОшибки));
...
Функция ИдентификаторыИзКлючей()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КлючОбъектаМетаданных = Неопределено Или КлючОбъектаМетаданных = Тип("Неопределено") Тогда
			Продолжить;
		КонецЕсли;
...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, ВызыватьИсключение)
	...
	Для Каждого ТекущийИдентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли ВызыватьИсключение Тогда
			Если ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Или ТипЗнч(
				ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка при выполнении функции %1.
						 |
						 |Неверный идентификатор: пустая ссылка типа ""%2"".'"),
					"ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору", ТипЗнч(ТекущийИдентификатор));
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка при выполнении функции %1.
						 |
						 |Неверный тип идентификатора объекта метаданных:
						 |""%2"".'"), "ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору", ТипЗнч(ТекущийИдентификатор));
			КонецЕсли;
			ВызватьИсключение ТекстОшибки;
		Иначе
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы, ИдентификаторыКонфигурации,
	ИдентификаторыРасширений, ВызыватьИсключение)
	...
	Выгрузка = Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при выполнении функции %1.'"), "ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору");
			...
			Если ИдентификаторыРасширений.Найти(Идентификатор) = Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификатор объекта метаданных ""%1""
						 ...
						 |был вновь добавлен, тогда настройки следует сделать заново.'"), Строка(Идентификатор));
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Если РезультатПроверки.НеСоответствует Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				...
				Если Свойства.ОбъектРасширений Тогда
					...
					ИмяРасширения = "";
					ИдентификаторРасширения = ?(СтрНачинаетсяС(Свойства.ИдентификаторРасширения, "? "), Сред(
						Свойства.ИдентификаторРасширения, 3), Свойства.ИдентификаторРасширения);
					Если СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ИдентификаторРасширения) Тогда
						Отбор = Новый Структура("УникальныйИдентификатор",
							Новый УникальныйИдентификатор(ИдентификаторРасширения));
						УстановленныеРасширения = РасширенияКонфигурации.Получить(Отбор,
							ИсточникРасширенийКонфигурации.БазаДанных);
						ОтключенныеРасширения   = РасширенияКонфигурации.Получить(Отбор,
							ИсточникРасширенийКонфигурации.СеансОтключенные);
						АктивныеРасширения      = РасширенияКонфигурации.Получить(Отбор,
					...
					Если Не ЗначениеЗаполнено(ИмяРасширения) Тогда
						ИмяРасширения = ?(СтрНачинаетсяС(Свойства.ИмяРасширения, "? "), Сред(Свойства.ИмяРасширения,
							3), Свойства.ИмяРасширения);
					...
					Если УстановленныеРасширения.Количество() = 0 Тогда
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение конфигурации ""%1"" удалено.
								 ...
								 |Следует выполнить настройки заново с учетом изменений.'"), ИмяРасширения,
					...
					ИначеЕсли Не УстановленныеРасширения[0].Активно Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение конфигурации ""%1"" установлено, но выключено.
								 |Включите расширение и перезапустите сеанс.'"), ИмяРасширения);
					...
					ИначеЕсли ОтключенныеРасширения.Количество() > 0 И Не ОтключенныеРасширения[0].Активно
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение конфигурации ""%1"" установлено после запуска сеанса, поэтому не подключено.
								 |Перезапустите сеанс.'"), ИмяРасширения);
					...
					ИначеЕсли ОтключенныеРасширения.Количество() > 0 И ОтключенныеРасширения[0].Активно Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение конфигурации ""%1"" установлено, но отключено при запуске сеанса.
								 |Это значит, что при его подключении возникла ошибка.'"), ИмяРасширения);
					...
					Иначе // АктивныеРасширения.Количество() > 0
						...
						Если РезультатПроверки.ОбъектМетаданныхУдаленного <> Неопределено
							...
							И РезультатПроверки.ОбъектМетаданныхУдаленного.РасширениеКонфигурации().Имя
							= АктивныеРасширения[0].Имя Тогда
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Расширение конфигурации ""%1"" установлено и подключено,
									 ...
									 |более недоступны и их следует сделать заново.'"), ИмяРасширения,
								ПредставлениеИдентификатора);
						Иначе
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Расширение конфигурации ""%1"" установлено и подключено,
				...
				ИначеЕсли РезультатПроверки.ОбъектМетаданныхУдаленного <> Неопределено
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект ""%1""
							 ...
							 |более недоступны и их следует сделать заново.'"), ПредставлениеИдентификатора);
					ВызватьИсключение ТекстОшибки;
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект ""%1"" не существует.
							 ...
							 |Объект и все настройки, сделанные до его удаления, более недоступны.
							 |Следует выполнить настройки заново с учетом изменений.'"), ПредставлениеИдентификатора);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
			...
			Иначе
				ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификатору ""%1""
						 ...
						 |полное имя которого отличается от заданного в идентификаторе.'"), Свойства.Представление,
					НазваниеСправочника(Свойства.ОбъектРасширений), РезультатПроверки.ОбъектМетаданных.ПолноеИмя())
					+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика();
			КонецЕсли;
		...
		Если Не Свойства.ОбъектРасширений И Свойства.ПометкаУдаления И Не КонфигурацияБазыДанныхИзмененаДинамически Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1""
					 ...
					 |но значение реквизита ""Пометка удаления"" установлено Истина.'"), Свойства.Представление,
...
Процедура ЗаменитьДублиПодчиненногоУзлаНайденныеПриЗагрузке(ТолькоПроверка, ЕстьИзменения)
	...
	Пока Истина Цикл
		...
		Если ТекущаяПопытка > 10 Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось выполнить замену дублей идентификаторов объектов метаданных.
...
//
Функция ВыполнитьЗаменуЭлементов(Знач Заменяемые, Знач ТаблицаСсылок, Знач ОтключатьКонтрольЗаписи = Ложь,
	Знач ОбъектыРасширений = Ложь)
	...
	Попытка
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'При записи объекта ""%1"" возникла ошибка:
								 |%2'"), ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
						...
						Исключение
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'При записи движений объекта ""%1"" в ""%2"" возникла ошибка:
									 |%3'"), ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
								НаборЗаписей.Метаданные().ПолноеИмя(), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
							Если ТранзакцияАктивна() Тогда
								ВызватьИсключение ТекстОшибки;
							КонецЕсли;
							СообщитьОбОшибке(ТекстОшибки, ОбъектыРасширений);
								...
								Исключение
									...
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'При записи по регистратору ""%1"" в ""%2"" возникла ошибка:
												 |%3'"), ПолучитьНавигационнуюСсылку(СтрокаТаблицы.Данные),
										НаборЗаписи.Метаданные().ПолноеИмя(), ПодробноеПредставлениеОшибки(
										ИнформацияОбОшибке));
									Если ТранзакцияАктивна() Тогда
										ВызватьИсключение ТекстОшибки;
									КонецЕсли;
			...
			ИначеЕсли Метаданные.Справочники.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				Если СтрокаТаблицы.Метаданные.Владельцы.Содержит(Ссылка.Метаданные()) И Параметры.Объект.Владелец
					= Ссылка Тогда
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				КонецЦикла;
				Если СтрокаТаблицы.Метаданные.ПериодичностьРегистраСведений
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'При удалении записи ""%1"" возникла ошибка:
								 |%2'"), ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(
							СтруктураИзмерений)), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
					КонецПопытки;
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Значения не заменяются в данных типа: %1'"), Строка(СтрокаТаблицы.Метаданные));
		...
		Если Параметры.Объект <> Неопределено Тогда
			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При записи объекта ""%1"" возникла ошибка:
						 |%2'"), ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка), ПодробноеПредставлениеОшибки(
					ИнформацияОбОшибке));
				Если ТранзакцияАктивна() Тогда
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений, НСтр("ru = 'Идентификаторы объектов расширений.Замена идентификатора'",
		ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru = 'Идентификаторы объектов метаданных.Замена идентификатора'",
		ОбщегоНазначения.КодОсновногоЯзыка())), УровеньЖурналаРегистрации.Ошибка, , , Описание,
