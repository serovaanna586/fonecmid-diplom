...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПоказатьДополнительныеРеквизиты") Тогда
		СтандартныеПодсистемыСервер.УстановитьКлючНазначенияФормы(ЭтотОбъект, "НаборыДополнительныхРеквизитов");
		Элементы.ЭтоНаборыДополнительныхСведений.Видимость = Ложь;

	ИначеЕсли Параметры.Свойство("ПоказатьДополнительныеСведения") Тогда
		СтандартныеПодсистемыСервер.УстановитьКлючНазначенияФормы(ЭтотОбъект, "НаборыДополнительныхСведений");
		Элементы.ЭтоНаборыДополнительныхСведений.Видимость = Ложь;
	...
	Если СуффиксТекущегоЯзыка = Неопределено Тогда
		...
		СвойстваСписка.ТекстЗапроса = СтрЗаменить(НаборыСвойств.ТекстЗапроса, "ГДЕ", "ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаборыДополнительныхРеквизитовИСведений.Представления КАК НаборыПредставления
		...
		СвойстваСписка.ТекстЗапроса = СтрЗаменить(СвойстваСписка.ТекстЗапроса,
			"НаборыПереопределяемый.Наименование КАК Наименование",
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ДополнительныеРеквизитыИСведения" Или ИмяСобытия = "Запись_ЗначенияСвойствОбъектов"
...
Процедура СвойстваОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		ИначеЕсли ВыбранноеЗначение.Свойство("ОбщееСвойство") Тогда
			...
			Оповестить("Запись_НаборыДополнительныхРеквизитовИСведений", Новый Структура("Ссылка", ИзменяемыйНабор),
				ИзменяемыйНабор);
		Иначе
...
Процедура СкопироватьВыделенныйРеквизит(Команда)
	...
	КопируемыйРеквизит.Вставить("ВладелецДополнительныхЗначений",
		Элементы.Свойства.ТекущиеДанные.ВладелецДополнительныхЗначений);
...
Процедура НастроитьОтображениеНаборов()
	...
	Если ЭтоНаборыДополнительныхСведений Тогда
		...
		МетаданныеТабличнаяЧасть = Метаданные.Справочники.НаборыДополнительныхРеквизитовИСведений.ТабличныеЧасти.ДополнительныеСведения;
		...
		Элементы.СвойстваТипЗначения.Подсказка = НСтр(
			"ru = 'Типы значения, которое можно ввести при заполнении сведения.'");

		Элементы.СвойстваОбщиеЗначения.Подсказка = НСтр("ru = 'Сведение использует список значений сведения-образца.'");
	...
	Иначе
		...
		МетаданныеТабличнаяЧасть = Метаданные.Справочники.НаборыДополнительныхРеквизитовИСведений.ТабличныеЧасти.ДополнительныеРеквизиты;

		Элементы.СвойстваЗаголовок.Подсказка = МетаданныеТабличнаяЧасть.Реквизиты.Свойство.Подсказка;

		Элементы.СвойстваЗаполнятьОбязательно.Видимость = Истина;
		Элементы.СвойстваЗаполнятьОбязательно.Подсказка = Метаданные.ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.Реквизиты.ЗаполнятьОбязательно.Подсказка;

		Элементы.СвойстваТипЗначения.Подсказка = НСтр(
			"ru = 'Типы значения, которое можно ввести при заполнении реквизита.'");

		Элементы.СвойстваОбщиеЗначения.Подсказка = НСтр("ru = 'Реквизит использует список значений реквизита-образца.'");
	...
	Для Каждого Ссылка Из Наборы Цикл
		...
		Если ЭтоНаборыДополнительныхСведений И ВидыСвойствНабора.ДополнительныеСведения Или Не ЭтоНаборыДополнительныхСведений
...
Процедура ИзменитьПометкуУдаления()
	...
	Если Элементы.Свойства.ТекущиеДанные <> Неопределено Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ИзменитьПометкуУдаленияЗавершение", ЭтотОбъект, ТекущийНабор), ТекстВопроса,
...
Процедура ИзменитьПометкуУдаленияЗавершение(Ответ, ТекущийНабор) Экспорт
	...
	Оповестить("Запись_НаборыДополнительныхРеквизитовИСведений", Новый Структура("Ссылка", ТекущийНабор), ТекущийНабор);
...
Процедура ПриИзмененииТекущегоНабораНаСервере()
	...
	Если ЗначениеЗаполнено(ТекущийНабор) И Не ТекущийНаборЭтоГруппа Или ПоказатьНеиспользуемыеРеквизиты Тогда
...
Процедура СвойстваУстановитьДоступностьКоманд(Контекст)
	...
	ДоступностьДляСтроки = ОбщаяДоступность И Контекст.Элементы.Свойства.ТекущаяСтрока <> Неопределено;
	...
	Элементы.СвойстваКонтекстноеМенюСкопировать.Доступность        = ДоступностьДляСтроки
		И Не ПоказатьНеиспользуемыеРеквизиты;
...
Процедура ДобавитьОбщееСвойствоПеретаскиванием(ДобавляемоеСвойство)
	...
	Попытка
		...
		Попытка
			...
			ТабличнаяЧасть = НаборПриемникОбъект[?(ЭтоНаборыДополнительныхСведений, "ДополнительныеСведения",
	...
	Попытка
		...
		Попытка
			...
			Если ТекущийНаборОбъект.ВерсияДанных <> ТекущийНаборВерсияДанных Тогда
				ПриИзмененииТекущегоНабораНаСервере();
				Если ЭтоНаборыДополнительныхСведений Тогда
					ВызватьИсключение НСтр("ru = 'Действие не выполнено, так как состав дополнительных сведений
										   |был изменен другим пользователем.
										   |Новый состав дополнительных сведений прочитан.
										   |
										   |Повторите действие, если требуется.'");
				Иначе
					ВызватьИсключение НСтр("ru = 'Действие не выполнено, так как состав дополнительных реквизитов
										   |был изменен другим пользователем.
			...
			ТабличнаяЧасть = ТекущийНаборОбъект[?(ЭтоНаборыДополнительныхСведений, "ДополнительныеСведения",
				"ДополнительныеРеквизиты")];
					...
					ИначеЕсли Команда = "ИзменитьПометкуУдаления" Тогда
						...
						Если Строка.Общее Тогда
							...
							ИначеЕсли ТабличнаяЧасть.Количество() > 0 Тогда
								Элементы.Свойства.ТекущаяСтрока = Свойства[Свойства.Количество()
						...
						Иначе
							...
							ИзменитьПометкуУдаленияИВладельцаЗначений(
								ТабличнаяЧасть[Индекс].Свойство, ТабличнаяЧасть[Индекс].ПометкаУдаления);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
