...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	КонецЕсли;
#Иначе
		Если СтрСравнить(СтандартныеПодсистемыКлиент.ПараметрКлиента("КодОсновногоЯзыка"),
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка()) = 0 Тогда
			Возврат;
		КонецЕсли;
#КонецЕсли
...
Процедура ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого ОбновляемыйНабор Из Результат Цикл
		...
		Попытка
			...
			ИмяНабора = Сред(ОбновляемыйНабор.ИмяПредопределенныхДанных, ДлинаПрефикса + 1, СтрДлина(
				ОбновляемыйНабор.ИмяПредопределенныхДанных) - ДлинаПрефикса);
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать набор свойств %1 по причине:
					 |%2'"), ОбновляемыйНабор.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ПроблемныхОбъектов <> 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
			"ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию");
...
Функция СвойстваНабора(НаборыСвойств, Набор, Родитель = Неопределено, Наименования = Неопределено) Экспорт
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1 общего модуля %2.'"), "ПриСозданииПредопределенныхНаборовСвойств",
		"УправлениеСвойствамиПереопределяемый") + Символы.ПС + Символы.ПС;
	...
	Если НаборыСвойств.Получить(Набор.Имя) <> Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Имя набора свойств ""%1"" уже определено.'"), Набор.Имя);
	КонецЕсли;

	Если Не ЗначениеЗаполнено(Набор.Идентификатор) Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Идентификатор набора свойств ""%1"" не заполнен.'"), Набор.Имя);
	КонецЕсли;
	...
	Если НаборыСвойств.Получить(НаборСсылка) <> Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Идентификатор ""%1"" набора свойств
				 |""%2"" уже используется для набора ""%3"".'"), Набор.Идентификатор, Набор.Имя, СвойстваНабора.Имя);
	КонецЕсли;
...
Функция СвойстваНабораНачальногоЗаполнения(НаборыСвойств, Набор, Родитель = Неопределено, Наименование = Неопределено)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1 общего модуля %2.'") + Символы.ПС + Символы.ПС,
		"ПриНачальномЗаполненииЭлементов", "УправлениеСвойствамиПереопределяемый");
	...
	Если НаборыСвойств.Получить(Набор.ИмяПредопределенногоНабора) <> Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 набора свойств ""%2"" уже определено.'"), "ИмяПредопределенногоНабора",
			Набор.ИмяПредопределенногоНабора);
	КонецЕсли;

	Если Не ЗначениеЗаполнено(Набор.Ссылка) Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ссылка набора свойств ""%1"" не заполнен.'"), Набор.ИмяПредопределенногоНабора);
	КонецЕсли;
	...
	Если НаборыСвойств.Получить(НаборСсылка) <> Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ссылка ""%1"" набора свойств
				 |""%2"" уже используется для набора ""%3"".'"), Набор.Ссылка, Набор.ИмяПредопределенногоНабора,
			СвойстваНабора.ИмяПредопределенногоНабора);
	КонецЕсли;
