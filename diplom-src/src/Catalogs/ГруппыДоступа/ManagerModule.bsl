...
Функция ГруппаДоступаАдминистраторы(ПрофильАдминистратор = Неопределено) Экспорт
	...
	Если ВыборкаПоПрофилю.Следующий() И ВыборкаПоПредопределенному.Следующий() И ВыборкаПоПрофилю.Количество() = 1
		И ВыборкаПоПредопределенному.Количество() = 1 И ВыборкаПоПрофилю.Ссылка = ВыборкаПоПредопределенному.Ссылка Тогда
	...
	Попытка
		...
		ГруппаДоступаОбъект.Наименование = НСтр("ru = 'Администраторы'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОбновитьГруппыДоступаПрофиля(Профиль, ОбновлятьГруппыДоступаСУстаревшимиНастройками = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ГруппаДоступа.Ссылка = УправлениеДоступом.ГруппаДоступаАдминистраторы() И ГруппаДоступа.Профиль
			<> УправлениеДоступом.ПрофильАдминистратор() Тогда
		...
		Если ГруппаДоступа.ВидыДоступа.Количество() <> ВидыДоступаПрофиля.НайтиСтроки(
			Новый Структура("Предустановленный", Ложь)).Количество() Тогда
			СоставВидовДоступаИзменен = Истина;
		Иначе
			Для Каждого СтрокаВидаДоступа Из ГруппаДоступа.ВидыДоступа Цикл
				Если ВидыДоступаПрофиля.НайтиСтроки(Новый Структура("ВидДоступа, Предустановленный",
					СтрокаВидаДоступа.ВидДоступа, Ложь)).Количество() = 0 Тогда
					СоставВидовДоступаИзменен = Истина;
					Если ГруппаДоступа.ЗначенияДоступа.Найти(СтрокаВидаДоступа.ВидДоступа, "ВидДоступа")
						<> Неопределено Тогда
		...
		Если СоставВидовДоступаИзменен И (ОбновлятьГруппыДоступаСУстаревшимиНастройками
			...
			Пока ТекущийНомерСтроки >= 0 Цикл
				...
				Если ВидыДоступаПрофиля.НайтиСтроки(Новый Структура("ВидДоступа, Предустановленный", ТекущийВидДоступа,
					Ложь)).Количество() = 0 Тогда
					СтрокиЗначенийВидаДоступа = ГруппаДоступа.ЗначенияДоступа.НайтиСтроки(Новый Структура("ВидДоступа",
						ТекущийВидДоступа));
			...
			Для Каждого СтрокаВидаДоступа Из ВидыДоступаПрофиля Цикл
				Если Не СтрокаВидаДоступа.Предустановленный И ГруппаДоступа.ВидыДоступа.Найти(
					СтрокаВидаДоступа.ВидДоступа, "ВидДоступа") = Неопределено Тогда
		...
		Если ГруппаДоступа.Модифицированность() Тогда
			...
			Если Не Справочники.ВерсииРасширений.ВсеРасширенияПодключены() Тогда
				СтарыеЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ГруппаДоступа.Ссылка,
...
Функция ИзменилисьВидыИлиЗначенияДоступа(СтарыеЗначения, ТекущийОбъект) Экспорт
	...
	Если ВидыДоступа.Количество() <> ТекущийОбъект.ВидыДоступа.Количество() Или ЗначенияДоступа.Количество()
		<> ТекущийОбъект.ЗначенияДоступа.Количество() Тогда
...
Функция ПользователиДляОбновленияРолей(СтарыеЗначения, ЭлементДанных) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("УдалениеОбъекта") Или ЭлементДанных.Профиль <> СтарыеЗначения.Профиль
...
Функция РолиДляОбновленияПрав(СтарыеЗначения, ЭлементДанных) Экспорт
	...
	Запрос.УстановитьПараметр("НовыйПрофиль", ?(ТипЗнч(ЭлементДанных) <> Тип("УдалениеОбъекта")
		И Не ЭлементДанных.ПометкаУдаления, ЭлементДанных.Профиль, Справочники.ПрофилиГруппДоступа.ПустаяСсылка()));

	Запрос.УстановитьПараметр("СтарыйПрофиль", ?(ЭлементДанных.Ссылка = СтарыеЗначения.Ссылка
		И Не СтарыеЗначения.ПометкаУдаления, СтарыеЗначения.Профиль, Справочники.ПрофилиГруппДоступа.ПустаяСсылка()));

	Если ТипЗнч(ЭлементДанных) = Тип("УдалениеОбъекта") Или ЭлементДанных.ПометкаУдаления
		<> СтарыеЗначения.ПометкаУдаления Тогда
...
Процедура ЗарегистрироватьСсылки(ВидСсылок, Знач ДобавляемыеСсылки) Экспорт
	...
	ИначеЕсли Ссылки.Количество() = 1 И Ссылки[0] = Неопределено Тогда
...
Процедура ЗарегистрироватьГруппуДоступаИзмененнуюПриЗагрузке(ЭлементДанных) Экспорт
	...
	ИначеЕсли ЭлементДанных.ПометкаУдаления <> СтарыеЗначения.ПометкаУдаления Или ЭлементДанных.Профиль
		<> СтарыеЗначения.Профиль Тогда
	...
	Иначе
		...
		Если НаличиеУчастников <> СтароеНаличиеУчастников Или ИзменилисьВидыИлиЗначенияДоступа(СтарыеЗначения,
			ЭлементДанных) Тогда
...
Процедура ОбновитьВспомогательныеДанныеГруппДоступаИзмененныхПриЗагрузке() Экспорт
	...
	Если ИзмененныеГруппыДоступа.Количество() = 1 И ИзмененныеГруппыДоступа[0] = Неопределено Тогда
	...
	Если УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
		...
		Если СоставИзмененийРолей.Количество() = 1 И СоставИзмененийРолей[0] = Неопределено Тогда
...
Процедура ЗарегистрироватьПользователейГруппыПользователейИзмененнойПриЗагрузке(ЭлементДанных) Экспорт
	...
	СтарыеЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлементДанных.Ссылка, "Ссылка, ПометкаУдаления, Состав");
	...
	Если ПользователиДляОбновления.Количество() > 0 Тогда
		ЗарегистрироватьСсылки("ГруппыПользователей", ПользователиСлужебный.СсылкаОбъекта(ЭлементДанных));
	КонецЕсли;
...
Процедура ЗарегистрироватьПользователяИзмененногоПриЗагрузке(ЭлементДанных) Экспорт
	...
	ИначеЕсли ЭлементДанных.Недействителен <> СтарыеЗначения.Недействителен Или ЭлементДанных.ПометкаУдаления
		<> СтарыеЗначения.ПометкаУдаления Тогда
	...
	ЗарегистрироватьСсылки("ГруппыПользователей", ?(ТипЗнч(ЭлементДанных.Ссылка) = Тип("СправочникСсылка.Пользователи"),
		Справочники.ГруппыПользователей.ВсеПользователи, Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи));
...
Процедура ОбновитьРолиПользователейИзмененныхПриЗагрузке() Экспорт
	...
	Если ИзмененныеПользователи.Количество() = 1 И ИзмененныеПользователи[0] = Неопределено Тогда
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(
		"Ссылка"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если Справочники.ВерсииРасширений.РасширенияИзмененыДинамически() И (Не ОбщегоНазначения.ИнформационнаяБазаФайловая()
		...
		НаименованиеЗадания = НСтр("ru = 'Обновление вспомогательных данных групп доступа'",
		...
		ФоновоеЗадание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(
			...
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыОбновления), , НаименованиеЗадания);
			...
			Если ФоновоеЗадание.ИнформацияОбОшибке <> Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Фоновое задание ""%1"" завершилось с ошибкой:
						 |%2'"), НаименованиеЗадания, ПодробноеПредставлениеОшибки(ФоновоеЗадание.ИнформацияОбОшибке));
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ГруппыДоступа, , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработана очередная порция групп доступа: %1'"), ОбъектовОбработано));
	КонецЕсли;
...
Процедура ОбновитьВспомогательныеДанныеГруппДоступа(Параметры) Экспорт
	...
	ШаблонОшибкиОбработкиГруппыДоступа = НСтр("ru = 'Не удалось обработать группу доступа ""%1"" по причине:
											  |%2'");
	ШаблонОшибкиОбновленияТаблицГруппДоступа = НСтр("ru = 'Не удалось обновить таблицы группы доступа ""%1"" по причине:
													|%2'");
	ШаблонОшибкиОбновленияЗначенийГруппДоступа = НСтр("ru = 'Не удалось обновить значения доступа группы доступа ""%1"" по причине:
													  |%2'");
	...
	Для Каждого ГруппаДоступа Из Параметры.ГруппыДоступа Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, Строка(ГруппаДоступа),
...
Функция СвойстваВидаСсылок(ВидСсылок)
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение ""%1"" параметра %2 функции %3.'"), ВидСсылок, "ВидСсылок",
			"СвойстваВидаСсылок");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
