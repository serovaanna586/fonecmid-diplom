...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = УправлениеДоступом.ГруппаДоступаАдминистраторы() Тогда
		...
		Если Не ПривилегированныйРежим() И Не УправлениеДоступом.ЕстьРоль("ПолныеПрава") Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если Профиль <> СтарыеЗначения.Профиль Или ПометкаУдаления <> СтарыеЗначения.ПометкаУдаления Или НаличиеУчастников
		<> СтароеНаличиеУчастников Тогда
	...
	Если Справочники.ГруппыДоступа.ИзменилисьВидыИлиЗначенияДоступа(СтарыеЗначения, ЭтотОбъект) Или ПометкаУдаления
		<> СтарыеЗначения.ПометкаУдаления Или НаличиеУчастников <> СтароеНаличиеУчастников Тогда
	...
	Если УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
		...
		УправлениеДоступомСлужебный.ЗапланироватьОбновлениеНаборовГруппДоступа(
			"ГруппыДоступаПриИзмененииУчастников", ТипыИзмененныхУчастников.Пользователи,
...
Функция ТипыИзмененныхУчастников(НовыеУчастники, СтарыеУчастники)
	...
	Для Каждого Строка Из ВсеУчастники Цикл
		Если Строка.ВидИзмененияСтроки = 0 Тогда
			Продолжить;
		КонецЕсли;
		Если ТипЗнч(Строка.Пользователь) = Тип("СправочникСсылка.Пользователи") Или ТипЗнч(Строка.Пользователь) = Тип(
			"СправочникСсылка.ГруппыПользователей") Тогда
			ТипыИзмененныхУчастников.Пользователи = Истина;
		КонецЕсли;
		Если ТипЗнч(Строка.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Или ТипЗнч(Строка.Пользователь)
...
Процедура ОбновитьРолиПользователейПриИзмененииГруппыДоступа()
	...
	Если ОбщегоНазначения.РазделениеВключено() И Ссылка = УправлениеДоступом.ГруппаДоступаАдминистраторы() И Не СеансЗапущенБезРазделителей
	...
	ПользователиДляОбновления = Справочники.ГруппыДоступа.ПользователиДляОбновленияРолей(СтарыеЗначения, ЭтотОбъект);
	...
	Если Ссылка = УправлениеДоступом.ГруппаДоступаАдминистраторы() Тогда
		...
		Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
			Если ПользовательИБ.Роли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
				...
				Если ЗначениеЗаполнено(НайденныйПользователь) И ПользователиДляОбновления.Найти(НайденныйПользователь)
					= Неопределено Тогда
