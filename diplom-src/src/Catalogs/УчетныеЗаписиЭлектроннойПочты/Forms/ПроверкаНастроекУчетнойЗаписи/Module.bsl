...
&НаКлиенте
Процедура СпособыУстраненияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
...
Функция НачатьВыполнениеНаСервере()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"Справочники.УчетныеЗаписиЭлектроннойПочты.ПроверитьНастройкиУчетнойЗаписи", Параметры.УчетнаяЗапись);
КонецФункции
...
Процедура УстановитьКлючСохраненияПоложенияОкна()
	...
	КлючСохраненияПоложенияОкна = ОбщегоНазначения.КонтрольнаяСуммаСтрокой(Строка(ВозможныеПричины) + Строка(
		СпособыУстранения));
...
Процедура ПерейтиКНастройкамПочты()
	...
	Если ВладелецФормы <> Неопределено И ВладелецФормы.ИмяФормы
		= "Справочник.УчетныеЗаписиЭлектроннойПочты.Форма.ФормаЭлемента" Тогда
