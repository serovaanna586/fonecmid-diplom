...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаУчетнойЗаписиЗаголовок.Заголовок = ?(КонтекстныйРежим, НСтр(
		"ru = 'Для отправки писем настройте почту'"), НСтр("ru = 'Введите настройки почты'"));
		...
		Если Выборка.Следующий() Тогда
			...
			Если Выборка.АвторизацияСПомощьюПочтовогоСервиса Или ТолькоАвторизация Тогда
				...
				Если ТолькоАвторизация Тогда
					...
					Если ЗначениеЗаполнено(НастройкиАвторизации) И ЗначениеЗаполнено(
						НастройкиАвторизации.ИдентификаторПриложения) Тогда
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
			...
			Для Каждого Контакт Из КонтактнаяИнформацияОбъекта Цикл
				...
				Если Справочники.УчетныеЗаписиЭлектроннойПочты.НайтиПоРеквизиту("АдресЭлектроннойПочты",
					Адрес).Пустая() Тогда
...
Процедура Назад(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ПроверкаНастроекУчетнойЗаписи Или ТекущаяСтраница
		= Элементы.ТехническиеПодробностиОшибки Тогда
		ПредыдущаяСтраница = Элементы.НастройкаУчетнойЗаписи;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Отмена, НСтр(
		"ru = 'Настройка почты'"));
КонецПроцедуры
...
Процедура ПерейтиНаСледующуюСтраницу(Команда = Неопределено)
	...
	Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи Или ТекущаяСтраница = Элементы.ТехническиеПодробностиОшибки Тогда
		...
		Если Не Отказ И СпособАутентификации = "OAuth" Тогда
			...
			Если Не ЗначениеЗаполнено(АдресПеренаправления) Или ЗначениеЗаполнено(НастройкиАвторизации)
				И Не ЗначениеЗаполнено(НастройкиАвторизации.АдресАвторизации) Или ЭтоВебКлиент()
				И Не ДоступнаАвторизацияПоКоду() Тогда
				СообщенияОбОшибках = НСтр(
					"ru = 'Не найдены настройки авторизации почтового сервиса. Используйте авторизацию по паролю.'");
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Авторизация Тогда
		Если ЭтоВебКлиент() И Не ЗначениеЗаполнено(НастройкиАвторизации.АдресРегистрацииУстройства) Тогда
			Если Не ПолучитьКлючиДоступаКПочтовомуСерверу(КодПодтверждения, НастройкиАвторизации.АдресПолученияКлюча,
				КлиентскоеПриложение.ПолучитьКраткийЗаголовок()) Тогда
				ПроверкаЗавершиласьСОшибками = Истина;
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьТекстыПоясненийПоРегистрацииПриложения()
	...
	Если Не ЗначениеЗаполнено(НастройкиАвторизации.ПояснениеПоАдресуПеренаправления) И Не ЗначениеЗаполнено(
		НастройкиАвторизации.ПояснениеПоИдентификаторуПриложения) И Не ЗначениеЗаполнено(
		НастройкиАвторизации.ПояснениеПоПаролюПриложения) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ИспользоватьДляОтправки Тогда
		Результат.Добавить(
			МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				"SMTP", СерверИсходящейПочты, ПортСервераИсходящейПочты, НСтр("ru = 'Электронная почта.'")));
	КонецЕсли;

	Если ИспользоватьДляПолучения Тогда
		Результат.Добавить(
			МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				Протокол, СерверВходящейПочты, ПортСервераВходящейПочты, НСтр("ru = 'Электронная почта.'")));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты,
		Истина) Тогда
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		...
		КонецЕсли;
	Иначе
		Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи И ПроверкаЗавершиласьСОшибками Или ТекущаяСтраница
			= Элементы.ТехническиеПодробностиОшибки Тогда
			ЗаголовокКнопкиДалее = НСтр("ru = 'Повторить'");
		ИначеЕсли ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи И СпособНастройки = "Автоматически" Тогда
	...
	Элементы.КнопкаДалее.Видимость = Не (ТекущаяСтраница = Элементы.ПроверкаНастроекУчетнойЗаписи И СпособНастройки
		= "Вручную") И Не (ТекущаяСтраница = Элементы.Авторизация И Не ЭтоВебКлиент()) И Не ТекущаяСтраница
		= Элементы.ПриПроверкеОбнаруженыОшибки;
	
	// КнопкаНазад
	Элементы.КнопкаНазад.Видимость = ТекущаяСтраница <> Элементы.НастройкаУчетнойЗаписи И ТекущаяСтраница <> Элементы.УчетнаяЗаписьУспешноНастроена
		И ТекущаяСтраница <> Элементы.ПроверкаНастроекУчетнойЗаписи И Не ТолькоАвторизация;
	...
	Элементы.КнопкаПерейтиКНастройкам.Видимость = Не ИспользуютсяПрофилиБезопасности И (ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
		И ПроверкаЗавершиласьСОшибками Или Не КонтекстныйРежим И Не Перенастройка И ТекущаяСтраница
		= Элементы.УчетнаяЗаписьУспешноНастроена);
	...
	Если ТекущаяСтраница = Элементы.Авторизация И Элементы.ВариантыАвторизации.ТекущаяСтраница
		= Элементы.ВстроенныйБраузер Тогда
		Активизировать();
	КонецЕсли;
...
Функция НастройкиПоУмолчанию(АдресЭлектроннойПочты, Пароль)
	...
	НастройкиIMAPПоУмолчанию = Справочники.УчетныеЗаписиЭлектроннойПочты.ВариантыНастройкиПодключенияКСерверуIMAP(
		АдресЭлектроннойПочты)[0];
	НастройкиSMTPПоУмолчанию = Справочники.УчетныеЗаписиЭлектроннойПочты.ВариантыНастройкиПодключенияКСерверуSMTP(
		АдресЭлектроннойПочты)[0];
...
Процедура СоздатьУчетнуюЗапись()
	...
	Попытка
		...
		УчетнаяЗапись.ПериодХраненияСообщенийНаСервере = ?(ОставлятьКопииПисемНаСервере И УдалятьПисьмаССервера
		...
		Если СпособАутентификации = "OAuth" Тогда
			...
			ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(УчетнаяЗаписьСсылка, СрокДействияТокенаДоступа,
...
Процедура НастроитьПараметрыПодключенияАвтоматически()
	...
	Если СтрНайти(НРег(АдресЭлектроннойПочты), "@gmail.com") > 0 Тогда
		СообщенияОбОшибках = СообщенияОбОшибках + Символы.ПС + НСтр("ru = 'См. также рекомендации по настройке почты Gmail:
...
Функция НачатьПоискНастроекУчетнойЗаписи()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"Справочники.УчетныеЗаписиЭлектроннойПочты.ОпределитьНастройкиУчетнойЗаписи", АдресЭлектроннойПочты,
		ПарольДляПолученияПисем, ИспользоватьДляОтправки, ИспользоватьДляПолучения);
...
Функция ПараметрыЗапросаАвторизации()
	...
	Если НастройкиАвторизации.ИспользоватьКлючПроверкиПодлинностиPKCE Тогда
		ПараметрыЗапроса.Вставить("code_challenge", РаботаСПочтовымиСообщениямиСлужебный.КодироватьСтрокуМетодомS256(
			ПроверочныйКод));
...
Процедура ПриПолученииОтветаПочтовогоСервера(СтрокаПараметров, АдресПолученияКлюча)
	...
	ИначеЕсли ИдентификаторЗапроса <> Ответ["state"] Тогда
		СообщенияОбОшибках = НСтр(
			"ru = 'Не удалось авторизоваться на почтовом сервере. Неверный идентификатор ответа.'");
		ПроверкаЗавершиласьСОшибками = Истина;
...
Функция ПолучитьКлючиДоступаКПочтовомуСерверу(КодАвторизации, АдресПолученияКлюча, ЗаголовокПриложения = "")
	...
	РезультатЗапроса = РаботаСПочтовымиСообщениямиСлужебный.ВыполнитьЗапрос(АдресСервера, АдресРесурса,
		ПараметрыЗапроса);
...
Функция НастройкиПодключенияПоАдресуЭлектроннойПочты()
	...
	Возврат Справочники.УчетныеЗаписиЭлектроннойПочты.НастройкиПодключенияПоАдресуЭлектроннойПочты(
		АдресЭлектроннойПочты, ПарольДляПолученияПисем);
...
Функция ДоступнаАвторизацияПоКоду()
	...
	Возврат ЗначениеЗаполнено(НастройкиАвторизации) И (ЗначениеЗаполнено(НастройкиАвторизации.АдресРегистрацииУстройства)
