	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрСравнить(Выборка.Код, Код) = 0 Тогда
			...
			Сообщение.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'С кодом %1 уже существует страна %2. Измените код или используйте уже существующие данные.'"),
		...
		ИначеЕсли СтрСравнить(Выборка.Наименование, Наименование) = 0 Тогда
			...
			Сообщение.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'Страна с наименованием %1 уже существует. Измените наименование или используйте уже существующие данные.'"),
				Выборка.Наименование);
			Сообщение.ИмяПоля = "Объект.Наименование";

		ИначеЕсли ЗначениеЗаполнено(НаименованиеПолное) И СтрСравнить(Выборка.НаименованиеПолное, НаименованиеПолное)
			= 0 Тогда

			Сообщение.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'Уже существует страна %2 с полным наименованием %1. Измените полное наименование или используйте уже существующие данные.'"),
				НаименованиеПолное, Выборка.Наименование);
			Сообщение.ИмяПоля = "Объект.НаименованиеПолное";

		ИначеЕсли ЗначениеЗаполнено(КодАльфа2) И СтрСравнить(Выборка.КодАльфа2, КодАльфа2) = 0 Тогда

			Сообщение.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'С кодом Альфа-2 %1 уже существует страна %2. Измените код Альфа-2 или используйте уже существующие данные.'"),
				КодАльфа2, Выборка.Наименование);
			Сообщение.ИмяПоля = "Объект.КодАльфа2";

		ИначеЕсли ЗначениеЗаполнено(КодАльфа3) И СтрСравнить(Выборка.КодАльфа3, КодАльфа3) = 0 Тогда

			Сообщение.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'С кодом Альфа-3 %1 уже существует страна %2. Измените код Альфа-3 или используйте уже существующие данные.'"),
