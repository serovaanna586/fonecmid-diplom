...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФормаНастроитьСинхронизацию.Видимость = ПравоДоступа("Редактирование",
		Метаданные.Справочники.УчетныеЗаписиСинхронизацииФайлов);
	Элементы.Ответственный.Видимость = ТипЗнч(Пользователи.АвторизованныйПользователь()) <> Тип(
...
Процедура РабочийКаталогВладельцаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Обработчик           = Новый ОписаниеОповещения("РаботыСФайламиПодключеноРабочийКаталогВладельцаНачалоВыбораПродолжение",
		ЭтотОбъект);
...
Процедура РабочийКаталогВладельцаОчистка(Элемент, СтандартнаяОбработка)
	...
	РабочийКаталогПапкиУнаследованный = РабочийКаталогРодителя + Объект.Наименование + ПолучитьРазделительПути();
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Неверный путь или отсутствуют права на запись в папку ""%1""'"), ИмяКаталога);
...
Процедура ОбновитьДоступностьКомандПоНастройкеПрав()
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Или Элементы.Найти(
		"ФормаОбщаяКомандаНастроитьПрава") = Неопределено Тогда
		Возврат;
	КонецЕсли;

	МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");

	Если ЗначениеЗаполнено(Объект.Ссылка) И Не МодульУправлениеДоступом.ЕстьПраво("ИзменениеПапок", Объект.Ссылка) Тогда

		ТолькоПросмотр = Истина;
	КонецЕсли;

	УправлениеПравами = ЗначениеЗаполнено(Объект.Ссылка) И МодульУправлениеДоступом.ЕстьПраво("УправлениеПравами",
		Объект.Ссылка);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Элементы.ДекорацияПояснение.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				...
				АдресПапкиВОблачномСервисе), Строка(ВыборкаДетальныеЗаписи.УчетнаяЗаписьНаименование));
