...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ДобавленПользовательИБ" Или ИмяСобытия = "ИзмененПользовательИБ" Или ИмяСобытия = "УдаленПользовательИБ"
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ОтменитьСопоставлениеПродолжение", ЭтотОбъект), НСтр("ru = 'Отмена сопоставления пользователя информационной базы с пользователем в справочнике.
																					   |
...
Процедура ЗаполнитьПользователейИБ()
	...
	ТекущийИдентификаторПользователяИБ = ?(Строка = Неопределено, ПустойУникальныйИдентификатор,
	...
	Для Каждого ПользовательИБ Из ВсеПользователиИБ Цикл
		...
		Если ПоказатьТолькоОбработанныеВКонфигураторе И Строка <> Неопределено И Не ИзмененВКонфигураторе Тогда
...
Процедура ОткрытьПользователяПоСсылке()
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		ОткрытьФорму(
			?(ТекущиеДанные.СопоставленСВнешнимПользователем, "Справочник.ВнешниеПользователи.ФормаОбъекта",
			"Справочник.Пользователи.ФормаОбъекта"), Новый Структура("Ключ", ТекущиеДанные.Ссылка));
	КонецЕсли;
...
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("УдалитьТекущегоПользователяИБЗавершение", ЭтотОбъект, УдалитьСтроку), НСтр(
		"ru = 'Удалить пользователя информационной базы?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура СопоставитьПользователяИБ(СНовым = Ложь)
	...
	Если ТипыПользователей.Количество() > 1 Тогда
		ТипыПользователей.ПоказатьВыборЭлемента(
			Новый ОписаниеОповещения("СопоставитьПользователяИБДляТипаЭлемента", ЭтотОбъект, СНовым), НСтр(
			"ru = 'Выбор типа данных'"), ТипыПользователей[0]);
	Иначе
...
Процедура СопоставитьПользователяИБДляТипаЭлемента(ЭлементСписка, СНовым) Экспорт
	...
	ИмяСправочника = ?(ЭлементСписка.Значение = Тип("СправочникСсылка.Пользователи"), "Пользователи",
		"ВнешниеПользователи");

	Если Не СНовым Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("РежимВыбора", Истина);
		ПараметрыФормы.Вставить("ИдентификаторыНесуществующихПользователейИБ",
