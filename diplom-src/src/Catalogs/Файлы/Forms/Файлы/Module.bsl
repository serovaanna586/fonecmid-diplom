...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПапкиКонтекстноеМенюНастроитьСинхронизацию.Видимость = ПравоДоступа("Редактирование",
		Метаданные.Справочники.УчетныеЗаписиСинхронизацииФайлов);
	Элементы.Сравнить.Видимость = Не ОбщегоНазначения.ЭтоLinuxКлиент() И Не ОбщегоНазначения.ЭтоВебКлиент();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ПапкиФайлов" Тогда
		...
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "Запись_Файл" И ТипЗнч(Источник) <> Тип("Массив") Тогда
...
Процедура СохранитьИзменения(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.СохранитьИзмененияФайлаСОповещением(
		Обработчик, Элементы.Список.ТекущаяСтрока, УникальныйИдентификатор);
...
Процедура ПеретаскиваниеВПапку(ПапкаДляДобавления, ЗначениеПеретаскивания, Действие)
	...
	ИначеЕсли ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") Тогда
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.Файлы") Тогда
			...
			Если Действие = ДействиеПеретаскивания.Копирование Тогда
				...
				РаботаСФайламиСлужебныйВызовСервера.СкопироватьФайлы(
					ЗначениеПеретаскивания, ПапкаДляДобавления);
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					...
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файл ""%1""
							 |скопирован в папку ""%2""'"), ЗначениеПеретаскивания[0], Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ru = 'Файлы скопированы.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файлы (%1 шт.) скопированы в папку ""%2""'"), ЗначениеПеретаскивания.Количество(),
			...
			Иначе
				...
				ВладелецУстановлен = РаботаСФайламиСлужебныйВызовСервера.УстановитьВладельцаФайла(
					ЗначениеПеретаскивания, ПапкаДляДобавления);
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					...
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файл ""%1""
							 |перенесен в папку ""%2""'"), Строка(ЗначениеПеретаскивания[0]), Строка(
						ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ru = 'Файлы перенесены.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файлы (%1 шт.) перенесены в папку ""%2""'"), Строка(
						ЗначениеПеретаскивания.Количество()), Строка(ПапкаДляДобавления));
				КонецЕсли;
				ПоказатьОповещениеПользователя(ОповещениеЗаголовок, , ОповещениеТекст, БиблиотекаКартинок.Информация32);
			КонецЕсли;
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.ПапкиФайлов") Тогда
			...
			РодительСменен = РаботаСФайламиСлужебныйВызовСервера.СменитьРодителяПапок(ЗначениеПеретаскивания,
				ПапкаДляДобавления, НайденоЗацикливание);
			Если РодительСменен <> Истина Тогда
				Если НайденоЗацикливание = Истина Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
						ПапкаДляДобавления, ЗначениеПеретаскивания[0]);
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				...
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Папка ""%1""
						 |перенесена в папку ""%2""'"), Строка(ЗначениеПеретаскивания[0]), Строка(ПапкаДляДобавления));
			Иначе
				ОповещениеЗаголовок = НСтр("ru = 'Папки перенесены.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Папки (%1 шт.) перенесены в папку ""%2""'"), Строка(ЗначениеПеретаскивания.Количество()),
					Строка(ПапкаДляДобавления));
			КонецЕсли;
			ПоказатьОповещениеПользователя(ОповещениеЗаголовок, , ОповещениеТекст, БиблиотекаКартинок.Информация32);
		КонецЕсли;
...
Процедура УстановитьДоступностьКомандПриИзмененииПапки()
	...
	Иначе
		...
		Элементы.СписокКонтекстноеМенюУстановитьПометкуУдаления.Доступность = ДобавлениеФайлов
...
Функция НастройкиПравПапки(Папка)
	...
	НастройкиПрав.Вставить("ИзменениеПапок", МодульУправлениеДоступом.ЕстьПраво("ИзменениеПапок", Папка));

	НастройкиПрав.Вставить("ИзменениеФайлов", МодульУправлениеДоступом.ЕстьПраво("ИзменениеФайлов", Папка));

	НастройкиПрав.Вставить("ДобавлениеФайлов", МодульУправлениеДоступом.ЕстьПраво("ДобавлениеФайлов", Папка));

	НастройкиПрав.Вставить("ПометкаУдаленияФайлов", МодульУправлениеДоступом.ЕстьПраво("ПометкаУдаленияФайлов", Папка));
...
Процедура УстановитьДоступностьФайловыхКоманд(Результат = Неопределено, ПараметрыВыполнения = Неопределено) Экспорт
	...
	Если ТекущиеДанные <> Неопределено И ТипЗнч(Элементы.Список.ТекущаяСтрока) <> Тип(
...
Процедура УстановитьДоступностьКоманд(Знач ДанныеКоманд, Знач ВыделеноНесколькоСтрок)
	...
	Элементы.ФормаЗанять.Доступность                 = ИзменениеФайлов И Не ЗначениеЗаполнено(Редактирует)
		И Не ПодписанЭП И Не Служебный;
	Элементы.СписокКонтекстноеМенюЗанять.Доступность = ИзменениеФайлов И Не ЗначениеЗаполнено(Редактирует)
		И Не ПодписанЭП И Не Служебный;
	...
	Элементы.ФормаЗашифровать.Доступность                 = ИзменениеФайлов И Не ЗначениеЗаполнено(Редактирует)
		И Не Зашифрован И Не Служебный;
	Элементы.СписокКонтекстноеМенюЗашифровать.Доступность = ИзменениеФайлов И Не ЗначениеЗаполнено(Редактирует)
		И Не Зашифрован И Не Служебный;
	...
	Элементы.ФормаДобавитьПодписьИзФайла.Доступность                 = ИзменениеФайлов И Не ЗначениеЗаполнено(
		Редактирует);
	Элементы.СписокКонтекстноеМенюДобавитьПодписьИзФайла.Доступность = ИзменениеФайлов И Не ЗначениеЗаполнено(
		Редактирует);
	...
	Элементы.ФормаУдалить.Доступность = ИзменениеФайлов И АвторТекущийПользователь И Не ЗначениеЗаполнено(Редактирует)
		И Не ФайлыРедактируютсяВОблачномСервисе;

	Элементы.СписокКонтекстноеМенюУдалить.Доступность = ИзменениеФайлов И АвторТекущийПользователь
		И Не ЗначениеЗаполнено(Редактирует) И Не ФайлыРедактируютсяВОблачномСервисе;
...
Процедура ОбновитьИСохранитьПараметрыСпискаФайлов()
	...
	ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить(
		"Файлы", "ТекущаяПапка", Элементы.Папки.ТекущаяСтрока);
