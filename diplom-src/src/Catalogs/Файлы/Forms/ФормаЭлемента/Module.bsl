...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступнаПечатьСоШтампом = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись")
		И ЭтотОбъект.Объект.Расширение = "mxl" И ЭтотОбъект.Объект.ПодписанЭП;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НаименованиеДоЗаписи <> ТекущийОбъект.Наименование Тогда
		...
		Если ТекущийОбъект.ТекущаяВерсия.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			РаботаСФайламиВТомахСлужебный.ПереименоватьФайл(ТекущийОбъект.ТекущаяВерсия, ТекущийОбъект.Наименование,
				НаименованиеДоЗаписи, УникальныйИдентификатор);
		КонецЕсли;
...
Процедура ЗашифроватьПослеШифрованияНаКлиенте(Результат, ПараметрыВыполнения) Экспорт
	...
	РаботаСФайламиСлужебныйКлиент.ИнформироватьОШифровании(
		МассивФайловВРабочемКаталогеДляУдаления, ПараметрыВыполнения.ДанныеФайла.Владелец, ЭтотОбъект.Объект.Ссылка);
...
Процедура РасшифроватьПослеРасшифровкиНаКлиенте(Результат, ПараметрыВыполнения) Экспорт
	...
	РаботаСФайламиСлужебныйКлиент.ИнформироватьОРасшифровке(
		ПараметрыВыполнения.ДанныеФайла.Владелец, ЭтотОбъект.Объект.Ссылка);
...
Процедура Освободить(Команда)
	...
	Если Модифицированность Тогда
		ПоказатьВопрос(ОсвободитьЗавершение, НСтр(
			"ru = 'При отмене редактирования внесенные изменения будут утеряны. Продолжить?'"),
...
Процедура СохранитьИзменения(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.СохранитьИзмененияФайлаСОповещением(Обработчик, ЭтотОбъект.Объект.Ссылка,
...
Процедура ПечатьСоШтампом(Команда)
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Или Записать() Тогда
		ДокументСоШтампом = РаботаСФайламиСлужебныйВызовСервера.ТабличныйДокументСоШтампом(ЭтотОбъект.Объект.Ссылка,
...
Процедура Удалить(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.УдалитьДанные(
		Новый ОписаниеОповещения("ПослеУдаленияДанных", ЭтотОбъект), Объект.Ссылка, УникальныйИдентификатор);
...
Процедура ЗашифроватьСервер(МассивДанныхДляЗанесенияВБазу, МассивОтпечатков, МассивФайловВРабочемКаталогеДляУдаления,
	...
	РаботаСФайламиСлужебный.ЗаписатьИнформациюОШифровании(
		ЭтотОбъект.Объект.Ссылка, Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор,
