...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	УправлениеДоступомСлужебный.ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект,
		Артефакты, Отказ);
...
Процедура ОбновитьОписаниеПоставляемыхПрофилей(ЕстьИзменения = Неопределено) Экспорт
	...
	Попытка
		...
		СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы(
			"СтандартныеПодсистемы.УправлениеДоступом.ОписаниеПоставляемыхПрофилей", НовоеЗначение,
		...
		СтандартныеПодсистемыСервер.ДобавитьИзмененияПараметраРаботыПрограммы(
			...
			Новый ФиксированнаяСтруктура("ЕстьИзменения", Истина), Новый ФиксированнаяСтруктура));
...
Процедура ОбновитьСоставПредопределенныхПрофилей(ЕстьИзменения = Неопределено) Экспорт
	...
	Попытка
		...
		СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы(
			"СтандартныеПодсистемы.УправлениеДоступом.ПредопределенныеПрофилиГруппДоступа", ПредопределенныеПрофили, ,
			СтароеЗначение);
		...
		СтандартныеПодсистемыСервер.ДобавитьИзмененияПараметраРаботыПрограммы(
			"СтандартныеПодсистемы.УправлениеДоступом.ПредопределенныеПрофилиГруппДоступа", ?(ЗначениеЗаполнено(
			Удаленные), Новый ФиксированнаяСтруктура("Удаленные", Новый ФиксированныйМассив(Удаленные)),
	...
	Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
		...
		Если ПоследниеИзменения <> Неопределено Тогда
			Для Каждого ЧастьИзменений Из ПоследниеИзменения Цикл
				Если ТипЗнч(ЧастьИзменений) = Тип("ФиксированнаяСтруктура") И ЧастьИзменений.Свойство("Удаленные")
...
Процедура ОбновитьПоставляемыеПрофили(ЕстьИзменения = Неопределено, Удаленные = Неопределено) Экспорт
	...
	ОбновитьПоставляемыеПрофилиБезПапок(ОбновленныеПрофили, ТекущиеПрофили, ТекущиеПапкиПрофилей, ПоставляемыеПрофили,
		Удаленные, ЕстьИзменения);
...
Процедура ОбновитьВспомогательныеДанныеПрофилей(Профили = Неопределено, ЕстьИзменения = Ложь) Экспорт
	...
	ИначеЕсли Профили.Количество() > 0 Тогда
		...
		ПользователиДляОбновления = Справочники.ГруппыДоступа.ПользователиДляОбновленияРолейПоПрофилю(Профили);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Или МодульТекущиеДелаСервер.ДелоОтключено("ПрофилиГруппДоступа") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтандартныеРолиРасширений(РолиПрофиля, СтандартныеРолиРасширений = Неопределено) Экспорт
	...
	УстановитьРолиВПрофиле(РолиПрофиля, СтандартныеРолиРасширений.АдминистраторСистемы,
		СтандартныеРолиПрофиля.АдминистраторСистемы);
	
	// ПолныеПрава.
	УстановитьРолиВПрофиле(РолиПрофиля, СтандартныеРолиРасширений.ПолныеПрава, СтандартныеРолиПрофиля.ПолныеПрава);
	
	// БазовыеПраваБСП.
	УстановитьРолиВПрофиле(РолиПрофиля, СтандартныеРолиРасширений.БазовыеПрава, СтандартныеРолиПрофиля.БазовыеПраваБСП);
	
	// БазовыеПраваВнешнихПользователейБСП.
	УстановитьРолиВПрофиле(РолиПрофиля, СтандартныеРолиРасширений.БазовыеПраваВнешнихПользователей,
		СтандартныеРолиПрофиля.БазовыеПраваВнешнихПользователейБСП);
	
	// Общие права.
	УстановитьРолиВПрофиле(РолиПрофиля, СтандартныеРолиРасширений.ОбщиеПрава, СтандартныеРолиПрофиля.ПолныеПрава
		Или СтандартныеРолиПрофиля.БазовыеПраваБСП Или СтандартныеРолиПрофиля.БазовыеПраваВнешнихПользователейБСП);
...
Функция ПрофильАдминистратор() Экспорт
	...
	Если ВыборкаПоИдентификатору.Следующий() И ВыборкаПоПредопределенному.Следующий()
		И ВыборкаПоИдентификатору.Количество() = 1 И ВыборкаПоПредопределенному.Количество() = 1
...
//
Функция ПоставляемыйПрофильПоИдентификатору(Идентификатор, ВызыватьИсключениеЕслиНетВБазеДанных = Ложь,
	БезПапок = Ложь) Экспорт
	...
	Если СвойстваПрофиля = Неопределено Или СвойстваПрофиля.ЭтоГруппа И БезПапок Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Профиль c идентификатором ""%1""
				 |не поставляется в программе.'"), Строка(Идентификатор));
	...
	Если ВызыватьИсключениеЕслиНетВБазеДанных Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Поставляемый профиль с идентификатором ""%1""
				 |не существует в программе.'"), Строка(Идентификатор));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция ПоставляемыйПрофильИзменен(Профиль) Экспорт
	...
	Если ТипЗнч(Профиль) = Тип("СправочникСсылка.ПрофилиГруппДоступа") Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Профиль, "ПоставляемыйПрофильИзменен") = Истина;
	КонецЕсли;
	...
	Если Профиль.Роли.Количество() <> ОписаниеРолейПрофиля.Количество() Или Профиль.ВидыДоступа.Количество()
		<> СвойстваПрофиля.ВидыДоступа.Количество() Или Профиль.ЗначенияДоступа.Количество()
		<> СвойстваПрофиля.ЗначенияДоступа.Количество() Или Профиль.Назначение.Количество()
		<> СвойстваПрофиля.Назначение.Количество() Тогда
		Возврат Истина;
	КонецЕсли;
		...
		Если МетаданныеРоли = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В поставляемом профиле ""%1""
					 |указана несуществующая роль ""%2"".'"), СвойстваПрофиля.Наименование, Роль);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	...
	Для Каждого ОписаниеЗначенияДоступа Из СвойстваПрофиля.ЗначенияДоступа Цикл
		Отбор = Новый Структура;
		Запрос = Новый Запрос(СтрЗаменить("ВЫБРАТЬ Значение(ЗначениеДоступа) КАК Значение", "ЗначениеДоступа",
...
Функция ЗапретИзмененияПрофиля(Знач Профиль, РодительТолькоПросмотр = Ложь) Экспорт
	...
	Если Профиль.ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор(ИдентификаторПрофиляАдминистратор()) Тогда
	...
	ЗапретИзменения = СвойстваПрофиля <> Неопределено И (ПоставляемыеПрофили.ПараметрыОбновления.ЗапретитьИзменениеПрофилей
...
Функция НесовместимыеПрофилиГруппДоступа() Экспорт
	...
	Для Каждого ОписаниеПрофиля Из Выгрузка Цикл
		...
		Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
			...
			Если ОбъектМетаданных = Null Или НедоступныеРоли.Получить(ОбъектМетаданных.Имя) <> Неопределено Или ВРег(
				Лев(ОбъектМетаданных.Имя, СтрДлина("Удалить"))) = ВРег("Удалить") Тогда
...
Функция ИзменилисьВидыИлиЗначенияДоступаИлиНазначение(СтарыеЗначения, ТекущийОбъект) Экспорт
	...
	Если ВидыДоступа.Количество() <> ТекущийОбъект.ВидыДоступа.Количество() Или ЗначенияДоступа.Количество()
		<> ТекущийОбъект.ЗначенияДоступа.Количество() Или Назначение.Количество()
		<> ТекущийОбъект.Назначение.Количество() Тогда
...
Процедура ПриИзмененииЯзыкаИнформационнойБазы(ИзменениеЯзыков) Экспорт
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Если Не ЗначениеЗаполнено(Наименование) Или Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(
			ОписаниеПрофиля.Идентификатор) Тогда
			Продолжить;
		КонецЕсли;
		НоваяСтрока = НовыеНаименованияПрофилей.Добавить();
		НоваяСтрока.ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор(ОписаниеПрофиля.Идентификатор);
...
Процедура ВосстановитьНесуществующиеВидыИЗначенияДоступа(СтарыеЗначения, НовыеЗначения) Экспорт
	...
	Для Каждого Строка Из СтарыеВидыДоступа Цикл
		Если Строка.ВидДоступа = Неопределено Или ПоСсылкам.Получить(Строка.ВидДоступа) <> Неопределено
	...
	Для Каждого Строка Из СтарыеЗначенияДоступа Цикл
		Если Строка.ВидДоступа = Неопределено Или Строка.ЗначениеДоступа = Неопределено
			Или НовыеЗначения.ВидыДоступа.Найти(Строка.ВидДоступа, "ВидДоступа") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СвойстваВидаДоступа <> Неопределено И СвойстваВидаДоступа.Ссылка = Строка.ВидДоступа Тогда
			Продолжить;
		КонецЕсли;
		Отбор = Новый Структура("ВидДоступа, ЗначениеДоступа", Строка.ВидДоступа, Строка.ЗначениеДоступа);
...
Функция СтандартныеРолиПрофиля(РолиПрофиля)
	...
	Результат.Вставить("АдминистраторСистемы", ЕстьРольВПрофиле(РолиПрофиля, Метаданные.Роли.АдминистраторСистемы));

	Результат.Вставить("ПолныеПрава", ЕстьРольВПрофиле(РолиПрофиля, Метаданные.Роли.ПолныеПрава));

	Результат.Вставить("БазовыеПраваБСП", ЕстьРольВПрофиле(РолиПрофиля, Метаданные.Роли.БазовыеПраваБСП));

	Результат.Вставить("БазовыеПраваВнешнихПользователейБСП", ЕстьРольВПрофиле(РолиПрофиля,
		Метаданные.Роли.БазовыеПраваВнешнихПользователейБСП));
...
Процедура ОчиститьУдаленныеСтандартныеРолиРасширений(РолиПрофиля)
	...
	Если ТипЗнч(РолиПрофиля) = Тип("Массив") Или ТипЗнч(РолиПрофиля) = Тип("ДанныеФормыКоллекция") Тогда
		Возврат;
	КонецЕсли;
	...
	ПолныеИменаУдаленныхИОтключенныхРолей = Справочники.ИдентификаторыОбъектовМетаданных.ПолныеИменаОбъектовМетаданныхВключаяУдаленные(
	...
	Для Каждого ОписаниеРоли Из ПолныеИменаУдаленныхИОтключенныхРолей Цикл
		...
		Если ЧастиИмени.Количество() <> 2 Или ВРег(ЧастиИмени[0]) <> ВРег("Роль") Тогда
			Продолжить;
		КонецЕсли;
		ИмяРоли = ЧастиИмени[1];
		Если Не СтрЗаканчиваетсяНа(ВРег(ИмяРоли), ВРег("ОбщиеПрава")) И Не СтрЗаканчиваетсяНа(ВРег(ИмяРоли), ВРег(
			"ПолныеПрава")) И Не СтрЗаканчиваетсяНа(ВРег(ИмяРоли), ВРег("БазовыеПрава")) И Не СтрЗаканчиваетсяНа(ВРег(
			ИмяРоли), ВРег("БазовыеПраваВнешнихПользователей")) И Не СтрЗаканчиваетсяНа(ВРег(ИмяРоли), ВРег(
			"АдминистраторСистемы")) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗарегистрироватьПрофильИзмененныйПриЗагрузке(ЭлементДанных) Экспорт
	...
	ИначеЕсли СтарыеЗначения.Ссылка <> ЭлементДанных.Ссылка Тогда
		...
		ИзмененныеРоли = ?(ЭлементДанных.ПометкаУдаления, Новый Массив, ЭлементДанных.Роли.ВыгрузитьКолонку("Роль"));

	ИначеЕсли СтарыеЗначения.ПометкаУдаления <> ЭлементДанных.ПометкаУдаления Тогда
		ТребуетсяРегистрация = Истина;
		ИзмененныеРоли = ?(ЭлементДанных.ПометкаУдаления, СтарыеЗначения.Роли.Выгрузить().ВыгрузитьКолонку("Роль"),
...
Процедура ОбновитьВспомогательныеДанныеПрофилейИзмененныхПриЗагрузке() Экспорт
	...
	Если ИзмененныеПрофили.Количество() = 1 И ИзмененныеПрофили[0] = Неопределено Тогда
	...
	Если ИзмененныеРоли.Количество() = 1 И ИзмененныеРоли[0] = Неопределено Тогда
...
Процедура ЗаполнитьПрофильАдминистратор(ОписаниеПрофиляАдминистратор, КромеОписания = Ложь)
	...
	Иначе
		Описание = НСтр("ru = 'Предназначен для:
	...
	ЗаполнитьЗначенияСвойств(ОписаниеПрофиляАдминистратор, УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа());
...
Функция ПроверенныеПоставляемыеПрофилиСеанса(СвойстваВидовДоступа = Неопределено) Экспорт
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Заданы недопустимые значения в процедуре %1
			 |общего модуля %2.'"), "ПриЗаполненииПоставляемыхПрофилейГруппДоступа",
		"УправлениеДоступомПереопределяемый") + Символы.ПС + Символы.ПС;

	Если ПараметрыОбновления.ЗапретитьИзменениеПрофилей И Не ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
		...
		ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Когда в параметре %1 свойство
				 ...
				 |должно быть установлено Ложь.'"), "ПараметрыОбновления", "ОбновлятьИзмененныеПрофили",
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Если Не ЗначениеЗаполнено(ОписаниеПрофиля.Идентификатор) Тогда
			ШаблонОшибки = ?(ЭтоГруппа, НСтр("ru = 'В описании папки профилей ""%1"" не заполнено свойство %2.'"), НСтр(
				"ru = 'В описании профиля ""%1"" не заполнено свойство %2.'"));
		...
		ИначеЕсли Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ОписаниеПрофиля.Идентификатор) Тогда
			ШаблонОшибки = ?(ЭтоГруппа, НСтр(
				"ru = 'В описании папки профилей ""%1"" указан некорректный идентификатор: ""%2"".'"), НСтр(
				"ru = 'В описании профиля ""%1"" указан некорректный идентификатор: ""%2"".'"));
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ?(
				ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование),
		...
		Если ВсеИдентификаторы.Получить(ВРег(СвойстваПрофиля.Идентификатор)) <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'Значение ""%1"" свойства %2 уже используется в описании другого профиля или папки профилей.'"),
		...
		Если ЭтоГруппа И Не ЗначениеЗаполнено(СвойстваПрофиля.Имя) Тогда
			ТекстОшибки =  ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании папки профилей ""%1"" не заполнено свойство %2.'"), ?(ЗначениеЗаполнено(
				ОписаниеПрофиля.Наименование), ОписаниеПрофиля.Наименование, ОписаниеПрофиля.Идентификатор), "Имя");
			ВызватьИсключение ТекстОшибки;
		...
		ПодготовитьРолиПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, НазначениеПрофиля, ВсеРоли,
			ЗаголовокОшибки);

		ПодготовитьВидыДоступаПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, НазначениеПрофиля,
			СвойстваВидовДоступа, ЗаголовокОшибки);
		ПодготовитьЗначенияДоступаПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, СвойстваВидовДоступа,
			ЗаголовокОшибки);
	КонецЦикла;
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		КонецЕсли;
		ЭтоГруппа = Не ОписаниеПрофиля.Свойство("Роли");
		ШаблонОшибки = ?(ЭтоГруппа, НСтр("ru = 'В описании папки профилей ""%1"" в свойстве %2
										 |указано несуществующее имя папки профилей ""%3"".'"), НСтр("ru = 'В описании профиля ""%1"" в свойстве %2
																									 |указано несуществующее имя папки профилей ""%3"".'"));
		ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ?(
			ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование), "Родитель",
		...
		Пока Истина Цикл
			...
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании папки профилей ""%1"" в свойстве %2
					 ...
					 |создающее циклическую зависимость: ""%4"".'"), ?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
				ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование), "Родитель", ОписаниеПрофиля.Родитель, СтрСоединить(
				РодителиПапки, " -> "));
			ВызватьИсключение ТекстОшибки;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПодготовитьНазначениеПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, ЗаголовокОшибки)
	...
	Для Каждого Тип Из ОписаниеПрофиля.Назначение Цикл
		...
		Для Каждого Тип Из Типы Цикл
			Если ТипЗнч(Тип) <> Тип("Тип") Или Не ОбщегоНазначения.ЭтоСсылка(Тип)
				Или Не Метаданные.ОпределяемыеТипы.Пользователь.Тип.СодержитТип(Тип) Или Тип <> Тип(
				"СправочникСсылка.Пользователи") И Не Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.СодержитТип(
				Тип) Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В описании профиля ""%1""
						 ...
						 |(кроме типа %7).'"), ?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя,
					ОписаниеПрофиля.Идентификатор), Строка(Тип), Строка(ТипЗнч(Тип)), "Тип", "Пользователь",
					"ВнешнийПользователь", "СправочникСсылка.Пользователи");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
...
// Для функции ПоставляемыеПрофили.
Процедура ПодготовитьРолиПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, НазначениеПрофиля, ВсеРоли,
	ЗаголовокОшибки)
		...
		Если ВсеРоли.Получить(Роль) = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1 (%2)""
					 |указана несуществующая роль ""%3"".'"), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Идентификатор, Роль);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		Если НедоступныеРоли.Получить(Роль) <> Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1 (%2)""
					 ...
					 |""%4"".'"), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Идентификатор, Роль,
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		СвойстваПрофиля.Вставить("РолиНедоступныеВСервисе", РолиПрофиляНедоступныеВСервисе(ОписаниеПрофиля,
			НазначениеПрофиля));
	КонецЕсли;
...
// Для функции ПоставляемыеПрофили.
Процедура ПодготовитьВидыДоступаПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, НазначениеПрофиля,
	СвойстваВидовДоступа, ЗаголовокОшибки)
	...
	Для Каждого ЭлементСписка Из ОписаниеПрофиля.ВидыДоступа Цикл
		...
		Если СвойстваВидовДоступа.ПоИменам.Получить(ИмяВидаДоступа) = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1""
					 |указан несуществующий вид доступа ""%2"".'"), ?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
				ОписаниеПрофиля.Имя, ОписаниеПрофиля.Идентификатор), ИмяВидаДоступа);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;

		ВидДоступаСоответствуетНазначениюПрофиля = УправлениеДоступомСлужебныйКлиентСервер.ВидДоступаСоответствуетНазначениюПрофиля(
		...
		Если Не ВидДоступаСоответствуетНазначениюПрофиля Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1""
					 ...
					 |""%3"".'"), ?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя,
				ОписаниеПрофиля.Идентификатор), ИмяВидаДоступа, ПредставлениеНазначенияПрофиля(НазначениеПрофиля));
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		Если УточнениеВидаДоступа <> "" И УточнениеВидаДоступа <> "ВначалеВсеЗапрещены" И УточнениеВидаДоступа <> "Предустановленный"
			...
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1""
					 ...
					 |- ""%4"" или """",
					 |- ""%5"",
					 |- ""%6"".'"), ?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя,
				ОписаниеПрофиля.Идентификатор), ИмяВидаДоступа, УточнениеВидаДоступа, "ВначалеВсеЗапрещены",
...
// Для функции ПоставляемыеПрофили.
Процедура ПодготовитьЗначенияДоступаПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля, СвойстваВидовДоступа,
	ЗаголовокОшибки)
	;
	...
	Для Каждого ЭлементСписка Из ОписаниеПрофиля.ЗначенияДоступа Цикл
		...
		Если СвойстваВидаДоступа = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1""
					 ...
					 |""%3"".'"), ?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя,
				ОписаниеПрофиля.Идентификатор), ВидДоступа, ЗначениеДоступа);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		Если СвойстваВидаДоступаПоТипу = Неопределено Или СвойстваВидаДоступаПоТипу.Имя <> ВидДоступа Тогда

			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1""
					 |указано значение доступа ""%3""
					 |типа, который не указан в свойствах вида доступа ""%2"".'"), ?(ЗначениеЗаполнено(
				ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Идентификатор), ВидДоступа, ЗначениеДоступа);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция ПредопределенныеПрофилиСовпадают(НовыеПрофили, СтарыеПрофили, Удаленные)
	...
	ПредопределенныеПрофилиСовпадают = НовыеПрофили.Количество() = СтарыеПрофили.Количество();
...
Процедура ОбновитьПоставляемыеПапкиПрофилей(Родитель, ТекущиеПапкиПрофилей, ПоставляемыеПрофили, Удаленные,
	ЕстьИзменения)
	...
	Для Каждого КлючИЗначение Из ПапкиРодителя Цикл
		...
		ОбновитьПоставляемыеПапкиПрофилей(КлючИЗначение.Ключ, ТекущиеПапкиПрофилей, ПоставляемыеПрофили, Удаленные,
			ЕстьИзменения);
	КонецЦикла;
...
// Для процедуры ОбновитьПоставляемыеПрофили.
Процедура ПометитьНаУдалениеУстаревшиеПоставляемыеДанные(ПапкиИлиПрофили, ЕстьИзменения,
	ОбновленныеПрофили = Неопределено)
	...
	Для Каждого СтрокаТекущегоПрофиля Из ПапкиИлиПрофили Цикл
		Если СтрокаТекущегоПрофиля.Найден Тогда
			Продолжить;
		КонецЕсли;
		Если Не СтрокаТекущегоПрофиля.ЭтоГруппа И СтрокаТекущегоПрофиля.ПоставляемыйПрофильИзменен Тогда
			Продолжить;
		КонецЕсли;
...
// Для процедуры ОбновитьПоставляемыеПрофили.
Процедура ОбновитьПоставляемыеПрофилиБезПапок(ОбновленныеПрофили, ТекущиеПрофили, ТекущиеПапкиПрофилей,
	ПоставляемыеПрофили, Удаленные, ЕстьИзменения)
	...
	Для Каждого СвойстваПрофиля Из ОписанияПрофилей Цикл
		...
		Иначе
			...
			Если Не СтрокаТекущегоПрофиля.ПоставляемыйПрофильИзменен Или ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
...
Функция ОбновитьПрофильИлиПапкуПрофилей(СвойстваПрофиля, Удаленные = Неопределено, НеОбновлятьРолиПользователей = Ложь)
	...
	ПрофильИзменен = Ложь;
	УдаленПредопределенныйЭлементПапкиИлиПрофиля = ТипЗнч(Удаленные) = Тип("Массив") И Удаленные.Найти(
		СвойстваПрофиля.Имя) <> Неопределено;
	...
	Если ПрофильСсылка = Неопределено Тогда
		...
		Если ПрофильСсылка = Неопределено Тогда
			...
			Запрос.УстановитьПараметр("ЭтоГруппа", СвойстваПрофиля.ЭтоГруппа);
			Запрос.УстановитьПараметр("Ссылка", ПолучитьСсылку(
				Новый УникальныйИдентификатор(СвойстваПрофиля.Идентификатор)));
		...
		Если ПрофильСсылка = Неопределено И СвойстваПрофиля.ЭтоГруппа И ЗначениеЗаполнено(СвойстваПрофиля.Наименование) Тогда
		...
		ПрофильОбъект.ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор(СвойстваПрофиля.Идентификатор);

		ПрофильИзменен = Истина;
	Иначе
		ПрофильОбъект = ПрофильСсылка.ПолучитьОбъект();
		ПрофильИзменен = ПоставляемыйПрофильИзменен(ПрофильОбъект) Или УдаленПредопределенныйЭлементПапкиИлиПрофиля;
	КонецЕсли;
	...
	Если ПрофильИзменен Тогда
		...
		Если Не СвойстваПрофиля.ЭтоГруппа Тогда
				...
				Если МетаданныеРоли = Неопределено Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При обновлении поставляемого профиля ""%1""
							 |обнаружена несуществующая роль ""%2"".'"), СвойстваПрофиля.Наименование, Роль);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
				ПрофильОбъект.Роли.Добавить().Роль = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеРоли);
			КонецЦикла;
			...
			Для Каждого ОписаниеЗначенияДоступа Из СвойстваПрофиля.ЗначенияДоступа Цикл
				СвойстваВидаДоступа = УправлениеДоступомСлужебный.СвойстваВидаДоступа(
					ОписаниеЗначенияДоступа.ВидДоступа);
		...
		Если Не Справочники.ВерсииРасширений.ВсеРасширенияПодключены() Тогда
			СтарыеЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрофильОбъект.Ссылка,
				"ВидыДоступа, ЗначенияДоступа");
			Справочники.ПрофилиГруппДоступа.ВосстановитьНесуществующиеВидыИЗначенияДоступа(СтарыеЗначения,
				ПрофильОбъект);
		КонецЕсли;
