...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Объект.Сервис) Тогда
		Если Объект.Сервис = "https://webdav.yandex.com" Или Объект.Сервис = "https://webdav.yandex.ru" Тогда
			Сервис = НСтр("ru = 'Яндекс.Диск'");
		ИначеЕсли Объект.Сервис = "https://webdav.4shared.com" Тогда
			Сервис = "4shared.com";
		ИначеЕсли
		Объект.Сервис = "https://dav.box.com/dav" Тогда
			Сервис = "Box";
		ИначеЕсли
		Объект.Сервис = "https://dav.dropdav.com" Тогда
			Сервис = "Dropbox";
		КонецЕсли
		;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Объект.Наименование) Тогда
		Элементы.ВКачествеАвтораФайлов.СписокВыбора[0].Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Элементы.ВКачествеАвтораФайлов.Заголовок, "(" + Объект.Наименование + ")");
	КонецЕсли;
	...
	Если АвтоНаименование Тогда
		Если ЗначениеЗаполнено(Объект.Сервис) Тогда
			Объект.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Синхронизация с %1'"), Элементы.Сервис.СписокВыбора.НайтиПоЗначению(Сервис).Представление);
		Иначе
			Объект.Наименование = "";
		КонецЕсли;
	КонецЕсли;
	...
	Если СтруктураРезультата.Отказ Тогда
		...
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректные параметры для синхронизации файлов.
				 ...
				 |%4'"), Сервис, СтруктураРезультата.КодОшибки, ТекстОшибки, ТекстПротокола));
	Иначе
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПроверитьСоединение(Сервис, ТекстПротокола)
	...
	Иначе
		...
		Возврат Новый Структура("ОписаниеОшибки, ЖурналДиагностики", НСтр(
			"ru = 'Проверьте соединение с сетью Интернет.'"), ТекстПротокола);
