...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если МодульУправлениеКонтактнойИнформацией.СодержитКонтактнуюИнформацию(Объект.ОбъектАвторизации) Тогда
			...
			МодульУправлениеКонтактнойИнформацией.ПриСозданииНаСервере(ЭтотОбъект,
				ЭтотОбъект[ИмяРеквизитаОбъектАвторизации]);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если МодульУправлениеКонтактнойИнформацией.СодержитКонтактнуюИнформацию(Объект.ОбъектАвторизации) Тогда
			ИмяРеквизитаОбъектАвторизации = ИмяРеквизитаОбъектАвторизации(ТипЗнч(Объект.ОбъектАвторизации));
			МодульУправлениеКонтактнойИнформацией.ПриЧтенииНаСервере(ЭтотОбъект,
				ЭтотОбъект[ИмяРеквизитаОбъектАвторизации]);
			...
			Если ТипЗнч(УровеньДоступа) = Тип("Структура")
				...
				ИмяРеквизитаСПочтойДляВосстановленияПароля = МодульУправлениеКонтактнойИнформацией.ОпределитьЭлементСПочтойДляВосстановленияПароля(
					ЭтотОбъект, ПользовательИБАдресЭлектроннойПочты,
					ПользователиСлужебный.ДоступноРедактированиеПочтыДляВосстановленияПароля(УровеньДоступа,
					Объект.ОбъектАвторизации), Истина);
			КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ДействияВФорме.Роли = "Редактирование" И Объект.УстановитьРолиНепосредственно
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		ИмяРеквизитаОбъектАвторизации = ИмяРеквизитаОбъектАвторизации(ТипЗнч(Объект.ОбъектАвторизации));
		МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
		МодульУправлениеКонтактнойИнформацией.ПередЗаписьюНаСервере(ЭтотОбъект,
			ЭтотОбъект[ИмяРеквизитаОбъектАвторизации]);
	КонецЕсли;
	// Конец СтандартныеПодсистемы.КонтактнаяИнформация
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ГруппаНовогоВнешнегоПользователя) Тогда
		...
		Оповестить(
			"Запись_ГруппыВнешнихПользователей", Новый Структура, ГруппаНовогоВнешнегоПользователя);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Роль ""%1"" недоступна для внешних пользователей.'"), Строка.Синоним), "Роли",
					ЭлементыДерева.Индекс(Строка), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Роль ""%1"" в строке %2 недоступна для внешних пользователей.'"), Строка.Синоним, "%1"));
			КонецЕсли;
		КонецЦикла;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если МодульУправлениеКонтактнойИнформацией.СодержитКонтактнуюИнформацию(Объект.ОбъектАвторизации) Тогда
			ИмяРеквизитаОбъектАвторизации = ИмяРеквизитаОбъектАвторизации(ТипЗнч(Объект.ОбъектАвторизации));
			ОбъектАвторизацииОбъект = РеквизитФормыВЗначение(ИмяРеквизитаОбъектАвторизации);
			МодульУправлениеКонтактнойИнформацией.ОбработкаПроверкиЗаполненияНаСервере(ЭтотОбъект,
				ОбъектАвторизацииОбъект, Отказ);
		КонецЕсли;
	КонецЕсли;
	// Конец СтандартныеПодсистемы.КонтактнаяИнформация
...
Процедура НедействителенПриИзменении(Элемент)
	...
	Если Объект.Недействителен Тогда
		ВходВПрограммуРазрешен = Ложь;
		Если Не ПользовательИБАутентификацияOpenID И Не ПользовательИБАутентификацияСтандартнаяЗначениеПрямогоИзменения
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	...
	Если Не ВходВПрограммуРазрешен И Не ПользовательИБАутентификацияOpenID И Не ПользовательИБАутентификацияСтандартнаяЗначениеПрямогоИзменения
		И ПользовательИБАутентификацияСтандартная Тогда
...
Функция ОбъектАвторизации(ИмяРеквизитаОбъектАвторизации)
	...
	Если ТипЗнч(Объект[ИмяРеквизитаОбъектАвторизации]) = Тип("Строка") Или Не ЗначениеЗаполнено(
		Объект[ИмяРеквизитаОбъектАвторизации]) Тогда
...
Процедура ОбработатьИнтерфейсРолейУстановитьТолькоПросмотрРолей(ТекущийОбъект = Неопределено)
	...
	ОбработатьИнтерфейсРолей("УстановитьТолькоПросмотрРолей", ЗапретРедактированияРолей Или ДействияВФорме.Роли <> "Редактирование"
		Или Не УровеньДоступа.НастройкиДляВхода Или Не ТекущийОбъект.УстановитьРолиНепосредственно);
...
Процедура НастроитьФорму(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.УстановитьРолиНепосредственно.Видимость = ЗначениеЗаполнено(ДействияВФорме.Роли)
...
Процедура АутентификацияПриИзменении()
	...
	Если Не ПользовательИБАутентификацияOpenID И Не ПользовательИБАутентификацияСтандартная Тогда
...
Функция ОписаниеПользователяИБ()
	...
	Если УровеньДоступа.НастройкиДляВхода И Не ПользователиСлужебный.ЗапретРедактированияРолей()
...
Процедура ОбновитьОтображаемыйТипПользователя()
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект.ОбъектАвторизации)) Тогда
		Элементы.ОбъектАвторизации.Заголовок = Метаданные.НайтиПоТипу(ТипЗнч(
			Объект.ОбъектАвторизации)).ПредставлениеОбъекта;
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ЗначениеЗаполнено(УточнениеСвойств) Тогда
			ПоказатьКомандыУстраненияРазличий = УровеньДоступа.НастройкиДляВхода
				И ДействияВФорме.СвойстваПользователяИБ = "Редактирование";
		КонецЕсли;
		...
		Если УточнениеСвойств.Количество() > 0 Тогда
			...
			КонецЕсли;
			Если ПоказатьКомандыУстраненияРазличий Или ЕстьРазличияУстранимыеБезАдминистратора
	...
	ЕстьНовоеСопоставлениеСНесуществующимПользователемИБ = Не ПользовательИБСуществует И ЗначениеЗаполнено(
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.ОбъектАвторизации.ТолькоПросмотр
		=   ДействияВФорме.СвойстваЭлемента <> "Редактирование" Или Форма.ОбъектАвторизацииЗаданПриОткрытии
		Или ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.ОбъектАвторизации);

	Элементы.Недействителен.ТолькоПросмотр = Не (ДействияВФорме.СвойстваЭлемента = "Редактирование"
...
Функция ТребуетсяЗаписьПользователяИБ(Форма, УчитыватьСтандартноеИмя = Истина)
	...
	Если Форма.ПользовательИБСуществует Или Форма.ВходВПрограммуРазрешен
		...
		Или Форма.ПользовательИБПароль <> Неопределено Или Форма.ПользовательИБАутентификацияOpenID <> Шаблон.АутентификацияOpenID
