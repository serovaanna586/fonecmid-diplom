...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПеренестиВсеФайлыВТома.Видимость = ОбщегоНазначения.ПодсистемаСуществует(
...
Процедура НачатьИзменениеПометкиУдаления(ТекущиеДанные)
	...
	ПоказатьВопрос(
		...
		Новый ФорматированнаяСтрока(СодержимоеВопроса), РежимДиалогаВопрос.ДаНет);
...
Процедура ПродолжитьИзменениеПометкиУдаления(Ответ, ТекущиеДанные) Экспорт
	...
	ОповещениеОПродолжении = Новый ОписаниеОповещения("ПродолжитьУстановкуСнятиеПометкиУдаления", ЭтотОбъект,
