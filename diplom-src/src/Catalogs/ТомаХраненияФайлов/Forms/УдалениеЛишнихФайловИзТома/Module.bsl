...
Процедура ПутьКПапкеДляКопированияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещенияДиалогаВыбора = Новый ОписаниеОповещения("ПутьКПапкеДляКопированияНачалоВыбораЗавершение",
		ЭтотОбъект, Контекст);
...
Процедура ПутьКПапкеДляКопированияНачалоВыбораЗавершение(ВыбранныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		ПутьКПапкеДляКопирования = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(
...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(
		Новый ОписаниеОповещения("ПодключениеРасширенияРаботыСФайламиЗавершение", ЭтотОбъект), , Ложь);
...
Процедура ПодключениеРасширенияРаботыСФайламиЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Не установлено расширение для работы с 1С:Предприятием. Действие не доступно.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		ПапкаДляКопирования.НачатьПроверкуСуществования(
			Новый ОписаниеОповещения("ПроверкаСуществованияКаталогаЗавершение", ЭтотОбъект));
	КонецЕсли;
...
Процедура ПослеПроверкиЗаписиВКаталог(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьОбработкуОповещения(Новый ОписаниеОповещения("ОбработатьОчереднойФайл", ЭтотОбъект,
		Новый Структура("ИтоговоеОповещение, ТекущийФайл", ИтоговоеОповещение, Неопределено),
		"ОбработатьОчереднойФайлОшибка", ЭтотОбъект));
...
Процедура ПроверкаСуществованияФайлаЗавершение(ФайлСуществует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		КаталогТекущегоДня.НачатьПроверкуСуществования(Новый ОписаниеОповещения("КаталогДняСуществуетЗавершение",
...
Процедура КаталогДняСуществуетЗавершение(КаталогСуществует, ДополнительныеПараметры) Экспорт
	...
	Если Не КаталогСуществует Тогда
		НачатьСозданиеКаталога(Новый ОписаниеОповещения("СоздатьКаталогДняЗавершение", ЭтотОбъект,
			ДополнительныеПараметры), ДополнительныеПараметры.КаталогДляКопирования);
	Иначе
		...
		Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ПроверкаСуществованияКонечногоФайлаЗавершение",
			ЭтотОбъект, ДополнительныеПараметры));
	КонецЕсли;
...
Процедура ПроверкаСуществованияКонечногоФайлаЗавершение(ФайлСуществует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		КонечноеИмяФайла    = КаталогДляКопирования + ИмяБезРасширения + "_" + Строка(Новый УникальныйИдентификатор)
			+ "." + Расширение;
	КонецЕсли;

	НачатьПеремещениеФайла(Новый ОписаниеОповещения("ОбработатьПеремещениеОчередногоФайлаЗавершение", ЭтотОбъект,
		ДополнительныеПараметры, "ОбработатьОчереднойФайлОшибка", ЭтотОбъект), ПолноеИмяТекущегоФайла, КонечноеИмяФайла);
...
Процедура ОбработатьОчереднойФайлЗавершение(ДополнительныеПараметры)
	...
	Иначе
		ВыполнитьОбработкуОповещения(Новый ОписаниеОповещения("ОбработатьОчереднойФайл", ЭтотОбъект,
			Новый Структура("ИтоговоеОповещение, ТекущийФайл", ИтоговоеОповещение, ТекущийФайл),
			"ОбработатьОчереднойФайлОшибка", ЭтотОбъект));
	КонецЕсли;
...
Процедура ПослеОбработкиФайлов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЧислоУдаленныхФайлов <> 0 Тогда
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удалено файлов: %1'"), ЧислоУдаленныхФайлов);
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Завершено удаление лишних файлов.'"), , ТекстОповещения, БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура ПравоЗаписиВКаталог(ИсходноеОповещение)
	...
	ОповещениеСозданияКаталога = Новый ОписаниеОповещения("ПослеСозданияКаталога", ЭтотОбъект,
		ПараметрыУдаленияКаталога, "ПослеОшибкиСозданияКаталога", ЭтотОбъект);
...
Процедура ПослеСозданияКаталога(Результат, ДополнительныеПараметры) Экспорт
	...
	НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПослеУдаленияКаталога", ЭтотОбъект, ДополнительныеПараметры,
		"ПослеОшибкиУдаленияКаталога", ЭтотОбъект), ДополнительныеПараметры.ИмяКаталога);
...
Процедура ОбработатьОшибкуПравДоступа(ИнформацияОбОшибке, ИсходноеОповещение)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, КраткоеПредставлениеОшибки(
		ИнформацияОбОшибке));
