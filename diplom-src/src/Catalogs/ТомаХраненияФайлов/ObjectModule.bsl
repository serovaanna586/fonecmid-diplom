...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ДополнительныеСвойства.Свойство("ПропуститьОсновнуюПроверкуЗаполнения") Тогда
		...
		ПроверятьУникальностьПутей = ПутиКТомам.ПолныйПутьLinux <> ПолныйПутьLinux Или ПутиКТомам.ПолныйПутьWindows
			<> ПолныйПутьWindows;
		Если ПроверятьУникальностьПутей Тогда
			ПроверитьУникальностьПутиКТомам(Отказ);
		КонецЕсли;

		Если Не ИспользуютсяПрофилиБезопасности И Не ПустаяСтрока(ПолныйПутьWindows) И (Лев(ПолныйПутьWindows, 2) <> "\\"
	...
	Если Не ДополнительныеСвойства.Свойство("ПропуститьПроверкуДоступаКПапке") Тогда
		...
		Исключение
			...
			Если ИспользуютсяПрофилиБезопасности Тогда
				ШаблонОшибки = НСтр("ru = 'Некорректный путь к тому или сервер с томом в данный момент недоступен.
