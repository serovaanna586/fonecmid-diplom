...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		ИмяОбъектаСопоставления = Обработки.ЗагрузкаДанныхИзФайла.ПолноеИмяОбъектаТабличнаяЧасть(
			Параметры.ПолноеИмяТабличнойЧасти);

		ТаблицаИнформацияПоКолонкам = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЗагрузкаДанныхИзФайла",
			ИмяОбъектаСопоставления, , ИмяПользователя()); // ТаблицаЗначений
		...
		ПараметрыЗагрузкиДанных.Макет = ?(ЗначениеЗаполнено(Параметры.ИмяМакетаСШаблоном),
			Параметры.ИмяМакетаСШаблоном, "ЗагрузкаИзФайла");
		...
		Если Параметры.Свойство("КолонкиМакета") И Параметры.КолонкиМакета <> Неопределено Тогда
			...
			ЗагрузкаДанныхИзФайла.ДобавитьСтатистическуюИнформацию(
				"РежимЗапуска.ЗагрузкаВТабличнуюЧасть.ДинамическийМакет", , Параметры.ПолноеИмяТабличнойЧасти);
		Иначе
			Обработки.ЗагрузкаДанныхИзФайла.ОпределитьИнформацияПоКолонкам(ПараметрыЗагрузкиДанных,
				ТаблицаИнформацияПоКолонкам);
			ИзменитьБланкПоИнформацииПоКолонкам();
			ЗагрузкаДанныхИзФайла.ДобавитьСтатистическуюИнформацию(
				"РежимЗапуска.ЗагрузкаВТабличнуюЧасть.СтатическийМакет", , Параметры.ПолноеИмяТабличнойЧасти);
			Если Отказ Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура СписокКолонокСопоставленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СопоставитьПоКолонке.Количество() = 0 Тогда
		Для Каждого СведенияОКолонке Из ИнформацияПоКолонкам Цикл
			Если (ТипЗнч(СведенияОКолонке.ТипКолонки) = Тип("Строка")
				И СведенияОКолонке.ТипКолонки.КвалификаторыСтроки.Длина = 0) Или СтрНачинаетсяС(
				СведенияОКолонке.ИмяКолонки, "Свойство_") Тогда
				Продолжить;
			КонецЕсли;
			ПредставлениеКолонки = ?(ПустаяСтрока(СведенияОКолонке.Синоним), СведенияОКолонке.ПредставлениеКолонки,
				СведенияОКолонке.Синоним);
	...
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.ВыборКолонок", ПараметрыФормы, ЭтотОбъект, , , ,
...
Процедура УстранитьНеоднозначность(Команда)
	ОткрытьФормуРазрешенияНеоднозначности(Элементы.ТаблицаСопоставленияДанных.ТекущаяСтрока,
		Элементы.ТаблицаСопоставленияДанных.ТекущийЭлемент.Имя, Истина);
КонецПроцедуры
...
Процедура ШагНазад()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных
		...
		Элементы.Далее.Заголовок = ?(ТипЗагрузки = "ВставкаИзБуфераОбмена", НСтр("ru = 'Вставить в список'"), НСтр(
			"ru = 'Далее >'"));
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр(
					"ru = 'Строки, в которых не заполнены обязательные колонки, будут пропущены.'") + Символы.ПС + НСтр(
					"ru = 'Продолжить?'"), РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
...
Процедура ПослеСопоставленияНеоднозначностей(Результат, Параметр) Экспорт
	...
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		Если Результат <> Неопределено Тогда
			...
			Строка.РезультатСопоставленияСтроки = ?(СтрДлина(Строка.ОписаниеОшибки) = 0, "СтрокаСопоставлена",
				"НеСопоставлен");
		КонецЕсли;
	Иначе
		Строка = ТаблицаСопоставленияДанных.НайтиПоИдентификатору(Параметр.Идентификатор);
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Выполнено сопоставление'"), , НСтр("ru = 'Сопоставлено элементов:'")
		+ " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Функция СтатисткаСопоставления()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		Отбор                   = Новый Структура("РезультатСопоставленияСтроки", "НеСопоставлен");
		Результат               = ТаблицаСопоставленияДанных.НайтиСтроки(Отбор);
		КоличествоНеоднозначных = ТаблицаСопоставленияДанных.Количество() - КоличествоСопоставленных
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ru = 'Несопоставленные ('") + Статистика.НеСопоставленных
		+ ")";
	Элементы.ОбновлятьСуществующие.Заголовок       = НСтр("ru = 'Сопоставленные элементы ('") + Строка(
		Статистика.Сопоставленных) + ")";

	СписокВыбора = Элементы.ФильтрТаблицаСопоставления.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("Все", ТекстВсе, Истина);
	СписокВыбора.Добавить("Несопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Несопоставленные (%1 из %2)'"), Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Сопоставленные (%1 из %2)'"), Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Неоднозначные (%1 из %2)'"), Статистика.Неоднозначных, Статистика.Всего));

	Если Статистика.Неоднозначных > 0 Тогда
		Элементы.ОписаниеНеоднозначность.Видимость = Истина;
		Элементы.ОписаниеНеоднозначность.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = '(неоднозначностей: %1)'"), Статистика.Неоднозначных);
	Иначе
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбораФайла.Фильтр                      = НСтр(
			"ru = 'Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl'");
	...
	Иначе
		...
		ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.РасширениеФайла", , ЭтотОбъект, Истина, , , Оповещение,
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Вставка из буфера обмена (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
	...
	Обработки.ЗагрузкаДанныхИзФайла.УстановитьРежимВставкиИзБуфераОбмена(ШаблонСДанными, ИнформацияПоКолонкам,
		Параметры.ОписаниеТипов);
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаКлиенте()
	...
	ВысотаШапкиШаблонаСДанными = ?(ЗагрузкаДанныхИзФайлаКлиентСервер.КолонкиИмеютГруппировку(ИнформацияПоКолонкам), 2,
		1);
...
Процедура ОткрытьФормуИзмененияБланка()
	...
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.РедактированиеБланка", ПараметрыФормы, ЭтотОбъект, , , ,
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	КонецЕсли;
	ЗагрузкаДанныхИзФайла.ДобавитьСтатистическуюИнформацию("РежимЗапуска.ЗагрузкаВСправочник."
		+ ИмяОбъектаСопоставления, , ТипЗагрузки);
	...
	КонецЕсли;
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Загрузка данных в справочник ""%1""'"), ЗаголовокОкна);
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	ИнформацияПоКолонкамТаблица = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЗагрузкаДанныхИзФайла",
		ИмяОбъектаСопоставления, , ИмяПользователя());
	...
	КонецЕсли;
	Обработки.ЗагрузкаДанныхИзФайла.ОпределитьИнформацияПоКолонкам(ПараметрыЗагрузкиПоУмолчанию,
...
Процедура СопоставитьЗагружаемыеДанныеНаСервере()
	...
	ЗагрузкаДанныхИзФайла.ДобавитьСтатистическуюИнформацию(?(ВариантЗагрузки = 0, "ВариантЗагрузки.ЗаполнениеТаблицы",
		"ВариантЗагрузки.ИзВнешнегоФайла"));
	...
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		...
		Обработки.ЗагрузкаДанныхИзФайла.ТабличныйДокументВТаблицуЗначений(ШаблонСДанными, ТаблицаИнформацияПоКолонкам,
			АдресЗагруженныхДанных);

		СкопироватьСтруктуруТабличнойЧасти(АдресКопииТабличнойЧасти);

		МенеджерОбъекта = МенеджерОбъекта(ИмяОбъектаСопоставления);
		МенеджерОбъекта.СопоставитьЗагружаемыеДанные(АдресЗагруженныхДанных, АдресКопииТабличнойЧасти,
			СписокНеоднозначностей, ИмяОбъектаСопоставления, ДополнительныеПараметры);
	...
	ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Обработки.ЗагрузкаДанныхИзФайла.ЗаполнитьТаблицуСопоставленияДаннымиИзШаблона(ШаблонСДанными,
			ТаблицаСопоставления, ИнформацияПоКолонкам);
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеСопоставленияНаСервере(АдресРезультата)
	...
	Если Результат.Количество() = ТаблицаСопоставления.Количество() Тогда
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
			"ru = 'Данные сопоставлены. При необходимости возможно сопоставить данные вручную, заполнив колонку ""%1"".'"),
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		Если ПустаяСтрока(ШаблонСДаннымиОднаКолонка) Тогда
			ПоказатьПредупреждение( , (НСтр(
				"ru = 'Для вставки сопоставленных данных в список, заполните текстовое поле.'")));
			Возврат;
		КонецЕсли;
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ru = 'Обязательная колонка""'") + " " + СписокНезаполненныхКолонок[0] + НСтр(
				"ru = '"" содержит незаполненные строки, эти строки будут пропущены при загрузке.'");
		Иначе
...
Процедура ЗаписатьЗагружаемыеДанныеКлиент()
	...
	Элементы.ОткрытьСправочникПослеЗакрытияПомощника.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ОткрытьСправочникПослеЗакрытияПомощника.Заголовок, ПредставлениеОбъекта);
	Элементы.ПояснениеОтчетаОЗагрузке.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ПояснениеОтчетаОЗагрузке.Заголовок, ПредставлениеОбъекта);
...
Процедура ВывестиСообщениеОбОшибке(ТекстОшибкиДляПользователя, ТехническаяИнформация)
	ТекстСообщенияОбОшибке = ТекстОшибкиДляПользователя + Символы.ПС + НСтр("ru = 'Возможная причина: Загружаемые данные некорректные.
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			...
			Элементы.НадписьРезультатСопоставления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстНайдено, Статистика.Всего, Статистика.Сопоставленных);
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда
				...
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено,
					Статистика.НеНайдено, Статистика.Неоднозначных, Статистика.Несопоставленных);
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр(
					"ru = 'Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено,
					Статистика.Неоднозначных);
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр(
					"ru = 'Строки, для которых в программе нет соответствующих данных, будут пропущены: %1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено,
					Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр(
				"ru = 'Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".'");
			...
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПоискаСсылок, Ложь, НСтр(
				"ru = 'Далее >'"));
		Иначе
	...
	Иначе
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляСопоставленияДанных, Истина, НСтр(
				"ru = 'Загрузить данные >'"));
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			Если ТаблицаСопоставленияДанных.НайтиСтроки(Новый Структура("РезультатСопоставленияСтроки",
				"НеСопоставлен")).Количество() = 0 И ТаблицаСопоставленияДанных.НайтиСтроки(
				Новый Структура("РезультатСопоставленияСтроки", "Неоднозначность")).Количество() = 0 Тогда
				// Все строки сопоставлены
				ПереходКСледующемуШагуЗагрузкиДанных();
			КонецЕсли;

			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляТабличнойЧасти, Истина, НСтр(
				"ru = 'Загрузить данные'"));
			УстановитьОформлениеДляПолейСНеоднозначностью(Новый Структура("РезультатСопоставленияСтроки",
				"Неоднозначность"));
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр(
				"ru = 'Загрузить данные >'"));
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр(
				"ru = 'Загрузить данные >'"));
...
&НаКлиенте
Процедура УстановитьОформлениеДляСтраницыСопоставления(ВидимостьКнопкиСопоставление, ЭлементДляПоясняющегоТекста,
	ВидимостьКнопкиУстранитьНеоднозначность, ТекстКнопкиДалее)
		...
		Если Элемент.Пометка Тогда
			...
			Если Колонка <> Неопределено Тогда
				...
				Если СтрНачинаетсяС(Колонка.ИмяКолонки, "КонтактнаяИнформация_") Тогда
					...
					ИмяВидаКИ = СтандартныеПодсистемыСервер.ПреобразоватьАдаптированноеНаименованиеКолонкиВСтроку(Сред(
						Колонка.ИмяКолонки, СтрДлина("КонтактнаяИнформация_") + 1));
					...
					СтрокаУсловие = СтрокаУсловие + РазделительИ + "ВЫРАЗИТЬ(СправочникСопоставления.Представление
																   | КАК Строка(500)) = ВЫРАЗИТЬ(ТаблицаСопоставления."
						+ Колонка.ИмяКолонки + " КАК  Строка(500)) ";
					Если НайденныеВиды <> Неопределено Тогда
						СтрокаУсловие = СтрокаУсловие + " И СправочникСопоставления.Вид.Наименование = """
							+ НайденныеВиды.Ссылка.Наименование + """";
					КонецЕсли;
				...
				ИначеЕсли СтрНачинаетсяС(Колонка.ИмяКолонки, "ДополнительныйРеквизит_") Тогда
					...
					СтрокаУсловие = СтрокаУсловие + РазделительИ
						+ " СправочникСопоставления.Значение =  ТаблицаСопоставления." + Колонка.ИмяКолонки;
					...
					Если ОбъектаТипаКолонки <> Неопределено Тогда
						ОтборГде = ОтборГде + " И СправочникСопоставления.Значение <> ЗНАЧЕНИЕ("
							+ ОбъектаТипаКолонки.ПолноеИмя() + ".ПустаяСсылка)";
					КонецЕсли;
					РазделительИ = " И ";
					Продолжить;
				КонецЕсли;
				...
				Если ТипКолонки = Тип("Строка") Тогда
					Если Колонка.ТипКолонки.КвалификаторыСтроки.Длина = 0 Тогда
						СтрокаУсловие = СтрокаУсловие + РазделительИ + "ВЫРАЗИТЬ(СправочникСопоставления." + ИмяКолонкиСправочника
							+ " КАК Строка(500)) = ВЫРАЗИТЬ(ТаблицаСопоставления." + Колонка.ИмяКолонки
							+ " КАК  Строка(500))";
				...
				Иначе
					...
					Если ИнфоОбъект.ТипОбъекта = "Справочник" Тогда
						...
						Для Каждого СтрокаВвода Из Справочник.ВводПоСтроке Цикл
							Если СтрокаВвода.Имя = "Код" И Не Справочник.Автонумерация Тогда
								ТекстУсловияВВодаПоСтроке = "СправочникСопоставления." + ИмяКолонкиСправочника
									+ ".Код " + ТипСравнения + " ТаблицаСопоставления." + Колонка.ИмяКолонки;
							Иначе
								ТекстУсловияВВодаПоСтроке = "СправочникСопоставления." + ИмяКолонкиСправочника + "."
									+ СтрокаВвода.Имя + ТипСравнения + " ТаблицаСопоставления." + Колонка.ИмяКолонки;
							КонецЕсли;
					...
					ИначеЕсли ИнфоОбъект.ТипОбъекта = "Перечисление" Тогда
						СтрокаУсловие = СтрокаУсловие + РазделительИ + "СправочникСопоставления."
							+ ИмяКолонкиСправочника + " =  ТаблицаСопоставления." + Колонка.ИмяКолонки;
					КонецЕсли;
				КонецЕсли;
...
Процедура ВыполнитьСопоставлениеПоВыбранномуРеквизиту(КоличествоСопоставленных = 0, СписокКолонокСопоставления = "")
	...
	Для Каждого Колонка Из ТаблицаСопоставления.Колонки Цикл
		Если Колонка.Имя <> "СписокНеоднозначностей" И Колонка.Имя <> "РезультатСопоставленияСтроки" И Колонка.Имя
	...
	Для Каждого Колонка Из СопоставитьПоКолонке Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ЗагрузкаДанныхИзФайла.ДобавитьСтатистическуюИнформацию("СопоставлениеКолонок", КоличествоСопоставленных,
		СписокКолонокСопоставления);
...
Процедура ПоместитьДанныеВТаблицуСопоставления(АдресЗагруженныхДанных, АдресКопииТабличнойЧасти, СписокНеоднозначностей)
	...
	Для Каждого Строка Из ТабличнаяЧасть Цикл
		...
		Для Каждого Колонка Из ТабличнаяЧасть.Колонки Цикл
			...
			Если ЗначениеЗаполнено(КолонкиОбязательныеДляЗаполнения.Получить(Колонка.Имя)) И ЗаполненыВсеОбязательныеКолонки
				И Не ЗначениеЗаполнено(Строка[Колонка.Имя]) Тогда
				ЗаполненыВсеОбязательныеКолонки = Ложь;
			КонецЕсли;
		КонецЦикла;

		НоваяСтрока["РезультатСопоставленияСтроки"] = ?(ЗаполненыВсеОбязательныеКолонки, "СтрокаСопоставлена",
			"НеСопоставлен");
	...
	Для Каждого Строка Из ЗагружаемыеДанные Цикл
		...
		Если Строки.Количество() > 0 Тогда
			...
			Для Каждого Колонка Из ЗагружаемыеДанные.Колонки Цикл
				Если Колонка.Имя <> "Идентификатор" И Колонка.Имя <> "РезультатСопоставленияСтроки" И Колонка.Имя
					<> "ОписаниеОшибки" Тогда
...
Функция АдресВХранилищеТаблицыСопоставления()
	...
	Для Каждого Колонка Из Таблица.Колонки Цикл
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			ТаблицаДляТЧ.Колонки.Добавить(Сред(Колонка.Имя, СтрДлина("ТЧ_") + 1), Колонка.ТипЗначения,
				Колонка.Заголовок, Колонка.Ширина);
		ИначеЕсли Колонка.Имя = "РезультатСопоставленияСтроки" Или Колонка.Имя = "ОписаниеОшибки" Или Колонка.Имя
			= "Идентификатор" Тогда
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Объект ""%1"" не найден'"), ИмяОбъектаСопоставления);
	КонецЕсли;
...
Процедура СкопироватьСтруктуруТабличнойЧасти(АдресТабличнойЧасти)
	...
	Если ЗначениеЗаполнено(ИмяОбъектаСопоставления) Тогда
		...
		Для Каждого РеквизитТабличнойЧасти Из ТабличнаяЧасть.Реквизиты Цикл
			ДанныеДляТабличнойЧасти.Колонки.Добавить(РеквизитТабличнойЧасти.Имя, РеквизитТабличнойЧасти.Тип,
				РеквизитТабличнойЧасти.Представление());
		КонецЦикла;
	Иначе
		Для Каждого Колонка Из ИнформацияПоКолонкам Цикл
			ДанныеДляТабличнойЧасти.Колонки.Добавить(Колонка.ИмяКолонки, Колонка.ТипКолонки,
				Колонка.ПредставлениеКолонки);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Иначе
		ТекстПодсказки = НСтр(
			"ru = 'Для заполнения таблицы скопируйте данные в таблицу из внешнего файла через буфер обмена.'")
			+ Символы.ПС;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления,
	ДобавитьИдентификатор, ДобавитьОписаниеОшибки, ДобавитьРезультатСопоставленияСтроки, ДобавитьСписокНеоднозначностей)
	...
	Если ЗначениеЗаполнено(СтруктураОбъектаСопоставления) Тогда
		...
		КонецЕсли;
		ВременнаяТЗ.Колонки.Добавить("ОбъектСопоставления",
			СтруктураОбъектаСопоставления.ОписаниеТипаОбъектаСопоставления, ЗаголовокКолонки);
	КонецЕсли;

	Если ДобавитьРезультатСопоставленияСтроки Тогда
		ВременнаяТЗ.Колонки.Добавить("РезультатСопоставленияСтроки", Новый ОписаниеТипов("Строка"), НСтр(
			"ru = 'Результат'"));
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьСтандартныеКолонкиВМассивРеквизитов(МассивРеквизитов, ОписаниеТипаОбъектаСопоставления,
	ДобавитьИдентификатор, ДобавитьОписаниеОшибки, ДобавитьРезультатСопоставленияСтроки, ДобавитьСписокНеоднозначностей)

	ТипСтрока = Новый ОписаниеТипов("Строка");
	Если ДобавитьИдентификатор Тогда
		ТипЧисло = Новый ОписаниеТипов("Число");
		МассивРеквизитов.Добавить(Новый РеквизитФормы("Идентификатор", ТипЧисло, "ТаблицаСопоставленияДанных",
			"Идентификатор"));
	КонецЕсли;

	Если ЗначениеЗаполнено(ОписаниеТипаОбъектаСопоставления) Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ОбъектСопоставления", ОписаниеТипаОбъектаСопоставления,
			"ТаблицаСопоставленияДанных", ИмяОбъектаСопоставления));
	КонецЕсли;

	Если ДобавитьРезультатСопоставленияСтроки Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы("РезультатСопоставленияСтроки", ТипСтрока,
			"ТаблицаСопоставленияДанных", "Результат"));
	КонецЕсли;
	Если ДобавитьОписаниеОшибки Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ОписаниеОшибки", ТипСтрока, "ТаблицаСопоставленияДанных",
			"Причина"));
	КонецЕсли;

	Если ДобавитьСписокНеоднозначностей Тогда
		ТипСЗ = Новый ОписаниеТипов("СписокЗначений");
		МассивРеквизитов.Добавить(Новый РеквизитФормы("СписокНеоднозначностей", ТипСЗ, "ТаблицаСопоставленияДанных",
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахАвто(ОписаниеТипаОбъектаСопоставления)
	...
	ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, Истина, Ложь, Истина,
		Истина);
	ДобавитьСтандартныеКолонкиВМассивРеквизитов(МассивРеквизитов, ОписаниеТипаОбъектаСопоставления, Истина, Ложь,
		Истина, Истина);

	Для Каждого Колонка Из ИнформацияПоКолонкам Цикл
		ВременнаяТЗ.Колонки.Добавить(Колонка.ИмяКолонки, Колонка.ТипКолонки, Колонка.ПредставлениеКолонки);
		МассивРеквизитов.Добавить(Новый РеквизитФормы(Колонка.ИмяКолонки, Колонка.ТипКолонки,
			"ТаблицаСопоставленияДанных", Колонка.ПредставлениеКолонки));
	КонецЦикла;
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		НовыйЭлемент = Элементы.Добавить("ТаблицаСопоставленияДанных_" + Колонка.Имя, Тип("ПолеФормы"),
			Элементы.ТаблицаСопоставленияДанных); // РасширениеПоляФормыДляПоляВвода
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонках()
	...
	Синоним = ?(ЗначениеЗаполнено(ПараметрыЗагрузки.ПредставлениеОбъекта), ПараметрыЗагрузки.ПредставлениеОбъекта,
		ОбъектМетаданных.Представление());

	ВременнаяТЗ  = ЗагрузкаДанныхИзФайла.ОписаниеЗагружаемыхДанныхДляСправочников(ВременнаяТЗ,
		ОписаниеТипаОбъектаСопоставления, Синоним);
	ДобавитьСтандартныеКолонкиВМассивРеквизитов(МассивРеквизитов, ОписаниеТипаОбъектаСопоставления, Истина, Истина,
		Истина, Истина);
	...
	Для Каждого Колонка Из ИнформацияПоКолонкам Цикл
		Если ВременнаяТЗ.Колонки.Найти(Колонка.ИмяКолонки) = Неопределено Тогда
			...
			МассивРеквизитов.Добавить(Новый РеквизитФормы(Колонка.ИмяКолонки, Колонка.ТипКолонки,
				"ТаблицаСопоставленияДанных", ПредставлениеКолонки));
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахДляТЧ()
	...
	Для Каждого Колонка Из РеквизитыТЧ Цикл
		...
		ТипРеквизита = ?(Колонка.Тип.СодержитТип(Тип("УникальныйИдентификатор")), ОбщегоНазначения.ОписаниеТипаСтрока(
			36), Колонка.Тип);

		ВременнаяТЗ.Колонки.Добавить("ТЧ_" + Колонка.Имя, ТипРеквизита, Колонка.Представление());
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ТЧ_" + Колонка.Имя, ТипРеквизита, "ТаблицаСопоставленияДанных",
	...
	Для Каждого Колонка Из ИнформацияПоКолонкам Цикл
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ФЛ_" + Колонка.ИмяКолонки, ТипСтрока,
			"ТаблицаСопоставленияДанных", Колонка.ПредставлениеКолонки));
	КонецЦикла;
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			ГруппаКолонокЗагружаемыеДанныеТЧ = Элементы.Добавить("ЗагружаемыеДанные_" + Колонка.Имя, Тип(
				"ГруппаФормы"), ГруппаКолонокЗагружаемыеДанные);
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
				...
				Для Каждого СвязьПараметраВыбора Из КолонкаСвязьПараметровВыбора Цикл // СвязьПараметраВыбора
					...
					Если Позиция > 0 Тогда
						...
						НоваяСвязь = Новый СвязьПараметраВыбора(СвязьПараметраВыбора.Имя,
							"Элементы.ТаблицаСопоставленияДанных.ТекущиеДанные.ТЧ_" + ИмяЭлемента,
			...
			ИначеЕсли КолонкиГруппировки.Количество() > 1 Тогда
				ГруппаКолонокЗагружаемыеДанныеТЧ = Элементы.Добавить("ЗагружаемыеДанные_ФЛ_" + Колонка.Имя, Тип(
...
Процедура ПослеВыбораРасширенияФайла(Результат, Параметр) Экспорт
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, АдресВоВременномХранилище, ИмяОбъектаСопоставления + "."
			+ Результат, ПараметрыСохраненияФайла);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьНаименованияКолонокТаблицыСопоставления()
	...
	Для Каждого СтрокаТаблицы Из ИнформацияПоКолонкам Цикл
		...
		Если Колонка <> Неопределено Тогда
			Колонка.Заголовок = ?(Не ПустаяСтрока(СтрокаТаблицы.Синоним), СтрокаТаблицы.Синоним + " ("
				+ СтрокаТаблицы.ПредставлениеКолонки + ")", СтрокаТаблицы.ПредставлениеКолонки);
		КонецЕсли;
	КонецЦикла;
...
Процедура ИзменитьБланкПоИнформацииПоКолонкам(Бланк = Неопределено, СохранитьНастройки = Ложь)
	...
	Если СохранитьНастройки Тогда
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ЗагрузкаДанныхИзФайла", ИмяОбъектаСопоставления,
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр(
		"ru = 'Заполнение таблицы сопоставления загруженными данными из файла.'");

	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ЗагрузкаДанныхИзФайла.ЗаполнитьТаблицуСопоставленияДаннымиИзШаблонаФон", ПараметрыВызоваСервера,
