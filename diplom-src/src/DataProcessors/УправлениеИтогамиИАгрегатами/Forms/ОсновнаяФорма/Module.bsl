...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОписаниеУстановкиПериода.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ОписаниеУстановкиПериода.Заголовок, Формат(ОкончаниеПериода(ДобавитьМесяц(РассчитатьИтогиНа, -1)),
		"DLF=D"), Формат(ОкончаниеПериода(РассчитатьИтогиНа), "DLF=D"));
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.УправлениеИтогамиИАгрегатами.Форма.ФормаВыбораПериода") Тогда
		...
		ПараметрыИтогов.Вставить("ТекстСообщенияОбОшибке", НСтр(
			"ru = 'Не удалось установить период рассчитанных итогов.'"));

		УправлениеИтогами(ПараметрыИтогов);

	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег(
		"Обработка.УправлениеИтогамиИАгрегатами.Форма.ФормаПараметровПерестроения") Тогда
...
Процедура ВыключитьИспользованиеТекущихИтогов(Команда)
	...
	ПараметрыИтогов.Вставить("Значение2", Неопределено);
	ПараметрыИтогов.Вставить("ТекстСообщенияОбОшибке", НСтр(
		"ru = 'Не удалось выключить использование текущих итогов.'"));
...
Процедура ЗаполнитьАгрегатыИВыполнитьПерестроение(Команда)
	...
	МассивДействий = АгрегатыПоРегистрам.НайтиСтроки(Новый Структура("РежимАгрегатов,ИспользованиеАгрегатов", Истина,
		Истина));
...
Процедура ИзменитьАгрегатыКлиент(Знач ПараметрыИтогов, Знач ОчищатьСообщения = Истина)
	...
	Для Счетчик = 1 По Выбранные.Количество() Цикл
		...
		Если Не ПустаяСтрока(ПараметрыИтогов.Поле) Тогда
			ПолеСообщенияОбОшибке = "АгрегатыПоРегистрам[" + Формат(Выбранные[Счетчик - 1], "ЧГ=0") + "]."
				+ ПараметрыИтогов.Поле;
		КонецЕсли;

		Если Не ВыбраннаяСтрока.РежимАгрегатов И ВРег(ПараметрыИтогов.Действие) <> ВРег("УстановитьРежимАгрегатов") Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Операция невозможна в режиме итогов'"), , ПолеСообщенияОбОшибке);
			Продолжить;
		КонецЕсли;
	...
	Если ВРег(ПараметрыИтогов.Действие) = ВРег("УстановитьРежимАгрегатов") Или ВРег(ПараметрыИтогов.Действие) = ВРег(
		"УстановитьИспользованиеАгрегатов") Тогда
		ОбновитьСписокИтоговНаСервере();
	КонецЕсли;
...
Процедура УправлениеИтогами(Знач ПараметрыИтогов)
	...
	Для Счетчик = 1 По Выбранные.Количество() Цикл
		...
		ПолеСообщенияОбОшибке = "";
		Если Не ПустаяСтрока(ПараметрыИтогов.Поле) Тогда
			ПолеСообщенияОбОшибке = "СписокИтогов[" + Формат(Выбранные[Счетчик - 1], "ЧГ=0") + "]."
				+ ПараметрыИтогов.Поле;
		КонецЕсли;
		...
		Результат = УстановитьПараметрыРегистраНаСервере(
			ВыбраннаяСтрока.Тип, ВыбраннаяСтрока.ИмяМетаданного, ПараметрыИтогов.Действие, ПараметрыИтогов.Значение1,
			ПараметрыИтогов.Значение2, ПолеСообщенияОбОшибке, ПараметрыИтогов.ТекстСообщенияОбОшибке);
	...
	ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Пересчитаны (%1 из %2)'"), КоличествоОбработанныхСтрок, Выбранные.Количество());
...
Процедура ПолучитьОптимальныеАгрегатКлиентЗавершение(ПолученныеФайлы, РезультатВыполнения) Экспорт
	...
	КонецЕсли;
	ПоказатьОповещениеПользователя(НСтр("ru = 'Агрегаты успешно получены.'"), , РезультатВыполнения.ТекстСообщения,
...
Функция ПолучитьОптимальныеАгрегатыСервер()
	...
	Для НомерСтроки = 1 По Всего Цикл
		...
		Исключение
			...
			ПодробныйТекстОшибок = ПодробныйТекстОшибок + ?(ПустаяСтрока(ПодробныйТекстОшибок), "", Символы.ПС
				+ Символы.ПС) + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: %2'"),
				ИмяРегистраНакопления, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			Продолжить;
		КонецПопытки;
	...
	Если Успешно > 0 Тогда
		...
		Иначе
			КраткоеИмяФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Оптимальные агрегаты регистров накопления %1.zip'"), Формат(ТекущаяДатаСеанса(),
				"ДФ=yyyy-MM-dd"));
	...
	ИначеЕсли Результат.ЕстьОшибки Тогда
		...
		Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Агрегаты успешно получены для %1 из %2 регистров.
					 ...
					 |%4'"), Успешно, Всего, Всего - Успешно, ПодробныйТекстОшибок);
...
Функция УстановитьПараметрыРегистраНаСервере(Знач ВидРегистра, Знач ИмяРегистра, Знач Действие, Знач Значение1,
	Знач Значение2, // Значение по умолчанию: Неопределено.

	Знач ПолеОшибки, Знач СообщениеОбОшибке)
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(
			СообщениеОбОшибке + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), , ПолеОшибки);
		Возврат Ложь;
...
Функция ИзменитьАгрегатыСервер(Знач ПараметрыСервера)
	...
	Попытка
		...
		ИначеЕсли ВРег(ПараметрыСервера.Действие) = ВРег("ПерестроитьАгрегаты") Тогда
			МенеджерРегистра.ПерестроитьИспользованиеАгрегатов(ПараметрыСервера.ЗначениеДействия1,
				ПараметрыСервера.ЗначениеДействия2);
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неправильное имя параметра: %1'"), ПараметрыСервера.Действие);
		КонецЕсли;

	Исключение

		СообщениеОбОшибке = ПараметрыСервера.СообщениеОбОшибке + " (" + КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()) + ")";
