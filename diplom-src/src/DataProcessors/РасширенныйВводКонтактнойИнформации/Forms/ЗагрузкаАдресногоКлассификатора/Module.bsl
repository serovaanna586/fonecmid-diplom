...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.НазваниеРегиона) Тогда
		ТекстВопроса = НСтр(
			"ru = 'Для автоподбора и выбора адресных сведений загрузите классификатор по региону ""%1"".'");
		Элементы.Надпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса,
			Параметры.НазваниеРегиона);
	КонецЕсли;
КонецПроцедуры
