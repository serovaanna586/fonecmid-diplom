...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидКИ = УправлениеКонтактнойИнформациейСлужебный.СтруктураВидаКонтактнойИнформации(
		Параметры.ВидКонтактнойИнформации);
	...
	Если ПустаяСтрока(ЗначенияПолей) Тогда
		НаселенныйПунктДетально = УправлениеКонтактнойИнформацией.ОписаниеНовойКонтактнойИнформации(
			Перечисления.ТипыКонтактнойИнформации.Адрес); // Новый адрес
	...
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(ЗначенияПолей) Тогда
		ДанныеАдреса = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(ЗначенияПолей,
	...
	Иначе
		...
		ДанныеАдреса = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(XDTOКонтактная,
			ТипКонтактнойИнформации);
		НаселенныйПунктДетально = РаботаСАдресами.ПодготовитьАдресДляВвода(ДанныеАдреса);
	КонецЕсли;

	Если МеждународныйФорматАдреса И Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(
		НаселенныйПунктДетально.AddressType) Тогда
	...
	Если ТолькоНациональныйАдрес Тогда
		...
		Если Страна <> ОсновнаяСтрана Тогда
			...
			ТекстПредупрежденияПриОткрытии = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстПредупрежденияПриОткрытии, ОсновнаяСтрана.Наименование);
	...
	Если Не ЕстьПравоЗагружатьКлассификатор Тогда
		Элементы.ИнформацияОЗагрузкеАдресныеСведений.Заголовок = НСтр(
			"ru = 'Для автоподбора и проверки адресов обратитесь к администратору для загрузки адресных сведений.'");
		Элементы.ЗагрузитьКлассификатор.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И ВыбранноеЗначение.Свойство("КраткоеПредставлениеОшибки")
			...
			Иначе
				Если Не ПодборПоИндексуДоступен Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
						"ru = 'Индекс %1 не найден в адресном классификаторе'"), Формат(Результат.Индекс,
						"ЧРГ=' '; ЧГ=0"));
...
Процедура ТипДомаПриИзменении(Элемент)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(НаселенныйПунктДетально.HouseNumber) Или НаселенныйПунктДетально.buildings.Количество()
			> 0 Тогда
...
Процедура НаселенныйПунктНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.НаселенныйПунктАдреса", ПараметрыФормы, Элемент, ,
...
Процедура НаселенныйПунктОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЕстьПравоЗагружатьКлассификатор И ВыбранноеЗначение.ПредлагатьЗагрузкуКлассификатора = Истина Тогда
		...
		ПредложениеЗагрузкиКлассификатора(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Данные для ""%1"" не загружены.'"), ВыбранноеЗначение.Представление), ВыбранноеЗначение);
	Иначе
...
&НаКлиенте
Процедура НаселенныйПунктАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
...
Процедура ПослеВыбораУлицы(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация об улицах для введенного населенного пункта.'"));
	КонецЕсли;
...
Процедура УлицаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	КонецЕсли;
	СведенияОбАдресе.Вставить("Муниципальный", РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(
		НаселенныйПунктДетально.addressType));

	НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, СведенияОбАдресе, Истина,
		ИсключатьГородИзМуниципальногоАдреса);
	ОбработкаИзмененияАдреса();
...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		Если ЗначениеЗаполнено(Результат.ДействуетПо) Тогда
			ТекстИсторическийАдрес = " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'действует по %1'"), Формат(Результат.ДействуетПо - 10, "ДЛФ=DD"));
		Иначе
...
Функция АдресСИсторией(ЗначенияПолей)
	...
	Возврат УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(ЗначенияПолей,
		Перечисления.ТипыКонтактнойИнформации.Адрес);

КонецФункции

&НаКлиенте
Процедура ИнформацияОЗагрузкеАдресныеСведенийОбработкаНавигационнойСсылки(Элемент,
	НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	Если НавигационнаяСсылкаФорматированнойСтроки = "load" И ОбщегоНазначенияКлиент.ПодсистемаСуществует(
...
&НаКлиенте
Процедура ДекорацияНужнаПомощьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	СписокОшибок = СписокОшибокЗаполнения(НаселенныйПунктДетально, ВидКонтактнойИнформации,
		ПредупреждатьОбОтсутствииОшибок, ТребуетсяОбновление);
...
Функция КонтактнаяИнформацияПоПредставлению(ПредставлениеАдреса, Комментарий = "", РазбиватьНаПоля = Ложь)
	...
	Адрес = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияПоПредставлению(ПредставлениеАдреса,
		Перечисления.ТипыКонтактнойИнформации.Адрес, РазбиватьНаПоля);
...
Процедура ПоказатьАдресВСвободнойФорме()
	...
	НаселенныйПунктДетально.Country     =  НаименованиеСтраны(Страна, ОписаниеВидаКонтактнойИнформации(
		ЭтотОбъект).МеждународныйФорматАдреса);
...
Процедура ИсторияИзменений(Команда)
	...
	СписокКонтактнойИнформации = ЗаполнитьСписокКонтактнойИнформации(ОписаниеВидаКонтактнойИнформации(
		ЭтотОбъект).Ссылка, ОписаниеДополнительныхРеквизитов);
	...
	ОткрытьФорму("Обработка.ВводКонтактнойИнформации.Форма.ИсторияКонтактнойИнформации", ПараметрыФормы, ЭтотОбъект, , ,
		, ОповещениеОЗакрытие);
...
Процедура АдресныйКлассификаторУстарел(Команда)
	...
	Иначе
		СообщениеОПроверкеАдреса = СообщениеОПроверкеАдреса + Символы.ПС + НСтр(
			"ru = 'Для обновления адресного классификатора обратитесь к администратору.'");
...
Процедура УстановитьМуниципальныеСведения(НаселенныйПунктДетально)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторАдреса) Тогда
			...
			Результат = МодульАдресныйКлассификаторСлужебный.АктуальныеАдресныеСведения(
				Новый Структура("Идентификатор", ИдентификаторАдреса));
			Если Не Результат.Отказ Тогда
...
Процедура ФормаВыбораУлицПоИндексу(Знач Текст)
	...
	Если АвтоподборДоступен Тогда
		...
		ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ВыборАдресаПоПочтовомуИндексу",
			ПараметрыФормы, Элементы.Индекс, , , , Оповещение);
	Иначе
		ОбновитьПредставлениеАдреса();
	КонецЕсли;
...
Процедура ОтобразитьЭлементыФормыВСоответствиеСоСтраной()
	...
	Если Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		...
		КоличествоПолейПредыдущийТипАдреса = РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(
			НаселенныйПунктДетально.AddressType, Ложь).Количество();

		ПереопределитьТипаАдресаПоСтране(Страна, НаселенныйПунктДетально, СведенияОСтране);

		КоличествоПолейНовыйТипАдреса = РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(
			НаселенныйПунктДетально.AddressType, Ложь).Количество();
...
Процедура ПереопределитьТипаАдресаПоСтране(Страна, НаселенныйПунктДетально, СведенияОСтране)
	...
	Если РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(Страна) Тогда
		Если СтрСравнить(НаселенныйПунктДетально.AddressType,
			РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес()) <> 0 Тогда
			НаселенныйПунктДетально.AddressType = РаботаСАдресамиКлиентСервер.МуниципальныйАдрес();
		КонецЕсли;
...
Процедура ПеренестиЗначенияПолейАдресВПоляНовогоТипа()
	...
	Для Счетчик = 0 По СведенияАдреса.Количество() - 1 Цикл
		...
		Иначе
			НаселенныйПунктДетально[ИмяУровня] = НаселенныйПунктДетально[ИмяУровня] + Разделитель
				+ СведенияАдреса.Получить(Счетчик).Представление;
			Разделитель = ", ";
		КонецЕсли;
	...
	Если ЭтоМеждународныйАдрес(НаселенныйПунктДетально.AddressType) И НаселенныйПунктДетально.Свойство("Street") Тогда
		...
		СписокЭлементовАдреса.Добавить(СокрЛП(НаселенныйПунктДетально["Street"] + " "
			+ НаселенныйПунктДетально["StreetType"]));
		...
		Если НаселенныйПунктДетально.Свойство("HouseNumber") И ЗначениеЗаполнено(НаселенныйПунктДетально["HouseType"]) Тогда
			СписокЭлементовАдреса.Добавить(СокрЛП(НаселенныйПунктДетально["HouseType"] + " №"
				+ НаселенныйПунктДетально["HouseNumber"]));
			НаселенныйПунктДетально["HouseNumber"]  = "";
			НаселенныйПунктДетально["HouseType"]    = "";
		КонецЕсли;

		Если НаселенныйПунктДетально.Свойство("Buildings") И ТипЗнч(НаселенныйПунктДетально["Buildings"]) = Тип(
			"Массив") Тогда
		...
		Если НаселенныйПунктДетально.Свойство("Apartments") И ТипЗнч(НаселенныйПунктДетально["Apartments"]) = Тип(
			"Массив") Тогда
...
Процедура УдалитьСтрануИзПредставления()
	...
	НаименованиеСтраны = ВРег(НаименованиеСтраны(Страна, ОписаниеВидаКонтактнойИнформации(
		ЭтотОбъект).МеждународныйФорматАдреса));
	...
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр(
				"ru = 'Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.'");
		Иначе
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда // При немодифицированности работает как "отмена".
		...
		Если (ВидКонтактнойИнформации.ПроверятьКорректность И Не РазрешитьВводАдресаВСвободнойФорме
			Или ВидКонтактнойИнформации.МеждународныйФорматАдреса) И Результат.ОшибкиЗаполнения.Количество() > 0 Тогда
...
Процедура ОбработатьКонтактнуюИнформациюСИсторией(Результат)
	...
	Если ДатаБылаИзменена Тогда
		...
		Если ДанныеБылиИзменены Тогда
			...
			КонецЕсли;
		ИначеЕсли СтрСравнить(Результат.Комментарий, СтрокаДействующегоАдреса.Комментарий) <> 0
...
Функция ДанныеБылиИзменены(ЗначениеПолейДо, ЗначениеПолейПосле)
	...
	Если ЗначениеЗаполнено(ЗначениеПолейДо) И ЗначениеЗаполнено(ЗначениеПолейПосле) Тогда
		...
		ЗначенияДо    = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(ЗначениеПолейДо,
			Перечисления.ТипыКонтактнойИнформации.Адрес);
		ЗначенияПосле = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(ЗначениеПолейПосле,
			Перечисления.ТипыКонтактнойИнформации.Адрес);

		Если СтрСравнить(ЗначенияДо.value, ЗначенияПосле.value) <> 0 Или СтрСравнить(ЗначенияДо.country,
			ЗначенияПосле.country) <> 0 Или СтрСравнить(ЗначенияДо.comment, ЗначенияПосле.comment) <> 0 Или СтрСравнить(
			ЗначенияДо.oktmo, ЗначенияПосле.oktmo) <> 0 Тогда
			Возврат Истина;
		КонецЕсли;
...
Процедура ПослеЗакрытияФормыИстории(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВводНовогоАдреса Тогда
		...
		Страна = ОсновнаяСтрана;
		НаселенныйПунктДетально = РаботаСАдресамиКлиентСервер.ОписаниеНовойКонтактнойИнформации(
...
Функция СтрокаJSONВСтруктуру(Значение)
	Возврат УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(Значение,
		Перечисления.ТипыКонтактнойИнформации.Адрес);
КонецФункции
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	НаселенныйПунктДетально.Value = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(НаселенныйПунктДетально,
		ВидКИ.ВключатьСтрануВПредставление, , Контекст.ИсключатьГородИзМуниципальногоАдреса);

	Если ВозвращатьСписокЗначений Тогда
		ДанныеВыбора = Обработки.РасширенныйВводКонтактнойИнформации.КонтактнаяИнформацияВСтаруюСтруктуру(
	...
	Если ТипЗнч(ДанныеВыбора) = Тип("Структура") Тогда
		...
		КонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзJSONВXML(ДанныеВыбора,
			Перечисления.ТипыКонтактнойИнформации.Адрес);
	Иначе
		Значение             = "";
...
Процедура ОпределитьНеобходимостьОбновленияКлассификатора(Знач НаселенныйПунктДетально, ТребуетсяОбновление)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		ИнформацияОбОбновление = МодульАдресныйКлассификаторСлужебный.ОписаниеПоследнейЗагрузки(
			НаселенныйПунктДетально.AreaID);
...
&НаСервереБезКонтекста
Функция ЗаполнитьСписокКонтактнойИнформации(ВидКонтактнойИнформации,
	КонтактнаяИнформацияОписаниеДополнительныхРеквизитов)
...
Процедура СообщениеОНеобходимостиОбновленияКлассификатора(Идентификатор = Неопределено)
	...
	Элементы.АдресныйКлассификаторУстарел.Видимость = Ложь;
	Если Не ВебСервисИспользуется И ОбщегоНазначения.ПодсистемаСуществует(
		"СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Элементы.ОКТМО.Видимость                     = ВидКонтактнойИнформации.УказыватьОКТМО;
		ВебСервисИспользуется                        = ВебКлассификаторДоступен;
		АвтоподборДоступен                           = ИспользуетсяАдресныйКлассификатор И (ВебКлассификаторДоступен
...
Процедура УстановитьЗначенияНациональныхРеквизитовКонтактнойИнформации(ДанныеАдреса)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		ИсключатьГородИзМуниципальногоАдреса = МодульАдресныйКлассификаторСлужебный.ИсключатьГородИзМуниципальногоАдреса(
			ДанныеАдреса);
	Иначе
		ИсключатьГородИзМуниципальногоАдреса = Ложь;
		;
...
Функция ДобавитьПолеСтроенияИлиПомещения(Идентификатор, Тип, Значение)
	...
	Если Тип = "Помещение" Тогда
		...
		НоваяГруппа = Элементы.Добавить("ГруппаПомещение" + Идентификатор, Тип("ГруппаФормы"),
			Элементы.ГруппаПомещенияДополнительно);
		...
		ЗаполнитьЗначенияСвойств(НовыйТип, Элементы.ТипПомещения, ,
			"ПутьКДанным, СписокВыбора, ВыделенныйТекст, СвязьПоТипу");
		...
		ЗаполнитьЗначенияСвойств(НовыйЭлемент, Элементы.Помещение, ,
			"ПутьКДанным, СписокВыбора, ВыделенныйТекст, СвязьПоТипу");
	...
	Иначе
		...
		НоваяГруппа = Элементы.Добавить("ГруппаСтроение" + Идентификатор, Тип("ГруппаФормы"),
			Элементы.ГруппаСтроенияДополнительно);
		...
		ЗаполнитьЗначенияСвойств(НовыйТип, Элементы.ТипСтроения, ,
			"ПутьКДанным, СписокВыбора, ВыделенныйТекст, СвязьПоТипу");
		...
		ЗаполнитьЗначенияСвойств(НовыйЭлемент, Элементы.Строение, ,
			"ПутьКДанным, СписокВыбора, ВыделенныйТекст, СвязьПоТипу");
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС)
	...
	Иначе
		...
		Отбор = Новый Структура("Вид", ОписаниеВидаКонтактнойИнформации(ЭтотОбъект).Ссылка);
		НайденныеСтроки = КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() = 0 Или (НайденныеСтроки.Количество() = 1 И ПустаяСтрока(
			НайденныеСтроки[0].Представление)) Тогда
...
Процедура ОтобразитьКоличествоЗаписейВИсторииИзменений()
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'История изменений (%1)'"), НайденныеСтроки.Количество());
...
Процедура ОбновитьМенюДобавленияСтроенийИПомещений()
	...
	Для Каждого СтроениеПомещение Из ДополнительныеСтроенияИПомещения Цикл
		...
		Если ЗначениеЗаполнено(СтроениеПомещение.Ключ) Тогда
			...
			Если ЗначениеЗаполнено(СтроениеПомещение.ИмяЭлемента) Или СтрСравнить(СтроениеПомещение.Значение,
				ТипПомещения) = 0 Или СтрСравнить(СтроениеПомещение.Значение, ТипСтроения) = 0 Тогда
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].АдресПроверен = Ложь И Не ВебСервисИспользуется Тогда
			ТекстВопроса = НСтр("ru = 'Адрес не может быть проверен.'") + Символы.ПС + СписокОшибок[0].ТекстОшибки
				+ Символы.ПС + НСтр("ru = 'Загрузить классификатор для проверки адреса?'");
	...
	Для Каждого Элемент Из СписокОшибок Цикл
		Если Элемент.ВариантАдреса.Количество() > 0 Тогда
			...
			Оповещение = Новый ОписаниеОповещения("ПослеВопросаПроЗаменуИсторическогоАдреса", ЭтотОбъект,
...
Процедура НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, Знач ВыбранноеЗначение, Знач ВключатьУлица,
	...
	Если ВыбранноеЗначение.Свойство("Адрес") И ЗначениеЗаполнено(ВыбранноеЗначение.Адрес) Тогда
		ПолученныйАдрес = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
			ВыбранноеЗначение.Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
	Иначе
	...
	Если ЗначениеЗаполнено(НаселенныйПунктДетально.Street) Или ЗначениеЗаполнено(НаселенныйПунктДетально.HouseNumber)
...
Процедура ЗаполнитьПоляДомовСтроенийЗемельныхУчастков(Знач ВыбранноеЗначение)
	...
	УстановитьМуниципальныеЧастиАдреса(НаселенныйПунктДетально, ТипЧисло.ПривестиЗначение(ВыбранноеЗначение.ОКТМО),
		ВыбранноеЗначение.Идентификатор);
	ОбработкаИзмененияАдреса();
...
Процедура ЗаполнитьПоляДомовИСтроений(Знач ВыбранноеЗначение)
	...
	Если ВыбранноеЗначение.Свойство("Строение") И ТипЗнч(ВыбранноеЗначение.Строение) = Тип("Структура") Тогда
		СведенияОКорпусе = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
			ПерваяБукваЗаглавная(ВыбранноеЗначение.Строение.ВидСтроения), ВыбранноеЗначение.Строение.НомерСтроения);
		НаселенныйПунктДетально.Buildings.Добавить(СведенияОКорпусе);
	КонецЕсли;
...
Процедура ЗаполнитьПредопределенныеВариантыАдреса(Параметры)
	...
	Если Параметры.Свойство("ТипПомещения") И ВариантыДанныхПомещение.ВариантыТипа.Найти(Параметры.ТипПомещения)
		<> Неопределено Тогда
...
Функция СписокАвтоподбораНаселенногоПункта(Текст, ТипАдреса)
	...
	Результат = Обработки.РасширенныйВводКонтактнойИнформации.СписокАвтоподбораНаселенногоПункта(Текст,
		ДополнительныеПараметры);
	УправлениеКонтактнойИнформациейСлужебный.ФорматированиеРезультатовАвтоподбора(Результат.Данные, Текст, ПустаяСтрока(
		ТипАдреса));
	Возврат Результат;
...
Процедура УстановитьТипАдресаИзПараметра(ТипАдресаИзПараметра)
	...
	Если СтрСравнить(ТипАдресаИзПараметра, "АдминистративноТерриториальный") = 0 Или СтрСравнить(ТипАдресаИзПараметра,
		РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес()) = 0 Тогда
...
Процедура ПредложениеЗагрузкиКлассификатора(Знач Текст = "", Знач ДанныеОбАдресе = Неопределено)
	...
	Если ДанныеОбАдресе <> Неопределено И (НеЗагружатьАдресныйКлассификатор = Неопределено
...
Функция ИзвлечьСтарыйФорматАдреса(Знач ЗначенияПолей, Знач ТипКонтактнойИнформации)
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(ЗначенияПолей)
		...
		XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзXML(ЗначенияПолей,
			ТипКонтактнойИнформации, РезультатыЧтения);
...
Процедура СформироватьМенюДобавленияСтроенийИПомещений()
	...
	Для Каждого СтроениеИлиПомещение Из ДополнительныеСтроенияИПомещения Цикл
		...
		Если ЗначениеЗаполнено(СтроениеИлиПомещение.Ключ) Тогда
			...
			Группа = ?(СтроениеИлиПомещение.Тип = "Строение", Элементы.ГруппаДобавлениеСтроения,
				Элементы.ГруппаДобавлениеПомещения);
...
Процедура ОбновитьПредставлениеАдреса()
	...
	РаботаСАдресамиКлиентСервер.ОбновитьПредставлениеАдреса(НаселенныйПунктДетально,
		ВидКонтактнойИнформации.ВключатьСтрануВПредставление, ИсключатьГородИзМуниципальногоАдреса);
	ПредставлениеАдреса = НаселенныйПунктДетально.Value;
	Если ВидКонтактнойИнформации.ВключатьСтрануВПредставление
		И УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		УдалитьСтрануИзПредставления();
	КонецЕсли;
	...
	Если Не АдресВосстановлен Тогда
		...
		Для ИндексАдреса = 0 По ИменаУровнейАдреса.Количество() - 1 Цикл
			Если ЭлементыАдреса.Количество() > 0 Тогда
				...
				Если Позиция > 0 И Не РаботаСАдресамиКлиент.ПредставлениеУровняБезСокращения(
					ИменаУровнейАдреса[ИндексАдреса]) Тогда
					НаселенныйПунктДетально[ИменаУровнейАдреса[ИндексАдреса]] = СокрЛП(Лев(ЗначениеПоля, Позиция));
					НаселенныйПунктДетально[ИменаУровнейАдреса[ИндексАдреса] + "Type"] = СокрЛП(Сред(ЗначениеПоля,
						Позиция + 1));
				Иначе
		...
		Если ЭлементыАдреса.Количество() > 0 Тогда
			НаселенныйПунктДетально[ИменаУровнейАдреса[ИменаУровнейАдреса.Количество() - 1]] = СокрЛП(СтрСоединить(
				ЭлементыАдреса, ","));
		КонецЕсли;
...
Функция ВосстановитьАдрес(НаселенныйПункт, НаселенныйПунктДетально, ИменаУровнейАдреса)
	...
	Результат = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(НаселенныйПункт,
		Перечисления.ТипыКонтактнойИнформации.Адрес);

	Адрес = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(Результат,
		Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Процедура ОбработкаИзмененияАдреса()
	...
	Улица = ?(ПустаяСтрока(НаселенныйПунктДетально.Street), "", УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
			НаселенныйПунктДетально.Street, НаселенныйПунктДетально.StreetType));

	Если ПустаяСтрока(НаселенныйПунктДетально.country) И (ЗначениеЗаполнено(НаселенныйПункт) Или ЗначениеЗаполнено(
		Улица)) Тогда
		НаселенныйПунктДетально.country = ?(ЗначениеЗаполнено(Страна), Строка(Страна),
			РаботаСАдресамиКлиентСервер.ОсновнаяСтрана());
	КонецЕсли;
...
Функция ПредставлениеНаселенногоПунктаАдреса(Адрес)
	...
	Для Каждого ИмяУровня Из РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(Адрес.AddressType, Ложь, ,
		ИсключатьГородИзМуниципальногоАдреса) Цикл
...
Процедура ПереключитьНаСтруктурированныйВвод(ТипАдреса = Неопределено)
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПереключитьНаСтруктурированныйВводЗавершение", ЭтотОбъект,
			ДополнительныеПараметры);
...
Процедура ПереключитьНаСтруктурированныйВводЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Комментарий = ?(Не ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "") + НСтр("ru = 'Исходный адрес:'") + " "
		+ ПредставлениеАдреса;
	НаселенныйПунктДетально.comment = Комментарий;
...
Функция ИдентификаторАдреса(Адрес, ВключатьУлицу)
	...
	Для Каждого Уровень Из Уровни Цикл
		...
		Если УровеньКлюч > МаксимальныйУровень И Адрес.Свойство(Уровень.Значение + "Id") И ЗначениеЗаполнено(
			Адрес[Уровень.Значение + "Id"]) Тогда
...
Функция УровеньАдреса(Знач Уровень, Знач ТипАдреса)
	...
	Если РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса) И Уровень = 4 Тогда
...
Процедура СформироватьТекстНужнаПомощь()
	...
	ДополнительныйТекст = Символы.ПС + Символы.ПС + НСтр(
		"ru = 'Также адрес можно ввести в свободной форме (см. Еще - Адрес в свободной форме).'");
	...
	Если ВидКонтактнойИнформации.МожноИзменятьСпособРедактирования И ВидКонтактнойИнформации.ПроверятьКорректность Тогда
		ДополнительныйТекст = ДополнительныйТекст + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
			"ru = 'Для этого предварительно снять флажок ""Запрещать ввод некорректных адресов"" в виде контактной информации ""%1"".'"),
			ОписаниеВидаКонтактнойИнформации(ЭтотОбъект).Ссылка);

	КонецЕсли;

	Элементы.ДекорацияНужнаПомощь.Заголовок = Новый ФорматированнаяСтрока(Элементы.ДекорацияНужнаПомощь.Заголовок,
		ДополнительныйТекст);
