...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КомпонентаНечеткогоПоиска = ОбщегоНазначения.ПодключитьКомпонентуИзМакета("FuzzyStringMatchExtension",
...
Процедура ОбновитьМестаИспользованияКандидата(Знач ИдентификаторСтроки)
	...
	Если ТекущаяСтрока.ПолучитьРодителя() = Неопределено Тогда
		...
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для элемента ""%1"" найдено дублей: %2'"), НаименованиеОригинала, ТекущаяСтрока.Количество);
		Если ПомеченыНаУдалениеИНеИспользуются Тогда
			Описание = Описание + Символы.ПС + НСтр(
				"ru = 'Однако все они помечены на удаление и не используются, поэтому удаление дублей уже не требуется.'");
		КонецЕсли;
	...
	Если ТекущаяСтрока.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент ""%1"" не используется'"), ТекущаяСтрока.Наименование);
		Элементы.ГруппаПодробно.Заголовок = НСтр("ru = 'Сведения:'");
		Элементы.СтраницыМестаИспользования.ТекущаяСтраница = Элементы.ОписаниеГруппы;
	Иначе
		Элементы.ГруппаПодробно.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Места использования ""%1"" (%2):'"), ТекущаяСтрока.Наименование, ТекущаяСтрока.Количество);
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		МестаИспользованияНеобработанных.Очистить();
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = НСтр(
			"ru = 'Выбранный дубль не заменен из-за невозможности замены других дублей.'");
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
...
Процедура ОбновитьСостояниеВозможныхДублей()
	...
	Элементы.ГруппаВозможныеДубли.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокГруппы,
		КоличествоНеобработанных);
...
Процедура Подробно(Команда)
	...
	Элементы.Подробнее.Заголовок = ?(Элементы.ГруппаПодробно.Видимость, НСтр("ru = '<< Скрыть'"), НСтр(
		"ru = 'Подробнее >>'"));
КонецПроцедуры
...
Процедура ШагВыборОсновногоЭлементаПриНажатииКнопкиДалее(ОписаниеСледующегоШага, ДополнительныеПараметры) Экспорт
	...
	ПараметрыОбработчика = Новый Структура("ОписаниеСледующегоШага, ОбработчикЗавершения", ОписаниеСледующегоШага,
		ДополнительныеПараметры.ОбработчикЗавершения);
	Если ВозвращеноМеньшеЧемНайдено Тогда

		ОбработчикОтвета = Новый ОписаниеОповещения("ШагВыборОсновногоЭлементаПриНажатииКнопкиДалееЗавершение",
			ЭтотОбъект, ПараметрыОбработчика);
		ПоказатьВопрос(
			ОбработчикОтвета, НСтр("ru='Отображены не все найденные дубли. Будут обработаны все найденные дубли.
...
Процедура ШагВыборОсновногоЭлементаПриНажатииКнопкиДалееЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ВыполнитьОбработкуОповещения(
			ДополнительныеПараметры.ОбработчикЗавершения, ДополнительныеПараметры.ОписаниеСледующегоШага);
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей()
	...
	Если ПустаяСтрока(ОписаниеОшибкиПоискаДублей) Тогда
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбрано дублей: %1 из %2.'"), ВсегоНайденоДублей, ВсегоЭлементов);
	Иначе
	...
	Если ЕстьПодсистемаУдаленияПомеченных Тогда
		...
		Если СпособУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр(
				"ru = 'Выбранные элементы будут <a href = ""[Действие]"">помечены на удаление</a> и заменены на оригиналы (отмечены стрелкой).'");
		...
		Иначе
			ТекстПодсказки = НСтр(
				"ru = 'Выбранные элементы будут <a href = ""[Действие]"">безвозвратно удалены</a> и заменены на оригиналы (отмечены стрелкой).'");
			ПараметрыСтроки = Новый Структура("Действие", "СпособУдаления");
			ТекстПодсказки = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ТекстПодсказки, ПараметрыСтроки);
		КонецЕсли;
	...
	КонецЕсли;
	ОписаниеСостоянияНайденныхДублей = СтроковыеФункцииКлиент.ФорматированнаяСтрока(Описание + Символы.ПС
		+ ТекстПодсказки);
...
Процедура ИнициализироватьКомпоновщикОтбораИПравила(НастройкиФормы)
	...
	ДобавитьПравилаРеквизитов(ТаблицаПравил, ИсключаемыеРеквизиты, ВсеВариантыСравнения,
		ОбъектМетаданных.СтандартныеРеквизиты, НечеткийПоиск);
	ДобавитьПравилаРеквизитов(ТаблицаПравил, ИсключаемыеРеквизиты, ВсеВариантыСравнения, ОбъектМетаданных.Реквизиты,
		НечеткийПоиск);
	...
	Если ЭтоАдресВременногоХранилища(АдресВыбранныхМестИспользования) Тогда
		...
		Если ВыбранныеМестаИспользования.ЗначениеОтбора.Количество() > 0 Тогда
			...
			ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
				...
				ВыбранныеМестаИспользования.ПредставлениеОтбора, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
		...
		Для Каждого РеквизитЗначениеОтбора Из ВыбранныеМестаИспользования.ЗначениеОтбора Цикл
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
				ГруппаОтбора, РеквизитЗначениеОтбора.Ключ, РеквизитЗначениеОтбора.Значение,
	...
	Если СтрокаТаблицыНастроек.СобытиеПараметрыПоискаДублей Тогда
		ПрикладныеПараметры = ПоискИУдалениеДублей.ПараметрыПоискаДублей(ТаблицаПравил,
	...
	ИдентификаторОбластиПоискаДублей = Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("ПолноеИмя",
		ОбластьПоискаДублей);
	ТипОбластиПоискаДублей = ТипЗнч(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИдентификаторОбластиПоискаДублей,
		"ЗначениеПустойСсылки"));
	ТипыОбработанныхОбъектов = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		ТипОбластиПоискаДублей));
...
Процедура ДобавитьПравилаРеквизитов(ТаблицаПравил, Знач ИсключаемыеРеквизиты, Знач ВсеВариантыСравнения,
	...
	Для Каждого МетаданныеРеквизита Из КоллекцияРеквизитов Цикл
		...
		ВариантыСравнения = ВариантыСравненияДляТипа(МетаданныеРеквизита.Тип, ВсеВариантыСравнения,
			ДоступенНечеткийПоиск);
...
Процедура НайтиИУдалитьДублиПрогресс(Прогресс, ДополнительныеПараметры) Экспорт
	...
	Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Тогда
		...
		ИначеЕсли Прогресс.Прогресс.Процент > 0 Тогда
			Сообщение = Сообщение + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗаполнитьВозможныеДубли()
	...
	ОбрабатываемыйОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(
		ИдентификаторОбрабатываемогоОбъекта);
	...
	Для Каждого МестоИспользования Из ПредполагаемыеВозможныеДубли Цикл
		...
		Если ОбрабатываемыйОбъектМетаданных = МестоИспользования Или МестоИспользования = Неопределено Тогда
		...
		Если КлючиСвязи.Количество() > 0 Тогда
			ДобавитьВозможныйДубль(ОбрабатываемыйОбъектМетаданных, МестоИспользования, КлючиСвязи,
				ОтборПоискаВозможныхДублей);
		КонецЕсли;
...
Функция СформироватьВозможныеДубли(МестаИспользования)
	...
	Для Каждого МестоИспользования Из МестаИспользования Цикл
		...
		Если МестоИспользования.ВспомогательныеДанные Или МестоИспользования.ЭтоСлужебныеДанные
			Или Не ОбщегоНазначения.ЭтоСсылка(ТипМестаИспользования) Или ИсключаемыеТипы.Найти(ТипМестаИспользования)
			<> Неопределено Тогда

			Продолжить;
		КонецЕсли;

		ОписаниеМетаданных = ?(МестоИспользования.Метаданные <> Неопределено, МестоИспользования.Метаданные,
			МестоИспользования.Данные.Метаданные());
...
Процедура ДобавитьВозможныйДубль(ОбрабатываемыйОбъектМетаданных, МестоИспользования, КлючиСвязи, Отбор = Неопределено)
	...
	ТекущийПорядковыйНомер = ?(ВозможныеДубли.Количество(), ВозможныеДубли[ВозможныеДубли.Количество() - 1].Порядок
		+ 1, 0);
	...
	ВозможныйДубль.АдресОтбора = ПоместитьВоВременноеХранилище(ИнформацияОбОтбореВозможныхДублей,
...
Функция КлючиСвязиМетаданных(Знач ИдентификаторОбрабатываемогоОбъекта, Знач МестоИспользования)
	...
	Для Каждого РеквизитМестаИспользования Из МестоИспользования.Реквизиты Цикл
		...
		Если РеквизитМестаИспользования.Тип.Типы().Найти(ТипЗнч(
			ИдентификаторОбрабатываемогоОбъекта.ЗначениеПустойСсылки)) <> Неопределено Тогда
...
Процедура ОтобратьВыбранныеМестаИспользования(ВыбранныйТип)
	...
	АдресВыбранныхМестИспользования = ПоместитьВоВременноеХранилище(ВыбранныеМестаИспользования,
		УникальныйИдентификатор);
