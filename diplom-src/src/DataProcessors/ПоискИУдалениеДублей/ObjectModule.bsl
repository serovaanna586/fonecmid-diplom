...
Функция ГруппыДублей(Знач ПараметрыПоиска, Знач ЭталонныйОбъект = Неопределено) Экспорт
	...
	РазмерВозвращаемойПорции = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПоиска,
	...
	РассчитыватьМестаИспользования = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПоиска,
	...
	СкрыватьНезначимыеДубли = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПоиска,
		"РассчитыватьМестаИспользования");
	...
	КонецЕсли;
	ИменаПолейВВыборе  = СтрРазделить(ИменаПолейДляСравненияНаРавенство + "," + ИменаПолейДляСравненияНаПодобие, ",",
		Ложь);
	...
	Пока СледующийЭлементВыборки(ВыборкаЭталонныхОбъектов) Цикл
		...
		Если ПоляДляСравненияНаПодобие.Количество() > 0 Тогда
			...
			Если НечеткийПоиск = Неопределено Тогда
				Результат.ОписаниеОшибки = НСтр(
					"ru = 'Не удалось подключить компоненту нечеткого поиска. Подробнее см. в журнале регистрации.'");
				Возврат Результат;
			КонецЕсли;
					...
					Если ЕстьПравилаПоиска Тогда
						...
						Если ТаблицаКандидатов.Количество() = КоличествоЭлементовДляСравнения Тогда
							ЗарегистрироватьДублиПоПрикладнымПравилам(КоллекцияДублей, ПолноеИмяОбъектаМетаданных,
								МенеджерОбластиПоиска, ЭталонныйЭлемент, ТаблицаКандидатов, СтруктураПолей,
...
//
Процедура ЗарегистрироватьДублиПоПрикладнымПравилам(СтрокиДереваРезультата, Знач ПолноеИмяОбъектаМетаданных,
	Знач МенеджерОбластиПоиска, Знач ОсновныеДанные, Знач ТаблицаКандидатов, Знач СтруктураПолей,
	...
	КонецЕсли;
	ПоискИУдалениеДублейПереопределяемый.ПриПоискеДублей(ПолноеИмяОбъектаМетаданных, ТаблицаКандидатов,
		ДополнительныеПараметры);
...
Функция ДобавитьСтрокуКандидатов(ТаблицаКандидатов, Знач ДанныеОсновногоЭлемента, Знач ДанныеКандидата,
	...
	Строка.Поля1 = Новый Структура("Код, Наименование, ПометкаУдаления", ДанныеОсновногоЭлемента.Код,
		ДанныеОсновногоЭлемента.Наименование, ДанныеОсновногоЭлемента.ПометкаУдаления);
	Строка.Поля2 = Новый Структура("Код, Наименование, ПометкаУдаления", ДанныеКандидата.Код,
		ДанныеКандидата.Наименование, ДанныеКандидата.ПометкаУдаления);
...
Процедура ЗарегистрироватьДубль(КоллекцияДублей, Знач Элемент1, Знач Элемент2, Знач СтруктураПолей)
	...
	Если Дубль1Зарегистрирован И Дубль2Зарегистрирован Или СтрокаНезначимыхДублей1 <> Неопределено
	...
	СписокСвойств = "Ссылка,Код,Наименование,ПометкаУдаления," + СтруктураПолей.СписокПолейИдентичности + ","
	...
	Если ТаблицаДублей.Количество() - ДублиКПроверке.Количество() <= КоллекцияДублей.КоличествоЭлементовДляСравнения Тогда
		...
		Для Сч = 0 По ДублиКПроверке.ВГраница() Цикл
			...
			Для Каждого Дубль Из ПроверяемыеДубли Цикл
				...
				МестаИспользованияВнутренний = МестаИспользования.НайтиСтроки(
					Новый Структура("Ссылка, ВспомогательныеДанные", Дубль.Ссылка, Ложь));
...
Функция КоллекцияДублей(Знач ПараметрыПоиска, ПоляДляСравненияНаПодобие, ПоляДляСравненияНаРавенство)
	...
	КоллекцияДублей.Вставить("КоличествоЭлементовДляСравнения", ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыПоиска, "МаксимальноеЧислоДублей", 0));
	КоллекцияДублей.Вставить("ДублиКПроверке", Новый Массив);

	КоллекцияДублей.Вставить("СкрыватьНезначимыеДубли", Ложь);
	Если ПараметрыПоиска.Свойство("СкрыватьНезначимыеДубли") И ПараметрыПоиска.СкрыватьНезначимыеДубли = Истина
