...
Процедура ПользовательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОтбора = Новый Структура("РежимВыбора, СкрываемыеПользователи", Истина, СкрываемыеПользователи);
...
Процедура ОткрытьФормуВыбораПользователей(ВыбранныйТипПользователей, ПараметрыОтбора)
	...
	Если ВыбранныйТипПользователей = "Пользователи" Или ТипПереданногоПользователя = Тип(
	...
	ИначеЕсли ВыбранныйТипПользователей = "ВнешниеПользователи" Или ТипПереданногоПользователя = Тип(
...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если ПользовательСсылка <> Неопределено И ИмяПользователяИБ(ПользовательСсылка) = Неопределено Тогда
	...
	Если ПользовательСсылка <> Неопределено И ПользователиПолучателиНастроек.Свойство("МассивПользователей") Тогда
...
Процедура Скопировать(Команда)
	...
	Если РезультатПроверки = "ЕстьАктивныеПользователиПолучатели" Или (ЗначениеЗаполнено(ОткрытыеКопируемыеФормы)
		И РезультатПроверки = "ТекущийПользовательПолучатель") Тогда

		Если ПереключательКопируемыеНастройки = "СкопироватьВсе" Или (ПереключательКопируемыеНастройки = "СкопироватьОтдельные"
			...
			ПараметрыФормы.Вставить("ЕстьАктивныеПользователиПолучатели", РезультатПроверки
				= "ЕстьАктивныеПользователиПолучатели");
...
Функция ОткрытыеКопируемыеФормы()
	...
	Для Каждого ОткрытоеОкно Из ОткрытыеОкна Цикл
		...
		Если СтрНайти(ИмяОткрытойФормы, "Обработка.НастройкиПользователей") > 0 Или СтрНайти(ИмяОткрытойФормы,
			".ПанельАдминистрированияБСП.") > 0 Тогда
			Продолжить;
		КонецЕсли;

		Если СтрНайти(СтрокаВсеКопируемыеНастройки, ИмяОткрытойФормы) > 0 Тогда
			СтрокаОткрытыеФормы = ?(ЗначениеЗаполнено(СтрокаОткрытыеФормы), СтрокаОткрытыеФормы + Символы.ПС + "- "
				+ ОткрытоеОкно.Заголовок, НСтр("ru = 'Открытые окна'") + ":" + Символы.ПС + "- "
				+ ОткрытоеОкно.Заголовок);
		КонецЕсли
		;
...
Процедура СкопироватьПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("Действие") И Результат.Действие
		= "СкопироватьИЗакрыть" Тогда
...
Процедура ВыбратьНастройкиПослеВыбора(Параметр, Контекст) Экспорт
	...
	Иначе
		ТекстЗаголовка = Формат(КоличествоНастроек, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНастроек, "", НСтр(
			"ru = 'настройка,настройки,настроек,,,,,,0'"));
	КонецЕсли;
	...
	Иначе
		...
		Если Не НастройкиСкопированы Тогда
			...
			ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
				"ru = 'Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.'"),
...
Процедура СкопироватьВыбранныеНастройки(Отчет)
	...
	ИначеЕсли ПереключательКомуКопироватьНастройки = "ВсемПользователям" Тогда
		...
		Обработки.НастройкиПользователей.ПользователиДляКопирования(ПользовательСсылка, ТаблицаПользователей, ТипЗнч(
			ПользовательСсылка) = Тип("СправочникСсылка.ВнешниеПользователи"));
	...
	Если ВыбранныеНастройки.НастройкиОтчетов.Количество() > 0 Тогда
		...
		Обработки.НастройкиПользователей.СкопироватьНастройкиОтчетовИПерсональныеНастройки(
			ХранилищеПользовательскихНастроекОтчетов, Пользователь, Приемники, ВыбранныеНастройки.НастройкиОтчетов,
	...
	Если ВыбранныеНастройки.ВнешнийВид.Количество() > 0 Тогда
		Обработки.НастройкиПользователей.СкопироватьНастройкиВнешнегоВида(Пользователь, Приемники,
			ВыбранныеНастройки.ВнешнийВид);
	КонецЕсли;

	Если ВыбранныеНастройки.ПрочиеНастройки.Количество() > 0 Тогда
		Обработки.НастройкиПользователей.СкопироватьНастройкиВнешнегоВида(Пользователь, Приемники,
			ВыбранныеНастройки.ПрочиеНастройки);
	КонецЕсли;
...
Функция КопированиеВсехНастроек()
	...
	Иначе
		...
		ТаблицаПользователей.Колонки.Добавить("Подразделение");
		ТаблицаПользователей.Колонки.Добавить("ФизическоеЛицо");
		ТаблицаПользователей = Обработки.НастройкиПользователей.ПользователиДляКопирования(ПользовательСсылка,
			ТаблицаПользователей, ТипЗнч(ПользовательСсылка) = Тип("СправочникСсылка.ВнешниеПользователи"));
	...
	НастройкиСкопированы = Обработки.НастройкиПользователей.КопированиеНастроекПользователей(ПользовательСсылка,
		Приемники, КопируемыеНастройки);
...
Процедура ПроверитьАктивныхПользователей()
	...
	Если МассивПользователей.Количество() = 1 И МассивПользователей[0] = ТекущийПользователь Тогда
