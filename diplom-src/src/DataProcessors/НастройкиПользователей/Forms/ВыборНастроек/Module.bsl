...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ДанныеВХранилищеНастроекСохранены И ТипЗнч(ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") Тогда
...
Процедура Выбрать(Команда)
	...
	КоличествоНастроек = ВыбранныеНастройкиВнешнегоВида.КоличествоНастроек + ВыбранныеНастройкиОтчетов.КоличествоНастроек
	...
	Результат.Вставить("ПрочиеПользовательскиеНастройки",
		ВыбранныеПрочиеНастройкиСтруктура.ПрочиеПользовательскиеНастройки);
...
Процедура ЗаполнитьНастройки()
	...
	Если ПоискНастроек Или Не НачальныеНастройкиЗагружены И ВсеВыбранныеНастройки <> Неопределено Тогда
...
Процедура ОбновитьЗаголовокСтраницы(КоличествоНастроек)
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ВнешнийВидСтраница" Тогда
		...
		Иначе
			...
			ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗаголовка,
				КоличествоНастроекВнешнегоВида);
		КонецЕсли;
...
Процедура ЗаполнитьПомеченныеЭлементыДерева()
	...
	Иначе
		...
		ВсеВыбранныеНастройки.ПомеченныеНастройкиОтчетов = ПомеченныеПослеСравненияНастройки(
			ПомеченныеНастройкиОтчетов, НастройкиОтчетовДерево, "НастройкиОтчетов");
		ВсеВыбранныеНастройки.ПомеченныеНастройкиВнешнегоВида = ПомеченныеПослеСравненияНастройки(
			ПомеченныеНастройкиВнешнегоВида, ВнешнийВидДерево, "ВнешнийВид");
		ВсеВыбранныеНастройки.ПомеченныеПрочиеНастройки = ПомеченныеПослеСравненияНастройки(ПомеченныеПрочиеНастройки,
			ПрочиеНастройкиДерево, "ПрочиеНастройки");
...
Процедура ЗагрузитьЗначенияПометок(ДеревоЗначений, ПомеченныеНастройки, ВидНастроек)
	...
	Если КоличествоПомеченных > 0 Тогда
		...
		Если ВидНастроек = "НастройкиОтчетов" Тогда
			...
			Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Настройки отчетов (%1)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ВнешнийВид" Тогда
			КоличествоНастроекВнешнегоВида = КоличествоПомеченных;
			Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Внешний вид (%1)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ПрочиеНастройки" Тогда
			КоличествоПрочихНастроек = КоличествоПомеченных;
			Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Прочие настройки (%1)'"), КоличествоПомеченных);
		КонецЕсли;
