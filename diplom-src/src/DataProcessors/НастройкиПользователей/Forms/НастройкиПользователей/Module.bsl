...
Процедура ПользовательСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИспользоватьВнешнихПользователей Тогда
		...
		ВыборТипаПользователей.ПоказатьВыборЭлемента(
			Новый ОписаниеОповещения("ПользовательСсылкаНачалоВыбораЗавершение", ЭтотОбъект, ПараметрыОтбора));
	Иначе
...
Процедура ДеревоНастроекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ОткрытьОтчетИлиФорму(ТекущийЭлемент, ПользовательИнформационнойБазы,
		ТекущийПользовательИнформационнойБазы, ИмяФормыПерсональныхНастроек);
	...
	Если РезультатПолученияНастроек() = "НетПользователяИБ" Тогда
		...
		ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У выбранного пользователя ""%1"" не указан
				 ...
				 |Устранить проблему можно в карточке данного пользователя.'"), ПользовательСсылка));
		Возврат;
	КонецЕсли;
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("ПрочиеПользовательскиеНастройки", НСтр(
		"ru = 'Настройки дополнительных отчетов и обработок'"));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?'"), Строка(
		ПользовательСсылка));
...
Процедура ОчиститьУстаревшиеНастройкиВсехПользователей(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Сейчас будут очищены устаревшие настройки всех пользователей.
						|Продолжить?'");
...
Процедура СкопироватьНаСервере(ПользователиПриемник, ЛичныхНастроекОтчетов, Отчет)
	...
	Если ВыбраннаяСтраницаНастроек = "НастройкиОтчетовСтраница" Тогда
		...
		Обработки.НастройкиПользователей.СкопироватьНастройкиОтчетовИПерсональныеНастройки(
			ХранилищеПользовательскихНастроекОтчетов, ПользовательИнформационнойБазы, ПользователиПриемник,
			Результат.МассивНастроек, НеСкопированныеНастройкиОтчетов);
		...
		Если НеСкопированныеНастройкиОтчетов.Количество() <> 0 Или ТаблицаПользовательскихВариантовОтчетов.Количество()
			<> 0 Тогда
	...
	Иначе
		...
		Если Результат.МассивПерсональныхНастроек.Количество() <> 0 Тогда
			Обработки.НастройкиПользователей.СкопироватьНастройкиОтчетовИПерсональныеНастройки(
				ХранилищеОбщихНастроек, ПользовательИнформационнойБазы, ПользователиПриемник,
...
Процедура ОчиститьНаСервере(Пользователи = Неопределено, ВыбраннымПользователям = Ложь)
	...
	Если ВыбраннымПользователям Тогда
		...
		Обработки.НастройкиПользователей.УдалитьНастройкиВыбраннымПользователям(Пользователи, Результат.МассивНастроек,
			НазваниеХранилища);
	...
	Обработки.НастройкиПользователей.УдалитьВыбранныеНастройки(СведенияОПользователе, Результат.МассивНастроек,
		НазваниеХранилища);
...
Процедура ОчиститьВсеНастройкиНаСервере(ОчищаемыеНастройки, ОчиститьВсе = Ложь)
	...
	Обработки.НастройкиПользователей.УдалитьНастройкиПользователей(ОчищаемыеНастройки, МассивПользователей,
		ТаблицаПользовательскихВариантовОтчетов, ОчиститьВсе);
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек'"), , НСтр(
		"ru = 'Очищены все настройки всех пользователя'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьУстаревшиеНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек'"), , НСтр(
		"ru = 'Очищены устаревшие настройки всех пользователя'"), БиблиотекаКартинок.Информация32);
...
Функция КоличествоСкопированныхИлиУдаленныхНастроек(ДеревоНастроек)
	...
	Для Каждого ВыделеннаяСтрока Из СписокВыделенныхСтрок Цикл
		...
		ИначеЕсли КоличествоПодчиненныхЭлементов = 0 И РодительЭлемента = Неопределено Тогда
			КоличествоНастроек = КоличествоНастроек + 1;
		Иначе
...
&НаКлиенте
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено,
	КоличествоПользователей = Неопределено)

	ПрописьПредмета = Формат(КоличествоНастроек, "ЧДЦ=0") + " " + ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(
		КоличествоНастроек, "", НСтр("ru = 'настройка,настройки,настроек,,,,,,0'"));

	Если КоличествоНастроек = 1 И КоличествоПользователей = Неопределено Тогда
...
Процедура СкопироватьНастройки()
	...
	ДеревоНастроек = ТаблицаФормыВыбраннойСтраницыНастроек();
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'Выберите настройки, которые требуется скопировать.'"));
		Возврат;
...
Функция ХранилищеНастроекДляВыбраннойСтраницы()
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ВнешнийВидСтраница" Или ВыбраннаяСтраницаНастроек
		= "ПрочиеНастройкиСтраница" Тогда
...
Функция ВыделенныеНастройки()
	...
	Для Каждого ВыделенныйЭлемент Из ВыделенныеЭлементы Цикл
		...
		Если ВыделеннаяНастройка.Тип = "НастройкаЛичная" Тогда
			...
			Если ТекущийВариантОтчета <> Неопределено И ТекущийВариантОтчета.НайтиПоЗначению(
				ВыделеннаяНастройка.Ключи[0].Значение) <> Неопределено Тогда
...
Функция ПараметрыДлительнойОперации()
	...
	ПараметрыДлительнойОперации.Вставить("ПользовательскиеВариантыОтчетов", РеквизитФормыВЗначение(
		"ТаблицаПользовательскихВариантовОтчетов"));
