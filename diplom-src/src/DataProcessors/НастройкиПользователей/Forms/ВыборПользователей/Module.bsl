...
Процедура Выбрать(Команда)
	...
	Результат = Новый Структура("ПользователиПриемник, КопироватьВсе, ОчисткаНастроек", ПользователиПриемник,
		КопироватьВсе, ОчисткаНастроек);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",
		Новый ПолеКомпоновкиДанных("ГруппыПользователей.НаименованиеГруппыИПомеченоПользователей"));
...
Процедура ОбновитьЗаголовокГруппы(Форма, ГруппаПользователей, СтрокаСпискаПользователей, ЗначениеПометки)
	...
	Если Форма.ПоказыватьПользователейДочернихГрупп Или Форма.ГруппаВсеПользователи = ГруппаПользователей.Группа Тогда
	...
	Если ПомеченныйПользователь <> Неопределено И ЗначениеПометки <> СтрокаСпискаПользователей.Пометка Тогда
		...
		ГруппаПользователей.ПомеченоПользователей = ?(ЗначениеПометки, ПомеченоПользователей + 1, ПомеченоПользователей
			- 1);
		ГруппаПользователей.НаименованиеГруппыИПомеченоПользователей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (%2)'"), Строка(ГруппаПользователей.Группа), ГруппаПользователей.ПомеченоПользователей);
	КонецЕсли;
...
Процедура ПрименитьФильтрГрупп(ТекущаяГруппа)
	...
	Для Каждого Элемент Из СписокВсехПользователей Цикл
		Если СоставГруппы.НайтиПоЗначению(Элемент.Пользователь) <> Неопределено Или ГруппаВсеПользователи
			= ТекущаяГруппа.Группа Тогда
