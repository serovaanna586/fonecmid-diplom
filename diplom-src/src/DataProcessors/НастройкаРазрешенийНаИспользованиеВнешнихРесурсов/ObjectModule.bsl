...
//  Имя - Строка - имя профиля безопасности.
//
Процедура ДобавитьОперациюАдминистрирования(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля,
	Знач Операция, Знач Имя) Экспорт
...
//
Процедура ДобавитьЗапросРазрешенийНаИспользованиеВнешнихРесурсов(Знач ТипПрограммногоМодуля,
	Знач ИдентификаторПрограммногоМодуля, Знач ТипВладельца, Знач ИдентификаторВладельца, Знач РежимЗамещения,
	Знач ДобавляемыеРазрешения = Неопределено, Знач УдаляемыеРазрешения = Неопределено) Экспорт
	...
	Если Строка = Неопределено Тогда
		...
		ДобавитьОперациюАдминистрирования(
			ТипПрограммногоМодуля, ИдентификаторПрограммногоМодуля,
			Перечисления.ОперацииАдминистрированияПрофилейБезопасности.Обновление, Имя);
	...
	Если ДобавляемыеРазрешения <> Неопределено Тогда
		...
		Для Каждого ДобавляемоеРазрешение Из ДобавляемыеРазрешения Цикл
			...
			КлючРазрешения = РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения(
				ДобавляемоеРазрешение);
			ДополнениеРазрешения = РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.ДополнениеРазрешения(
				ДобавляемоеРазрешение);
			...
			Если ЗначениеЗаполнено(ДополнениеРазрешения) Тогда
				Строка.ДополненияРазрешений.Вставить(КлючРазрешения, ОбщегоНазначения.ЗначениеВСтрокуXML(
					ДополнениеРазрешения));
			КонецЕсли;
...
Процедура РассчитатьПрименениеЗапросов() Экспорт
	...
	Попытка
		...
		ИсходныйСрезРазрешенийБезУчетаВладельцев = РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.СрезРазрешений(
			Ложь, Истина);
...
Функция Представление(Знач КакТребуемые = Ложь) Экспорт
	...
	Возврат Отчеты.ИспользуемыеВнешниеРесурсы.ПредставлениеЗапросовРазрешенийНаИспользованиеВнешнихРесурсов(
		ОперацииАдминистрирования, ДельтаБезУчетаВладельцев.Добавляемые, ДельтаБезУчетаВладельцев.Удаляемые,
		КакТребуемые);
...
Функция СценарийПрименения() Экспорт
	...
	Для Каждого Описание Из ОперацииАдминистрирования Цикл
		...
		ЭлементРезультата.Разрешения = ПрофильВНотацииИнтерфейсаАдминистрированияКластера(ЭлементРезультата.Профиль,
			Описание.ТипПрограммногоМодуля, Описание.ИдентификаторПрограммногоМодуля);

		ЭтоПрофильКонфигурации = (Описание.ТипПрограммногоМодуля
			...
			Для Каждого Удаляемые Из ДельтаВРазрезеВладельцев.Удаляемые Цикл
				...
				Для Каждого КлючИЗначение Из Удаляемые.Разрешения Цикл
					...
					РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.УдалитьРазрешение(
						Удаляемые.ТипПрограммногоМодуля, Удаляемые.ИдентификаторПрограммногоМодуля,
						Удаляемые.ТипВладельца, Удаляемые.ИдентификаторВладельца, КлючИЗначение.Ключ,
			...
			Для Каждого Добавляемые Из ДельтаВРазрезеВладельцев.Добавляемые Цикл
				...
				Для Каждого КлючИЗначение Из Добавляемые.Разрешения Цикл
					...
					РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.ДобавитьРазрешение(
						Добавляемые.ТипПрограммногоМодуля, Добавляемые.ИдентификаторПрограммногоМодуля,
						Добавляемые.ТипВладельца, Добавляемые.ИдентификаторВладельца, КлючИЗначение.Ключ,
						ОбщегоНазначения.ОбъектXDTOИзСтрокиXML(КлючИЗначение.Значение), Дополнение);
	...
	Для Каждого СтрокаДобавленных Из ПланПримененияЗапросов.Добавляемые Цикл
		...
		Для Каждого КлючИЗначение Из СтрокаДобавленных.Разрешения Цикл
			...
			Если СтрокаДобавленных.ДополненияРазрешений.Получить(КлючИЗначение.Ключ) <> Неопределено Тогда
				Строка.ДополненияРазрешений.Вставить(КлючИЗначение.Ключ,
					СтрокаДобавленных.ДополненияРазрешений.Получить(КлючИЗначение.Ключ));
			КонецЕсли;
	...
	Для Каждого СтрокаУдаляемых Из ПланПримененияЗапросов.Удаляемые Цикл
		...
		Для Каждого КлючИЗначение Из СтрокаУдаляемых.Разрешения Цикл
			...
			Если СтрокаУдаляемых.ДополненияРазрешений.Получить(КлючИЗначение.Ключ) <> Неопределено Тогда
				...
				Строка.ДополненияРазрешений.Вставить(КлючИЗначение.Ключ, СтрокаУдаляемых.ДополненияРазрешений.Получить(
					КлючИЗначение.Ключ));
				...
				Если СтрокаРезультата.Тип = "FileSystemAccess" Тогда
					...
					Если ИсходноеРазрешение.AllowedRead Тогда
						...
						Если ИсходноеРазрешение.AllowedWrite Тогда
							...
							КопияРазрешения = ОбщегоНазначения.ОбъектXDTOИзСтрокиXML(
								ОбщегоНазначения.ОбъектXDTOВСтрокуXML(ИсходноеРазрешение));
							КопияРазрешения.AllowedWrite = Ложь;
							КлючКопии = РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения(
								КопияРазрешения);
...
Процедура РассчитатьДельтуВРазрезеВладельцев()
	...
	ДельтаВРазрезеВладельцев.Добавляемые.Колонки.Добавить("ТипПрограммногоМодуля",
		Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	ДельтаВРазрезеВладельцев.Добавляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
		Новый ОписаниеТипов("УникальныйИдентификатор"));
	ДельтаВРазрезеВладельцев.Добавляемые.Колонки.Добавить("ТипВладельца",
		Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	ДельтаВРазрезеВладельцев.Добавляемые.Колонки.Добавить("ИдентификаторВладельца",
		Новый ОписаниеТипов("УникальныйИдентификатор"));
	...
	ДельтаВРазрезеВладельцев.Удаляемые.Колонки.Добавить("ТипПрограммногоМодуля",
		Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	ДельтаВРазрезеВладельцев.Удаляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
		Новый ОписаниеТипов("УникальныйИдентификатор"));
	ДельтаВРазрезеВладельцев.Удаляемые.Колонки.Добавить("ТипВладельца",
		Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	ДельтаВРазрезеВладельцев.Удаляемые.Колонки.Добавить("ИдентификаторВладельца",
		Новый ОписаниеТипов("УникальныйИдентификатор"));
	...
	Для Каждого Строка Из ИсходныйСрезРазрешенийВРазрезеВладельцев Цикл
		...
		Для Каждого КлючИЗначение Из Строка.Разрешения Цикл
			...
			Если СтрокаРезультата = Неопределено Или СтрокаРезультата.Разрешения.Получить(КлючИЗначение.Ключ)
				= Неопределено Тогда
				...
				Если СтрокаУдаляемых.Разрешения.Получить(КлючИЗначение.Ключ) = Неопределено Тогда
					...
					Если Строка.ДополненияРазрешений.Получить(КлючИЗначение.Ключ) <> Неопределено Тогда
						СтрокаУдаляемых.ДополненияРазрешений.Вставить(КлючИЗначение.Ключ,
							Строка.ДополненияРазрешений.Получить(КлючИЗначение.Ключ));
					КонецЕсли;
	...
	Для Каждого Строка Из РезультатПримененияЗапросовВРазрезеВладельцев Цикл
		...
		Для Каждого КлючИЗначение Из Строка.Разрешения Цикл
			...
			Если ИсходнаяСтрока = Неопределено Или ИсходнаяСтрока.Разрешения.Получить(КлючИЗначение.Ключ)
				= Неопределено Тогда
				...
				Если СтрокаДобавляемых.Разрешения.Получить(КлючИЗначение.Ключ) = Неопределено Тогда
					...
					Если Строка.ДополненияРазрешений.Получить(КлючИЗначение.Ключ) <> Неопределено Тогда
						СтрокаДобавляемых.ДополненияРазрешений.Вставить(КлючИЗначение.Ключ,
							Строка.ДополненияРазрешений.Получить(КлючИЗначение.Ключ));
					КонецЕсли;
...
Процедура РассчитатьДельтуБезУчетаВладельцев()
	...
	ДельтаБезУчетаВладельцев.Добавляемые.Колонки.Добавить("ТипПрограммногоМодуля",
		Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	ДельтаБезУчетаВладельцев.Добавляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
		Новый ОписаниеТипов("УникальныйИдентификатор"));
	...
	ДельтаБезУчетаВладельцев.Удаляемые.Колонки.Добавить("ТипПрограммногоМодуля",
		Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	ДельтаБезУчетаВладельцев.Удаляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
		Новый ОписаниеТипов("УникальныйИдентификатор"));
...
//
Функция ПрофильВНотацииИнтерфейсаАдминистрированияКластера(Знач ИмяПрофиля, Знач ТипПрограммногоМодуля,
	Знач ИдентификаторПрограммногоМодуля)
	...
	Для Каждого Строка Из Строки Цикл
		...
		Для Каждого КлючИЗначение Из Строка.Разрешения Цикл
			...
			ИначеЕсли Строка.Тип = "AttachAddin" Тогда
				...
				Дополнение = ОбщегоНазначения.ЗначениеИзСтрокиXML(Строка.ДополненияРазрешений.Получить(
					КлючИЗначение.Ключ));
...
Функция НовоеОписаниеПрофиляБезопасности(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля)
	...
	Если ТипПрограммногоМодуля = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяИБ, НСтр(
			"ru = 'Профиль безопасности для информационной базы'"), СтрокаСоединенияИнформационнойБазы());
	Иначе
		ПрограммныйМодуль = РаботаВБезопасномРежимеСлужебный.СсылкаИзРегистраРазрешений(ТипПрограммногоМодуля,
			ИдентификаторПрограммногоМодуля);
		Словарь = РаботаВБезопасномРежимеСлужебный.МенеджерВнешнегоМодуля(
			ПрограммныйМодуль).СловарьКонтейнераВнешнегоМодуля();
		НаименованиеМодуля = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрограммныйМодуль, "Наименование");
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяИБ, Словарь.Именительный,
			НаименованиеМодуля);
	КонецЕсли;
...
ПланПримененияЗапросов.Замещаемые.Колонки.Добавить("ТипПрограммногоМодуля",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ПланПримененияЗапросов.Замещаемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
	Новый ОписаниеТипов("УникальныйИдентификатор"));
ПланПримененияЗапросов.Замещаемые.Колонки.Добавить("ТипВладельца",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ПланПримененияЗапросов.Замещаемые.Колонки.Добавить("ИдентификаторВладельца",
	Новый ОписаниеТипов("УникальныйИдентификатор"));

ПланПримененияЗапросов.Вставить("Добавляемые", Новый ТаблицаЗначений);
ПланПримененияЗапросов.Добавляемые.Колонки.Добавить("ТипПрограммногоМодуля",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ПланПримененияЗапросов.Добавляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
	Новый ОписаниеТипов("УникальныйИдентификатор"));
ПланПримененияЗапросов.Добавляемые.Колонки.Добавить("ТипВладельца",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ПланПримененияЗапросов.Добавляемые.Колонки.Добавить("ИдентификаторВладельца",
	Новый ОписаниеТипов("УникальныйИдентификатор"));
...
ПланПримененияЗапросов.Удаляемые.Колонки.Добавить("ТипПрограммногоМодуля",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ПланПримененияЗапросов.Удаляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
	Новый ОписаниеТипов("УникальныйИдентификатор"));
ПланПримененияЗапросов.Удаляемые.Колонки.Добавить("ТипВладельца",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ПланПримененияЗапросов.Удаляемые.Колонки.Добавить("ИдентификаторВладельца",
	Новый ОписаниеТипов("УникальныйИдентификатор"));
...
ОперацииАдминистрирования.Колонки.Добавить("ТипПрограммногоМодуля",
	Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
ОперацииАдминистрирования.Колонки.Добавить("ИдентификаторПрограммногоМодуля",
	Новый ОписаниеТипов("УникальныйИдентификатор"));
ОперацииАдминистрирования.Колонки.Добавить("Операция",
	Новый ОписаниеТипов("ПеречислениеСсылка.ОперацииАдминистрированияПрофилейБезопасности"));
