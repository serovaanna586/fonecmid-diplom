...
Функция ВыполнитьПереносФайлов(ФайлыДляПереноса, Параметры) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Начало переноса файлов.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация, , , НСтр("ru = 'Начат перенос файлов'") + " " + ДействиеДляЖурнала);
...
Процедура ПеренестиФайл(Файл, СвойстваТома, Параметры, ОшибкиПереноса, ПеренесеноФайлов)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка переноса файла.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , ФайлСсылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При переносе в том файла
					 |""%1""
					 |возникла ошибка:
					 |""%2"".'"), ИмяДляЖурнала, Ошибка.ПодробноеПредставлениеОшибки));
...
Процедура ПеренестиМеждуТомами(ФайлОбъект, СвойстваТома, Параметры)
	...
	НовыйПутьКФайлу = РаботаСФайламиВТомахСлужебный.ПолноеИмяФайлаВТоме(СвойстваФайла,
...
Процедура ПеренестиВТома(ФайлОбъект, СвойстваТома, Параметры)
	...
	РаботаСФайламиВТомахСлужебный.ДобавитьФайл(ФайлОбъект, ДанныеФайла, ФайлОбъект.ДатаМодификацииУниверсальная, , ?(
		Параметры.ПереноситьВТом, Параметры.ТомХраненияПриемник, Неопределено));
...
Процедура ВыброситьИсключениеПриПревышенииРазмераТома(Знач СвойстваТома, Знач ФайлОбъект, Знач Параметры)
	...
	Если Параметры.ПереноситьВТом И СвойстваТома.МаксимальныйОбъемТома > 0 И СвойстваТома.ТекущийОбъемТома
		+ ФайлОбъект.Размер > СвойстваТома.МаксимальныйОбъемТома Тогда
