...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ХранитьФайлыВТомахНаДиске И КоличествоТомовХранения > 0 Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если КоличествоТомовХранения = 0 Тогда
		...
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр(
			"ru = 'Невозможно перенести файлы между томами, т.к. нет ни одного тома для размещения файлов.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДействиеПриИзменении(Элемент)
	...
	Если Действие = "ПеренестиМеждуТомами" И КоличествоТомовХранения = 1 Тогда

		Действие = "ПеренестиВТома";
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Невозможно перенести файлы между томами: в настройках указан один том для размещения файлов.'"));
	КонецЕсли;
...
Процедура ВыполнитьПереносЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатПереноса.ОшибкиПереноса.Количество() = 0 Тогда
		Если РезультатПереноса.ПеренесеноФайлов > 0 Тогда
			Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Завершен перенос файлов. Перенесено файлов %1.'"), Формат(РезультатПереноса.ПеренесеноФайлов,
				"ЧН=0; ЧГ="));
		Иначе
			...
			Если ДополнительныеУсловия.Настройки.Отбор.Элементы.Количество() > 1 Тогда
				Пояснение = Пояснение + Символы.ПС + НСтр(
					"ru = 'Обратите внимание, что дополнительные условия объединяются по ""И"", если они не сгруппированы в группу ""Или"".'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение( , Пояснение);
	Иначе
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Перенесено файлов: %1, не удалось перенести: %2'"), РезультатПереноса.ПеренесеноФайлов,
			РезультатПереноса.ОшибкиПереноса.Количество());
...
Функция ФайлыДляПереноса()
	...
	СхемаКомпоновкиДанных = СхемаКомпоновки(Ложь, ?(ПереноситьФайлыВладельца И Не ПустаяСтрока(ВладелецФайлов),
		ВладелецФайлов, ""));

	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	КомпоновщикНастроек.ЗагрузитьНастройки(ДополнительныеУсловия.Настройки);

	ОтборПоДействию = ?(Действие = "ПеренестиВТома", Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе,
		Перечисления.ТипыХраненияФайлов.ВТомахНаДиске);

	ДобавитьНастройкуОтбора(КомпоновщикНастроек, "ТипХраненияФайла", ВидСравненияКомпоновкиДанных.Равно,
		ОтборПоДействию);

	Если ПереноситьВТом Тогда
		ДобавитьНастройкуОтбора(КомпоновщикНастроек, "Том", ВидСравненияКомпоновкиДанных.НеРавно, ТомХраненияПриемник);
	КонецЕсли;

	Если ПереноситьИзТома Тогда
		ДобавитьНастройкуОтбора(КомпоновщикНастроек, "Том", ВидСравненияКомпоновкиДанных.Равно, ТомХраненияИсточник);
	КонецЕсли;

	Если ПереноситьПоДатеСоздания Тогда

		ДобавитьНастройкуОтбора(КомпоновщикНастроек, "ДатаСоздания", ?(УточнениеДатыСоздания = "до",
			ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, ВидСравненияКомпоновкиДанных.БольшеИлиРавно), ДатаСоздания);
	КонецЕсли;
	...
	Если ПереноситьПоРасширению Тогда
		...
		ДобавитьНастройкуОтбора(КомпоновщикНастроек, "Расширение", ВидСравненияКомпоновкиДанных.ВСписке,
			ОтборПоРасширению);
...
Функция СхемаКомпоновки(ТолькоСправочникиФайлов = Истина, ВладелецФайлов = "")
	...
	Иначе
		...
		Если Не ПустаяСтрока(ВладелецФайлов) Тогда
			...
			ТипВладельцаСтрокой = Лев(ВладелецФайлов, ПозицияРазделителя - 1) + "Ссылка" + Сред(ВладелецФайлов,
				ПозицияРазделителя);
		КонецЕсли;
		...
		Для Каждого ТипМетаданных Из ТипыМетаданныхВладельцевВерсий.Типы() Цикл
			...
			Если МетаданныеВладельца.Реквизиты.Найти("Служебный") <> Неопределено Тогда
				...
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Служебный",
					"ЕСТЬNULL(ВерсииФайлов.Владелец.Служебный, ЛОЖЬ)");
			...
			Если Не ОтборПоВладельцуУстановлен И МетаданныеВладельца.Реквизиты.ВладелецФайла.Тип.СодержитТип(Тип(
				ТипВладельцаСтрокой)) Тогда
		...
		Если Не ПустаяСтрока(ВладелецФайлов) И Не ОтборПоВладельцуУстановлен Тогда
			ТекстЗапроса = "";
		Иначе
	...
	Для Каждого ТипФайла Из ТипыПрисоединенныхФайлов Цикл
		...
		Если ТипФайла = Тип("СправочникСсылка.ВерсииФайлов") Или ТипФайла = Тип(
			"СправочникСсылка.ИдентификаторыОбъектовМетаданных") Или (Не ТолькоСправочникиФайлов
		...
		ТекстЗапросаВладельца = СтрЗаменить(ТекстЗапросаВладельца, "&Служебный", ?(ОбъектМетаданных.Реквизиты.Найти(
			"Служебный") <> Неопределено, "СправочникФайлов.Служебный", "ЛОЖЬ"));
