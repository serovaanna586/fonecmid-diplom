...
Функция ЗапускОбновленияНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр(
		"ru = 'Фоновое обновление информационной базы под ограниченными правами'");
...
Процедура ПродолжитьОбратныйОтсчет()
	...
	Если ОжиданиеСчетчик <= 0 Тогда
		...
		ПрекратитьРаботуСистемы(Ложь);
	Иначе
		НовыйЗаголовок = (НСтр("ru = 'Завершить работу'") + " ("
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'осталось %1 сек.'"), Строка(
			ОжиданиеСчетчик)) + ")");
