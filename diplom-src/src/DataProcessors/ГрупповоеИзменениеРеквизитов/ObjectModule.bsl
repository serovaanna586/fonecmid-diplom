		...
		Для Каждого ДанныеОбъекта Из ОбрабатываемыеОбъекты Цикл
			...
			Попытка
				...
				Если Параметры.ТипОперации = "ВыполнитьАлгоритм" Тогда
					ВыполнитьКодАлгоритма(ИзменяемыйОбъект, Параметры.КодАлгоритма,
						ВыполнятьКодАлгоритмаВБезопасномРежиме);
				Иначе
				...
				НастроеныИзменения = ЗначениеЗаполнено(Параметры.ИзменяемыеРеквизиты) Или ЗначениеЗаполнено(
					Параметры.ИзменяемыеТабличныеЧасти);
	...
	Для Каждого Операция Из Параметры.ИзменяемыеРеквизиты Цикл
		...
		ИначеЕсли Операция.ВидОперации = 2 Тогда // изменение дополнительного реквизита
			...
			Если ЗначениеЗаполнено(Значение) Тогда
				...
				Если МодульУправлениеСвойствамиСлужебный.ИспользоватьНеограниченнуюСтроку(ОписаниеСвойства.ТипЗначения,
					ОписаниеСвойства.МногострочноеПолеВвода) Тогда
			...
			ИмяРеквизитаФормы = ПрефиксИмениДопРеквизита() + СтрЗаменить(Строка(
				Операция.Свойство.УникальныйИдентификатор()), "-", "_");
		...
		ИначеЕсли Операция.ВидОперации = 3 Тогда // изменение дополнительного сведения
			...
			ИмяРеквизитаФормы = ПрефиксИмениДопСведения() + СтрЗаменить(Строка(
				Операция.Свойство.УникальныйИдентификатор()), "-", "_");
...
Функция СвойствоНужноИзменять(Ссылка, Свойство, Параметры)
	...
	Если (ВидОбъектаПоСсылке = "Справочник" Или ВидОбъектаПоСсылке = "ПланВидовХарактеристик") И ОбъектЯвляетсяГруппой(
		Ссылка) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьРезультатИзмененияДополнительныхСвойств(Результат, Ссылка, ИзменяемыйОбъект, Изменения = Неопределено)
	...
	Если Изменения <> Неопределено Тогда
		...
		КонецЦикла;
	КонецЕсли;
	СостояниеИзменения.Вставить("ЗначенияИзмененныхДопРеквизитов", ?(Изменения <> Неопределено,
		Изменения.ИзменяемыеДопРеквизитыОбъекта, Изменения));
	СостояниеИзменения.Вставить("ЗначенияИзмененныхДопСведений", ?(Изменения <> Неопределено,
		Изменения.ИзменяемыеДопСведенияОбъекта, Изменения));
...
Функция СтрокаСоответствуетОтбору(СтрокаТаблицы, ДанныеОбъекта, ИмяТаблицы)
	...
	Возврат ДанныеОбъекта.Строки.НайтиСтроки(Новый Структура(ИмяТаблицы + "НомерСтроки",
		СтрокаТаблицы.НомерСтроки)).Количество() = 1;
	...
	Для Каждого КоллекцияОбъектовМетаданных Из КоллекцииОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			Если Не ПоказыватьСкрытые Тогда
				Если СтрНачинаетсяС(НРег(ОбъектМетаданных.Имя), ПрефиксУдаляемыхОбъектов) Или ЭтоСлужебныйОбъект(
					ОбъектМетаданных, МенеджерыОбъектов) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
...
Функция ЭтоСлужебныйОбъект(ОбъектМетаданных, МенеджерыОбъектов)
	...
	Если МенеджерыОбъектов <> Неопределено Тогда
		ДоступныеМетоды = МетодыМенеджераОбъектаДляРедактированияРеквизитов(ОбъектМетаданных.ПолноеИмя(),
			МенеджерыОбъектов);
...
Функция ВычислитьВыражение(Знач Выражение, Объект, ДоступныеРеквизиты)
	...
	Для Каждого ОписаниеРеквизита Из ДоступныеРеквизиты Цикл
		...
		Иначе
			...
			Для Каждого СтрокаТаблицы Из ЗначенияСвойств.НайтиСтроки(Новый Структура("Свойство",
				ОписаниеРеквизита.Свойство)) Цикл
				Значение = СтрокаТаблицы.Значение;
			КонецЦикла;
		КонецЕсли;

		Выражение = СтрЗаменить(Выражение, "[" + ОписаниеРеквизита.Представление + "]", """" + СтрЗаменить(СтрЗаменить(
			Значение, """", """"""), Символы.ПС, Символы.ПС + "|") + """");
	КонецЦикла;
