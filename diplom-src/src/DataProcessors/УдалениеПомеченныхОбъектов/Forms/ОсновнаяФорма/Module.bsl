...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПоказатьДиалогПередЗакрытием Тогда
		...
		ПоказатьВопрос(Обработчик, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, НРег(
			ПредставлениеОперации)), Кнопки, 60, КодВозвратаДиалога.Пропустить);
...
Процедура СсылкаПодробнееНажатие(Элемент)
	...
	Иначе
		ТекстОшибки = ?(Элемент.Имя = Элементы.ПодробнееОшибкаФоновогоЗадания.Имя, ПодробныйТекстОшибкиФоновогоЗадания,
...
Процедура КВыборуОбъектов(Команда)
	...
	Если ТаблицаДействий.Количество() > 0 Тогда
		ПоказатьВопрос(Оповещение, НСтр("ru='Не завершено удаление объектов.
										|Вернуться к списку помеченных на удаление?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура УстановитьЗаменитьНаПродолжение(Результат)
	...
	Для Каждого ВыделеннаяСтрока Из МестаИспользованияНеудаленных.ПолучитьЭлементы() Цикл
		Если ВыделеннаяСтрока.ОсновнаяПричина И ВыделеннаяСтрока.Действие <> "Удалить" Тогда
			ЗарегистрироватьДействиеМестаИспользования(ВыделеннаяСтрока, "ЗаменитьСсылку", Результат);
		КонецЕсли;
	КонецЦикла;
...
Функция НастроитьПродолжениеСервер(АдресНастроек)
	...
	НовоеЧислоДополнительныхРеквизитов = УдалениеПомеченныхОбъектовСлужебный.ЧислоДополнительныхРеквизитов(
		Настройки.ВыбранныеРеквизиты);
	ТекущееЧислоСозданныхРеквизитов = ТекущееЧислоСозданныхРеквизитов(ТекущееЧислоДополнительныхРеквизитов,
		НовоеЧислоДополнительныхРеквизитов);
	...
	Если ТекущееЧислоДополнительныхРеквизитов < ТекущееЧислоСозданныхРеквизитов И ТекущееЧислоДополнительныхРеквизитов
		< НовоеЧислоДополнительныхРеквизитов Тогда

		УстановитьВидимостьДополнительныхРеквизитов(ТекущееЧислоДополнительныхРеквизитов + 1,
			ТекущееЧислоСозданныхРеквизитов, Истина);
	КонецЕсли;
...
Процедура УдалитьВыбранные(Команда)
	...
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Удалить элементы, помеченные на удаление (%1)?'"), СведенияОВыбранныхОбъектах.КоличествоВыбранных);
	КонецЕсли;

	ОповещениеПодтверждениеУдалитьВсе = Новый ОписаниеОповещения("ПриПодтвержденииУдалить", ЭтотОбъект,
...
Процедура УстановитьСнятьПометкуУдаления(Команда)
	...
	Иначе
		...
		КонецЕсли;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Формат(
			СсылкиДляОбработки.Количество(), "ЧН=0; ЧГ="));
	КонецЕсли;
...
Процедура УстановитьСнятьПометкуУдаленияПродолжение(Результат, СсылкиДляОбработки) Экспорт
	...
	Если ТипЗнч(Изменения) = Тип("Строка") Тогда
		Ошибка = НСтр("ru='Произошла ошибка при изменении пометки удаления по причине:'") + Символы.ПС + Изменения;
		ПоказатьПредупреждение( , Ошибка, , НСтр("ru='Ошибка при изменении пометки удаления.'"));
	Иначе
...
Функция УстановитьСнятьПометкуУдаленияНаСервере(Ссылки)
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Данные.НомерКартинки = УдалениеПомеченныхОбъектовСлужебный.НомерКартинки(
								Изменение.Ссылка, ИнформацияОТипе.Ссылочный, ИнформацияОТипе.Вид, ?(
...
Процедура УстановитьСостояниеВыбораПомеченныхНаУдаление()
	...
	Элементы.ГруппаАктивныПослеПоискаПомеченные.Доступность = ДеревоПомеченныхНаУдаление.ПолучитьЭлементы().Количество()
		> 0;
...
Процедура ОбновитьДеревоПомеченныхНаУдалениеНаСервере()
	...
	Для Каждого УзелТипа Из ПомеченныеНаУдаление.Строки Цикл
		...
		Если УзелТипа.Строки.Количество() > 0 Тогда
			...
			УзелТипа.Представление = ОбщегоНазначения.ПредставлениеСписка(ОбъектМетаданныхУзла) + " ("
				+ УзелТипа.Строки.Количество() + ")";
		КонецЕсли;
	КонецЦикла;
...
Функция НастройкиИзменились(ВыбранныеРеквизиты)
	...
	ТаблицаСравнения = УдалениеПомеченныхОбъектовСлужебный.ОбъединениеТаблиц(
		ВыбранныеРеквизиты, ДополнительныеРеквизитыПомеченныхНаУдаления.Выгрузить());
...
Процедура ДобавитьДополнительныеРеквизиты(НачальныйНомер, ЧислоДобавляемыхРеквизитов)
	...
	Для Сч = НачальныйНомер По ПоследняяЦифраВИмениРеквизита Цикл
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Реквизит" + Сч, Новый ОписаниеТипов,
			"ДеревоПомеченныхНаУдаление"));
	КонецЦикла;
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);

	Для Сч = НачальныйНомер По ПоследняяЦифраВИмениРеквизита Цикл
		ЭлементФормы = Элементы.Добавить("ДеревоПомеченныхНаУдалениеРеквизит" + Сч, Тип("ПолеФормы"),
			Элементы.ДеревоПомеченныхНаУдаление);
...
Процедура ЗарегистрироватьДействиеМестаИспользования(РедактируемоеМестоИспользования, Действие, Параметр = Неопределено)
	ДействиеПредставление = ?(Параметр = Неопределено, ДействияМестИспользованияНеудаленных.НайтиПоЗначению(
		Действие).Представление, ПредставлениеКомандыЗаменитьНа(РедактируемоеМестоИспользования, Параметр));
	...
	ИначеЕсли Действие = "ЗаменитьСсылку" Тогда
		...
		Фильтр = Новый Структура("Источник, Действие", РедактируемоеМестоИспользования.УдаляемыйСсылка,
			"ЗаменитьСсылку");
		ЗарегистрированноеДействие = ТаблицаДействий.НайтиСтроки(Фильтр);
	...
	Иначе
		...
		СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеПользователю,
			РедактируемоеМестоИспользования.ОбнаруженныйСсылка, РедактируемоеМестоИспользования.ДействиеПредставление);
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Шрифт",
		Метаданные.ЭлементыСтиля.ДействиеВКолонкеСпискаШрифт.Значение);
...
Процедура УстановитьСписокДействийМестИспользования(Форма)
	...
	Форма.ДействияМестИспользованияНеудаленных.Добавить("ЗаменитьСсылку", НСтр(
		"ru='Заменить %ПредставлениеСсылка% на'"));
	...
	Для Каждого ЭлементСписка Из Форма.ДействияМестИспользованияНеудаленных Цикл
		Форма.Элементы.СвязиНеУдаленныхДействие.СписокВыбора.Добавить(ЭлементСписка.Значение,
...
Процедура ЗаполнитьМестаИспользованияНеудаленныхОбъектов(Форма)
	...
	Иначе
		...
		Показываемые = Форма.СвязиНеудаленных.НайтиСтроки(Новый Структура("УдаляемыйСсылка",
			СтрокаДерева.УдаляемыйСсылка));
...
Процедура ЗаполнитьДействиеВСтрокеМестаИспользованияНеудаленных(Форма, Знач ПричинаНеудаления, Действие = "")
	...
	ИначеЕсли Действие = "Удалить" Тогда
		ПричинаНеудаления.ДействиеПредставление = Форма.ДействияМестИспользованияНеудаленных.НайтиПоЗначению(
...
Процедура ДобавитьПричиныНеудаленияРекурсивно(Форма, КоллекцияСтрокРодителя, ДанныеСтроки)
	...
	Если ДанныеСтроки.ЭтоОшибка Тогда
		...
		НоваяСтрока.ОбнаруженныйСсылка = ?(ЗначениеЗаполнено(ДанныеСтроки.Представление), ДанныеСтроки.Представление,
			ДанныеСтроки.ОбнаруженныйСсылка);
	Иначе
	...
	НоваяСтрока.ОсновнаяПричина = Не НоваяСтрока.ЭтоОшибка И (ПодчиненныеСтроки.Количество() = 0 Или СодержатьсяТолькоОшибки
		Или Показываемые.Количество() = 0);

	ЗаполнитьДействиеВСтрокеМестаИспользованияНеудаленных(Форма, НоваяСтрока, ?(Показываемые.Количество() <> 0,
		"Удалить", ""));
...
Процедура МестаИспользованияНеудаленныхПриАктивизацииСтроки(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ОбнаруженныйСсылка) Тогда
		...
		Если ТипЗнч(ТекущиеДанные.ОбнаруженныйСсылка) <> Тип("Строка") Тогда
			Элементы.СвязиНеУдаленныхДействие.СписокВыбора.Добавить("Удалить", ПредставлениеКомандыУдалить(
				ТекущиеДанные));
		КонецЕсли;

		Действия = ТаблицаДействий.НайтиСтроки(Новый Структура("ОбнаруженныйСсылка", ТекущиеДанные.ОбнаруженныйСсылка));
		Если Не (Действия.Количество() > 0 И Действия[0].Действие = "Удалить" И Действия[0].Источник
			<> ТекущиеДанные.УдаляемыйСсылка) Тогда

			Элементы.СвязиНеУдаленныхДействие.СписокВыбора.Добавить("ЗаменитьСсылку", ПредставлениеКомандыЗаменитьНа(
				ТекущиеДанные));
		КонецЕсли;
...
Функция ПредставлениеКомандыЗаменитьНа(ТекущиеДанные, Параметр = Неопределено)
	...
	ПредставлениеКомандыЗаменитьНа = СтрЗаменить(НСтр("ru='Заменить %ПредставлениеСсылка% на'"),
		"%ПредставлениеСсылка%", ПредставлениеСтрокиВерхнегоУровня) + Символы.НПП + ?(Параметр = Неопределено, "",
		Параметр);
...
Процедура ПоказатьОбъектТаблицы(ТаблицаЭлемент)
	...
	Если Не СтрокаТаблицы.Свойство("ОбнаруженныйСсылка", Значение) И Не СтрокаТаблицы.Свойство("УдаляемыйСсылка",
		Значение) Тогда
		Возврат;
	КонецЕсли;

	Если ТипЗнч(Значение) = Тип("Строка") И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтрокаТаблицы,
		"ЭтоОшибка", Ложь) Тогда
...
Процедура НачатьПоискПомеченных_НастройкаФормы(Форма)
	...
	Форма.Элементы.ДекорацияПредставлениеДлительнойОперации.Заголовок = НСтр(
		"ru='Выполняется поиск помеченных на удаление...'");
...
Функция НачатьПоискПомеченныхСервер(ОтборМетаданных, УникальныйИдентификаторФормы)
	...
	Задание = ДлительныеОперации.ВыполнитьФункцию(ПараметрыМетода, ИмяМетода, ОтборМетаданных,
		ДополнительныеРеквизитыПомеченныхНаУдаления.Выгрузить(), РеквизитФормыВЗначение("ДеревоПомеченныхНаУдаление"));
...
Процедура ЗаполнитьИнформациюОшибкамиФоновогоЗадания(Форма, Результат)
	...
	Форма.Элементы.ДекорацияПредставлениеЗавершенияНеУспешно.Заголовок = ?(ЭтоОшибкаУстановкиМонопольногоРежима(
		Результат.КраткоеПредставлениеОшибки), НСтр("ru='Не удалось установить монопольный режим'"),
...
Процедура НачатьУдалениеПомеченных(Параметр = Неопределено)
	...
	ВыполняемаяОперация = НачатьУдалениеПомеченныхСервер(
							УникальныйИдентификатор, РезультатПредыдущегоШага, Параметр = "ПолноеУдаление");
	ПриНачалеФоновогоЗадания(Обработчик);
КонецПроцедуры
	...
	Если МонопольныйРежим() <> МонопольныйРежим Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление помеченных'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, , , Результат.КраткоеПредставлениеОшибки + Символы.ПС + Символы.ПС
...
Функция НачатьУдалениеПомеченныхСервер(УникальныйИдентификаторФормы, РезультатПредыдущегоШага, ПовторныйПоиск = Ложь)
	...
	ПараметрыМетода.ЗапуститьВФоне = Истина;
	ЗначениеРезультатаПредыдущегоШага = ?(РезультатПредыдущегоШага <> Неопределено И ЭтоАдресВременногоХранилища(
		РезультатПредыдущегоШага.АдресРезультата), ПолучитьИзВременногоХранилища(
		РезультатПредыдущегоШага.АдресРезультата), Неопределено);

	ДопустимыеРежимыУдаления = УдалениеПомеченныхОбъектовСлужебный.ДопустимыеРежимыУдаления();
	Задание = ДлительныеОперации.ВыполнитьФункцию(ПараметрыМетода, ИмяМетода, ИсточникУдаляемыхОбъектов, ?(
...
Процедура ПослеЗавершенияУдаленияПомеченных(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		СведенияОРезультатахУдаления = ОбработатьРезультатУдаленияУспех(
										Результат, УникальныйИдентификаторСУчетомВладельца(),
...
Функция ОбработатьРезультатУдаленияУспех(Результат, УникальныйИдентификаторФормы, СведенияОРезультатахУдаления)
	СведенияОРезультатахУдаления = ЗагрузитьРезультатУдаления(
										Результат, УникальныйИдентификаторФормы, СведенияОРезультатахУдаления);
...
Процедура ОбработатьРезультатВыполненияУдаления(СведенияОРезультатахУдаления)
	...
	Если НеУдаленные.ПолучитьЭлементы().Количество() > 0 Тогда
		...
		Элементы.НадписьСостояниеЧастичноеУдаление.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Удалено %1 из %2.'"), СведенияОРезультатахУдаления.УдаленныеКоличество,
...
Функция СформироватьРезультатУдаления(СведенияОРезультате, РезультатВыполненияВФоне, ИдентификаторХранилищаРезультата)
	Результат = ?(СведенияОРезультате = Неопределено,
		УдалениеПомеченныхОбъектовСлужебныйКлиентСервер.НовыйСведенияОРезультатахУдаления(), СведенияОРезультате);
	...
	Результат.Удаленные = УдалениеПомеченныхОбъектовСлужебный.ОбъединениеМассивов(
		Результат.Удаленные, РезультатВыполненияВФоне.Удаленные);
	...
	СвязиНеудаленныхРезультата = УдалениеПомеченныхОбъектовСлужебный.ОбъединениеТаблиц(
			РезультатПрепятствующиеУдалению, РезультатВыполненияВФоне.СвязиНеудаленных, Истина);
...
Процедура ПриВыбореДействияМестаИспользования(Знач ВыбранноеЗначение, Знач ТекущиеДанные)
	...
	ИначеЕсли ВыбранноеЗначение = "Удалить" И Не ТекущиеДанные.СсылочногоТипа Тогда
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нельзя установить действие удалить для %1'"), ТекущиеДанные.Представление));
	Иначе
...
Процедура НачатьВыполнениеДополнительнойОбработки(Параметр)
	...
	Элементы.СтраницыОтображениеСостояния.ТекущаяСтраница = Элементы.СостояниеВыполняется;
	Элементы.ДекорацияПредставлениеДлительнойОперации.Заголовок = НСтр(
		"ru='Выполняется дополнительная обработка причин неудаления...'");
	Элементы.КоманднаяПанельФормы.Доступность = Ложь;
	Элементы.СтраницыИнформация.ТолькоПросмотр = Истина;
...
Процедура ЗапуститьЗадание(Задание = "", Параметр = Неопределено)
	...
	Если Не ПустаяСтрока(ТекущееЗаданиеФормы.Имя) Тогда
		...
		Если РежимУдаления = "Монопольный" И ТекущееЗаданиеФормы.Имя <> ЗаданияФормы().ПоискПомеченных Тогда
