...
Процедура ПриУстановкеОбластиПоиска(ДеревоРазделовПоиска)
	...
	Для Каждого ОбластьПоиска Из ОбластиПоиска Цикл
		...
		Если ЭлементДерева <> Неопределено И ЭлементДерева.Раздел = ТекущийРаздел Тогда
...
Процедура ДобавитьСтрокиДереваРазделовПоискаПоСоставуРекурсивно(ТекущаяСтрокаДерева, СоставПодсистемы,
	...
	Для Каждого ОбъектПодсистемы Из СоставПодсистемы Цикл
		...
		Если ОбщегоНазначения.ЭтоСправочник(ОбъектПодсистемы) Или ОбщегоНазначения.ЭтоДокумент(ОбъектПодсистемы)
			Или ОбщегоНазначения.ЭтоПланВидовРасчета(ОбъектПодсистемы) Или ОбщегоНазначения.ЭтоПланВидовХарактеристик(
			ОбъектПодсистемы) Или ОбщегоНазначения.ЭтоПланОбмена(ОбъектПодсистемы) Или ОбщегоНазначения.ЭтоПланСчетов(
			ОбъектПодсистемы) Или ОбщегоНазначения.ЭтоБизнесПроцесс(ОбъектПодсистемы) Или ОбщегоНазначения.ЭтоЗадача(
			ОбъектПодсистемы) Тогда
		...
		ИначеЕсли ОбщегоНазначения.ЭтоЖурналДокументов(ОбъектПодсистемы) Тогда
			...
			Если ОбъектМетаданныхДоступен(ОбъектПодсистемы) Тогда
				...
				ДобавитьСтрокиДереваРазделовПоискаПоСоставуРекурсивно(НоваяСтрокаЖурнал,
					ОбъектПодсистемы.РегистрируемыеДокументы, ВыбранныеИдентификаторы);
...
Процедура ЗаполнениеСлужебныхСвойствПослеПолученияРазделовРекурсивно(ТекущаяСтрокаДерева)
	...
	Если ТипЗнч(ТекущаяСтрокаДерева) = Тип("СтрокаДереваЗначений") Тогда 
		...
		Иначе
			...
			ТекущаяСтрокаДерева.ПутьКДанным = ТекущаяСтрокаДерева.Родитель.ПутьКДанным + ","
				+ ТекущаяСтрокаДерева.Раздел;
		КонецЕсли;
...
Функция ОбъектМетаданныхДоступен(ОбъектМетаданных)
	...
	Возврат ДоступенПоПравам И ДоступенПоФункциональнымОпциям И ВключатьВКомандныйИнтерфейс;
...
Функция ПодчиненныеСправочники(ОбъектМетаданных)
	...
	Для Каждого Справочник Из Метаданные.Справочники Цикл
		Если Справочник.Владельцы.Содержит(ОбъектМетаданных) И ОбъектМетаданныхДоступен(Справочник) Тогда
