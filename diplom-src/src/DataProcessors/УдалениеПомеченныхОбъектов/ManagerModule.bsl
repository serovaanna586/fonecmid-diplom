...
Функция ОбработатьУдаляемыеОбъекты(ПараметрыУдаления, УдаляемыеОбъекты, СведенияОМетаданных)
		...
		Если Не Пакет.Найти(ЭлементОчереди.УдаляемыйСсылка, "УдаляемыйСсылка") <> Неопределено Тогда
			...
			Если ЗапрещенаОбработкаЭлемента(ЭлементОчередиВПакете) Тогда
				ОбъектыСЗапретомУдаления = ОбъединениеТаблиц(
					ОбъектыСЗапретомУдаления, ЭлементОчередиВПакете);
			Иначе
				Пакет = ОбъединениеТаблиц(Пакет, ЭлементОчередиВПакете, Истина);
			КонецЕсли;
		КонецЕсли;

		Если Пакет.Количество() >= ПараметрыУдаления.РазмерПакета Или УдаляемыеОбъекты.Индекс(ЭлементОчереди)
			= УдаляемыеОбъекты.Количество() - 1 Тогда

			РезультатОбработкиПакета = ОбработатьПакет(ПараметрыУдаления, Пакет);
			РезультатыУдаления = ОбъединитьРезультатУдаления(
				РезультатыУдаления, РезультатОбработкиПакета);
	...
	РезультатыУдаления = ОбъединитьРезультатУдаления(
		РезультатыУдаления, РезультатУдаленияИзНеобработанногоПакета(ПараметрыУдаления, ОбъектыСЗапретомУдаления));	
	...
	Пока РезультатыУдаления.НеудаленныеОбъекты.Количество() <> КоличествоНеудаленныхЭлементов Цикл
		...
		Если РезультатОбработкиЦиклическихСсылок.УдаленныеОбъекты.Количество() > 0 Тогда
			РезультатыУдаления = РезультатПослеУдаленияЦиклическихСсылок(РезультатыУдаления,
				РезультатОбработкиЦиклическихСсылок);
		КонецЕсли;
	КонецЦикла;
...
Функция РезультатУдаленияИзНеобработанногоПакета(ПараметрыУдаления, ОбъектыСЗапретомУдаления)
	...
	РезультатыУдаления.НеудаленныеОбъекты = ОбъединениеТаблиц(
		РезультатыУдаления.НеудаленныеОбъекты, ОбъектыСЗапретомУдаления);

	РезультатыУдаления.МестаИспользования = ОбъединениеТаблиц(
		РезультатыУдаления.МестаИспользования, ОставшиесяМестаИспользованияУдаляемогоОбъекта(ПараметрыУдаления,
...
Функция ЭлементОчередиВПакет(ЭлементОчереди, СведенияОМетаданных, Исключения)
	...
	Если Не ОбъектУдален(ЭлементОчереди.УдаляемыйСсылка) И Исключения.Найти(ЭлементОчереди.УдаляемыйСсылка,
		"УдаляемыйСсылка") = Неопределено Тогда
		...
		ДочерниеПодчиненныеЭлементы = ?(ПометкаУдаления, ДочерниеПодчиненныеЭлементы(ЭлементОчереди.УдаляемыйСсылка,
			ИнформацияОМетаданных), ТаблицаДочернихПодчиненныхЭлементов());
		...
		ИначеЕсли РазрешеноУдалениеЭлемента(ДочерниеПодчиненныеЭлементы) Тогда
			...
			ДополнитьПакетДочернимиИПодчиненнымиЭлементами(Пакет, ЭлементОчереди.УдаляемыйСсылка,
				ДочерниеПодчиненныеЭлементы, Исключения);
		Иначе
		...
		Попытка
			...
			Для Каждого УдаляемыйСсылка Из ОсновнойИПодчиненныеОбъекты Цикл
				...
				РезультатУдаления = УдалитьЭлементПакета(
										ПараметрыУдаления, УдаляемыйСсылка, МестаИспользования.Скопировать(Фильтр),
					НеудаленныеЭлементы);
				...
				ОтразитьРезультатУдаленияЭлементаПакета(
					ПараметрыУдаления, РезультатОбработкиПакета, РезультатУдаления);

				Если Не УдаляемыйСсылка.ЭтоОсновнойОбъект И РезультатУдаления.Код
					= КодыРезультатаУдаления().ОшибкаПриУдалении Тогда
					...
					Фильтр = Новый Структура("УдаляемыйСсылка, МестоИспользования", ОсновнойОбъект.УдаляемыйСсылка,
						УдаляемыйСсылка.УдаляемыйСсылка);
					РезультатУдаленияОсновногоЭлемента.МестаИспользования = МестаИспользования.Скопировать(Фильтр);

					ОтразитьРезультатУдаленияЭлементаПакета(
						ПараметрыУдаления, РезультатОбработкиПакета, РезультатУдаленияОсновногоЭлемента);
...
Процедура ОчиститьСсылкиВМестеИспользования(МестоИспользование, УдаляемыеОбъектыВМестеИспользования)
	...
	ОчиститьСсылкиВКоллекцииРеквизитов(РедактируемыйОбъект, МетаданныеОбъекта.СтандартныеРеквизиты,
		УдаляемыеОбъектыВМестеИспользования);
	ОчиститьСсылкиВКоллекцииРеквизитов(РедактируемыйОбъект, МетаданныеОбъекта.Реквизиты,
		УдаляемыеОбъектыВМестеИспользования);

	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		ОчиститьСсылкиВТабличнойЧасти(РедактируемыйОбъект[ТабличнаяЧасть.Имя], ТабличнаяЧасть.Реквизиты,
			УдаляемыеОбъектыВМестеИспользования);
	КонецЦикла;
	...
	СтандартныеТабличныеЧасти = ?(ЕстьСтандартныеТабличныеЧасти, МетаданныеОбъекта.СтандартныеТабличныеЧасти,
		Новый Массив);

	Для Каждого ТабличнаяЧасть Из СтандартныеТабличныеЧасти Цикл
		ОчиститьСсылкиВТабличнойЧасти(РедактируемыйОбъект[ТабличнаяЧасть.Имя], ТабличнаяЧасть.СтандартныеРеквизиты,
			УдаляемыеОбъектыВМестеИспользования);
	КонецЦикла;
...
Процедура ОчиститьСсылкиВКоллекцииРеквизитов(РедактируемыйОбъект, КоллекцияРеквизитов,
	УдаляемыеОбъектыВМестеИспользования)
...
Функция РезультатПослеУдаленияЦиклическихСсылок(РезультатыУдаления, РезультатОбработкиЦиклическихСсылок)
	...
	Для Каждого Элемент Из РезультатыУдаления.НеудаленныеОбъекты Цикл
		...
		Иначе
			...
			Для Каждого МестоИспользования Из РезультатыУдаления.МестаИспользования.НайтиСтроки(
				Новый Структура("УдаляемыйСсылка", Элемент.УдаляемыйСсылка)) Цикл
...
Функция ДочерниеПодчиненныеЭлементы(УдаляемыйСсылка, ИнформацияОМетаданных)
	...
	Если ПакетыЗапроса.Количество() > 0 Тогда
		...
		Если ИнформацияОМетаданных.ОписаниеПодчиненныхОбъектов.Количество() > 0 Тогда
			...
			ДочерниеПодчиненныеЭлементы = ОбъединениеТаблиц(ДочерниеПодчиненныеЭлементы, ПодчиненныеЭлементы(
				ВыборкаПодчиненныхЭлементов));
		КонецЕсли;

		Если ИнформацияОМетаданных.Иерархический Тогда
			ВыборкаДочернихЭлементов = РезультатПакета[0].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
			ДочерниеПодчиненныеЭлементы = ОбъединениеТаблиц(ДочерниеПодчиненныеЭлементы, ДочерниеЭлементы(
				УдаляемыйСсылка, ВыборкаДочернихЭлементов));
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстЗапросаПодчиненныхЭлементов(ИнформацияОМетаданных)
	...
	Для Каждого ПодчиненныйОбъект Из ИнформацияОМетаданных.ОписаниеПодчиненныхОбъектов Цикл
		УсловиеСвязи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонУсловияСвязи,
			ПодчиненныйОбъект.ИмяРеквизита);
...
Функция ОбъединитьРезультатУдаления(РезультатыУдаления, РезультатОбработки)
	...
	Результат.НеудаленныеОбъекты = ОбъединениеТаблиц(
		РезультатыУдаления.НеудаленныеОбъекты, РезультатОбработки.НеудаленныеОбъекты);
	Результат.МестаИспользования = ОбъединениеТаблиц(
		РезультатыУдаления.МестаИспользования, РезультатОбработки.МестаИспользования);

	Результат.УдаленныеОбъекты = УдалениеПомеченныхОбъектовСлужебный.ОбъединениеМассивов(
		РезультатыУдаления.УдаленныеОбъекты, РезультатОбработки.УдаленныеОбъекты);
...
Функция ИсточникаПодмножествоПриемника(Приемник, Источник, КолонкаПриемника, КолонкаИсточника)
	...
	Для Каждого СведенияОМестеИспользования Из Источник Цикл
		Если Не ОбъектУдален(СведенияОМестеИспользования.МестоИспользования) И Приемник.Найти(
			СведенияОМестеИспользования[КолонкаИсточника], КолонкаПриемника) = Неопределено Тогда
			Результат = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Функция ОбъектУдален(УдаляемыйСсылка)
	Возврат ЗначениеЗаполнено(УдаляемыйСсылка) И (ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		УдаляемыйСсылка, "ВерсияДанных") И Не ЗначениеЗаполнено(УдаляемыйСсылка.ВерсияДанных));
КонецФункции
...
Процедура ОтразитьРезультатУдаленияЭлементаПакета(ПараметрыУдаления, РезультатОбработкиПакета, РезультатУдаления)
	...
	Если РезультатУдаления.Код <> КодыРезультатаУдаления().Успешно Тогда
		...
		РезультатОбработкиПакета.МестаИспользования = ОбъединениеТаблиц(
			РезультатОбработкиПакета.МестаИспользования, РезультатУдаления.МестаИспользования);
	Иначе
		...
		РезультатОбработкиПакета.МестаОчисткиСсылок = ОбъединениеТаблиц(
			РезультатОбработкиПакета.МестаОчисткиСсылок, РезультатУдаления.МестаИспользования);
	КонецЕсли;
...
Функция УдалитьЭлементПакета(ПараметрыУдаления, Элемент, МестаИспользования, НеудаленныеЭлементы)
	...
	Для Каждого ДочернийПодчиненныйЭлемент Из Элемент.ДочерниеПодчиненныеОбъекты Цикл
		Если Не ОбъектУдален(ДочернийПодчиненныйЭлемент.Элемент) Или НеудаленныеЭлементы.Найти(
			ДочернийПодчиненныйЭлемент.Элемент) <> Неопределено Тогда
	...
	ОставшиесяМестаИспользования = ОставшиесяМестаИспользованияУдаляемогоОбъекта(
										ПараметрыУдаления, МестаИспользования);

	Если ОставшиесяМестаИспользования.Количество() = 0 Или МестаОчисткиСсылок.Количество()
		= ОставшиесяМестаИспользования.Количество() Тогда
...
Функция ЗаписьСуществует(СведенияОМестеИспользования)
	МенеджерРегистра = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
		СведенияОМестеИспользования.Метаданные.ПолноеИмя());// РегистрСведенийМенеджер, РегистрБухгалтерииМенеджер, РегистрРасчетаМенеджер, РегистрНакопленияМенеджер
...
Функция СсылкаВВедущемИзмерении(СведенияОМестеИспользования)
	...
	Для Каждого Измерение Из Измерения Цикл
		...
		Если Контейнер.Ведущее И Контейнер[Измерение.Имя] = СведенияОМестеИспользования.УдаляемыйСсылка Тогда
...
Функция МестаИспользованияЭлементовПакета(ПараметрыУдаления, Пакет)
	...
	МестаИспользования = ОбщегоНазначения.МестаИспользования(Пакет.ВыгрузитьКолонку("УдаляемыйСсылка"), ,
...
Процедура ДополнитьПакетДочернимиИПодчиненнымиЭлементами(Пакет, ОсновнойЭлемент, ДочерниеПодчиненныеЭлементы, Исключения)
	Для Каждого ОписаниеЭлемента Из ДочерниеПодчиненныеЭлементы Цикл
		Если Пакет.Найти(ОписаниеЭлемента.Элемент, "УдаляемыйСсылка") <> Неопределено Или Исключения.Найти(
			ОписаниеЭлемента.Элемент, "УдаляемыйСсылка") <> Неопределено Тогда
...
Процедура ОповеститьОПрогрессе(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ЭтоРегламентноеЗадание Или ТекущееВремя - ПараметрыВыполнения.ВремяОповещения
		<= ПараметрыВыполнения.ПериодОповещения Тогда
		Возврат;
	КонецЕсли;
...
Функция ОставшиесяМестаИспользованияУдаляемогоОбъекта(ПараметрыУдаления, МестаИспользования,
	...
	Для Каждого СведенияОМестеИспользования Из МестаИспользования Цикл
		ЭтоСсылка = СведенияОМестеИспользования.МестоИспользования <> Неопределено
			...
			СведенияОМестеИспользования.МестоИспользования, "ВерсияДанных");

		ЭтоКонстанта = Метаданные.Константы.Содержит(СведенияОМестеИспользования.Метаданные);

		Если ЭтоСсылка И (Не ОбъектУдален(СведенияОМестеИспользования.МестоИспользования)
		...
		Если Не ЭтоСсылка И Не ЭтоКонстанта И ЗаписьСуществует(СведенияОМестеИспользования)
			И Не СведенияОМестеИспользования.ЭтоСлужебныеДанные И Не СсылкаВВедущемИзмерении(
			СведенияОМестеИспользования) Тогда

			ЗаполнитьЗначенияСвойств(Результат.Добавить(), СведенияОМестеИспользования);
		КонецЕсли;

		Если Не ЭтоСсылка И ЭтоКонстанта И СведенияОМестеИспользования.МестоИспользования = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(Результат.Добавить(), СведенияОМестеИспользования);
		КонецЕсли;
	КонецЦикла;
...
Функция МестаОчисткиСсылок(ПараметрыУдаления, МестаИспользования)
	...
	Для Каждого СведенияОМестеИспользования Из МестаИспользования Цикл
		...
		Если ЭтоСсылка И Не ОбъектУдален(СведенияОМестеИспользования.МестоИспользования)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СведенияОМестеИспользования.МестоИспользования,
			"ПометкаУдаления") И Не СведенияОМестеИспользования.ЭтоСлужебныеДанные Тогда
...
Функция СсылкиТолькоНаЭлементыВнутриПакета(Пакет, МестаИспользования)
	...
	Для Каждого Элемент Из Пакет Цикл
		Если Элемент.РезультатУдаления <> КодыРезультатаУдаления().ЕстьМестаИспользования И Элемент.РезультатУдаления
			<> КодыРезультатаУдаления().ЕстьДочерниеПодчиненныеЭлементы Тогда
			Продолжить;
		КонецЕсли;

		МестаИспользованияЭлемента = МестаИспользования.Скопировать(Новый Структура("УдаляемыйСсылка",
			Элемент.УдаляемыйСсылка), "МестоИспользования");
...
Функция ПараметрыУдаления(УдаляемыеОбъекты, Параметры)
	...
	ПараметрыОбработки.Вставить("Монопольно", ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "Монопольно",
		Ложь));
	...
	ПрепятствующиеУдалению.Колонки.Добавить("ОбнаруженныйСтатус", Новый ОписаниеТипов("Строка", ,
...
Процедура ЗаполнитьСведенияОМетаданных(СведенияОМетаданных, УдаляемыеОбъекты)
	...
	Для Каждого Элемент Из УдаляемыеОбъекты Цикл
		Если СведенияОМетаданных.Найти(Элемент.Метаданные(), "Метаданные") = Неопределено Тогда
			...
			ИнформацияОМетаданных.Иерархический = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Кеш, "Иерархический",
				Ложь);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ОбъектМетаданных Из Метаданные.Справочники Цикл
		...
		Если ОписаниеПодчиненногоОбъекта = Неопределено И ОбъектМетаданных.Владельцы.Количество() > 0 Тогда
		...
		ИначеЕсли ОбъектМетаданных.Владельцы.Количество() > 0 Тогда
			...
			ОписаниеПодчиненногоОбъекта.ПоляСвязей = ?(СтрНайти(ОписаниеПодчиненногоОбъекта.ПоляСвязей, "Владелец") > 0,
				ОписаниеПодчиненногоОбъекта.ПоляСвязей, ОписаниеПодчиненногоОбъекта.ПоляСвязей + ",Владелец");
		КонецЕсли;

		ЗарегистрироватьРеквизиты(СведенияОМетаданных, ОбъектМетаданных.СтандартныеРеквизиты,
...
Процедура ЗарегистрироватьРеквизит(СведенияОМетаданных, Реквизит, ОписаниеПодчиненногоОбъекта)
	ИменаПолейСвязи = ?(ЗначениеЗаполнено(ОписаниеПодчиненногоОбъекта), СтрРазделить(
		ОписаниеПодчиненногоОбъекта.ПоляСвязей, ","), Новый Массив);
...
Процедура УдалитьПомеченныеОбъектыМонопольно(УдаляемыеОбъекты, ПараметрыВыполнения)
	...
	Для Сч = -(УдаляемыеОбъекты.Количество() - 1) По 0 Цикл
		...
		Если Не ОбъектУдален(УдаляемыйОбъект) И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УдаляемыйОбъект,
	...
	Пока УдаляемыеОбъекты.Количество() > 0 Цикл
		...
		Для Каждого СтрокаТаблицы Из ПрепятствующиеУдалению Цикл
			...
			Если ОбщегоНазначения.ЭтоСлужебнаяСвязьДанных(ОписаниеСвязиДанных,
				ПараметрыВыполнения.ИсключенияПоискаСсылок) Или СвязьТолькоСВедущимиИзмерениямиРегистров(
				ОписаниеСвязиДанных) Или ОбщегоНазначения.ЭтоПоследовательность(СтрокаТаблицы.Метаданные) Тогда
	...
	Если МестоИспользования = Неопределено Тогда
		Если Не Метаданные.Константы.Содержит(СтрокаТаблицы.Метаданные) Тогда
			Причина.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обнаружены неразрешимые ссылки (%1)'"), СтрокаТаблицы.Метаданные.Представление());
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СвязьТолькоСВедущимиИзмерениямиРегистров(ОписаниеСвязиДанных)
	...
	Если ОбщегоНазначения.ЭтоРегистр(ОписаниеСвязиДанных.Метаданные) Тогда
		...
		Для Каждого Измерение Из ОписаниеСвязиДанных.Метаданные.Измерения Цикл
			...
			Если ОписаниеСвязиДанных.Данные[Измерение.Имя] = ОписаниеСвязиДанных.Ссылка И Измерение.Ведущее Тогда
				Результат = Истина;
				Прервать;
			КонецЕсли;
...
Процедура ОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Процент = Окр(100 * (ПараметрыВыполнения.КоличествоОбработанных) / ПараметрыВыполнения.КоличествоУдаляемых);
	...
	ИначеЕсли ИмяКоллекции = "ПользовательскиеОбъекты" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Обработано %1 из %2'"), ПараметрыВыполнения.КоличествоОбработанных,
