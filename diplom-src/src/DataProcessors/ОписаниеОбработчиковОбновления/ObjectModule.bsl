...
Процедура ЗагрузитьОбработчики() Экспорт
	...
	Замеры.Вставить("ВсегоЗагрузкаОбработчиков", РазностьВремени((ТекущаяУниверсальнаяДатаВМиллисекундах()
		- НачалоВсего) / 1000));
...
Функция ОбновитьСведенияОКонфликтахОбработчиков() Экспорт
	...
	Для Каждого Приоритет Из Приоритеты Цикл
		Если Приоритет.Порядок <> "Любой" И Приоритет.ПорядокАвто = "Любой" И Не Приоритет.ПовторнаяЗапись Тогда
			Приоритет.НеЛюбой = 1;
		КонецЕсли;
		Если Приоритет.Порядок = "До" И Приоритет.ПорядокАвто = "После" Или Приоритет.Порядок = "После"
			И Приоритет.ПорядокАвто = "До" Тогда
...
Функция ПостроитьОчередь(БросатьИсключение = Ложь) Экспорт
	...
	Иначе
		...
		ОбновитьСведенияОКонфликтахОбработчиков();
		Замеры.Вставить("ОбновитьСведенияОКонфликтахОбработчиков", РазностьВремени(
			(ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала) / 1000));
	КонецЕсли;

	ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
	МожноСтроитьОчередь = МожноСтроитьОчередь();
	Замеры.Вставить("МожноСтроитьОчередь", РазностьВремени((ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала)
		/ 1000));
	...
	Замеры.Вставить("ЕстьЦиклВыполненияОбработчиков", РазностьВремени((ТекущаяУниверсальнаяДатаВМиллисекундах()
		- ВремяНачала) / 1000));
	...
	Пока ВыборкаПоДобавляемымОбработчикам.Следующий() Цикл
		...
		СдвинутьРекурсивноСвязанныеОбработчики(ОчередьОбработчиков, НаличиеОбработчиковВОчереди, ДобавляемыйОбработчик,
			МаксимальнаяОчередь, ДанныеРекурсии);
	...
	Замеры.Вставить("ПостроениеОчереди", РазностьВремени((ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала)
		/ 1000));

	ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ВсеХорошо = Не ЕстьОшибкиВПостроенииОчереди(Запрос);
	Замеры.Вставить("ЕстьОшибкиВПостроенииОчереди", РазностьВремени((ТекущаяУниверсальнаяДатаВМиллисекундах()
		- ВремяНачала) / 1000));
	...
	Замеры.Вставить("ВсегоВПостроенииОчереди", РазностьВремени((ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоВсего)
		/ 1000));
...
Процедура ЗагрузитьТаблицуОбработчиков(Библиотека, ДополнительныеСведения)
	...
	Для Каждого ОписаниеОбработчика Из Библиотека.Обработчики Цикл
		...
		НовоеОписание.ИмяБиблиотеки = СтрЗаменить(Библиотека.ИмяОсновногоСерверногоМодуля,
...
Функция ЗапросВременныхТаблиц()
	...
	Запрос.УстановитьПараметр("СтандартнаяПроцедураПроверки",
		"ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы");
...
Процедура СдвинутьРекурсивноСвязанныеОбработчики(ОчередьОбработчиков, НаличиеОбработчиковВОчереди, Обработчик,
	МаксимальнаяОчередь, ДанныеРекурсии)
	...
	Для Каждого СвязанныйОбработчик Из СвязанныеОбработчики Цикл
		...
		Если СвязанныйОбработчик.Значение.ПорядокПомещенияВОчередь = "Любой" Тогда
			...
			Если СвязанныйОбработчик.Значение.ПовторнаяЗаписьОбъекта И ОчередьОбработчиков[Обработчик].Очередь
				= ОчередьОбработчиков[СвязанныйОбработчик.Ключ].Очередь Тогда

				УдалитьОбработчикИзНаличияВОчереди(НаличиеОбработчиковВОчереди,
					ОчередьОбработчиков[Обработчик].Очередь, Обработчик);
				ОчередьОбработчиков[Обработчик].Очередь = ОчередьОбработчиков[Обработчик].Очередь + 1;
				ДобавитьОбработчикВНаличияВОчереди(НаличиеОбработчиковВОчереди,
					ОчередьОбработчиков[Обработчик].Очередь, Обработчик, МаксимальнаяОчередь);

				// Сдвинуть уже отработанные связанные обработчики
				СдвинутьРекурсивноСвязанныеОбработчики(ОчередьОбработчиков, НаличиеОбработчиковВОчереди, Обработчик,
					МаксимальнаяОчередь, ДанныеРекурсии);
			КонецЕсли;
		...
		ИначеЕсли СвязанныйОбработчик.Значение.ПорядокПомещенияВОчередь = "До" Тогда
			...
			Если ОчередьОбработчиков[Обработчик].Очередь >= ОчередьОбработчиков[СвязанныйОбработчик.Ключ].Очередь Тогда
				...
				УдалитьОбработчикИзНаличияВОчереди(НаличиеОбработчиковВОчереди,
					ОчередьОбработчиков[СвязанныйОбработчик.Ключ].Очередь, СвязанныйОбработчик.Ключ);
				...
				СдвинутьРекурсивноСвязанныеОбработчики(ОчередьОбработчиков, НаличиеОбработчиковВОчереди,
					СвязанныйОбработчик.Ключ, МаксимальнаяОчередь, ДанныеРекурсии);
			КонецЕсли;
...
Функция ЕстьОшибкиВПостроенииОчереди(Запрос, СообщатьОбОшибках = Истина)
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = НСтр(
			"ru = 'Ошибка в алгоритме построения очереди: обработчик %Обработчик1% должен быть отнесен к очереди 1.'");
		Если Выборка.Проблема = "ПроблемаВПорядкеСледованияОбработчиков" Тогда
			ТекстСообщения = НСтр(
				"ru = 'Ошибка в алгоритме построения очереди: обработчики %Обработчик1% и %Обработчик2% размещены в неправильном порядке.'");
...
Функция МожноСтроитьОчередь()
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПорядокПомещенияВОчередь%", НСтр(
			"ru = 'Не задан приоритет выполнения.'"));
		ДобавитьОшибку(Выборка.Обработчик1, ТекстСообщения);
	КонецЦикла;
	...
	Если БросатьИсключение Тогда
		...
		Для Каждого Ошибка Из Ошибки Цикл
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, ОбъектМетаданных, Ошибка.Обработчик, Ошибка.Сообщение);
		КонецЦикла;
...
//
Функция ЕстьЦиклВыполненияОбработчиков(ПроверяемыйОбработчик = Неопределено, Знач ПриоритетыОбработчика = Неопределено,
	...
	Для ИндексВершины = 0 По КоличествоВершин - 1 Цикл
		...
		Если КоличествоВершинВхода > 0 Тогда
			...
			Для ИндексВершиныВхода = 0 По КоличествоВершинВхода - 1 Цикл
				...
				СписокВершинВхода[ИндексВершиныВхода] = СписокВершин.Индекс(СписокВершин.НайтиСтроки(
					ОтборВСпискеВершин)[0]);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция РазностьВремени(ВремяОкончания, ВремяНачала = 0, СтруктураВремени = Неопределено)
	...
	Возврат ?(СтрДлина(Строка(Часы)) = 1, "0" + Строка(Часы), Строка(Часы)) + ":" + ?(СтрДлина(Строка(Минуты)) = 1, "0"
		+ Строка(Минуты), Строка(Минуты)) + ":" + ?(СтрДлина(Строка(Окр(Секунды))) = 1, "0" + Строка(Секунды), Строка(
		Секунды));
КонецФункции
...
Функция ВерсияЧислом(Версия) Экспорт
	...
	Если МассивВерсий.Количество() = 4 Тогда
		Результат = Число(МассивВерсий[0]) * 10000000 + Число(МассивВерсий[1]) * 1000000 + Число(МассивВерсий[2])
			* 10000 + Число(МассивВерсий[3]);

	ИначеЕсли МассивВерсий.Количество() = 3 Тогда
		Результат = Число(МассивВерсий[0]) * 1000000 + Число(МассивВерсий[1]) * 10000 + Число(МассивВерсий[2]);
	КонецЕсли;
