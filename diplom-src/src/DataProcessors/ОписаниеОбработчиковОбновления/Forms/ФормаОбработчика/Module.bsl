...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьИдентификаторыОбъектовМетаданных = Метаданные.Справочники.Найти("ИдентификаторыОбъектовМетаданных")
		<> Неопределено;
	...
	Если Не ПустаяСтрока(Параметры.АдресОбработчика) Тогда
		...
		Если Не ПустаяСтрока(ПроцедураОбработки) Тогда
			...
			АвтоЗаголовок = Ложь;
		КонецЕсли;
		ОсновнойОбъектМетаданных = ИмяОсновногоОбъектаМетаданных(Объект.ОбработчикиОбновления[0].Процедура,
		...
		КонецЕсли;
		Элементы.ИмяОсновногоСерверногоМодуля.Видимость = Не ЗначениеЗаполнено(
			Объект.ОбработчикиОбновления[0].ИмяОсновногоСерверногоМодуля);
...
&НаКлиенте
Процедура ОсновнойОбъектМетаданныхАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
...
&НаКлиенте
Процедура ОсновнойОбъектМетаданныхОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
	СтандартнаяОбработка)
...
Процедура ПроцедураЗаполненияДанныхОбновленияПриИзменении(Элемент)
	...
	Если Не ПустаяСтрока(Объект.ОбработчикиОбновления[0].ПроцедураЗаполненияДанныхОбновления) Тогда
		ИмяОсновногоОбъектаМетаданных(Объект.ОбработчикиОбновления[0].ПроцедураЗаполненияДанныхОбновления,
			ЕдинственноеЧисло, МножественноеЧисло);
	КонецЕсли;
...
Процедура ПроцедураПроверкиПриИзменении(Элемент)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроцедураПроверкиОкончаниеВводаТекстаПослеВопроса", ЭтотОбъект,
		Элемент.ТекстРедактирования);
	Если Элемент.ТекстРедактирования <> СтандартнаяПроцедураПроверки() Тогда
		ПоказатьВопрос(ОписаниеОповещения, ТекстПредупрежденияИзмененаПроцедураПроверки(Истина),
...
Процедура ПроцедураПроверкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ЧитаемыеОбъекты.НайтиСтроки(Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0
		...
		ТекстСообщения = НСтр(
			"ru = 'Процедура ""%1"" не предназначена для проверки блокировки в обработчиках, которые контролируют блокировку читаемых данных или прочих объектов.'");
...
&НаКлиенте
Процедура ОбъектМетаданныхАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	СтандартнаяОбработка)
...
&НаКлиенте
Процедура ОбъектМетаданныхОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
	СтандартнаяОбработка)
...
Процедура ДобавитьТипыРеквизитаВБлокируемыеОбъекты(Команда)
	...
	ОбработчикВыбора = Новый ОписаниеОповещения("ЗавершениеВыбораОбъектаМетаданных", ЭтотОбъект,
...
Функция ЗаполненоПравильно()
	...
	Если ТекущийОбработчик.РежимВыполнения = "Отложенно" И Объект.ИзменяемыеОбъекты.Количество() = 0 Тогда
		ТекстСообщения = НСтр(
			"ru = 'Отложенный обработчик не может быть записан с пустой табличной частью ""Изменяемые объекты""'");
...
Процедура ВыборОбъектаМетаданных(Элемент, ТекущееЗначение = "", ИмяТаблицы = Неопределено)
	...
	ОбработчикВыбора = Новый ОписаниеОповещения("ОбработкаВыбораОбъектаМетаданных", ЭтотОбъект,
		ПараметрыОбработчикаВыбора);
	...
	ОткрытьФорму(ИмяФормыВыбора, ПараметрыФормы, Элемент, ЭтотОбъект.УникальныйИдентификатор, , , ОбработчикВыбора,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		...
		ИначеЕсли ДополнительныеПараметры.Свойство("ИмяРеквизита") Тогда
			...
			Если ЗначениеЗаполнено(ПредставлениеОбъекта.ОбъектМетаданных) Тогда
				...
				Если ЧастиИмени.Количество() > 0 Тогда
					...
					ОбновитьИмяПроцедуры("Процедура", ИмяОбъекта, ДополнительныеПараметры.ПредыдущееЗначение,
						"ОбработатьДанныеДляПереходаНаНовуюВерсию");
					ОбновитьИмяПроцедуры("ПроцедураЗаполненияДанныхОбновления", ИмяОбъекта,
...
Процедура ОбновитьИмяПроцедуры(КонтейнерИмени, ИмяОбъекта, Знач ПредыдущееИмяОбъекта, ИмяНовойПроцедуры)
	...
	ИначеЕсли Не ПустаяСтрока(ПредыдущееИмяОбъекта) Тогда
		...
		ТекущийОбработчик[КонтейнерИмени] = СтрЗаменить(ТекущийОбработчик[КонтейнерИмени], ПредыдущееИмяОбъекта,
			ИмяОбъекта);
			...
			Для Каждого ОбъектПересечения Из ОбъектыПересечения Цикл
				Если ИмяОсновногоОбъекта <> ОбъектПересечения.ОбъектМетаданных Тогда
					НоваяСтрока.ОбъектыПересечения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														"%1 +%2", ОбъектПересечения.ОбъектМетаданных,
						ОбъектыПересечения.Количество() - 1);
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
Процедура ЗаполнитьПересеченияОбработчика(ТекущийОбработчик, КонфликтующийОбработчик)
	...
	Для Каждого Конфликт Из Объект.КонфликтыОбработчиков Цикл
		...
		Если Конфликт.ОбработчикПисатель = ТекущийОбработчик И Конфликт.ОбработчикЧитательИлиПисатель2 = КонфликтующийОбработчик
			Или Конфликт.ОбработчикПисатель = КонфликтующийОбработчик И Конфликт.ОбработчикЧитательИлиПисатель2 = ТекущийОбработчик
...
Процедура УстановитьОтметкуНезаполненногоПроцедураПроверки()
	...
	Если ЗначениеЗаполнено(Объект.ОбработчикиОбновления[0].Процедура)
		...
		Или Не (Объект.БлокируемыеОбъекты.Количество() > 0 Или Объект.ЧитаемыеОбъекты.НайтиСтроки(
		Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0 Или Объект.ИзменяемыеОбъекты.НайтиСтроки(
		Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0) Тогда
		Отметка = Ложь;
	КонецЕсли;
...
Процедура ВыбратьРеквизитОбъектаМетаданных(Реквизиты, ИмяТаблицы, ИмяОбъекта)
	...
	ОбработчикВыбора = Новый ОписаниеОповещения("ЗавершениеВыбораРеквизитаОбъектаМетаданных", ЭтотОбъект,
		ДополнительныеПараметры);
...
Процедура ПроцедураПроверкиОкончаниеВводаТекстаПослеВопроса(Результат, Текст) Экспорт
	...
	Элементы.ПроцедураПроверки.ОтметкаНезаполненного = Не ЗначениеЗаполнено(
		Объект.ОбработчикиОбновления[0].ПроцедураПроверки);
	Если Не ПустаяСтрока(Объект.ОбработчикиОбновления[0].ПроцедураПроверки) Тогда
		ИмяОсновногоОбъектаМетаданных(Объект.ОбработчикиОбновления[0].ПроцедураПроверки, ЕдинственноеЧисло,
			МножественноеЧисло);
	КонецЕсли;
КонецПроцедуры
