...
#Если Не ВебКлиент Тогда
	...
	ОбработчикВыбораФайла = Новый ОписаниеОповещения("ДиалогВыбораКаталогаЗавершение", ЭтотОбъект,
		"КаталогФайловОшибок");
...
&НаКлиенте
Процедура КаталогФайловОшибокОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
...
&НаКлиенте
Процедура БыстрыеОтборыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
...
Процедура ЗавершениеРедактированияОбработчика(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЭтотОбъект.ТекущийЭлемент <> Неопределено И ЭтотОбъект.ТекущийЭлемент.Имя <> "ОбработчикиОбновления" Тогда
		...
		Если ЭтотОбъект.ТекущийЭлемент.ТекущийЭлемент <> Неопределено И ЭтотОбъект.ТекущийЭлемент.ТекущийЭлемент.Имя
			<> "ОбработчикиОбновленияПроцедура" Тогда
	...
	Если Команда <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(КаталогФайловОшибок) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не указан каталог файлов ошибок.'"), , , "КаталогФайловОшибок");
			Возврат;
		КонецЕсли;
	КонецЕсли;
#КонецОбласти
...
Функция ПравильноУказанКаталогSRC()
	...
	Если Не ЗначениеЗаполнено(Объект.КаталогSRC) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не указан путь к каталогу SRC'"), , , "Объект.КаталогSRC");
		Результат = Ложь;
	КонецЕсли;
	...
	Если Не КаталогНаДиске.Существует() Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не найден каталог SRC'"), , , "Объект.КаталогSRC");
		Результат = Ложь;
	КонецЕсли;
...
Функция ДобавитьПроцедуруМодуля(Код)
	...
	Если ЭтоМодульМенеджера И СтрокиМодуля.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
			Код.ИмяМодуля, ИмяПроцедуры));
		Возврат Истина;
	КонецЕсли;
	...
	Если Код.ЭтоПроцедураОписания Тогда
		Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
			...
			НайденныйЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка,
				ИмяБиблиотеки);
	...
	Если ЕстьПереданнаяПроцедура Или ЕстьПроцедураОписания Или ЕстьБиблиотечнаяПроцедураОписания
		И ПереданаПростаяПроцедура Тогда
		...
		Если УстановитьВерсию И СокрЛП(СтрокаМодуля) = Код.ПроцедураВерсии Тогда
			...
			Пока СтрНайти(СтрокаМодуля, "Описание.Версия =") = 0 Цикл
				...
				Если СтрНайти(СтрокаМодуля, "Описание.Версия =") > 0 И СтрНайти(СтрокаМодуля, Код.Версия) = 0 Тогда
					...
					СтрокаМодуля = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"	Описание.Версия = ""%1"";", Код.Версия);
				КонецЕсли;
...
#Область ЗаменитьКомментарий
		...
		Если Код.ЭтоПроцедураОписания И СтрНайти(СтрокаМодуля,
			"--// Добавляет в список процедуры-обработчики обновления данных ИБ") > 0 Тогда
			СтрокаМодуля = ПропуститьСтрокиДо(Модуль,
...
#Область ПриДобавленииОбработчиковОбновления
		...
		Если Не ВыполненаЗамена И Код.ЭтоПроцедураОписания И (СтрНайти(СтрокаМодуля,
			"Процедура ОписаниеОбработчиковОбновления") > 0 Или СтрНайти(СтрокаМодуля,
			"Процедура ПриДобавленииОбработчиковОбновления") > 0) И СтрНайти(СтрокаМодуля,
			БиблиотечныйЗаголовокПроцедуры) = 0 Тогда
			Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
				БиблиотечныйЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"Процедура ОписаниеОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
				Если СтрНайти(СтрокаМодуля, БиблиотечныйЗаголовокПроцедуры) > 0 Тогда
					ЭтоБиблиотечнаяПроцедураОписания = Истина;
					Прервать;
				КонецЕсли;
				БиблиотечныйЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"Процедура ПриДобавленииОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
				Если СтрНайти(СтрокаМодуля, БиблиотечныйЗаголовокПроцедуры) > 0 Тогда
					ЭтоБиблиотечнаяПроцедураОписания = Истина;
					Прервать;
				КонецЕсли;
		...
		Если СокрЛП(СтрокаМодуля) = Код.ЗаголовокПроцедуры Или (Не ЭтоМодульОбновления И Код.ЭтоПроцедураОписания
			И (СокрЛП(СтрокаМодуля) = ПростойЗаголовокПроцедурыОписания Или СокрЛП(СтрокаМодуля)
			= "Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт")) Или ЭтоБиблиотечнаяПроцедураОписания
			И Код.ЭтоПроцедураОписания И (Код.ЗаголовокПроцедуры = БиблиотечныйЗаголовокПроцедуры
...
Функция ОбработчикиТехническогоПроекта()
	...
	Для Каждого Обработчик Из Обработчики Цикл
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1:%2", Обработчик.Идентификатор,
...
#Если Не ВебКлиент Тогда
	...
	Пока СтрокаМодуля <> Неопределено Цикл
		...
		Если СтрНайти(СтрокаМодуля, "<version>") > 0 И СтрНайти(СтрокаМодуля, НоваяВерсияКонфигурации) = 0 Тогда
			УстановленаВерсия = Истина;
			СтрокаМодуля = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("  <version>%1</version>",
				НоваяВерсияКонфигурации);
		КонецЕсли;
...
Процедура Подключаемый_ОткрытьРедакторОписанияОбработчика()
	...
	ОбработчикЗакрытияФормы = Новый ОписаниеОповещения("ЗавершениеРедактированияОбработчика", ЭтотОбъект,
	...
	ОткрытьФорму(ИмяФормыОбработчика, ПараметрыФормы, ЭтотОбъект, ЭтотОбъект.УникальныйИдентификатор, , ,
...
Функция ОбновитьДанныеОбработчика(АдресОбработчика)
	...
	Если ОписаниеОбработчика <> Неопределено Тогда
		...
		Если МодулиПодсистем <> Неопределено И НовоеОписание.Подсистема <> "" Тогда
			...
			ОписаниеОбработчика.ИмяБиблиотеки = СтрЗаменить(ОписаниеОбработчика.ИмяОсновногоСерверногоМодуля,
				"ОбновлениеИнформационнойБазы", "");
		КонецЕсли;
...
Процедура ПриоритетыКонфликтовЧитаемыхИлиДвойнойЗаписи(ПриоритетыВыполнения, Конфликты)
	...
	Для Каждого ПриоритетОбработчика1 Из ПриоритетыОбработчика1 Цикл
		ЧитаемыеДанныеИзменяютсяДругими = Конфликты.Найти(ПриоритетОбработчика1.Процедура2, "ПроцедураПисатель")
			<> Неопределено;
		РазрабатываемаяБиблиотека_ИзменяетЧитаемыеДанные = РазрабатываемыеПодсистемы.Найти(
			ПриоритетОбработчика1.Подсистема2) <> Неопределено;
...
Процедура ЗаполнитьБыстрыеОтборы(Знач ОписаниеОбработчиков = Неопределено)
	...
	ДобавитьБыстрыйОтбор(ОписаниеОбработчиков, МассивСтрок, "СтатусПроблемы", СтатусНеобходимАнализ,
		ЦветаСтиля.ЦветОсобогоТекста);
	ДобавитьБыстрыйОтбор(ОписаниеОбработчиков, МассивСтрок, "ИзмененаПроцедураПроверки", НСтр(
		"ru = 'Изменена процедура проверки'"), WebЦвета.Золотистый);
	...
	ДобавитьПрочийБыстрыйОтбор(ОписаниеОбработчиков, "РежимВыполненияОтложенныхОбработчиков", НСтр(
		"ru = 'Параллельно'"));
	ДобавитьПрочийБыстрыйОтбор(ОписаниеОбработчиков, "РежимВыполненияОтложенныхОбработчиков", НСтр(
		"ru = 'Последовательно'"));
	...
	СтрокаГиперссылки = Новый ФорматированнаяСтрока(ТекстСтатуса, , ЦветаСтиля.ЦветТекстаКнопки, , "Прочие");
	...
	ТекстСтатуса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", НСтр("ru = 'Все'"),
		ОписаниеОбработчиков.Количество());
	СтрокаГиперссылки = Новый ФорматированнаяСтрока(ТекстСтатуса, , ЦветаСтиля.ЦветТекстаКнопки, , "Все");
...
&НаСервере
Процедура ДобавитьБыстрыйОтбор(ОписаниеОбработчиков, МассивСтрок, ИмяПоляОтбора, Знач ЗначениеОтбора,
	Знач Цвет = Неопределено)
	...
	Если СтрокиОтбора.Количество() > 0 Тогда
		...
		ТекстСтатуса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", ПредставлениеОтбора,
			СтрокиОтбора.Количество());
		СтрокаГиперссылки = Новый ФорматированнаяСтрока(ТекстСтатуса, , Цвет, , СтрЗаменить(ТРег(ПредставлениеОтбора),
			" ", ""));
			...
			Если ТэгБиблиотеки <> Неопределено Тогда
				...
				Если ТэгЛокализации <> Неопределено Тогда
					Шаблон = "%1
							 |%2
							 |%3";
					ТекстОбработчиков = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
						ТэгЛокализации.Начало, ТекстОбработчиков, ТэгЛокализации.Конец);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;

		ТекстПроцедуры = "Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
						 |
						 |" + ТекстОбработчиков + "
												  |
												  |КонецПроцедуры";
...
#Область ДополнимВыделенныеОбработчикиДругихБиблиотек
	Если ВыделенныеОбработчики.Количество() > 0 И (ВсеМодулиОбработчиков = Неопределено
		Или ВсеМодулиОбработчиков.Количество() = 0) Тогда
...
#Область ДополнимКонфликтующиеОбработчики
	Если РазметкаКода.УстановитьТэги И Не ТолькоВыделенные Тогда
		ИзмененныеОбработчики = Объект.ОбработчикиОбновления.Выгрузить(Новый Структура("Изменен",
			Истина)).ВыгрузитьКолонку("Ссылка");
		...
		Для Каждого Измененный Из ИзмененныеОбработчики Цикл
			КонфликтыОбработчика = Объект.КонфликтыОбработчиков.Выгрузить(Новый Структура("ОбработчикПисатель",
				Измененный));
...
#Область СформироватьПроцедурыОписаниеОбработчиковОбновления
	...
	Для Каждого Модуль Из ВсеМодулиОбработчиков Цикл
		...
		Для Каждого Библиотека Из ИменаБиблиотек Цикл
			...
			Если ИменаБиблиотек.Количество() > 1 Или СтрНайти(Модуль.МодульОбработчика, "ОбновлениеИнформационнойБазы")
				> 0 Тогда
			...
			Если ТолькоИзмененные И Модуль.Изменен Или Не ТолькоИзмененные Тогда
				...
				Если СтрНайти(Модуль.МодульОбработчика, ".") > 0 Тогда
					...
					Если СтрНайти(КодМодуля.ПутьМодуля, "Constants") > 0 Тогда
						КодМодуля.ПутьМодуля = СтрЗаменить(КодМодуля.ПутьМодуля, "\ManagerModule.",
							"\ValueManagerModule.");
					КонецЕсли;
				КонецЕсли;
...
#КонецОбласти
	...
	Если ТолькоВыделенные И Не РасширенныйРежимФормы Тогда
		...
		Для Каждого КодМодуля Из ТекстыМодулей Цикл
			КодОбработчиков.ТекстПроцедуры = КодОбработчиков.ТекстПроцедуры + КодМодуля.ТекстПроцедуры + Символы.ПС
				+ Символы.ПС;
		КонецЦикла;
		ТекстыМодулей = Новый Массив;
...
Процедура ДобавитьВерсиюМодуля(ВерсииМодулей, ОбработчикиМодуля)
	...
	Если ЭтоРазрабатываемаяБиблиотека И Число(ПерваяЦифраМаксимум) >= Число(ПерваяЦифраКонфигурации)
	...
	НомераКонфигурации = СтрРазделить(НоваяВерсияКонфигурации, ".");
	Если ЭтоРазрабатываемаяБиблиотека И НовыйНомерСборкиКонфигурации > Число(НомераКонфигурации[3]) Тогда
...
Функция ТекстОбработчиков(Обработчики, РазметкаКода, ВнешниеТэги = Неопределено)
		...
		Если ВнешниеТэги <> Неопределено Тогда
			Для Каждого Обработчик Из Обработчики Цикл
				Если СтрНайти(ВнешниеТэги, Обработчик.ТэгиМодуляОбновления) = 0 Тогда
					Обработчик.Тэги = Обработчик.Тэги + ?(ПустаяСтрока(Обработчик.Тэги), "", ",")
						+ Обработчик.ТэгиМодуляОбновления;
				КонецЕсли;
	...
	Для Каждого Ветка Из ДеревоТэгов.Строки Цикл
		ТекстГруппыОбработчиков = ТекстГруппыОбработчиковРекурсивно(Ветка, РазметкаКода,
			СтрокиГруппОбработчиков.Количество() = 0);
...
Функция ТекстГруппыОбработчиковРекурсивно(Ветка, РазметкаКода, ПервыйВГруппе = Истина)
	...
	Для Каждого Обработчик Из Ветка.Строки Цикл
		Если Обработчик.Строки.Количество() > 0 Тогда
			ТекстГруппыОбработчиков = ТекстГруппыОбработчиковРекурсивно(Обработчик, РазметкаКода,
				СтрокиОбработчиков.Количество() = 0);
	...
	Если РазметкаКода.УстановитьТэги И Не ПустаяСтрока(Ветка.Тэги) И Не РазметкаКода.ЭтоПроцедураОписания Тогда
		...
		Для Каждого Тэг Из СтрРазделить(Ветка.Тэги, ",") Цикл
			...
			ТекстОбработчиков = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ОписаниеТэга.Начало,
				ТекстОбработчиков, ОписаниеТэга.Конец);
		КонецЦикла;
...
Функция ТекстГруппыВызововРекурсивно(Ветка, РазметкаКода)
	...
	Для Каждого МодульОбработчика Из Ветка.Строки Цикл
		...
		ТекстВызова = Символы.Таб + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2;",
			МодульОбработчика.МодульОбработчика, ИмяПроцедуры);
...
Функция ТекстОписанияОбработчика(Обработчик, РазметкаКода, ТэгиОбработчика)
	...
	Если Не ПустаяСтрока(Обработчик.Идентификатор) Тогда
		...
		Если ЗначениеЗаполнено(ИД) Тогда
			СтрокиОбработчика.Добавить("Обработчик.Идентификатор = Новый УникальныйИдентификатор("""
	...
	Если ОтложенныйРежимВыполнения Тогда
		СтрокиОбработчика.Добавить("Обработчик.ПроцедураЗаполненияДанныхОбновления = """
	...
	Если ОтложенныйРежимВыполнения Тогда
		ТекстБлокируемыеОбъекты = ТекстИнформацииОбОбъектах(Обработчик, "БлокируемыеОбъекты", РазметкаКода,
			ТэгиОбработчика);
...
Функция ТекстПриоритетовВыполнения(Обработчик, РазметкаКода, ТэгиОбработчика)
	...
	Для Каждого Тэги Из ТэгиВсехОбъектов Цикл
		...
		Для Каждого Приоритет Из ПриоритетыТэга Цикл
			СтрокаПриоритета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПриоритета,
				Приоритет.Процедура2, Приоритет.Порядок);
		...
		Если Не ПустаяСтрока(Тэги) Тогда
			Для Каждого Тэг Из СтрРазделить(Тэги, ",") Цикл
				Шаблон = "	%1
						 ...
						 |	%3";
				ОписаниеТэга = РазметкаКода.Тэги.Описание[Тэг]; // см. ОписаниеТэга
				ТекстТэга = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ОписаниеТэга.Начало,
					ТекстТэга, ОписаниеТэга.Конец);
	...
	Для Каждого ОписаниеТэга Из ОписанияВсехТэгов Цикл
		КонецНачало = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ОписаниеТэга.Значение.Конец,
			ОписаниеТэга.Значение.Начало);
...
Функция ТекстИнформацииОбОбъектах(Обработчик, ИмяНабораОбъектов, РазметкаКода, ТэгиОбработчика)
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяНабора, ТекстОбъектов,
		ИмяНабораОбъектов);
...
Функция ТекстГруппыОбъектовРекурсивно(Ветка, РазметкаКода, ИмяНабораОбъектов)
	...
	Для Каждого ОбъектМетаданных Из Ветка.Строки Цикл
		Если ОбъектМетаданных.Строки.Количество() > 0 Тогда
			...
			Продолжить;
		КонецЕсли;
		ТекстОбъекта = Символы.Таб + ИмяНабораОбъектов + ".Добавить(" + ПолныйПутьМетаданныхОбъекта(
	...
	Если Не ПустаяСтрока(Ветка.Тэги) Тогда
		...
		Для Каждого Тэг Из СтрРазделить(Ветка.Тэги, ",") Цикл
			ОписаниеТэга = РазметкаКода.Тэги.Описание[Тэг]; // см. ОписаниеТэга
			ТекстОбъектов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ОписаниеТэга.Начало,
				ТекстОбъектов, ОписаниеТэга.Конец);
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Функция ЗаполнитьТэгиОбъектов(РеквизитИмяОбъекта, ОбъектыОбработчика, РазметкаКода, ВнешниеТэги = Неопределено,
	СортироватьПоТэгам = Истина)
	...
	Для Каждого ИспользуемыйОбъект Из ОбъектыОбработчика Цикл
		...
		Если Тэги.Количество() > 0 Тогда
			...
			Если ЕстьТэгиМодуляОбновления Тогда
				ВнешниеТэги = ИспользуемыйОбъект.Тэги + ?(ПустаяСтрока(ИспользуемыйОбъект.Тэги), "", ",")
					+ ИспользуемыйОбъект.ТэгиМодуляОбновления;
...
Функция ЗаполнитьТэгиМодулейПоПодсистемам(МодулиМенеджеров, РазметкаКода)
	...
	Если МодулиМенеджеров.Колонки.Найти("ВнешниеТэги") = Неопределено Тогда
		МодулиМенеджеров.Колонки.Добавить("ВнешниеТэги", Новый ОписаниеТипов("Строка", ,
...
Процедура ДобавитьОбработчикиТэга(Индекс, Обработчики, ВеткаТэга, Вложенность)
	...
	Пока Индекс < Обработчики.Количество() Цикл
		...
		Если ВеткаТэга.Тэги <> Обработчик.Тэги Тогда
			Если ОбщийТэг = Неопределено Или ОбщийТэг.Строки.Найти(Обработчик.Тэги) = Неопределено Тогда
				Прервать;
			КонецЕсли;
	...
	Если РазметкаКода.УстановитьТэги И РазметкаКода.МодульЛокализации И Не РазметкаКода.ЭтоПроцедураОписания Тогда
		...
		Если ТэгЛокализации <> Неопределено Тогда
			Шаблон = "%1
					 |%2
					 |%3";
			ТекстПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ТэгЛокализации.Начало,
				ТекстПроцедуры, ТэгЛокализации.Конец);
		КонецЕсли;
	КонецЕсли;

	ТекстПроцедуры = "Процедура " + ИмяПроцедуры + "(Обработчики) Экспорт
												   |
												   |" + ТекстПроцедуры + "
																		 |
																		 |КонецПроцедуры";
	...
	Для Каждого ОбъектБиблиотеки Из ОбъектыБиблиотек Цикл
		...
		Для Каждого ИмяБиблиотеки Из ВсеБиблиотеки Цикл
			...
			Если Не ОбъектБиблиотеки[ИмяБиблиотеки] Тогда
				ИсключитьИзСтрокой = ИсключитьИзСтрокой + ?(Не ПустаяСтрока(ИсключитьИзСтрокой), ",", "")
					+ ИмяБиблиотеки;
		...
		ТэгиОбъекта = Новый Массив;
		ИсключитьИзКонфигурации = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СтрРазделить(ИсключитьИзСтрокой, ",",
			Ложь));
...
#Если Не ВебКлиент Тогда
	...
	Исключение
		...
		ТекстСообщения = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Не удалось прочитать файл %1.'"), ИмяФайла);
...
Процедура ПрочитатьСтрокиYAML(Результат, ТекстФайла, НомерСтроки = 0, Уровень = 0, ЧтениеМногострочнойСтроки = Ложь)
	...
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Не удалось разобрать строку: %1'"), СтрокаФайла);
		ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ru = 'По причине'") + ":" + Символы.ПС;
		ТекстСообщения = ТекстСообщения + ТекстОшибки;
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
...
&НаКлиенте
Процедура ЗаписатьОшибку(ИмяОбъекта, ОписаниеОшибки = "", ИнформацияОбОшибке = Неопределено, Ключ = Неопределено)
	...
	Если ЗначениеЗаполнено(КаталогФайловОшибок) Тогда
		...
		ИмяФайлаОшибки = ИмяФайлаОшибки + Формат(ОбщегоНазначенияКлиент.ДатаСеанса(), "ДФ=yyyyMMddHHmmss") + "_"
...
Функция ТекстОшибкиXML(Ошибка)
	...
	Если ЗначениеЗаполнено(Ошибка.ОбъектМетаданных) Или ЗначениеЗаполнено(Ошибка.УточнениеРасположения) Тогда
		Ошибка.ОбъектыМетаданных.Добавить(Новый Структура("ОбъектМетаданных, УточнениеРасположения",
			Ошибка.ОбъектМетаданных, Ошибка.УточнениеРасположения));
	КонецЕсли;
...
Функция ПолучитьТекстыНастроек()
	...
	ТекстыНастроек.Добавить(
		ТэгиВырезки.ПолучитьТекст(), НСтр(
		"ru = 'Файл cut_tags.yml сохранить в каталог .settings локальной папки проекта в кодировке UTF-8'"));
...
Процедура ДиалогВыбораКаталогаЗавершение(ВыбранныйКаталог, ИмяРеквизита) Экспорт
	...
	Если ВыбранныйКаталог <> "" Тогда
		Если ИмяРеквизита = "КаталогФайловОшибок" Тогда
			ЭтотОбъект[ИмяРеквизита] = ВыбранныйКаталог + ?(ЗначениеЗаполнено(ВыбранныйКаталог),
...
Функция ЗаполнитьПолеОтбораПоОбъектуНаСервере(ИмяТабличнойЧасти, Текст)
	...
	Запрос.Выполнить();
	УникальныеСсылки = Запрос.МенеджерВременныхТаблиц.Таблицы["вт"].ПолучитьДанные().Выгрузить().ВыгрузитьКолонку(
		"Ссылка");
