...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИдГруппы) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Ключ группы должен содержать только цифры.'"), , "ИдГруппы", ,
			Отказ);
	КонецЕсли;
КонецПроцедуры
...
Процедура АктивироватьБота(Команда)
	...
	Попытка
		...
		Закрыть(Истина);
	Исключение
		ПоказатьПредупреждение( , НСтр("ru='Во время активизации чат-бота произошла ошибка'") + Символы.ПС
...
Процедура Отключить(Команда)
	Попытка
		...
		Закрыть(Истина);
	Исключение
		ПоказатьПредупреждение( , НСтр("ru='Во время отключение чат-бота произошла ошибка'") + Символы.ПС
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
