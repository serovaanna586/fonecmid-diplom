...
Процедура ПриРегистрации()
	...
	Оповещение = Новый ОписаниеОповещения("ПослеУспешнойРегистрации", ЭтотОбъект, , "ПриОбработкеОшибкиРегистрации",
		ЭтотОбъект);
...
Процедура ПриОтказеОтВводаКодаПодтверждения()
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'При отказе от ввода высланный на электронную почту код станет недействительным.
...
Процедура ОбновитьЭлементыВыбораСервера(Форма)
	ПредставлениеСервераВзаимодействия = ?(Форма.ВыборСервераВзаимодействия = 0, НСтр("ru='1С:Диалог'"),
		Форма.СерверВзаимодействия);
	ПредставлениеЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Подключаться к %1'"), ПредставлениеСервераВзаимодействия);

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ГруппаЛокальныйСерверВзаимодействия", "Заголовок", ПредставлениеЗаголовка);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ГруппаЛокальныйСерверВзаимодействия", "Доступность", Форма.ВыборСервераВзаимодействия <> 0);
КонецПроцедуры
