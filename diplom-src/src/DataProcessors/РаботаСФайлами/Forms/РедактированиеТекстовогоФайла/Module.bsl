...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДанныеФайла.Версия <> ДанныеФайла.ТекущаяВерсия Или ДанныеФайла.Служебный Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" И Параметр.Свойство("Событие") И Параметр.Событие = "ФайлРедактировался" И Источник
		= Файл Тогда
	...
	Если ИмяСобытия = "Запись_Файл" И Параметр.Свойство("Событие") И Параметр.Событие = "ДанныеФайлаИзменены"
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ОбработчикРезультата = Новый ОписаниеОповещения("ПередЗакрытиемПослеОтветаНаВопросПриВыходеИзТекстовогоРедактора",
		ЭтотОбъект);
...
Процедура Записать(Команда)
	...
	ПараметрыОбновленияФайла = РаботаСФайламиСлужебныйКлиент.ПараметрыОбновленияФайла(Обработчик, Файл,
...
Процедура ЗаписатьТекстВФайл(ИмяФайла)
	...
	Иначе
		...
		Текст.Записать(ИмяФайла, ?(ЗначениеЗаполнено(КодировкаТекстаФайла), КодировкаТекстаФайла, Неопределено));
...
Процедура ВыполнитьСравнениеФайлов(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 2 Тогда
		...
		Если ДанныеФайла.ТекущаяВерсия = ДанныеФайла.Версия Тогда
			ДанныеФайлаСлева = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляСохранения(Файл, ,
				ИдентификаторВладельца);
		...
		Иначе
			АдресФайлаСлева = РаботаСФайламиСлужебныйВызовСервера.ПолучитьНавигационнуюСсылкуДляОткрытия(
				ДанныеФайла.Версия, ИдентификаторВладельца);
		КонецЕсли;
		ПередаваемыеФайлы = Новый Массив;
		ПередаваемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(ПараметрыВыполнения.ПолноеИмяФайлаСлева,
			АдресФайлаСлева));
		...
		РаботаСФайламиСлужебныйКлиент.ВыполнитьСравнениеФайлов(
			ПараметрыВыполнения.ПолноеИмяФайлаСлева, ИмяОткрываемогоФайла,
