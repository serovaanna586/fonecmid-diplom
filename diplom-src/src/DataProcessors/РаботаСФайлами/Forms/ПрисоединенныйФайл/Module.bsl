...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		ДоступнаЭлектроннаяПодпись = РаботаСФайламиСлужебный.ДоступнаЭлектроннаяПодпись(ТипЗнч(
	...
	ПараметрыОтправки = ?(ЗначениеЗаполнено(Параметры.ПараметрыОтправки), Параметры.ПараметрыОтправки,
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ГруппаСведенияЧасть1.ВыравниваниеЭлементовИЗаголовков = ВариантВыравниванияЭлементовИЗаголовков.ЭлементыПравоЗаголовкиЛево;
		Элементы.ГруппаСведенияЧасть2.ВыравниваниеЭлементовИЗаголовков = ВариантВыравниванияЭлементовИЗаголовков.ЭлементыПравоЗаголовкиЛево;
		Элементы.ГруппаХарактеристикиФайла.ВыравниваниеЭлементовИЗаголовков = ВариантВыравниванияЭлементовИЗаголовков.ЭлементыПравоЗаголовкиЛево;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" И Источник = ТекущаяСсылкаНаФайл() И Параметр <> Неопределено И ТипЗнч(Параметр)
		= Тип("Структура") И Параметр.Свойство("Событие") И (Параметр.Событие = "ЗаконченоРедактирование"
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	Если ЭтоНовый() Или ЭтотОбъект.Объект.Зашифрован Или ЭтотОбъект.Объект.ПодписанЭП Или ЗначениеЗаполнено(
		ЭтотОбъект.Объект.Редактирует) Тогда
...
Процедура СтандартныеСкопировать(Команда)
	...
	ОткрытьФорму(
		"Обработка.РаботаСФайлами.Форма.ПрисоединенныйФайл", Новый Структура("ЗначениеКопирования",
		ТекущаяСсылкаНаФайл()));
...
Процедура ПечатьСоШтампом(Команда)
	...
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект) Или ОбработатьКомандуЗаписиФайла() Тогда
...
Процедура ДобавитьЭПИзФайла(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.ДобавитьПодписьИзФайла(
		ПрисоединенныйФайл, УникальныйИдентификатор, Новый ОписаниеОповещения("ПриПолученииПодписей", ЭтотОбъект));
...
Процедура Зашифровать(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.Зашифровать(
		Обработчик, ДанныеФайла, УникальныйИдентификатор);
...
Процедура ЗашифроватьПослеШифрованияНаКлиенте(Результат, ПараметрыВыполнения) Экспорт
	...
	РаботаСФайламиСлужебныйКлиент.ИнформироватьОШифровании(
		МассивФайловВРабочемКаталогеДляУдаления, ПараметрыВыполнения.ДанныеФайла.Владелец, СсылкаНаТекущийОбъект);
...
Процедура РасшифроватьПослеРасшифровкиНаКлиенте(Результат, ПараметрыВыполнения) Экспорт
	...
	РаботаСФайламиСлужебныйКлиент.ИнформироватьОРасшифровке(
		ПараметрыВыполнения.ДанныеФайла.Владелец, ТекущаяСсылкаНаФайл());
...
Процедура ПроверитьЭП(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.ПроверитьПодписи(
		ЭтотОбъект, ДанныеФайла.СсылкаНаДвоичныеДанныеФайла, Элементы.ЭлектронныеПодписи.ВыделенныеСтроки);
...
Процедура Редактировать(Команда)
	...
	Если ЭтоНовый() Или ЭтотОбъект.Объект.ПодписанЭП Или ЭтотОбъект.Объект.Зашифрован Тогда
		Возврат;
	КонецЕсли;

	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Редактирует) И ЭтотОбъект.Объект.Редактирует <> ТекущийПользователь Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Если ЭтоНовый() Или Не ЗначениеЗаполнено(ЭтотОбъект.Объект.Редактирует) Или ЭтотОбъект.Объект.Редактирует
	...
	ПараметрыОбновленияФайла = РаботаСФайламиСлужебныйКлиент.ПараметрыОбновленияФайла(ОписаниеОповещения,
...
Процедура Освободить(Команда)
	...
	Если ЭтоНовый() Или Не ЗначениеЗаполнено(ЭтотОбъект.Объект.Редактирует) Или ЭтотОбъект.Объект.Редактирует
		<> ТекущийПользователь И Не ИзменениеФайлов Тогда
...
Процедура СохранитьИзменения(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ПрочитатьИУстановитьДоступностьЭлементовФормы", ЭтотОбъект);
	РаботаСФайламиСлужебныйКлиент.СохранитьИзмененияФайлаСОповещением(Обработчик, ТекущаяСсылкаНаФайл(),
...
Процедура Удалить(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.УдалитьДанные(
		Новый ОписаниеОповещения("ПослеУдаленияДанных", ЭтотОбъект), ТекущаяСсылкаНаФайл(), УникальныйИдентификатор);
...
Процедура ОтображатьВкладкиДополнительныхДанных()
	...
	ИспользоватьВкладки = ВидимостьДополнительныхРеквизитов Или Элементы.ГруппаЭлектронныеПодписи.Видимость
		Или Элементы.ГруппаСертификатыШифрования.Видимость;
	Элементы.ГруппаДополнительныеДанныеСтраницы.ОтображениеСтраниц = ?(ИспользоватьВкладки,
...
Процедура ОткрытьФайлДляПросмотра()
	...
	ФайлРедактируется = ЗначениеЗаполнено(ЭтотОбъект.Объект.Редактирует) И ЭтотОбъект.Объект.Редактирует
		= ТекущийПользователь;
...
Процедура ЗашифроватьСервер(МассивДанныхДляЗанесенияВБазу, МассивОтпечатков, МассивФайловВРабочемКаталогеДляУдаления,
	...
	РаботаСФайламиСлужебный.ЗаписатьИнформациюОШифровании(
		ТекущаяСсылкаНаФайлСервер(), Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор,
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗначениеКопирования) Тогда
		...
		Попытка
			...
			ТипХраненияФайла = РаботаСФайламиСлужебный.ТипХраненияФайла(ЗначениеКопирования.Размер,
				ЗначениеКопирования.Расширение);
			...
			Если ДоступнаЭлектроннаяПодпись И ОбщегоНазначения.ПодсистемаСуществует(
				"СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
				МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
	...
	Иначе
		...
		Попытка
			Если ДоступнаЭлектроннаяПодпись Тогда
				РаботаСФайламиСлужебный.ПеренестиРезультатыПроверкиПодписей(ЭлектронныеПодписи,
					ЗаписываемыйОбъект.Ссылка);
			КонецЕсли;
			ЗаписываемыйОбъект.Записать();
			ЗафиксироватьТранзакцию();
		Исключение
...
Процедура ГруппаДополнительныеДанныеСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") И ТекущаяСтраница.Имя = "ГруппаДополнительныеРеквизиты"
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Если СведенияОСинхронизации.Количество() > 0 Тогда
			...
			Элементы.ДекорацияПояснение.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр(
				"ru = 'Файл доступен только для просмотра, работа с ним ведется в облачном сервисе <a href=""%1"">%2</a>.'"),
			...
			Элементы.ДекорацияДатаСинхронизации.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Синхронизирован: <a href=""%1"">%2</a>'"), "OpenJournal", Формат(
