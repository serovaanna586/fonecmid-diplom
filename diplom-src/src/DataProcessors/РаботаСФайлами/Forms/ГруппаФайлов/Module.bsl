...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНоваяГруппа Тогда
		...
		Если Параметры.Свойство("Родитель") И Параметры.Родитель <> Неопределено И ТипЗнч(Параметры.Родитель) = ТипЗнч(
			ОбъектЗначение.Ссылка) Тогда

			Если Параметры.Родитель.ЭтоГруппа Тогда
				ОбъектЗначение.Родитель = Параметры.Родитель;
			ИначеЕсли Параметры.Родитель.Родитель <> Неопределено И ТипЗнч(Параметры.Родитель.Родитель) = ТипЗнч(
				ОбъектЗначение.Ссылка) Тогда
	...
	Если ТолькоПросмотр Или Не ПравоДоступа("Изменение", ЭтотОбъект.Объект.ВладелецФайла.Метаданные()) Тогда
	...
	Если Не ТолькоПросмотр И Не ТекущаяСсылкаНаФайлСервер().Пустая() Тогда
...
Процедура ЗакрытьФормуПослеОтветаНаВопрос(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да И ОбработатьКомандуЗаписиФайла() Тогда
		Закрыть();
