...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Файл.ХранитьВерсии Тогда
		...
		Если Не Параметры.СоздатьНовуюВерсиюДоступность Тогда
			Элементы.СоздатьНовуюВерсию.Подсказка = НСтр(
				"ru = 'Настройка сохранения версии присоединенного файла задана его автором и не может быть изменена.'");
...
Процедура Сохранить(Команда)
	...
	СтруктураВозврата = Новый Структура("КомментарийКВерсии, СоздатьНовуюВерсию, КодВозврата", КомментарийКВерсии,
		СоздатьНовуюВерсию, КодВозвратаДиалога.ОК);
...
Процедура Отмена(Команда)
	...
	СтруктураВозврата = Новый Структура("КомментарийКВерсии, СоздатьНовуюВерсию, КодВозврата", КомментарийКВерсии,
		СоздатьНовуюВерсию, КодВозвратаДиалога.Отмена);
