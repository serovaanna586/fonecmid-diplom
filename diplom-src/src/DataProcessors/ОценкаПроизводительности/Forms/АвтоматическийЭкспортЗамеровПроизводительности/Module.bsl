...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ЕстьБСП = Истина;
		ЕстьПрофилиБезопасности = ОценкаПроизводительностиСлужебный.ПодсистемаСуществует(
	...
	Если ТипЗнч(КаталогиЭкспорта) <> Тип("Структура") Или КаталогиЭкспорта.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ФлажокПуть Из ЭлементыНаКонтроле Цикл
		...
		Если Выполнять И ПустаяСтрока(СокрЛП(ЭтотОбъект[ПутьЭлемент.ПутьКДанным])) Тогда
			...
			ОценкаПроизводительностиСлужебный.СообщитьПользователю(
				СообщениеТекст, , ПутьЭлемент.Имя, ПутьЭлемент.ПутьКДанным);
			ОшибокНет = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьКаталогЭкспорта(ВыполнятьЛокальныйКаталогЭкспорта, ВыполнятьFTPКаталогЭкспорта)
	...
	Каталоги.Вставить(ОценкаПроизводительностиКлиентСервер.ЛокальныйКаталогЭкспортаКлючЗадания(),
		ВыполнятьЛокальныйКаталогЭкспорта);
...
Процедура ПроверитьРазрешенияНаДоступКВнешнимРесурсам(ЗакрытьФорму)
	...
	Если ВнешниеРесурсыРазрешены <> Истина Тогда
		...
		Если ЕстьПрофилиБезопасности Тогда
			...
			МодульРаботаВБезопасномРежимеКлиент.ПрименитьЗапросыНаИспользованиеВнешнихРесурсов(ЗапросВМассив,
