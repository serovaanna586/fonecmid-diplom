...
Функция ПодборЦелевоеВремяНаСервере(ПараметрыПодбора)
	...
	Иначе
		РезультатПодбора.Вставить("ОписаниеОшибки", НСтр(
	...
	Если РезультатПодбора.КоличествоЗамеров = 0 Тогда
		РезультатПодбора.Вставить("ОписаниеОшибки", ОценкаПроизводительностиКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
			"ru = 'Получено 0 замеров по ключевой операции %1. Измените период или выберите другую ключевую операцию.'"),
	...
	Пока Отклонение > ДопустимаяРазница И Счетчик < МаксимальноеКоличествоИтераций Цикл
		Счетчик = Счетчик + 1;
