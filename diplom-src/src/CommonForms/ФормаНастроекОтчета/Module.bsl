...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОформлениеНастроитьКолонтитулы.Видимость = ГлобальныеНастройки.ВыводитьИндивидуальныеНастройкиКолонтитулов
		И Не РежимИзмененияЭлементаСтруктурыНастроек И (Не ОбщегоНазначения.РазделениеВключено()
	...
	Если ОписаниеНастроекОтчета.СхемаМодифицирована Тогда
		Отчет.КомпоновщикНастроек.Инициализировать(
			Новый ИсточникДоступныхНастроекКомпоновкиДанных(ОписаниеНастроекОтчета.АдресСхемы));
	КонецЕсли;
...
Процедура ПередЗагрузкойВариантаНаСервере(НовыеНастройкиКД)
	...
	Если ФормаПараметры.НачальныйКлючВарианта = КлючТекущегоВарианта И ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") Тогда
...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	Если НеобходимоОбновитьНастройки Тогда
		...
		ПараметрыЗаполнения.Вставить("ОбновитьНастройкиВарианта", Не РежимИзмененияЭлементаСтруктурыНастроек
...
Процедура РасширенныйРежимПриИзменении(Элемент)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЦентрМониторинга") Тогда
		...
		МодульЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики(
			"ОбщаяФорма.ФормаНастроекОтчета.РасширенныйРежим.ПриИзменении", 1);
...
&НаКлиенте
Процедура ПредупрежденияОбОтсутствииПользовательскихНастроекОбработкаНавигационнойСсылки(Элемент,
...
Процедура ТекущийТипДиаграммыПриИзменении(Элемент)
	ЭлементСтруктуры = Отчет.КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(
		ИдентификаторЭлементаСтруктурыНастроек);
...
Процедура Подключаемый_Список_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		...
		ПоказатьОповещениеПользователя(
			ЗаголовокОповещения, , Строка(Выбранные), БиблиотекаКартинок.ВыполнитьЗадачу);
	КонецЕсли;
...
Процедура ВыбранныеПоля_Сгруппировать(Команда)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ГруппаВыбранныхПолей", ПараметрыФормы, ЭтотОбъект,
...
Процедура ВыбранныеПоля_Разгруппировать(Команда)
	...
	РодительЭлементаНастройкиИсточника = СвойствоЭлементаСтруктуры.ПолучитьОбъектПоИдентификатору(
		РодительСтрокиИсточника.Идентификатор);
	...
	ИзменитьГруппировкуВыбранныхПолей(СвойствоЭлементаСтруктуры, РодительСтрокиИсточника.ПолучитьЭлементы(),
		НаследникиЭлементовНастроек, НаследникиСтрок);
...
&НаКлиенте
Процедура ВыбранныеПоляПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ТекущаяСтрока,
	Поле)
	ПриемникПеретаскиванияНаКлиенте = Элемент.Имя;
...
Процедура Отборы_Сгруппировать(Команда)
	...
	Если ТипЗнч(ПараметрыГруппировки.Родитель.Идентификатор) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		ЭлементНастройкиИсточник = СвойствоЭлементаСтруктуры.ПолучитьОбъектПоИдентификатору(
			ПараметрыГруппировки.Родитель.Идентификатор);
	КонецЕсли;

	ЭлементНастройкиПриемник = ЭлементНастройкиИсточник.Элементы.Вставить(ПараметрыГруппировки.Индекс, Тип(
	...
	ИзменитьГруппировкуОтборов(СвойствоЭлементаСтруктуры, ПараметрыГруппировки.Строки, НаследникиЭлементовНастроек,
		НаследникиСтрок);
...
Процедура Отборы_Разгруппировать(Команда)
	...
	ИзменитьГруппировкуОтборов(СвойствоЭлементаСтруктуры, РодительСтрокиИсточника.ПолучитьЭлементы(),
		НаследникиЭлементовНастроек, НаследникиСтрок);
...
Процедура ОтборыВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.ОтборыКартинкаРежимаОтображения Тогда // Изменение быстрого доступа к отбору.
		...
		Иначе
			СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "Отбор",
				ИдентификаторЭлементаСтруктурыНастроек);
		КонецЕсли;
		ВыбратьРежимОтображенияДляСтроки(СвойствоЭлементаСтруктуры, "Отборы", ИдентификаторСтроки, Истина,
			Не Строка.ЭтоПараметр);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтборыПолеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если Строка = Неопределено Или Строка.ЭтоРаздел Или Строка.ЭтоГруппа Или Строка.ЭтоПараметр Тогда
...
Процедура ОтборыВидСравненияПриИзменении(Элемент)
	...
	ПолеЗначения = ?(Строка.ЭтоПараметр, Элементы.ОтборыЗначение, Элементы.ОтборыПравоеЗначение);
	ПолеЗначения.ВыборГруппИЭлементов = ОтчетыКлиентСервер.ЗначениеТипаГруппыИЭлементы(Строка.ВыборГруппИЭлементов,
		Условие);
...
Процедура ИзменитьФормулу(Команда)
	...
	Формула = ВариантыОтчетовСлужебныйКлиентСервер.ФормулаПоПутиКДанным(Отчет.КомпоновщикНастроек.Настройки, Строка(
		Строка.Поле));
...
Процедура СтруктураВариантаВыбор(Элемент, СтрокаИдентификатор, Поле, СтандартнаяОбработка)
	...
	Если Строка.Тип = "НастройкиКомпоновкиДанных" Или Строка.Тип = "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных"
		Или Строка.Тип = "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных" Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;

	Если Поле = Элементы.СтруктураВариантаПредставление Или Поле = Элементы.СтруктураВариантаСодержитОтборы Или Поле
		= Элементы.СтруктураВариантаСодержитПоляИлиСортировки Или Поле
		= Элементы.СтруктураВариантаСодержитУсловноеОформление Тогда
...
Процедура СтруктураВариантаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Или Не Элементы.СтруктураВарианта_Добавить.Доступность Тогда
		Возврат;
	КонецЕсли;
...
Процедура СтруктураВариантаНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Если Строка = Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	Если Строка.Тип = "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных" Или Строка.Тип = "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных"
...
&НаКлиенте
Процедура СтруктураВариантаПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка,
	ИдентификаторЦели, Поле)
	// Проверка общих условий.
	...
	Если НовыйРодитель = Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	Если НовыйРодитель.Тип = "ТаблицаКомпоновкиДанных" Или НовыйРодитель.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Отмена;
		Возврат;
	КонецЕсли;
	...
	Если НовыйРодитель.Тип = "КоллекцияЭлементовСтруктурыТаблицы" Или НовыйРодитель.Тип = "КоллекцияЭлементовСтруктурыДиаграммы"
		Или НовыйРодитель.Тип = "ГруппировкаТаблицы" Или НовыйРодитель.Тип = "ГруппировкаДиаграммы" Тогда
		ДопустимоТолькоНаличиеГруппировок = Истина;
	КонецЕсли;

	Если ДопустимоТолькоНаличиеГруппировок И Строка.Тип <> "Группировка" И Строка.Тип <> "ГруппировкаТаблицы"
	...
	Пока Количество > 0 Цикл
		...
		Для Каждого ВложеннаяСтрока Из Коллекция Цикл
			Если ВложеннаяСтрока = НовыйРодитель Тогда
				...
				Возврат;
			КонецЕсли;
			Если ДопустимоТолькоНаличиеГруппировок И ВложеннаяСтрока.Тип <> "Группировка" И ВложеннаяСтрока.Тип <> "ГруппировкаТаблицы"
...
&НаКлиенте
Процедура СтруктураВариантаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ИдентификаторЦели,
	Поле)
...
Процедура СтруктураВарианта_Переместить(Результат, ПараметрыВыполнения) Экспорт
	...
	Результат = ПереместитьЭлементыСтруктурыВарианта(СтрокаТаблицыВверх, СтрокаТаблицыВниз.ПолучитьРодителя(),
		СтрокаТаблицыВниз);
...
Функция АдресНастроекВСтрокеXML()
	Возврат ПоместитьВоВременноеХранилище(
		ОбщегоНазначения.ЗначениеВСтрокуXML(Отчет.КомпоновщикНастроек.Настройки), УникальныйИдентификатор);
КонецФункции
...
Процедура СоставГруппировкиВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.СоставГруппировкиПоле Тогда
		...
		КонецЕсли;
	ИначеЕсли Поле <> Элементы.СоставГруппировкиТипГруппировки И Поле <> Элементы.СоставГруппировкиТипДополнения Тогда
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОформлениеВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Строка.ЭтоПараметрВывода Тогда
		Если Строка(Строка.Идентификатор) = "TITLE" И Поле = Элементы.ОформлениеЗаголовок Тогда
...
Процедура НастроитьКолонтитулы(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.НастройкаКолонтитулов", Новый Структура("Настройки", Настройки), ЭтотОбъект,
		УникальныйИдентификатор, , , Новый ОписаниеОповещения("ЗапомнитьНастройкиКолонтитулов", ЭтотОбъект),
...
Процедура ИзменитьУсловияОтборов(Команда)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.УсловияОтборовОтчета", ПараметрыФормы, ЭтотОбъект,
		Истина, , , Обработчик);
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьУсловияОтборовЗавершение(УсловияОтборов, Контекст) Экспорт
	Если УсловияОтборов = Неопределено Или УсловияОтборов = КодВозвратаДиалога.Отмена Или УсловияОтборов.Количество()
		= 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_Список_ВставитьИзБуфера(Команда)
	...
	ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(Сведения.Настройки, ПользовательскиеНастройки,
		Сведения.Элемент);
...
Процедура ОбновитьПараметрыДанных()
	Если РасширенныйРежим = 0 Или РежимИзмененияЭлементаСтруктурыНастроек Тогда
		Возврат;
	КонецЕсли;
	...
	Если СвойствоЭлементаСтруктуры = Неопределено Или СвойствоЭлементаСтруктуры.Элементы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл
		...
		Если ОписаниеНастройки <> Неопределено Тогда
			...
			Строка.ОтображатьИспользование = (ОписаниеНастройки.Использование
...
&НаКлиентеНаСервереБезКонтекста
Функция УстановитьДанныеСтрокиОтборов(Строка, СвойствоЭлементаСтруктуры, ЭлементНастройки,
	...
	Если ДоступныеВидыСравнения <> Неопределено И ДоступныеВидыСравнения.Количество() > 0
...
Процедура ОтборыПередВыборомПоля(ИдентификаторСтроки = Неопределено)
	Если Не РежимИзмененияЭлементаСтруктурыНастроек Тогда
		...
		Если (Строка = Неопределено) Или (Строка.ЭтоПараметр) Или (Строка.ЭтоРаздел И Строка.Идентификатор
			= "ПараметрыДанных") Тогда
...
Процедура УстановитьПараметрыРедактированияЭлементаОтбора()
	...
	Если Строка = Неопределено Или Строка.ЭтоРаздел Или Строка.ЭтоГруппа Тогда
...
Функция ПараметрыГруппировкиОтборов()
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если Родитель.ПолучитьЭлементы().Индекс(Строка) < 0 Или ТипЗнч(Строка.Идентификатор) <> Тип(
			"ИдентификаторКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Родители.Количество() > 1 Тогда
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ИзменитьГруппировкуОтборов(ОтборыУзлаНастроек, Строки, НаследникиЭлементовНастроек, НаследникиСтрок)
	Для Каждого СтрокаИсточник Из Строки Цикл
		...
		Если Индекс < 0 Тогда
			ЭлементНастройкиПриемник = РодительЭлементаНастройкиПриемника.Элементы.Добавить(ТипЗнч(
				ЭлементНастройкиИсточник));
			СтрокаПриемник = РодительСтрокиПриемника.ПолучитьЭлементы().Добавить();
		Иначе // Это удаление группировки.
			ЭлементНастройкиПриемник = РодительЭлементаНастройкиПриемника.Элементы.Вставить(Индекс, ТипЗнч(
				ЭлементНастройкиИсточник));
		...
		ИзменитьГруппировкуОтборов(ОтборыУзлаНастроек, СтрокаИсточник.ПолучитьЭлементы(), НаследникиЭлементовНастроек,
			НаследникиСтрок);
	КонецЦикла;
КонецПроцедуры
...
Функция ПараметрыСдвигаОтборов()
	...
	Если Родители.Количество() > 1 Тогда
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПослеАктивизацииВыбранныхПолей()
	...
	Элементы.ВыбранныеПоляИзменитьФормулу.Доступность = Строка <> Неопределено И Строка.ЭтоФормула;
КонецПроцедуры
...
Процедура ВыбранныеПоляПередГруппировкойПолей(СвойстваГруппы, ПараметрыГруппировки) Экспорт
	...
	ИзменитьГруппировкуВыбранныхПолей(СвойствоЭлементаСтруктуры, ПараметрыГруппировки.Строки,
		НаследникиЭлементовНастроек, НаследникиСтрок);
...
&НаКлиенте
Процедура ИзменитьГруппировкуВыбранныхПолей(ВыбранныеПоляУзлаНастроек, Строки, НаследникиЭлементовНастроек,
	НаследникиСтрок)
	Для Каждого СтрокаИсточник Из Строки Цикл
		ЭлементНастройкиИсточник = ВыбранныеПоляУзлаНастроек.ПолучитьОбъектПоИдентификатору(
			СтрокаИсточник.Идентификатор);
		...
		ЗаполнитьЗначенияСвойств(ЭлементНастройкиПриемник, ЭлементНастройкиИсточник);
		ЗаполнитьЗначенияСвойств(СтрокаПриемник, СтрокаИсточник);
		СтрокаПриемник.Идентификатор = ВыбранныеПоляУзлаНастроек.ПолучитьИдентификаторПоОбъекту(
			ЭлементНастройкиПриемник);
		СтрокаПриемник.ЭтоГруппа = (ТипЗнч(ЭлементНастройкиПриемник) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных"));

		НаследникиЭлементовНастроек.Вставить(ЭлементНастройкиИсточник, ЭлементНастройкиПриемник);
		НаследникиСтрок.Вставить(СтрокаИсточник, СтрокаПриемник);

		ИзменитьГруппировкуВыбранныхПолей(ВыбранныеПоляУзлаНастроек, СтрокаИсточник.ПолучитьЭлементы(),
			НаследникиЭлементовНастроек, НаследникиСтрок);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПроверитьПеретаскиваемыеСтрокиИзВыбранныхПолей(ИдентификаторыСтрок)
	...
	Пока Индекс >= 0 Цикл
		...
		Если Родитель = Неопределено Или Родитель.ПолучитьЭлементы().Индекс(Строка) < 0 Или ТипЗнч(
			Строка.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Тогда
...
Процедура ПеретащитьВыбранныеПоляВПределахКоллекции(ПараметрыПеретаскивания, ТекущаяСтрока)
	...
	Если ТипЗнч(СтрокаПриемник.Идентификатор) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		ЭлементНастройкиПриемник = СвойствоЭлементаСтруктуры.ПолучитьОбъектПоИдентификатору(
			СтрокаПриемник.Идентификатор);
	КонецЕсли;
...
//
&НаКлиенте
Процедура ПеретащитьВыбранныеПоля(ВыбранныеПоляУзлаНастроек, Индекс, Строки, НаследникиЭлементовНастроек,
	НаследникиСтрок)
	Для Каждого СтрокаИсточник Из Строки Цикл
		ЭлементНастройкиИсточник = ВыбранныеПоляУзлаНастроек.ПолучитьОбъектПоИдентификатору(
			СтрокаИсточник.Идентификатор);
		...
		Если Индекс > РодительЭлементаНастройкиПриемник.Элементы.Количество() - 1 Тогда
			ЭлементНастройкиПриемник = РодительЭлементаНастройкиПриемник.Элементы.Добавить(ТипЗнч(
				ЭлементНастройкиИсточник));
			СтрокаПриемник = РодительСтрокиПриемник.ПолучитьЭлементы().Добавить();
		Иначе
			ЭлементНастройкиПриемник = РодительЭлементаНастройкиПриемник.Элементы.Вставить(Индекс, ТипЗнч(
				ЭлементНастройкиИсточник));
		...
		Если ТипЗнч(ЭлементНастройкиПриемник) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			СтрокаПриемник.Картинка = ОтчетыКлиентСервер.ИндексКартинки("Группа");
			ПеретащитьВыбранныеПоля(ВыбранныеПоляУзлаНастроек, Индекс, СтрокаИсточник.ПолучитьЭлементы(),
				НаследникиЭлементовНастроек, НаследникиСтрок);
		КонецЕсли
		;
	КонецЦикла;
...
Процедура ПривестиЗначениеКВидуСравнения(Строка, ЭлементНастройки = Неопределено)
	...
	Если Строка.ДоступенСписокЗначений Или ОтчетыКлиентСервер.ЭтоВидСравненияСписка(Строка.ВидСравнения) Тогда
	...
	Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Или ТипЗнч(ЭлементНастройки) = Тип(
		"ЭлементОтбораКомпоновкиДанных") Тогда
...
Процедура УстановитьПараметрыРедактирования(Строка)
	...
	ПолеЗначения.ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(ТекущиеНастройки, ПользовательскиеНастройки,
		ЭлементНастройки, РасширенныйРежим = 1);
...
Процедура ПоказатьСписокВыбора(Строка, СтандартнаяОбработка, Элемент)
	...
	Если ОтчетыКлиент.ЭтоВыборОбъектовМетаданных(Строка.ТипЗначения, ТекущееЗначение, Обработчик)
		Или ОтчетыКлиент.ЭтоВыборПользователей(ЭтотОбъект, Элемент, Строка.ТипЗначения, ТекущееЗначение,
		ПолеЗначения.ПараметрыВыбора, Обработчик) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПеретащитьВыбранныеПоляВСортировку(Строки)
	...
	Для Каждого СтрокаИсточник Из Строки Цикл
		ЭлементНастройкиИсточник = ВыбранныеПоляЭлементаСтруктуры.ПолучитьОбъектПоИдентификатору(
			СтрокаИсточник.Идентификатор);
		...
		Иначе
			...
			ЭлементНастройкиПриемник = СортировкаЭлементаСтруктуры.Элементы.Добавить(Тип(
				"ЭлементПорядкаКомпоновкиДанных"));
			ЗаполнитьЗначенияСвойств(ЭлементНастройкиПриемник, ЭлементНастройкиИсточник);
			ЭлементНастройкиПриемник.Использование = Истина;

			ОписаниеНастройки = СортировкаЭлементаСтруктуры.ДоступныеПоляПорядка.НайтиПоле(
				ЭлементНастройкиИсточник.Поле);

			СтрокаПриемник = Раздел.ПолучитьЭлементы().Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПриемник, ЭлементНастройкиПриемник);
			ЗаполнитьЗначенияСвойств(СтрокаПриемник, ОписаниеНастройки);
			СтрокаПриемник.Идентификатор = СортировкаЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(
				ЭлементНастройкиПриемник);
			СтрокаПриемник.Картинка = ВариантыОтчетовСлужебныйКлиентСервер.ИндексКартинкиПоля(
				ОписаниеНастройки.ТипЗначения);
		КонецЕсли;
	КонецЦикла;
...
Функция НайтиПолеПорядка(СортировкаУзлаНастроек, Поле)
	Для Каждого ЭлементНастройки Из СортировкаУзлаНастроек.Элементы Цикл
		Если ТипЗнч(ЭлементНастройки) <> Тип("АвтоЭлементПорядкаКомпоновкиДанных") И ЭлементНастройки.Поле = Поле Тогда
...
Процедура ПеретащитьПоляСортировкиВВыбранныеПоля(Строки)
	...
	Пока Индекс >= 0 Цикл
		...
		ЭлементНастройкиИсточник = СортировкаЭлементаСтруктуры.ПолучитьОбъектПоИдентификатору(
			СтрокаИсточник.Идентификатор);

		Если НайтиВыбранноеПоле(ВыбранныеПоляЭлементаСтруктуры, ЭлементНастройкиИсточник.Поле) = Неопределено Тогда
			ЭлементНастройкиПриемник = ВыбранныеПоляЭлементаСтруктуры.Элементы.Добавить(Тип(
				"ВыбранноеПолеКомпоновкиДанных"));
			ЗаполнитьЗначенияСвойств(ЭлементНастройкиПриемник, ЭлементНастройкиИсточник);

			ОписаниеНастройки = ВыбранныеПоляЭлементаСтруктуры.ДоступныеПоляВыбора.НайтиПоле(
				ЭлементНастройкиИсточник.Поле);

			СтрокаПриемник = РазделВыбранныхПолей.ПолучитьЭлементы().Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПриемник, ЭлементНастройкиПриемник);
			ЗаполнитьЗначенияСвойств(СтрокаПриемник, ОписаниеНастройки);
			СтрокаПриемник.Идентификатор = ВыбранныеПоляЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(
				ЭлементНастройкиПриемник);
			СтрокаПриемник.Картинка = ОтчетыКлиентСервер.ИндексКартинки("Элемент");
		КонецЕсли;
...
Функция НайтиВыбранноеПоле(ВыбранныеПоляУзлаНастроек, Поле)
	...
	Для Каждого ЭлементНастройки Из ВыбранныеПоляУзлаНастроек.Элементы Цикл
		...
		ИначеЕсли ТипЗнч(ЭлементНастройки) = Тип("ВыбранноеПолеКомпоновкиДанных") И ЭлементНастройки.Поле = Поле Тогда
			НайденноеПоле = Поле;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выводить заголовок для печати: %1'"), ?(ЗначениеЗаполнено(Объект.Значение), Объект.Значение, НСтр(
		"ru = '<Отсутствует>'")));
	...
	Строка = Оформление.ПолучитьЭлементы().Добавить();
	Строка.Использование = (Объект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить Или СвязанныйОбъект.Значение
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ЭлементУсловногоОформленияОтчета", ПараметрыФормы,
...
Процедура ОформлениеИзменитьЭлементЗавершение(Результат, ИдентификаторСтроки) Экспорт
	...
	Если Результат.Свойство("ОписаниеФормул") Тогда
		ВариантыОтчетовСлужебныйКлиент.ДобавитьФормулы(Настройки, Настройки.ДоступныеПоляВыбора,
			Результат.ОписаниеФормул);
	КонецЕсли;
...
Процедура ОформлениеВводЗаголовкаЗавершение(Значение, Идентификатор) Экспорт
	...
	Строка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выводить заголовок для печати: %1'"), ?(ЗначениеЗаполнено(Значение), Значение, "<Отсутствует>"));
...
Процедура ОбновитьКоллекциюСтруктуры(Знач Узел, Знач ИмяКоллекции, Строки)
	...
	ИначеЕсли ИменаКоллекций.Найти(ИмяКоллекции) <> Неопределено И (ТипКоллекции = Тип(
		"КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипКоллекции = Тип(
		"КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных")) Тогда
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	Если ТипЭлемента = Тип("ГруппировкаКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаТаблицыКомпоновкиДанных")
...
Процедура УстановитьПризнакиВложенныхЭлементовНастроек(ЭлементСтруктуры, СвойстваЭлементаСтруктуры)
	...
	Если ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Или ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Тогда
		Возврат;
	КонецЕсли;
	...
	СодержитПоля = ВложенныеЭлементы.Количество() > 0 И Не (ВложенныеЭлементы.Количество() = 1 И ТипЗнч(
		ВложенныеЭлементы[0]) = Тип("АвтоВыбранноеПолеКомпоновкиДанных"));

	ВложенныеЭлементы = Элемент.Порядок.Элементы;
	СодержитСортировки = ВложенныеЭлементы.Количество() > 0 И Не (ВложенныеЭлементы.Количество() = 1 И ТипЗнч(
		ВложенныеЭлементы[0]) = Тип("АвтоЭлементПорядкаКомпоновкиДанных"));
	...
	Если Строка <> Неопределено Тогда
		...
		Если СледующийУровень Тогда
			...
			ИначеЕсли Строка.ПолучитьЭлементы().Количество() > 1 Тогда
				ПараметрыВыполнения.Оборачивать = Ложь;
			ИначеЕсли Строка.Тип = "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных" И Строка.Подтип
				= "ТаблицаКолонки" Тогда
				ПараметрыВыполнения.Оборачивать = Ложь;
			КонецЕсли;
		КонецЕсли;

		Пока Строка <> Неопределено Цикл
			Если Строка.Тип = "НастройкиКомпоновкиДанных" Или Строка.Тип = "НастройкиВложенногоОбъектаКомпоновкиДанных"
				Или Строка.Тип = "ГруппировкаКомпоновкиДанных" Или Строка.Тип = "ГруппировкаТаблицыКомпоновкиДанных"
...
Процедура ДобавитьЭлементСтруктурыНастроек(ТипЭлемента)
	...
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		...
		УстановитьПараметрВывода(ЭлементНастройки, "ТипДиаграммы.СоединениеЗначенийПоСериям",
		...
		УстановитьПараметрВывода(ЭлементНастройки, "ТипДиаграммы.ЦветСоединенияЗначенийПоСериям",
			WebЦвета.СеребристоСерый);
		УстановитьПараметрВывода(ЭлементНастройки, "ТипДиаграммы.РежимСглаживания",
			РежимСглаживанияДиаграммы.ГладкаяКривая);
		УстановитьПараметрВывода(ЭлементНастройки, "ТипДиаграммы.РежимПолупрозрачности",
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(
			ПодчиненныйЭлементНастройки);
...
Процедура СтруктураВариантаПослеВыбораПоля(ОписаниеНастройки, ПараметрыВыполнения) Экспорт
	...
	Результат = ВставитьЭлементСтруктурыНастроек(Тип("ГруппировкаКомпоновкиДанных"), ТекущаяСтрока,
		ПараметрыВыполнения.СледующийУровень);
...
&НаКлиенте
Функция ПереместитьЭлементыСтруктурыВарианта(Знач Строка, Знач НовыйРодитель, Знач ПередЧемВставить = Неопределено,
	Знач Индекс = Неопределено, Знач ИндексЭлементаНастройки = Неопределено)
	...
	Иначе
		...
		Результат.ЭлементНастройки = ОтчетыКлиентСервер.СкопироватьРекурсивно(УзелКД, ЭлементКД, КудаВставитьКД,
			ИндексЭлементаНастройки, ПоискЭлементовКД);

		ПоискСтрокТаблицы = Новый Соответствие;
		Результат.Строка = ОтчетыКлиентСервер.СкопироватьРекурсивно(Неопределено, Строка, КудаВставить, Индекс,
			ПоискСтрокТаблицы);
...
Процедура УстановитьДоступностьКомандСтруктурыВарианта(ИдентификаторСтроки)
	...
	МожноДобавлятьВложенные = (Строка.Тип <> "ТаблицаКомпоновкиДанных" И Строка.Тип <> "ДиаграммаКомпоновкиДанных");

	МожноГруппировать = (Строка.Тип <> "НастройкиКомпоновкиДанных" И Строка.Тип <> "НастройкиВложенногоОбъектаКомпоновкиДанных"
		И Строка.Тип <> "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных" И Строка.Тип
		<> "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных");

	МожноОткрывать = (Строка.Тип <> "НастройкиКомпоновкиДанных" И Строка.Тип <> "НастройкиВложенногоОбъектаКомпоновкиДанных"
		И Строка.Тип <> "ТаблицаКомпоновкиДанных" И Строка.Тип <> "ДиаграммаКомпоновкиДанных" И Строка.Тип <> "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных"
		И Строка.Тип <> "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных");

	МожноУдалятьИПеремещать = (Строка.Тип <> "НастройкиКомпоновкиДанных" И Строка.Тип <> "НастройкиВложенногоОбъектаКомпоновкиДанных"
		И Строка.Тип <> "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных" И Строка.Тип
		<> "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных");

	МожноДобавлятьТаблицыИДиаграммы = (Строка.Тип = "НастройкиКомпоновкиДанных" Или Строка.Тип = "НастройкиВложенногоОбъектаКомпоновкиДанных"
		Или Строка.Тип = "ГруппировкаКомпоновкиДанных");

	РодителяМожноПеремещать = (ЕстьРодитель И Родитель.Тип <> "НастройкиКомпоновкиДанных" И Родитель.Тип <> "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных"
	...
	Элементы.СтруктураВарианта_ПереместитьВышеИЛевее.Доступность  = МожноУдалятьИПеремещать И РодителяМожноПеремещать
		И МожноДобавлятьВложенные И МожноГруппировать;
	Элементы.СтруктураВарианта_ПереместитьВышеИЛевее1.Доступность = МожноУдалятьИПеремещать И РодителяМожноПеремещать
		И МожноДобавлятьВложенные И МожноГруппировать;
	Элементы.СтруктураВарианта_ПереместитьНижеИПравее.Доступность  = МожноУдалятьИПеремещать И ЕстьПодчиненные
		И МожноДобавлятьВложенные И МожноГруппировать;
	Элементы.СтруктураВарианта_ПереместитьНижеИПравее1.Доступность = МожноУдалятьИПеремещать И ЕстьПодчиненные
		И МожноДобавлятьВложенные И МожноГруппировать;
	...
	Элементы.СтруктураВарианта_ПереместитьНаУровеньВыше.Доступность = МожноУдалятьИПеремещать И ЕстьРодитель
		И РодителяМожноПеремещать;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньВыше1.Доступность = МожноУдалятьИПеремещать И ЕстьРодитель
		И РодителяМожноПеремещать;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньНиже.Доступность = МожноУдалятьИПеремещать
		И ЕстьПотенциальныеРодители;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньНиже1.Доступность = МожноУдалятьИПеремещать
		И ЕстьПотенциальныеРодители;
КонецПроцедуры
...
Процедура ИзменитьЭлементСтруктуры(Строка, ИмяСтраницы = Неопределено, ИспользоватьФормуВарианта = Неопределено)
	...
	Если ИспользоватьФормуВарианта = Неопределено Тогда
		ИспользоватьФормуВарианта = (Строка.Тип = "ТаблицаКомпоновкиДанных" Или Строка.Тип
...
Процедура ИзменитьЭлементСтруктурыЗавершение(Результат, Контекст) Экспорт
	Если ТипЗнч(Результат) <> Тип("Структура") Или Результат = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
...
Процедура НачатьЗаполнениеСписка(Элемент, ПараметрыЗаполнения, ВыборПереопределен = Неопределено)
	...
	Если ОтчетыКлиент.ВыборПереопределен(ЭтотОбъект, Обработчик, Сведения.Описание, Список.ТипЗначения,
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПодбор", Ложь)
		...
		Или ОтчетыКлиент.ЭтоВыборПользователей(ЭтотОбъект, Элемент, Список.ТипЗначения, ОтмеченныеЗначения,
...
Процедура ЗавершитьЗаполнениеСписка(ВыбранныеЗначения, ПараметрыЗаполнения) Экспорт
	...
	Пока Индекс >= 0 Цикл
		...
		Если Элемент.Пометка И ВыбранныеЗначения.НайтиПоЗначению(Элемент.Значение) = Неопределено Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, Ключ, ИдентификаторЭлемента = Неопределено,
	Режим = Неопределено)
	...
	Если ЭлементСтруктуры = Неопределено Или (ТипЭлементаСтруктуры = Тип("ТаблицаКомпоновкиДанных") И Ключ <> "Выбор"
		И Ключ <> "УсловноеОформление") Или (ТипЭлементаСтруктуры = Тип("ДиаграммаКомпоновкиДанных") И Ключ <> "Выбор"
		И Ключ <> "УсловноеОформление") Или (ТипЭлементаСтруктуры = Тип("НастройкиКомпоновкиДанных") И Ключ
		= "ПоляГруппировки") Или (ТипЭлементаСтруктуры = Тип("ГруппировкаКомпоновкиДанных") И Ключ = "ПараметрыДанных")
	...
	Если Режим = 0 И (ТипЗнч(СвойствоЭлементаСтруктуры) = Тип("ВыбранныеПоляКомпоновкиДанных") Или ТипЗнч(
		СвойствоЭлементаСтруктуры) = Тип("ПорядокКомпоновкиДанных")) И ЗначениеЗаполнено(
		СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки) Тогда
...
Процедура УдалитьСтроки(Элемент, Отказ)
	...
	Пока Индекс >= 0 Цикл
		...
		Если ТипЗнч(Строка.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Или (Строка.Свойство("ЭтоРаздел")
			И Строка.ЭтоРаздел) Или (Строка.Свойство("ЭтоПараметр") И Строка.ЭтоПараметр) Или (Строка.Свойство(
			"ЭтоПараметрВывода") И Строка.ЭтоПараметрВывода) Тогда
			Продолжить;
		КонецЕсли;
		...
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
		Если ТипЗнч(ЭлементНастройки) = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипЗнч(
			ЭлементНастройки) = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ИзменитьИспользование(ИмяКоллекции, Использование = Истина, Строки = Неопределено)
	...
	Для Каждого Строка Из Строки Цикл
		...
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
		Если ТипЗнч(ЭлементНастройки) <> Тип("НастройкиКомпоновкиДанных") И ТипЗнч(ЭлементНастройки) <> Тип(
			"КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") И ТипЗнч(ЭлементНастройки) <> Тип(
			"КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
			ЭлементНастройки.Использование = Использование;
		КонецЕсли;
...
Процедура ИзменитьИспользованиеЭлементаНастройки(ИмяКоллекции)
	...
	Если Строка.Свойство("ЭтоПараметрВывода") И Строка.ЭтоПараметрВывода И Строка(Строка.Идентификатор)
		= "DATAPARAMETERSOUTPUT" Тогда

		ЭлементНастройки.Использование = Истина;
		ЭлементНастройки.Значение = ?(Строка.Использование, ТипВыводаТекстаКомпоновкиДанных.Авто,
			ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
	КонецЕсли;
...
Функция РезультатВыбора(Переформировать)
	...
	РезультатВыбора.Вставить(
		"ПользовательскиеНастройкиМодифицированы", ВариантИзменен Или ПользовательскиеНастройкиМодифицированы);
...
Процедура УстановитьПараметрВывода(ЭлементСтруктуры, ИмяПараметра, Значение = Неопределено, Использование = Истина)
	ЗначениеПараметра = ЭлементСтруктуры.ПараметрыВывода.НайтиЗначениеПараметра(
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	Особенности.МогутБытьРазделы = (Контекст.ТаблицаИмя = "Отборы" Или Контекст.ТаблицаИмя = "ВыбранныеПоля"
		Или Контекст.ТаблицаИмя = "Сортировка" Или Контекст.ТаблицаИмя = "Оформление");
	...
	Особенности.ТребоватьОдногоРодителя = (Контекст.Действие = "Переместить" Или Контекст.Действие = "ПереместитьВИерархии"
...
Функция СтрокаДобавлена(Строки, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьРазделы И СтрокаДерева.ЭтоРаздел Тогда
		Возврат Истина; // Пропустить строку.
	КонецЕсли;
	Если (Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр) Или (Особенности.МогутБытьПараметрыВывода
		И СтрокаДерева.ЭтоПараметрВывода) Тогда
...
Процедура ПереместитьВИерархии(Контекст)
	...
	Если НовыйРодитель = Неопределено Или НовыйРодитель.Тип = "ТаблицаКомпоновкиДанных" Или НовыйРодитель.Тип
		= "ДиаграммаКомпоновкиДанных" Тогда
...
&НаКлиенте
Процедура ВыбратьРежимОтображенияДляСтрок(СвойстваЭлементовНастроек, ИмяКоллекции, ПоказыватьРежимыФлажков = Ложь,
	ТекущийРежимОтображения = Неопределено)
...
&НаКлиенте
Процедура ВыбратьРежимОтображенияДляСтроки(ОтборыУзлаНастроек, ИмяКоллекции, ИдентификаторСтроки,
	ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков, ТекущийРежимОтображения = Неопределено)

	Контекст = Новый Структура("ОтборыУзлаНастроек, ИмяКоллекции, ИдентификаторСтроки", ОтборыУзлаНастроек,
		ИмяКоллекции, ИдентификаторСтроки);
...
Функция ДоступныеРежимыОтображения(ПоказыватьРежимыФлажков)
	...
	ДоступныеРежимыОтображения.Добавить("ПоказыватьВШапкеОтчета", НСтр("ru = 'В шапке отчета'"), ,
		БиблиотекаКартинок.БыстрыйДоступ);

	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр(
			"ru = 'Только флажок в шапке отчета'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	...
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр(
			"ru = 'Только флажок в настройках отчета'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;

	ДоступныеРежимыОтображения.Добавить("НеПоказывать", НСтр("ru = 'Не показывать'"), ,
		БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Функция КартинкаРежимаОтображенияЭлементаНастройки(ЭлементНастройки)
	...
	Если ЗначениеЗаполнено(ЭлементНастройки.ИдентификаторПользовательскойНастройки) Тогда
		Если ЭлементНастройки.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ Тогда
			Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Или ТипЗнч(
				ЭлементНастройки) = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
...
Функция ЭлементНастройки(Знач УзелНастроек, Знач Строка)
	...
	Если Строка <> Неопределено И ТипЗнч(Строка.Идентификатор) = Тип("ИдентификаторКомпоновкиДанных") Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭлементыНастроек(Знач СвойствоЭлементаСтруктуры, Знач ЭлементНастройки = Неопределено,
	Знач ИмяКоллекции = Неопределено)
	Если ЭлементНастройки = Неопределено Тогда
	...
	Если ИмяКоллекции <> Неопределено Тогда
		...
		Если ТипОбъекта = Тип("ТаблицаКомпоновкиДанных") И СтрРазделить("Строки, Колонки", ", ", Ложь).Найти(
			ИмяКоллекции) <> Неопределено Или ТипОбъекта = Тип("ДиаграммаКомпоновкиДанных") И СтрРазделить(
			"Серии, Точки", ", ", Ложь).Найти(ИмяКоллекции) <> Неопределено Тогда

			Возврат ЭлементНастройки[ИмяКоллекции];
		КонецЕсли;
	КонецЕсли;

	Если ТипОбъекта = Тип("НастройкиКомпоновкиДанных") Или ТипОбъекта = Тип("ГруппировкаКомпоновкиДанных")
		Или ТипОбъекта = Тип("ГруппировкаТаблицыКомпоновкиДанных") Или ТипОбъекта = Тип(
		"ГруппировкаДиаграммыКомпоновкиДанных") Тогда

		Возврат ЭлементНастройки.Структура;
	ИначеЕсли ТипОбъекта = Тип("КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных") Или ТипОбъекта = Тип(
		"КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипОбъекта = Тип(
		"КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
...
Функция ПолучитьРодителяЭлементаНастройки(Знач СвойствоЭлементаСтруктуры, Знач ЭлементНастройки)
	...
	Если ЭлементНастройки <> Неопределено И ТипЭлемента <> Тип("ПолеГруппировкиКомпоновкиДанных") И ТипЭлемента <> Тип(
		"АвтоПолеГруппировкиКомпоновкиДанных") И ТипЭлемента <> Тип("АвтоЭлементПорядкаКомпоновкиДанных") И ТипЭлемента
		<> Тип("ЭлементПорядкаКомпоновкиДанных") И ТипЭлемента <> Тип("ЭлементУсловногоОформленияКомпоновкиДанных")
...
Процедура ВыбратьПоле(ИмяКоллекции, Обработчик, Поле = Неопределено, ИдентификаторУзлаНастроек = Неопределено)
	...
	ПараметрыВыбора.Вставить("ИдентификаторЭлементаСтруктурыНастроек", ?(ИдентификаторУзлаНастроек = Неопределено,
		ИдентификаторЭлементаСтруктурыНастроек, ИдентификаторУзлаНастроек));

	ОткрытьФорму(
		"ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборПоляОтчета", ПараметрыВыбора, ЭтотОбъект,
		УникальныйИдентификатор, , , Обработчик, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ОбновитьЗаголовокЭлементаСтруктурыВарианта(Строка)
	...
	ЗначениеПараметра = ЭлементНастройки.ПараметрыВывода.НайтиЗначениеПараметра(
...
Процедура УстановитьТипДиаграммы()
	...
	ЭлементНастройки = ЭлементСтруктуры.ПараметрыВывода.НайтиЗначениеПараметра(
		Новый ПараметрКомпоновкиДанных("ТипДиаграммы"));
	Если ЭлементНастройки <> Неопределено Тогда
...
#Область СвойстваЭлементовСтраницыСоставГруппировки
	...
	ОтображатьСоставГруппировки = (ЭтоРасширенныйРежим И РежимИзмененияЭлементаСтруктурыНастроек
...
#Область СвойстваЭлементовСтраницыОтборы
	...
	ОтображатьОтборы = (ЭтоРасширенныйРежим И ТипЭлементаСтруктурыНастроек <> "ДиаграммаКомпоновкиДанных");
	...
	Элементы.ГруппаЕстьВложенныеОтборы.Видимость = ОтображатьОтборы И СодержитВложенныеОтборы И ОтображатьИнформацию;
...
#Область СвойстваЭлементовСтраницыПоляИСортировки
	...
	ОтображатьВыбранныеПоля = СвойствоЭлементаСтруктуры <> Неопределено И (ЗначениеЗаполнено(
		СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки) Или ЭтоРасширенныйРежим);

	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(
		КомпоновщикНастроек, "Порядок", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	ОтображатьСортировки = ТипЭлементаСтруктурыНастроек <> "ДиаграммаКомпоновкиДанных" И СвойствоЭлементаСтруктуры <> Неопределено
...
#Область СвойстваЭлементовСтраницыОформление
	...
	Элементы.ГруппаЕстьВложенноеОформление.Видимость = ОтображатьОформление И СодержитВложенноеУсловноеОформление
...
#Область СвойстваЭлементовСтраницыСтруктураВарианта
	...
	ОтображатьСтруктуруВарианта = НастройкиОтчета.РазрешеноИзменятьСтруктуру И Не РежимИзмененияЭлементаСтруктурыНастроек
...
#Область ОбщиеСвойстваЭлементов
	...
	ИначеЕсли Не ЭтоРасширенныйРежим И ЭтоМобильныйКлиент Тогда
		ОтображатьСтраницы = Ложь;
	Иначе
		ОтображатьСтраницы = ОтображатьСоставГруппировки Или ОтображатьОтборы Или ОтображатьВыбранныеПоля Или ОтображатьСортировки
			Или ОтображатьОформление Или ОтображатьСтруктуруВарианта;
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	ОбновитьНастройкиВарианта = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗагрузки,
		"ОбновитьНастройкиВарианта", Ложь);
	...
	Если НастройкиОтчета.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		ОтчетОбъект.ПередЗагрузкойНастроекВКомпоновщик(
			ЭтотОбъект, НастройкиОтчета.КлючСхемы, КлючТекущегоВарианта, ДоступныеНастройки.Настройки,
	...
	Если НастройкиЗагружены И ФормаПараметры.НачальныйКлючВарианта = КлючТекущегоВарианта И ТипЗнч(
		ФормаПараметры.Отбор) = Тип("Структура") Тогда
...
Процедура ИнициализироватьРеквизитыПредопределенныхПараметровВывода()
	...
	Если ПолеВыводаЗаголовка <> Неопределено Тогда
		ПолеВыводаЗаголовка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выводить заголовок для печати: %1'"), ?(ЗначениеЗаполнено(Объект.Значение), Объект.Значение,
			НСтр("ru = '<Отсутствует>'")));
	КонецЕсли;
	...
	ВыводитьФильтры = (Объект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить Или СвязанныйОбъект.Значение
		<> ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
КонецПроцедуры
...
Функция РазрешитьИзменятьУсловияОтборов()
	...
	Для Каждого ЭлементПользовательскойНастройки Из ПользовательскиеНастройки.Элементы Цикл
		ЭлементНастройки = ОтчетыКлиентСервер.ПолучитьОбъектПоПользовательскомуИдентификатору(
			КомпоновщикНастроек.Настройки, ЭлементПользовательскойНастройки.ИдентификаторПользовательскойНастройки, ,
			ПользовательскиеНастройки);

		Если ТипЗнч(ЭлементНастройки) <> Тип("ЭлементОтбораКомпоновкиДанных") Или ТипЗнч(
			ЭлементНастройки.ПравоеЗначение) = Тип("СтандартныйПериод") Или ЭлементНастройки.РежимОтображения
...
Процедура НайтиПомеченныеНаУдалениеПоля(Знач ЭлементыСтруктуры = Неопределено)
	...
	Обработанные.Сортировать(
		"ИдентификаторЭлементаСтруктуры Убыв, КлючСвойстваЭлементаСтруктуры, ИдентификаторЭлемента");
...
Процедура НайтиПомеченныеНаУдалениеВыбранныеПоля(Настройки, Знач ЭлементСтруктуры = Неопределено, Группа = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ТипЗнч(ЭлементНастройки) = ТипАвтоПоля Или ДоступныеПоля.НайтиПоле(ЭлементНастройки.Поле) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура НайтиПомеченныеНаУдалениеПоляОтбора(Настройки, Знач ЭлементСтруктуры = Неопределено, Группа = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ТипЗнч(ЭлементНастройки.ЛевоеЗначение) <> Тип("ПолеКомпоновкиДанных") Или ДоступныеПоля.НайтиПоле(
			ЭлементНастройки.ЛевоеЗначение) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
&НаСервере
Процедура НайтиПомеченныеНаУдалениеЭлементыОтбораУсловногоОформления(Настройки, ЭлементСтруктуры, ЭлементОформления,
	Группа = Неопределено)
	СвойствоЭлементаСтруктуры = ЭлементСтруктуры.УсловноеОформление;
...
Функция ИменаКоллекцийЭлементаСтруктуры(Элемент)
	...
	ТипЭлемента = ТипЗнч(Элемент);
	Если ТипЭлемента = Тип("ГруппировкаКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаТаблицыКомпоновкиДанных")
		Или ТипЭлемента = Тип("ГруппировкаДиаграммыКомпоновкиДанных") Или ТипЭлемента = Тип(
		"НастройкиВложенногоОбъектаКомпоновкиДанных") Или ТипЭлемента = Тип(
		"КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных") Тогда

		ИменаКоллекций = "Структура";

	ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Или ТипЭлемента = Тип(
		"КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Тогда

		ИменаКоллекций = "Строки, Колонки";

	ИначеЕсли ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Или ТипЭлемента = Тип(
		"КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
...
Процедура УдалитьПомеченныеНаУдалениеПоля()
	...
	Для Каждого Запись Из ПомеченныеНаУдаление Цикл
		СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(
			КомпоновщикНастроек, Запись.КлючСвойстваЭлементаСтруктуры, Запись.ИдентификаторЭлементаСтруктуры,
			РасширенныйРежим);
		Если СвойствоЭлементаСтруктуры = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЭлементыНастроек.Количество() = 0 И ТипЗнч(РодительЭлементаНастройки) = Тип(
			"ПоляГруппировкиКомпоновкиДанных") Тогда

			ЭлементСтруктуры = КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(
				Запись.ИдентификаторЭлементаСтруктуры);
			...
			Иначе // ГруппировкаТаблицыКомпоновкиДанных или ГруппировкаДиаграммыКомпоновкиДанных.
				РодительЭлементаСтруктуры = ПолучитьРодителяЭлементаНастройки(СвойствоЭлементаСтруктуры,
					ЭлементСтруктуры);
...
&НаКлиенте
Процедура ОпределитьДанныеВыбораЭлементаОтбораПоЗапросу(ИсходныйЗапрос, ДанныеВыбора, ДоступныеПоля,
	Запрос = Неопределено, Родитель = Неопределено, Индекс = 0)
...
Процедура ЗаполнитьДанныеВыбораЭлементаОтбора(ИсходныйЗапрос, ДочерниеЭлементы, ДанныеВыбора)
	...
	Для Каждого Элемент Из ДочерниеЭлементы Цикл
		...
		Если Не Элемент.Папка И Не Элемент.Таблица И СтрНачинаетсяС(ВРег(Элемент.Заголовок), ВРег(ИсходныйЗапрос)) Тогда
...
Функция ПредставлениеЭлементаОтбора(ЭлементОтбора, ИсходныйЗапрос)
	...
	ФрагментЗапроса = ФрагментЗапросаЭлементаОтбора(Сред(ЭлементОтбора.Заголовок, НачалоЗапроса,
		ДлинаНайденногоФрагмента));
	ФрагментПослеЗапроса = Сред(ЭлементОтбора.Заголовок, КонецЗапроса);

	Возврат Новый ФорматированнаяСтрока(ФрагментДоЗапроса, ФрагментЗапроса, ФрагментПослеЗапроса);
...
Функция ФрагментЗапросаЭлементаОтбора(Запрос)
	...
	ШаблонФрагментаЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"<span style=""color: %1; font: %2"">%3</span>", "МоиВариантыОтчетовЦвет", "ВажнаяНадписьШрифт", Запрос);
...
Функция ДоступноеПолеОтбораПоЗаголовку(ЗаголовокПоля, ДочерниеЭлементы)
	...
	Для Каждого Элемент Из ДочерниеЭлементы Цикл
		...
		Если Не Элемент.Папка И Не Элемент.Таблица И ВРег(Элемент.Заголовок) = ВРег(ЗаголовокПоля) Тогда
...
Процедура ОткрытьФормуНастроекДляТехническогоСпециалиста(ИмяСтраницы, ОбработчикЗавершения = Неопределено)
	...
	ПараметрыФормы.Вставить("ПользовательскиеНастройкиМодифицированы", ВариантИзменен
	...
	ОткрытьФорму(НастройкиОтчета.ПолноеИмя + ".ФормаВарианта", ПараметрыФормы, ВладелецФормы, , , ,
		ОбработчикЗавершения, РежимОкна);
