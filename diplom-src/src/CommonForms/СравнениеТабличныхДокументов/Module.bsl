...
Функция ПолучитьДанныеДляСравнения(ТаблицаЗначенийИсточник, ПоСтрокам)
	...
	Граница1 = ?(ПоСтрокам, ТаблицаЗначенийИсточник.Количество(), ТаблицаЗначенийИсточник.Колонки.Количество()) - 1;

	Граница2 = ?(ПоСтрокам, ТаблицаЗначенийИсточник.Колонки.Количество(), ТаблицаЗначенийИсточник.Количество()) - 1;
...
Процедура ОбработатьАктивизациюОбласти(ТабДокИсточник, ТабДокПриемник, СоответствияИсточник, СоответствияПриемник)
	...
	Иначе
		...
		Иначе
			НомерСтроки = ТекОбласть.Низ - СоответствияИсточник.Строки.Количество()
		...
		Иначе
			НомерСтолбца = ТекОбласть.Лево - СоответствияИсточник.Столбцы.Количество()
...
Процедура ПредыдущееИзменение(ЭлементФормы, РеквизитФормы, ТаблицаРазличий)
	...
	Для Каждого ТекСтрока Из ТаблицаРазличий Цикл
		Если ТекСтрока.НомерСтроки < НомерСтроки Или ТекСтрока.НомерСтроки = НомерСтроки И ТекСтрока.НомерСтолбца
			< НомерСтолбца Тогда
...
Процедура СледующееИзменение(ЭлементФормы, РеквизитФормы, ТаблицаРазличий)
	...
	НомерСтолбца = ТекЯчейка.Лево;
	Для Каждого ТекСтрока Из ТаблицаРазличий Цикл
		Если ТекСтрока.НомерСтроки = НомерСтроки И ТекСтрока.НомерСтолбца > НомерСтолбца Или ТекСтрока.НомерСтроки
			> НомерСтроки Тогда
			Индекс = ТаблицаРазличий.Индекс(ТекСтрока);
			Прервать;
		КонецЕсли;
