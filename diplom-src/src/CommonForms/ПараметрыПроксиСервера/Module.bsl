...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.НастройкаПроксиНаКлиенте И Не Пользователи.ЭтоПолноправныйПользователь( , Истина) Тогда
	...
	Если ТипЗнч(НастройкаПроксиСервера) = Тип("Соответствие") Тогда
		...
		Если ИспользоватьПрокси И Не ИспользоватьСистемныеНастройки Тогда
			...
			НеИспользоватьПроксиДляЛокальныхАдресов = НастройкаПроксиСервера.Получить(
...
#Если ВебКлиент Тогда
		ПоказатьПредупреждение( , НСтр(
			"ru = 'В веб-клиенте параметры прокси-сервера необходимо задавать в настройках браузера.'"));
		Отказ = Истина;
		Возврат;
#КонецЕсли
...
Процедура ВариантыИспользованияПроксиСервераПриИзменении(Элемент)
	...
	ИначеЕсли ВариантИспользованияПроксиСервера = 1 Тогда
		НастройкиПрокси = ?(НастройкаПроксиНаКлиенте, СистемныеНастройкиПроксиСервера(),
...
Процедура ИнициализироватьЭлементыФормы(Форма, НастройкиПрокси)
	...
	Если НастройкиПрокси <> Неопределено Тогда
		...
		Форма.ОдинПроксиДляВсехПротоколов = (Форма.Сервер = Форма.СерверHTTP И Форма.СерверHTTP = Форма.СерверHTTPS
			И Форма.СерверHTTPS = Форма.СерверFTP И Форма.Порт = Форма.ПортHTTP И Форма.ПортHTTP = Форма.ПортHTTPS
...
Процедура СохранитьНастройкиПроксиСервера(ЗакрытьФорму = Истина)
	...
	Если Не ОдинПроксиДляВсехПротоколов Тогда
		...
		Если Не ПустаяСтрока(СерверHTTP) Тогда
			ДополнительныеНастройки.Вставить("http", Новый Структура("Адрес,Порт", НормализованныйАдресПроксиСервера(
				СерверHTTP), ПортHTTP));
		КонецЕсли;

		Если Не ПустаяСтрока(СерверHTTPS) Тогда
			ДополнительныеНастройки.Вставить("https", Новый Структура("Адрес,Порт", НормализованныйАдресПроксиСервера(
				СерверHTTPS), ПортHTTPS));
		КонецЕсли;

		Если Не ПустаяСтрока(СерверFTP) Тогда
			ДополнительныеНастройки.Вставить("ftp", Новый Структура("Адрес,Порт", НормализованныйАдресПроксиСервера(
				СерверFTP), ПортFTP));
		КонецЕсли;
...
Процедура ЗаписатьНастройкиПроксиСервераВИнформационнуюБазу(НастройкаПроксиНаКлиенте, НастройкаПроксиСервера)
	...
	Если НастройкаПроксиНаКлиенте Или ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
