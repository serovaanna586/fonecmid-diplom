...
Функция ПредставлениеОбъектаДляВыводаВОтчет(Выборка)
	...
	Если ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Тогда
		Если (Выборка.СуммаДокумента <> 0) И (Выборка.СуммаДокумента <> Null) Тогда
			ПредставлениеОбъекта = ПредставлениеОбъекта + " " + НСтр("ru = 'на сумму'") + " " + Выборка.СуммаДокумента
				+ " " + Выборка.Валюта;
		КонецЕсли;
	Иначе
...
Процедура ОбновитьДеревоСтруктурыПодчиненности()
	...
	Если Не ОсновнойДокументДоступен() Тогда
		...
		ТекстСообщения = НСтр(
			"ru = 'Документ, для которого сформирован отчет о структуре подчиненности, стал недоступен.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Возврат;

	КонецЕсли;

	Заголовок = Метаданные.ОбщиеФормы.СвязанныеДокументы.Представление() + ": " + ОбщегоНазначения.ПредметСтрокой(
		ОсновнойОбъект);
...
Процедура ВывестиРодительскиеОбъекты(ТекущийОбъект, ДеревоРодитель, ВыведенныеОбъекты, СлужебныеОбъекты = Неопределено,
	ИндексСвязейОбъектов = Неопределено)
	...
	Для Каждого Реквизит Из МетаданныеОбъекта.Реквизиты Цикл
		...
		Для Каждого ТекущийТип Из Реквизит.Тип.Типы() Цикл
			...
			Если Не Метаданные.Документы.Содержит(МетаданныеРеквизита) И Не Метаданные.Справочники.Содержит(
				МетаданныеРеквизита) И Не Метаданные.ПланыВидовХарактеристик.Содержит(МетаданныеРеквизита) Тогда
				Продолжить;
			КонецЕсли;

			ЗначениеРеквизита = ТекущийОбъект[Реквизит.Имя];
			Если ЗначениеЗаполнено(ЗначениеРеквизита) И ТипЗнч(ЗначениеРеквизита) = ТекущийТип И ЗначениеРеквизита <> ТекущийОбъект
	...
	Для Каждого ТЧ Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		Для Каждого КолонкаТЧ Из СодержимоеТЧ.Колонки Цикл
			...
			Для Каждого СтрокаТЧ Из СодержимоеТЧ Цикл
				...
				Если ЗначениеРеквизита = ТекущийОбъект Или СписокРеквизитов.НайтиПоЗначению(ЗначениеРеквизита)
					<> Неопределено Тогда
					Продолжить;
				КонецЕсли;
	...
	Для Каждого ЭлементСписка Из СписокРеквизитов Цикл
		...
		Если Выборка.Следующий() Тогда
			...
			Если НоваяСтрока <> Неопределено И Не ДобавляемыйОбъектИмеетсяСредиРодителей(ДеревоРодитель,
				ЭлементСписка.Значение) Тогда

				ВывестиРодительскиеОбъекты(ЭлементСписка.Значение, НоваяСтрока, ВыведенныеОбъекты, СлужебныеОбъекты,
					ИндексСвязейОбъектов);
...
Процедура ВывестиПодчиненныеОбъекты(ТекущийОбъект, ДеревоРодитель, ВыведенныеОбъекты, СлужебныеОбъекты = Неопределено,
	...
	Для Каждого КлючИЗначение Из КэшПоТипамОбъектов Цикл
		...
		ТекстПоТипуОбъекта = СтрЗаменить(ТекстПоТипуОбъекта, "&Проведен",
			КэшРеквизитовОбъекта[КлючИЗначение.Ключ]["Проведен"]);
		ТекстПоТипуОбъекта = СтрЗаменить(ТекстПоТипуОбъекта, "&СуммаДокумента",
			КэшРеквизитовОбъекта[КлючИЗначение.Ключ]["СуммаДокумента"]);
		ТекстПоТипуОбъекта = СтрЗаменить(ТекстПоТипуОбъекта, "&Валюта",
			КэшРеквизитовОбъекта[КлючИЗначение.Ключ]["Валюта"]);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НоваяСтрока <> Неопределено И Не ДобавляемыйОбъектИмеетсяСредиРодителей(ДеревоРодитель, Выборка.Ссылка) Тогда
...
Функция ВыводитьТекущийОбъект(Родитель, ТекущийОбъект, ВыведенныеОбъекты, ЭтоПодчиненный, Отказ = Ложь)
	...
	Если СвойстваОбъекта.ЭтоОсновной Или ТекущийОбъектВыводилсяВВетке(Родитель, ТекущийОбъект) Тогда
...
Процедура ДополнитьТекстЗапросаПоРеквизитамОбъекта(ТекстЗапроса, МассивРеквизитов)
	...
	Для Инд = 1 По 3 Цикл
		...
		ТекстПредставление = ТекстПредставление + ",
												  |	" + ?(МассивРеквизитов.Количество() >= Инд, МассивРеквизитов[инд
			- 1], "NULL") + " Как ДополнительныйРеквизит" + Инд;
...
Функция ИмяРеквизитаДокумента(Знач МетаданныеОбъекта, Знач ИмяРеквизита)
	...
	ИмяРеквизитаДокумента = СтруктураПодчиненностиПереопределяемый.ИмяРеквизитаДокумента(МетаданныеОбъекта.Имя,
		ИмяРеквизита); // АПК:223
...
Функция РеквизитыДляПредставления(Знач ПолноеИмяОбъектаМетаданных, Знач ИмяОбъектаМетаданных)
	...
	// Для обратной совместимости.
	Возврат СтруктураПодчиненностиПереопределяемый.МассивРеквизитовОбъектаДляФормированияПредставления(
		ИмяОбъектаМетаданных); // АПК:223
...
Функция ПредставлениеОбъектаДляВывода(Данные)
	...
	СтруктураПодчиненностиПереопределяемый.ПриПолученииПредставления(ТипЗнч(Данные.Ссылка), Данные, Результат,
...
Процедура ВыполнитьСценарийИзмененияПометкиУдаления(Ответ, Сценарий) Экспорт
	...
	Иначе
		ПоказатьОповещениеПользователя(
			Сценарий.Оповещение, Сценарий.Ссылка, Сценарий.Пояснение, БиблиотекаКартинок.Информация32);
	КонецЕсли;
	...
	Для Каждого Элемент Из ВыбранныеЭлементы Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Связанные документы.Изменение пометки удаления'",
				ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , Элемент,
...
Функция ОбработанныеДокументы(ВыбранныеДокументы, Режим, Ошибки)
	...
	Пока Индекс >= 0 Цикл
		...
		Если Не ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или МетаданныеОбъекта.Проведение <> ПроведениеРазрешено Тогда
	...
	Пока Индекс >= 0 Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Связанные документы.Проведение документа'",
				ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , Документ,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Ошибки.Добавить(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПредупредитьОбОшибкеПриИзмененииЭлементов(Ошибки, Сценарий)
	...
	Иначе
		ШаблонПредупреждения = ?(Сценарий = "ПометкаУдаления", НСтр("ru = 'Не удалось изменить пометку удаления документов:
																	|%1'"), НСтр("ru = 'Не удалось провести документы:
																				 |%1'"));
