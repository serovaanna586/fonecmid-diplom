...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДоступнаОтправкаПисем Тогда
		Если НастройкиОтчета.РазрешеноИзменятьВарианты И ОбщегоНазначения.ПодсистемаСуществует(
			"СтандартныеПодсистемы.РассылкаОтчетов") И ВариантыОтчетов.ОтчетПодключенКХранилищу(РеквизитФормыВЗначение(
			"Отчет").Метаданные()) И Не НастройкиОтчета.СкрытьКомандыРассылки Тогда
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	// Ничего не делать если отчет не на СКД и никаких настроек не загружено.
	Если Не ВариантыОтчетовСлужебныйКлиентСервер.РежимВариантаОтчета(КлючТекущегоВарианта) И НовыеНастройкиКД
		= Неопределено Тогда
	...
	Если РежимРасшифровки Тогда
		...
		Если Параметры <> Неопределено И Параметры.Свойство("Расшифровка") И ТипЗнч(Параметры.Расшифровка) = Тип(
			"ОписаниеОбработкиРасшифровкиКомпоновкиДанных") Тогда
...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	Если Не ВариантМодифицирован Тогда
		ВосстановитьСостояниеОпцииВыводитьЗаголовкиНастроек(КлючХраненияЗначения, ВыводитьЗаголовкиНастроек,
			РежимРасшифровки);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		Если ТипЗнч(ЭлементНастройки) <> Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Или ТипЗнч(
			ЭлементНастройки.Значение) <> Тип("СтандартныйПериод") Или Не ЭлементНастройки.Использование Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПриСохраненииВариантаНаСервере(НастройкиКД)
	...
	Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
		КлючХраненияЗначения = КлючХраненияЗначенияВРамкахВариантаОтчета(НастройкиОтчета.ПолноеИмя,
			КлючТекущегоВарианта);
		СохранитьСостояниеОпцииВыводитьЗаголовкиНастроек(КлючХраненияЗначения, ВыводитьЗаголовкиНастроек,
			РежимРасшифровки);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПрименитьНастройкиИПереформироватьОтчет(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат.Свойство("ИмяСобытия") И Результат.ИмяСобытия
		= ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияФормыНастроек() И Результат.ВариантМодифицирован Тогда
...
#Иначе
		ПоказатьПредупреждение( , (НСтр(
			"ru = 'Для того чтобы редактировать схему компоновки, запустите приложение в режиме толстого клиента.'")));
#КонецЕсли
КонецПроцедуры
...
Процедура ОбновитьВариантОтчетаИзФайла(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ОбновитьВариантОтчетаИзФайла", ВариантыОтчетовКлиент, СвойстваВариантаОтчета);
...
Процедура Сформировать()
	...
	Результат = РезультатФормированияОтчета(ФормированиеПриОткрытии, НастройкиОтчета.Внешний
		Или НастройкиОтчета.Безопасный, Ложь);
...
Процедура НачатьЗамер()
	...
	Если ВыполнятьЗамеры Тогда
		Комментарий = НастройкиОтчета.ПрефиксЗамеров + "; " + НСтр("ru = 'Непосредственно:'") + " " + Строка(
			Непосредственно);
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно, ПослеПредупреждения)
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"ВариантыОтчетов.СформироватьОтчетВФоне", ПараметрыФормирования, ПараметрыВыполнения);
...
Функция ПараметрыФормированияОтчета(ИмяОтчета, Непосредственно)
	...
	ПараметрыФормированияОтчета.Вставить("ПользовательскиеНастройкиКД",
...
Процедура ПоказатьСписокВыбора(Элемент, СтандартнаяОбработка)
	...
	ЗначенияДляВыбора = ЗначенияДляВыбора(
		Элемент.СписокВыбора, Сведения.ЭлементПользовательскойНастройки, Элемент.ДоступныеТипы, ДоступныеЗначения);
...
Функция ПараметрыУниверсальногоПоискаПрименимыКСсылкам(ПараметрыПоиска)
	...
	Возврат СтрДлина(ПараметрыПоиска.СтрокаПоиска) > 2 Или ОбщегоНазначенияКлиентСервер.ЭтоЧисло(
...
#Область ОбработкаРезультатаЗапроса
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			...
			Если Не ЗначениеЗаполнено(Выборка.ПолеПоУсловию) Или ИндексДополнительныхЗначений[ПолеПоиска.Ключ]
				<> Неопределено Тогда
...
Функция ПолеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"КОГДА ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 ТОГДА ""%3""", Поле.Имя, """%"
			+ ПараметрыПоиска.СтрокаПоискаНормализованная + "%""", Поле.Имя);
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если ТипРеквизита.СодержитТип(Тип("Строка")) Тогда
			...
			Условие = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2", Поле.Имя, """%"
...
Функция ФрагментУсловия(Условие, ОтступДоУсловия = " ", ОтступПослеУсловия = " ")
	...
	ШаблонФрагментаУсловия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1<span style=""color: %2"">%3</span>%4'"), ОтступДоУсловия, "СкрытыйВариантОтчетаЦвет",
		УсловиеНормализованное, ОтступПослеУсловия);
...
Функция УсловиеУниверсальногоПоиска(ЗначениеПоиска, УправляющиеСимволы)
	...
	Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") И (Не ЗначениеЗаполнено(УправляющиеСимволы) Или УправляющиеСимволы
		= "=") Тогда
...
Функция ОписаниеПараметровОтчета(КомпоновщикНастроек)
	...
	Для Каждого ПолеПараметровДанных Из ПоляПараметровДанных Цикл
		...
		Для Каждого Элемент Из ПолеПараметровДанных.Элементы Цикл
			...
			Если ОписаниеПараметра = Неопределено Или Не ОписаниеПараметра.Видимость Тогда
...
Процедура УточнитьСвойстваФильтра(Фильтр, Поиск)
	...
	Если Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно И Поиск.ВидСравнения
		= ВидСравненияКомпоновкиДанных.Равно И Значения.Количество() > 1 Тогда

		Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
		Фильтр.ПравоеЗначение = Значения;

	ИначеЕсли Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке И Поиск.ВидСравнения
		= ВидСравненияКомпоновкиДанных.Равно Тогда

		Фильтр.ПравоеЗначение = Значения;

	ИначеЕсли Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно И Поиск.ВидСравнения
		= ВидСравненияКомпоновкиДанных.НеРавно И Значения.Количество() > 1 Тогда

		Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке;
		Фильтр.ПравоеЗначение = Значения;

	ИначеЕсли Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке И Поиск.ВидСравнения
...
Функция ДоступныеФорматыСохраненияОтчета(ИндексФорматовСохраненияОтчета)
	...
	Для Каждого ФорматСохранения Из ФорматыСохранения Цикл
		...
		ПредставлениеФорматаСохранения = СтрЗаменить(
			ФорматСохранения.Представление, "." + ФорматСохранения.Расширение, РасширениеФорматаСохранения);
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	...
	Если Не ПриСохраненииВарианта Тогда
		ПоказыватьКомандыИзмененияВарианта = ПоказыватьКомандыВыбораВариантов
			И НастройкиОтчета.РазрешеноИзменятьВарианты;
		...
		ДоступноСохранятьТекущийВариант = ЗначениеЗаполнено(НастройкиОтчета.ВариантСсылка)
			И Не ЭтоПредопределенныйВариант;
...
Процедура ЗагрузитьВариант(КлючВарианта, ОчиститьСтекНастроек = Истина)
	Если Не РежимРасшифровки И Не ВариантМодифицирован Тогда
		КлючОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1/%2/%3'"), НастройкиОтчета.ПолноеИмя, КлючТекущегоВарианта,
	...
	УстановитьТекущийВариант(КлючВарианта);
	ОтчетыКлиентСервер.ОтобразитьСостояниеОтчета(
		ЭтотОбъект, НСтр("ru = 'Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.'"),
...
Процедура УстановитьКлючТекущегоВарианта(ОтчетПолноеИмя, ОтчетОбъект)
	...
	Если ЗначениеЗаполнено(КонтекстВарианта) И ЗначениеЗаполнено(Параметры.КлючВарианта) И Параметры.КлючВарианта
		<> КлючТекущегоВарианта Тогда 
...
Функция СобытияНастроек()
	...
	НайтиСобытияНастроек(
		Элементы.ОтчетТабличныйДокумент.КонтекстноеМеню, УточненияСобытийНастроек(), СобытияНастроекИсключения(),
		СобытияНастроек);

	СобытияНастроек.Вставить("ФильтроватьИСформировать", НСтр("ru = 'Фильтровать...'"));

	СобытияНастроек.Вставить(
		ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияФормыНастроек(), НСтр("ru = 'Расширенная настройка'"));

	СобытияНастроек.Вставить(
		ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияИзмененияСоставаБыстрыхНастроек(), НСтр(
		"ru = 'Изменение состава быстрых настроек'"));
...
Процедура НайтиСобытияНастроек(Меню, Уточнения, Исключения, СобытияНастроек)
	...
	Для Каждого Элемент Из Меню.ПодчиненныеЭлементы Цикл
		...
		ИначеЕсли СобытияНастроек[Элемент.ИмяКоманды] = Неопределено И Исключения.Найти(Элемент.ИмяКоманды)
			= Неопределено Тогда
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	ОтчетыСервер.ЗаполнитьДополнительныеСвойства(ОтчетОбъект, ДоступныеНастройки.Настройки, КлючТекущегоВарианта,
		НастройкиОтчета.КлючПредопределенногоВарианта, ?(ФормаПараметры.НачальныйКлючВарианта = КлючТекущегоВарианта,
		КонтекстВарианта, ""), ?(ФормаПараметры.НачальныйКлючВарианта = КлючТекущегоВарианта, ФормаПараметры.Отбор,
		Неопределено));
...
Процедура ПоказатьОшибкиФормирования(ИнформацияОбОшибке)
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Ошибка при формировании:'") + Символы.ПС
...
Процедура СохранитьТекущийВариантОтчета()
	...
	ХранилищаНастроек.ХранилищеВариантовОтчетов.Сохранить(
		КлючОтчета, КлючТекущегоВарианта, Отчет.КомпоновщикНастроек.Настройки, ОписаниеНастроек);
...
Процедура ПрименитьНастройкиИзКонтекстногоМеню(Результат)
	...
	Если Не ВариантыОтчетовСлужебныйКлиент.ЭтоСобытиеКонтекстнойНастройки(Действие) Или ИдентификаторВладельца
...
Процедура УточнитьПризнакАвтоформированияОтчета(ПараметрыОбновления)
	...
	Если Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОбновления, "Переформировать", Ложь) Тогда
		...
		ПараметрыОбновления.Вставить("Переформировать", ВремяФормированияОтчета
			<= ДопустимоеВремяДляАвтоформированияОтчета);
	КонецЕсли;
