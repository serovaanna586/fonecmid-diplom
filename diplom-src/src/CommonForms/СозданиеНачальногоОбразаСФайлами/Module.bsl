...
Процедура ПутьКАрхивуСФайламиТомовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикСохраненияФайла(
		Элемент, "ПутьКАрхивуСФайламиТомовWindows", СтандартнаяОбработка, "files.zip", "Архивы zip(*.zip)|*.zip");
...
Процедура ПолноеИмяФайловойБазыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикСохраненияФайла(
		Элемент, "ПолноеИмяФайловойБазыWindows", СтандартнаяОбработка, "1Cv8.1CD", "Любой файл(*.*)|*.*");
...
Процедура СоздатьНачальныйОбраз(Команда)
	...
	Если ВидБазы = 0 И Не МожноСоздатьФайловуюБазу Тогда
		...
		ВызватьИсключение НСтр("ru = 'Создание начального образа файловой информационной базы
	...
	Иначе
		...
		Если ВидБазы = 0 Тогда
			...
			ПараметрыЗадания.Вставить("НаименованиеПроцедуры",
				"РаботаСФайламиСлужебный.СоздатьФайловыйНачальныйОбразНаСервере");
		Иначе
			// Серверный начальный образ.
			СтрокаСоединения = "Srvr=""" + Сервер + """;" + "Ref=""" + ИмяБазы + """;" + "DBMS=""" + ТипСУБД + """;"
				+ "DBSrvr=""" + СерверБазыДанных + """;" + "DB=""" + ИмяБазыДанных + """;" + "DBUID="""
				+ ПользовательБазыДанных + """;" + "DBPwd=""" + ПарольПользователя + """;" + "SQLYOffs=""" + Формат(
				СмещениеДат, "ЧГ=") + """;" + "Locale=""" + Язык + """;" + "SchJobDn=""" + ?(
				УстановитьБлокировкуРегламентныхЗаданий, "Y", "N") + """;";
...
Функция ПроверкаПолногоИмениФайловойБазыНаСервере(ПолноеИмяФайловойБазы, СообщениеОбОшибке = "")
	...
	Если Не ПустаяСтрока(СтруктураПути.Путь) Тогда
		...
		Если ПустаяСтрока(СтруктураПути.Расширение) Тогда
			...
			ПолноеИмяФайловойБазы = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(СтруктураПути.ПолноеИмя,
				"1Cv8.1CD");
		КонецЕсли;
		...
		Если НайтиФайлы(ПутьКФайлу).Количество() = 0 Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Папка ""%1"" не существует или не доступна.
					 |Проверьте правильность указания пути.'"), ПутьКФайлу);
			Возврат Ложь;
		КонецЕсли;
	Иначе
...
&НаКлиенте
Процедура ОбработчикСохраненияФайла(Элемент, ИмяСвойства, СтандартнаяОбработка, ИмяФайла, Фильтр = "")
	...
	Диалог.ПолноеИмяФайла = ?(ЭтотОбъект[Контекст.ИмяСвойства] = "", Контекст.ИмяФайла,
		ЭтотОбъект[Контекст.ИмяСвойства]);

	ОписаниеОповещенияДиалогаВыбора = Новый ОписаниеОповещения("ОбработчикСохраненияФайлаПослеВыбораВДиалоге",
		ЭтотОбъект, Контекст);
...
Процедура ЗапуститьСозданиеНачальногоОбраза()
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		ПараметрыОжидания.ОповещениеОПрогрессеВыполнения = Новый ОписаниеОповещения("СоздатьНачальныйОбразНаСервереПрогресс",
			ЭтотОбъект);
		;
...
Функция СоздатьНачальныйОбразНаСервере(Знач Действие)
	...
	Если ЭтоАдресВременногоХранилища(АдресПараметровЗадания) Тогда
		...
		Если ТипЗнч(ПараметрыЗадания) = Тип("Структура") Тогда
			...
			Возврат ДлительныеОперации.ВыполнитьВФоне(ПараметрыЗадания.НаименованиеПроцедуры, ПараметрыЗадания,
				ПараметрыВыполнения);
		КонецЕсли;
	КонецЕсли;
