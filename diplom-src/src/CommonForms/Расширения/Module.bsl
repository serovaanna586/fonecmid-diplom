...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СписокРасширенийОбщее.Видимость = ОбщегоНазначения.РазделениеВключено() И ПравоДоступа(
		"Администрирование", Метаданные);
	...
	Элементы.ФормаУстановленныеПатчи.Видимость = ОбщегоНазначения.ПодсистемаСуществует(
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ВыполненВыходИзОбластиДанных" Или ИмяСобытия = "ВыполненВходВОбластьДанных" Тогда
...
&НаКлиенте
Процедура ПредупреждениеОписаниеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
...
Процедура СписокРасширенийПодключатьПриИзменении(Элемент)
	...
	Если Не ТекущееРасширение.Подключать И ЭтоРасширениеСДанными(ТекущееРасширение.ИдентификаторРасширения) Тогда
...
Процедура СохранитьКак(Команда)
	...
	ПараметрыСохранения.Диалог.Фильтр    = НСтр(
		"ru = 'Файлы расширений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*'");
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		Если НеВыводитьОбщиеРасширения И (Расширение.ОбластьДействия
			= ОбластьДействияРасширенияКонфигурации.ИнформационнаяБаза) Тогда
			Продолжить;
		КонецЕсли;
		...
		ЭлементРасширения.Общее = (Расширение.ОбластьДействия
			= ОбластьДействияРасширенияКонфигурации.ИнформационнаяБаза);

		ЭлементРасширения.ПриоритетНазначения = ?(Расширение.Назначение
			= Метаданные.СвойстваОбъектов.НазначениеРасширенияКонфигурации.Исправление, 1, ?(Расширение.Назначение
			= Метаданные.СвойстваОбъектов.НазначениеРасширенияКонфигурации.Адаптация, 2, 3));
	...
	ИзменилосьСостояниеРасширений = (ПараметрыСеанса.УстановленныеРасширения.ОсновныеСостояние
...
Процедура ЗапуститьОбновлениеПараметровРаботыРасширенийВФонеНаСервере(ТекстПредупреждения)
	...
	ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '1. Включено и запущено регламентное задание
			 ...
			 |""%3"".'"), РегистрыСведений.ПараметрыРаботыВерсийРасширений.НаименованиеЗаданияЗаполненияПараметров(),
		РегистрыСведений.ПараметрыРаботыВерсийРасширений.ИмяСобытияЗаполненияПараметров(), Команды.Найти(
		"ЗапуститьОбновлениеПараметровРаботыРасширенийВФоне").Заголовок);
...
Процедура УдалитьРасширения(ВыделенныеСтроки)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если СтрокаРасширения.Общее И ЭтоНеразделенныйПользовательВОбласти
...
Процедура УдалитьРасширениеПослеПодтверждения(Результат, Контекст) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ПрофилиБезопасности") Тогда
		...
		МодульРаботаВБезопасномРежимеКлиент.ПрименитьЗапросыНаИспользованиеВнешнихРесурсов(Запросы, ЭтотОбъект,
			Оповещение);
	Иначе
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
		...
		ИначеЕсли ВыбранныеФайлыСодержатИсправления(ПомещенныеФайлы, МодульОбновлениеКонфигурацииКлиент) Тогда
			ПоказатьПредупреждение( , НСтр(
				"ru = 'Выбранные файлы не должны одновременно содержать исправления (патчи) и другие виды расширений.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьРасширениеЗавершение()
	...
	КонецЕсли;
	Попытка
		ИзменитьРасширенияНаСервере(Контекст.ПомещенныеФайлы, Контекст.ВыделенныеСтроки, НеподключенныеРасширения,
	...
	Если Не Контекст.Свойство("ЗаменаИмениПодтверждена") И ЗначениеЗаполнено(ПодтверждениеЗаменыИмени.СтароеИмя) Тогда
		...
		ОбработчикЗавершения = Новый ОписаниеОповещения("ЗагрузитьРасширениеПослеВопросаЗаменыИмени", ЭтотОбъект,
			Контекст);
		...
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(ОбработчикЗавершения, ТекстВопроса, Кнопки,
			ПараметрыВопроса);
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ИзменитьРасширенияНаСервере(ПомещенныеФайлы, ИдентификаторыСтрок, НеподключенныеРасширения,
	РасширенияИзменены, ПодтверждениеЗаменыИмени)
				...
				Если Расширения.Количество() > 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Нельзя добавить расширение ""%1""
							 |из файла ""%2"",
							 |так как расширение с таким именем уже существует.'"), ОписаниеРасширения.Имя,
						ПомещенныйФайл.Имя);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		ИначеЕсли ЗначениеЗаполнено(ИмяФайлаДобавляемогоРасширения) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось добавить расширение ""%1""
					 ...
					 |%3'"), ОписаниеРасширения.Имя, ИмяФайлаДобавляемогоРасширения, КраткоеПредставлениеОшибки(
				ИнформацияОбОшибке));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		Попытка
			РегистрыСведений.ПараметрыРаботыВерсийРасширений.ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения,
				НеподключенныеРасширения);
			...
			Если ИдентификаторыСтрок <> Неопределено Тогда
				Если ПроверяемыеРасширения.Количество() > 0 Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
						"ru = 'Измененное расширение было восстановлено.'");
				Иначе
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ru = 'Расширение не было изменено.'");
				КонецЕсли;
			Иначе
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
					"ru = 'Добавленные расширения были удалены.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокРасширенийБезопасныйРежимФлагПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	Если Расширение = Неопределено Или Расширение.БезопасныйРежим = СтрокаСписка.БезопасныйРежимФлаг Тогда
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Если ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
				"ru = 'Отменено изменение флажка безопасного режима расширения.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокРасширенийПодключатьПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	Если Расширение = Неопределено Или Расширение.Активно = СтрокаСписка.Подключать Тогда
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Если ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
				"ru = 'Отменено изменение флажка подключения расширения.'");
		КонецЕсли;
	КонецЕсли;
		...
		Попытка
			...
			Иначе
				...
				Если ВременноеРасширение = Неопределено Тогда
					ВызватьИсключение НСтр("ru = 'Текущее расширение не существует в программе,
				...
				Если ПомещенныеФайлы.Количество() > 1 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'При попытке временно добавить расширение из файла
								 |""%1""
								 |возникла ошибка:
								 |%2'"), ПомещенныйФайл.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура УстановитьДоступностьКнопокКомандныхПанелей()
	...
	ИначеЕсли Элементы.СтраницыОбновление.ТекущаяСтраница = Элементы.СтраницаСписокРасширений Тогда
		...
		Если ВыбранаОднаСтрока Тогда
			...
			РедактированиеДоступно = (Не ТекущееРасширение.Общее Или Не ЭтоНеразделенныйПользовательВОбласти())
