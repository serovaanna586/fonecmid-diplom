...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОписаниеМобильногоПриложения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ГлобальныеНастройки,
...
Процедура ОбработкаОповещения(ИмяСобытия, Изменения, Источник)
	...
	ИначеЕсли ИмяСобытия = ВариантыОтчетовКлиент.ИмяСобытияИзменениеОбщихНастроек() Тогда
		Если Изменения.ПоказыватьПодсказки <> ПоказыватьПодсказки Или Изменения.ИскатьВоВсехРазделах
			<> ИскатьВоВсехРазделах Тогда
			ПараметрыКлиента.Обновлять = Истина;
		КонецЕсли;
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий + "; " + НСтр("ru = 'Поиск:'") + " " + Строка(СтрокаПоиска) + "; " + НСтр(
			"ru = 'Во всех разделах:'") + " " + Строка(ИскатьВоВсехРазделах);
	Иначе
...
Функция ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	Если Событие = "" Или Событие = "СтрокаПоискаПриИзменении" Или Событие = "СброситьНастройки" Тогда
		Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Событие = "ПоказыватьПодсказкиПриИзменении" Или Событие = "ИскатьВоВсехРазделахПриИзменении" Тогда
	...
	Элементы.БыстрыйДоступШапкаНадпись.ОтображениеПодсказки = ?(РежимНастройки, ОтображениеПодсказки.Кнопка,
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.БыстрыйДоступПодсказкаКогдаНеНастроен.Заголовок = СтрЗаменить(
		Элементы.БыстрыйДоступПодсказкаКогдаНеНастроен.Заголовок, ПодстрокаПоиска, ПодстрокаЗамены);

	Элементы.БыстрыйДоступШапкаНадпись.РасширеннаяПодсказка.Заголовок = СтрЗаменить(
		Элементы.БыстрыйДоступШапкаНадпись.РасширеннаяПодсказка.Заголовок, ПодстрокаПоиска, ПодстрокаЗамены);
КонецПроцедуры
...
Процедура ОпределитьПодсистемыИЗаголовок(Параметры)
	...
	Если ТекущийРазделСсылка = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для панели отчетов указан несуществующий раздел ""%1"" (см. %2).'"), Параметры.ПутьКПодсистеме,
...
Процедура ЗагрузитьВсеНастройки()
	...
	ЛокальныеНастройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы(), КлючНазначенияИспользования);
...
Процедура СохранитьНастройкиЭтойПанелиОтчетов()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы(), КлючНазначенияИспользования, ЛокальныеНастройки);
КонецПроцедуры
...
Функция ЗаполнитьПанельОтчетовВФоне()
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне("ВариантыОтчетов.НайтиВариантыОтчетовДляВывода",
...
Процедура ЗаполнитьПанельОтчетов(ПараметрыЗаполненияВременноеХранилище)
	...
	Иначе
		...
		Если ПараметрыЗаполнения.ДругиеРазделы.Количество() = 0 Тогда
			Надпись = Элементы.Вставить("ВДругихРазделах", Тип("ДекорацияФормы"),
				Элементы.РезультатыПоискаИзДругихРазделов);
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			...
			ЗаголовокНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокНадписи,
				ПараметрыЗаполнения.ПределВывода);
			Надпись = Элементы.Вставить("ПределВыводаПревышен", Тип("ДекорацияФормы"),
				Элементы.РезультатыПоискаИзДругихРазделов);
...
Процедура ИнициализироватьПараметрыЗаполнения(ПараметрыЗаполнения)
	...
	ШаблонГруппыВарианта = Новый Структура("Вид, РастягиватьПоГоризонтали,
	...
	ШаблонКартинкиБыстрогоДоступа = Новый Структура("Вид, Ширина, Высота, Картинка,
	...
	ШаблонКартинкиОтступа = Новый Структура("Вид, Ширина, Высота,
											|РастягиватьПоГоризонтали, РастягиватьПоВертикали");
	...
	ШаблонНадписиВарианта = Новый Структура("Вид, Гиперссылка, ЦветТекста,
	...
	Если РежимНастройки Тогда
		ПараметрыЗаполнения.Вставить("КонтекстноеМеню",
			Новый Структура("УбратьИзБыстрогоДоступа, ПереместитьВБыстрыйДоступ, Изменить"));
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Если Не ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела И ПараметрыЗаполнения.ОсталосьВывести = 0 Тогда
	...
	Элементы[ПараметрыЗаполнения.Префикс + "БыстрыйДоступПодвал"].Видимость = (РежимНастройки Или (ЕстьБыстрыйДоступ И (ЕстьВариантыБезГруппы
		Или ЕстьВариантыСГруппой Или ЕстьВариантыСмТакже)));
...
Процедура ОпределитьГруппыИДекорацииДляВыводаВариантов(ПараметрыЗаполнения)
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		ПараметрыПодсветки = ПараметрыЗаполнения.РезультатПоиска.ПодсветкаПодсистем.Получить(
			ПараметрыЗаполнения.РазделСсылка);
		Если ПараметрыПодсветки = Неопределено Тогда
			ПодсветкаПредставления = Новый Структура("Значение, КоличествоНайденныхСлов, ПодсветкаСлов",
				ИнформацияОРазделе.Представление, 0, Новый СписокЗначений);
...
Процедура ВывестиВариантыСГруппой(ПараметрыЗаполнения)
	...
	Для Каждого ВариантРодитель Из НайденныеРодители Цикл
		НайденныеПодчиненные = Варианты.НайтиСтроки(Новый Структура("Родитель, Подсистема", ВариантРодитель.Ссылка,
			ВариантРодитель.Подсистема));
		...
		Если СтрокаРодителя <> Неопределено Тогда
			Пока СтрокаРодителя <> Неопределено Цикл
				Если ЕстьВарианты Тогда
					СтрокаРодителя.ВсегоВложенныхВариантов = СтрокаРодителя.ВсегоВложенныхВариантов
						+ СтрокаДерева.КоличествоВариантов;
	...
	КоличествоСтрок = КорневаяСтрока.КоличествоВариантов + КорневаяСтрока.ВсегоВложенныхВариантов
		+ КорневаяСтрока.ВсегоВложенныхПодсистем + Макс(КорневаяСтрока.ВсегоВложенныхПустыхСтрок - 2, 0);
	...
	Рекурсия.Вставить("ЭтоПоследняяКолонка", Рекурсия.НомерТекущейКолонки = Рекурсия.КоличествоКолонок
		Или КоличествоСтрок <= 6);
	...
	Для Каждого СтрокаПорядкаВывода Из ПорядокВывода Цикл
		...
		ИначеЕсли СтрокаПорядкаВывода.ЭтоВариант Тогда
			...
			ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, СтрокаПорядкаВывода.Вариант, ТекущаяГруппа,
				СтрокаПорядкаВывода.УровеньВложенности);
...
Функция ДеревоРаспределения()
	...
	ОписаниеИдентификатора = Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений");
...
Процедура ЗаполнитьПорядокВывода(ПорядокВывода, СтрокаРодителя, СтрокаДерева, Рекурсия, ПараметрыЗаполнения)
	...
	Если Не Рекурсия.ЭтоПоследняяКолонка И Рекурсия.СвободноСтрок <= 0 Тогда // текущая колонка исчерпана
		...
		Пока ТекущийРодитель <> Неопределено И ТекущийРодитель.ПодсистемаСсылка <> ПараметрыЗаполнения.РазделСсылка Цикл
			...
			ЗаполнитьЗначенияСвойств(ВывестиПодсистему, ТекущийРодитель,
				"Подсистема, ПодсистемаСсылка, УровеньВложенности");
	...
	Если (СтрокаДерева.КоличествоВариантов > 0 Или СтрокаДерева.ВсегоВложенныхВариантов > 0)
		И Рекурсия.НачатВыводВТекущейКолонке И СтрокаРодителя.НачатВывод Тогда
		// Вывод пустой строки.
	...
	Если СтрокаДерева.КоличествоВариантов > 0 Тогда
		...
		Если Рекурсия.ЭтоПоследняяКолонка Или СтрокаРодителя <> Неопределено И (СтрокаДерева.КоличествоВариантов <= 5
		...
		Для Каждого Вариант Из СтрокаДерева.Варианты Цикл
			...
			Если ВозможноПродолжение И Не Рекурсия.ЭтоПоследняяКолонка И Не Вариант.ВыводитсяВместеСОсновным
				...
				Иначе
					КоличествоВТекущуюКолонку = Макс(Мин(Рекурсия.СвободноСтрок, СтрокаДерева.КоличествоВариантов
						- ВариантовВыведено), 3);
				КонецЕсли;
				ВариантовВыведено = 0;
				...
				ТекущийРодитель = СтрокаРодителя;
				Пока ТекущийРодитель <> Неопределено И ТекущийРодитель.ПодсистемаСсылка
					<> ПараметрыЗаполнения.РазделСсылка Цикл
				...
				ВывестиПодсистему.СтрокаДерева        = СтрокаДерева;
				ВывестиПодсистему.ПодсистемаПриоритет = СтрокаДерева.Подсистема.Приоритет;
				ЗаполнитьЗначенияСвойств(ВывестиПодсистему, СтрокаДерева,
					"Подсистема, ПодсистемаСсылка, УровеньВложенности");
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если СтрокаДерева.КоличествоВариантов = 0 И СтрокаДерева.ВсегоВложенныхВариантов = 0 И Не (РежимНастройки
		И ПараметрыЗаполнения.ИмяГруппы = "СГруппой") Тогда
		Возврат ВГруппу;
	КонецЕсли;
	...
	Если СтрокаПорядкаВывода.УровеньВложенности > 1 Тогда
		...
		// Картинка.
		КартинкаОтступа = Элементы.Вставить(ГруппаПодсистемыИмя + "_КартинкаОтступа", Тип("ДекорацияФормы"),
			ГруппаОтступа);
		...
		Если СтрокаПорядкаВывода.СтрокаДерева.КоличествоВариантов = 0
			И СтрокаПорядкаВывода.СтрокаДерева.ВсегоВложенныхВариантов = 0 Тогда
	...
	Если ТребуетсяПодсветка Тогда
		...
		ЗаголовокПодсистемы = Элементы.Вставить(ГруппаПодсистемы.Имя + "_РасширеннаяПодсказка", Тип("ДекорацияФормы"),
			ГруппаПодсистемы);
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Если ЗначениеЗаполнено(Вариант.Подсистема) Тогда
		НадписьИмя = НадписьИмя + "_Подсистема_" + ОтчетыСервер.ПривестиИдентификаторКИмени(
			Вариант.Подсистема.УникальныйИдентификатор());
	КонецЕсли;
	Если Не ПараметрыЗаполнения.ТолькоТекущийРаздел И Элементы.Найти(НадписьИмя) <> Неопределено Тогда
		Если ЗначениеЗаполнено(Вариант.РазделСсылка) Тогда
			Номер = 0;
			Суффикс = "_Раздел_" + ОтчетыСервер.ПривестиИдентификаторКИмени(
				Вариант.РазделСсылка.УникальныйИдентификатор());
		Иначе
			Номер = 1;
			Суффикс = "_1";
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка + Символы.ПС) + НСтр("ru = 'Автор:'") + " " + СокрЛП(Строка(
			Вариант.Автор));
	КонецЕсли;
	...
	КонецЕсли;
	Если Вариант.Важный И ПараметрыЗаполнения.ИмяГруппы <> "СмТакже" И ПараметрыЗаполнения.ИмяГруппы <> "БыстрыйДоступ" Тогда
		Надпись.Шрифт = ШрифтВажнойНадписи;
	КонецЕсли;
...
Функция СформироватьСтрокуСПодсветкой(ОбластьПоиска, Содержимое = Неопределено)
	...
	Для Каждого ЭлементСписка Из ОбластьПоиска.ПодсветкаСлов Цикл
		...
		Если Подсвечивать И КоличествоОткрыто = 1 Тогда
			...
			ФрагментОбычногоТекста = Сред(ИсходныйТекст, ПозицияНачалаОбычногоТекста, ПозицияНачалаПодсветки
				- ПозицияНачалаОбычногоТекста);
		...
		ИначеЕсли Не Подсвечивать И КоличествоОткрыто = 0 Тогда
			...
			ПодсвечиваемыйФрагмент = Сред(ИсходныйТекст, ПозицияНачалаПодсветки, ПозицияНачалаОбычногоТекста
				- ПозицияНачалаПодсветки);
