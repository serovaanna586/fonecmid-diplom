...
Функция НастройкиВосстановленияПароляКорректные()
	...
	ИначеЕсли ВариантОтправкиПоПочте = "РучнаяНастройка" Тогда
		...
		Если ПустаяСтрока(ПарольSMTP) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнен пароль SMTP'"), , "ПарольSMTP");
			НастройкиКорректные = Ложь;
		...
		Если ПустаяСтрока(ИмяОтправителя) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не заполнено имя отправителя'"), , "ИмяОтправителя");
			НастройкиКорректные = Ложь;
...
Процедура ЗапуститьФоновоеЗаполнениеПочтыПользователей()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр(
		"ru = 'Заполнение почты пользователей для восстановления пароля'");
...
Функция ТребуетсяЗапускЗаполненияПочтыПользователей()
	...
	Если Настройки.СпособВосстановленияПароля
		<> ПользователиСлужебный.СпособВосстановленияПароляПользователяИнформационнойБазы("Нет") Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура СохранитьНастройкиВосстановленияПароля()
	...
	Для Каждого Вложение Из Вложения Цикл
		...
		ФорматИзображения = ?(Вложение.Значение.Формат() <> ФорматКартинки.НеизвестныйФормат, НРег(
			Вложение.Значение.Формат()), НРег(ФорматКартинки.PNG));

		ТекстКартинки = "src=""data:image/" + ФорматИзображения + ";base64," + Base64Строка(
			Вложение.Значение.ПолучитьДвоичныеДанные()) + """";
		ТекстСообщенияHTML = СтрЗаменить(ТекстСообщенияHTML, "src=""" + Вложение.Ключ + """", ТекстКартинки);
	...
	Если ВосстанавливатьПароль Тогда
		...
		Если ВариантВосстановленияПароля = "ПереходПоСсылке" Тогда
			Настройки.СпособВосстановленияПароля = ПользователиСлужебный.СпособВосстановленияПароляПользователяИнформационнойБазы(
		...
		ИначеЕсли ВариантОтправкиПоПочте = "ВнешнийПочтовыйСервер" Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
				...
				НастройкиУчетнойЗаписиДляОтправкиПочта = МодульРаботаСПочтовымиСообщениямиСлужебный.НастройкиУчетнойЗаписиДляОтправкиПочта(
					УчетнаяЗаписьПочты);
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(Настройки, ЭтотОбъект,
				"АдресСервераSMTP, ПарольSMTP, ПользовательSMTP, ПортSMTP,ИмяОтправителя");
...
Процедура ПрочитьНастройкиВосстановленияПароля()
	...
	ТемаПисьма = ?(ЗначениеЗаполнено(Настройки.Заголовок), Настройки.Заголовок, НСтр("ru='Восстановление пароля'"));
	...
	Иначе
		...
		Если Настройки.СпособВосстановленияПароля
			...
			"ОтправкаКодаПодтвержденияЧерезСтандартныйСервис") Тогда
			ВариантОтправкиПоПочте = "СтандартныйСервис";
		Иначе
			Если Настройки.СпособВосстановленияПароля
				= ПользователиСлужебный.СпособВосстановленияПароляПользователяИнформационнойБазы(
				"ОтправкаКодаПодтвержденияПоЗаданнымПараметрам") Тогда
				Элементы.ВариантыВосстановления.ТекущаяСтраница = Элементы.ЭлектроннаяПочта;
				ВариантВосстановленияПароля = "ЧерезЭлектроннуюПочту";
...
Процедура УстановитьУчетнуюЗаписьДляВосстановленияПароля(УчетнаяЗаписьПочты, Используется)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		...
		МодульРаботаСПочтовымиСообщениямиСлужебный.СохранитьНастройкиУчетнойЗаписиДляВосстановленияПароля(
