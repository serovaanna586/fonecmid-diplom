...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВыделятьКнопкуПоУмолчанию = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ВыделятьКнопкуПоУмолчанию",
		Истина);
	...
	Иначе
		Если ОжиданиеИмяКнопки <> "" Тогда
			НовыйЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (осталось %2 сек.)'"), ОжиданиеЗаголовокКнопки, Строка(ОжиданиеСчетчик));
