...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КлючСохраненияПоложенияОкна = Строка(ВариантСсылка.УникальныйИдентификатор()) + "\" + Строка(
		ПодсистемаСсылка.УникальныйИдентификатор());
...
Процедура СохранитьНастройкиЭтойФормы()
	...
	ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить(
		ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы(), "ПанельДругихОтчетов", НастройкиФормы);
КонецПроцедуры
...
Процедура ПрочитатьНастройкиЭтойФормы()
	...
	НастройкиФормы = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы(), "ПанельДругихОтчетов", НастройкиПоУмолчанию);
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		ВывестиГиперссылкуВПанель(ТаблицаВывода, СтрокаТаблицы, Элементы.ГруппаДругиеВариантыОтчета,
			ПоказыватьПодсказки);
	КонецЦикла;
