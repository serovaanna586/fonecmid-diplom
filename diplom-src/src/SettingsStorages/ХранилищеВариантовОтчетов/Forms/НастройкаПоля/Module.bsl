...
Процедура ВидСравненияПриИзменении(Элемент)
	...
	Иначе
		...
		ПолеПравогоЗначения.ВыборГруппИЭлементов = ОтчетыКлиентСервер.ЗначениеТипаГруппыИЭлементы(ВыборГруппИЭлементов,
			ВидСравнения);
...
Процедура УточнитьДоступныеТипыЗначений()
	...
	ПолеПравогоЗначения.ВыбиратьТип = Не ЭтоСписок И ДоступныеТипы.Количество() <> 1
...
Функция НачатьЗаполнениеЗначений(ИдентификаторЗадания)
	...
	РезультатЗаполнения = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыМетода, "ВариантыОтчетовСлужебный.ЗаполнитьЗначенияФильтра", ПараметрыЗаполненияЗначений());
...
Процедура ОбработатьОшибкуЗаполненияЗначений(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		...
		УровеньЖурналаРегистрации.Ошибка, , НастройкиОтчета.ВариантСсылка, Комментарий);
...
Процедура ЗаполнитьЗначенияГруппировкиКолонки(ДанныеРасшифровки)
	...
	Для Каждого ЭлементРасшифровки Из ДанныеРасшифровки.Элементы Цикл
		...
		Если Не ЗначениеЗаполнено(Значение) Или Индексы[Значение] <> Неопределено Или Не ДоступныеТипы.СодержитТип(
			ТипЗнч(Значение)) Тогда
...
Процедура ОбновитьИндексыГруппИЭлементов()
	...
	Для Каждого Элемент Из Значения Цикл
		...
		Если Не ОбщегоНазначения.ЭтоСправочник(МетаданныеЗначения) И Не ОбщегоНазначения.ЭтоПланВидовХарактеристик(
			МетаданныеЗначения) Тогда
...
Процедура ОпределитьВидыИерархииЗначений()
	...
	Для Каждого ТипЗначения Из ТипыЗначений Цикл
		...
		Если Не ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) И Не ОбщегоНазначения.ЭтоПланВидовХарактеристик(
		...
		Если ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) И МетаданныеОбъекта.ВидИерархии
			= ВидыИерархии.ИерархияЭлементов Тогда
...
Процедура ПроверитьИспользованиеГруппИЭлементов(ЗначенияФильтра, ИспользуютсяГруппы, ИспользуютсяЭлементы)
	...
	Для Каждого Элемент Из ЗначенияФильтра Цикл
		...
		Если ИндексыГруппИЭлементов.Группы[Элемент.Значение] = Истина
			Или (ИндексыГруппИЭлементов.Элементы[Элемент.Значение] = Истина И ТипГруппировкиПоля
			<> ТипГруппировкиКомпоновкиДанных.Элементы И ВидыИерархииЗначений[ТипЗнч(Элемент.Значение)]
			= ВидИерархииИерархияЭлементов()) Тогда
...
Функция ТипГруппировкиПоля()
	...
	ТекущаяГруппировка = КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(
		СвойстваЗаголовка.ИдентификаторГруппировки);

	Если ТипЗнч(ТекущаяГруппировка) <> Тип("ГруппировкаКомпоновкиДанных") И ТипЗнч(ТекущаяГруппировка) <> Тип(
		"ГруппировкаТаблицыКомпоновкиДанных") Тогда

		Возврат ТипГруппировкиПоля;
	КонецЕсли;

	ПолеГруппировки = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(ТекущаяГруппировка.ПоляГруппировки,
...
Функция Фильтр(КомпоновщикНастроек, СвойстваЗаголовка, Фильтры = Неопределено)
	...
	Если Фильтры = Неопределено Тогда
		Фильтры = ВариантыОтчетовСлужебныйКлиентСервер.ФильтрыРазделаОтчета(КомпоновщикНастроек.Настройки,
			СвойстваЗаголовка);
	КонецЕсли;
...
Процедура ОпределитьДоступностьПоляПравогоЗначения(Поле, ВидСравнения)
	...
	Поле.Доступность = ВидСравнения <> ВидСравненияКомпоновкиДанных.Заполнено И ВидСравнения
...
Процедура ПоказатьСтатистикуЗаполненияЗначений()
	...
	Элементы.ВывестиВсеЗначенияРазделаОтчета.РасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Значения.Количество(), КоличествоПервыхЧитаемыхСтрок, КоличествоСтрокВРазделеОтчета);
