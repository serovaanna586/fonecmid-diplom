...
Процедура СкрытьНеиспользуемыеКоманды()
	...
	Для Каждого Элемент Из Элементы.ДоступныеПоляКонтекстноеМеню.ПодчиненныеЭлементы Цикл // АПК:275  элемент добавляется программно.
		...
		Элемент.Видимость = ТипЗнч(Элемент) = Тип("КнопкаФормы") И ИменаКомандРедактированияФормул.Найти(
			Элемент.ИмяКоманды) <> Неопределено;
...
Процедура УдалитьФормулу(Команда)
	...
	Формула = ВариантыОтчетовСлужебныйКлиентСервер.ФормулаПоПутиКДанным(КомпоновщикНастроек.Настройки,
		Строка.ПутьКДанным);
...
Процедура УстановитьИдентификаторЭлементаСтруктурыНастроек()
	...
	ЭлементСтруктурыНастроек = КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(
	...
	Если ТипЗнч(ЭлементСтруктурыНастроек) = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипЗнч(
		ЭлементСтруктурыНастроек) = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Или ТипЗнч(
		ЭлементСтруктурыНастроек) = Тип("ТаблицаКомпоновкиДанных") Или ТипЗнч(ЭлементСтруктурыНастроек) = Тип(
...
Процедура ИнициализироватьСписокДоступныхПолей()
	...
	ПараметрыДобавленияСпискаПолей.ОбработчикиСписка.Вставить("ПриАктивизацииСтроки",
		"Подключаемый_СписокПолейПриАктивизацииСтроки");
...
Процедура ВыбратьИЗакрыть()
	...
	Если Родитель <> Неопределено И Родитель.Имя = ИдентификаторГруппыФормул() Тогда
...
Процедура ПослеДобавленияФормулы(ОписаниеФормулы, ДополнительныеПараметры) Экспорт
	...
	ВариантыОтчетовСлужебныйКлиент.ДобавитьФормулу(КомпоновщикНастроек.Настройки, КоллекцияПолей(ЭтотОбъект),
		ОписаниеФормулы);
	ОбновитьКоллекцииПолей();
	АктивизироватьГруппуФормул();
