...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.КонтекстВарианта = Метаданные.Справочники.Пользователи.ПолноеИмя() И Форма.Параметры.Свойство(
		"ПараметрКоманды") Тогда
			...
			Если ЗначениеЗаполнено(РеквизитыФормы.КонтекстВарианта) Тогда
				Если Вариант = "ПраваПользователейНаТаблицу" Тогда
					...
					Если ЗначениеЗаполнено(ОбъектМетаданных) Тогда
						ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НовыеНастройкиКД.Отбор,
							"ОбъектМетаданных", ОбъектМетаданных, ВидСравненияКомпоновкиДанных.Равно, , Истина);
					КонецЕсли;
				ИначеЕсли Вариант = "ПраваПользователейНаТаблицы" Или Вариант = "ПраваПользователяНаТаблицы" Тогда
					Если Контекст.Параметры.Свойство("ПараметрКоманды") Тогда
						...
						УстановитьОтбор("Пользователь", СписокПользователей, НовыеНастройкиКД,
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если ПараметрПользователь.Использование И Не ЗначениеЗаполнено(ПараметрПользователь.Значение) Тогда
	...
	Если Не ЗначениеЗаполнено(НастройкиПрав.НастройкиПравЛегенда) Тогда
		ОтключитьГруппировки(НастройкиКомпоновщика, "НастройкиПрав,ЛегендаНастроекПрав,ДополнительныйЗаголовокТаблицы");
	КонецЕсли;
...
Процедура ЗавершитьВывод(ДокументРезультат, ДанныеРасшифровки, НастройкиПрав)
	...
	Для НомерСтроки = 1 По ВысотаТаблицы Цикл
		Для НомерКолонки = 1 По ШиринаТаблицы Цикл
			...
			Если ЗначенияПолей.Найти("Право") <> Неопределено И ЗначенияПолей.Найти("Право").Значение > 0
				И ЗначенияПолей.Найти("ПравоБезОграничения") <> Неопределено И ЗначенияПолей.Найти("Право").Значение
				> ЗначенияПолей.Найти("ПравоБезОграничения").Значение Или ЗначенияПолей.Найти("ПравоПросмотр") <> Неопределено
				И ЗначенияПолей.Найти("ПравоПросмотр").Значение = Истина И ЗначенияПолей.Найти(
				"ПравоЧтениеБезОграничения").Значение = Ложь Или ЗначенияПолей.Найти("ПравоРедактирование") <> Неопределено
				И ЗначенияПолей.Найти("ПравоРедактирование").Значение = Истина И ЗначенияПолей.Найти(
				"ПравоИзменениеБезОграничения").Значение = Ложь Или ЗначенияПолей.Найти("ПравоИнтерактивноеДобавление")
				<> Неопределено И ЗначенияПолей.Найти("ПравоИнтерактивноеДобавление").Значение = Истина
...
Функция ТаблицыОтчетов()
	...
	Если ЗначениеЗаполнено(ВыбранныйОтчет)
		...
		ИспользуемыеТаблицы) И ИспользуемыеТаблицы <> Неопределено Тогда
	...
	Если ЗначениеЗаполнено(ВыбранныйОтчет) И ОписаниеТиповИдентификатора.СодержитТип(ТипЗнч(ВыбранныйОтчет)) Тогда

		ОбъектМетаданныхВыбранныйОтчет = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(ВыбранныйОтчет, Ложь);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Для Каждого ОбъектМетаданныхОтчет Из Метаданные.Отчеты Цикл
			Если ТипЗнч(ОбъектМетаданныхВыбранныйОтчет) = Тип("ОбъектМетаданных") И ОбъектМетаданныхОтчет
				<> ОбъектМетаданныхВыбранныйОтчет Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПраваПользователей()
	...
	ИначеЕсли ВариантСОграничениямиДоступа Тогда
		УниверсальноеОграничение = УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально(Истина,
			Истина);
		Если УниверсальноеОграничение Тогда
			ТекстЗапросаБезГруппировкиПоОтчетамСОграничениямиДоступаВидыОграничений
				= ТекстЗапросаБезГруппировкиПоОтчетамСОграничениямиДоступаВидыОграниченийПоНовому;
			Запрос.УстановитьПараметр("ВидыОграниченийПрав", Отчеты.АнализПравДоступа.ВидыОграниченийПрав( , Истина));
		Иначе
			ТекстЗапросаБезГруппировкиПоОтчетамСОграничениямиДоступаВидыОграничений
				= ТекстЗапросаБезГруппировкиПоОтчетамСОграничениямиДоступаВидыОграниченийПоСтарому;
			Запрос.УстановитьПараметр("ВидыОграниченийПрав", Отчеты.АнализПравДоступа.ВидыОграниченийПрав());
		КонецЕсли;
		ТекстЗапросаОсновной = ТекстЗапросаБезГруппировкиПоОтчетамСОграничениямиДоступаНачало + УсловиеОтбораПоПользователям
		...
		Запрос.УстановитьПараметр("ТекстРазрешенныеПользователи", " (" + НСтр("ru = 'Разрешенные'") + ") - " + НСтр(
			"ru = 'авторизованный пользователь всегда разрешен'"));
		Запрос.УстановитьПараметр("ТекстЗапрещенныеПользователи", " (" + НСтр("ru = 'Запрещенные'") + ") - " + НСтр(
			"ru = 'авторизованный пользователь всегда разрешен'"));
	...
	Если Не УпрощенныйИнтерфейс Тогда
		ТекстЗапросаОсновной = СтрЗаменить(ТекстЗапросаОсновной, "ВЫБРАТЬ РАЗЛИЧНЫЕ", "ВЫБРАТЬ"); // @query-part-1 @query-part-2
	КонецЕсли;
	Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов() + ТекстЗапросаОсновной;
...
Функция НастройкиПравПоТаблицеВОтборе()
	...
	Если Не ЗначениеЗаполнено(Таблица) Или Не ОписаниеТиповИдентификатора().СодержитТип(ТипЗнч(Таблица)) Тогда
		Возврат Неопределено;
	КонецЕсли;

	МетаданныеТаблицы = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(Таблица, Ложь);
	Если МетаданныеТаблицы = Неопределено Или Не ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеТаблицы) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ЗаполнитьСписокПолейГруппировок(КоллекцияЭлементов, СписокПолей)
	...
	Для Каждого Элемент Из КоллекцияЭлементов Цикл
		Если (ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных") Или ТипЗнч(Элемент) = Тип(
			"ГруппировкаТаблицыКомпоновкиДанных")) И Элемент.Использование Тогда
...
Функция ОтборПоПользователям()
	...
	Для Каждого ЭлементСписка Из ЗначениеОтбора Цикл
		Если ТипЗнч(ЭлементСписка.Значение) = Тип("СправочникСсылка.ГруппыПользователей") Или ТипЗнч(
			ЭлементСписка.Значение) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
...
Функция ОтборПоТаблицам()
	...
	Для Каждого Элемент Из Отбор.Элементы Цикл
		Если Элемент.Использование И Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОбъектМетаданных") Тогда
			Если Элемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Или Элемент.ВидСравнения
...
Функция ОтборПоВходВПрограммуРазрешен()
	...
	Для Каждого Элемент Из Отбор.Элементы Цикл
		Если Элемент.Использование И Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВходВПрограммуРазрешен")
			И Элемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно И Элемент.ПравоеЗначение = Истина Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция НастройкиПравПоОбъектам()
	...
	Запрос.УстановитьПараметр("ПредставлениеГруппыПользователей", " (" + НСтр("ru = 'Группа пользователей'") + ")");
	Запрос.УстановитьПараметр("ПредставлениеГруппыВнешнихПользователей", " (" + НСтр(
		"ru = 'Группа внешних пользователей'") + ")");
...
Функция ЗаголовкиПрав(НастройкиПрав, ИмяДляПодпапок)
	...
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("ИмяПрава", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(60,
		ДопустимаяДлина.Переменная)));
	Результат.Колонки.Добавить("ИндексПрава", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(2, 0,
		ДопустимыйЗнак.Неотрицательный)));
	Результат.Колонки.Добавить("ЗаголовокПрава", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(60,
		ДопустимаяДлина.Переменная)));
	Результат.Колонки.Добавить("ПодсказкаПрава", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(150,
		ДопустимаяДлина.Переменная)));
...
Функция ОписаниеКолонкиДляПодпапок()
	...
	Результат.Вставить("Подсказка", НСтр("ru = 'Права не только для текущей папки, но и для ее нижестоящих папок'"));
