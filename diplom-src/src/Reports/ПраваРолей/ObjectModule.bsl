...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД,
	...
	Если ТипЗнч(Контекст) <> Тип("ФормаКлиентскогоПриложения") Или ТипЗнч(НовыеПользовательскиеНастройкиКД) <> Тип(
	...
	Если Вариант = "ПраваРолей" И Не ФормаПараметрыОтбор.Свойство("НачальныйОтбор") Тогда

		Если Не ПараметрИспользуется(НовыеПользовательскиеНастройкиКД, "ОбъектМетаданных") И Не ПараметрИспользуется(
			НовыеПользовательскиеНастройкиКД, "Роль") Тогда
	...
	Если Не ФормаПараметрыОтбор.НачальныйОтбор.Свойство("НачальныйОтборУстановлен") Тогда
		ФормаПараметрыОтбор.НачальныйОтбор.Вставить("НачальныйОтборУстановлен", Не ЗначениеЗаполнено(
			ФормаПараметрыОтбор.НачальныйОтбор) И ФормаПараметрыОтбор.Количество() = 1);
	КонецЕсли;
	...
	Для Каждого ОписаниеПараметра Из ВсеПараметры Цикл
		Если Не ФормаПараметрыОтбор.Свойство(ОписаниеПараметра.Ключ) Тогда
			...
			КонецЕсли;
			ВыключитьПараметр(НовыеПользовательскиеНастройкиКД, ОписаниеПараметра.Ключ, НедоступныеПараметры.Свойство(
				ОписаниеПараметра.Ключ));
		КонецЕсли;
	КонецЦикла;
	...
	Если Вариант = "ПраваРолиНаОбъектыМетаданных" И ФормаПараметрыОтбор.НачальныйОтбор.Свойство("Профиль") Тогда
		...
		УстановитьРежимОтображения("Роль", РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный, НовыеНастройкиКД,
	...
	Если ТипЗнч(ФормаПараметрыОтбор.НачальныйОтбор) = Тип("Структура") Тогда
		Для Каждого КлючИЗначение Из ФормаПараметрыОтбор.НачальныйОтбор Цикл
			Если КлючИЗначение.Ключ = "НачальныйОтборУстановлен" Тогда
				Продолжить;
			КонецЕсли;
			ВключитьПараметр(НовыеПользовательскиеНастройкиКД, КлючИЗначение.Ключ, КлючИЗначение.Значение,
...
Процедура ПослеЗагрузкиНастроекВКомпоновщик(ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Вариант = "ПраваРолиНаОбъектыМетаданных" Тогда
		Если УстановленОтборПоПрофилю(ФормаПараметрыОтбор, КомпоновщикНастроек.Настройки) Или Не ПараметрИспользуется(
			КомпоновщикНастроек.ПользовательскиеНастройки, "Роль") Тогда
...
Процедура ПередФормированиемОтчета(ФормаОтчета, ДополнительныеПараметры) Экспорт
	...
	Если Вариант <> "ПраваРолей" И Вариант <> "ПраваРолиНаОбъектыМетаданных" Тогда
		Возврат;
	КонецЕсли;

	Значения = ЗначенияВыбранногоПараметра("НеПредупреждатьОБольшомРазмереОтчета", Истина);
	Если ТипЗнч(Значения) = Тип("СписокЗначений") И Значения[0].Значение = Истина Тогда
		Возврат;
	КонецЕсли;
	...
	МинутДо = Окр(КоличествоЯчеек / 1000000 * (1.15 + (КоличествоЯчеек / 65000000) * (КоличествоЯчеек / 65000000))
		* Коэффициент, 1) + 0.2;

	ДополнительныеПараметры.ИмяПараметраХраненияОтказаОтПредупреждения = "НеПредупреждатьОБольшомРазмереОтчета";
	ДополнительныеПараметры.ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При текущих отборах отчет может быть очень большим
				 ...
				 |
				 |Рекомендуется усилить отборы.'"), Формат(КоличествоСтрок, ""), Формат(КоличествоКолонок, ""), Формат(
		КоличествоЯчеек, ""), Формат(МинутДо, "ЧДЦ=1"), Формат(ГигабайтДо, "ЧДЦ=1"));
...
Процедура ПриОпределенииПараметровВыбора(Форма, СвойстваНастройки) Экспорт
	...
	Если СвойстваНастройки.ПолеКД = Новый ПолеКомпоновкиДанных("ПараметрыДанных.Роль") Тогда
		...
		ИначеЕсли ТипЗнч(СвойстваНастройки.ПользовательскаяНастройкаКД.Значение) = Тип("Строка") Тогда
			...
			СписокЗначений.Добавить(СвойстваНастройки.ПользовательскаяНастройкаКД.Значение,
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ПраваРолей = ПраваРолей(Вариант = "ПраваРолей" Или Вариант = "ПраваРолиНаОбъектыМетаданных", Вариант
		= "ПраваРолиНаОбъектыМетаданных");
	...
	ВнешниеНаборыДанных.Вставить("ПодробныеПраваОбъекта", ПодробныеПраваОбъекта(
		Вариант = "ПодробныеПраваРолиНаОбъектМетаданных", ПрофилиВместоРолей));
...
Процедура ЗавершитьВывод(ДокументРезультат, ДанныеРасшифровки, Вариант, ПрофилиВместоРолей, ВысотаШапки)
	...
	Если Вариант = "ПраваРолей" Или Вариант = "ПраваРолиНаОбъектыМетаданных" Тогда
	...
	НачальноеЗначениеЕстьПравоСОграничением = Вариант = "ПодробныеПраваРолиНаОбъектМетаданных";
				...
				ИначеЕсли Область.Текст = "***" Тогда
					...
					Если ЗначениеЗаполнено(ВысотаШапки) Тогда
						Если Вариант = "ПраваРолей" Или Вариант = "ПраваРолиНаОбъектыМетаданных" Тогда
							...
							Если ЗначениеЗаполнено(Область.ВысотаСтроки) И ЗначениеЗаполнено(ОбластьНиже.ВысотаСтроки) Тогда
...
Функция ПараметрИспользуется(ПользовательскиеНастройки, ИмяПараметра)
	...
	Для Каждого ЭлементНастройки Из ПользовательскиеНастройки.Элементы Цикл
		...
		Если Свойства.Параметр = ПараметрКомпоновки Тогда
			Возврат ЭлементНастройки.Использование И ЗначениеЗаполнено(ЭлементНастройки.Значение);
		КонецЕсли;
	КонецЦикла;
...
Функция УстановленОтборПоПрофилю(ФормаПараметрыОтбор, Настройки)
	...
	Возврат ФормаПараметрыОтбор.Свойство("НачальныйОтбор") И ФормаПараметрыОтбор.НачальныйОтбор.Свойство("Профиль")
...
Функция ПодробныеПраваОбъекта(Заполнить, ПрофилиВместоРолей)
	...
	Для Каждого ОписаниеПрава Из СписокПрав Цикл
		...
		Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
			ЕстьПраво = ПравоДоступа(ОписаниеПрава.Значение, ОбъектМетаданных, ОписаниеРоли.Метаданные,
				ИмяСтандартногоРеквизита);
			Если Не ЕстьПраво Тогда
				Продолжить;
			КонецЕсли;
			Если ВыбранныеПрава <> Неопределено И ВыбранныеПрава.НайтиПоЗначению(ОписаниеПрава.Значение) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ОписаниеПрава.Пометка Тогда
				ЕстьОграничение = ЕстьОграничение(ОписаниеПрава.Значение, ОбъектМетаданных, ПоляОбъектов,
					ОписаниеРоли.Метаданные);
			КонецЕсли;
			Если ПрофилиВместоРолей Тогда
	...
	Если ПоказыватьОбъектыИРолиБезПрав() И Не ПрофилиВместоРолей Тогда
		...
		Если Права.Количество() = 0 Тогда
			...
			Если СписокПрав.Количество() > 0 Тогда
				...
				Для Каждого ОписаниеПрава Из СписокПрав Цикл
					Если ВыбранныеПрава = Неопределено Или ВыбранныеПрава.НайтиПоЗначению(ПервоеПраво.Значение)
						<> Неопределено Тогда
	...
	Для Каждого ОписаниеПрава Из СписокПрав Цикл
		Если НайденныеПрава.Получить(ОписаниеПрава) <> Неопределено Или ВыбранныеПрава <> Неопределено
...
Функция КонтекстОтчетаПоОбъекту(ЗаполнитьТолькоОписаниеПрав = Ложь)
	...
	Если Не ЗначениеЗаполнено(ВыбранныеОбъекты) Или ВыбранныеОбъекты.Количество() <> 1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.ОписаниеПрав = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Иначе
		...
		Для НомерИмени = 1 По КоличествоИмен Цикл
			...
			ИначеЕсли ЧастиИмени[Индекс] = "СтандартныйРеквизит" Тогда
				ИмяСтандартногоРеквизита = ?(ЗначениеЗаполнено(ИмяСтандартногоРеквизита), ИмяСтандартногоРеквизита
					+ ".", "") + ЧастиИмени[Индекс + 1];
				СтандартныйРеквизит = СтандартныйРеквизит(?(СтандартнаяТабличнаяЧасть = Неопределено, ОбъектМетаданных,
					СтандартнаяТабличнаяЧасть), ЧастиИмени[Индекс + 1]);
			...
			КонецЕсли;
			ПредставлениеОбъекта = ?(ЗначениеЗаполнено(ПредставлениеОбъекта), ПредставлениеОбъекта + ".", "")
			...
			Иначе
				...
				КонецЕсли;
			КонецЕсли;
			ПредставлениеВидаОбъектов = ?(ЗначениеЗаполнено(ПредставлениеВидаОбъектов), ПредставлениеВидаОбъектов
				+ ".", "") + ТекущаяСтрокаДерева.ПредставлениеОбъекта;
		КонецЦикла;
...
Функция ОтборРоли(Заполнить)
	...
	Если Не ЗначениеЗаполнено(ВыбранныеРоли) Или ВыбранныеРоли.Количество() <> 1 Тогда
		Возврат ОтборРоли;
	КонецЕсли;
	...
	Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
		Для Каждого ОписаниеУровня Из УровниДоступа Цикл
			Если ОписаниеУровня.ЭтоНаборПрав Тогда
				...
				КонецЦикла;
			Иначе
				ЕстьПраво = ПравоДоступа(ОписаниеУровня.Право, ОбъектМетаданных, ОписаниеРоли.Метаданные,
					ИмяСтандартногоРеквизита);
			КонецЕсли;
			Если Не ЕстьПраво Тогда
				Продолжить;
			КонецЕсли;
			Если ВыбранныеУровни <> Неопределено И ВыбранныеУровни.НайтиПоЗначению(ОписаниеУровня.Уровень)
				= Неопределено Тогда
				Прервать;
			КонецЕсли;
			ОписаниеРоли.ЕстьПраваРоли = Истина;
			...
			Если ЗначениеЗаполнено(ОписаниеУровня.ПравоСОграничением) Тогда
				ЕстьОграничение = ЕстьОграничение(ОписаниеУровня.ПравоСОграничением, ОбъектМетаданных, ПоляОбъектов,
					ОписаниеРоли.Метаданные);
			КонецЕсли;
...
Функция ПрофилиРолей()
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПрофилей", "РолиПрофилей.Ссылка В(&Профили)");
...
Функция ОписаниеПрофилей()
	...
	ОписаниеПрофилей.Колонки.Добавить("Профиль", Новый ОписаниеТипов("СправочникСсылка.ПрофилиГруппДоступа"));
...
Функция ПоказыватьПраваНеинтерфейсныхПодсистем()
	...
	ПолеОтбора = КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы.Найти(
		"ПоказыватьПраваНеинтерфейсныхПодсистем");
...
Функция ТипЧисло(ЧислоРазрядов)
	...
	Возврат Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(ЧислоРазрядов, 0, ДопустимыйЗнак.Неотрицательный));
...
Функция ОписаниеРолей(Заполнить = Истина, СИдентификаторами = Ложь)
	...
	Для Каждого Роль Из Метаданные.Роли Цикл
		Если ВыбранныеРоли <> Неопределено И ВыбранныеРоли.НайтиПоЗначению(Роль.Имя) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Если СИдентификаторами Тогда
		...
		Пока Индекс >= 0 Цикл
			...
			Если РолиВыбранныхПрофилей <> Неопределено И РолиВыбранныхПрофилей.Получить(РольСсылка) = Неопределено Тогда
...
Процедура ОбработатьСтрокиДерева(Контекст, СтрокиДерева, КодРодителя, СтрокиЕстьПрава)
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если ТипЗнч(СтрокаДерева.Метаданные) = Тип("КоллекцияОбъектовМетаданных") Или ТипЗнч(СтрокаДерева.Метаданные)
			= Тип("ОписанияСтандартныхРеквизитов") Или ТипЗнч(СтрокаДерева.Метаданные) = Тип(
			"ОписанияСтандартныхТабличныхЧастей") Тогда
			...
			Если ТипЗнч(СтрокаДерева.Метаданные) = Тип("КоллекцияОбъектовМетаданных") Тогда
				...
				Если ВыбранныеОбъектыМетаданных <> Неопределено И ВыбранныеОбъектыМетаданных.НайтиПоЗначению(
					СтрокаДерева.Имя) <> Неопределено Тогда
					ВыбранныеОбъектыМетаданных = Неопределено;
				КонецЕсли;
			Иначе
				ВыбранныеОбъектыМетаданных = Неопределено;
			КонецЕсли;
			ЧислоЦифрДляОбъектов = ?(СтрокаДерева.БезГруппы, ЧислоЦифр, СтрДлина(КоличествоЭлементов(
				СтрокаДерева.Метаданные)));
			Для Каждого ОбъектМетаданных Из СтрокаДерева.Метаданные Цикл
				ТолькоИерархия = Ложь;
				Если ВыбранныеОбъектыМетаданных <> Неопределено Тогда
					ПолноеИмя = ОбъектМетаданных.ПолноеИмя();
					Если ВыбранныеОбъектыМетаданных.НайтиПоЗначению(ПолноеИмя) = Неопределено И (СтрокаДерева.Имя <> "Подсистемы"
						И СтрРазделить(ПолноеИмя, ".", Ложь).Количество() < 3 Или СтрокаДерева.Имя = "Подсистемы"
				...
				Если Не ТолькоИерархия Тогда
					Если Не СтрокаДерева.БезРасшифровки Тогда
						...
						ИначеЕсли ТипЗнч(ОбъектМетаданных) = Тип("ОписаниеСтандартногоРеквизита") Тогда
							Если ТипЗнч(СтрокаДерева.МетаданныеРодителяВложения) = Тип(
								"ОписаниеСтандартнойТабличнойЧасти") Тогда
					...
					ИначеЕсли ТипЗнч(СтрокаДерева.МетаданныеРодителяВложения) = Тип(
						"ОписаниеСтандартнойТабличнойЧасти") Тогда
						ДобавитьПрава(Контекст, СтрокаДерева.Родитель.МетаданныеРодителяВложения, СтрокаДерева,
							КодЭлемента, ЕстьПрава, СОграничением, СтрокаДерева.МетаданныеРодителяВложения.Имя + "."
		...
		ИначеЕсли ТипЗнч(СтрокаДерева.Метаданные) = Тип("ОбъектМетаданныхКонфигурация") Тогда
			Если Контекст.ВыбранныеОМД = Неопределено Или Контекст.ВыбранныеОМД.НайтиПоЗначению("Конфигурация")
				<> Неопределено Тогда
...
Функция КоличествоЭлементов(Коллекция)
	...
	Если ТипЗнч(Коллекция) = Тип("КоллекцияОбъектовМетаданных") Или ТипЗнч(Коллекция) = Тип(
		"ОписанияСтандартныхРеквизитов") Тогда
...
Процедура ДобавитьПрава(Контекст, ОбъектМетаданных, СтрокаДерева, КодЭлемента, ЕстьПрава, СОграничением = Ложь,
	ИмяСтандартногоРеквизита = Неопределено)
...
Процедура ДобавитьПраво(Контекст, ОписаниеУровня, ОбъектМетаданных, Роль)
	...
	Если Контекст.ВыбранныеУровни <> Неопределено И Контекст.ВыбранныеУровни.НайтиПоЗначению(ОписаниеУровня.Уровень)
		= Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ЕстьОграничение = Ложь;
	Если ЗначениеЗаполнено(ОписаниеУровня.ПравоСОграничением) Тогда
		ЕстьОграничение = ЕстьОграничение(ОписаниеУровня.ПравоСОграничением, ОбъектМетаданных, Контекст.ПоляОбъектов,
			ОписаниеРоли.Метаданные);
	КонецЕсли;
	Если Контекст.ПрофилиВместоРолей Тогда
...
Процедура ДобавитьКомандыИПоля(СПолями, СтрокаДерева, Поля = "Реквизиты, ТабличныеЧасти, СтандартныеРеквизиты",
	БезРедактирования = Ложь)
	...
	Если СПолями И СтруктураПолей.Свойство("СтандартныеТабличныеЧасти") Тогда
		...
		СтрокаСтандартныеТабличныеЧасти.ПутьКОбъекту         = СтрокаДерева.ПутьКОбъекту
		...
		СтрокаСтандартныеТабличныеЧастиСтандартныеРеквизиты.ПутьКОбъекту         = СтрокаСтандартныеТабличныеЧасти.ПутьКОбъекту
			+ ".СтандартныйРеквизит.*";
...
Функция ПраваКонфигурации()
	...
	СписокПрав.Добавить("ОбновлениеКонфигурацииБазыДанных", НСтр("ru = 'Обновление конфигурации базы данных'"));

	СписокПрав.Добавить("МонопольныйРежим", НСтр("ru = 'Монопольный режим'"));

	СписокПрав.Добавить("АктивныеПользователи", НСтр("ru = 'Активные пользователи'"));

	СписокПрав.Добавить("ЖурналРегистрации", НСтр("ru = 'Журнал регистрации'"));

	СписокПрав.Добавить("ТонкийКлиент", НСтр("ru = 'Тонкий клиент'"));

	СписокПрав.Добавить("ВебКлиент", НСтр("ru = 'Веб-клиент'"));

	СписокПрав.Добавить("МобильныйКлиент", НСтр("ru = 'Мобильный клиент'"));

	СписокПрав.Добавить("ТолстыйКлиент", НСтр("ru = 'Толстый клиент'"));

	СписокПрав.Добавить("ВнешнееСоединение", НСтр("ru = 'Внешнее соединение'"));

	СписокПрав.Добавить("Automation", НСтр("ru = 'Automation'"));

	СписокПрав.Добавить("РежимВсеФункции", НСтр("ru = 'Режим ""Все функции""'"));

	СписокПрав.Добавить("РегистрацияИнформационнойБазыСистемыВзаимодействия", НСтр(
		"ru = 'Регистрация системы взаимодействия'"));

	СписокПрав.Добавить("РежимОсновногоОкнаОбычный", НСтр("ru = 'Режим основного окна ""Обычный""'"));

	СписокПрав.Добавить("РежимОсновногоОкнаРабочееМесто", НСтр("ru = 'Режим основного окна ""Рабочее место""'"));

	СписокПрав.Добавить("РежимОсновногоОкнаВстроенноеРабочееМесто", НСтр(
		"ru = 'Режим основного окна ""Встроенное рабочее место""'"));

	СписокПрав.Добавить("РежимОсновногоОкнаПолноэкранноеРабочееМесто", НСтр(
		"ru = 'Режим основного окна ""Полноэкранное рабочее место""'"));

	СписокПрав.Добавить("РежимОсновногоОкнаКиоск", НСтр("ru = 'Режим основного окна ""Киоск""'"));

	СписокПрав.Добавить("СохранениеДанныхПользователя", НСтр("ru = 'Сохранение данных пользователя'"));

	СписокПрав.Добавить("АдминистрированиеРасширенийКонфигурации", НСтр(
		"ru = 'Администрирование расширений конфигурации'"));

	СписокПрав.Добавить("ИнтерактивноеОткрытиеВнешнихОбработок", НСтр(
		"ru = 'Интерактивное открытие внешних обработок'"));

	СписокПрав.Добавить("ИнтерактивноеОткрытиеВнешнихОтчетов", НСтр("ru = 'Интерактивное открытие внешних отчетов'"));
...
Функция ПраваПараметраСеанса()
	...
	СписокПрав.Добавить("Получение", НСтр("ru = 'Получение'"));

	СписокПрав.Добавить("Установка", НСтр("ru = 'Установка'"));
...
Функция ПраваДокумента()
	...
	СписокПрав.Добавить("ИнтерактивноеПроведениеНеоперативное", НСтр("ru = 'Интерактивное проведение неоперативное'"));
...
Функция ПраваЗадачи()
	...
	СписокПрав.Добавить("ИнтерактивноеВыполнение", НСтр("ru = 'Интерактивный выполнение'"));
...
Функция ПраваВнешнегоИсточникаДанных()
	...
	СписокПрав.Добавить("ИзменениеСтандартнойАутентификации", НСтр(
		"ru = 'Изменение стандартной аутентификации текущего пользователя'"));

	СписокПрав.Добавить("ИзменениеСтандартнойАутентификацииСеанса", НСтр(
		"ru = 'Изменение стандартной аутентификации сеанса'"));

	СписокПрав.Добавить("ИзменениеАутентификацииОССеанса", НСтр(
		"ru = 'Изменение аутентификации операционной системы сеанса'"));
...
Процедура ДобавитьПраваПрограммнойРаботыСОбъектами(СписокПрав, БезИзменения = Ложь, Ссылочные = Истина,
	СОграничением = Истина)

	СписокПрав.Добавить("Чтение", НСтр("ru = 'Чтение'"), СОграничением);

	Если БезИзменения Тогда
		Возврат;
	КонецЕсли;

	СписокПрав.Добавить("Изменение", НСтр("ru = 'Изменение'"), СОграничением);

	Если Не Ссылочные Тогда
		Возврат;
	КонецЕсли;

	СписокПрав.Добавить("Добавление", НСтр("ru = 'Добавление'"), СОграничением);

	СписокПрав.Добавить("Удаление", НСтр("ru = 'Удаление'"), СОграничением);

КонецПроцедуры

Процедура ДобавитьПраваИнтерактивнойРаботыСОбъектами(СписокПрав, БезИзменения = Ложь, Ссылочные = Истина,
	...
	СписокПрав.Добавить("ИнтерактивнаяПометкаУдаления", НСтр("ru = 'Интерактивная пометка на удаление'"));
...
Процедура ДобавитьПраваРаботыСПредопределеннымиДанными(СписокПрав)
	...
	СписокПрав.Добавить("ИнтерактивноеУдалениеПредопределенныхДанных", НСтр(
		"ru = 'Интерактивное удаление предопределенных'"));

	СписокПрав.Добавить("ИнтерактивнаяПометкаУдаленияПредопределенныхДанных", НСтр(
		"ru = 'Интерактивная пометка на удаление предопределенных'"));

	СписокПрав.Добавить("ИнтерактивноеСнятиеПометкиУдаленияПредопределенныхДанных", НСтр(
		"ru = 'Интерактивное снятие пометки удаления предопределенных'"));

	СписокПрав.Добавить("ИнтерактивноеУдалениеПомеченныхПредопределенныхДанных", НСтр(
...
Процедура ДобавитьПраваРаботыСИсторией(СписокПрав, ПраваНаОтсутствующиеДанные = Истина)
	...
	СписокПрав.Добавить("ЧтениеИсторииДанных", НСтр("ru = 'Чтение истории данных'"));

	Если ПраваНаОтсутствующиеДанные Тогда
		СписокПрав.Добавить("ЧтениеИсторииДанныхОтсутствующихДанных", НСтр(
			"ru = 'Чтение истории данных отсутствующих данных'"));
	КонецЕсли;

	СписокПрав.Добавить("ИзменениеИсторииДанных", НСтр("ru = 'Изменение истории данных'"));

	Если ПраваНаОтсутствующиеДанные Тогда
		СписокПрав.Добавить("ИзменениеИсторииДанныхОтсутствующихДанных", НСтр(
			"ru = 'Изменение истории данных отсутствующих данных'"));
	КонецЕсли;

	СписокПрав.Добавить("ИзменениеНастроекИсторииДанных", НСтр("ru = 'Изменение настроек истории данных'"));

	СписокПрав.Добавить("ИзменениеКомментарияВерсииИсторииДанных", НСтр(
		"ru = 'Изменение комментария версии истории данных'"));

	СписокПрав.Добавить("ПросмотрИсторииДанных", НСтр("ru = 'Просмотр истории данных'"));

	СписокПрав.Добавить("РедактированиеКомментарияВерсииИсторииДанных", НСтр(
		"ru = 'Редактирование комментария версии истории данных'"));

	СписокПрав.Добавить("ПереходНаВерсиюИсторииДанных", НСтр("ru = 'Переход на версию истории данных'"));
