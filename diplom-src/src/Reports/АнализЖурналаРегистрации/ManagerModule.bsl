...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета,
		"АнализАктивностиПользователей");
	НастройкиВарианта.Описание = НСтр("ru = 'Позволяет выполнять мониторинг активности пользователей
	...
	НастройкиВарианта.Описание = НСтр("ru = 'Подробная информация о том,
	...
	НастройкиВарианта = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета,
		"ПродолжительностьРаботыРегламентныхЗаданий");
...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	ИсходныеДанные.Колонки.Добавить("СозданоБизнесПроцессов", Новый ОписаниеТипов("Число",
	...
	ИсходныеДанные.Колонки.Добавить("ИзмененоБизнесПроцессов", Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(10)));
	...
	ОдновременноСессий.Колонки.Добавить("ДатаОдновременноРаботавшихПользователей", Новый ОписаниеТипов("Дата", , ,
		Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	ОдновременноСессий.Колонки.Добавить("ОдновременноРаботавшихПользователей", Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(10)));
	...
	Если ПользовательФильтр.Количество() = 0 Тогда
		Возврат Новый Структура("АнализАктивностиПользователей, ОдновременноСессий, ОтчетПустой", ИсходныеДанные,
			ОдновременноСессий, Истина);
	КонецЕсли;
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанныхЖурналаРегистрации.Сеанс) Или Не ЗначениеЗаполнено(
			СтрокаДанныхЖурналаРегистрации.Дата) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ТекущийСеанс <> Сеанс Или СтрокаДанныхЖурналаРегистрации.Событие = "_$Session$_.Start" Тогда
		...
		Если СтрокаДанныхЖурналаРегистрации.Событие = "_$Data$_.New" Тогда
			...
			Если СтрНайти(СтрокаДанныхЖурналаРегистрации.Метаданные, "Документ.") > 0 И ВыводитьДокументы Тогда
			...
			Если СтрНайти(СтрокаДанныхЖурналаРегистрации.Метаданные, "Справочник.") > 0 И ВыводитьСправочники Тогда
				ВидОбъекта = СтрокаДанныхЖурналаРегистрации.ПредставлениеМетаданных;
				ОбъектСправочникДокумент = СтрокаДанныхЖурналаРегистрации.Данные;
		...
		Если СтрокаДанныхЖурналаРегистрации.Событие = "_$Data$_.New" Тогда
			...
			Если СтрНайти(СтрокаДанныхЖурналаРегистрации.Метаданные, "БизнесПроцесс.") > 0 И ВыводитьБизнесПроцессы Тогда
				ВидОбъекта = СтрокаДанныхЖурналаРегистрации.ПредставлениеМетаданных;
				ОбъектСправочникДокумент = СтрокаДанныхЖурналаРегистрации.Данные;
			...
			Если СтрНайти(СтрокаДанныхЖурналаРегистрации.Метаданные, "Задача.") > 0 И ВыводитьЗадачи Тогда
				ВидОбъекта = СтрокаДанныхЖурналаРегистрации.ПредставлениеМетаданных;
				ОбъектСправочникДокумент = СтрокаДанныхЖурналаРегистрации.Данные;
	...
	Возврат Новый Структура("АнализАктивностиПользователей, ОдновременноСессий, ОтчетПустой", ИсходныеДанные,
		ОдновременноСессий, ОтчетПустой);
...
Функция УникальныеИдентификаторыПользователей(ИдентификаторыПользователей)
	...
	Для Каждого Элемент Из МассивУникальныеИдентификаторыПользователей Цикл
		...
		Если ЗначениеЗаполнено(Элемент) Тогда
			...
			ИдентификаторПользователяИБ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИмяПользователяСсылка,
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)'"),
					Элемент, СчетчикКоличестваСеансовПользователя);
			КонецЕсли;
		КонецЦикла;
...
Функция СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения) Экспорт
	...
	МинимальнаяПродолжительностьСеансовРегламентныхЗаданий = ПараметрыЗаполнения.МинимальнаяПродолжительностьСеансовРегламентныхЗаданий;
	...
	Если ВыводитьЗаголовок.Значение = ТипВыводаТекстаКомпоновкиДанных.Выводить И ВыводитьЗаголовок.Использование
		Или Не ВыводитьЗаголовок.Использование Тогда
		Отчет.Вывести(ОписаниеОбластиМакета(Макет, "ШапкаОтчета"));
	КонецЕсли;

	Если ВыводитьОтбор.Значение = ТипВыводаТекстаКомпоновкиДанных.Выводить И ВыводитьОтбор.Использование
	...
	Если ЗначениеЗаполнено(ОдновременноСессий) Тогда
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Если ТекущееКоличествоСессий <> 0 И ТекущееКоличествоСессий <> СтрокаОдновременноСессий.ОдновременноРегламентныхЗаданий
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число") И Не ТипЗнч(Элемент) = Тип("Дата") Тогда
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (сеанс %2)'"), Элемент, МассивРегламентныхЗаданий[ИндексРегламентногоЗадания
						+ 1]);
	...
	Для Каждого СтрокаРегламентныеЗадания Из ТаблицаСеансыРегламентныхЗаданий Цикл
		...
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено И ПризнакСменыТочки Тогда
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2 из %3)'"),
						Точка.Значение, ЗапусковРегламентногоЗадания, Строка(СтрокаКоличествоЗапусков.Запусков));
			...
			Интервал.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 - %2'"), Формат(
				Интервал.Начало, "ДЛФ=T"), Формат(Интервал.Конец, "ДЛФ=T"));
			ПризнакСменыТочки = Ложь;
			// Не заполняем расшифровку для фоновых заданий.
			Если СтрокаРегламентныеЗадания.СобытиеМетаданные <> "" Тогда
...
Функция ДанныеДляОтчетаПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения)
	...
	Для Каждого РегЗадание Из СписокВсехРегламентныхЗаданий Цикл
		...
		СоответствиеНаименованиеИдентификатор.Вставить(РегЗадание.Наименование, Строка(
			РегЗадание.УникальныйИдентификатор));
	...
	Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
		Если СтрокаОдновременноСессий.ОдновременноРегламентныхЗаданий >= РазмерОдновременноСессий
			...
			СтрокаИтогоОдновременноРегламентныхЗаданий.ДатаОдновременноРегламентныхЗаданий = СтрокаОдновременноСессий.ДатаОдновременноРегламентныхЗаданий;
			СтрокаИтогоОдновременноРегламентныхЗаданий.ОдновременноРегламентныхЗаданий = СтрокаОдновременноСессий.ОдновременноРегламентныхЗаданий;
			СтрокаИтогоОдновременноРегламентныхЗаданий.СписокРегламентныхЗаданий = СтрокаОдновременноСессий.СписокРегламентныхЗаданий;
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыСеансыРегламентныхЗаданий.Вставить("СоответствиеНаименованиеИдентификатор",
		СоответствиеНаименованиеИдентификатор);
	ПараметрыСеансыРегламентныхЗаданий.Вставить("СоответствиеМетаданныеИдентификатор",
		СоответствиеМетаданныеИдентификатор);
	...
	ТаблицаСеансыРегламентныхЗаданий = СеансыРегламентныхЗаданий(
		ПараметрыСеансыРегламентныхЗаданий).ТаблицаСеансыРегламентныхЗаданий;
...
Функция ОдновременноРегламентныхЗаданий(ПараметрыОдновременноСессий)
	...
	МинимальнаяПродолжительностьСеансовРегламентныхЗаданий = ПараметрыОдновременноСессий.МинимальнаяПродолжительностьСеансовРегламентныхЗаданий;

	ОдновременноСессий = Новый ТаблицаЗначений;

	ОдновременноСессий.Колонки.Добавить("ДатаОдновременноРегламентныхЗаданий", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(50)));
	ОдновременноСессий.Колонки.Добавить("ОдновременноРегламентныхЗаданий", Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(10)));
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл
		Если Не ЗначениеЗаполнено(СтрокаДанныхЖурналаРегистрации.Дата) Или Не ЗначениеЗаполнено(
			СтрокаДанныхЖурналаРегистрации.Метаданные) Тогда
			Продолжить;
		КонецЕсли;

		НазваниеИУникальныйИдентификатор = НазваниеИУникальныйИдентификаторСеансаРегламентногоЗадания(
			СтрокаДанныхЖурналаРегистрации, СоответствиеНаименованиеИдентификатор, СоответствиеМетаданныеИдентификатор,
		...
		Если Не СкрытьРегламентныеЗадания = Неопределено И Не ТипЗнч(СкрытьРегламентныеЗадания) = Тип("Строка") Тогда
			...
			Если Не ФильтрРегламентныхЗаданий = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		ИначеЕсли Не СкрытьРегламентныеЗадания = Неопределено И ТипЗнч(СкрытьРегламентныеЗадания) = Тип("Строка") Тогда
		...
		Если ТекущаяДата <> ДатаОдновременноРегламентныхЗаданий Тогда
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1 - %2'"), Формат(ТекущаяДата, "ДЛФ=T"), Формат(КонецЧаса(ТекущаяДата), "ДЛФ=T"));
		...
		Иначе
			...
			Если ЗначениеЗаполнено(МаксМассивРегламентныхЗаданий) Тогда
				...
				Если ИндексСтрокиМассива <> Неопределено И МаксМассивРегламентныхЗаданий[ИндексСтрокиМассива + 1]
					= МассивРегламентныхЗаданий[ИндексРегламентногоЗадания + 1] И СтрокаДанныхЖурналаРегистрации.Дата
					- МаксМассивРегламентныхЗаданий[ИндексСтрокиМассива + 2]
...
Функция СеансыРегламентныхЗаданий(ПараметрыСеансыРегламентныхЗаданий)
	...
	ТаблицаСеансыРегламентныхЗаданий.Колонки.Добавить("ДатаЗапускаЗадания", Новый ОписаниеТипов("Дата", , ,
		Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	ТаблицаСеансыРегламентныхЗаданий.Колонки.Добавить("ДатаОкончанияЗадания", Новый ОписаниеТипов("Дата", , ,
		Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	ТаблицаСеансыРегламентныхЗаданий.Колонки.Добавить("НазваниеСобытия", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(100)));
	ТаблицаСеансыРегламентныхЗаданий.Колонки.Добавить("СобытиеМетаданные", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(100)));
	ТаблицаСеансыРегламентныхЗаданий.Колонки.Добавить("Сеанс", Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(10)));

	КоличествоЗапусков = Новый ТаблицаЗначений;
	КоличествоЗапусков.Колонки.Добавить("НазваниеСобытия", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(100)));
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл
		Если Не ЗначениеЗаполнено(СтрокаДанныхЖурналаРегистрации.Метаданные) И ОтображатьФоновыеЗадания = Ложь Тогда
			Продолжить;
		КонецЕсли;
	...
	Возврат Новый Структура("ТаблицаСеансыРегламентныхЗаданий, КоличествоЗапусков", ТаблицаСеансыРегламентныхЗаданий,
		КоличествоЗапусков);
КонецФункции
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ПродолжительностьСекунды = Формат((Формат(ПродолжительностьРЗ / 60, "ЧДЦ=2") - Цел(ПродолжительностьРЗ / 60))
			* 60, "ЧДЦ=0");
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = '%1 мин %2 сек'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		ПродолжительностьЧасы    = Формат(ПродолжительностьРЗ / 60 / 60, "ЧДЦ=0");
		ПродолжительностьМинуты  = (Формат(ПродолжительностьРЗ / 60 / 60, "ЧДЦ=2") - Цел(ПродолжительностьРЗ / 60 / 60))
			* 60;
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 ч %2 мин'"),
			ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
...
Функция НазваниеИУникальныйИдентификаторСеансаРегламентногоЗадания(СтрокаДанныхЖурналаРегистрации,
	...
	Если Не СтрокаДанныхЖурналаРегистрации.Данные = "" Тогда
		УникальныйИдентификаторРегламентногоЗадания = СоответствиеНаименованиеИдентификатор[СтрокаДанныхЖурналаРегистрации.Данные];
		НазваниеСеанса = СтрокаДанныхЖурналаРегистрации.Данные;
	Иначе
		УникальныйИдентификаторРегламентногоЗадания = СоответствиеМетаданныеИдентификатор[МетаданныеРегламентногоЗадания(
			СтрокаДанныхЖурналаРегистрации.Метаданные)];
		НазваниеСеанса = СоответствиеМетаданныеНазвание[МетаданныеРегламентногоЗадания(
														СтрокаДанныхЖурналаРегистрации.Метаданные)];
	КонецЕсли;

	Возврат Новый Структура("НазваниеСеанса, УникальныйИдентификаторРегламентногоЗадания", НазваниеСеанса,
		УникальныйИдентификаторРегламентногоЗадания)
КонецФункции
...
Функция ИнформацияПоОшибкамЖурналаРегистрации(Знач ДатаНачала, Знач ДатаОкончания, СмещениеВремениСервера)
	...
	УстановитьПривилегированныйРежим(Истина);
	ВыгрузитьЖурналРегистрации(ДанныеЖурналаРегистрации, Новый Структура("Уровень, ДатаНачала, ДатаОкончания",
		УровниРегистрацииОшибок, ДатаНачала, ДатаОкончания));
	УстановитьПривилегированныйРежим(Ложь);
	...
	Если СвернутыеДанные.Количество() > 0 Тогда
		...
		Для Каждого Запись Из СвернутыеДанные Цикл
			...
			МассивСтрок = ДанныеЖурналаРегистрации.НайтиСтроки(
				Новый Структура("Уровень, Комментарий", УровеньЖурналаРегистрации.Ошибка, Запись.Комментарий));
