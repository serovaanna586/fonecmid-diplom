...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ИзменитьРасписание.Видимость = ОбщегоНазначения.ПодсистемаСуществует(
...
Процедура ОтчетОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ОтборЖурналаРегистрации = Новый Структура("Сеанс, ДатаНачала, ДатаОкончания", СеансРегламентногоЗадания,
...
Процедура НастроитьРасписаниеРегламентногоЗадания(Команда)
	...
	Иначе
		ПоказатьПредупреждение( , НСтр(
			"ru = 'Невозможно изменить расписание регламентного задания: регламентное задание не существует в этой версии программы.'"));
	КонецЕсли;
...
Процедура ПерейтиКЖурналуРегистрации(Команда)
	...
	Для Каждого Область Из ВыделенныеОбласти Цикл
		...
		КонецЕсли;
		Если Расшифровка = Неопределено Или Область.Верх <> Область.Низ Тогда
