...
//
Функция БлокировкаСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ) Экспорт

	Результат = СвойстваИнформационнойБазы(ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ,
...
//
Процедура УстановитьБлокировкуСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ, Знач СвойстваБлокировкиСеансовИЗаданий) Экспорт

	УстановитьСвойстваИнформационнойБазы(
		ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ,
		СловарьСвойствБлокировкиСеансовИРегламентныхЗаданий(), СвойстваБлокировкиСеансовИЗаданий);
...
//
Процедура ПроверитьПараметрыАдминистрирования(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено, ПроверятьПараметрыАдминистрированияИнформационнойБазы = Истина,
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключиться с компьютера %1 к кластеру серверов по причине:
					 ...
					 |
					 |Также рекомендуется проверить параметры подключения и сетевого экрана.'"), ИмяКомпьютера(),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), КаталогПрограммы() + ?(
				ОбщегоНазначения.ЭтоWindowsСервер(), "ras.exe", "ras"), XMLСтрока(
				ПараметрыАдминистрированияКластера.ПортСервераАдминистрирования),
				ПараметрыАдминистрированияКластера.АдресАгентаСервера, XMLСтрока(
...
//   Булево
//
Функция БлокировкаРегламентныхЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ) Экспорт
...
//
Процедура УстановитьБлокировкуРегламентныхЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ, Знач БлокировкаРегламентныхЗаданий) Экспорт

	Словарь = Новый Структура("БлокировкаРегламентныхЗаданий", "scheduled-jobs-deny");
	Свойства = Новый Структура("БлокировкаРегламентныхЗаданий", БлокировкаРегламентныхЗаданий);

	УстановитьСвойстваИнформационнойБазы(
		ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ, Словарь, Свойства);
...
//
Функция СеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ,
	Знач Фильтр = Неопределено) Экспорт

	ИдентификаторКластера = ИдентификаторКластера(ПараметрыАдминистрированияКластера);
	ИдентификаторИнформационнойБазы = ИдентификаторИнформационнойБазы(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ);
	...
	Для Каждого Процесс Из ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, , ,
		ТипыСвойствРабочегоПроцесса()) Цикл
	...
	Для Каждого Соединение Из ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, , ,
		ТипыСвойствОписанияСоединения()) Цикл
	...
	Для Каждого ЛицензияСеанса Из ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, , ,
		ТипыСвойствЛицензии()) Цикл
	...
	Для Каждого БлокировкаСеанса Из ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, , ,
		ТипыСвойствБлокировки()) Цикл
		АдминистрированиеКластера.ДанныеСеансаИзБлокировки(БлокировкиСеансов, БлокировкаСеанса["descr"],
			БлокировкаСеанса["session"], ПараметрыАдминистрированияИБ.ИмяВКластере);
	...
	Для Каждого Сеанс Из ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, , Фильтр, ТипыСвойствСеансов()) Цикл
		...
		РазобранныйСеанс.Вставить("РежимБлокировкиБД", ?(БлокировкиСеансов[Сеанс["session"]] <> Неопределено,
			БлокировкиСеансов[Сеанс["session"]].РежимБлокировкиБД, ""));
		РазобранныйСеанс.Вставить("Разделитель", ?(БлокировкиСеансов[Сеанс["session"]] <> Неопределено,
			БлокировкиСеансов[Сеанс["session"]].Разделитель, ""));
...
//   Фильтр - см. АдминистрированиеКластера.ФильтрСеансов, Массив из см. АдминистрированиеКластера.ФильтрСеансов
//
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ,
	Знач Фильтр = Неопределено) Экспорт
	...
	Для ТекущаяПопытка = 0 По КоличествоПопыток Цикл
		...
		Сеансы = СвойстваСеансов(ИдентификаторКластера, ПараметрыАдминистрированияКластера,
			ИдентификаторИнформационнойБазы, Фильтр, Ложь);
...
Функция СоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ,
	...
	ИдентификаторИнформационнойБазы = ИдентификаторИнформационнойБазы(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ);
	Возврат СвойстваСоединений(ИдентификаторКластера, ПараметрыАдминистрированияКластера,
		ИдентификаторИнформационнойБазы, ПараметрыАдминистрированияИБ, Фильтр, Истина);
...
//   Фильтр - см. АдминистрированиеКластера.ФильтрСоединений, Массив из см. АдминистрированиеКластера.ФильтрСоединений
//
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	АдминистрированиеКластера.ДобавитьУсловиеФильтра(Фильтр, "ИдентификаторКлиентскогоПриложения",
		ВидСравнения.ВСписке, Значение);
	...
	Для ТекущаяПопытка = 0 По КоличествоПопыток Цикл
		...
		Соединения = СвойстваСоединений(ИдентификаторКластера, ПараметрыАдминистрированияКластера,
			ИдентификаторИнформационнойБазы, ПараметрыАдминистрированияИБ, Фильтр, Ложь);
		...
		Для Каждого Соединение Из Соединения Цикл
			...
			Попытка
				...
				Команда = "connection disconnect --process=%1 --connection=%2 --infobase-user=%3 --infobase-pwd=%4 "
					+ ПараметрыКластера;
				ПодставитьПараметрыВКоманду(Команда, Соединение.Получить("process"), Соединение.Получить("connection"),
...
//
Функция ПрофильБезопасностиИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ) Экспорт
...
//
Функция ПрофильБезопасностиБезопасногоРежимаИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ) Экспорт
...
//
Процедура УстановитьПрофильБезопасностиИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ, Знач ИмяПрофиля = "") Экспорт
	...
	УстановитьСвойстваИнформационнойБазы(
		ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ, Словарь, Значения);
...
//
Процедура УстановитьПрофильБезопасностиБезопасногоРежимаИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ, Знач ИмяПрофиля = "") Экспорт
...
//
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля,
	Знач ИдентификаторКластера = Неопределено) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'В кластере серверов %1 не зарегистрирован профиль безопасности %2'"), ИдентификаторКластера,
			ИмяПрофиля);
	КонецЕсли;
	...
	Результат.Вставить("ВиртуальныеКаталоги", ПолучитьВиртуальныеКаталоги(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ИмяПрофиля));
	
	// Разрешенные COM-классы
	Результат.Вставить("COMКлассы", ПолучитьРазрешенныеCOMКлассы(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ИмяПрофиля));
	
	// Внешние компоненты
	Результат.Вставить("ВнешниеКомпоненты", ПолучитьРазрешенныеВнешниеКомпоненты(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ИмяПрофиля));
	
	// Внешние модули
	Результат.Вставить("ВнешниеМодули", ПолучитьРазрешенныеВнешниеМодули(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ИмяПрофиля));
	
	// Приложения ОС
	Результат.Вставить("ПриложенияОС", ПолучитьРазрешенныеПриложенияОС(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ИмяПрофиля));
	
	// Интернет-ресурсы
	Результат.Вставить("ИнтернетРесурсы", ПолучитьРазрешенныеИнтернетРесурсы(ИдентификаторКластера,
		ПараметрыАдминистрированияКластера, ИмяПрофиля));
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'В кластере серверов %1 уже зарегистрирован профиль безопасности %2'"), ИдентификаторКластера,
			ИмяПрофиля);
	КонецЕсли;
...
//   СвойстваПрофиляБезопасности - см. АдминистрированиеКластера.СвойстваПрофиляБезопасности
//
Процедура УстановитьСвойстваПрофиляБезопасности(Знач ПараметрыАдминистрированияКластера,
	Знач СвойстваПрофиляБезопасности) Экспорт
...
//   Строка
//
Функция ИдентификаторИнформационнойБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'В кластере серверов %1 не зарегистрирована информационная база %2'"), ИдентификаторКластера,
			ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
//   Массив из Структура
//
Функция СвойстваИнформационныхБаз(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач Фильтр = Неопределено) Экспорт
...
Функция ИдентификаторКластера(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Не обнаружен кластер серверов с портом %1'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
...
Функция СвойстваКластеров(Знач ПараметрыАдминистрированияКластера, Знач Фильтр = Неопределено) Экспорт
	...
	Возврат ЗапуститьКоманду("cluster list " + Сервер, ПараметрыАдминистрированияКластера, , Фильтр,
		ТипыСвойствКластера());
...
//
Функция СвойстваРабочихПроцессов(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач Фильтр = Неопределено) Экспорт

	Возврат ЗапуститьКоманду("process list " + ПараметрыКластера(ПараметрыАдминистрированияКластера,
...
//
Функция СвойстваРабочихСерверов(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач Фильтр = Неопределено) Экспорт

	ПараметрыКластера = ПараметрыКластера(ПараметрыАдминистрированияКластера, ИдентификаторКластера);

	Возврат ЗапуститьКоманду("server list " + ПараметрыКластера, ПараметрыАдминистрированияКластера, , Фильтр,
		ТипыСвойствРабочихСерверов());
...
//
Функция СвойстваСеансов(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИдентификаторИнформационнойБазы, Знач Фильтр = Неопределено, Знач ИспользоватьСловарь = Истина) Экспорт

	Команда = "session list --infobase=%1 " + ПараметрыКластера(ПараметрыАдминистрированияКластера,
...
//   - Массив из Соответствие
//
Функция СвойстваСоединений(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИдентификаторИнформационнойБазы, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено,
	Знач ИспользоватьСловарь = Ложь) Экспорт
	...
	Для Каждого РабочийПроцесс Из РабочиеПроцессы Цикл
		...
		ПодставитьПараметрыВКоманду(Команда, РабочийПроцесс.Получить("process"), ИдентификаторИнформационнойБазы,
			ПараметрыАдминистрированияИБ.ИмяАдминистратораИнформационнойБазы,
			ПараметрыАдминистрированияИБ.ПарольАдминистратораИнформационнойБазы);

		СоединенияРабочегоПроцесса = ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, Словарь, Фильтр,
			ТипыСвойствСоединений());
		Для Каждого Соединение Из СоединенияРабочегоПроцесса Цикл
			Если ИспользоватьСловарь Тогда
				...
				КонецЕсли;
				Соединение.Вставить("РежимСоединенияСИнформационнойБазой", ?(
					Соединение.РежимСоединенияСИнформационнойБазой = "shared", 0, 1));
				Соединение.Вставить("УправлениеНаходитсяНаСервере", ?(Соединение.УправлениеНаходитсяНаСервере
					= "client", 0, 1));
			Иначе
...
Функция СвойстваИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ,
	Знач Словарь)
	...
	Результат = ЗапуститьКоманду(Команда + " " + ПараметрыКластера, ПараметрыАдминистрированияКластера, Словарь, ,
		ТипыСвойствИнформационнойБазы());

	Возврат Результат[0];

КонецФункции

Процедура УстановитьСвойстваИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ, Знач Словарь, Знач ЗначенияСвойств)
	...
	Для Каждого КлючИЗначение Из Словарь Цикл
		Если КлючИЗначение.Значение = "scheduled-jobs-deny" Или КлючИЗначение.Значение = "sessions-deny" Тогда
			НовыеЗначенияСвойств.Вставить(КлючИЗначение.Ключ, Формат(НовыеЗначенияСвойств[КлючИЗначение.Ключ],
				"БЛ=off; БИ=on"));
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьПрофилиБезопасности(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач Фильтр = Неопределено)

	Команда = "profile list " + ПараметрыКластера(ПараметрыАдминистрированияКластера, ИдентификаторКластера);
	Результат = ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, СловарьСвойствПрофиляБезопасности(),
		Фильтр, ТипыСвойствПрофиля());

	Возврат Результат;

КонецФункции

Функция ПолучитьВиртуальныеКаталоги(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИмяПрофиля, Знач Фильтр = Неопределено)

	Возврат СпискиКонтроляДоступа(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "directory", // Не локализуется

		СловарьСвойствВиртуальногоКаталога(), , ТипыСвойствВиртуальногоКаталога());

КонецФункции

Функция ПолучитьРазрешенныеCOMКлассы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИмяПрофиля, Знач Фильтр = Неопределено)

	Возврат СпискиКонтроляДоступа(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "com", // Не локализуется

		СловарьСвойствCOMКласса(), , ТипыСвойствCOMКласса());

КонецФункции

Функция ПолучитьРазрешенныеВнешниеКомпоненты(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИмяПрофиля, Знач Фильтр = Неопределено)

	Возврат СпискиКонтроляДоступа(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "addin", // Не локализуется

		СловарьСвойствВнешнейКомпоненты(), , ТипыСвойствВнешнейКомпоненты());

КонецФункции

Функция ПолучитьРазрешенныеВнешниеМодули(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИмяПрофиля, Знач Фильтр = Неопределено)

	Возврат СпискиКонтроляДоступа(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "module", // Не локализуется

		СловарьСвойствВнешнегоМодуля(), , ТипыСвойствВнешнегоМодуля());

КонецФункции

Функция ПолучитьРазрешенныеПриложенияОС(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИмяПрофиля, Знач Фильтр = Неопределено)

	Возврат СпискиКонтроляДоступа(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "app", // Не локализуется

		СловарьСвойствПриложенияОС(), , ТипыСвойствПриложенияОС());

КонецФункции

Функция ПолучитьРазрешенныеИнтернетРесурсы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	Знач ИмяПрофиля, Знач Фильтр = Неопределено)

	Возврат СпискиКонтроляДоступа(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "inet", // Не локализуется

		СловарьСвойствИнтернетРесурса(), , ТипыСвойствИнтернетРесурса());

КонецФункции

Функция СпискиКонтроляДоступа(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля,
	Знач ИмяСписка, Знач Словарь, Знач Фильтр = Неопределено, Знач ТипыСвойств = Неопределено)

	Команда = "profile acl --name=%1 %2 list " + ПараметрыКластера(ПараметрыАдминистрированияКластера,
		ИдентификаторКластера);
...
Процедура ОбновитьСвойстваПрофиляБезопасности(Знач ПараметрыАдминистрированияКластера, Знач НовыеСвойства,
	Знач СтарыеСвойства = Неопределено)
	...
	Для Каждого ФрагментСловаря Из СловарьСвойствПрофиля Цикл
		Если НовыеСвойства.Свойство(ФрагментСловаря.Ключ) Тогда
			Если НовыеСвойства[ФрагментСловаря.Ключ] <> СтарыеСвойства[ФрагментСловаря.Ключ] Тогда
				...
				ДобавитьПараметрыКомандыПоСловарю(Команда, СловарьСвойствПрофиля, НовыеСвойства,
					ПоддерживаемыеСвойства.profile);
				ЗапуститьКоманду(Команда + " " + ПараметрыКластера, ПараметрыАдминистрированияКластера);
				Прервать;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого ФрагментСловаря Из СловарьСвойствИспользованияСписокКонтроляДоступа() Цикл
		Если НовыеСвойства[ФрагментСловаря.Ключ] = СтарыеСвойства[ФрагментСловаря.Ключ] Тогда
			Продолжить;
		КонецЕсли;
		УстановитьИспользованиеСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера,
			ИмяПрофиля, ФрагментСловаря.Значение, Не НовыеСвойства[ФрагментСловаря.Ключ]);
	КонецЦикла;
	
	// Виртуальные каталоги
	ОбновитьЭлементыСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля,
		"directory", СловарьСвойствВиртуальногоКаталога(), НовыеСвойства.ВиртуальныеКаталоги,
		СтарыеСвойства.ВиртуальныеКаталоги);
	
	// Разрешенные COM-классы
	ОбновитьЭлементыСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "com",
		СловарьСвойствCOMКласса(), НовыеСвойства.COMКлассы, СтарыеСвойства.COMКлассы);
	
	// Внешние компоненты
	ОбновитьЭлементыСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "addin",
		СловарьСвойствВнешнейКомпоненты(), НовыеСвойства.ВнешниеКомпоненты, СтарыеСвойства.ВнешниеКомпоненты);
	
	// Внешние модули
	ОбновитьЭлементыСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля,
		"module", СловарьСвойствВнешнегоМодуля(), НовыеСвойства.ВнешниеМодули, СтарыеСвойства.ВнешниеМодули);
		
	// Приложения ОС
	ОбновитьЭлементыСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "app",
		СловарьСвойствПриложенияОС(), НовыеСвойства.ПриложенияОС, СтарыеСвойства.ПриложенияОС);
	
	// Интернет-ресурсы
	ОбновитьЭлементыСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, "inet",
		СловарьСвойствИнтернетРесурса(), НовыеСвойства.ИнтернетРесурсы, СтарыеСвойства.ИнтернетРесурсы);

КонецПроцедуры

Процедура УстановитьИспользованиеСпискаКонтроляДоступа(Знач ИдентификаторКластера,
	Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля, Знач ИмяСписка, Знач Использование)

	Команда = "profile acl --name=%1 %2 --access=%3 " + ПараметрыКластера(ПараметрыАдминистрированияКластера,
		ИдентификаторКластера);
...
Процедура УдалитьЭлементСпискаКонтроляДоступа(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	...
	Команда = "profile acl --name=%1 %2 remove --%3=%4 " + ПараметрыКластера(ПараметрыАдминистрированияКластера,
		ИдентификаторКластера);
...
Процедура ОбновитьЭлементСпискаКонтроляДоступа(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	...
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля, ИмяСписка);
	ДобавитьПараметрыКомандыПоСловарю(Команда, Словарь, СвойстваЭлемента, ПоддерживаемыеСвойства["profile_"
		+ ИмяСписка]);
...
Процедура ОбновитьЭлементыСпискаКонтроляДоступа(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	...
	Если СтарыеЭлементы = Неопределено Или СтарыеЭлементы.Количество() = 0 Тогда
		Для Каждого НовыйЭлемент Из НовыеЭлементы Цикл
			ОбновитьЭлементСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля,
				ИмяСписка, Словарь, НовыйЭлемент);
		КонецЦикла;
		Возврат;
	КонецЕсли;
		...
		Иначе
			...
			Для Каждого КлючИЗначение Из НовыйЭлемент Цикл
				Если КлючИЗначение.Значение <> СтарыйЭлемент[КлючИЗначение.Ключ] Тогда
					ОбновитьЭлементСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера,
						ИмяПрофиля, ИмяСписка, Словарь, НовыйЭлемент);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ЭлементыДляУдаления Цикл
		УдалитьЭлементСпискаКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля,
...
Функция РазобратьЭлементВывода(Знач ЭлементВывода, Знач Словарь)
	...
	Для Каждого ФрагментСловаря Из Словарь Цикл
		Если ТипЗнч(ФрагментСловаря.Значение) = Тип("ФиксированнаяСтруктура") Тогда
			...
			Иначе
				Свойство = РазобратьЭлементВывода(ЭлементВывода[ФрагментСловаря.Значение.Ключ],
					ФрагментСловаря.Значение.Словарь);
...
Функция ФильтрВНотациюRac(Знач Фильтр, Знач Словарь)
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Если ТипЗнч(Условие) = Тип("КлючИЗначение") Тогда
			...
			Результат.Добавить(Новый Структура("Свойство, ВидСравнения, Значение", Словарь[Условие.Ключ],
				ВидСравнения.Равно, Условие.Значение));

		ИначеЕсли ТипЗнч(Условие) = Тип("Структура") Тогда

			Результат.Добавить(Новый Структура("Свойство, ВидСравнения, Значение", Словарь[Условие.Свойство],
				Условие.ВидСравнения, Условие.Значение));
...
Функция ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, Словарь = Неопределено, Фильтр = Неопределено,
	ТипыСвойств = Неопределено)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр(
			"ru = 'В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера'");
	КонецЕсли;
	...
	Если Не ФайлКлиента.Существует() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно выполнить операцию администрирования кластера серверов по причине: не существует файл %1
				 ...
				 |- Для компьютеров с ОС Linux установите пакет 1c-enterprise83-server*.'"), ФайлКлиента.ПолноеИмя);
...
Функция ПараметрыКластера(ПараметрыАдминистрированияКластера, ИдентификаторКластера = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыАдминистрированияКластера.ИмяАдминистратораКластера) Тогда
		ПараметрыКластера = ПараметрыКластера + " --cluster-user="""
		...
		Если ЗначениеЗаполнено(ПараметрыАдминистрированияКластера.ПарольАдминистратораКластера) Тогда
			ПараметрыКластера = ПараметрыКластера + " --cluster-pwd="""
...
Процедура ПодставитьПараметрыВКоманду(Команда, Знач Параметр1, Знач Параметр2 = Неопределено,
	Знач Параметр3 = Неопределено, Знач Параметр4 = Неопределено)

	Команда = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Команда, ПривестиЗначение(Параметр1),
		ПривестиЗначение(Параметр2), ПривестиЗначение(Параметр3), ПривестиЗначение(Параметр4));
...
Функция ПоддерживаемыеСвойстваПрофиля(ПараметрыАдминистрированияКластера)
	...
	Свойства.Вставить("profile", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
		ТипыСвойствПрофиля()));

	Команда = "profile acl --name=%1 directory list " + ПараметрыКластера;
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля);
	Свойства.Вставить("profile_directory", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
		ТипыСвойствВиртуальногоКаталога()));

	Команда = "profile acl --name=%1 com list " + ПараметрыКластера;
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля);
	Свойства.Вставить("profile_com", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
		ТипыСвойствCOMКласса()));

	Команда = "profile acl --name=%1 addin list " + ПараметрыКластера;
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля);
	Свойства.Вставить("profile_addin", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
		ТипыСвойствВнешнейКомпоненты()));

	Команда = "profile acl --name=%1 module list " + ПараметрыКластера;
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля);
	Свойства.Вставить("profile_module", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
		ТипыСвойствВнешнегоМодуля()));

	Команда = "profile acl --name=%1 app list " + ПараметрыКластера;
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля);
	Свойства.Вставить("profile_app", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
		ТипыСвойствПриложенияОС()));

	Команда = "profile acl --name=%1 inet list " + ПараметрыКластера;
	ПодставитьПараметрыВКоманду(Команда, ИмяПрофиля);
	Свойства.Вставить("profile_inet", ПоддерживаемыеСвойства(Команда, ПараметрыАдминистрированияКластера,
...
Функция ПоддерживаемыеСвойстваИнформационнойБазы(ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ)
	...
	СвойстваБазы = СвойстваИнформационнойБазы(ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ,
		Неопределено);
...
Функция ПреобразоватьКоманднуюСтрокуВМассив(Знач КоманднаяСтрока)
	...
	Для Индекс = 1 По ДлинаСтроки Цикл
		...
		Если ПредыдущийСимвол = """" И Символ = """" И КавычкиОткрыты Тогда
