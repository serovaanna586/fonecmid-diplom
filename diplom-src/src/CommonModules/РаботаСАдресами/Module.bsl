...
Функция РегионАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	Если ТипЗнч(Адрес) = Тип("Строка") Тогда
		...
		Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Адрес) Тогда
			Адрес = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(Адрес,
...
Функция ГородАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	Если ЭтоГородФедеральногоЗначения(Регион) Или СтрСравнить(Адрес.AddressType,
		УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес()) = 0 Тогда
		Возврат Регион;
	КонецЕсли;
...
Функция СведенияОбАдресеПриказМинфинаРФ171н(Адрес, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		АдресПоПолям.МуниципальныйРайон = МодульАдресныйКлассификаторСлужебный.КраткоеНаписаниеМуниципальныхРайонов(
			АдресПоПолям.МуниципальныйРайон, Сокращение);

		Сокращение = НРег(СокрЛП(АдресПоПолям.ПоселениеТипКраткий));
		АдресПоПолям.Поселение = МодульАдресныйКлассификаторСлужебный.КраткоеНаписаниеПоселений(АдресПоПолям.Поселение,
			Сокращение);
	...
	Если Не Параметры.БезПредставлений Тогда
		...
		АдресИзJSON = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(АдресJSON,
			Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Функция ПроверитьАдреса(Знач Адреса, ДополнительныеПараметры = Неопределено) Экспорт
	...
	РезультатыПроверки = МодульАдресныйКлассификаторСлужебный.ПроверкаАдресов(АдресаПолями,
		ПараметрыПроверки.ПроверятьУстареваниеАдресов);
	...
	Для Каждого КлючЗначение Из РезультатыПроверки Цикл
		...
		Для Каждого ОписаниеВарианта Из СтрокаАдреса.Варианты Цикл
			...
			Представление = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(ОписаниеВарианта,
				ПараметрыПроверки.ВключатьСтрануВПредставление);
...
Функция ПоляАдресаВJSON(ПоляАдреса) Экспорт
	...
	Если ПоляАдреса.ТипАдреса <> РаботаСАдресамиКлиентСервер.МуниципальныйАдрес() И ПоляАдреса.ТипАдреса
		<> РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес() Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный тип адреса (%1)'"), ПоляАдреса.ТипАдреса);
	...
	Если ПоляАдреса.Свойство("Идентификаторы") Тогда
		Если ТипЗнч(ПоляАдреса.Идентификаторы.РегионИдентификатор) = ТипУникальныйИдентификатор И ЗначениеЗаполнено(
			ПоляАдреса.Идентификаторы.РегионИдентификатор) Тогда
			Результат.AreaID = Формат(ПоляАдреса.Идентификаторы.РегионИдентификатор, "ЧРГ=' '; ЧГ=0");
			Результат.ID = Результат.AreaID;
		КонецЕсли;

		Если ТипЗнч(ПоляАдреса.Идентификаторы.РайонИдентификатор) = ТипУникальныйИдентификатор И ЗначениеЗаполнено(
			ПоляАдреса.Идентификаторы.РайонИдентификатор) Тогда
			Результат.DistrictID = Строка(ПоляАдреса.Идентификаторы.РайонИдентификатор);
			Результат.ID         = Результат.DistrictID;
		КонецЕсли;

		Если ТипЗнч(ПоляАдреса.Идентификаторы.ГородИдентификатор) = ТипУникальныйИдентификатор И ЗначениеЗаполнено(
			ПоляАдреса.Идентификаторы.ГородИдентификатор) Тогда
		...
		Если ТипЗнч(ПоляАдреса.Идентификаторы.ПоселениеИдентификатор) = ТипУникальныйИдентификатор И ЗначениеЗаполнено(
			ПоляАдреса.Идентификаторы.ПоселениеИдентификатор) Тогда
		...
		Если ТипЗнч(ПоляАдреса.Идентификаторы.ТерриторияИдентификатор) = ТипУникальныйИдентификатор И ЗначениеЗаполнено(
			ПоляАдреса.Идентификаторы.ТерриторияИдентификатор) Тогда
			Результат.TerritoryID = Строка(ПоляАдреса.Идентификаторы.ТерриторияИдентификатор);
			Результат.ID          = Результат.TerritoryID;
		КонецЕсли;

		Если ТипЗнч(ПоляАдреса.Идентификаторы.УлицаИдентификатор) = ТипУникальныйИдентификатор И ЗначениеЗаполнено(
			ПоляАдреса.Идентификаторы.УлицаИдентификатор) Тогда
			Результат.StreetID = Строка(ПоляАдреса.Идентификаторы.УлицаИдентификатор);
			Результат.ID       = Результат.StreetID;
		КонецЕсли;
	КонецЕсли;

	Если ПоляАдреса.Свойство("ИдентификаторЗемельногоУчастка") И ТипЗнч(ПоляАдреса.ИдентификаторЗемельногоУчастка) = ТипУникальныйИдентификатор
	...
	Если ПоляАдреса.Свойство("ИдентификаторДома") И ТипЗнч(ПоляАдреса.ИдентификаторДома) = ТипУникальныйИдентификатор
		И ЗначениеЗаполнено(ПоляАдреса.ИдентификаторДома) Тогда
		Результат.HouseID = Строка(ПоляАдреса.ИдентификаторДома);
	КонецЕсли;

	Если ПоляАдреса.Свойство("ИдентификаторАдресногоОбъекта") И ТипЗнч(ПоляАдреса.ИдентификаторАдресногоОбъекта) = ТипУникальныйИдентификатор
	...
	Для Каждого ТекущийКорпус Из ПоляАдреса.Корпуса Цикл
		Результат.Buildings.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
			ТекущийКорпус.ТипКорпуса, ТекущийКорпус.Номер));
	КонецЦикла;
	...
	Для Каждого ТекущееПомещение Из ПоляАдреса.Помещения Цикл
		Результат.Apartments.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
			ТекущееПомещение.ТипПомещения, ТекущееПомещение.Номер));
	КонецЦикла;
...
Функция АдресПоИдентификатору(ИдентификаторАдреса, ДополнительнаяИнформацияАдреса = Неопределено) Экспорт
	...
	Муниципальный = ?(ДополнительнаяИнформацияАдреса.Свойство("Муниципальный"), Булево(
		ДополнительнаяИнформацияАдреса.Муниципальный), Ложь);
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерДома") И ЗначениеЗаполнено(
		ДополнительнаяИнформацияАдреса.НомерДома) Тогда
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерСтроения") И ЗначениеЗаполнено(
		ДополнительнаяИнформацияАдреса.НомерСтроения) Тогда

		ОписаниеСтроения = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.НомерСтроения, "Корпус");
		Строение = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(ОписаниеСтроения.Тип,
			ОписаниеСтроения.Номер);
		СписокСтроенийАдреса = Адрес.buildings; // Массив
		СписокСтроенийАдреса.Добавить(Строение);

	КонецЕсли;

	Если ДополнительнаяИнформацияАдреса.Свойство("НомерОфиса") И ЗначениеЗаполнено(
		ДополнительнаяИнформацияАдреса.НомерОфиса) Тогда

		ОписаниеПомещения = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.НомерОфиса, "Офис");
		Помещение = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(ОписаниеПомещения.Тип,
			ОписаниеПомещения.Номер);
		СписокПомещенийАдреса = Адрес.apartments; // Массив
		СписокПомещенийАдреса.Добавить(Помещение);

	КонецЕсли;

	Если ДополнительнаяИнформацияАдреса.Свойство("АбонентскийЯщик") И ЗначениеЗаполнено(
		ДополнительнаяИнформацияАдреса.АбонентскийЯщик) Тогда

		ОписаниеАбонентскийЯщик = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.АбонентскийЯщик, "А/Я");
		АбонентскийЯщик = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
			ОписаниеАбонентскийЯщик.Тип, ОписаниеАбонентскийЯщик.Номер);
		СписокПомещенийАдреса = Адрес.apartments; // Массив
		СписокПомещенийАдреса.Добавить(АбонентскийЯщик);
	КонецЕсли;

	Если ДополнительнаяИнформацияАдреса.Свойство("ПочтовыйИндекс") И ЗначениеЗаполнено(
		ДополнительнаяИнформацияАдреса.ПочтовыйИндекс) Тогда
		Адрес.ZIPcode = ДополнительнаяИнформацияАдреса.ПочтовыйИндекс;
	КонецЕсли;

	Если ДополнительнаяИнформацияАдреса.Свойство("ДополнительнаяИнформация") И ЗначениеЗаполнено(
		ДополнительнаяИнформацияАдреса.ДополнительнаяИнформация) Тогда
	...
	Возврат УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзJSONВXML(Адрес,
		Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Функция ПредыдущийФорматКонтактнойИнформацииXML(Знач Данные, Знач СокращенныйСоставПолей = Ложь) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Данные) Тогда
		СтарыйФормат = Обработки.РасширенныйВводКонтактнойИнформации.КонтактнаяИнформацияВСтаруюСтруктуру(Данные,
			СокращенныйСоставПолей);
...
Функция ПредыдущаяСтруктураКонтактнойИнформацииXML(Знач Данные, Знач ВидКонтактнойИнформации = Неопределено) Экспорт
	...
	Если ВидКонтактнойИнформации <> Неопределено И ((ТипЗнч(ВидКонтактнойИнформации) = Тип("Структура")
		И ВидКонтактнойИнформации.Свойство("Тип")) Или ТипЗнч(ВидКонтактнойИнформации) = Тип(
		"СправочникСсылка.ВидыКонтактнойИнформации")) Тогда
	...
	СтруктураЗначенийПолей = УправлениеКонтактнойИнформациейСлужебный.СтруктураЗначенийПолей(Данные,
...
Функция АдресВФорматеКЛАДР(Знач Данные) Экспорт
	...
	Если Результат.Свойство("Страна") И СтрСравнить(Результат.Страна,
		РаботаСАдресамиКлиентСервер.ОсновнаяСтрана().Наименование) = 0 Тогда
...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Причины                      = НСтр(
		"ru = 'В октябре 2019 г. в федеральной информационной адресной системе тип здания ""Литер"" был переименован в ""Литера"".'");
	...
	Проверка.Причины                      = НСтр(
		"ru = 'Адрес контактной информации введен некорректно, устарел, был некорректно загружен из других источников или еще не внесен в Федеральную информационную адресную систему.'");
	...
	Проверка.Рекомендация                 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Рекомендация,
		"https://fias.nalog.ru/ExtendedSearch", НавигационнаяСсылкаЗагрузкиАдресногоКлассификатора);
...
Функция ДополнительныеПравилаПреобразования() Экспорт
	...
	Для Каждого ДополнительныйАдресныйЭлемент Из ТипыОбъектовАдресацииАдресаРФ() Цикл
		КодыДополнительныхАдресныхЭлементов.ДобавитьСтроку("<data:item data:title="""
			+ ДополнительныйАдресныйЭлемент.Наименование + """>" + ДополнительныйАдресныйЭлемент.Код + "</data:item>");
		КодыДополнительныхАдресныхЭлементов.ДобавитьСтроку("<data:item data:title=""" + НРег(
			ДополнительныйАдресныйЭлемент.Наименование) + """>" + ДополнительныйАдресныйЭлемент.Код + "</data:item>");
	КонецЦикла;
	...
	РасширенныйТекстПреобразования = "
									 ...
									 |          <xsl:attribute name=""ТипАдрЭл"">" + КодСериализацииПочтовогоИндекса()
		+ "</xsl:attribute>
...
Процедура ЗаполнитьМаскиНомераТелефона(СписокВыбора) Экспорт
	...
	МаскаНомера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='999999999, <span style=""color: %1""> (Пример: 9001234567)</span>'"), "ПоясняющийТекст");
...
Функция ТипыОбъектовАдресацииАдресаРФ() Экспорт
	...
	Результат.Добавить(СтрокаОбъектаАдресации("1039", РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка(), 1,
		9, 9));
	...
	Для Каждого ЭлементСписка Из ЗначенияПолей Цикл
		...
		ИначеЕсли ИмяПоля = "DISTRICT" Тогда
			Если НациональныйАдрес.СвРайМО = Неопределено Тогда
				НациональныйАдрес.СвРайМО = ФабрикаXDTO.Создать(НациональныйАдрес.Тип().Свойства.Получить(
					"СвРайМО").Тип);
			КонецЕсли
			;
...
//
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Знач Тип = Неопределено, Представление = "",
	ОбновлятьИдентификаторы = Истина) Экспорт
	...
	Если ТипЗнч(КонтактнаяИнформация) = Тип("Строка") Тогда
		...
		Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(КонтактнаяИнформация) Тогда
			...
			XDTOКонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзXML(
				КонтактнаяИнформация, Тип, РезультатПреобразования, Представление);
	...
	Если Не Формат9Запятых Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
			...
			Страна = ?(ПустаяСтрока(Состав.Страна), ОсновнаяСтрана, Справочники.СтраныМира.НайтиПоНаименованию(
				Состав.Страна, Истина));
			НаименованиеСтраны = Страна.Наименование;
			Результат.Вставить("CountryCode", СокрЛП(Страна.Код));

			АдресРФ = Состав.Состав;

		ИначеЕсли XDTOТип = ФабрикаXDTO.Тип(УправлениеКонтактнойИнформациейСлужебный.ПространствоИмен(), "НомерТелефона")
		...
		ИначеЕсли ТипЗнч(АдресРФ) = Тип("Строка") Тогда
			...
			Если СтрЧислоВхождений(АдресРФ, ",") = 9 Тогда
				...
				НаименованиеСокращение = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(
					ЧастиАдреса[2]);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(АдресРФ.Адрес_по_документу) Или (ПустаяСтрока(АдресРФ.СубъектРФ) И ЗначениеЗаполнено(
				Результат.Value)) Тогда
			...
			ЗначениеДополнительныхЭлементов = Обработки.РасширенныйВводКонтактнойИнформации.ЗначениеДополнительныхЭлементов(
				АдресРФ);
			Если ЗначениеЗаполнено(ЗначениеДополнительныхЭлементов.ДополнительныйЭлемент) Тогда
				ДополнительныйЭлемент = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(
					ЗначениеДополнительныхЭлементов.ДополнительныйЭлемент);
			...
			Если ЗначениеЗаполнено(ЗначениеДополнительныхЭлементов.ПодчиненныйЭлемент) Тогда
				ПодчиненныйЭлемент = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(
					ЗначениеДополнительныхЭлементов.ПодчиненныйЭлемент);
			...
			Для Каждого Здание Из ЗданияИПомещения.Здания Цикл
				...
				Иначе
					Результат.Buildings.Добавить(
						УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(Здание.Тип,
						Здание.Значение));
				КонецЕсли;
			КонецЦикла;
			...
			Для Каждого Помещение Из ЗданияИПомещения.Помещения Цикл
				Результат.Apartments.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
					Помещение.Тип, Помещение.Значение));
			КонецЦикла;
...
Функция ПреобразоватьКлючЗначениеВСтруктуру(Знач ЗначенияПолей, Знач Представление)
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		ИначеЕсли ИмяПоля = "КОДРЕГИОНА" Тогда
			...
			НаименованиеИТип = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(РегионКода(
	...
	ИначеЕсли ЗначениеЗаполнено(ПолеПредставления) Тогда
		Результат.value = ПолеПредставления;
	Иначе
		Результат.value = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(Результат, Ложь, ТипАдреса);
	КонецЕсли;
	Возврат Результат;
...
Функция ПодготовитьАдресДляВвода(Данные) Экспорт
	...
	Если Данные.Свойство("id") И ПустаяСтрока(Данные.id) И ОбщегоНазначения.ПодсистемаСуществует(
		"СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		МодульАдресныйКлассификаторСлужебный.УстановитьИдентификаторыАдреса(Данные);
	КонецЕсли;

	НаселенныйПунктДетально = РаботаСАдресамиКлиентСервер.ОписаниеНовойКонтактнойИнформации(ПредопределенноеЗначение(
		"Перечисление.ТипыКонтактнойИнформации.Адрес"));
	...
	Для Каждого ЭлементАдреса Из НаселенныйПунктДетально Цикл
		...
		Если СтрЗаканчиваетсяНа(ЭлементАдреса.Ключ, "Id") И ТипЗнч(ЭлементАдреса.Значение) = Тип("Строка") И СтрДлина(
			ЭлементАдреса.Значение) = 36 Тогда
...
Функция СведенияОбАдресахВВидеСтруктуры(Адреса, ДополнительныеПараметры)
	...
	Для ИндексАдреса = 0 По Адреса.ВГраница() Цикл
		...
		Если Не (ТипЗнч(АдресСтрокой) = Тип("Структура") И АдресСтрокой.Свойство("Value")) Тогда
			...
			Если Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(АдресСтрокой) Тогда
				АдресСтрокой = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(АдресСтрокой,
					Перечисления.ТипыКонтактнойИнформации.Адрес);
			КонецЕсли;

		КонецЕсли;

		АдресИзJSON = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(АдресСтрокой,
			Перечисления.ТипыКонтактнойИнформации.Адрес);
	...
	Для Каждого СтрокаАдреса Из ТаблицаАдресов Цикл
		...
		ЭтоНациональныйАдрес = ?(СтрСравнить(АдресИзJSON.Country,
			РаботаСАдресамиКлиентСервер.ОсновнаяСтрана().Наименование) = 0, Истина, Ложь);
		...
		Для Каждого ЧастьАдреса Из СоответствиеУровней Цикл
			...
			Иначе
				...
				НаименованиеУровня = ?(Параметры.НаименованиеВключаетСокращение, СокрЛП(АдресИзJSON[ЧастьАдреса.Ключ]
					+ " " + ТипКраткий), АдресИзJSON[ЧастьАдреса.Ключ]);
			КонецЕсли;
			...
			УровеньИдентификатора = ?(ЧастьАдреса.Значение.Уровень < 10, ЧастьАдреса.Значение.Уровень * 10,
				ЧастьАдреса.Значение.Уровень);
			Если ЗначениеЗаполнено(ИдентификаторТекущегоУровня) И МаксимальныйУровеньИдентификатора
				< УровеньИдентификатора Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
			...
			Для Каждого ЧастьАдреса Из СоответствиеУровней Цикл
				...
				АдресПоПолям[ЧастьАдреса.Значение.Имя
					+ "ТипПолный"] = СоответствиеСокращений[ЧастьАдреса.Значение.Уровень];

				Если Параметры.ПолноеНаименованиеСокращений = Истина Тогда
					АдресПоПолям[ЧастьАдреса.Значение.Имя
						+ "Сокращение"] = СоответствиеСокращений[ЧастьАдреса.Значение.Уровень];
				КонецЕсли;
		...
		Если ЭтоНациональныйАдрес Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
				МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
				Если Параметры.ВозвращатьКоды Тогда
					ДополнительныеКоды = МодульАдресныйКлассификаторСлужебный.КодыАдресаИКодыКЛАДР(АдресИзJSON,
						ИдентификаторАдресногоОбъекта);
		...
		Если Не Параметры.БезПредставлений Тогда
			...
			АдресПоПолям.Представление = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(АдресИзJSON,
				Параметры.ВключатьСтрануВПредставление,
		...
		АдресПоПолям.Регион = СтрЗаменить(АдресПоПолям.Регион, "Кемеровская область - Кузбасс обл",
...
Функция КодМуниципальногоРайона(Знач ТипМуниципальногоРайона, ОКТМО, ЭтоГородФедеральногоЗначения)
	...
	Если СтрСравнить(ТипМуниципальногоРайона, "м.р-н") = 0 Тогда
		КодМуниципальногоРайона = "1";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, "г.о.") = 0 Или СтрСравнить(ТипМуниципальногоРайона, "ф.т.") = 0 Тогда
		КодМуниципальногоРайона = "2";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, "вн.тер.") = 0 Или СтрСравнить(ТипМуниципальногоРайона,
		"вн.тер. г.") = 0 Тогда
		КодМуниципальногоРайона = "3";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, "мун. окр.") = 0 Или СтрСравнить(ТипМуниципальногоРайона, "м.о.")
		= 0 Тогда
		КодМуниципальногоРайона = "4";
	КонецЕсли;
	// АПК: 1297-вкл
...
Функция СтруктураАдресаВСтруктуруJSON(Знач КонтактнаяИнформация)
	...
	ОписаниеКонтактнойИнформации = РаботаСАдресамиКлиентСервер.ОписаниеНовойКонтактнойИнформации(
		Перечисления.ТипыКонтактнойИнформации.Адрес);
	...
	Если КонтактнаяИнформация.Свойство("ТипАдреса") И РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(
		КонтактнаяИнформация.ТипАдреса) Тогда
		ОписаниеКонтактнойИнформации.AddressType = РаботаСАдресамиКлиентСервер.МуниципальныйАдрес();
		Если КонтактнаяИнформация.Свойство("МуниципальноеПредставление") И ЗначениеЗаполнено(
			КонтактнаяИнформация.МуниципальноеПредставление) Тогда
	...
	Если КонтактнаяИнформация.Свойство("Здание") И ТипЗнч(КонтактнаяИнформация.Здание) = Тип("Структура")
		И КонтактнаяИнформация.Здание.Свойство("Номер") Тогда

		ОписаниеКонтактнойИнформации.HouseNumber = ?(КонтактнаяИнформация.Здание.Свойство("Номер"),
			КонтактнаяИнформация.Здание.Номер, "");
		ОписаниеКонтактнойИнформации.HouseType = ?(КонтактнаяИнформация.Здание.Свойство("ТипЗдания"),
			КонтактнаяИнформация.Здание.ТипЗдания, "Дом"); // Тип владения не локализуется
	...
	Если КонтактнаяИнформация.Свойство("Корпус") И ЗначениеЗаполнено(КонтактнаяИнформация.Корпус) Тогда
		...
		ОписаниеКонтактнойИнформации.buildings.Добавить(
			УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(ТипКорпуса,
			КонтактнаяИнформация.Корпус));
	...
	Если КонтактнаяИнформация.Свойство("Квартира") И ЗначениеЗаполнено(КонтактнаяИнформация.Квартира) Тогда
		...
		ОписаниеКонтактнойИнформации.apartments.Добавить(
			УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(ТипКвартиры,
			КонтактнаяИнформация.Квартира));
	...
	Если ПустаяСтрока(ОписаниеКонтактнойИнформации.value) Тогда
		ОписаниеКонтактнойИнформации.value = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(
			ОписаниеКонтактнойИнформации, Ложь, ОписаниеКонтактнойИнформации.AddressType);
	КонецЕсли;
...
Функция СтрокаПолей(СтруктураЗначенийПолей, БезПустыхПолей = Истина) Экспорт
	...
	Для Каждого ЗначениеПоля Из СтруктураЗначенийПолей Цикл
		...
		Результат = Результат + ?(Результат = "", "", Символы.ПС) + ЗначениеПоля.Ключ + "=" + СтрЗаменить(
			ЗначениеПоля.Значение, Символы.ПС, Символы.ПС + Символы.Таб);
	КонецЦикла;
...
Процедура ДополнитьНастройкиПроверкиАдреса(ПараметрыВида, ВидКонтактнойИнформации) Экспорт
	...
	НастройкиПроверки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидКонтактнойИнформации,
		"ПроверятьПоФИАС, УказыватьОКТМО");
...
Функция ЗаполнитьДанныеВыбораАвтоподбораПоСтранам(Знач Параметры) Экспорт
		...
		Если ЗначениеЗаполнено(ШаблонОтбора) Тогда
			ШаблонЗапроса = ШаблонЗапроса + "
											|	И ((" + СтрЗаменить(ШаблонОтбора, "%1", "Классификатор") + ")
																											|	ИЛИ ("
				+ СтрЗаменить(ШаблонОтбора, "%1", "СтраныМира") + "))
																  |
																  |";
		КонецЕсли;
	...
	Для Каждого ДанныеПоля Из ИменаПолей.СписокПолей Цикл
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Представление", СтрЗаменить(ДанныеПоля.ШаблонПредставления, "%1",
			"Классификатор"));
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДругоеПредставление", СтрЗаменить(ДанныеПоля.ШаблонПредставления,
			"%1", "СтраныМира"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			РезультатВыбора = Новый Структура("Код, Наименование", Выборка.Код, Выборка.Наименование);

			ЭлементВыбора = Новый Структура("Значение, ПометкаУдаления, Предупреждение", РезультатВыбора,
				Выборка.ПометкаУдаления, Неопределено);
		КонецЕсли;

		ВыделенныйТекст = Новый ФорматированнаяСтрока(Лев(Выборка.Представление, ДлинаСтрокиПоиска),
...
Функция ПоляПоиска(ПоляОтбора)
	...
	Для Индекс = 0 По ГраницаПолей Цикл
		...
		Если ОстальныеПоля.Количество() > 0 Тогда
			ШаблонПредставления = ШаблонПредставления + " + "" ("" + " + СтрСоединить(ОстальныеПоля, " + "", "" + ")
				+ " + "")""";
		КонецЕсли;
...
Процедура ПроверитьАдресаНаТипЗданияЛитер(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Если Проверки[РезультатЗапроса.Вид] = Неопределено Тогда
			...
			ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки(НСтр(
				"ru='Устаревший тип здания в адресах контактной информации'", ОбщегоНазначения.КодОсновногоЯзыка()),
				РезультатЗапроса.Вид);
...
Процедура ИсправитьТипЗданияЛитерВАдресахВФоне(Знач ПараметрыПроверки, АдресХранилища = Неопределено) Экспорт
	...
	ВключатьСтрануВПредставление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидКонтактнойИнформации,
	...
	Пока ПроблемныеОбъекты.Количество() > 0 Цикл
		...
		Для Каждого ОписаниеОбъекта Из ПроблемныеОбъекты Цикл
			Если ИсправитьТипЗданияЛитерВАдресахОбъекта(ОписаниеОбъекта, ВидКонтактнойИнформации,
				ВключатьСтрануВПредставление) Тогда
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтрольВеденияУчета") Тогда
					МодульКонтрольВеденияУчета = ОбщегоНазначения.ОбщийМодуль("КонтрольВеденияУчета");
					МодульКонтрольВеденияУчета.ОчиститьРезультатПоВидуПроверки(ОписаниеОбъекта.ПроблемныйОбъект,
						ВидПроверки);
				КонецЕсли;
...
Функция ИсправитьТипЗданияЛитерВАдресахОбъекта(ОписаниеОбъекта, Знач ВидКонтактнойИнформации,
	Знач ВключатьСтрануВПредставление)

	КоличествоИсправленныхАдресов = 0;
	ОбъектБылИсправлен = Ложь;

	СвойстваВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидКонтактнойИнформации,
		"ВключатьСтрануВПредставление, ХранитьИсториюИзменений, Ссылка");
	СсылкаНаОбъект = ОписаниеОбъекта.ПроблемныйОбъект;
	ЕстьДействуетС = СсылкаНаОбъект.Метаданные().ТабличныеЧасти.КонтактнаяИнформация.Реквизиты.Найти("ДействуетС")
		<> Неопределено;
			...
			Для Каждого СтрокаКонтактнойИнформации Из НайденныеСтроки Цикл
				...
				Если ПустаяСтрока(СтрокаКонтактнойИнформации.Значение) Тогда
					Если ПустаяСтрока(СтрокаКонтактнойИнформации.ЗначенияПолей) Тогда
						Продолжить;
					Иначе
						АдресВJSON = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(
							СтрокаКонтактнойИнформации.ЗначенияПолей, ВидКонтактнойИнформации);
					КонецЕсли;
				Иначе
					АдресВJSON =  СтрокаКонтактнойИнформации.Значение;
				КонецЕсли;

				АдресПоПолям = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(АдресВJSON,
					Перечисления.ТипыКонтактнойИнформации.Адрес);
				...
				Если АдресБылИзменен Тогда
					...
					ЗаменитьАдресНаАктуальный(ОбъектКИ, СтрокаКонтактнойИнформации, АдресПоПолям, СвойстваВида,
						ЕстьДействуетС);
...
Процедура ИсправитьУстаревшиеАдресаВФоне(Знач ПараметрыПроверки, АдресХранилища = Неопределено) Экспорт
	...
	Пока ПроблемныеОбъекты.Количество() > 0 Цикл
		...
		ПоследнийПроблемныйОбъект = ПроблемныеОбъекты.Получить(ПроблемныеОбъекты.Количество() - 1).ПроблемныйОбъект;
		ВсегоОбъектов = ВсегоОбъектов + ПроблемныеОбъекты.Количество();
		ИсправленоОбъектов = ИсправленоОбъектов + ИсправитьПорциюУстаревшихАдресовКонтактнойИнформации(
			ПроблемныеОбъекты, Проверка, ВидПроверки);
		ПроблемныеОбъекты = МодульКонтрольВеденияУчета.ПроблемныеОбъекты(Проверка, ПоследнийПроблемныйОбъект);
...
Процедура ВыявитьУстаревшиеАдреса(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока СтрокаСАдресом.Следующий() Цикл
		...
		Если Проверки[СтрокаСАдресом.Вид] = Неопределено Тогда
			...
			ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки(НСтр(
				"ru='Адреса контактной информации'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаСАдресом.Вид);
			МодульКонтрольВеденияУчета.ОчиститьРезультатыПредыдущихПроверок(Проверка, ПараметрыВыполненияПроверки);
			ВидПроверки = МодульКонтрольВеденияУчета.ВидПроверки(ПараметрыВыполненияПроверки);
		...
		Если ПустаяСтрока(АдресВJSON) Тогда
			АдресВJSON = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(СтрокаСАдресом.ЗначенияПолей,
				СтрокаСАдресом.Вид);
		КонецЕсли;
...
Функция ИсправитьПорциюУстаревшихАдресовКонтактнойИнформации(Знач ПроблемныеОбъекты, Проверка, ВидПроверки)
	...
	СвойстваВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Вид,
		"ВключатьСтрануВПредставление, ХранитьИсториюИзменений, Ссылка");
				...
				Для Каждого АдресОбъекта Из АдресаОбъекта Цикл
					Если СтрСравнить(АдресОбъекта.Представление, СтрокаВидаКИ.Представление) = 0 Тогда
						...
						Если СведенияОбАдресе.Варианты.Количество() > 0 Тогда
							...
							ЗаменитьАдресНаАктуальный(ОбъектКИ, СтрокаВидаКИ, СведенияОбАдресе.Варианты[0],
								СвойстваВида, ЕстьДействуетС);
...
Процедура ЗаменитьАдресНаАктуальный(ОбъектКИ, СтрокаВидаКИ, АдресПоПолям, СвойстваВида, ЕстьДействуетС)
	...
	НовоеПредставление = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(АдресПоПолям,
		СвойстваВида.ВключатьСтрануВПредставление);
	...
	ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru='Адрес ""%1"" устарел и был переименован в ""%2""'"), СтрокаВидаКИ.Представление, НовоеПредставление);
	...
	Если СвойстваВида.ХранитьИсториюИзменений И ЕстьДействуетС Тогда
		УправлениеКонтактнойИнформацией.ДобавитьКонтактнуюИнформацию(ОбъектКИ, НовыйАдресВJSON, СвойстваВида.Ссылка,
...
Процедура ПроверитьАдресаНаСоответствиеАдресномуКлассификатору(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока СтрокаСАдресом.Следующий() Цикл
		...
		Если Проверки[СтрокаСАдресом.ИмяОбъекта] = Неопределено Тогда
			...
			ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки(НСтр(
				"ru = 'Адреса контактной информации'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаСАдресом.ИмяОбъекта);
			МодульКонтрольВеденияУчета.ОчиститьРезультатыПредыдущихПроверок(Проверка, ПараметрыВыполненияПроверки);
			ВидПроверки = МодульКонтрольВеденияУчета.ВидПроверки(ПараметрыВыполненияПроверки);
...
Процедура ОбработкаАдресаПослеПроверки(ПроверенныеАдреса, СоответствиеАдресаИСсылки, Проверка, ПараметрыПроверки,
	Проверки)
	...
	Для Каждого КлючЗначение Из ПроверенныеАдреса Цикл
		...
		Если ПроверенныйАдрес.Ошибки.Количество() > 0 Тогда
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 ""%2"" не соответствует адресному классификатору, по причине: %3'"), Строка(
				ОписаниеАдреса.Вид), ОписаниеАдреса.Представление, ПроверенныйАдрес.Ошибки[0].Текст);
		Иначе
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 ""%2"" не соответствует адресному классификатору'"), Строка(ОписаниеАдреса.Вид),
				ОписаниеАдреса.Представление);
		КонецЕсли;
	...
	Для Каждого ОбъектДляПроверки Из ОбъектыДляПроверки Цикл
		...
		Пока ПереборОбъектов Цикл
			...
			Пока СтрокаСАдресом.Следующий() Цикл
				...
				Если ПустаяСтрока(АдресВJSON) Тогда
					АдресВJSON = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(
						СтрокаСАдресом.ЗначенияПолей, СтрокаСАдресом.Вид);
				КонецЕсли;
...
Процедура ИсправитьУстаревшиеАдресаПослеПроверкиАвто(Адреса, СоответствиеАдресаИСсылки)
	...
	Для Каждого Адрес Из Адреса Цикл
		...
		Если РезультатПроверки.Варианты.Количество() > 0 Тогда
			...
			Если ГруппировкаПоОбъекту[НовыйАдрес.Ссылка] = Неопределено Тогда
				ГруппировкаПоОбъекту.Вставить(НовыйАдрес.Ссылка, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
					ДанныеНовогоАдреса));
			Иначе
	...
	Для Каждого СтрокаТаблицы Из ГруппировкаПоОбъекту Цикл
		...
		Попытка
			...
			ЕстьДействуетС = СсылкаНаОбъект.Метаданные().ТабличныеЧасти.КонтактнаяИнформация.Реквизиты.Найти(
				"ДействуетС") <> Неопределено;
			...
			Для Каждого СтрокаВидаКИ Из СтрокаТаблицы.Значение Цикл // СправочникСсылка.ВидыКонтактнойИнформации
				...
				СвойстваВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаВидаКИ.Вид,
					"ВключатьСтрануВПредставление, ХранитьИсториюИзменений, Ссылка");
				...
				Для Каждого АдресОбъекта Из АдресаОбъекта Цикл
					Если СтрСравнить(АдресОбъекта.Представление, СтрокаВидаКИ.Представление) = 0 Тогда
						...
						ЗаменитьАдресНаАктуальный(ОбъектКИ, АдресОбъекта, СтрокаВидаКИ.Варианты[0], СвойстваВида,
							ЕстьДействуетС);
						ТребуетсяЗаписьОбъекта = Истина;
