...
Функция ПолеИспользуетсяВРодительскихГруппировкахОтчета(Родитель, Поле, Содержится = Ложь) Экспорт
	...
	Если (ТипЗнч(Родитель) = Тип("ГруппировкаКомпоновкиДанных") Или ТипЗнч(Родитель) = Тип(
...
Функция ГруппировкаФильтра(Настройки, СвойстваЗаголовка, ЭтоГруппировка = Ложь) Экспорт
	...
	Если ТипЗнч(Раздел) = Тип("ТаблицаКомпоновкиДанных") Тогда
		...
		Иначе
			...
			Группировка = ИспользуемыеНастройки.ПолучитьОбъектПоИдентификатору(
...
Процедура НайтиПолеОтчета(Поля, Поле, НайденноеПоле, ДополнительныеПараметры)
	...
	Для Каждого Элемент Из Поля.Элементы Цикл
		...
		ПроверкаИспользованияПоляПройденаУспешно = Не ПроверятьИспользование Или ПроверятьИспользование
			И Элемент.Использование;

		ПроверкаЗаголовкаПройденаУспешно = Не ЗначениеЗаполнено(ЗаголовокПоля) Или Элемент.Заголовок = ЗаголовокПоля
			Или ОписаниеПоля <> Неопределено И ОписаниеПоля.Заголовок = ЗаголовокПоля;

		Если ТипЭлемента <> Тип("АвтоВыбранноеПолеКомпоновкиДанных") И Элемент.Поле = Поле И ПроверкаИспользованияПоляПройденаУспешно
