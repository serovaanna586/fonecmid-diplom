...
Функция ЕстьРоль(Знач Роль, Знач СсылкаНаОбъект = Неопределено, Знач Пользователь = Неопределено) Экспорт
	...
	Для Каждого Строка Из НаборыЗначенийДоступа Цикл
		...
		Если ВРег(Строка.ВидДоступа) = ВРег("ПравоЧтения") Или ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
			...
			Если ТипЗнч(Строка.ЗначениеДоступа) <> Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
				Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Строка.ЗначениеДоступа)) Тогда
					Строка.ЗначениеДоступа = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(
			...
			Если ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка в функции %1 модуля %2.
						 ...
						 |может быть зависимость только от права Чтения.'"), "ЕстьРоль", "УправлениеДоступом",
					"ПравоИзменения", Строка.ЗначениеДоступа, "Чтения");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено Или Строка.ВидДоступа = "НастройкиПрав" Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в функции %1 модуля %2.
					 ...
					 |""%4"", ""%5"", если они используются.'"), "ЕстьРоль", "УправлениеДоступом", Строка.ВидДоступа,
				"ПравоЧтения", "ПравоИзменения");
			ВызватьИсключение ТекстОшибки;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в функции %1 модуля %2.
					 |Набор значений доступа содержит неизвестный вид доступа ""%1"".'"), "ЕстьРоль",
				"УправлениеДоступом", Строка.ВидДоступа);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция ЕстьПраво(Право, СсылкаНаОбъект, Знач Пользователь = Неопределено) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нет описания возможных прав для таблицы ""%1""'"), СсылкаНаОбъект.Метаданные().ПолноеИмя());
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если ОписаниеПрава = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нет описания права ""%1"" для таблицы ""%2""'"), Право, СсылкаНаОбъект.Метаданные().ПолноеИмя());
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ПриСозданииФормыЗначенияДоступа(Форма, ДополнительныеПараметры = Неопределено,
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1
			 |общего модуля %2.'"), "ПриСозданииФормыЗначенияДоступа", "УправлениеДоступом");
		...
		Исключение
			...
			ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр %1 не указан, а автоматическое заполнение
					 ...
					 |%3'"), "СозданиеНового", "Объект.Ссылка", КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			ВызватьИсключение ТекстОшибки;
		КонецПопытки;
	КонецЕсли;
	...
	Если ТипЗнч(ТипЗначения) <> Тип("Тип") Тогда
		...
		Исключение
			...
			ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр %1 не указан, а автоматическое заполнение
					 |из реквизита формы ""%2"" недоступно по причине:
					 |%3'"), "ТипЗначения", "Объект.Ссылка", КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			ВызватьИсключение ТекстОшибки;
		КонецПопытки;
	Иначе
		...
		Исключение
			...
			ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр Реквизит не указан, а автоматическое заполнение
					 |из реквизита формы ""%2"" недоступно по причине:
					 |%3'"), "Реквизит", "Объект.ГруппаДоступа", КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			ВызватьИсключение ТекстОшибки;
		КонецПопытки;
	Иначе
			...
			Исключение
				...
				ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось получить значение реквизита формы ""%1"",
						 ...
						 |%3'"), Реквизит, "Реквизит", КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				ВызватьИсключение ТекстОшибки;
			КонецПопытки;
		Иначе
			Попытка
	...
	Если ТипЗнч(ГруппаЗначенийДоступа) <> СвойстваГрупп.Тип Тогда
		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для значений доступа типа ""%1""
				 ...
				 |доступа у реквизита %5.'"), Строка(ТипЗначенияДоступа), Строка(СвойстваГрупп.ВидДоступа), Строка(
			СвойстваГрупп.Тип), Строка(ТипЗнч(ГруппаЗначенийДоступа)), "ГруппаДоступа");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	ГруппыЗначенийДляИзменения = ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа(ТипЗначенияДоступа);

	Если ГруппыЗначенийДляИзменения.Количество() = 0 И СозданиеНового Тогда

		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для добавления требуются разрешенные ""%1"".'"), Метаданные.НайтиПоТипу(
			СвойстваГрупп.Тип).Представление());
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;

	Если ГруппыЗначенийДляИзменения.Количество() = 0 Или Не СозданиеНового И ГруппыЗначенийДляИзменения.Найти(
		ГруппаЗначенийДоступа) = Неопределено Тогда

		Форма.ТолькоПросмотр = Истина;
		Возврат;
	КонецЕсли;

	Если СозданиеНового И Не ЗначениеЗаполнено(ГруппаЗначенийДоступа) И ГруппыЗначенийДляИзменения.Количество() = 1 Тогда
	...
	НовыйПараметрВыбора = Новый ПараметрВыбора("Отбор.Ссылка", Новый ФиксированныйМассив(ГруппыЗначенийДляИзменения));
...
Функция ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа(ТипЗначенийДоступа, ВозвращатьВсе = Ложь) Экспорт
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка в процедуре %1
			 |общего модуля %2.'"), "ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа", "УправлениеДоступом");
	...
	Запрос.УстановитьПараметр("ИдентификаторЗначенийДоступа", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
		ТипЗначенийДоступа));
...
Процедура НастроитьОтборыДинамическогоСписка(Список, ОписаниеОтборов) Экспорт
	...
	Если ТипЗнч(Список) <> Тип("ДинамическийСписок") Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			"НастроитьОтборыДинамическогоСписка", "УправлениеДоступом", "Список", Строка(Список));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			"НастроитьОтборыДинамическогоСписка", "УправлениеДоступом");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если Не Список.ПроизвольныйЗапрос Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка вызова процедуры %1 общего модуля %2.
				 |У переданного динамического списка не установлен флажок %3.'"), "НастроитьОтборыДинамическогоСписка",
			"УправлениеДоступом", "ПроизвольныйЗапрос");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Псевдоним) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка вызова процедуры %1 общего модуля %2.
					 ...
					 |""%1"".'"), "НастроитьОтборыДинамическогоСписка", "УправлениеДоступом", Список.ОсновнаяТаблица);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция РазрешенныеЗначенияДляДинамическогоСписка(Таблица, ТипЗначений, Пользователь = Неопределено, ВернутьВсе = Ложь) Экспорт
	...
	Если Не ОграничиватьДоступНаУровнеЗаписей() Или Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
		Возврат Неопределено;
	КонецЕсли;

	Возврат УправлениеДоступомСлужебный.РазрешенныеЗначенияДляДинамическогоСписка(Таблица, ТипЗначений, , Пользователь,
		ВернутьВсе);
...
Функция ПраваПоИдентификаторам(Идентификаторы = Неопределено) Экспорт
	...
	ОбъектыМетаданныхИдентификаторов = ОбщегоНазначения.ОбъектыМетаданныхПоИдентификаторам(Идентификаторы);
...
Функция ТаблицаНаборыЗначенийДоступа() Экспорт
	...
	Таблица.Колонки.Добавить("НомерНабора", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(4, 0,
...
Процедура ЗаполнитьНаборыЗначенийДоступа(Знач Объект, Таблица, Знач СсылкаНаПодчиненныйОбъект = Неопределено) Экспорт
	...
	Если Не НаборыЗаполняются Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверные параметры.
				 |Тип объекта ""%1""
				 |отсутствует в подписках на события %2, %3.'"), ТипЗначенияОбъект, "ЗаписатьНаборыЗначенийДоступа",
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 ""%2""
				 ...
				 |%3'"), ТипЗнч(СсылкаНаОбъект), Объект, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 ""%2""
				 |сформировал пустой набор значений доступа.'"), ТипЗнч(СсылкаНаОбъект), Объект);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
// 
Процедура ДобавитьДополнительныеТипыВидаДоступа(ВидДоступа, ТипЗначений, ТипГруппЗначений = Неопределено,
	НесколькоГруппЗначений = Ложь) Экспорт
	...
	Попытка
		...
		Если ПовторяющиесяНовыеИмена.Количество() > 0 Тогда
			...
			Для Каждого Строка Из ПовторяющиесяНовыеИмена Цикл
				ПовторяющиесяНовыеИменаПрав = ПовторяющиесяНовыеИменаПрав + ?(ЗначениеЗаполнено(
					ПовторяющиесяНовыеИменаПрав), "," + Символы.ПС, "") + Строка.НовоеИмя;
			КонецЦикла;
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в параметрах процедуры %1
					 ...
					 |%1.'"), "ЗаменитьПраваВНастройкахПравОбъектов", "УправлениеДоступом", ПовторяющиесяНовыеИменаПрав);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
//
Процедура ОбновитьРолиПользователей(Знач МассивПользователей = Неопределено,
	Знач ПарольПользователяСервиса = Неопределено) Экспорт
...
Процедура ОтключитьОбновлениеКлючейДоступа(Отключить, ПланироватьОбновление = Истина) Экспорт
	...
	Если Отключить И Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный вызов процедуры %1 общего модуля %2.
				 ...
				 |полноправного пользователя или в привилегированном режиме.'"), "ОтключитьОбновлениеКлючейДоступа",
			"УправлениеДоступом");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если ОтключениеОбновления.Стандартное = Стандартное И ОтключениеОбновления.Полное = Полное Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьОбработчикОбновленияДляВключенияУниверсальногоОграничения(Версия, Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ПодсистемаСуществует(
		...
		Для Каждого Строка Из Таблица Цикл
			Если Не ЗначениеЗаполнено(Строка.ДатаПоследнегоЗапуска) Или Строка.ДатаПоследнегоЗапуска > Граница Тогда
				ВключенРИБ = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция ЕстьОграничениеТаблицыПоВидуДоступа(Таблица, ВидДоступа, ВсеВидыДоступа) Экспорт
	...
	Возврат УправлениеДоступомСлужебный.ЕстьОграничениеТаблицыПоВидуДоступа(Таблица, ВидДоступа, ВсеВидыДоступа);
...
Процедура УточнитьНаборыЗначенийДоступа(СсылкаНаОбъект, Таблица)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если ТипыВладельцевНастроекПрав.Получить(ТипЗнч(Строка.ЗначениеДоступа)) <> Неопределено И Не ЗначениеЗаполнено(
			Строка.Уточнение) Тогда
		...
		Если Строка.ВидДоступа = "ПравоЧтения" Или Строка.ВидДоступа = "ПравоИзменения" Тогда
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено Или Строка.ВидДоступа = "НастройкиПрав" Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект ""%1"" сформировал набор значений доступа,
					 ...
					 |
					 |Указывать требуется только специальные виды доступа
					 |""%3"", ""%4"", если они используются.'"), ТипЗнч(СсылкаНаОбъект), Строка.ВидДоступа,
				"ПравоЧтения", "ПравоИзменения");
			ВызватьИсключение ТекстОшибки;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект ""%1"" сформировал набор значений доступа,
					 |содержащий неизвестный вид доступа ""%2"".'"), ТипЗнч(СсылкаНаОбъект), Строка.ВидДоступа);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура ЗаполнитьКодыТиповИТаблицуСтрокНабора(КодыТипов, КодыПеречислений, ТаблицаСтрокНабора)
	...
	ДлинаИдентификатораСтроки = 20 // Строка имени вида доступа

		+ ДлинаКодаТипа + 36 // Длина строкового представления уникального идентификатора (значения доступа).
	...
	ТаблицаСтрокНабора = Новый ТаблицаЗначений;
	ТаблицаСтрокНабора.Колонки.Добавить("ИдентификаторСтроки", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(ДлинаИдентификатораСтроки)));
	...
	ТаблицаСтрокНабора.Колонки.Добавить("Уточнение",
...
Процедура ЗаполнитьИдентификаторСтроки(Строка, КодыТипов, КодыПеречислений)
	...
	Строка.ИдентификаторСтроки = Строка.ВидДоступа + ";" + КодыТипов.Получить(ТипЗнч(Строка.ЗначениеДоступа)) + ";"
		+ ИдентификаторЗначенияДоступа + ";" + Строка.Уточнение.УникальныйИдентификатор() + ";";

КонецПроцедуры

Процедура ДобавитьНаборыКРезультатуСУпрощением(НаборыРезультата, ДобавляемыеНаборы, ТекущийНомерНабора, КодыТипов,
	КодыПеречислений, ТаблицаСтрокНабора)
...
Функция СвойстваГруппЗначенияДоступа(ТипЗначенияДоступа, ЗаголовокОшибки)
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для значений доступа типа ""%1""
				 |не используются группы значений доступа.'"), Строка(ТипЗначенияДоступа));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	СвойстваГрупп.Вставить("ПустаяСсылкаТипаЗначений", УправлениеДоступомСлужебный.ПустаяСсылкаОбъектаМетаданных(
		ТипЗначенияДоступа));
...
Процедура ВключитьОтключитьПрофильПользователя(Пользователь, Профиль, Включить)
	...
	Если Не УправлениеДоступомСлужебный.УпрощенныйИнтерфейсНастройкиПравДоступа() Тогда
		ТекстОшибки = НСтр("ru = 'Данная операция возможна только для упрощенного
	...
	Если ТипЗнч(Пользователь) <> Тип("СправочникСсылка.Пользователи") И ТипЗнч(Пользователь) <> Тип(
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 в %2.
				 |Ожидалось: %3; передано значение: %4 (тип %5).'"), ИмяПараметра, ИмяПроцедурыИлиФункции, ОжидаемыеТипы,
			?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, НСтр("ru = 'Неопределено'")), ТипЗнч(
			ЗначениеПараметра));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	// Проверка типов значения параметра Профиль.
	Если ТипЗнч(Профиль) <> Тип("СправочникСсылка.ПрофилиГруппДоступа") И ТипЗнч(Профиль) <> Тип("Строка") И ТипЗнч(
		Профиль) <> Тип("УникальныйИдентификатор") И Не (Не Включить И ТипЗнч(Профиль) = Тип("Неопределено")) Тогда
	...
	Если ТипЗнч(Профиль) = Тип("СправочникСсылка.ПрофилиГруппДоступа") Или ТипЗнч(Профиль) = Тип("Неопределено") Тогда
	...
	Если ТекущийПрофиль <> Неопределено Тогда
		СвойстваПрофиля = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийПрофиль, "Наименование, ВидыДоступа");
	КонецЕсли;
			...
			Если ПерсональнаяГруппаДоступа = ГруппаДоступаАдминистраторы() Тогда
				...
				Если Не ОбщегоНазначения.РазделениеВключено() Тогда
					...
					Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
						ТекстОшибки = НСтр("ru = 'Профиль Администратор должен быть хотя бы у одного пользователя,
