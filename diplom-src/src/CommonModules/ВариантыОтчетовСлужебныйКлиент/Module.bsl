...
Процедура ПоказатьКонтекстнуюНастройкуОтчета(Форма, Элемент, Область, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Область) <> Тип("ОбластьЯчеекТабличногоДокумента") Или Область.ТипОбласти
		<> ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Или Область.Расшифровка <> Неопределено И ТипЗнч(
		Область.Расшифровка) <> Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
	...
	Если ТипЗнч(СвойстваЗаголовка) <> Тип("Структура") Или ТипЗнч(СвойстваЗаголовка.Поле) <> Тип("ПолеКомпоновкиДанных")
	...
	ОткрытьФорму(
		"ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.НастройкаПоля", ПараметрыФормы, Форма,
...
Процедура ОбработкаДополнительнойРасшифровки(Форма, Данные, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Иначе
		...
		ДополнительноеМеню = КонтекстноеМенюОбластиДанных(СвойстваОбласти.СвойстваЗаголовка,
			Данные.ДоступныеВидыСравнения);
	КонецЕсли;

	ОбработкаРасшифровки = Новый ОбработкаРасшифровкиКомпоновкиДанных(Форма.ОтчетДанныеРасшифровки,
		Новый ИсточникДоступныхНастроекКомпоновкиДанных(Форма.НастройкиОтчета.АдресСхемы));
...
Процедура ПриАктивизацииРезультатаОтчета(Форма, Элемент) Экспорт
	...
	ЭтоРасшифровка = Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И ТипЗнч(
		Область.Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных");

	Элементы.КонтекстноеМенюОбластиЗаголовка.Видимость = (ТипЗнч(СвойстваЗаголовка) = Тип("Структура")) И ТипЗнч(
		СвойстваЗаголовка.Поле) = Тип("ПолеКомпоновкиДанных");
...
//
Процедура УстановитьВысотуСтроки(Форма, Команда, СвойстваЗаголовка = Неопределено, ВысотаСтроки = Неопределено,
	ПараметрыВысотыСтроки = Неопределено) Экспорт
...
//
Процедура УстановитьШиринуКолонки(Форма, Команда, СвойстваЗаголовка = Неопределено, ШиринаКолонки = Неопределено,
	ПараметрыШириныКолонки = Неопределено) Экспорт
...
Процедура ОформитьЕще(Форма, Команда, СвойстваЗаголовка = Неопределено, Значение = Неопределено) Экспорт
	...
	Если ТипЗнч(Группировка) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
		...
		ИдентификаторОформления = ИдентификаторЭлементаОформленияГруппировкиОтчета(Группировка.Настройки,
			СвойстваЗаголовка.Поле);
	ИначеЕсли СвойстваЗаголовка.КоличествоРазделов = 1 Тогда
		ИдентификаторГруппировки = Неопределено;
		ИдентификаторОформления = ИдентификаторЭлементаОформленияГруппировкиОтчета(ИспользуемыеНастройки,
			СвойстваЗаголовка.Поле);
	Иначе
		ИдентификаторГруппировки = ИспользуемыеНастройки.ПолучитьИдентификаторПоОбъекту(Группировка);
	...
	Обработчик = Новый ОписаниеОповещения("ПослеИзмененияЭлементаОформленияГруппировкиОтчета", ЭтотОбъект,
...
Процедура ПоказатьРасширеннуюНастройкуФильтра(Форма, СвойстваЗаголовка) Экспорт
	...
	Если ТекущаяОбласть.Верх <> СвойстваЗаголовка.Верх Или ТекущаяОбласть.Низ <> СвойстваЗаголовка.Низ
		Или ТекущаяОбласть.Лево <> СвойстваЗаголовка.Лево Или ТекущаяОбласть.Право <> СвойстваЗаголовка.Право Тогда
	...
	ОткрытьФорму(
		"ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ФильтроватьПоПолю", ПараметрыФормы, Форма,
		Форма.УникальныйИдентификатор);
...
Процедура СнятьФильтр(Форма, СвойстваЗаголовка, ДанныеРасшифровки = Неопределено) Экспорт
	...
	ЭтоГруппировка = (ДанныеРасшифровки <> Неопределено И ДанныеРасшифровки.Тип
		= ВариантыОтчетовСлужебныйКлиентСервер.ТипЭлементаРасшифровкиГруппировка());
...
Процедура ПереименоватьПоле(Форма, Команда, Заголовок = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(Заголовок) Тогда
		...
		ЗаголовокДиалога = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Заголовок поля: %1'"), ?(ОписаниеПоля = Неопределено, Строка(ПолеОтчета),
...
Процедура Сортировать(Форма, Команда, СвойстваЗаголовка = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Раздел) = Тип("ТаблицаКомпоновкиДанных") И Раздел.Строки.Количество() = 1 Тогда
			...
			ПолеГруппировки = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(ОсновнаяГруппировка.ПоляГруппировки,
				СвойстваЗаголовка.Поле);
...
Процедура ВставитьПоле(КомпоновщикНастроек, Настройки, ВыбранноеПоле, Действие, СвойстваЗаголовка, РолиПолей)
	...
	ВставитьПолеВПоляГруппировки(Группировка.ПоляГруппировки, ВыбранноеПоле, СвойстваЗаголовка.Поле, Действие,
		РолиПолей);
...
Процедура ВставитьПолеВПоляГруппировки(Поля, ВыбранноеПоле, ТекущееПоле, Действие, РолиПолей)
	...
	Если ТекущееПолеЯвляетсяПериодом И Не ВыбранноеПолеЯвляетсяПериодом Или Не ТекущееПолеЯвляетсяПериодом
		И ВыбранноеПолеЯвляетсяПериодом Тогда
...
Процедура ВставитьПолеВГруппировкуРаздела(Группировка, ВыбранноеПоле, ТекущееПоле, Действие, РолиПолей)
	...
	Если ТипЗнч(Группировка) <> Тип("ГруппировкаКомпоновкиДанных") И ТипЗнч(Группировка) <> Тип(
		"ГруппировкаТаблицыКомпоновкиДанных") Тогда
...
Процедура УстановитьВыводРеквизитовГруппировкиОтдельно(Группировка)
	...
	Если ТипЗнч(Группировка) <> Тип("ГруппировкаКомпоновкиДанных") И Не ТипЗнч(Группировка) <> Тип(
		"ГруппировкаТаблицыКомпоновкиДанных") Тогда
...
Функция ТипСортировкиРазделаДоступен(Команда, СвойстваЗаголовка)
	...
	Если СтрЗаканчиваетсяНа(ДействиеКоманды(Команда), "ПоВозрастанию") И Не СвойстваЗаголовка.СортироватьПоВозрастанию Тогда

		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонПредупреждения, СвойстваЗаголовка.Текст, НСтр("ru = 'возрастанию'"));

	ИначеЕсли СтрЗаканчиваетсяНа(ДействиеКоманды(Команда), "ПоУбыванию") И Не СвойстваЗаголовка.СортироватьПоУбыванию Тогда
...
Процедура ОчиститьОформлениеГруппировкиОтчета(Группировка, СвойстваЗаголовка)
	...
	Для Каждого ЭлементОформления Из Группировка.УсловноеОформление.Элементы Цикл
		...
		Если ОформляемыеПоляИспользуемые.Количество() = 1 И ОформляемыеПоляИспользуемые[0].Поле
			= СвойстваЗаголовка.Поле Тогда
...
Функция ОформлениеПрименимоКГруппировке(Группировка, Поле, ЭтоРесурс)
	...
	Возврат ЭтоРесурс Или Поле = Неопределено Или ПоляГруппировки.Элементы.Количество() = 0
...
Функция УсловиеРаскраскиГруппировкиОтчета(Отбор, Поиск)
	...
	Для Каждого Элемент Из Отбор.Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных") И Элемент.ЛевоеЗначение = Поиск.ЛевоеЗначение
			И Элемент.ВидСравнения = Поиск.ВидСравнения И Элемент.ПравоеЗначение = Поиск.ПравоеЗначение Тогда
...
Процедура ПослеВводаВысотыСтрокиОтчета(ВысотаСтроки, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВысотаСтроки) Тогда
		...
		УстановитьВысотуСтроки(
			ДополнительныеПараметры.Форма, ДополнительныеПараметры.Команда, ДополнительныеПараметры.СвойстваЗаголовка,
			ВысотаСтроки, ДополнительныеПараметры.ПараметрыРазмераПоля);
...
Процедура ПослеВводаШириныКолонкиОтчета(ШиринаКолонки, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ШиринаКолонки) Тогда
		...
		УстановитьШиринуКолонки(
			ДополнительныеПараметры.Форма, ДополнительныеПараметры.Команда, ДополнительныеПараметры.СвойстваЗаголовка,
			ШиринаКолонки, ДополнительныеПараметры.ПараметрыРазмераПоля);
...
Процедура ВыбратьПолеОтчета(Форма, Действие, ИмяКоллекции, Обработчик = Неопределено, Поле = Неопределено,
	...
	ПараметрыВыбора.Вставить("ИдентификаторЭлементаСтруктурыНастроек", ИдентификаторЭлементаСтруктурыНастроек(Форма,
		ИдентификаторУзлаНастроек));

	ОткрытьФорму(
		"ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборПоляОтчета", ПараметрыВыбора, Форма,
		Форма.УникальныйИдентификатор, , , Обработчик);
...
Процедура ВыбратьПолеОтчетаИзМеню(Форма, Команда, ИмяКоллекции = "ВыбранныеПоля")
	...
	Если ПоляОтчета.Количество() > 20 Или ПоляОтчета.Количество() = 1 И ПоляОтчета.НайтиПоЗначению("Еще")
		<> Неопределено Тогда
...
Функция ОсновныеПоляОтчета(СвойстваРезультата, ДоступныеПоля)
	...
	Для Каждого Поле Из ОсновныеПоля Цикл
		...
		Если ДоступноеПоле <> Неопределено И ОсновныеПоляИзДоступных.НайтиПоЗначению(ДоступноеПоле) = Неопределено Тогда
			...
			ОсновныеПоляИзДоступных.Добавить(ДоступноеПоле, ДоступноеПоле.Заголовок, , КартинкаПоля(
...
Процедура ИсключитьИспользуемыеПоляОтчета(ОсновныеПоля, Форма, Команда)
	...
	Если ТипЗнч(Владелец) <> Тип("КнопкаФормы") И (ТипЗнч(Владелец) <> Тип("ПолеФормы") Или Владелец.Вид
		<> ВидПоляФормы.ПолеТабличногоДокумента) Тогда

		Возврат;
	КонецЕсли;

	Если ДействиеКоманды(Команда) <> "ВставитьПолеСлева" И ДействиеКоманды(Команда) <> "ВставитьПолеСправа" Тогда
...
Процедура ИсключитьНедоступныеПоляОтчета(ОсновныеПоля, Форма, Команда)
	...
	Пока Индекс >= 0 Цикл
		...
		ИначеЕсли СвойстваЗаголовка.ИспользуетсяВПоляхГруппировки И СвойстваЗаголовка.ПереместитьПолеВправо
			И ДействиеКоманды(Команда) = "ВставитьПолеСправа" И ДоступноеПоле.Ресурс Тогда

			ОсновныеПоля.Удалить(Индекс);

		ИначеЕсли СвойстваЗаголовка.ИспользуетсяВПоляхГруппировки И ДействиеКоманды(Команда) = "ВставитьПолеСлева"
...
Процедура ПереместитьПолеГоризонтально(Форма, Команда)
	...
	ИначеЕсли
	ДействиеКоманды(Команда) = "ПереместитьПолеВправо" Тогда
...
Функция ИндексВыбранногоПоля(ИсходныйИндексПоля, КонечнаяКоллекцияПолей, Поле, ПараметрыПеремещения)
	...
	Если НовоеПоле <> Неопределено Тогда
		...
		Если ПараметрыПеремещения.Ответ = Неопределено Тогда
			...
			Обработчик = Новый ОписаниеОповещения("ПродолжитьПеремещениеПоляВертикально", ЭтотОбъект,
				ПараметрыПеремещения);
	...
	Для Каждого Элемент Из КонечнаяКоллекцияПолей.Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Или ТипЗнч(Элемент) = Тип(
			"ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
...
Функция СвойстваОбластиРасшифровки(Форма, Область, ИмяПоля)
	...
	Для Каждого Элемент Из СвойстваРезультата.Заголовки Цикл
		...
		Если СвойстваЗаголовка.ПорядокРаздела = ПорядокРаздела И СвойстваЗаголовка.Поле = Поле И СвойстваЗаголовка.Лево
			= Область.Лево Тогда
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	Если СвойстваОбласти.ЭтоЗаголовок Тогда
		...
		Если ТипЗнч(ВыполненноеДействие) = Тип("Строка") И СвойстваЗаголовка.Свойство(ВыполненноеДействие)
	...
	Если ТипЗнч(ВыполненноеДействие) = Тип("ВидСравненияКомпоновкиДанных") Или ТипЗнч(ВыполненноеДействие) = Тип(
		"Строка") И СтрНачинаетсяС(ВыполненноеДействие, "Фильтровать") Тогда
	...
	Если ТипЗнч(ВыполненноеДействие) = Тип("Строка") И (ВыполненноеДействие = "ОформитьКрасным" Или ВыполненноеДействие
		= "ОформитьЖелтым" Или ВыполненноеДействие = "ОформитьЗеленым" Или ВыполненноеДействие = "ОформитьОтрицательные"
		Или ВыполненноеДействие = "ОформитьПоложительные" Или ВыполненноеДействие = "УстановитьВысотуСтроки"
		Или ВыполненноеДействие = "УстановитьШиринуКолонки" Или ВыполненноеДействие = "ОформитьЕще") Тогда
...
Процедура ПодготовитьОтборОбработчикаРасшифровки(ДополнительныеПараметры)
	...
	ДополнитьОтборОбработчикаРасшифровки(
		ДополнительныеПараметры.Отбор, ДополнительныеПараметры.Данные.Настройки.Структура);
...
Процедура ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементыСтруктуры)
	...
	Для Каждого ЭлементСтруктуры Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ТипЗнч(ЭлементСтруктуры) = Тип("ГруппировкаКомпоновкиДанных") Или ТипЗнч(ЭлементСтруктуры) = Тип(
			"ГруппировкаТаблицыКомпоновкиДанных") Тогда
...
Процедура ДобавитьЭлементОтбораОбработчикаРасшифровки(Отбор, ЭлементОтбора)
	...
	Если ТипЗнч(ЭлементОтбора) <> Тип("ЭлементОтбораКомпоновкиДанных") Или Не ЭлементОтбора.Использование Тогда
...
Функция СвойстваЭлементаОтбораОбработчикаРасшифровки(Отбор, ОсновныеСвойства)
	...
	Если ТипЗнч(ЗначениеЭлементаОтбора) = Тип("СписокЗначений") И ЗначениеЭлементаОтбора.Количество() = 1 Тогда
	...
	Если ЭлементыОтбора.Количество() > 0 Тогда
		ЗаполнитьЗначенияСвойств(СвойстваЭлементаОтбора, ЭлементыОтбора[0],
			"РежимОтображения, Представление, ИдентификаторПользовательскойНастройки");
	КонецЕсли;
...
Процедура ОткрытьФормуОтчета(Форма, ПараметрыОткрытия)
	...
	Расшифровка = Новый ОписаниеОбработкиРасшифровкиКомпоновкиДанных(Форма.ОтчетДанныеРасшифровки,
		ПараметрыОткрытия.Расшифровка, ПараметрыОткрытия.Настройки);
	...
	Если ПараметрыОткрытия.Свойство("Отбор") И ТипЗнч(ПараметрыОткрытия.Отбор) = Тип("Структура") Тогда
...
Функция ЗаголовкиОтчета(Форма)
	...
	Если СтрЗаканчиваетсяНа(Форма.ИмяФормы, "Форма") Или СтрЗаканчиваетсяНа(Форма.ИмяФормы, "ФормаОтчета")
...
Процедура ОповеститьОЗавершенииКонтекстнойНастройки(Форма, Действие, ТекущееПоле = Неопределено,
	...
	ИдентификаторВладельца = ?(ЭтоФормОтчета, Форма.УникальныйИдентификатор,
		Форма.ВладелецФормы.УникальныйИдентификатор);
...
Функция ДействиеДоступно(Форма, Команда = Неопределено, СвойстваЗаголовка = Неопределено)
	...
	Если ТипЗнч(СвойстваЗаголовка) <> Тип("Структура") Или ДействиеКоманды <> Неопределено
		И Не СвойстваЗаголовка[ДействиеКоманды] Тогда
...
Процедура ДобавитьФормулу(Настройки, КоллекцияПолей, ОписаниеФормулы) Экспорт
	...
	ЭтоПользовательскоеПолеВыражениеКомпоновкиДанных = ТипЗнч(ОписаниеФормулы) = Тип(
...
Процедура ИзменитьФормулу(Форма, Настройки, ПутьКДанным, ИмяКоллекции = "ДоступныеПоляВыбора") Экспорт
	...
	Если ТипЗнч(Формула) <> Тип("ПользовательскоеПолеВыражениеКомпоновкиДанных") Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
			"ru = 'Редактирование поля ""%1""доступно в форме настроек для технического специалиста на закладке ""Пользовательские поля""'"),
			Формула.Заголовок);
...
Процедура УстановитьВыражениеИтоговыхЗаписей(Формула, ОписаниеФормулы)
	...
	Если ОписаниеФормулы = Неопределено Или ОписаниеФормулы.Тип.Типы().Количество() > 1
...
Функция ПараметрыСохраненияВариантаОтчетаВФайл(Форма) Экспорт
	...
	ПараметрыСохранения.Вставить("ПредставлениеПользовательскихНастроек",
		Форма.ПредставлениеТекущихПользовательскихНастроек);
...
Функция ЭтоФормаОтчета(Форма)
	...
	Возврат СтрЗаканчиваетсяНа(Форма.ИмяФормы, "Форма") Или СтрЗаканчиваетсяНа(Форма.ИмяФормы, "ФормаОтчета");
...
Функция СтекНастроекОтчета(Форма)
	...
	Если ВладелецФормы <> Неопределено И ЭтоФормаОтчета(ВладелецФормы) Тогда
