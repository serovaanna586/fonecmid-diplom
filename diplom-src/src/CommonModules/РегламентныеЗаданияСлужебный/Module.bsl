...
Процедура УстановитьИспользованиеРегламентныхЗаданийПоФункциональнымОпциям(ВключатьЗадания = Ложь) Экспорт
	...
	Для Каждого СтрокаЗадание Из Задания Цикл
		...
		НайденныеСтроки = ЗависимыеРегламентныеЗадания.НайтиСтроки(Новый Структура("РегламентноеЗадание",
			СтрокаЗадание.РегламентноеЗадание));

		Для Каждого СтрокаЗависимости Из НайденныеСтроки Цикл
			ОтключитьВПодчиненномУзлеРИБ = (СтрокаЗависимости.ДоступноВПодчиненномУзлеРИБ = Ложь)
		...
		Если Использование = Неопределено Или (Использование И Не ВключатьЗадания) // Автоматически при обновлении только отключаем регламентные задания.
...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки", НСтр(
		"ru = 'Блокировку также можно снять позднее в разделе <b>Администрирование - Обслуживание</b>.'"));
...
Процедура СформироватьТаблицуРегламентныхЗаданий(Параметры, АдресХранилища) Экспорт
	...
	ПодсистемаРаботаВМоделиСервиса = Метаданные.Подсистемы.СтандартныеПодсистемы.Подсистемы.Найти(
	...
	Для Каждого ОбъектМетаданных Из Метаданные.РегламентныеЗадания Цикл
		...
		Если Не ОбщегоНазначения.РазделениеВключено() И ПодсистемаРаботаВМоделиСервиса <> Неопределено Тогда
			...
			Если ПараметрыЗадания <> Неопределено И ОбщегоНазначения.ЭтоАвтономноеРабочееМесто()
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание) Экспорт
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено И СвойстваПоследнегоФоновогоЗадания.Состояние
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Запуск вручную: %1'"), ПредставлениеРегламентногоЗадания(Задание));
...
Процедура ИзменитьИспользованиеРегламентныхЗаданийПоФункциональнымОпциям(Источник, Знач Использование)
	...
	Если ФункциональнаяОпция = Неопределено Или ПолучитьФункциональнуюОпцию(ФункциональнаяОпция.Имя) = Использование Тогда
		Возврат;
	КонецЕсли;

	Задания = ЗависимыеРегламентныеЗадания.Скопировать(Новый Структура("ФункциональнаяОпция", ФункциональнаяОпция),
		"РегламентноеЗадание");
	Задания.Свернуть("РегламентноеЗадание");
...
Процедура ДобавитьСвойстваФоновыхЗаданий(Знач МассивФоновыхЗаданий, Знач ТаблицаСвойствФоновыхЗаданий)
	...
	Пока Индекс >= 0 Цикл
		...
		Если РегламентноеЗадание = Неопределено И СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(
