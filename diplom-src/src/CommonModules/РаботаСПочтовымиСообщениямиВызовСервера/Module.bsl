...
Функция СведенияДляОтправки(ПараметрыОтправки) Экспорт
	...
	Результат.Вставить("ПоказыватьДиалогВыбораФорматаСохраненияВложений", ЕстьТабличныеДокументыВоВложениях(
		ПараметрыОтправки.Вложения));
...
Функция ЕстьТабличныеДокументыВоВложениях(Вложения)
	...
	Для Каждого ОписаниеВложения Из Вложения Цикл
		Если ТипЗнч(ПолучитьИзВременногоХранилища(ОписаниеВложения.АдресВоВременномХранилище)) = Тип(
			"ТабличныйДокумент") Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
