...
Функция МакетИзДвоичныхДанных(ДвоичныеДанныеМакета) Экспорт
	...
	Если Не Расширение = "docx" Тогда
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", НСтр(
			"ru = 'Макет шаблона офисного документа имеет не верный формат.'"));
		ВызватьИсключение (НСтр(
			"ru = 'Ошибка анализа макета шаблона. Макет шаблона офисного документа имеет не верный формат.'"));
	КонецЕсли;
...
Процедура ЗакрытьСоединение(ПечатнаяФорма) Экспорт
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
...
Функция ДобавитьВерхнийКолонтитул(ПечатнаяФорма, ВерхнийКолонтитул) Экспорт
	...
	СтруктураКолонтитула = ПрисоединитьКСтруктуреДокументаКолонтитул(ПечатнаяФорма.СтруктураДокумента,
		ВерхнийКолонтитул);
...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Попытка
		Архиватор.Добавить(МаскаУпаковкиФайлов, РежимСохраненияПутейZIP.СохранятьОтносительныеПути,
...
Функция СобратьФайлДокументаDOCX(ПечатнаяФорма)
	...
	ИзменяемыеФайлы.Вставить("СвязиКонтента", ПечатнаяФорма.ИмяКаталога + УстановитьРазделительПути(
		"\word\_rels\document.xml.rels"));
	ИзменяемыеФайлы.Вставить("ТипыКонтента", ПечатнаяФорма.ИмяКаталога + УстановитьРазделительПути(
		"\[Content_Types].xml"));
	...
	Для Каждого Область Из ПечатнаяФорма.СтруктураДокумента.ПрисоединенныеОбласти Цикл
		...
		ВывестиПромежуточныйРаздел = ?(НомерРаздела <> Неопределено И НомерРаздела <> Область.НомерРаздела, Истина,
			Ложь);
		...
		Если ВывестиПромежуточныйРаздел = Истина И ЭтоПоследняяОбласть = Ложь Тогда
			...
			Если РазделКВыводу <> Неопределено Тогда
				...
				ТэгОткрытияРаздела = "<w:p w:rsidR=""" + ИдентификаторДокумента + """ w:rsidRDefault="""
					+ ИдентификаторДокумента + """><w:pPr>";
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			Если ПорядковыйНомер > 1 Тогда
				...
				Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И (ЧтениеXML.Имя = "w:document" Или ЧтениеXML.Имя
					= "w:body") Тогда
					Продолжить;
				КонецЕсли;
...
Процедура ИнициализироватьСтруктуруМакета(Макет)
	...
	Для Каждого Файл Из СтруктурныеФайлы Цикл
		...
		СтруктураДокумента.Колонтитулы.Вставить(Колонтитул.Имя + "_" + Формат(Колонтитул.НомерРаздела, "ЧГ=0"),
			Колонтитул);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ОбработатьТекст И ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
			...
			Для Каждого ПараметрТекст Из ПараметрыИзТекста Цикл
				...
				Если ТипЗнч(ЗначениеПараметра) = Тип("Строка") И Не СтрНачинаетсяС(ЗначениеПараметра,
					"e1cib/tempstorage") Тогда
					ТекстУзла = СтрЗаменить(ТекстУзла, "{v8 " + ПараметрТекст + "}", ЗначениеПараметра);
				ИначеЕсли ТипЗнч(ЗначениеПараметра) = Тип("Строка") И СтрНачинаетсяС(ЗначениеПараметра,
					"e1cib/tempstorage") Тогда
			...
			ИначеЕсли ВыводКартинки Тогда
				...
				Если Не ШиринаКлеткиТаблицы = 0 Тогда
					...
					ВысотаКартинки = ОтношениеВысотыКШирине * ШиринаКлеткиТаблицы * 914400 / РазрешениеОсновногоЭкрана
						/ 20;
				...
				СтруктураКартинка.Вставить("РасширениеКартинки", СтрЗаменить(ПараметрыКартинки.ТипИзображения,
					"image/", ""));
...
Процедура ПодготовитьШаблонКартинки(ШаблонКартинки, СтруктураКартинка)
	...
	// 6 - cx
	// 7 - cy
	ОбработанныйШаблонКартинки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКартинки, "0",
		СтруктураКартинка.ИмяКартинки, СтруктураКартинка.ИмяКартинки, СтруктураКартинка.rId,
		"{28A0092B-C50C-407E-A947-70E740481C1C}", Формат(СтруктураКартинка.ШиринаКартинки, "ЧГ=0"), Формат(
		СтруктураКартинка.ВысотаКартинки, "ЧГ=0"));
...
Процедура ВнестиКартинкуВБиблиотекуДокумента(СтруктураДокумента, СтруктураКартинка)
	...
	МаксНомерРесурса = СтруктураДокумента.ТаблицаСвязейКонтента[СтруктураДокумента.ТаблицаСвязейКонтента.Количество()
		- 1].НомерРесурса;
	...
	ДвоичныеДанные.Записать(СтруктураКартинка.КаталогКартинок + СтруктураКартинка.ИмяКартинки + "."
		+ СтруктураКартинка.РасширениеКартинки);
...
Процедура ИнициализироватьЗаписьВПоток(СтруктураРазбораXML, ИмяПотока, КорневойТэг = "w:next",
	ЗаписатьОбъявление = Истина)
	...
	Если Не СтруктураРазбораXML.ПотокиЗаписи.Свойство(ИмяПотока) Тогда
		СтруктураРазбораXML.ПотокиЗаписи.Вставить(ИмяПотока,
			Новый Структура("Поток, ЗаписьРазрешена, Уровень, ПотокЗакрыт, ТекстПотока, УровеньАбзаца"));
	КонецЕсли;
	...
	СтруктураРазбораXML.ПотокиЗаписи[ИмяПотока].Поток            = ИнициализироватьЗаписьXML(КорневойТэг, , ,
		ЗаписатьОбъявление);
...
Процедура ПеренестиЗаписьВПоток(СтруктураРазбораXML, ИмяПотокаИсточника, ИмяПотокаПриемника)
	...
	Если Не ПустаяСтрока(СтруктураРазбораXML.БлокирующийПоток) И Не СтруктураРазбораXML.БлокирующийПоток
		= ИмяПотокаПриемника Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПеренестиОткрывающиеТэгиЗаписиВПоток(СтруктураРазбораXML, ИмяПотокаИсточника, ИмяПотокаПриемника, СтопТэг)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			СтруктураРазбораXML.ПотокиЗаписи[ИмяПотокаПриемника].Уровень = СтруктураРазбораXML.ПотокиЗаписи[ИмяПотокаПриемника].Уровень
				+ 1;
		КонецЕсли;
...
Процедура ЗаписатьИнструкциюОбработкиВПотоки(СтруктураРазбораXML, ЧтениеXML, ИмяПотока = "")
	...
	Для Каждого ЭлементПотока Из СтруктураРазбораXML.ПотокиЗаписи Цикл
		...
		Если Не ПустаяСтрока(СтруктураРазбораXML.БлокирующийПоток) И Не ЭлементПотока.Ключ
			= СтруктураРазбораXML.БлокирующийПоток Тогда
			Продолжить;
		КонецЕсли;
	...
	ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
			...
			Если ИмяУзла = "w:document" Или ИмяУзла = "w:ftr" Или ИмяУзла = "w:hdr" Тогда
				СтруктураРазбораXML.АтрибутыXML = СтруктураРазбораXML.АтрибутыXML + " " + ЧтениеXML.Имя + "="""
					+ ЧтениеXML.Значение + """";
			КонецЕсли;
...
Процедура СформироватьРазобранныеСтроки(СтруктураРазбораXML)
	...
	Если СтруктураРазбораXML.СтатусТэга1С = 7 И СтруктураРазбораXML.ТипТэга1С = 0 Или СтруктураРазбораXML.СтатусТэга1С
		= 3 И СтруктураРазбораXML.ТипТэга1С = 1 И Не ПустаяСтрока(СтруктураРазбораXML.ИмяОбласти)
		Или СтруктураРазбораXML.СтатусТэга1С = 7 И СтруктураРазбораXML.ТипТэга1С = 1 И Не СтруктураРазбораXML.ИмяТэга1С
		= СтруктураРазбораXML.ИмяОбласти Тогда
	...
	Если Не СтруктураРазбораXML.ПолноеИмяТэга1С = "" Тогда
		Если СтруктураРазбораXML.СтатусТэга1С = 3 Или СтруктураРазбораXML.СтатусТэга1С = 7 Тогда
			...
			СтруктураРазбораXML.ПолноеИмяТэга1С = СтрЗаменить(СтруктураРазбораXML.ПолноеИмяТэга1С, "{/v8", "{/v8 ");
		КонецЕсли;
		ДобавитьРазобраннуюСтроку(СтруктураРазбораXML, СтруктураРазбораXML.ПолноеИмяТэга1С, ?(
			СтруктураРазбораXML.ТипТэга1С = 1, СтруктураРазбораXML.СтатусТэга1С, 0), ?(СтруктураРазбораXML.ТипТэга1С
			= 1, СтруктураРазбораXML.ИмяТэга1С, ""), "ФорматТэга1С");
	КонецЕсли;
...
Процедура РазбитьТекстШаблонаНаОбласти(ЧтениеXML, СтруктураДокумента, ПараметрыАнализа)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		ЗавершитьЗаписьВЗаголовок = ЧтениеНачалаТелаДокумента(СтруктураРазбораXML, ЧтениеXML)
			Или ЧтениеНачалаТелаКолонтитула(СтруктураРазбораXML, ЧтениеXML);
		...
		Если ЧтениеКонцаСтроки(СтруктураРазбораXML, ЧтениеXML) Тогда
			...
			ИначеЕсли СтруктураРазбораXML.РазобранныеСтроки.Количество() > 0 Тогда
				...
				Для Каждого ЭлементСтроки Из СтруктураРазбораXML.РазобранныеСтроки Цикл
					...
					Если ПараметрыАнализа.ТипАнализа = 1 И ТекущийУровень = УровеньТэгаОбласть
						И ЭлементСтроки.СтатусТэга1С = 3 И ПустаяСтрока(СтруктураРазбораXML.ИмяОбласти) Тогда
					...
					ИначеЕсли ПараметрыАнализа.ТипАнализа = 1 И ТекущийУровень = УровеньТэгаОбласть
						И ЭлементСтроки.СтатусТэга1С = 7 И СтруктураРазбораXML.ИмяОбласти = ЭлементСтроки.ИмяОбласти Тогда
					...
					ИмеетсяАтрибутПробела = СтрНайти(
						СтруктураРазбораXML.ПотокиЗаписи[ЭлементСтроки.ПотокФормата].ТекстПотока, "w:t xml:space",
						НаправлениеПоиска.СКонца) > 0;

					ПеренестиОткрывающиеТэгиЗаписиВПоток(СтруктураРазбораXML, ЭлементСтроки.ПотокФормата,
						"СтрокиПарам", "w:t");

					Если Не ИмеетсяАтрибутПробела И (ПустаяСтрока(Лев(ЭлементСтроки.Текст, 1)) Или ПустаяСтрока(Прав(
						ЭлементСтроки.Текст, 1))) Тогда
			...
			Если СтруктураРазбораXML.СтатусТэга1С = 0 И СтруктураРазбораXML.РазобранныеСтроки.Количество() = 0
				Или СтруктураРазбораXML.РазобранныеСтроки.Количество() > 0 Тогда
		...
		Если ЧтениеКонцаПараграфа(СтруктураРазбораXML, ЧтениеXML) Тогда
			...
			Если ПараметрыАнализа.ТипАнализа <> 1 Или Не ПустаяСтрока(СтруктураРазбораXML.ИмяОбласти)
				И СтруктураРазбораXML.СтатусОбласти = 1 Тогда
	...
	Если ПараметрыАнализа.ТипАнализа = 1 Тогда
		...
		ТекстРазрыва = "<w:p w:rsidRDefault=""" + СтруктураДокумента.ИдентификаторДокумента + """ w:rsidR="""
			+ СтруктураДокумента.ИдентификаторДокумента + """></w:p>";
...
Процедура АнализироватьПараметрыВСтроке(Знач Строка, СтруктураРазбораXML)
	...
	Для ф = 1 По ДлинаСтроки Цикл
		...
		Если ДлинаСтрокиТэга1С + 1 <= СтрДлина(ПризнакНачалаТэга1С) И Лев(ПризнакНачалаТэга1С, ДлинаСтрокиТэга1С + 1)
		...
		ИначеЕсли ДлинаСтрокиТэга1С <= СтрДлина(ПризнакКонцаТэга1С) И Лев(ПризнакКонцаТэга1С, ДлинаСтрокиТэга1С + 1)
			= СтруктураРазбораXML.ПолноеИмяТэга1С + Символ Тогда
		...
		Если СтруктураРазбораXML.СтатусТэга1С = 0 И СтрНачинаетсяС(СтруктураРазбораXML.ПолноеИмяТэга1С,
			ПризнакНачалаТэга1С) Тогда
			СтруктураРазбораXML.СтатусТэга1С = 1;
		ИначеЕсли СтруктураРазбораXML.СтатусТэга1С = 0 И СтрНачинаетсяС(СтруктураРазбораXML.ПолноеИмяТэга1С,
			ПризнакКонцаТэга1С) Тогда
			СтруктураРазбораXML.СтатусТэга1С = 5;
		КонецЕсли;

		Если СтруктураРазбораXML.СтатусТэга1С = 1 И Не СтрНачинаетсяС(СтруктураРазбораXML.ПолноеИмяТэга1С,
			ПризнакНачалаТэга1С) Или СтруктураРазбораXML.СтатусТэга1С = 5 И Не СтрНачинаетсяС(
			СтруктураРазбораXML.ПолноеИмяТэга1С, ПризнакКонцаТэга1С) Или СтруктураРазбораXML.СтатусТэга1С = 5
			И ПустаяСтрока(СтруктураРазбораXML.ИмяОбласти) Тогда
			Текст = СтруктураРазбораXML.ТекстДоТэга1С + СтруктураРазбораXML.ПолноеИмяТэга1С
				+ СтруктураРазбораXML.ТекстПослеТэга1С;
			СброситьСтатусыТэгов1С(СтруктураРазбораXML);
		...
		Если СтруктураРазбораXML.СтатусТэга1С = 2 Или СтруктураРазбораXML.СтатусТэга1С = 6 Тогда
			...
			Если (КодСимвола = 32 Или (КодСимвола >= 48 И КодСимвола <= 57) Или (КодСимвола >= 65 И КодСимвола <= 90)
				Или КодСимвола = 95 Или (КодСимвола >= 97 И КодСимвола <= 122) Или (КодСимвола >= 1040 И КодСимвола
				<= 1103)) Тогда
...
Процедура ВыделитьИзРазделаКолонтитулы(ЧтениеXML, СтруктураДокумента, Раздел)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если Не (ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И (ЧтениеXML.Имя = "w:headerReference" Или ЧтениеXML.Имя
			= "w:footerReference")) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ОбработатьРазделДокумента(СтруктураДокумента, Раздел)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И (ЧтениеXML.Имя = "w:headerReference" Или ЧтениеXML.Имя
			= "w:footerReference") Тогда
...
Функция ПолучитьАтрибутыИзображения(ДанныеЧтения)
	...
	Иначе
		...
		Если ((Символ(Символ1) = "M" И Символ(Символ2) = "M" И Символ3 = 0 И Символ4 = 42) Или (Символ(Символ1) = "I"
			И Символ(Символ2) = "I" И Символ3 = 42 И Символ4 = 0)) Тогда //TIFF
...
Функция ОпределитьРасширениеФайлаДанныхПоСигнатуре(ДанныеИлиСтруктура) Экспорт
	...
	Если ТипЗнч(ДанныеИлиСтруктура) = Тип("Структура") Тогда
		Попытка
			...
			ДвоичныеДанныеМакета = МакетИДанныеОбъекта.Макеты.ДвоичныеДанныеМакетов.Получить(
				ДанныеИлиСтруктура.Идентификатор);
		Исключение
			Возврат Неопределено;
		КонецПопытки;
	Иначе
	...
	Если Символ1 = 208 И Символ2 = 207 И Символ3 = 17 И Символ4 = 224 И Символ5 = 161 И Символ6 = 177 И Символ7 = 26
		И Символ8 = 225 Тогда
		Возврат "doc";
	ИначеЕсли Символ1 = 80 И Символ2 = 75 И Символ3 = 3 И Символ4 = 4 И Символ5 = 20 И Символ6 = 0 И Символ7 = 0
		И Символ8 = 8 Или Символ1 = 80 И Символ2 = 75 И Символ3 = 3 И Символ4 = 4 И Символ5 = 10 И Символ6 = 0
		И Символ7 = 0 И Символ8 = 0 Тогда
		Возврат "odt";
