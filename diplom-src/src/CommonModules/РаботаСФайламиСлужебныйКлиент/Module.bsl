...
Процедура ДобавитьИзФайловойСистемыСРасширением(ПараметрыВыполнения) Экспорт
	...
	Если Не Результат.ФайлДобавлен Тогда
		Если ЗначениеЗаполнено(Результат.ТекстОшибки) Тогда
			ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, Результат.ТекстОшибки,
				Неопределено);
		Иначе
...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	ФильтрДиалога = ?(ПараметрыВыполнения.Свойство("ФильтрДиалогаВыбора"), ПараметрыВыполнения.ФильтрДиалогаВыбора,
		НСтр("ru = 'Все файлы (*.*)|*.*'"));
	...
	Если Не ПараметрыВыполнения.Свойство("ПолноеИмяФайла") Тогда
		...
		ДиалогВыбораФайла.Каталог = РаботаСФайламиСлужебныйВызовСервера.РабочийКаталогПапки(
	...
	Если ПараметрыВыполнения.Свойство("МаксимальныйРазмер") И ПараметрыВыполнения.МаксимальныйРазмер > 0
	...
	ФайлыПомещены = ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь,
		ПараметрыВыполнения.ФормаВладелец.УникальныйИдентификатор);
...
#КонецЕсли
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Создание:'"), ПолучитьНавигационнуюСсылку(Результат.ФайлСсылка), Результат.ФайлСсылка,
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Если ВыбранныеФайлы.Количество() > 0 Тогда
			...
			ПоместитьВыбранныеФайлыВХранилище(
				ВыбранныеФайлы, ВладелецФайла, ПрисоединенныеФайлыМассив, ИдентификаторФормы, ИмяСоздаваемогоФайла,
			...
			Если ПрисоединенныеФайлыМассив.Количество() = 1 И ОткрыватьКарточкуПослеСозданияИзФайла Тогда
				...
				ПоказатьОповещениеПользователя(
					НСтр("ru = 'Создание:'"), ПолучитьНавигационнуюСсылку(ПрисоединенныйФайл), ПрисоединенныйФайл,
					БиблиотекаКартинок.Информация32);
...
Процедура ОтправитьФайлыПоПочте(МассивФайлов, ИдентификаторФормы, ПараметрыОтправки, ЭтоФайл = Ложь) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтправитьФайлыПоПочтеНастройкаУчетнойЗаписиПредложена", ЭтотОбъект,
		Параметры);
...
Функция ПоказыватьСлужебныеФайлыНажатие(Список) Экспорт
	...
	ЭлементОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементОтбораПоПредставлению(Список.Отбор.Элементы,
		"СкрыватьСлужебные");
...
Процедура ЗакончитьРедактированиеСОповещением(Параметры) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеСОповещениемЗавершение", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура ПроверитьПодписи(Форма, СсылкаНаДвоичныеДанные, ВыделенныеСтроки = Неопределено) Экспорт
	...
	ОбработчикПродолжения = Новый ОписаниеОповещения("ПослеРасшифровкиФайлаПриПроверкеПодписи", ЭтотОбъект,
...
Процедура ОткрытьВерсиюФайла(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("ОткрытьВерсиюФайлаПослеУстановкиРасширения", ЭтотОбъект,
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Открыть список редактируемых файлов (%1)'"), Ответ.КоличествоЗанятыхФайлов);
	...
	Если ИмяФайлаСПутем <> "" Тогда
		...
		Если Отказ = Ложь Тогда
			...
			Если Не ПустаяСтрока(Текст) Тогда
				...
				РезультатЗагрузки = ПоместитьФайлСДискаВоВременноеХранилище(ИмяВременногоФайла, ,
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Загрузка файлов с расширением ""%1"" запрещена.
					 |Обратитесь к администратору.'"), РасширениеФайла);
		Иначе
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьРабочийКаталогДанныхПользователя(Оповещение)
	...
	НачатьПолучениеРабочегоКаталогаДанныхПользователя(
		Новый ОписаниеОповещения("ПолучитьРабочийКаталогДанныхПользователяПослеПолучения", ЭтотОбъект, Контекст,
...
Процедура ПолучитьРабочийКаталогДанныхПользователяПослеОшибкиПолучения(ИнформацияОбОшибке, СтандартнаяОбработка,
	Контекст) Экспорт
...
Процедура ПроверитьПодписиЦиклНачало(ДополнительныеПараметры)
	...
	МодульЭлектроннаяПодписьКлиент.ПроверитьПодпись(
		...
		ДополнительныеПараметры.Данные, ДополнительныеПараметры.СтрокаПодписи.АдресПодписи, Неопределено,
...
Процедура ДействиеПриОткрытииФайлаВРабочемКаталоге(ОбработчикРезультата, ИмяФайлаСПутем, ДанныеФайла)
	...
	Параметры.Вставить("ДатаИзмененияУниверсальнаяВХранилищеФайлов", ДанныеФайла.ДатаМодификацииУниверсальная);
	Параметры.Вставить("ДатаИзмененияУниверсальнаяВРабочемКаталоге", Файл.ПолучитьУниверсальноеВремяИзменения());
	Параметры.Вставить("ДатаИзмененияВРабочемКаталоге", МестноеВремя(
		Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге));
	Параметры.Вставить("ДатаИзмененияВХранилищеФайлов", МестноеВремя(
	...
	Если РазницаДат <= 1 Тогда // С секунда - допустимая разница (на Win95 может быть такое).
		...
		Если Параметры.РазмерВХранилищеФайлов <> 0 И Параметры.РазмерВХранилищеФайлов
			<> Параметры.РазмерВРабочемКаталоге Тогда
			// Дата одинаковая, но размер отличается - редкий, но возможный случай.

			Параметры.Вставить("Заголовок", НСтр("ru = 'Размер файла отличается'"));

			Параметры.Вставить("Сообщение", НСтр("ru = 'Размер файла в рабочем каталоге на компьютере и его копии в программе отличается.
												 |
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			...
			Параметры.Вставить("Сообщение", НСтр("ru = 'Файл в программе, отмеченный как занятый для редактирования,
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь И ДанныеФайла.Редактирует
			= ПользователиКлиент.АвторизованныйПользователь() Тогда
		...
		Иначе
			...
			Параметры.Вставить(
				"Сообщение", НСтр("ru = 'Копия файла в рабочем каталоге на компьютере имеет более позднюю дату изменения (новее), чем в программе. Возможно, эта копия была отредактирована.
								  |
	...
	ОткрытьФорму("ОбщаяФорма.ВыборДействияПриОбнаруженииОтличийФайла", Параметры, , , , , ОбработчикРезультата,
...
Процедура ПолучитьРабочийКаталогПользователя(Оповещение)
	...
	ИмяКаталога = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ПерсональныеНастройкиРаботыСФайлами.ПутьКЛокальномуКэшуФайлов;
	...
	ПолучитьРабочийКаталогДанныхПользователя(
		Новый ОписаниеОповещения("ПолучитьРабочийКаталогПользователяПослеПолученияКаталогаДанных", ЭтотОбъект,
		Контекст));
...
Процедура ВывестиНапоминаниеПриРедактировании(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если Не РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = НСтр("ru = 'Сейчас будет предложено открыть или сохранить файл.
									|
			...
			Если СтрНайти(СистемнаяИнформация.ИнформацияПрограммыПросмотра, "Firefox") <> 0 Тогда
				ТекстНапоминания = ТекстНапоминания + "
													  |
													  |" + НСтр(
					"ru = '(По умолчанию браузер Mozilla Firefox автоматически сохраняет файлы в каталоге ""Мои документы"")'");
			КонецЕсли;
			...
			ПараметрыНапоминания.Вставить("ТекстФлажка", НСтр("ru = 'Больше не показывать это сообщение'"));
			ПараметрыНапоминания.Вставить("Заголовок", НСтр("ru = 'Получение файла для просмотра или редактирования'"));
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ТехнологияСервиса.ЭлектроннаяПодписьВМоделиСервиса") Тогда
		МодульЭлектроннаяПодписьВМоделиСервисаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса Или ИспользоватьОблачнуюПодпись
	...
	Для Каждого Элемент Из Коллекция Цикл
		...
		ДанныеСтроки.Вставить("СрокДействияПоследнейМеткиВремени", СтрокаПодписи.СрокДействияПоследнейМеткиВремени);
...
//
Процедура ПоместитьВыбранныеФайлыВХранилище(Знач ВыбранныеФайлы, Знач ВладелецФайла, ПрисоединенныеФайлыМассив,
	Знач ИдентификаторФормы, Знач ИмяСоздаваемогоФайла = "", Знач ГруппаФайлов = Неопределено)
...
Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы, Знач Файл, Знач ТекущаяПозиция = Неопределено,
	...
	Если ВыбранныеФайлы.Количество() > 1 Тогда
		Если ТекущаяПозиция = Неопределено Тогда
			ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение файлов'"), , НСтр(
				"ru = 'Сохранение файлов успешно завершено'"));
		КонецЕсли;
	Иначе
		Если ТекущаяПозиция = Неопределено Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл ""%1"" (%2 Мб) сохранен.'"), СохраняемоеИмяФайла, РазмерВМб);
			ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение файла'"), , ТекстПояснения,
				БиблиотекаКартинок.Информация32);
		КонецЕсли;
	КонецЕсли;
	...
	Если РазмерЗагружаемогоФайла > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Размер файла ""%1"" (%2 Мб)
				 |превышает максимально допустимый размер файла (%3 Мб).'"), ВыбранноеИмяФайла,
	...
	ИначеЕсли Не ЗначениеЗаполнено(Редактирует) Тогда
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно освободить файл ""%1"",
				 |т.к. он никем не занят.'"), Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		...
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно освободить файл ""%1"",
				 |т.к. он занят пользователем ""%2"".'"), Строка(ОбъектСсылка), Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОсвободитьФайлы(СписокФайлов) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокФайлов.ВыделенныеСтроки Цикл
		...
		Если Не ВозможностьОсвободитьФайл(
				ДанныеСтроки.Ссылка, ДанныеСтроки.ФайлРедактируетТекущийПользователь, ДанныеСтроки.Редактирует) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Файл освобожден'"), ДанныеФайла.НавигационнаяСсылка,
		ДанныеФайла.ПолноеНаименованиеВерсии, БиблиотекаКартинок.Информация32);
...
Процедура ПеренестиФайлыВПапку(ОбъектыСсылка, Папка) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Перенос файла'"), ДанныеФайла.НавигационнаяСсылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл ""%1""
					 |перенесен в папку ""%2"".'"), Строка(ДанныеФайла.Ссылка), Строка(Папка)),
			БиблиотекаКартинок.Информация32);
...
Функция ПроверитьВозможностьЗагрузкиФайла(Файл, ВызыватьИсключение = Истина, МассивИменФайловСОшибками = Неопределено)
	...
	Если Файл.Размер() > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Размер файла ""%1"" (%2 Мб)
				 |превышает максимально допустимый размер файла (%3 Мб).'"), Файл.Имя,
			РаботаСФайламиСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб),
			РаботаСФайламиСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМбМакс));
	...
	Если Не ПроверитьРасширениеФайлаДляЗагрузки(Файл.Расширение, Ложь) Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Загрузка файлов с расширением ""%1"" запрещена.
				 |Обратитесь к администратору.'"), Файл.Расширение);
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
	...
	МожноОсвободитьФайл = ВозможностьОсвободитьФайл(
		ДанныеФайла.Ссылка, ДанныеФайла.ФайлРедактируетТекущийПользователь, ДанныеФайла.Редактирует, ТекстОшибки);
			...
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось поместить файл
						 ...
						 |
						 |Освободить файл?'"), Строка(ДанныеФайла.Ссылка), ПараметрыВыполнения.ПолныйПутьКФайлу);
			Иначе
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось поместить файл ""%1""
						 |в программу, т.к. он не существует в рабочем каталоге на компьютере.
						 |
						 |Освободить файл?'"), Строка(ДанныеФайла.Ссылка));
			КонецЕсли;

			Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеСРасширениемПослеОтветаНаВопросОсвободитьФайл",
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось поместить файл ""%1"" в рабочий каталог на компьютер,
				 |так как возможно он заблокирован другой программой.'"), Строка(ДанныеФайла.Ссылка));
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Если ДанныеФайла.ХранитьВерсии Тогда
			...
			Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеСРасширениемПослеПомещенияФайлаНаСервер",
				ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиНовойВерсии(ПараметрыВыполнения)
	...
	ПараметрыВыполнения.Вставить("ОповещениеПослеШифрования",
		Новый ОписаниеОповещения("ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован", ЭтотОбъект,
		ПараметрыВыполнения));
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
		...
		Исключение
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось поместить файл с компьютера в программу по причине:
					 |""%1"".
					 |
					 |Повторить операцию?'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));

			Оповещение  = Новый ОписаниеОповещения("ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрованПовтор",
				ЭтотОбъект, ПараметрыВыполнения);
	...
	Если Не ОбщиеНастройкиРаботыСФайлами.ИзвлекатьТекстыФайловНаСервере Тогда
		Попытка
			СведенияОФайле.АдресВременногоХранилищаТекста = ИзвлечьТекстВоВременноеХранилище(
				ПараметрыВыполнения.ПолныйПутьКФайлу, ПараметрыВыполнения.ИдентификаторФормы, ,
	...
	Попытка
		ВерсияОбновлена = РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияИОсвободитьФайл(
			ПараметрыВыполнения.ДанныеФайла, СведенияОФайле, НеМенятьЗаписьВРабочемКаталоге,
			ПараметрыВыполнения.ПолныйПутьКФайлу, РабочийКаталогПользователя(), ПараметрыВыполнения.ИдентификаторФормы);
	Исключение
		ЗакончитьРедактированиеСРасширениемОбработкаИсключения(ИнформацияОбОшибке(), ПараметрыВыполнения);
		Возврат;
	КонецПопытки;
	...
	Если ПараметрыВыполнения.ПереданныйПолныйПутьКФайлу = "" Тогда
		...
		Если УдалятьФайлИзЛокальногоКэшаФайловПриЗавершенииРедактирования Тогда
			Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеСРасширениемПослеУдаленияФайлаИзРабочегоКаталога",
				ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ЗакончитьРедактированиеСРасширениемПослеУдаленияФайлаИзРабочегоКаталога(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Редактирование закончено'"), ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонПояснения, Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка)), БиблиотекаКартинок.Информация32);

		Если Не ПараметрыВыполнения.ВерсияОбновлена Тогда
			Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеСРасширениемПослеПоказаОповещения",
				ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ЗакончитьРедактированиеСРасширениемОбработкаИсключения(ИнформацияОбОшибке, ПараметрыВыполнения)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось поместить файл ""%1""
			 ...
			 |%2.'"), Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Предупреждение",
...
Процедура ЗакончитьРедактированиеБезРасширения(ПараметрыВыполнения)
	...
	ТекстОшибки = "";
	МожноОсвободитьФайл = ВозможностьОсвободитьФайл(
		ПараметрыВыполнения.ОбъектСсылка, ПараметрыВыполнения.ФайлРедактируетТекущийПользователь,
		ПараметрыВыполнения.Редактирует, ТекстОшибки);
	Если Не МожноОсвободитьФайл Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстОшибки, Ложь);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеБезРасширенияПослеПомещенияФайлаНаСервер",
			ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеПроверкиНовойВерсии(ПараметрыВыполнения)
	...
	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеБезРасширенияПослеНапоминания", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеНапоминания(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("БольшеНеЗадаватьЭтотВопрос")
		И Результат.БольшеНеЗадаватьЭтотВопрос Тогда
		ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить(
			"НастройкиПрограммы", "ПоказыватьПодсказкиПриРедактированииФайлов", Ложь, , , Истина);
	КонецЕсли;

	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла", ЭтотОбъект,
		ПараметрыВыполнения);
...
// Продолжение процедуры (см. выше).
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла,
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеШифрованияФайла(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Попытка
		Результат = РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияИОсвободитьФайлПоСсылке(
			ПараметрыВыполнения.ОбъектСсылка, СведенияОФайле, ПараметрыВыполнения.ПолныйПутьКФайлу,
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		...
		Если Не Результат.Успешно Тогда
			Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеБезРасширенияПослеПоказаОповещения",
				ЭтотОбъект, ПараметрыВыполнения);
			ПоказатьИнформациюФайлНеБылИзменен(Обработчик);
			Возврат;
		КонецЕсли;
...
Процедура ЗакончитьРедактированиеОбработчикИсключения(ИнформацияОбОшибке, ПараметрыВыполнения)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось поместить файл ""%1""
			 ...
			 |""%2"".
			 |
			 |Повторить операцию?'"), Строка(ПараметрыВыполнения.ОбъектСсылка), КраткоеПредставлениеОшибки(
		ИнформацияОбОшибке));

	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеБезРасширенияПослеОтветаНаВопросПовторить",
		ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы,
	...
	Иначе
		ПутьВыбора = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("НастройкиПрограммы",
			"ПапкаДляОбновитьИзФайла");
	КонецЕсли;
	...
	Если ПараметрыДобавленияФайла <> Неопределено И ПараметрыДобавленияФайла.Свойство("ФильтрДиалогаВыбора")
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			Если МодульЭлектроннаяПодписьКлиент.ИспользоватьШифрование() Тогда
				РасширениеДляЗашифрованныхФайлов = МодульЭлектроннаяПодписьКлиент.ПерсональныеНастройки().РасширениеДляЗашифрованныхФайлов;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДобавленияФайла <> Неопределено И ПараметрыДобавленияФайла.Свойство("МаксимальныйРазмер")
		И ПараметрыДобавленияФайла.МаксимальныйРазмер > 0 И ФайлНаДиске.Размер()
		> ПараметрыДобавленияФайла.МаксимальныйРазмер * 1024 * 1024 Тогда
	...
	Если ПустаяСтрока(ДанныеФайла.РабочийКаталогВладельца) Тогда
		Если ПутьВыбораПрежний <> ПутьВыбора Тогда
			ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("НастройкиПрограммы",
				"ПапкаДляОбновитьИзФайла", ПутьВыбора);
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыВыполнения.Вставить("РасширениеФайлаНаДиске", ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(
		ПараметрыВыполнения.ФайлНаДиске.Расширение));

	ПараметрыВыполнения.Вставить("РасширениеДляЗашифрованныхФайлов", РасширениеДляЗашифрованныхФайлов);

	ПараметрыВыполнения.Вставить("ФайлЗашифрован", НРег(ПараметрыВыполнения.РасширениеФайлаНаДиске) = НРег(
		ПараметрыВыполнения.РасширениеДляЗашифрованныхФайлов));
	...
	НачатьПомещениеФайлов(Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеПередРасшифровкой", ЭтотОбъект,
		ПараметрыВыполнения), ПомещаемыеФайлы, , Ложь, ПараметрыВыполнения.ИдентификаторФормы);
...
Процедура ОбновитьИзФайлаНаДискеПередРасшифровкой(ПомещаемыеФайлы, ПараметрыВыполнения) Экспорт
	...
	ОбработчикПродолжения = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеПослеРасшифровки", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура ОбновитьИзФайлаНаДискеПродолжение(ПараметрыВыполнения)
	...
	ПараметрыВыполнения.ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаИРабочийКаталог(
...
#КонецЕсли
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстаВопроса, Строка(
			ПараметрыВыполнения.ДанныеФайла.Ссылка), МестноеВремя(ПараметрыВыполнения.ВремяИзмененияВыбранное),
			МестноеВремя(ПараметрыВыполнения.ДатаФайлаВБазе));

		ОписаниеОповещения = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеСтараяВерсия", ЭтотОбъект,
			ПараметрыВыполнения);
	...
	ПолноеИмяФайла = "";
	ФайлВРабочемКаталоге = ФайлНаходитсяВЛокальномКэшеФайлов(Неопределено, ПредыдущаяВерсия, ПолноеИмяФайла,
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл ""%1"" открыт для редактирования.
					 |Сначала необходимо закончить редактирование, а затем повторить действие.'"), ПолноеИмяФайла);
			ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстОшибки,
				Неопределено);
			Возврат;
		КонецПопытки;
	КонецЕсли;
#КонецЕсли
	...
	Если ФайлВРабочемКаталоге И ПараметрыВыполнения.ИмяИРасширениеФайлаНаДиске <> ИмяИРасширениеФайлаВБазе Тогда
		Обработчик = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеПослеУдаленияФайлаИзРабочегоКаталога", ЭтотОбъект,
			ПараметрыВыполнения);
...
Процедура ОбновитьИзФайлаНаДискеПослеПолученияФайлаВРабочийКаталог(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ФайлЗашифрован Тогда
		РаботаСФайламиСлужебныйВызовСервера.УстановитьПризнакЗашифрован(ПараметрыВыполнения.ДанныеФайла.Ссылка,
			ПараметрыВыполнения.ФайлЗашифрован);
	КонецЕсли;

	ПереданныйПолныйПутьКФайлу = "";

	Обработчик = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеПослеЗавершенияРедактирования", ЭтотОбъект,
		ПараметрыВыполнения);
	Если ПараметрыВыполнения.ФайлРедактируетТекущийПользователь Тогда // Файл уже был занят.
		ПараметрыОбработчика = ПараметрыОбновленияФайла(Обработчик, ПараметрыВыполнения.ДанныеФайла.Ссылка,
			ПараметрыВыполнения.ИдентификаторФормы);
...
Процедура ЗанятьФайлПоСсылке(ОбработчикРезультата, ОбъектСсылка, УникальныйИдентификатор = Неопределено)
	...
	Обработчик = Новый ОписаниеОповещения("ЗанятьФайлПоСсылкеПослеУстановкиРасширения", ЭтотОбъект,
		ПараметрыОбработчика);
...
Процедура ЗанятьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ДанныеФайлаПолученыИОнЗанят = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИЗанятьФайл(
		ПараметрыВыполнения.ОбъектСсылка, ПараметрыВыполнения.ДанныеФайла, ТекстОшибки,
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Редактирование файла'"), ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл ""%1""
				 |занят для редактирования.'"), Строка(ДанныеФайла.Ссылка)), БиблиотекаКартинок.Информация32);
...
Процедура ЗанятьФайлыПоСсылкам(ОбработчикРезультата, Знач МассивФайлов)
	...
	Обработчик = Новый ОписаниеОповещения("ЗанятьФайлыПоСсылкамПослеУстановкиРасширения", ЭтотОбъект,
		ПараметрыОбработчика);
...
Процедура ЗанятьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Для Инд = 0 По ВГраницаМассива Цикл
		...
		СтрокаОшибки = "";
		Если Не РаботаСФайламиКлиентСервер.МожноЛиЗанятьФайл(ДанныеФайла, СтрокаОшибки) Или ЗначениеЗаполнено(
			ДанныеФайла.Редактирует) Тогда // Занять невозможно.
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Занять файлы'"), , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файлы (%1 из %2) заняты для редактирования.'"), КоличествоЗанятых,
		ПараметрыВыполнения.МассивФайлов.Количество()), БиблиотекаКартинок.Информация32);
...
//
Процедура РедактироватьФайлПоСсылке(ОбработчикРезультата, ОбъектСсылка, УникальныйИдентификатор = Неопределено,
	...
	Обработчик = Новый ОписаниеОповещения("РедактироватьФайлПоСсылкеПослеУстановкиРасширения", ЭтотОбъект,
		ПараметрыОбработчика);
	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Обработчик);
...
Процедура РедактироватьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если Не Результат.ДанныеПолучены Тогда
		...
		РаботаСФайламиКлиентПереопределяемый.ПриОшибкеЗахватаФайла(ПараметрыВыполнения.ДанныеФайла,
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Редактирование файла'"),
		ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл ""%1""
				 |занят для редактирования.'"), Строка(ДанныеФайла.Ссылка)), БиблиотекаКартинок.Информация32);
	...
	Если ПараметрыВыполнения.РасширениеПодключено Тогда
		Обработчик = Новый ОписаниеОповещения("РедактироватьФайлПоСсылкеСРасширениемПослеПолученияФайлаВРабочийКаталог",
			ЭтотОбъект, ПараметрыВыполнения);
	...
	Иначе
		...
		ОткрытьФайлБезРасширения(Обработчик, ПараметрыВыполнения.ДанныеФайла,
			ПараметрыВыполнения.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура РедактироватьФайлПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	МожноЗанятьФайл = РаботаСФайламиКлиентСервер.МожноЛиЗанятьФайл(
		ПараметрыВыполнения.ДанныеФайла, ТекстОшибки);
	Если Не МожноЗанятьФайл Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстОшибки, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьФайлПослеЗанятияФайла(ДанныеФайла, ПараметрыВыполнения) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено() Тогда
		Обработчик = Новый ОписаниеОповещения("РедактироватьФайлСРасширениемПослеПолученияФайлаВРабочийКаталог",
			ЭтотОбъект, ПараметрыВыполнения);
		ПолучитьФайлВерсииВРабочийКаталог(
			Обработчик, ПараметрыВыполнения.ДанныеФайла, "", ПараметрыВыполнения.УникальныйИдентификатор);
	Иначе
		ЗаполнитьВременныйИдентификаторФормы(ПараметрыВыполнения.УникальныйИдентификатор, ПараметрыВыполнения);

		Обработчик = Новый ОписаниеОповещения("РедактироватьФайлБезРасширенияЗавершение", ЭтотОбъект,
			ПараметрыВыполнения);
		ОткрытьФайлБезРасширения(Обработчик, ПараметрыВыполнения.ДанныеФайла,
			ПараметрыВыполнения.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ОткрытьВерсиюФайлаПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено() Тогда
		Обработчик = Новый ОписаниеОповещения("ОткрытьВерсиюФайлаПослеПолученияФайлаВРабочийКаталог", ЭтотОбъект,
			ПараметрыВыполнения);
		ПолучитьФайлВерсииВРабочийКаталог(
			Обработчик, ПараметрыВыполнения.ДанныеФайла, "", ПараметрыВыполнения.УникальныйИдентификатор);
	Иначе
...
Процедура ОсвободитьФайлыПоСсылкам(ОбработчикРезультата, Знач МассивФайлов)
	...
	Обработчик = Новый ОписаниеОповещения("ОсвободитьФайлыПоСсылкамПослеУстановкиРасширения", ЭтотОбъект,
		ПараметрыОбработчика);
...
Процедура ОсвободитьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеДляМассиваФайлов(ПараметрыВыполнения.МассивФайлов,
		ПараметрыВыполнения.ДанныеФайлов);
	...
	ПоказатьВопрос(
		Обработчик, НСтр("ru = 'Отмена редактирования файлов может
...
Процедура ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Отменить редактирование файлов'"), , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отменено редактирование файлов (%1 из %2).'"), ПараметрыВыполнения.ДанныеФайлов.Количество(),
		ПараметрыВыполнения.МассивФайлов.Количество()), БиблиотекаКартинок.Информация32);
...
Процедура ОсвободитьФайл(ПараметрыОсвобожденияФайла)
	...
	Обработчик = Новый ОписаниеОповещения("ОсвободитьФайлПослеУстановкиРасширения", ЭтотОбъект,
		ПараметрыОсвобожденияФайла);
	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Обработчик);
...
Процедура ОсвободитьФайлПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ХранитьВерсии = Неопределено Тогда
		...
		ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла(
			?(ПараметрыВыполнения.ОбъектСсылка <> Неопределено, ПараметрыВыполнения.ОбъектСсылка,
	...
	Если ПараметрыВыполнения.НеЗадаватьВопрос = Ложь Тогда
		...
		Обработчик = Новый ОписаниеОповещения("ОсвободитьФайлПослеОтветаНаВопросОтменаРедактирования", ЭтотОбъект,
			ПараметрыВыполнения);
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отмена редактирования файла
				 ...
				 |
				 |Продолжить?'"), Строка(ПараметрыВыполнения.ОбъектСсылка));
...
Процедура ОсвободитьФайлПослеОтветаНаВопросОтменаРедактирования(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПродолжитьРаботу Тогда
		...
		Если Не ПараметрыВыполнения.НеЗадаватьВопрос Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Файл освобожден'"), ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии, БиблиотекаКартинок.Информация32);
		КонецЕсли;
...
Процедура СохранитьИзмененияФайла(ПараметрыОбновленияФайла)
	...
	Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаПослеУстановкиРасширения", ЭтотОбъект,
...
Процедура СохранитьИзмененияФайлаСРасширением(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ФайлНовойВерсии.Существует() Тогда
		...
		Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаСРасширениемПослеОтветаНаВопросОсвободитьФайл",
			ЭтотОбъект, ПараметрыВыполнения);
		ПоказатьВопрос(Обработчик, СтрокаПредупреждения, РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаСРасширениемПослеПомещенияФайлаНаСервер",
			ЭтотОбъект, ПараметрыВыполнения);
...
Процедура СохранитьИзмененияФайлаСРасширениемПослеПроверкиНовойВерсии(ПараметрыВыполнения)
	...
	ПараметрыВыполнения.Вставить("ОповещениеПослеШифрования",
		Новый ОписаниеОповещения("СохранитьИзмененияФайлаСРасширениемПослеПроверкиЗашифрован", ЭтотОбъект,
		ПараметрыВыполнения));
...
Процедура СохранитьИзмененияФайлаСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
	...
	ВерсияОбновлена = РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияФайла(
		ПараметрыВыполнения.ДанныеФайла.Ссылка, СведенияОФайле, НеМенятьЗаписьВРабочемКаталоге,
		ОтносительныйПутьКФайлу, ПараметрыВыполнения.ПолныйПутьКФайлу,
		ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца <> "", ПараметрыВыполнения.ИдентификаторФормы);
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ВерсияОбновлена Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Новая версия сохранена'"), ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии, БиблиотекаКартинок.Информация32);
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Новая версия не сохранена'"), , НСтр("ru = 'Файл не изменился'"),
				БиблиотекаКартинок.Информация32);
			Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаСРасширениемПослеПоказаОповещения",
				ЭтотОбъект, ПараметрыВыполнения);
			ПоказатьИнформациюФайлНеБылИзменен(Обработчик);
			Возврат;
		КонецЕсли;
...
Процедура СохранитьИзмененияФайлаБезРасширения(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаБезРасширенияПослеПомещенияФайлаНаСервер",
			ЭтотОбъект, ПараметрыВыполнения);
...
Процедура СохранитьИзмененияФайлаБезРасширенияПослеПроверкиНовойВерсии(ПараметрыВыполнения)
	...
	Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаБезРасширенияПослеНапоминания", ЭтотОбъект,
		ПараметрыВыполнения);
	ПоказатьНапоминаниеПередПоместитьФайл(Обработчик);
	...
	Если Результат.Значение = КодВозвратаДиалога.ОК Тогда
		...
		Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("БольшеНеЗадаватьЭтотВопрос")
			...
			ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить(
				"НастройкиПрограммы", "ПоказыватьПодсказкиПриРедактированииФайлов", Ложь, , , Истина);
		КонецЕсли;
		Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаБезРасширенияПослеЗагрузкиФайла", ЭтотОбъект,
			ПараметрыВыполнения);
		НачатьПомещениеФайла(Обработчик, , ПараметрыВыполнения.ПолныйПутьКФайлу, ,
			ПараметрыВыполнения.ИдентификаторФормы);
...
// Продолжение процедуры (см. выше).
Процедура СохранитьИзмененияФайлаБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла,
	ПараметрыВыполнения) Экспорт
...
Процедура СохранитьИзмененияФайлаБезРасширенияПослеШифрованияФайла(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Результат = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИСохранитьИзмененияФайла(
		ПараметрыВыполнения.ОбъектСсылка, СведенияОФайле, "", ПараметрыВыполнения.ПолныйПутьКФайлу, Ложь,
		ПараметрыВыполнения.ИдентификаторФормы);
	ПараметрыВыполнения.ДанныеФайла = Результат.ДанныеФайла;
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(
...
Функция СертификатыНеУказаны(МассивСертификатов)
	...
	Если МассивСертификатов.Количество() = 0 Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'У зашифрованного файла не указаны сертификаты.
...
Процедура ДобавитьФайлыЗавершение(ПрисоединенныйФайл, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ОбработчикРезультата") И ДополнительныеПараметры.ОбработчикРезультата
		<> Неопределено Тогда
		ПрисоединенныеФайлыМассив = Новый Массив;
		ПрисоединенныеФайлыМассив.Добавить(ПрисоединенныйФайл);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикРезультата, ПрисоединенныеФайлыМассив);
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось удалить файл в рабочем каталоге:
					 ...
					 |
					 |%2'"), ПолноеИмяФайла, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
...
//
Процедура ПолучитьФайлВерсииВЛокальныйКэшФайлов(ОбработчикРезультата, ДанныеФайла, НаЧтение, ИдентификаторФормы,
	ДополнительныеПараметры)
	...
	ФайлВРабочемКаталоге = ФайлНаходитсяВЛокальномКэшеФайлов(ДанныеФайла, ДанныеФайла.Версия,
		ПараметрыВыполнения.ПолноеИмяФайла, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
	...
	Обработчик = Новый ОписаниеОповещения("ПолучитьФайлВерсииВЛокальныйКэшФайловПослеВыбораДействия", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура ПолучитьФайлВерсииВЛокальныйКэшФайловПослеВыбораДействия(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат = "ВзятьИзХранилищаИОткрыть" Тогда
		...
		Обработчик = Новый ОписаниеОповещения("ПолучитьФайлВерсииВЛокальныйКэшФайловПослеУдаления", ЭтотОбъект,
			ПараметрыВыполнения);
	...
	ИначеЕсли Результат = "ОткрытьСуществующий" Тогда
		...
		Если ПараметрыВыполнения.ДанныеФайла.ВРабочемКаталогеНаЧтение <> ПараметрыВыполнения.НаЧтение Тогда
			...
			ПеререгистрироватьВРабочемКаталоге(
				ПараметрыВыполнения.ДанныеФайла.Версия, ПараметрыВыполнения.ПолноеИмяФайла, ПараметрыВыполнения.НаЧтение,
...
Процедура ПолучитьФайлВерсииВРабочийКаталог(ОбработчикРезультата, ДанныеФайла, ПолноеИмяФайла,
	...
	Если ДанныеФайла.РабочийКаталогВладельца = "" Или ДанныеФайла.Версия <> ДанныеФайла.ТекущаяВерсия
		И ЗначениеЗаполнено(ДанныеФайла.ТекущаяВерсия) Тогда
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы,
	...
	Если ТипЗнч(Контекст.ДанныеФайла) <> Тип("Структура") Или Не ЗначениеЗаполнено(
		Контекст.ДанныеФайла.СсылкаНаДвоичныеДанныеФайла) Тогда
	...
	Контекст.Вставить("ЗаголовокОшибки", НСтр("ru = 'Не удалось получить файл на компьютер из программы по причине:'")
		+ Символы.ПС);

	Если Контекст.ДляРедактирования И Контекст.ДанныеФайла.Редактирует
		<> ПользователиКлиент.АвторизованныйПользователь() Тогда
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(
		Новый ОписаниеОповещения("ПолучитьПрисоединенныйФайлПослеПодключенияРасширения", ЭтотОбъект, Контекст));
...
Процедура ПолучитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		Результат = Новый Структура;
		Результат.Вставить("ПолноеИмяФайла", "");
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки + НСтр(
			"ru = 'Не установлено расширение для работы с 1С:Предприятием.'"));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;

	ПолучитьРабочийКаталогПользователя(
		Новый ОписаниеОповещения("ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога", ЭтотОбъект, Контекст));
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки + НСтр(
		"ru = 'Создание каталога не выполнено по причине:'"));
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки + НСтр(
		"ru = 'Изменение свойства файла ""Только просмотр"" не выполнено по причине:'"));
	ДействияСФайлом.Добавить(Действие);
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки + НСтр(
		"ru = 'Установка свойств файла не выполнена по причине:'"));
	ДействияСФайлом.Добавить(Действие);

	ОбработатьФайл(Новый ОписаниеОповещения("ПолучитьПрисоединенныйФайлПослеОбработкиФайла", ЭтотОбъект, Контекст),
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы,
	ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки", НСтр("ru = 'Не удалось поместить файл с компьютера в программу по причине:'")
		+ Символы.ПС);

	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(
		Новый ОписаниеОповещения("ПоместитьПрисоединенныйФайлПослеПодключенияРасширения", ЭтотОбъект, Контекст));
...
Процедура ПоместитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	ПолучитьРабочийКаталогПользователя(
		Новый ОписаниеОповещения("ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталога", ЭтотОбъект, Контекст));
...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Файл.НачатьПроверкуСуществования(
		Новый ОписаниеОповещения("ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталогаПродолжение", ЭтотОбъект,
		Контекст));
...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталогаПродолжение(ФайлСуществует, Контекст) Экспорт
	...
	Если Контекст.ПолноеИмяФайла <> Контекст.ПолноеИмяПомещаемогоФайла Тогда
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки + НСтр(
			"ru = 'Копирование файла не выполнено по причине:'"));
		ДействияСФайлом.Добавить(Действие);
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки + НСтр(
		"ru = 'Получение свойств файла не выполнено по причине:'"));
	ДействияСФайлом.Добавить(Действие);
	...
	НачатьЗапросРазрешенияПользователя(Новый ОписаниеОповещения("ПоместитьПрисоединенныйФайлПослеПолученияРазрешений",
		ЭтотОбъект, Контекст), Вызовы);
КонецПроцедуры
...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если РазрешенияПолучены Тогда
		ОбработатьФайл(Новый ОписаниеОповещения("ПоместитьПрисоединенныйФайлПослеОбработкиФайла", ЭтотОбъект, Контекст),
...
Процедура ПоместитьПрисоединенныйФайлПослеОбработкиФайла(РезультатДействий, Контекст) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки + Символы.ПС + КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке));
	КонецПопытки;
...
#КонецЕсли
	...
	Обработчик = Новый ОписаниеОповещения("КаталогФайлаПослеОтветаНаВопросПолучитьФайл", ЭтотОбъект,
		ПараметрыОбработчика);
	...
	ПоказатьВопрос(Обработчик, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Каталог файла не существует. Возможно, на данном компьютере файл ""%1"" еще не открывался.
																							|Сохранить файл на компьютер и открыть его каталог?'"),
		ИмяФайла), КнопкиВопроса);
...
Процедура КаталогФайлаПослеОтветаНаВопросПолучитьФайл(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Обработчик = Новый ОписаниеОповещения("КаталогФайлаПослеПолученияФайлаВРабочийКаталог", ЭтотОбъект,
			ПараметрыВыполнения);
		ПолучитьФайлВерсииВРабочийКаталог(Обработчик, ПараметрыВыполнения.ДанныеФайла,
			ПараметрыВыполнения.ПолноеИмяФайла);
	Иначе
	...
	Если ПараметрыВыполнения.ПолноеИмяФайлаИзРегистра <> "" Тогда
		...
		Если Не ВРабочемКаталогеВладельца Или УдалитьВРабочемКаталоге = Истина Тогда
			...
			Если ФайлНаДиске.Существует() Тогда
				...
				ОбработчикЗавершения = Новый ОписаниеОповещения("УдалитьФайлИзРабочегоКаталогаПослеУдаленияФайла",
					ЭтотОбъект);
...
#Если ВебКлиент Тогда
	...
	ВернутьРезультатПослеПоказаПредупреждения(
		ОбработчикРезультата, НСтр("ru = 'Очистка рабочего каталога возможна только в тонком клиенте.'"), Неопределено);
	Возврат;
#КонецЕсли
...
#Если ВебКлиент Тогда
	ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр(
		"ru = 'Очистка рабочего каталога возможна только в приложении (тонкий клиент).'"), Неопределено);
	Возврат;
#КонецЕсли
...
Процедура ОчиститьРабочийКаталогДляОткрываемогоФайла(ПараметрыВыполнения)
	...
	Для Каждого Элемент Из ПараметрыВыполнения.ТаблицаФайлов Цикл
		...
		Если ПараметрыВыполнения.СколькоНадоОсвободитьМеста > 0 Тогда
			...
			Если ПараметрыВыполнения.СколькоОсталось < ПараметрыВыполнения.МаксРазмер
				- ПараметрыВыполнения.СколькоНадоОсвободитьМеста Тогда
		...
		Если Индекс % 10 = 0 Тогда
			Прогресс = ?(ПараметрыВыполнения.СколькоНадоОсвободитьМеста > 0,
				(ПараметрыВыполнения.СколькоНадоОсвободитьМеста - ПараметрыВыполнения.СколькоОсталось) * 100
				/ ПараметрыВыполнения.СколькоНадоОсвободитьМеста, Индекс * 100
				/ ПараметрыВыполнения.ТаблицаФайлов.Количество());
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайлов(Знач ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ПолноеИмяФайла = "" Тогда
		...
		Если НРег(ПараметрыВыполнения.ДанныеФайла.Расширение) = "xls" Или НРег(
			ПараметрыВыполнения.ДанныеФайла.Расширение) = "xlsx" Тогда
...
#Если Не ВебКлиент Тогда
			Если СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути Тогда
				...
				МаксимальнаяДлинаИмениФайла = ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - СтрДлина(
					ПутьКаталогаПользователя);
				...
				Если СтрДлина(ПараметрыВыполнения.ИмяФайла) > МаксимальнаяДлинаИмениФайла Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Длина пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов
							 |%2'"), ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
						ПараметрыВыполнения.ПолноеИмяФайла);
					ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК + НСтр(
						"ru = 'Измените имя файла на более короткое.'");
					ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения,
						ПараметрыВыполнения);
					Возврат;
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПредложитьВыборКаталога(Ответ, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Длина пути к файлу превышает %1 символов:
			 |%2
			 |
			 |Выбрать другой основной рабочий каталог?'"), ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
		ПараметрыВыполнения.ПолноеИмяФайла);
	Обработчик = Новый ОписаниеОповещения("ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога",
		ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.ПолноеИмяФайла = ПараметрыВыполнения.ИмяКаталога + РаботаСФайламиСлужебныйКлиентСервер.УникальноеИмяСПутем(
		ПараметрыВыполнения.ИмяКаталога, ПараметрыВыполнения.ИмяФайла);
	
	// уложились в 260 символов
	Если СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) <= ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути Тогда
		Обработчик = Новый ОписаниеОповещения("ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПослеПереносаСодержимогоРабочегоКаталога",
			ЭтотОбъект, ПараметрыВыполнения);
		ПеренестиСодержимоеРабочегоКаталога(Обработчик, ПараметрыВыполнения.ИмяКаталогаПрежнееЗначение,
...
// Продолжение процедуры (см. выше).
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПослеПереносаСодержимогоРабочегоКаталога(СодержимоеПеренесено,
	ПараметрыВыполнения) Экспорт
...
#КонецЕсли
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		...
		ОбработчикПродолжения = Новый ОписаниеОповещения("ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПослеРасшифровки",
			ЭтотОбъект, ПараметрыВыполнения);
...
// Продолжение процедуры (см. выше).
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПередачаФайла(ПараметрыВыполнения,
...
//
Процедура ПолучитьФайлВерсииВРабочийКаталогПапки(ОбработчикРезультата, ДанныеФайла, ПолноеИмяФайла, НаЧтение,
	ИдентификаторФормы, ДополнительныеПараметры)
...
Процедура ПолучитьФайлВерсииВРабочийКаталогПапкиПродолжение(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ФайлЕстьВРегистре И Не ФайлНаДискеСуществует Тогда
		ПолучитьССервераИЗарегистрироватьВРабочемКаталогеПапки(
			ПараметрыВыполнения.ОбработчикРезультата, ПараметрыВыполнения.ДанныеФайла,
			ПараметрыВыполнения.ПолноеИмяФайла, ПараметрыВыполнения.ДанныеФайла.ДатаМодификацииУниверсальная,
			ПараметрыВыполнения.НаЧтение, ПараметрыВыполнения.ИдентификаторФормы,
	...
	Если ПараметрыВыполнения.ФайлЕстьВРегистре И Версия <> ПараметрыВыполнения.ДанныеФайла.ТекущаяВерсия Тогда
		...
		Если ПараметрыВыполнения.ДанныеФайла.Владелец = ВРегистреПапка Тогда // Одна и та же папка.
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В рабочем каталоге на компьютере уже есть файл
					 ...
					 |
					 |Рекомендуется переименовать один из файлов в программе.'"), ПараметрыВыполнения.ПолноеИмяФайла);
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В рабочем каталоге на компьютере уже есть файл
					 |""%1"",
		...
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстПредупреждения,
			ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	...
	// Проверка даты изменения и принятие решения, что делать дальше.
	Обработчик = Новый ОписаниеОповещения("ПолучитьФайлВерсииВРабочийКаталогПапкиПослеВыбораДействия", ЭтотОбъект,
		ПараметрыВыполнения);

	ДействиеПриОткрытииФайлаВРабочемКаталоге(
		Обработчик, ПараметрыВыполнения.ПолноеИмяФайла, ПараметрыВыполнения.ДанныеФайла);
...
//
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталоге(ОбработчикРезультата, ДанныеФайла, ПолноеИмяФайла,
	НормальноеИмяФайла)
...
#КонецЕсли
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Длина полного пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов
			 |""%2"".'"), ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути, ПараметрыВыполнения.ПолноеИмяФайла);
	...
	Если СтрДлина(ДанныеФайла.РабочийКаталогВладельца) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК + НСтр(
			"ru = 'Измените имена папок или перенесите текущую папку в другую папку.'");
		ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеПредложитьВыборКаталога(ПараметрыВыполнения)
	...
	Обработчик = Новый ОписаниеОповещения("ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеНачатьВыборКаталога",
		ЭтотОбъект, ПараметрыВыполнения);
	ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	КаталогВыбран = ВыбратьПутьКРабочемуКаталогу(ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца, Заголовок,
		Истина);
	Если Не КаталогВыбран Тогда
		ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, Ложь);
		Возврат;
	...
	ПараметрыВыполнения.ПолноеИмяФайла = ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца
		+ ПараметрыВыполнения.НормальноеИмяФайла;
	
	// уложились в 260 символов
	Если СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) <= ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути Тогда
		Обработчик = Новый ОписаниеОповещения("ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеПослеПереносаСодержимогоРабочегоКаталога",
			ЭтотОбъект, ПараметрыВыполнения);
		ПеренестиСодержимоеРабочегоКаталога(Обработчик, ПараметрыВыполнения.ИмяКаталогаПрежнееЗначение,
...
// Продолжение процедуры (см. выше).
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеПослеПереносаСодержимогоРабочегоКаталога(СодержимоеПеренесено,
	ПараметрыВыполнения) Экспорт
	...
	Если СодержимоеПеренесено Тогда
		...
		РаботаСФайламиСлужебныйВызовСервера.СохранитьРабочийКаталогПапкиИЗаменитьПутиВРегистре(
			ПараметрыВыполнения.ДанныеФайла.Владелец, ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца,
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось скопировать файл
				 ...
				 |Возможно он занят другим приложением.
				 |
				 |Повторить операцию?'"), Результат.ПолноеИмяСбойногоФайла);
		...
		Обработчик = Новый ОписаниеОповещения("СкопироватьСодержимоеКаталогаПослеОтветаНаВопрос", ЭтотОбъект,
			ПараметрыВыполнения);
...
Процедура СкопироватьСодержимоеКаталогаПослеОтветаНаВопрос(Ответ, ПараметрыВыполнения) Экспорт
	...
	Иначе
		СкопироватьСодержимоеКаталога(
			ПараметрыВыполнения.ОбработчикРезультата, ПараметрыВыполнения.КаталогИсточника,
	...
	Если СтрНайти(НРег(КаталогПриемника), НРег(КаталогИсточника)) <> 0 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбранный рабочий каталог
				 ...
				 |""%2"".'"), КаталогПриемника, КаталогИсточника);
...
Процедура ПеренестиСодержимоеРабочегоКаталогаПослеКопированияВНовыйКаталог(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		Обработчик = Новый ОписаниеОповещения("ПеренестиСодержимоеРабочегоКаталогаПослеОтменыИОчисткиПриемника",
			ЭтотОбъект, ПараметрыВыполнения);
	...
	Иначе
		...
		Обработчик = Новый ОписаниеОповещения("ПеренестиСодержимоеРабочегоКаталогаПослеУспехаИОчисткиИсточника",
			ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ПеренестиСодержимоеРабочегоКаталогаПослеУспехаИОчисткиИсточника(КаталогИсточникаОчищен, ПараметрыВыполнения) Экспорт
	...
	Иначе
		...
		Обработчик = Новый ОписаниеОповещения("ПеренестиСодержимоеРабочегоКаталогаПослеУспехаИОтменыОчистки",
			ЭтотОбъект, ПараметрыВыполнения);
		СкопироватьСодержимоеКаталога(Обработчик, ПараметрыВыполнения.КаталогПриемника,
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось скопировать обратно содержимое каталога
				 |""%1""
				 |в каталог
				 |""%2"".'"), ПараметрыВыполнения.КаталогПриемника, ПараметрыВыполнения.КаталогИсточника);
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось удалить файл в рабочем каталоге:
					 ...
					 |Возможно он занят другим приложением.
					 |
					 |%2'"), Путь, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
				"Предупреждение", ТекстСообщения, , Истина);
		КонецПопытки;
	КонецЦикла;
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, НСтр(
			"ru = 'Нет файлов для добавления'"), Неопределено);
		Возврат;
	КонецЕсли;
	...
	ОбработчикЗавершения = Новый ОписаниеОповещения("ИмпортФайловЦиклПродолжитьИмпортПослеВопросовВРекурсии",
		ЭтотОбъект);
	ЗарегистрироватьОбработчикЗавершения(ПараметрыВыполнения, ОбработчикЗавершения);
...
Процедура ИмпортФайловЦикл(ПараметрыВыполнения)
	...
	Для Индекс = ПараметрыВыполнения.ВыбранныеФайлыИндекс По ПараметрыВыполнения.ВыбранныеФайлыВГраница Цикл
		...
		Если ВыбранКаталог Тогда
			...
			ПараметрыВыполнения.МассивФайловЭтогоКаталога = НайтиФайлыПсевдо(ПараметрыВыполнения.ПсевдоФайловаяСистема,
				ПараметрыВыполнения.Путь);
			...
			Если РаботаСФайламиСлужебныйВызовСервера.ЕстьПапкаСТакимИменем(
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Папка ""%1"" уже существует.
						 |
						 |Продолжить импорт папки?'"), ПараметрыВыполнения.ИмяПапки);
				Обработчик = Новый ОписаниеОповещения("ИмпортФайловЦиклПослеОтветаНаВопросПродолжить", ЭтотОбъект,
					ПараметрыВыполнения);
				ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
...
Процедура ИмпортФайловЦиклПродолжитьИмпорт(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ПапкаУжеНайдена Тогда
		...
		РабочийКаталог  = ПараметрыВыполнения.Путь + ?(Прав(ПараметрыВыполнения.Путь, 1) = РазделительПути, "",
			РазделительПути);
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Импорт файлов завершен. Импортировано файлов: %1'"), Строка(
			ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()));
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() = 1 Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Изменение:'"), Ссылка, Элемент0.Файл, БиблиотекаКартинок.Информация32);
	КонецЕсли;
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось удалить файл после добавления в программу:
					 ...
					 |Возможно он занят другим приложением.
					 |
					 |%2'"), ВыбранныйФайл.ПолноеИмя, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
				"Предупреждение", ТекстСообщения, , Истина);
		КонецПопытки;
	КонецЦикла;
...
Процедура СохранитьКакСРасширением(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда
		...
		ФайлВРабочемКаталоге = ФайлНаходитсяВЛокальномКэшеФайлов(ПараметрыВыполнения.ДанныеФайла,
			ПараметрыВыполнения.ДанныеФайла.Версия, ПараметрыВыполнения.ПолноеИмяФайла, ВРабочемКаталогеНаЧтение,
			ВРабочемКаталогеВладельца);
			...
			Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны).
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Дата изменения файла ""%1""
						 ...
						 |Возможно, файл на компьютере был отредактирован.'"), Строка(
					ПараметрыВыполнения.ДанныеФайла.Ссылка));

				ПараметрыОткрытияФормы.Вставить("Сообщение", Сообщение);

				Обработчик = Новый ОписаниеОповещения("СохранитьКакСРасширениемПослеОтветаНаВопросДатаНовее",
					ЭтотОбъект, ПараметрыВыполнения);
				ОткрытьФорму("Обработка.РаботаСФайлами.Форма.РежимСозданияФайлаДляСохранитьКак",
					ПараметрыОткрытияФормы, , , , , Обработчик, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьКакСРасширениемПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		Обработчик = Новый ОписаниеОповещения("СохранитьКакСРасширениемПослеВыбораРежимаСохранения", ЭтотОбъект,
			ПараметрыВыполнения);

		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВыборСохраненияШифрованногоФайла", , , , , , Обработчик,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	Иначе
...
Процедура СохранитьКакСРасширениемПослеВыбораРежимаСохранения(Результат, ПараметрыВыполнения) Экспорт
	...
	ОбработчикПродолжения = Новый ОписаниеОповещения("СохранитьКакСРасширениемПослеРасшифровки", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Иначе
		Если ПараметрыВыполнения.ДанныеФайла.Свойство("СсылкаНаДвоичныеДанныеФайла") Тогда
			АдресФайла = ПараметрыВыполнения.ДанныеФайла.СсылкаНаДвоичныеДанныеФайла;
			ПараметрыВыполнения.ДанныеФайла.Вставить("ПолноеНаименованиеВерсии",
				ПараметрыВыполнения.ДанныеФайла.Наименование);
		Иначе
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован И Не ПараметрыВыполнения.СохранятьСРасшифровкой Тогда
	...
	Если ПутьВыбораПрежний <> ПараметрыВыполнения.ПутьВыбора Тогда
		ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("НастройкиПрограммы", "ПапкаДляСохранитьКак",
			ПараметрыВыполнения.ПутьВыбора);
	КонецЕсли;
...
Процедура СохранитьКакБезРасширения(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		Обработчик = Новый ОписаниеОповещения("СохранитьКакБезРасширенияПослеВыбораРежимаСохранения", ЭтотОбъект,
			ПараметрыВыполнения);
...
Процедура СохранитьКакБезРасширенияЗавершение(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Не ПараметрыВыполнения.СохранятьСРасшифровкой И ПараметрыВыполнения.ДанныеФайла.Зашифрован И ЗначениеЗаполнено(
		ПараметрыВыполнения.РасширениеДляЗашифрованныхФайлов) Тогда
...
Процедура СохранитьФайлКакПослеСохранения(Результат, ОбработчикЗавершения) Экспорт
	...
	ПутьКФайлу = "";
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("ПолноеИмяФайла") Тогда
...
Процедура ПоказатьНапоминаниеПередПоместитьФайл(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если Не РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = НСтр("ru = 'Сейчас будет предложено выбрать файл для того,
			...
			ПараметрыНапоминания.Вставить("Картинка", БиблиотекаКартинок.Информация32);
			ПараметрыНапоминания.Вставить("ТекстФлажка", НСтр("ru = 'Больше не показывать это сообщение'"));
			ПараметрыНапоминания.Вставить("Заголовок", НСтр("ru = 'Помещение файла'"));
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
				ОбработчикРезультата, ТекстНапоминания, Кнопки, ПараметрыНапоминания);
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки)
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2 МБ)'"), Строка(
				Файл), Строка(РазмерФайлаВМб));
		...
		Обработчик = Новый ОписаниеОповещения("ПроверитьПредельныйРазмерФайловПослеОтветаНаВопрос", ЭтотОбъект,
			ПараметрыВыполнения);
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВопросПриИмпортеФайлов", Параметры, , , , , Обработчик,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьИнформациюФайлНеБылИзменен(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьИнформациюЧтоФайлНеБылИзменен Тогда
		...
		ПараметрыНапоминания.Вставить("Картинка", БиблиотекаКартинок.Информация32);
		ПараметрыНапоминания.Вставить("ТекстФлажка", НСтр("ru = 'Больше не показывать это сообщение'"));
		ПараметрыНапоминания.Вставить("Заголовок", НСтр("ru = 'Информация'"));
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
			ОбработчикРезультата, ТекстНапоминания, Кнопки, ПараметрыНапоминания);
	Иначе
...
Процедура ЗакончитьРедактированиеСОповещениемЗавершение(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат = Истина Тогда
		...
		Оповестить("Запись_Файл", Новый Структура("Событие", "ЗаконченоРедактирование"),
			ПараметрыВыполнения.ПараметрКоманды);
		ОповеститьОбИзменении(ПараметрыВыполнения.ПараметрКоманды);
		Оповестить("Запись_Файл", Новый Структура("Событие", "ДанныеФайлаИзменены"),
			ПараметрыВыполнения.ПараметрКоманды);
...
Процедура ЗакончитьРедактированиеПоСсылкамСОповещением(Параметры) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеПоСсылкамПослеУстановкиРасширения", ЭтотОбъект,
		Параметры);
	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Обработчик);
...
Процедура ЗакончитьРедактированиеПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПараметрыОбработчика = ПараметрыОбновленияФайла(Неопределено, ДанныеФайла.Ссылка,
			ПараметрыВыполнения.ИдентификаторФормы);
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Завершить редактирование файлов'"), , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершено редактирование файлов (%1 из %2).'"), ПараметрыВыполнения.ДанныеФайлов.Количество(),
		ПараметрыВыполнения.МассивФайлов.Количество()), БиблиотекаКартинок.Информация32);

	ОбработчикРезультата = Новый ОписаниеОповещения("ЗакончитьРедактированиеМассиваФайловСОповещениемЗавершение",
		ЭтотОбъект, ПараметрыВыполнения);
	ВернутьРезультат(ОбработчикРезультата, Истина);
...
Процедура ДобавитьФайлыПеретаскиванием(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач МассивИменФайлов) Экспорт
	...
	ПрисоединенныеФайлыМассив = Новый Массив;
	ПоместитьВыбранныеФайлыВХранилище(
		МассивИменФайлов, ВладелецФайла, ПрисоединенныеФайлыМассив, ИдентификаторФормы);
...
Процедура РедактироватьСОповещением(ОбработчикРезультата, ОбъектСсылка, УникальныйИдентификатор = Неопределено,
...
Процедура ЗанятьСОповещением(ОбработчикРезультата, ПараметрКоманды, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Массив") Тогда
		Обработчик = Новый ОписаниеОповещения("ЗанятьСОповещениемМассивФайловЗавершение", ЭтотОбъект,
			ПараметрыВыполнения);
...
Процедура ОсвободитьФайлСОповещением(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры.ОбъектСсылка) = Тип("Массив") Тогда
		Обработчик = Новый ОписаниеОповещения("ОсвободитьФайлСОповещениемМассивФайловЗавершение", ЭтотОбъект,
			ПараметрыВыполнения);
		ОсвободитьФайлыПоСсылкам(Обработчик, Параметры.ОбъектСсылка);
	Иначе
		Обработчик = Новый ОписаниеОповещения("ОсвободитьФайлСОповещениемОдинФайлЗавершение", ЭтотОбъект,
			ПараметрыВыполнения);
...
//   УникальныйИдентификатор - УникальныйИдентификатор - формы.
//
Процедура ОткрытьФайлСОповещением(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено,
	ДляРедактирования = Истина) Экспорт
	...
	Если Не ПараметрыВыполнения.ДанныеФайла.Свойство("Владелец") Или Не ЗначениеЗаполнено(
		ПараметрыВыполнения.ДанныеФайла.Владелец) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФайлРасширениеПредложено", РаботаСФайламиСлужебныйКлиент,
			ПараметрыВыполнения);
	...
	Обработчик = Новый ОписаниеОповещения("ОткрытьФайлСОповещениемПослеУстановкиРасширения", ЭтотОбъект,
		ПараметрыВыполнения);
	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Обработчик);
...
Процедура ОткрытьФайлСОповещениемПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено() Тогда
		Обработчик = Новый ОписаниеОповещения("ОткрытьФайлСОповещениемСРасширениемПослеПолученияВерсииВРабочийКаталог",
			ЭтотОбъект, ПараметрыВыполнения);
		ПолучитьФайлВерсииВРабочийКаталог(
			Обработчик, ПараметрыВыполнения.ДанныеФайла, "", ПараметрыВыполнения.УникальныйИдентификатор,
...
КонецПроцедуры
Процедура ОткрытьФайлБезРасширения(Оповещение, ДанныеФайла, ИдентификаторФормы, СПредупреждением = Истина,
	СохранятьСРасшифровкой = Истина, РасширениеДляЗашифрованныхФайлов = "")
	...
	Если Контекст.СохранятьСРасшифровкой И ДанныеФайла.Зашифрован Тогда
		...
		ОбработчикПродолжения = Новый ОписаниеОповещения("ОткрытьФайлБезРасширенияПослеРасшифровки", ЭтотОбъект,
			Контекст);
...
Процедура ОткрытьФайлБезРасширенияПослеРасшифровки(ОписаниеДанных, Контекст) Экспорт
	...
	Если ТипЗнч(ОписаниеДанных.РасшифрованныеДанные) = Тип("ДвоичныеДанные") Тогда
		АдресФайла = ПоместитьВоВременноеХранилище(ОписаниеДанных.РасшифрованныеДанные, Контекст.ИдентификаторФормы);
	Иначе
		АдресФайла = ОписаниеДанных.РасшифрованныеДанные;
	КонецЕсли;
...
Процедура ОткрытьФайлБезРасширенияНапоминание(Контекст)
	...
	Если Контекст.СПредупреждением И Контекст.ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда

		ВывестиНапоминаниеПриРедактировании(Новый ОписаниеОповещения("ОткрытьФайлБезРасширенияПередачаФайла",
			ЭтотОбъект, Контекст));
	Иначе
...
Процедура ОткрытьФайлБезРасширенияПередачаФайла(Результат, Контекст) Экспорт
	...
	Если Не Контекст.СохранятьСРасшифровкой И Контекст.ДанныеФайла.Зашифрован И ЗначениеЗаполнено(
		Контекст.РасширениеДляЗашифрованныхФайлов) Тогда
...
Процедура СохранитьИзмененияФайлаСОповещением(ОбработчикРезультата, ПараметрКоманды, ИдентификаторФормы) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("СохранитьИзмененияФайлаСОповещениемЗавершение", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура ОбновитьИзФайлаНаДискеСОповещением(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы,
	...
	ПараметрыВыполнения.Вставить("ОбработчикРезультата", ОбработчикРезультата);
	ПараметрыВыполнения.Вставить("ДанныеФайла", ДанныеФайла);
	Обработчик = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеСОповещениемЗавершение", ЭтотОбъект,
		ПараметрыВыполнения);
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	МассивВерсий = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаИНавигационнаяСсылкаВсехВерсийФайла(
		ПараметрыВыполнения.ДанныеФайла.Ссылка, ПараметрыВыполнения.УникальныйИдентификатор);
	...
	Для Каждого СвойстваВерсии Из МассивВерсий Цикл
		...
		ЭлементДанных.Вставить("РазмещениеРезультата", Новый ОписаниеОповещения("ПриПолученииЗашифрованныхДанных",
			ЭтотОбъект, ТекущиеПараметрыВыполнения));
...
Процедура Расшифровать(ОбработчикРезультата, ФайлСсылка, УникальныйИдентификатор, ДанныеФайла) Экспорт
	...
	Для Каждого СвойстваВерсии Из МассивВерсий Цикл
		...
		ЭлементДанных.Вставить("РазмещениеРезультата", Новый ОписаниеОповещения("ПриПолученииРасшифрованныхДанных",
			ЭтотОбъект, ТекущиеПараметрыВыполнения));
...
#Если Не ВебКлиент Тогда
	Если Не ПараметрыВыполнения.ИзвлекатьТекстыФайловНаСервере Тогда
		...
		АдресВременногоХранилищаТекста = ИзвлечьТекстВоВременноеХранилище(ПолныйПутьКФайлу,
			ПараметрыВыполнения.УникальныйИдентификатор);
		Попытка
...
//
Процедура ДобавитьФайл(ОбработчикРезультата, ВладелецФайла, ФормаВладелец, РежимСоздания = 1,
	ПараметрыДобавления = Неопределено) Экспорт

	ПараметрыВыполнения = Новый Структура;
	Если ПараметрыДобавления = Неопределено Или ТипЗнч(ПараметрыДобавления) = Тип("Булево") Тогда

		ПараметрыВыполнения.Вставить("МаксимальныйРазмер", 0);
		ПараметрыВыполнения.Вставить("ФильтрДиалогаВыбора", НСтр("ru = 'Все файлы (*.*)|*.*'"));
		ПараметрыВыполнения.Вставить("НеОткрыватьКарточкуПослеСозданияИзФайла", ?(ПараметрыДобавления = Неопределено,
			Ложь, ПараметрыДобавления));
...
Процедура ДобавитьНаОсновеШаблонаПослеВыбораШаблона(Результат, ПараметрыВыполнения) Экспорт
	...
	ПараметрыФормы.Вставить("ИмяСправочникаХранилищаФайлов", ?(ПараметрыВыполнения.Свойство(
		"ИмяСправочникаХранилищаФайлов"), ПараметрыВыполнения.ИмяСправочникаХранилищаФайлов,
		РаботаСФайламиСлужебныйВызовСервера.ИмяСправочникаХраненияФайлов(ПараметрыВыполнения.ВладелецФайла)));

	ОписаниеОповещенияОЗакрытии = ОбработчикЗавершения(ПараметрыВыполнения.ОбработчикРезультата);
	РаботаСФайламиКлиент.СкопироватьФайл(ПараметрыВыполнения.ВладелецФайла, Результат, ПараметрыФормы,
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла,
	...
	Если ПараметрыВыполнения.Свойство("МаксимальныйРазмер") И ПараметрыВыполнения.МаксимальныйРазмер > 0 Тогда
...
Процедура ПрочитатьСертификатыПодписей(Форма) Экспорт
	...
	Контекст.Вставить("МодульЭлектроннаяПодписьКлиент", ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектроннаяПодписьКлиент"));
	;
	...
	НачатьПодключениеРасширенияРаботыСКриптографией(
		Новый ОписаниеОповещения("ПрочитатьСертификатыПодписейПослеПодключенияРасширения", ЭтотОбъект, Контекст));
...
Процедура ПрочитатьСертификатыПодписейПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Контекст.МодульЭлектроннаяПодписьКлиент.СоздатьМенеджерКриптографии(
		Новый ОписаниеОповещения("ПрочитатьСертификатыПодписейПослеСозданияМенеджераКриптографии", ЭтотОбъект, Контекст),
...
Процедура ПрочитатьСертификатыПодписейЦиклНачало(Контекст)
	...
	Контекст.МенеджерКриптографии.НачатьПолучениеСертификатовИзПодписи(
		Новый ОписаниеОповещения("ПрочитатьСертификатыПодписейЦиклПослеПолученияСертификатовИзПодписи", ЭтотОбъект,
		Контекст, "ПрочитатьСертификатыПодписейЦиклПослеОшибкиПолученияСертификатовИзПодписи", ЭтотОбъект), Подпись);

КонецПроцедуры

// Продолжение процедуры ПрочитатьСертификатыПодписей.
Процедура ПрочитатьСертификатыПодписейЦиклПослеОшибкиПолученияСертификатовИзПодписи(ИнформацияОбОшибке,
...
Процедура ПрочитатьСертификатыПодписейЦиклПослеПолученияСертификатовИзПодписи(Сертификаты, Контекст) Экспорт
	...
	Сертификат.НачатьВыгрузку(Новый ОписаниеОповещения("ПрочитатьСертификатыПодписейЦиклПослеВыгрузкиСертификата",
		ЭтотОбъект, Контекст, "ПрочитатьСертификатыПодписейЦиклПослеОшибкиВыгрузкиСертификата", ЭтотОбъект));

КонецПроцедуры

// Продолжение процедуры ПрочитатьСертификатыПодписей.
Процедура ПрочитатьСертификатыПодписейЦиклПослеОшибкиВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка,
	Контекст) Экспорт
...
Процедура ПрочитатьСертификатыПодписейЦиклПослеВыгрузкиСертификата(ДанныеСертификата, Контекст) Экспорт
	...
	СтрокаТаблицы.АдресСертификата = ПоместитьВоВременноеХранилище(ДанныеСертификата,
		Контекст.Форма.УникальныйИдентификатор);
	СтрокаТаблицы.КомуВыданСертификат = Контекст.МодульЭлектроннаяПодписьКлиент.ПредставлениеСубъекта(
		Контекст.Сертификат);
...
//  ФайлСсылка  - СправочникСсылка.Файлы - файл.
//
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления, ВладелецФайла, ФайлСсылка) Экспорт
...
//
Процедура ПодписатьФайл(ПрисоединенныйФайл, ДанныеФайла, ИдентификаторФормы, ОбработчикЗавершения = Неопределено,
	ОбработчикПриПолученииПодписи = Неопределено) Экспорт
	...
	Если Не ПроверитьВозможностьПодписания(ДанныеФайла, ОбработчикЗавершения, ОбработчикПродолжения,
...
Процедура ДобавитьПодписьИзФайла(Файл, ИдентификаторФормы, ОбработчикЗавершения) Экспорт
	...
	Если Не ПроверитьВозможностьПодписания(ПараметрыВыполнения.ДанныеФайла, ОбработчикЗавершения, ОбработчикЗавершения,
		ПараметрыВыполнения) Тогда
	...
	ОписаниеДанных.Вставить("Объект", Новый ОписаниеОповещения("ПриПолученииПодписей", ЭтотОбъект, ПараметрыВыполнения));

	ОбработчикПродолжения = Новый ОписаниеОповещения("ПослеПодписанияФайла", ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ПриПолученииПодписей(Параметры, Контекст) Экспорт
	...
	РаботаСФайламиСлужебныйВызовСервера.ДобавитьПодписьКФайлу(
		Контекст.ДанныеФайла.Ссылка, Параметры.ОписаниеДанных.Подписи, Контекст.ИдентификаторФормы);
...
Процедура СохранитьФайлВместеСПодписью(Файл, ИдентификаторФормы) Экспорт
	...
	ОписаниеДанных.Вставить("Данные", Новый ОписаниеОповещения("ПриСохраненииДанныхФайла", ЭтотОбъект,
		ПараметрыВыполнения));
...
Процедура ПриСохраненииДанныхФайла(Параметры, Контекст) Экспорт
	...
	ОбработчикОповещения = Новый ОписаниеОповещения("ПриСохраненииДанныхФайлаВернутьРезультат", ЭтотОбъект,
		ДополнительныеПараметры);
...
Процедура ИмпортФайловРекурсивно(Владелец, ВыбранныеФайлы, ПараметрыВыполнения)
	...
	ИмпортФайловРекурсивноБезДиалогов(СлужебныеПараметры.Владелец, СлужебныеПараметры.ВыбранныеФайлы,
		СлужебныеПараметры, Истина);
...
Процедура ИмпортФайловРекурсивноЗадатьСледующийВопрос(ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Папка ""%1"" уже существует.
			 |Продолжить импорт папки?'"), ПараметрыВыполнения.ПапкаДляДобавленияВВыбранныеФайлы.Имя);
...
Процедура ИмпортФайловРекурсивноПослеОтветаНаВопрос(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.МассивПапокДляВопросаПапкаУжеСуществует.Количество() = 0 Тогда
		ИмпортФайловРекурсивноБезДиалогов(ПараметрыВыполнения.Владелец, ПараметрыВыполнения.ВыбранныеФайлы,
			ПараметрыВыполнения, Ложь); // ЗадаватьВопросПапкаУжеСуществует (используется только для первого уровня рекурсии).
...
//
Процедура ИмпортФайловРекурсивноБезДиалогов(Знач Владелец, Знач ВыбранныеФайлы, Знач ПараметрыВыполнения,
	Знач ЗадаватьВопросПапкаУжеСуществует)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Попытка
			...
			Если ВыбранныйФайл.ЭтоКаталог() Тогда
				...
				Если ПараметрыВыполнения.Рекурсивно = Истина Тогда
					...
					Если МассивФайлов.Количество() <> 0 Тогда
						...
						Если РаботаСФайламиСлужебныйВызовСервера.ЕстьПапкаСТакимИменем(ИмяФайла, Владелец,
						...
						Если Не ПапкаУжеНайдена Тогда
							ПапкаФайловСсылка = РаботаСФайламиСлужебныйВызовСервера.СоздатьПапкуФайлов(ИмяФайла,
								Владелец);
						КонецЕсли;
			...
			// Считаем проценты
			ПараметрыВыполнения.Индикатор = Цел(ПараметрыВыполнения.Счетчик * 100
			...
			НадписьПодробнее = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обрабатывается файл ""%1"" (%2 Мб)...'"), ВыбранныйФайл.Имя,
				РаботаСФайламиСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));

			ТекстСостояния = НСтр("ru = 'Импорт файлов с диска...'");

			Состояние(ТекстСостояния, ПараметрыВыполнения.Индикатор, НадписьПодробнее, БиблиотекаКартинок.Информация32);
			...
			Если Не ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, ПараметрыВыполнения.ИдентификаторФормы) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка помещения файла
						 |""%1""
						 |во временное хранилище.'"), ВыбранныйФайл.ПолноеИмя);
			КонецЕсли;
...
Процедура ИмпортироватьФайл(Знач ВыбранныйФайл, Знач Владелец, Знач ПараметрыВыполнения,
	...
	Иначе
		...
		ФайлСсылка = РаботаСФайламиСлужебныйВызовСервера.ДобавитьФайл(ПараметрыФайла, АдресВременногоХранилищаФайла,
			АдресВременногоХранилищаТекста, ПараметрыВыполнения.Комментарий);
...
Процедура ОбновитьИнформациюВРабочемКаталоге(ТекущаяВерсия, НовоеИмя) Экспорт
	...
	ВРабочемКаталогеНаЧтение = Истина;
	ВРабочемКаталогеВладельца = Ложь;
	ФайлВРабочемКаталоге = ФайлНаходитсяВЛокальномКэшеФайлов(Неопределено, ТекущаяВерсия, ПолноеИмяФайла,
		ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
	Если Не ФайлВРабочемКаталоге Тогда
		Возврат;
	КонецЕсли;
	...
	РаботаСФайламиСлужебныйВызовСервера.ЗанестиИнформациюФайлаВРегистр(ТекущаяВерсия, НовоеПолноеИмя, ИмяКаталога,
...
Процедура ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла, ИдентификаторВладельца = Неопределено)
	...
	Если ТекстовыеФайлыСпособОткрытия = ПредопределенноеЗначение(
		"Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе") Тогда

		ТекстовыеФайлыРасширение = ПерсональныеНастройкиРаботыСФайлами.ТекстовыеФайлыРасширение;
		Если РаботаСФайламиСлужебныйКлиентСервер.РасширениеФайлаВСписке(ТекстовыеФайлыРасширение,
			ДанныеФайла.Расширение) Тогда
			...
			ОткрытьФорму("Обработка.РаботаСФайлами.Форма.РедактированиеТекстовогоФайла", ПараметрыФормы, ,
				ДанныеФайла.Ссылка);
			Возврат;
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПерсональныеНастройкиРаботыСФайлами = СтандартныеПодсистемыКлиент.ПараметрКлиента(
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок", НСтр(
		"ru = 'Следующие файлы заняты для редактирования:'"));
...
#Если Не ВебКлиент Тогда
	...
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		...
		Если ФайлЕстьВРегистре Тогда
			РедактируетТекущийПользователь = РаботаСФайламиСлужебныйВызовСервера.ПолучитьРедактируетТекущийПользователь(
				Версия);
...
Функция ПолучитьПутьФайлаВРабочемКаталоге(ДанныеФайла)
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда
		Если Не ПустаяСтрока(ДанныеФайла.РабочийКаталогВладельца) Тогда
			ПолноеИмяФайла = ДанныеФайла.РабочийКаталогВладельца + ДанныеФайла.ПолноеНаименованиеВерсии + "."
	...
	НаЧтение = Истина;
	ВРабочемКаталогеВладельца = ДанныеФайла.РабочийКаталогВладельца <> "";
	РаботаСФайламиСлужебныйВызовСервера.ЗаписатьПолноеИмяФайлаВРегистр(ДанныеФайла.Версия, ПолноеИмяФайла, НаЧтение,
		ВРабочемКаталогеВладельца);
...
Функция ФайлНаходитсяВЛокальномКэшеФайлов(ДанныеФайла, ТекущаяВерсия, ПолноеИмяФайла, ВРабочемКаталогеНаЧтение,
	...
	Иначе
		ВРабочемКаталогеНаЧтение = Истина;
		ИмяКаталога = РабочийКаталогПользователя();
		ПолноеИмяФайла = РаботаСФайламиСлужебныйВызовСервера.ПолноеИмяФайлаВРабочемКаталоге(ТекущаяВерсия, ИмяКаталога,
			ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
	КонецЕсли;
			...
			Если МассивФайловВКаталоге.Количество() <> 0 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В выбранном рабочем каталоге
						 ...
						 |уже есть файлы.
						 |
						 |Выберите другой каталог.'"), ИмяКаталога);
				ПоказатьПредупреждение( , ТекстОшибки);
				Возврат Ложь;
			КонецЕсли;
#КонецЕсли
...
Процедура ПеререгистрироватьВРабочемКаталоге(ТекущаяВерсия, ПолноеИмяФайла, НаЧтение, ВРабочемКаталогеВладельца)
	...
	РаботаСФайламиСлужебныйВызовСервера.ЗанестиИнформациюФайлаВРегистр(ТекущаяВерсия, ПолноеИмяФайла, ИмяКаталога,
...
//                                         (подкаталоги и файлы).
//
Процедура НайтиСлишкомБольшиеФайлы(МассивФайлов, МассивСлишкомБольшихФайлов, Рекурсивно, КоличествоСуммарное,
		...
		Если ВыбранныйФайл.Существует() Тогда
			...
			Если ВыбранныйФайл.ЭтоКаталог() Тогда
				...
				Если Рекурсивно Тогда
					...
					// Рекурсия
					Если МассивФайловВКаталоге.Количество() <> 0 Тогда
						НайтиСлишкомБольшиеФайлы(МассивФайловВКаталоге, МассивСлишкомБольшихФайлов, Рекурсивно,
							КоличествоСуммарное, ПсевдоФайловаяСистема);
					КонецЕсли;
				КонецЕсли;
...
Процедура СравнитьФайлы(ИдентификаторФормы, ПервыйФайл, ВторойФайл, Расширение, ВладелецВерсии = Неопределено) Экспорт
	...
	РасширениеПоддерживается = (Расширение = "txt" Или Расширение = "md" Или Расширение = "doc" Или Расширение = "docx"
		Или Расширение = "rtf" Или Расширение = "htm" Или Расширение = "html" Или Расширение = "mxl" Или Расширение
		= "odt");

	Если Не РасширениеПоддерживается Тогда
		ТекстПредупреждения = НСтр("ru = 'Сравнение файлов возможно только для следующих типов:
...
Процедура СравнитьФайлыСлужебный(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ТекущийШаг = "Шаг1ВыборСпособаСравненияВерсий" Тогда
		Если ПараметрыВыполнения.СпособСравненияВерсийФайлов = Неопределено Тогда
			...
			Если ПараметрыВыполнения.СпособСравненияВерсийФайлов = Неопределено Тогда
				Обработчик = Новый ОписаниеОповещения("СравнитьФайлыОбработкаРезультата", ЭтотОбъект,
					ПараметрыВыполнения);
				ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВыборСпособаСравненияВерсий", , ЭтотОбъект, , , ,
					Обработчик);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыВыполнения.ТекущийШаг = "Шаг2ПолучитьДанныеФайлов" Тогда
		...
		Если ПараметрыВыполнения.Свойство("ВладелецВерсии") И ЗначениеЗаполнено(ПараметрыВыполнения.ВладелецВерсии) Тогда
			ПараметрыВыполнения.ДанныеФайла1 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
				ПараметрыВыполнения.ВладелецВерсии, ПараметрыВыполнения.Ссылка1,
				ПараметрыВыполнения.УникальныйИдентификатор);
			ПараметрыВыполнения.ДанныеФайла2 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
				ПараметрыВыполнения.ВладелецВерсии, ПараметрыВыполнения.Ссылка2,
				ПараметрыВыполнения.УникальныйИдентификатор);
		Иначе
			ПараметрыВыполнения.ДанныеФайла1 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
	...
	Если ПараметрыВыполнения.ТекущийШаг = "Шаг5СравнитьФайлы" Тогда
		...
		ЗаголовокФайла1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовкаФайла, ИмяФайла1,
		...
		ЗаголовокФайла2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовкаФайла, ИмяФайла2,
		...
		ВыполнитьСравнениеФайлов(ПолноеИмяФайлаСлева, ПолноеИмяФайлаСправа,
			ПараметрыВыполнения.СпособСравненияВерсийФайлов, ЗаголовокФайлаСлева, ЗаголовокФайлаСправа);
	КонецЕсли;
...
Процедура ВыполнитьСравнениеФайлов(ПутьКФайлу1, ПутьКФайлу2, СпособСравненияВерсийФайлов, ЗаголовокЛевый = "",
	ЗаголовокПравый = "") Экспорт
	...
	Исключение
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось сравнить файлы по причине: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Файлы.Сравнение файлов'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), "Ошибка",
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Файлы.Сравнение файлов'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), "Ошибка",
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), , Истина);
		ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС + НСтр(
			"ru = 'Установите (переустановите) приложение OpenOffice.org Writer.'");
	КонецПопытки;	
	
	// Параметры открытия: отключение исполнения макросов.
	ПараметрыДокумента = Новый COMSafeArray("VT_VARIANT", 1);
	РежимЗапуска = ПрисвоитьЗначениеСвойству(ОбъектServiceManager, "MacroExecutionMode", 0); // const short NEVER_EXECUTE = 0
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			...
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), , Истина);
		ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС + НСтр(
			"ru = 'Установите (переустановите) приложение Microsoft Word.'");
	КонецПопытки;
...
Процедура ВернутьРезультатПослеПоказаПредупреждения(Обработчик, ТекстПредупреждения, Результат)
	...
	Если Обработчик <> Неопределено Тогда
		...
		Обработчик = Новый ОписаниеОповещения("ВернутьРезультатПослеЗакрытияПростогоДиалога", ЭтотОбъект,
			ПараметрыОбработчика);
...
Процедура ОбработатьФайлЦиклНачало(Контекст)
	...
	Если Действие = "Удалить" Тогда
		НачатьУдалениеФайлов(Новый ОписаниеОповещения("ОбработатьФайлПослеУдаленияФайлов", ЭтотОбъект, Контекст,
			"ОбработатьФайлПослеОшибки", ЭтотОбъект), ПолноеИмяФайла);
		Возврат;

	ИначеЕсли Действие = "СкопироватьИзИсточника" Тогда
		НачатьКопированиеФайла(Новый ОписаниеОповещения("ОбработатьФайлПослеКопированияФайла", ЭтотОбъект, Контекст,
	...
	ИначеЕсли Действие = "СоздатьКаталог" Тогда
		НачатьСозданиеКаталога(Новый ОписаниеОповещения("ОбработатьФайлПослеСозданияКаталога", ЭтотОбъект, Контекст,
			"ОбработатьФайлПослеОшибки", ЭтотОбъект), ПолноеИмяФайла);
		Возврат;
	...
	ИначеЕсли Действие = "Получить" Тогда
		...
		НачатьПолучениеФайлов(Новый ОписаниеОповещения("ОбработатьФайлПослеПолученияФайлов", ЭтотОбъект, Контекст,
			"ОбработатьФайлПослеОшибки", ЭтотОбъект), ПолучаемыеФайлы, , Ложь);
		Возврат;
	...
	ИначеЕсли Действие = "Поместить" Тогда
		...
		ПомещаемыеФайлы = Новый Массив;
		ПомещаемыеФайлы.Добавить(ОписаниеФайла);
		НачатьПомещениеФайлов(Новый ОписаниеОповещения("ОбработатьФайлПослеПомещенияФайлов", ЭтотОбъект, Контекст,
			"ОбработатьФайлПослеОшибки", ЭтотОбъект), ПомещаемыеФайлы, , Ложь, Контекст.ИдентификаторФормы);
		Возврат;
	...
	Контекст.Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОбработатьФайлПослеПроверкиСуществования",
		ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект));
...
Процедура ОбработатьФайлПослеОшибки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеДействия.Свойство("ЗаголовокОшибки") Тогда
		Контекст.РезультатДействий.ОписаниеОшибки = Контекст.ОписаниеДействия.ЗаголовокОшибки + Символы.ПС
			+ Контекст.РезультатДействий.ОписаниеОшибки;
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПроверкиСуществования(Существует, Контекст) Экспорт
	...
	ИначеЕсли Контекст.СвойстваДляПолучения.Свойство("ВремяИзменения") Тогда
		Контекст.Файл.НачатьПолучениеВремениИзменения(
			Новый ОписаниеОповещения("ОбработатьФайлПослеПолученияВремениИзменения", ЭтотОбъект, Контекст,
...
Процедура ОбработатьФайлПослеПолученияВремениИзменения(ВремяИзменения, Контекст) Экспорт
	...
	Если Контекст.СвойстваДляПолучения.Свойство("УниверсальноеВремяИзменения") Тогда
		Контекст.Файл.НачатьПолучениеУниверсальногоВремениИзменения(
			Новый ОписаниеОповещения("ОбработатьФайлПослеПолученияУниверсальногоВремениИзменения", ЭтотОбъект, Контекст,
			"ОбработатьФайлПослеОшибки", ЭтотОбъект));
	Иначе
...
Процедура ОбработатьФайлПослеПолученияУниверсальногоВремениИзменения(УниверсальноеВремяИзменения, Контекст) Экспорт
	...
	Если Контекст.СвойстваДляПолучения.Свойство("ТолькоЧтение") Тогда
		Контекст.Файл.НачатьПолучениеТолькоЧтения(Новый ОписаниеОповещения("ОбработатьФайлПослеПолученияТолькоЧтения",
			ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект));
	Иначе
...
Процедура ОбработатьФайлПослеПолученияТолькоЧтения(ТолькоЧтение, Контекст) Экспорт
	...
	Если Контекст.СвойстваДляПолучения.Свойство("Невидимость") Тогда
		Контекст.Файл.НачатьПолучениеНевидимости(Новый ОписаниеОповещения("ОбработатьФайлПослеПолученияНевидимости",
			ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект));
	Иначе
...
Процедура ОбработатьФайлПослеПолученияНевидимости(Невидимость, Контекст) Экспорт
	...
	Если Контекст.СвойстваДляПолучения.Свойство("Размер") Тогда
		Контекст.Файл.НачатьПолучениеРазмера(Новый ОписаниеОповещения("ОбработатьФайлПослеПолученияРазмера",
			ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект));
	Иначе
...
Процедура ОбработатьФайлПослеПолученияРазмера(Размер, Контекст) Экспорт
	...
	Если Контекст.СвойстваДляПолучения.Свойство("ЭтоКаталог") Тогда
		Контекст.Файл.НачатьПроверкуЭтоКаталог(Новый ОписаниеОповещения("ОбработатьФайлПослеПроверкиЭтоКаталог",
			ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект));
	Иначе
...
Процедура ОбработатьФайлПослеПроверкиЭтоКаталог(ЭтоКаталог, Контекст) Экспорт
	...
	Если Контекст.СвойстваДляПолучения.Свойство("ЭтоФайл") Тогда
		Контекст.Файл.НачатьПроверкуЭтоФайл(Новый ОписаниеОповещения("ОбработатьФайлПослеПроверкиЭтоФайл", ЭтотОбъект,
			Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект));
	Иначе
		ОбработатьФайлПослеПроверкиЭтоФайл(Null, Контекст);
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПроверкиЭтоФайл(ЭтоФайл, Контекст) Экспорт
	...
	ИначеЕсли Контекст.СвойстваДляУстановки.Свойство("ВремяИзменения") Тогда
		Контекст.Файл.НачатьУстановкуВремениИзменения(
			Новый ОписаниеОповещения("ОбработатьФайлПослеУстановкиВремениИзменения", ЭтотОбъект, Контекст,
...
Процедура ОбработатьФайлПослеУстановкиВремениИзменения(Контекст) Экспорт
	...
	Если Контекст.СвойстваДляУстановки.Свойство("УниверсальноеВремяИзменения") Тогда
		Контекст.Файл.НачатьУстановкуУниверсальногоВремениИзменения(
			Новый ОписаниеОповещения("ОбработатьФайлПослеУстановкиУниверсальногоВремениИзменения", ЭтотОбъект, Контекст,
...
Процедура ОбработатьФайлПослеУстановкиУниверсальногоВремениИзменения(Контекст) Экспорт
	...
	Если Контекст.СвойстваДляУстановки.Свойство("ТолькоЧтение") Тогда
		Контекст.Файл.НачатьУстановкуТолькоЧтения(Новый ОписаниеОповещения("ОбработатьФайлПослеУстановкиТолькоЧтения",
			ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект), Контекст.СвойстваДляУстановки.ТолькоЧтение);
	Иначе
...
Процедура ОбработатьФайлПослеУстановкиТолькоЧтения(Контекст) Экспорт
	...
	Если Контекст.СвойстваДляУстановки.Свойство("Невидимость") Тогда
		Контекст.Файл.НачатьУстановкуНевидимости(Новый ОписаниеОповещения("ОбработатьФайлПослеУстановкиНевидимости",
			ЭтотОбъект, Контекст, "ОбработатьФайлПослеОшибки", ЭтотОбъект), Контекст.СвойстваДляУстановки.Невидимость);
	Иначе
		ОбработатьФайлПослеУстановкиНевидимости(Контекст);
	КонецЕсли;
...
Функция ПолучитьНастройку(ИмяУстройства, ИмяНастройки) Экспорт
	...
	Попытка
		Возврат ПараметрыПриложения["СтандартныеПодсистемы.КомпонентаTwain"].ПолучитьНастройку(ИмяУстройства,
			ИмяНастройки);
	Исключение
		Возврат -1;
	КонецПопытки;
...
Процедура ВыбратьРежимИРедактироватьФайл(ОбработчикРезультата, ДанныеФайла, ДоступностьКомандыРедактировать) Экспорт
	...
	Если СпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе") Тогда
		...
		РасширениеВСписке = РаботаСФайламиСлужебныйКлиентСервер.РасширениеФайлаВСписке(
			ПерсональныеНастройки.ТекстовыеФайлыРасширение, ДанныеФайла.Расширение);
	...
	Если СпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе") Тогда
		...
		РасширениеВСписке = РаботаСФайламиСлужебныйКлиентСервер.РасширениеФайлаВСписке(
			ПерсональныеНастройки.ГрафическиеСхемыРасширение, ДанныеФайла.Расширение);
	...
	Если Не ЗначениеЗаполнено(ДанныеФайла.Редактирует)
		...
		ПараметрыВыполнения = Новый Структура;
		ПараметрыВыполнения.Вставить("ОбработчикРезультата", ОбработчикРезультата);
		Обработчик = Новый ОписаниеОповещения("ВыбратьРежимИРедактироватьФайлЗавершение", ЭтотОбъект,
			ПараметрыВыполнения);

		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ФормаВыбораРежимаОткрытия", , , , , , Обработчик,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФайлРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ИначеЕсли ФайлНаДиске.Существует() Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФайлДиалогПоказан", ЭтотОбъект,
				ДополнительныеПараметры);
			ПоказатьДиалогНужноПолучитьФайлССервера(ОписаниеОповещения, ПолноеИмяФайлаНаКлиенте, ДанныеФайла,
				ДляРедактирования);
			Возврат;
		Иначе
			ПолучитьФайл = Истина;
	...
	Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФайлНапоминаниеПоказано", ЭтотОбъект,
			ДополнительныеПараметры);
...
Процедура ОткрытьФайлДиалогПоказан(ПолучитьФайл, ДополнительныеПараметры) Экспорт
	...
	Если ПолучитьФайл Тогда
		...
		ФайлМожноОткрывать = ПолучитьФайлВРабочийКаталог(
			ДанныеФайла.СсылкаНаДвоичныеДанныеФайла, ДанныеФайла.ОтносительныйПуть,
			ДанныеФайла.ДатаМодификацииУниверсальная, ДанныеФайла.ИмяФайла, РабочийКаталогПользователя,
			ПолноеИмяФайлаНаКлиенте);
	КонецЕсли;
...
Функция ПолучитьФайлВРабочийКаталог(Знач АдресДвоичныхДанныхФайла, Знач ОтносительныйПуть,
	Знач ДатаМодификацииУниверсальная, Знач ИмяФайла, Знач РабочийКаталогПользователя, ПолноеИмяФайлаНаКлиенте)
...
Процедура УдалитьДанные(ОбработчикЗавершения, ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	Если ТипЗнч(ФайлИлиВерсия) <> Тип("СправочникСсылка.ВерсииФайлов") Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Удалить файл %1 без возможности восстановления?'"), Строка(ФайлИлиВерсия));
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьДанныеПослеОтветаНаВопрос", ЭтотОбъект, Контекст), ТекстВопроса,
		РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, ТекстЗаголовка, КодВозвратаДиалога.Нет);
...
Процедура ИзменитьОтборПоПометкеУдаления(Область, КнопкаКоманды) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ИзменитьЭлементыОтбора(Область, "ПометкаУдаления",
		"СкрыватьПометкуУдаленияПоУмолчанию", Ложь, ВидСравненияКомпоновкиДанных.Равно, Не КнопкаКоманды.Пометка,
