...
Функция ВыполнитьОбновлениеИнформационнойБазы(ПараметрыОбновления) Экспорт
	...
	ВыполнитьОтложенноеОбновлениеСейчас  = Не ОтладкаОтложенногоОбновления И РежимВыполненияОтложенногоОбновления
		= "Монопольно";
...
Функция ВерсияИБ(Знач ИдентификаторБиблиотеки, Знач ПолучитьВерсиюОбщихДанных = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ПриОпределенииВерсииИБ(ИдентификаторБиблиотеки,
			ПолучитьВерсиюОбщихДанных, СтандартнаяОбработка, Результат);
...
//
Процедура УстановитьВерсиюИБ(Знач ИдентификаторБиблиотеки, Знач НомерВерсии, Знач ЭтоОсновнаяКонфигурация,
	ВыполненаРегистрация = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ПриУстановкеВерсииИБ(ИдентификаторБиблиотеки,
			НомерВерсии, СтандартнаяОбработка, ЭтоОсновнаяКонфигурация);
...
Процедура ОтметитьРегистрациюОтложенныхОбработчиковОбновления(ИмяПодсистемы = Неопределено, Значение = Истина) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ПриОтметкеРегистрацииОтложенныхОбработчиковОбновления(
			ИмяПодсистемы, Значение, СтандартнаяОбработка);
	КонецЕсли;
...
Функция РежимОбновленияДанных() Экспорт
	...
	Если Не СтандартнаяОбработка Тогда
		...
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Недопустимое значение параметра %1 в %2. 
			...
			"РежимОбновленияДанных", "ПриОпределенииРежимаОбновленияДанных", "НачальноеЗаполнение", "ОбновлениеВерсии",
			"ПереходСДругойПрограммы", РежимОбновленияДанных, ТипЗнч(РежимОбновленияДанных));
		ОбщегоНазначенияКлиентСервер.Проверить(РежимОбновленияДанных = "НачальноеЗаполнение" Или РежимОбновленияДанных
			= "ОбновлениеВерсии" Или РежимОбновленияДанных = "ПереходСДругойПрограммы", Сообщение);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ПриОпределенииРежимаОбновленияОбластиДанных(
...
Функция ОбработчикиОбновленияВИнтервале(Знач ИсходнаяТаблицаОбработчиков, Знач ВерсияОт, Знач ВерсияДо,
	...
	Макет = Компоновщик.Выполнить(Схема, Схема.НастройкиПоУмолчанию, , , Тип(
...
Функция ВыполненаРегистрацияОтложенныхОбработчиковОбновления() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ПриПроверкеРегистрацииОтложенныхОбработчиковОбновления(
...
Функция ПоказатьОписаниеИзмененийСистемы() Экспорт
	...
	ВывестиОписаниеИзмененийДляАдминистратора = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ОбновлениеИБ",
		"ВывестиОписаниеИзмененийДляАдминистратора", , , ИмяПользователя());
...
Функция СтатусНевыполненныхОбработчиков(ПриОбновлении = Ложь) Экспорт
	...
	Если ПриОбновлении Тогда
		...
		ВерсияМетаданныхБезНомераСборки = ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(
			Метаданные.Версия);
		...
		Если ВерсияДереваОбработчиков <> "" И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияДереваОбработчиков,
			ВерсияДанных) > 0 Тогда
...
Функция ИнформационнаяБазаЗаблокированаДляОбновления(УчитыватьПривилегированныйРежим = Истина,
	ПриЗапуске = Неопределено) Экспорт
	...
	СообщениеАдминистраторуСистемы = НСтр("ru = 'Вход в программу временно невозможен в связи с обновлением на новую версию.
	...
	Если НеобходимоОбновлениеНеразделенныхДанныхИнформационнойБазы() Тогда
		...
		СообщениеАдминистраторуОбластиДанных = НСтр("ru = 'Вход в приложение временно невозможен в связи с обновлением на новую версию.
	...
	Если ЕстьПраваНаОбновлениеИнформационнойБазы(УчитыватьПривилегированныйРежим, Истина) Тогда
		Если ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы() И ПриЗапуске = Истина Тогда
			...
			Если Не Результат.МожноВыполнятьОбновление Тогда
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение,
					Результат.СеансОбновления.ИмяКомпьютера, Результат.СеансОбновления.Пользователь,
					Результат.СеансОбновления.НомерСеанса, Результат.СеансОбновления.НачалоСеанса,
	...
	Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() И ОбщегоНазначения.ПодсистемаСуществует(
		"СтандартныеПодсистемы.ОбменДанными") Тогда
		МодульОбменДаннымиСлужебный = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСлужебный");
		ТребуетсяПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском = МодульОбменДаннымиСлужебный.ПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском();
	КонецЕсли;
...
Процедура ЗаписатьВремяВыполненияОбновления(ВремяНачалаОбновления, ВремяОкончанияОбновления) Экспорт
	...
	ПродолжительностьЧасы = ?(Часы = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = '%1 час'"), Часы));
	ПродолжительностьМинуты = ?(Минуты = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = '%1 мин'"), Минуты));
	ПродолжительностьСекунды = ?(Секунды = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = '%1 сек'"), Секунды));
...
Процедура ЗаписатьПодтверждениеЛегальностиПолученияОбновлений() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()
...
Процедура ПеререгистрироватьДанныеДляОтложенногоОбновления() Экспорт
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Если Обработчик.Многопоточный Тогда
			ПараметрыОбработчикаСтруктура.ПараметрыВыборки = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляМногопоточнойОбработки();
		Иначе
		...
		Попытка
			Сообщение = НСтр("ru = 'Выполняется процедура заполнения данных
							 ...
							 |""%2"".'");
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение,
				Обработчик.ПроцедураЗаполненияДанныхОбновления, Обработчик.ИмяОбработчика);
			ЗаписатьИнформацию(Сообщение);

			ОбщегоНазначения.ВыполнитьМетодКонфигурации(Обработчик.ПроцедураЗаполненияДанныхОбновления,
		...
		Исключение
			ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При вызове процедуры заполнения данных
					 ...
					 |""%2""
					 |произошла ошибка:
					 |""%3"".'"), Обработчик.ПроцедураЗаполненияДанныхОбновления, Обработчик.ИмяОбработчика,
...
Процедура ЗаписатьПараметрыОтложенногоОбработчикаОбновления(Идентификатор, Параметры) Экспорт
	...
	УстановитьСвойствоОбработчика(Идентификатор, "СтатистикаВыполнения", Новый ХранилищеЗначения(СтатистикаВыполнения));
КонецПроцедуры
...
Функция ИтерацииОбновления() Экспорт
	...
	Для Каждого ИмяПодсистемы Из ОписанияПодсистем.Порядок Цикл
		...
		ИтерацияОбновления = ИтерацияОбновления(ОписаниеПодсистемы.Имя, ОписаниеПодсистемы.Версия,
			ОбновлениеИнформационнойБазы.НоваяТаблицаОбработчиковОбновления(), ОписаниеПодсистемы.Имя
	...
	Если ИтерацияОбновлениеОсновнойПодсистемы = Неопределено И ИмяОсновнойКонфигурации
		= "БиблиотекаСтандартныхПодсистем" Тогда
...
Процедура ОбновитьСписокВыполняемыхОбработчиковОбновления(ИтерацииОбновления, ПервыйОбменВРИБ = Ложь,
	ТипыОбработчиков = Неопределено) Экспорт
	...
	Попытка
		...
		КонецЕсли;
		Если Не ТолькоОтложенные И Не ОбщегоНазначения.РазделениеВключено()
		...
		Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
			...
			Если Не ТолькоМонопольные И ЗаполнитьВерсииПодсистем Тогда
				...
				Если СохраненнаяВерсия = Неопределено Тогда
					ВерсииПодсистемНаНачалоОбновления.Вставить(ИтерацияОбновления.Подсистема,
						ИтерацияОбновления.ПредыдущаяВерсия);
				Иначе
					Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СохраненнаяВерсия,
						ИтерацияОбновления.ПредыдущаяВерсия) > 0 Тогда
						ВерсииПодсистемНаНачалоОбновления.Вставить(ИтерацияОбновления.Подсистема,
							ИтерацияОбновления.ПредыдущаяВерсия);
					КонецЕсли;
				КонецЕсли;
			...
			Если Не ТолькоОтложенные Тогда
				Если Не ПервыйОбменВРИБ Тогда
					...
					Если ПредыдущаяВерсия <> "0.0.0.0" И ОбщегоНазначения.РазделениеВключено()
						И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
						ОбработчикиПоВерсиям = ПолучитьПланОбновления(ИтерацияОбновления.Подсистема, ПредыдущаяВерсия,
							ВерсияМетаданных);
					КонецЕсли;
			...
			Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				ДобавитьНезавершенныеОтложенныеОбработчики(
					ИтерацияОбновления, ИмяБиблиотеки, НезавершенныеОтложенныеОбработчики, ОбработчикиПоВерсиям.Строки);

				Если ПервыйОбменВРИБ Тогда
					УдаляемыеСтроки = Новый Массив;
					Для Каждого СтрокаВерсия Из ОбработчикиПоВерсиям.Строки Цикл
						Если СтрокаВерсия.Версия = "*" Или (ЗначениеЗаполнено(ПараллельноСВерсии)
				...
				ДобавитьОтложенныеОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям.Строки, ГруппаОбновления,
					ТекстОшибок, ТекущийРежимВыполнения);
			КонецЕсли;
...
Процедура ПриВключенииОтложенногоОбновления(Использование) Экспорт
	...
	Для Каждого Задание Из Задания Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
			...
			ТребуетсяУстановитьЗапланированныйМоментЗапуска = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(
				ВерсияБиблиотекиБТС, "2.0.1.0") > 0;
		КонецЕсли;

		Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ПодсистемаСуществует(
			"СтандартныеПодсистемы.РаботаВМоделиСервиса.ОбновлениеВерсииИБВМоделиСервиса") И Использование
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0" И ОбщегоНазначения.РазделениеВключено()
		И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			...
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ИдентификаторБиблиотеки, ТекущаяВерсияИБ, ВерсияМетаданных);
			ЗаписатьИнформацию(Сообщение);

			ВыполняемыеОбработчики = ОбработчикиОбновленияВИнтервале(ОбработчикиОбновления, ТекущаяВерсияИБ,
				ВерсияМетаданных);
		КонецЕсли;
	Иначе
		ВыполняемыеОбработчики = ОбработчикиОбновленияВИнтервале(ОбработчикиОбновления, ТекущаяВерсияИБ,
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Иначе
			...
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Выполняется обновление информационной базы с версии %1 на версию %2.'"), ТекущаяВерсияИБ,
					НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.'"), ТекущаяВерсияИБ,
					НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
		...
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Выполнено обновление информационной базы с версии %1 на версию %2.'"), ТекущаяВерсияИБ,
					НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.'"), ТекущаяВерсияИБ,
					НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0" Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует(
			...
			МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
				"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
			МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.СформироватьПланОбновленияОбластиДанных(
				ИдентификаторБиблиотеки, ОбработчикиОбновления, ОбязательныеРазделенныеОбработчики, ИсходнаяВерсияИБ,
				ВерсияМетаданныхИБ);
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ПараметрыОбработчикаОбновления",
		"ОбновлениеИнформационнойБазыСлужебный.УстановкаПараметровСеанса");
...
Процедура ПриСбореПоказателейСтатистикиКонфигурации() Экспорт
	...
	Для Каждого Обработчик Из Обработчики Цикл
		МодульЦентрМониторинга.ЗаписатьСтатистикуОбъектаКонфигурации("ВремяВыполненияОтложенногоОбработчика."
			+ Обработчик.ИмяОбработчика, Обработчик.ДлительностьОбработки / 1000);
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(ВремяНачала) И ЗначениеЗаполнено(ВремяОкончания) Тогда
		МодульЦентрМониторинга.ЗаписатьСтатистикуОбъектаКонфигурации("ВремяВыполненияОбработчиков", ВремяОкончания
			- ВремяНачала);
	КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Пользователи.ЭтоПолноправныйПользователь( , Истина)
		...
		Если Значение = Неопределено Или ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Метаданные.Версия, Значение) > 0 Тогда
	...
	Если СтатусОбработчиков = "" Тогда
		Возврат;
	КонецЕсли;
	Если СтатусОбработчиков = "СтатусОшибка" И Пользователи.ЭтоПолноправныйПользователь( , Истина) Тогда
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь( , Истина) Или МодульТекущиеДелаСервер.ДелоОтключено(
		"ОтложенноеОбновление") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.ЕстьДела      = (ЕстьОбработчикиСОшибкой Или ЕстьНеВыполненныеОбработчики
			Или ЕстьПриостановленныеОбработчики);
...
Процедура ПриПолученииПервогоСообщенияОбменаРИБПослеОбновления() Экспорт
	...
	СведенияОбОбновлении       = СведенияОбОбновленииИнформационнойБазы();
	Если СведенияОбОбновлении.ОтложенноеОбновлениеЗавершеноУспешно = Неопределено И Не ИнформационнаяБазаФайловая Тогда
...
Функция НастройкиПодсистемы() Экспорт
	...
	Настройки.Вставить("ПараметрыСообщенияОНевыполненныхОтложенныхОбработчиках",
		ПараметрыСообщенияОНевыполненныхОтложенныхОбработчиках);
...
Функция НастройкиПредопределенныхЭлементов(МенеджерОбъекта, ПредопределенныеДанные) Экспорт
	...
	ПользовательскиеНастройкиЗаполненияЭлементов.ДополнительныеПараметры.Вставить("ПредопределенныеДанные",
	...
	ЗаполнитьЗначенияСвойств(НастройкиЗаполненияЭлементов.ПереопределяемыеНастройки,
		ПользовательскиеНастройкиЗаполненияЭлементов);

	НастройкиЗаполненияЭлементов.ЕстьКолонкаИмяПредопределенныхДанных = ПредопределенныеДанные.Колонки.Найти(
		"ИмяПредопределенныхДанных") <> Неопределено;

	НастройкиЗаполненияЭлементов.ПереопределяемыеНастройки.ИмяКлючевогоРеквизита = ?(ЗначениеЗаполнено(
...
Функция ПредопределенныеДанныеОбъекта(Знач МетаданныеОбъекта, МенеджерОбъекта, ЛокализуемыеРеквизитыОбъекта) Экспорт
	...
	Для Каждого Реквизит Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
		ДобавитьКолонкуТаблицыПредопределенныхДанных(ПредопределенныеДанные, Реквизит, ЛокализуемыеРеквизитыОбъекта,
			Языки);
	КонецЦикла;
	...
	ОбновлениеИнформационнойБазыПереопределяемый.ПриНачальномЗаполненииЭлементов(МетаданныеОбъекта.ПолноеИмя(), Языки,
		ПредопределенныеДанные, ТабличныеЧасти);
		...
		Для Каждого СтрокаПредопределенныхДанных Из ПредопределенныеДанные Цикл
			...
			Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				Если ТипЗнч(СтрокаПредопределенныхДанных.Ссылка) = Тип("УникальныйИдентификатор") Тогда
					СтрокаПредопределенныхДанных.Ссылка = МенеджерОбъекта.ПолучитьСсылку(
						СтрокаПредопределенныхДанных.Ссылка);
				ИначеЕсли ТипЗнч(СтрокаПредопределенныхДанных.Ссылка) = Тип("Строка") Тогда
					СтрокаПредопределенныхДанных.Ссылка = МенеджерОбъекта.ПолучитьСсылку(
						Новый УникальныйИдентификатор(СтрокаПредопределенныхДанных.Ссылка));
				КонецЕсли;
...
Процедура НачальноеЗаполнениеПредопределенныхДанных(ОбновитьМультиязычныеСтроки = Ложь,
	...
	Для Каждого МетаданныеОбъекта Из ОбъектыСНачальнымЗаполнением Цикл
		...
		Если ПроверятьНеобходимостьВыполнения И ДополнительныеПараметры.НовыеОбъекты.Найти(
			МетаданныеОбъекта.ПолноеИмя()) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Попытка
			...
			Для Каждого СтрокаТаблицы Из ПредопределенныеДанные Цикл
				...
				Если ПараметрыЗаполнения.ЛокализуемыеРеквизитыОбъекта.Количество() > 0
					И ПараметрыЗаполнения.ЛокализуемыеРеквизитыОбъекта["Наименование"] <> Неопределено Тогда
					Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
						МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
					...
					КонецЕсли;
				ИначеЕсли ОбновитьМультиязычныеСтроки И ПредопределенныеДанные.Колонки.Найти("Наименование") <> Неопределено
					И ЗначениеЗаполнено(СтрокаТаблицы["Наименование"]) Тогда
					ЗаполняемыйЭлемент["Наименование"] = СтрокаТаблицы["Наименование"];
				КонецЕсли;

				Если Не ОбновитьМультиязычныеСтроки
					...
					ОбновлениеИнформационнойБазыПереопределяемый.ПриНачальномЗаполненииЭлемента(
						МетаданныеОбъекта.ПолноеИмя(), ЗаполняемыйЭлемент, СтрокаТаблицы,
						НастройкиЗаполнения.ПереопределяемыеНастройки.ДополнительныеПараметры);
...
Функция НаборПараметровДляЗаполненияОбъекта(МетаданныеОбъекта) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		ОбъектСодержитТЧПредставления = МодульМультиязычностьСервер.ОбъектСодержитТЧПредставления(
	...
	Если ЕстьПредопределенныеДанные Тогда
		ПредопределенныеДанные = ПредопределенныеДанныеОбъекта(МетаданныеОбъекта, МенеджерОбъекта,
			ЛокализуемыеРеквизитыОбъекта);
	...
	Если ОбъектСодержитТЧПредставления И ПоддерживаетсяИерархия Тогда
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.Представления.Использование
			= Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппыИЭлемента
			Или МетаданныеОбъекта.ТабличныеЧасти.Представления.Использование
...
Функция ДействияПередОбновлениемИнформационнойБазы(ПараметрыОбновления)
	...
	Если Не ПараметрыОбновления.ПриЗапускеКлиентскогоПриложения Тогда
		Если Не ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ПодсистемаСуществует(
			"СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
			МодульОбновлениеКонфигурации = ОбщегоНазначения.ОбщийМодуль("ОбновлениеКонфигурации");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		МодульОбменДаннымиСервер.ПередОбновлениемИнформационнойБазы(
			ПараметрыОбновления.ПриЗапускеКлиентскогоПриложения, ПараметрыОбновления.Перезапустить);
	КонецЕсли;
	...
	Если РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Изменилось имя конфигурации на ""%1"".
				 |Будет выполнен переход с другой программы.'"), Метаданные.Имя);
	ИначеЕсли РежимОбновленияДанных = "ОбновлениеВерсии" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Изменился номер версии конфигурации: с ""%1"" на ""%2"".
				 |Будет выполнено обновление информационной базы.'"), ВерсияДанных, ВерсияМетаданных);
	Иначе
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполняется начальное заполнение данных до версии ""%1"".'"), ВерсияМетаданных);
	КонецЕсли;
	ЗаписатьИнформацию(Сообщение);
...
Процедура ВыполнитьДействияПриОбновленииИнформационнойБазы(ПараметрыОбновления, ДополнительныеПараметры)
	...
	Параметры.Вставить("ХодВыполненияОбработчиков", КоличествоОбработчиковНаТекущуюВерсию(ИтерацииОбновления,
		РежимВыполненияОтложенногоОбновления));
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение,
		Параметры.ХодВыполненияОбработчиков.ВсегоОбработчиков);
	ЗаписатьИнформацию(Сообщение);
...
Процедура ВыполнитьДействияПослеОбновленияИнформационнойБазы(ДополнительныеПараметры)
	...
	ВыполнитьОбработчикиПослеОбновленияИнформационнойБазы(
		ИтерацииОбновления, Константы.ДетализироватьОбновлениеИБВЖурналеРегистрации.Получить(),
		ВыводитьОписаниеОбновлений, ОперативноеОбновление);

	ОбновлениеИнформационнойБазыПереопределяемый.ПослеОбновленияИнформационнойБазы(
		ВерсияДанных, ВерсияМетаданных, ИтерацииОбновления, ВыводитьОписаниеОбновлений, Не ОперативноеОбновление);
...
Функция ИтерацияОбновления(ИмяКонфигурацииИлиБиблиотеки, Версия, Обработчики, ЭтоОсновнаяКонфигурация = Неопределено) Экспорт
	...
	ИтерацияОбновления.Вставить("ЭтоОсновнаяКонфигурация", ?(ЭтоОсновнаяКонфигурация <> Неопределено,
		ЭтоОсновнаяКонфигурация, ИмяКонфигурацииИлиБиблиотеки = Метаданные.Имя));
...
Функция ЕстьПраваНаОбновлениеИнформационнойБазы(УчитыватьПривилегированныйРежим = Истина,
	РазделенныеДанные = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И РазделенныеДанные Тогда
	...
	Возврат Пользователи.ЭтоПолноправныйПользователь( , ПроверятьПраваАдминистрированияСистемы,
...
Функция ОбновитьИнформационнуюБазуВФоне(УникальныйИдентификаторФормы, БлокировкаИБ) Экспорт
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"ОбновлениеИнформационнойБазыСлужебный.ВыполнитьОбновлениеИнформационнойБазыВФоне", ПараметрыОбновленияИБ,
		ПараметрыВыполнения);
...
Функция ЗаблокироватьИБ(БлокировкаИБ, ИсключениеПриНевозможностиБлокировкиИБ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.РазблокироватьВерсииОбластиДанных(
			БлокировкаИБ.КлючЗаписи);
	КонецЕсли;

	Если Не ИсключениеПриНевозможностиБлокировкиИБ И ОбщегоНазначения.ИнформационнаяБазаФайловая()
...
Функция НоваяТаблицаОбработчиковПереходаСДругойПрограммы()
	...
	Обработчики.Колонки.Добавить("ПредыдущееИмяКонфигурации", Новый ОписаниеТипов("Строка",
...
Функция ОбработчикиПереходаСДругойПрограммы(ПредыдущееИмяКонфигурации, ИтерацииОбновления)
	...
	Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
		...
		Для Каждого Обработчик Из ОбработчикиНачальногоЗаполнения Цикл
			Если Обработчик.РежимВыполнения = "Отложенно" Или Обработчик.НеВыполнятьПриПереходеСДругойПрограммы Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ПерейтиСДругойПрограммы(ИтерацииОбновления)
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ЗаполнитьПризнакЭтоОсновнаяКонфигурация(
		...
		Исключение
			...
			ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При вызове обработчика перехода с другой программы
					 |""%1""
					 |произошла ошибка:
					 |""%2"".'"), ИмяОбработчика, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
...
Процедура ДобавитьОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям, ДобавленныеОбработчики)
	...
	Для Каждого Версия Из ОбработчикиПоВерсиям Цикл
		Для Каждого Обработчик Из Версия.Строки Цикл
			...
			Если Не ЗначениеЗаполнено(Обработчик.РежимВыполнения) И (Обработчик.МонопольныйРежим = Истина
				Или Обработчик.МонопольныйРежим = Неопределено) Тогда
...
Процедура УстановитьФлагОтображенияОписанийПоТекущуюВерсию(Знач ИмяПользователя = Неопределено) Экспорт
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбновлениеИБ", "ПоследняяВерсияОтображенияИзмененийСистемы",
		Метаданные.Версия, , ИмяПользователя);

	Если ИмяПользователя = Неопределено И Пользователи.ЭтоПолноправныйПользователь() Тогда

		ОбщегоНазначения.ХранилищеОбщихНастроекУдалить("ОбновлениеИБ", "ВывестиОписаниеИзмененийДляАдминистратора",
			ИмяПользователя());
	...
	Попытка
		Если ФорсироватьОбновление(СведенияОбОбновлении) Тогда
			...
			Пока ОбработчикиВыполнялись Цикл
				...
				Если ДатаПоследнейПроверки = Неопределено Или ТекущаяДатаСеанса() - ДатаПоследнейПроверки > 600 Тогда
...
Функция ОбработчикиДляОтложеннойРегистрацииДанных(БезФильтра = Ложь, ПерезапускОбновления = Ложь,
	РегистрируемыеОбработчики = Неопределено) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("РежимВыполненияОбработчика",
	...
	Запрос.УстановитьПараметр("ОтключитьОтбор", (Не ПерезапускОбновления Или РегистрируемыеОбработчики.Количество()
		= 0));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОбработчикиОбновления.ИмяОбработчика КАК ИмяОбработчика,
...
Функция НайтиОбработчикОбновления(КонтекстОбработчика, ПараметрыОбновления = Неопределено)
	...
	Пока Истина Цикл
		...
		Если Результат.ОбработчикДляВыполнения = Неопределено И Результат.ЕстьНевыполненные
	...
	ПараллельныйРежим = (ОбработчикДляВыполнения.РежимВыполненияОтложенногоОбработчика
		= Перечисления.РежимыВыполненияОтложенныхОбработчиков.Параллельно);
	...
	Если ПараллельныйРежим Тогда
		ПараметрыОбновления.Вставить("ОчередьОбработчиков", ОчередьОбработчиковГруппыОбновления(
			ОбработчикДляВыполнения.ГруппаОбновления));
	...
	ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикДляВыполнения, ПараметрыОбновления,
...
Процедура ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков,
	ТекстОшибок)

	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно" И Не ЗначениеЗаполнено(
		Обработчик.ПроцедураЗаполненияДанныхОбновления) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не указана процедура заполнения данных
				 |отложенного обработчика обновления
				 |""%1"".'"), Обработчик.ИмяОбработчика);
	...
	Если Обработчик.МонопольныйРежим = Истина Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У отложенного обработчика ""%1""
			...
			Обработчик.ИмяОбработчика, "МонопольныйРежим");
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно" И Обработчик.ЗапускатьТолькоВГлавномУзле
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У отложенного обработчика ""%1""
			...
			Обработчик.ИмяОбработчика, "ЗапускатьТолькоВГлавномУзле", "ЗапускатьИВПодчиненномУзлеРИБСФильтрами");
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
Процедура ДобавитьОтложенныеОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям, ГруппаОбновления, ТекстОшибок,
	...
	Для Каждого СтрокаВерсия Из ОбработчикиПоВерсиям Цикл
		...
		ИначеЕсли РежимВыполненияОтложенныхОбработчиков = "Параллельно" И ЗначениеЗаполнено(ПараллельноСВерсии)
			И Не ПропуститьПроверку Тогда
	...
	Константы.ОтложенноеОбновлениеВГлавномУзлеЗавершеноУспешно.Установить(ТекущееЗначение
		И Не ЕстьОбработчикиТолькоГлавногоУзла);
...
Функция НовоеОписаниеГруппПотоковРегистрацииДанныхОтложенногоОбновления() Экспорт
	...
	ГруппаРегистрации.Процедура = "ОбновлениеИнформационнойБазыСлужебный.ЗаполнитьДанныеОтложенногоОбработчика";
	ГруппаРегистрации.ПроцедураЗавершения = "ОбновлениеИнформационнойБазыСлужебный.ЗавершитьРегистрациюДанныхОтложенногоОбновления";
...
//
Процедура ЗавершитьРегистрациюДанныхОтложенногоОбновления(ОписаниеОбрабатываемыхДанных, АдресРезультата) Экспорт
	...
	УстановитьСвойствоОбработчика(ОписаниеОбрабатываемыхДанных.ИмяОбработчика, "ОбрабатываемыеДанные",
		ОбрабатываемыеДанные);

	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		Если Результат.ДанныеОбновления <> Неопределено Тогда
			МодульОбменДаннымиСервер = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСервер");
			МодульОбменДаннымиСервер.СохранитьДанныеОбновления(Результат.ДанныеОбновления,
				Результат.ИмяФайлаСИзменениями);
		КонецЕсли;
	КонецЕсли;
...
Функция НовоеОписаниеГруппПотоковОбработчиковОтложенногоОбновления() Экспорт
	...
	ГруппаОбновления.Процедура = "ОбновлениеИнформационнойБазыСлужебный.ВыполнитьОтложенныйОбработчик";
	ГруппаОбновления.ПроцедураЗавершения = "ОбновлениеИнформационнойБазыСлужебный.ЗавершитьВыполнениеОтложенногоОбработчика";
	ГруппаОбновления.ПриАварийномЗавершении = "ОбновлениеИнформационнойБазыСлужебный.ПриАварийномЗавершенииПотокаОтложенногоОбработчика";
	ГруппаОбновления.ПриОтменеПотока = "ОбновлениеИнформационнойБазыСлужебный.ПриОтменеПотокаОтложенногоОбработчика";

	ГруппаПоиска = НовоеОписаниеГруппыПотоков();
	ГруппаПоиска.Процедура = "ОбновлениеИнформационнойБазыСлужебный.НайтиПорциюДляОбновления";
	ГруппаПоиска.ПроцедураЗавершения = "ОбновлениеИнформационнойБазыСлужебный.ЗавершитьПоискПорцииДляОбновления";
	ГруппаПоиска.ПриАварийномЗавершении = "ОбновлениеИнформационнойБазыСлужебный.ПриАварийномЗавершенииПотокаПоискаПорцииДляОбновления";
	ГруппаПоиска.ПриОтменеПотока = "ОбновлениеИнформационнойБазыСлужебный.ПриОтменеПотокаПоискаПорцииДляОбновления";
		...
		Если ТипЗнч(ОбработчикОбновления) = Тип("СтрокаТаблицыЗначений") Тогда
			Если КонтекстОбработчика.ВыполнитьОбработчик Тогда
				...
				Если ОбработчикОбновления.Многопоточный Тогда
					...
					Добавлен = ДобавитьПотокПоискаДанныхДляОбновления(Поток, ОбработчикОбновления, КонтекстОбработчика,
						СведенияОбОбновлении);
...
Функция ДобавитьПотокПоискаДанныхДляОбновления(Поток, Обработчик, КонтекстОбработчика, СведенияОбОбновлении)
	...
	Если Не Описание.ВыполняетсяПоискПорции Тогда
		...
		НачатьТранзакцию();
		Попытка
			УстановитьСвойствоОбработчика(ИмяОбработчика, "ОбрабатываемыеДанные", Новый ХранилищеЗначения(Описание));
			СохранитьПотокОбновления(Поток);
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
...
Процедура РассчитатьВремяВыполненияПроцедурыОбработчика(КонтекстОбработчика, ИмяОбработчика)
	...
	УстановитьСвойствоОбработчика(ОбработчикОбновления.ИмяОбработчика, "СтатистикаВыполнения",
...
Процедура ВыполнитьПоток(Группы, Поток, ИдентификаторФормы = Неопределено)
	...
	Если Не ПустаяСтрока(ОписаниеПотока.Процедура) И Поток.ПараметрыПроцедуры <> Неопределено Тогда
		...
		РезультатЗапуска = ДлительныеОперации.ВыполнитьВФоне(ОписаниеПотока.Процедура, Поток.ПараметрыПроцедуры,
			ПараметрыВыполнения);
...
Функция ЗавершитьПотокиВыполнившиеФЗ(Потоки, Группы)
	...
	Пока Индекс >= 0 Цикл
		...
		Если Не ЗначениеЗаполнено(ИдентификаторЗадания) Или ЗаданиеВыполнено <> Ложь Тогда
			Выполнять = Не ПустаяСтрока(ОписаниеПотока.ПроцедураЗавершения) И Поток.ПараметрыПроцедурыЗавершения <> Неопределено
				И (Не ЗначениеЗаполнено(ИдентификаторЗадания) Или ЗаданиеВыполнено = Истина) И Поток.АдресРезультата
				<> Неопределено;
...
Процедура НайтиПорциюДляОбновления(ПараметрыПоиска, АдресРезультата) Экспорт
	...
	Пока ИтераторОбхода <> Неопределено Цикл
		...
		ПараметрыВыборки.ДополнительныеИсточникиДанных = ОбновлениеИнформационнойБазы.ИсточникиДанных(
			ДополнительныеИсточникиДанных, СсылочныйОбъект, ТабличныйОбъект);
...
Процедура ПроверитьПараметрыВыборки(ПараметрыВыборки)
	...
	ИзвестныйСпособ = (СпособВыборки = ОбновлениеИнформационнойБазы.СпособВыборкиИзмеренияНезависимогоРегистраСведений())
		Или (СпособВыборки = ОбновлениеИнформационнойБазы.СпособВыборкиРегистраторыРегистра()) Или (СпособВыборки
	...
	Если Не ИзвестныйСпособ Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СпособВыборки,
			"Параметры.ПараметрыВыборки.СпособВыборки");
	КонецЕсли;

	ТаблицыУказаны = Не ПустаяСтрока(ПараметрыВыборки.ПолныеИменаОбъектов) Или Не ПустаяСтрока(
		ПараметрыВыборки.ПолныеИменаРегистров);
...
Процедура УстановитьГраницуКонцаВыборки(ПараметрыПоиска, СсылочныйОбъект, ТабличныйОбъект)
	...
	ЭтоПоследнийОбъект = ПоследняяЗапись <> Неопределено И СсылочныйОбъект = ПоследняяЗапись[0].Значение
...
Функция ВыбратьДанныеПорции(ПараметрыВыборки, Очередь, СсылочныйОбъект, ТабличныйОбъект)
	...
	Если СпособВыборки = ОбновлениеИнформационнойБазы.СпособВыборкиИзмеренияНезависимогоРегистраСведений() Тогда
		Данные = ОбновлениеИнформационнойБазы.ВыбратьИзмеренияНезависимогоРегистраСведенийДляОбработки(
			Очередь, ТабличныйОбъект, ПараметрыВыборки);
	ИначеЕсли СпособВыборки = ОбновлениеИнформационнойБазы.СпособВыборкиРегистраторыРегистра() Тогда
		Данные = ОбновлениеИнформационнойБазы.ВыбратьРегистраторыРегистраДляОбработки(
			Очередь, ?(ПустаяСтрока(СсылочныйОбъект), Неопределено, СсылочныйОбъект), ТабличныйОбъект, ПараметрыВыборки);
	ИначеЕсли СпособВыборки = ОбновлениеИнформационнойБазы.СпособВыборкиСсылки() Тогда
		Данные = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(
			Очередь, СсылочныйОбъект, ПараметрыВыборки);
	КонецЕсли;
...
Функция ПоляУпорядочиванияПриПоискеПорций(ПараметрыПоиска)
	...
	ПараметрыВыборки = ПараметрыПоиска.ПараметрыВыборки;
	Возврат ?(ПараметрыПоиска.ФорсироватьОбновление, ПараметрыВыборки.ПоляУпорядочиванияПриОбработкеДанных,
...
Процедура ЗавершитьПоискПорцииДляОбновления(ПараметрыПоиска, АдресРезультата) Экспорт
	...
	ЭтоПервыйПоиск = ПараметрыПоиска.ПоследняяВыбраннаяЗапись = Неопределено И ПараметрыПоиска.ПерваяЗапись = Неопределено
	...
	УстановитьСвойствоОбработчика(ОбработчикОбновления.ИмяОбработчика, "ОбрабатываемыеДанные",
...
Процедура ПриАварийномЗавершенииПотокаПоискаПорцииДляОбновления(Поток, ИнформацияОбОшибке) Экспорт
	...
	УстановитьСвойствоОбработчика(Поток.ПараметрыПроцедуры.ИмяОбработчика, "ОбрабатываемыеДанные",
...
Процедура СохранитьРезультатПовторногоПоиска(РезультатПоиска, ОписаниеПроцедурыЗаполнения, ИдентификаторПорции)
	...
	Если ОписаниеПроцедурыЗаполнения.Свойство("ИмяОбработчика") Тогда
		УстановитьСвойствоОбработчика(ОписаниеПроцедурыЗаполнения.ИмяОбработчика, "ОбрабатываемыеДанные",
			Новый ХранилищеЗначения(ОписаниеПроцедурыЗаполнения));
	КонецЕсли;
...
Функция РазбитьРезультатПоискаНаФрагменты(РезультатПоиска, Знач КоличествоФрагментов)
	...
	Для НомерФрагмента = 1 По КоличествоФрагментов Цикл // Разбивка идет с конца найденного набора данных.
		...
		КоличествоСвободных = ?(НомерФрагмента = КоличествоФрагментов, КоличествоНайденных - КоличествоОбработанных,
		...
		Пока ИндексНабораДанных >= 0 Цикл
			...
			Иначе
				...
				ДанныеФрагмента.Данные = ВырезатьСтрокиИзТаблицыЗначений(ТекущиеДанные, НачалоВырезки,
					КоличествоСвободных);
...
Функция КлючЗаписиПервойСтрокиНабораДанных(НаборДанных)
	...
	Возврат НовыйКлючЗаписиИзТаблицыДанных(ПерваяСтрокаДанных.СсылочныйОбъект, ПерваяСтрокаДанных.ТабличныйОбъект,
		ПерваяСтрокаДанных.Данные, 0);
...
Функция КлючЗаписиПоследнейСтрокиНабораДанных(НаборДанных)
	...
	Возврат НовыйКлючЗаписиИзТаблицыДанных(ПоследняяСтрокаДанных.СсылочныйОбъект, ПоследняяСтрокаДанных.ТабличныйОбъект,
		ПоследняяСтрокаДанных.Данные, ПоследняяСтрокаДанных.Данные.Количество() - 1);
...
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры, ОбъектМетаданных = Неопределено,
	...
	ОбъектыСПредопределеннымиЭлементами = ?(ОбъектМетаданных <> Неопределено,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОбъектМетаданных), ОбъектыСНачальнымЗаполнением());
	...
	Для Каждого ОбъектМетаданныхСЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			ЛокализуемыеРеквизитыОбъекта = МодульМультиязычностьСервер.МультиязычныеРеквизитыОбъекта(
				ОбъектМетаданныхСЭлементами);
		КонецЕсли;

		ПредопределенныеДанные       = ПредопределенныеДанныеОбъекта(ОбъектМетаданныхСЭлементами, МенеджерОбъекта,
			ЛокализуемыеРеквизитыОбъекта);
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			МодульМультиязычностьСервер.СформироватьИменаМультиязычныхРеквизитов(ИменаРеквизитовОбъекта,
				ЛокализуемыеРеквизитыОбъекта);
		КонецЕсли;
		...
		Пока ДанныеЗапроса.Следующий() Цикл
			...
			ПоставляемыеСведения = ПредопределенныеДанные.Найти(ДанныеЗапроса.ИмяПредопределенныхДанных,
			...
			Если ПоставляемыеСведения <> Неопределено Тогда
				...
				ИначеЕсли ПараметрыРегистрации.РежимОбновления = "НовыеИИзмененные" Тогда
					...
					Если ДанныеСодержатОтличия(ДанныеЗапроса, ИменаРеквизитовОбъекта, ПоставляемыеСведения,
						ЛокализуемыеРеквизитыОбъекта, ПараметрыРегистрации) Тогда
						ОбъектыКОбработке.Добавить(ДанныеЗапроса.Ссылка);
					КонецЕсли;
...
Процедура ЗаполнитьЭлементыНачальнымиДанными(Параметры, ОбъектМетаданных, НастройкиЗаполнения) Экспорт
	...
	СсылкиНаОбъекты = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь,
		ОбъектМетаданных.ПолноеИмя());

	Если НастройкиЗаполнения.ОбновитьТолькоМультиязычныеСтроки Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			Результат = МодульМультиязычностьСервер.ОбновитьМультиязычныеСтрокиПредопределенныхЭлементов(
				СсылкиНаОбъекты, ОбъектМетаданных);
		КонецЕсли;
	Иначе
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация, ОбъектМетаданных, , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Обработана очередная порция элементов: %1'"), Результат.ОбъектовОбработано));
	КонецЕсли;
...
Функция ДанныеСодержатОтличия(ДанныеЗапроса, ИмяРеквизитОбъекта, ПоставляемыеСведения, ЛокализуемыеРеквизитыОбъекта,
	ПараметрыРегистрации)

	Для Каждого ИмяРеквизитаОбъекта Из ИмяРеквизитОбъекта Цикл

		Если СтрСравнить(ИмяРеквизитаОбъекта, "Предопределенный") = 0 Или СтрСравнить(ИмяРеквизитаОбъекта, "Ссылка")
			= 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЛокализуемыеРеквизитыОбъекта[ИмяРеквизитаБезСуффиксаЯзык] = Неопределено Тогда
			...
			Если ТипЗнч(ПоставляемыеСведения[ИмяРеквизитаОбъекта]) = Тип("ТаблицаЗначений") И ТипЗнч(
				ДанныеЗапроса[ИмяРеквизитаОбъекта]) = Тип("РезультатЗапроса") Тогда

				Если ПараметрыРегистрации.СравниватьТабличныеЧасти Тогда
					ТаблицаДанных = ДанныеЗапроса[ИмяРеквизитаОбъекта].Выгрузить();
					Если Не ОбщегоНазначения.КоллекцииИдентичны(ПоставляемыеСведения[ИмяРеквизитаОбъекта],
						ТаблицаДанных) Тогда
						Возврат Истина;
					КонецЕсли;
				КонецЕсли;
		...
		Иначе
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
				МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
				Если МодульМультиязычностьСервер.МультиязычныеСтрокиРеквизитаИзменены(ПоставляемыеСведения,
					ДанныеЗапроса, ИмяРеквизитаБезСуффиксаЯзык, ПараметрыРегистрации) Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьРеквизитыНачальнымиДанными(ЗаполняемыйЭлемент, НаборДанныхДляЗаполнения, НастройкиЗаполнения)
	...
	Если НаборДанныхДляЗаполнения.ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа Тогда
		...
		КромеРеквизитов = НаборДанныхДляЗаполнения.РеквизитыИсключения + ?(ПустаяСтрока(
			НаборДанныхДляЗаполнения.РеквизитыИсключения), "", ",") + СтрСоединить(
			НаборДанныхДляЗаполнения.РеквизитыСЭлементами, ",");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		МодульМультиязычностьСервер.ЗаполнитьЭлементыМультиязычнымиНачальнымиДанными(ЗаполняемыйЭлемент,
			НаборДанныхДляЗаполнения.ЛокализуемыеРеквизитыОбъекта,
			НаборДанныхДляЗаполнения.МультиязычныеСтрокиВРеквизитах, НаборДанныхДляЗаполнения.ПоддерживаетсяИерархия,
			СтрокаТаблицы);
	КонецЕсли;
	...
	Если НаборДанныхДляЗаполнения.НастройкиПредопределенныхЭлементов.ПереопределяемыеНастройки.ПриНачальномЗаполненииЭлемента Тогда
		МенеджерОбъекта.ПриНачальномЗаполненииЭлемента(ЗаполняемыйЭлемент, СтрокаТаблицы,
			НаборДанныхДляЗаполнения.НастройкиПредопределенныхЭлементов.ПереопределяемыеНастройки.ДополнительныеПараметры);
	КонецЕсли;
...
Функция ОбновитьЭлементыПредопределенныхЭлементов(СсылкиНаОбъекты, МетаданныеОбъекта, НастройкиЗаполнения)
	...
	Пока СсылкиНаОбъекты.Следующий() Цикл
		...
		СтрокаТаблицы = ПараметрыДляЗаполненияОбъекта.ПредопределенныеДанные.Найти(ЗначениеКлючевогоРеквизита,
		...
		Попытка
			...
			Если ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа Тогда
				КромеРеквизитов = РеквизитыИсключения + ?(ПустаяСтрока(РеквизитыИсключения), "", ",") + СтрСоединить(
					ПараметрыДляЗаполненияОбъекта.РеквизитыСЭлементами, ",");
			КонецЕсли;
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
				МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
				МодульМультиязычностьСервер.ЗаполнитьЭлементыМультиязычнымиНачальнымиДанными(ЗаполняемыйЭлемент,
					ПараметрыДляЗаполненияОбъекта.ЛокализуемыеРеквизитыОбъекта,
					ПараметрыДляЗаполненияОбъекта.МультиязычныеСтрокиВРеквизитах, ПоддерживаетсяИерархия, СтрокаТаблицы);
			КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось заполнить элемент: %1 по причине: %2'"), СсылкиНаОбъекты.Ссылка,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение, МетаданныеОбъекта, СсылкиНаОбъекты.Ссылка, ТекстСообщения);
		КонецПопытки;
...
Функция СуществующиеПоставляемыеЭлементы(ПредопределенныеДанные, НастройкиПредопределенныхЭлементов,
	Знач МенеджерОбъекта, МетаданныеОбъекта)
	...
	ИначеЕсли ЕстьКолонкаИмяПредопределенныхДанных Тогда
		...
		Для Каждого ПоставляемыйЭлемент Из ПредопределенныеДанные Цикл
			...
			Если ЗначениеЗаполнено(ПоставляемыйЭлемент["ИмяПредопределенныхДанных"]) Тогда
				Результат.Вставить(ПоставляемыйЭлемент["ИмяПредопределенныхДанных"],
					МенеджерОбъекта[ПоставляемыйЭлемент[ИмяКлючевогоРеквизита]]);
				Продолжить;
			КонецЕсли;
		КонецЦикла;
...
Функция ВозможноОперативноеОбновление(ПроверяемыеИтерацииОбновления = Неопределено) Экспорт
	...
	Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
		...
		Для Каждого ФлагРазделенности Из ФильтрыРазделенностиОбработчиков Цикл
			...
			Если ФлагРазделенности И ОбщегоНазначения.РазделениеВключено()
			...
			НайденныеОбработчики = ДеревоОбработчиков.Строки[0].Строки.НайтиСтроки(Новый Структура("МонопольныйРежим",
				Неопределено));
	...
	Если ПроцедурыОбработчиков.Количество() <> 0 Тогда
		ТекстСообщения = НСтр(
			"ru = 'Следующие обработчики не поддерживают обновление без установки монопольного режима:'");
...
Функция ПолучитьПланОбновления(Знач ИдентификаторБиблиотеки, Знач ВерсияС, Знач ВерсияНа)
	...
	Иначе
		...
		Если ОписаниеПлана.ВерсияС <> ВерсияС Или ОписаниеПлана.ВерсияНа <> ВерсияНа Тогда
...
Процедура ВыполнитьОбработчикОбновления(Обработчик, Параметры, ДополнительныеПараметры)
	...
	ЗаписатьИнформациюОХодеОбновления(Обработчик, ДополнительныеПараметры.ХодВыполненияОбработчиков,
		ДополнительныеПараметры.ВФоне);
	ОписаниеОбработчика = ПодготовитьДетальнуюИнформациюОХодеОбновления(Обработчик, Параметры,
		ДополнительныеПараметры.ИдентификаторБиблиотеки);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		МодульОценкаПроизводительности = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительности");
		МодульОценкаПроизводительности.ЗакончитьЗамерВремениТехнологический("ВремяВыполненияОбработчикаОбновления."
			+ ОписаниеОбработчика.Процедура, НачалоЗамера);
	КонецЕсли;

КонецПроцедуры

Процедура ВыполнитьОбработчикиПослеОбновленияИнформационнойБазы(Знач ИтерацииОбновления, Знач ЗаписыватьВЖурнал,
	ВыводитьОписаниеОбновлений, Знач ОперативноеОбновление)
	...
	Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
		...
		Если ЗаписыватьВЖурнал Тогда
			...
			Обработчик.Вставить("Процедура", ИтерацияОбновления.ИмяОсновногоСерверногоМодуля
				+ ".ПослеОбновленияИнформационнойБазы");
			ОписаниеОбработчика =  ПодготовитьДетальнуюИнформациюОХодеОбновления(Обработчик, Неопределено,
		...
		Попытка
			...
			ИтерацияОбновления.ОсновнойСерверныйМодуль.ПослеОбновленияИнформационнойБазы(
				ИтерацияОбновления.ПредыдущаяВерсия, ИтерацияОбновления.Версия,
				ИтерацияОбновления.ВыполненныеОбработчики, ВыводитьОписаниеОбновлений, Не ОперативноеОбновление);
...
//   * Параметры - Неопределено
// 
Функция ПодготовитьДетальнуюИнформациюОХодеОбновления(Обработчик, Параметры, ИдентификаторБиблиотеки,
	ОбработчикОтложенный = Ложь)
	...
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ОписаниеОбработчика.Вставить("ОбластьДанныхЗначение", МодульРаботаВМоделиСервиса.ЗначениеРазделителяСеанса());
...
Процедура ЗаписатьДетальнуюИнформациюОХодеОбновления(ОписаниеОбработчика)
	...
	ЗаписьЖурналаРегистрации(
		СобытиеЖурналаРегистрацииПротокол(), УровеньЖурналаРегистрации.Информация, , ,
	...
	Для Каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		Если ПустаяСтрока(Обработчик.Версия) Тогда
			...
			Если Обработчик.НачальноеЗаполнение <> Истина Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У обработчика не заполнено свойство %1 или свойство %2.'"), "Версия",
					"НачальноеЗаполнение");
			КонецЕсли;
			...
			Исключение
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У обработчика неправильно заполнено свойство %1: ""%2"".
						 |Правильный формат, например: ""2.1.3.70"".'"), "Версия", Обработчик.Версия);
			КонецПопытки;

			Если НулеваяВерсия Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У обработчика неправильно заполнено свойство %1: ""%2"".
						 |Версия не может быть нулевой.'"), "Версия", Обработчик.Версия);
			КонецЕсли;

			Если Не ЗначениеЗаполнено(ОписаниеОшибки) И Обработчик.ВыполнятьВГруппеОбязательных <> Истина
				И Обработчик.Приоритет <> 0 Тогда

				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У обработчика неправильно заполнено свойство %1 или
						 |свойство %2.'"), "Приоритет", "ВыполнятьВГруппеОбязательных");
			КонецЕсли;
		КонецЕсли;

		Если Обработчик.РежимВыполнения <> "" И Обработчик.РежимВыполнения <> "Монопольно" И Обработчик.РежимВыполнения
			<> "Оперативно" И Обработчик.РежимВыполнения <> "Отложенно" Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У обработчика ""%1"" неправильно заполнено свойство %2.
					 |Допустимое значение: ""%3"", ""%4"", ""%5"".'"), Обработчик.Процедура, "РежимВыполнения",
				"Монопольно", "Отложенно", "Оперативно");
		КонецЕсли;

		Если Не ЗначениеЗаполнено(ОписаниеОшибки) И Обработчик.Опциональный = Истина И Обработчик.НачальноеЗаполнение
			= Истина Тогда

			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У обработчика не правильно заполнено свойство %1 или
					 |свойство %2.'"), "Опциональный", "НачальноеЗаполнение");
		КонецЕсли;
		...
		Иначе
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ИтерацияОбновления.Подсистема, ИтерацияОбновления.Версия);
		КонецЕсли;

		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС + НСтр("ru = '(%1).'") + Символы.ПС + Символы.ПС + ОписаниеОшибки,
...
Функция КоличествоОбработчиковНаТекущуюВерсию(ИтерацииОбновления, РежимВыполненияОтложенногоОбновления)
	...
	КоличествоОбработчиков = КоличествоРазделенныхОбработчиков + КоличествоНеразделенныхОбработчиков
		+ КоличествоПроцедурРегистрации;
...
Процедура ПриОтправкеВерсийПодсистем(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза = Ложь)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.ПриОтправкеВерсийПодсистем(ЭлементДанных,
			ОтправкаЭлемента, СозданиеНачальногоОбраза, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ЗаписатьИнформациюОХодеОбновления(Обработчик, ХодВыполненияОбработчиков, ВФоне)
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Сообщение, Обработчик.Процедура, ХодВыполненияОбработчиков.ВыполненоОбработчиков,
			ХодВыполненияОбработчиков.ВсегоОбработчиков);
		ЗаписатьИнформацию(Сообщение);
	КонецЕсли;

	Если ВФоне Тогда
		Прогресс = 10 + ХодВыполненияОбработчиков.ВыполненоОбработчиков / ХодВыполненияОбработчиков.ВсегоОбработчиков
			* 90;
...
Процедура ОпределитьВыводОписанияОбновлений(ВыводитьОписаниеОбновлений)
	...
	Если ВыводитьОписаниеОбновлений И Не ОбщегоНазначения.РазделениеВключено() Тогда
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбновлениеИБ", "ВывестиОписаниеИзмененийДляАдминистратора",
...
Функция РазделыОписанияИзменений() Экспорт
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		...
		Для Каждого Область Из Макет.Области Цикл
			...
			Версия = "" + Число(РазрядыВерсииСтроками[0]) + "." + Число(РазрядыВерсииСтроками[1]) + "." + Число(
				РазрядыВерсииСтроками[2]) + "." + Число(РазрядыВерсииСтроками[3]);
...
Функция ВесВерсииИзМассиваСтрок(РазрядыВерсииСтроками)
	...
	Возврат 0 + Число(РазрядыВерсииСтроками[0]) * 1000000000 + Число(РазрядыВерсииСтроками[1]) * 1000000 + Число(
		РазрядыВерсииСтроками[2]) * 1000 + Число(РазрядыВерсииСтроками[3]);
...
Функция КоличествоПроблемСДанными() Экспорт
	...
	Проверка          = МодульКонтрольВеденияУчета.ПроверкаПоИдентификатору(
		"ОбновлениеИнформационнойБазыПроблемаСДанными");
		...
		Попытка
			...
			Если ОбработчикОбновления.Многопоточный Тогда
				...
				Пока ИтераторОбхода <> Неопределено Цикл
					...
					ПараметрыВыборки.ДополнительныеИсточникиДанных = ОбновлениеИнформационнойБазы.ИсточникиДанных(
						ДополнительныеИсточникиДанных, СсылочныйОбъект, ТабличныйОбъект);
					ЗаписьДанных.Данные = ВыбратьДанныеПорции(ПараметрыВыборки, Очередь, СсылочныйОбъект,
						ТабличныйОбъект);
					...
					Если Количество > 0 Тогда
						ОбрабатываемыеДанные.ПоследняяВыбраннаяЗапись = КлючЗаписиПоследнейСтрокиНабораДанных(
							НаборДанных);
					Иначе
...
Процедура ЗавершитьВыполнениеОтложенногоОбработчикаОбновления(КонтекстОбработчика)
	...
	ИначеЕсли ОбработчикОбновления.Статус = Перечисления.СтатусыОбработчиковОбновления.Выполняется Тогда
		...
		Если ОбработчикОбновления.Приоритет = "ЖелательноБыстрее" Тогда
			...
			ЗапускиСПриоритетом = ?(ЗапускиСПриоритетом = Неопределено, 1, ?(ЗапускиСПриоритетом = 4, 0,
				ЗапускиСПриоритетом + 1));
	...
	Если ПараллельныйРежим И ОбработчикОбновления.Статус = Перечисления.СтатусыОбработчиковОбновления.Ошибка
		...
		ШаблонОшибки = НСтр(
			"ru = 'Не удалось выполнить обработчик обновления ""%1"". Подробнее в журнале регистрации.'");
...
Функция ПереданныеПараметрыОбработчикаОбновления(Параметры)
	...
	Для Каждого Параметр Из Параметры Цикл
		Если Параметр.Ключ <> "ОбработкаЗавершена" И Параметр.Ключ <> "ПрогрессВыполнения" И Параметр.Ключ <> "Очередь"
...
Функция РежимВыполненияОтложенногоОбновления(ПараметрыОбновления)
	...
	Если Не РазделениеВключено Или ДоступноИспользованиеРазделенныхДанных Тогда
		Если ИнформационнаяБазаФайловая Или СтрНайти(НРег(ПараметрЗапускаКлиента), НРег(
			"ВыполнитьОтложенноеОбновлениеСейчас")) > 0 Или ВыполнятьОтложенныеОбработчики Тогда
			Возврат "Монопольно";
		Иначе
			Возврат "Отложенно";
		КонецЕсли;
...
//  СведенияОбОбновлении - см. СведенияОбОбновленииИнформационнойБазы
//
Процедура ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, ПараметрыОбновления,
	СведенияОбОбновлении)
	...
	Попытка
		...
		СообщениеОЗапущенномОбработчике = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Выполняется процедура обновления ""%1"".'"), ИмяОбработчика);
		ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
		...
		СтатистикаВыполнения = ОбработчикОбновления.СтатистикаВыполнения.Получить();
		Если СтатистикаВыполнения["ПрогрессВыполнения"] <> Неопределено И ТипЗнч(
			СтатистикаВыполнения["ПрогрессВыполнения"]) = Тип("Структура") Тогда
		...
		Если КонтекстОбработчика.ЗаписыватьВЖурнал Тогда
			КонтекстОбработчика.ДетальноеОписаниеОбработчика = ПодготовитьДетальнуюИнформациюОХодеОбновления(
				ОбработчикОбновления, Параметры, ОбработчикОбновления.ИмяБиблиотеки, Истина);
		КонецЕсли;
		...
		Если КоличествоЗапусковПроцедурыОбновления > 10000 Тогда // Защита от зацикливания.
			Если ПараметрыОбновления.ПараллельныйРежим И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ()
			...
			Если Не ПараметрыОбновления.ПараллельныйРежим Или МинимальнаяОчередь
				= ОбработчикОбновления.ОчередьОтложеннойОбработки Тогда
			...
			Иначе
				Для Каждого ОбрабатываемыйОбъект Из ДанныеОбработчика Цикл
					Параметры.ОбработкаЗавершена = ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(
						ОбработчикОбновления.ОчередьОтложеннойОбработки, ОбрабатываемыйОбъект.Ключ);
...
Процедура ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ИмяОбработчика)
	...
	Попытка
		...
		Если Параметры.ОбработкаЗавершена И Не КонтекстОбработчика.Свойство("ОбработкаИсключенияОбработчика") Тогда
			...
			Если ПараметрыОбновления.Свойство("ВФоне") И ПараметрыОбновления.ВФоне Тогда
				...
				ХодВыполненияОбработчиков.ВыполненоОбработчиков = ХодВыполненияОбработчиков.ВыполненоОбработчиков + 1;
				Прогресс = 10 + ХодВыполненияОбработчиков.ВыполненоОбработчиков
					/ ХодВыполненияОбработчиков.ВсегоОбработчиков * 90;
				ДлительныеОперации.СообщитьПрогресс(Прогресс);
			КонецЕсли;
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим И Не КонтекстОбработчика.ПропуститьПроверкуОбработанныхДанных Тогда
			...
			Если Не ЕстьОбработанныеОбъекты И ОчередьОбработчика = МинимальнаяОчередь Тогда
				...
				Если ЧислоПопыток >= МаксимумПопыток Тогда
					ТекстИсключения = НСтр(
						"ru = 'Произошло зацикливание процедуры обработки данных. Выполнение прервано.'");
					ВызватьИсключение ТекстИсключения;
				Иначе
...
Процедура ЗавершитьПроцедуруОбработкиДанных(КонтекстОбработчика, ИмяОбработчика)
	...
	Если КонтекстОбработчика.ЕстьНеЗакрытыеТранзакции Тогда
		...
		ОбработчикОбновления.ИнформацияОбОшибке = Строка(ОбработчикОбновления.ИнформацияОбОшибке) + Символы.ПС
			+ КонтекстОбработчика.ИнформацияОбОшибке;
...
Процедура ЗаполнитьДанныеДляПараллельногоОтложенногоОбновления(Параметры) Экспорт
	...
	Если Не (СтандартныеПодсистемыПовтИсп.ИспользуетсяРИБ("СФильтром") И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ())
		И Не ПерезапускОбновления Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
	...
	Если Не ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() И ОбщегоНазначения.ПодсистемаСуществует(
		"СтандартныеПодсистемы.ОбменДанными") Тогда
		МодульОбменДаннымиСервер = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСервер");
...
Процедура ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеОбрабатываемыхДанных, АдресРезультата) Экспорт
	...
	Если МногопоточныйРежим Тогда
		ПараметрыОтметкиКОбработке.ПараметрыВыборки = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляМногопоточнойОбработки();
	Иначе
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
		ОписаниеОбрабатываемыхДанных.ПроцедураЗаполнения, ОписаниеОбрабатываемыхДанных.ИмяОбработчика);
	ЗаписатьИнформацию(ТекстСообщения);

	Попытка
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеОбрабатываемыхДанных.ПроцедураЗаполнения,
			ПараметрыОбработчика);
	Исключение
	...
	Если ЕстьПодсистемаОбменаДанными Тогда
		ДанныеОбновления = МодульОбменДаннымиСервер.ЗавершитьЗаписьФайлаИПолучитьДанныеОбновления(
			ПараметрыОтметкиКОбработке);
...
Процедура УстановитьПараметрыОбработчикаОбновления(ОбработчикОбновления, Отложенно = Ложь, Параллельно = Ложь)
	...
	ПараметрыОбработчикаОбновления.ИзменяемыеОбъекты = УникальныеЗначенияЧерезЗапятую(
		ПолучениеСвойств.ИзменяемыеОбъекты);
...
Процедура ОбработатьИсключениеОбработчика(КонтекстОбработчика, ОбработчикОбновления, ИнформацияОбОшибке)
	...
	Иначе
		ОбновлениеИнформационнойБазы.ЗаписатьСобытиеВЖурналРегистрации(ПодробноеПредставлениеОшибки, ,
			КонтекстОбработчика.Параметры);
	КонецЕсли;
...
Функция ДобавляемоеЧислоПопыток(ОбработчикОбновления, КонтекстОбработчика, Ошибка = Ложь)
	...
	Если ОбработчикОбновления.Многопоточный Тогда
		...
		Если ОбновляемыеДанные <> Неопределено Тогда
			ЕстьДанные = ОбновляемыеДанные.ПерваяЗапись <> Неопределено Или ОбновляемыеДанные.ПоследняяЗапись
				<> Неопределено;
...
Процедура ДобавитьИнформациюОбОшибкеВОбработчике(ИмяОбработчика) Экспорт
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось сохранить информацию об ошибке в обработчике %1
						   |по причине:
						   |%2'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ИмяОбработчика,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписатьОшибку(ТекстОшибки);
	КонецПопытки;
