...
Процедура ДобавитьФайл(ОбработчикРезультата, ВладелецФайла, ФормаВладелец, РежимСоздания = Неопределено,
	...
	Если ПараметрыДобавления = Неопределено Или ТипЗнч(ПараметрыДобавления) = Тип("Булево") Тогда
		...
		ПараметрыВыполнения.Вставить("НеОткрыватьКарточку", ?(ПараметрыДобавления = Неопределено, Ложь,
			ПараметрыДобавления));
	...
	Если РежимСоздания = Неопределено Тогда
		РаботаСФайламиСлужебныйКлиент.ДобавитьФайл(ОбработчикРезультата, ВладелецФайла, ФормаВладелец, ,
			ПараметрыВыполнения);
	Иначе
...
Процедура ПоказатьПодтверждениеЗакрытияФормыСФайлами(Форма, Отказ, ЗавершениеРаботы, ВладелецФайлов,
	...
	ТекстВопроса = НСтр("ru = 'Один или несколько файлов заняты для редактирования.
						|
						|Продолжить?'");
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияПроизвольнойФормы(Форма, Отказ, ЗавершениеРаботы, ТекстВопроса,
		ИмяРеквизита);
...
Процедура СохранитьВместеСЭП(Знач ПрисоединенныйФайл, Знач ИдентификаторФормы) Экспорт
	...
	ОписаниеДанных.Вставить("Данные", Новый ОписаниеОповещения("ПриСохраненииДанныхФайла",
		РаботаСФайламиСлужебныйКлиент, ПараметрыВыполнения));
...
Процедура СохранитьФайлКак(Знач ДанныеФайла, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("СохранитьФайлКакПослеСохранения", РаботаСФайламиСлужебныйКлиент,
...
Процедура ОткрытьФормуВыбораФайлов(Знач ВладелецФайлов, Знач ЭлементФормы, СтандартнаяОбработка = Ложь,
	...
	Если ВладелецФайлов.Пустая() Тогда
		ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВопросОНеобходимостиЗаписиПослеЗавершения",
			ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещенияОЗакрытии, НСтр("ru = 'Данные еще не записаны. 
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не МодульЭлектроннаяПодписьКлиент.ИспользоватьЭлектронныеПодписи() Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'Чтобы добавить электронную подпись, включите
	...
	РаботаСФайламиСлужебныйКлиент.ПодписатьФайл(ПрисоединенныйФайл, ДополнительныеПараметры.ДанныеФайла,
		ИдентификаторФормы, ОбработкаРезультата);
...
//
Функция ДанныеФайла(Знач ФайлСсылка, Знач ИдентификаторФормы = Неопределено,
	...
	Возврат РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайла(
		ФайлСсылка, ИдентификаторФормы, ПолучатьСсылкуНаДвоичныеДанные, ДляРедактирования);
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы,
	...
	РаботаСФайламиСлужебныйКлиент.ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы,
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы,
	...
	РаботаСФайламиСлужебныйКлиент.ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы,
		ДополнительныеПараметры);
...
Процедура ОткрытьФормуНастройкиСканирования() Экспорт
	...
	Если Не РаботаСФайламиСлужебныйКлиент.ДоступноСканирование() Тогда
		ТекстСообщения = НСтр(
			"ru = 'Для сканирования необходимо использовать 32-битную версию программы  для ОС Windows.'");
	...
	Если Не КомпонентаУстановлена Тогда
		ТекстВопроса = НСтр("ru = 'Для продолжения работы установите компоненту сканирования. 
							|Установить компоненту?'");
		Обработчик = Новый ОписаниеОповещения("ПоказатьВопросУстановкиКомпонентыСканирования", ЭтотОбъект,
...
Процедура ОбработкаОповещения(Форма, ИмяСобытия) Экспорт
	...
	Для НомерЭлемента = 0 По Форма.ПараметрыРаботыСФайлами.ВГраница() Цикл
		...
		ВладелецПрисоединенныхФайлов = ЗначениеПараметраПрисоединенногоФайла(Форма, НомерЭлемента,
			"ПутьКДаннымВладельца");
		КоличествоПрисоединенныхФайлов = РаботаСФайламиСлужебныйВызовСервера.КоличествоПрисоединенныхФайлов(
		...
		Если ПозицияКоличестваВЗаголовке = 0 Тогда
			Гиперссылка.Заголовок = Гиперссылка.Заголовок + ?(КоличествоПрисоединенныхФайлов = 0, "", " ("
				+ КоличествоПрисоединенныхФайловСтрокой + ")");
		Иначе
			Гиперссылка.Заголовок = Лев(Гиперссылка.Заголовок, ПозицияКоличестваВЗаголовке - 1) + ?(
				КоличествоПрисоединенныхФайлов = 0, "", "(" + КоличествоПрисоединенныхФайловСтрокой + ")");
		КонецЕсли;
...
Процедура ПолеПредпросмотраНажатие(Форма, Элемент, СтандартнаяОбработка, Просмотр = Ложь) Экспорт
	...
	ВладелецПрисоединенныхФайлов = ЗначениеПараметраПрисоединенногоФайла(Форма, Число(НомерЭлемента),
		"ПутьКДаннымВладельца");
...
Процедура ПолеПредпросмотраПеретаскивание(Форма, Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецПрисоединенныхФайлов) Тогда
		ЗадатьВопросОЗаписиВладельца(ПараметрыОбработчика);
	Иначе
		ОповещениеОбУстановке = Новый ОписаниеОповещения("ПолеПредпросмотраПеретаскиваниеЗавершение", ЭтотОбъект,
...
Процедура ПерейтиКФормеФайла(Знач Форма) Экспорт
	...
	Для Каждого ОкноПриложения Из ПолучитьОкна() Цикл
		...
		Если Содержимое.ИмяФормы = "Обработка.РаботаСФайлами.Форма.ПрисоединенныйФайл" Тогда
			Если Содержимое.Параметры.Свойство("ПрисоединенныйФайл") И Содержимое.Параметры.ПрисоединенныйФайл
...
#Если МобильныйКлиент Тогда
	ПоказатьПредупреждение( , НСтр(
		"ru = 'Печать файлов данного типа возможна только из приложения для Windows или Linux.'"));
	Возврат;
#Иначе
...
Процедура НапечататьФайлыВыполнение(ОбработчикРезультата, ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляПечати(
		ПараметрыВыполнения.ДанныеФайлов[ПараметрыВыполнения.НомерФайла], ПараметрыВыполнения.УникальныйИдентификатор);

#Если ВебКлиент Тогда
	Если ПараметрыВыполнения.ДанныеФайла.Расширение <> "mxl" Тогда
		Текст = НСтр(
			"ru = 'Сохраните файл на компьютер, после чего распечатайте его при помощи приложения, предназначенного для работы с данным файлом.'");
...
#Если Не МобильныйКлиент Тогда
	...
	ИначеЕсли ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НапечататьИзПриложенияПоИмениФайлаLinux", ЭтотОбъект,
			ДополнительныеПараметры);
...
Процедура НапечататьИзПриложенияПоИмениФайлаLinux(Результат, Параметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("НапечататьИзПриложенияПоИмениФайлаLinuxЗапускUnoconv", ЭтотОбъект,
		Параметры);
...
Процедура НапечататьИзПриложенияПоИмениФайлаLinuxЗапускUnoconv(ИмяВременногоФайла, Параметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("НапечататьИзПриложенияПоИмениФайлаLinuxЗапускLpr", ЭтотОбъект,
		ДополнительныеПараметры);
...
Процедура НапечататьИзПриложенияПоИмениФайлаLinuxЗапускLpr(Результат, Параметры) Экспорт
	...
	Иначе
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НапечататьИзПриложенияПоИмениФайлаLinuxЗавершение", ЭтотОбъект,
			Параметры);
		ПараметрыЗапускаПрограммы = ФайловаяСистемаКлиент.ПараметрыЗапускаПрограммы();
		ПараметрыЗапускаПрограммы.Вставить("ДождатьсяЗавершения", Истина);
		ПараметрыЗапускаПрограммы.Вставить("ПолучитьПотокОшибок", Истина);
...
Процедура НапечататьИзПриложенияПоИмениФайлаLinuxЗавершение(Результат, Параметры) Экспорт
	...
	Если Не Результат.ПриложениеЗапущено Или ЗначениеЗаполнено(Результат.ОписаниеОшибки) Или ЗначениеЗаполнено(
		Результат.ПотокОшибок) Тогда
		ПоказатьПредупреждение( , ?(ЗначениеЗаполнено(Результат.ОписаниеОшибки), Результат.ОписаниеОшибки + Символы.ПС,
...
Процедура ПолучитьПолноеИмяВременногоФайла(Знач Оповещение, Расширение = "")
	...
	Оповещение = Новый ОписаниеОповещения("ПолучитьИмяВременногоФайлаПослеПроверкиРасширенияРаботыСФайлами",
		ЭтотОбъект, Контекст);
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(Оповещение, НСтр(
		"ru = 'Для получения имени временного файла установите расширение для работы с 1С:Предприятием.'"), Ложь);
...
#Если ВебКлиент Тогда
		Оповещение = Новый ОписаниеОповещения("ПолучитьИмяВременногоФайлаПослеПолученияВременногоКаталога", ЭтотОбъект,
			Контекст, "ПолучитьИмяВременногоФайлаПриОбработкеОшибки", ЭтотОбъект);
		НачатьПолучениеКаталогаВременныхФайлов(Оповещение);
#Иначе
			ПолучитьИмяВременногоФайлаПослеПолученияВременногоКаталога("", Контекст);
#КонецЕсли
	Иначе
		ПолучитьИмяВременногоФайлаОповеститьОбОшибке(НСтр(
			"ru = 'Не удалось установить расширение для работы с 1С:Предприятием.'"), Контекст);
	КонецЕсли;
...
Процедура ЗадатьВопросОЗаписиВладельца(ПараметрыОбработчикаЗавершения)
	...
	ОбработчикОповещения = Новый ОписаниеОповещения("ПоказатьВопросЗаписиНовогоВладельца", ЭтотОбъект,
		ПараметрыОбработчикаЗавершения);
...
Процедура ПоказатьВопросЗаписиНовогоВладельца(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если ДополнительныеПараметры.Действие = "ВыполнениеКоманды" Тогда
			КомандаУправленияПрисоединеннымиФайламиЗавершение(Форма, ДополнительныеПараметры.Команда,
				ВладелецПрисоединенныхФайлов);
		ИначеЕсли ДополнительныеПараметры.Действие = "ПредпросмотрНажатие" Тогда
			ПолеПредпросмотраНажатиеЗавершение(Форма, ВладелецПрисоединенныхФайлов, ДополнительныеПараметры.Элемент,
				СтандартнаяОбработка, ДополнительныеПараметры.Просмотр, ДополнительныеПараметры.ТолькоОдинФайл);
...
Процедура КомандаУправленияПрисоединеннымиФайламиЗавершение(Форма, Команда, ВладелецПрисоединенныхФайлов)
	...
	ОбработчикЗавершения = Новый ОписаниеОповещения("ЗавершениеВыполненияКомандыСОповещением", ЭтотОбъект,
		ПараметрыВыполнения);
	...
	ИначеЕсли СтрНачинаетсяС(ИмяКоманды, "ЗагрузитьФайл") Тогда
		...
		ФайлыВладельца = РаботаСФайламиСлужебныйВызовСервера.КоличествоПрисоединенныхФайлов(
			ВладелецПрисоединенныхФайлов, Истина);
		Если СтрНачинаетсяС(ИмяКоманды, "ТолькоОдинФайл") И ФайлыВладельца.Количество > 0 Тогда
	...
	ИначеЕсли СтрНачинаетсяС(ИмяКоманды, "ЗаголовокПрисоединенногоФайла") Тогда
		...
		Размещение = ЗначениеПараметраПрисоединенногоФайла(Форма, ПараметрыВыполнения.НомерЭлемента,
			"ПутьКРеквизитуРазмещения");
...
Процедура ПолеПредпросмотраНажатиеЗавершение(Форма, ВладелецПрисоединенныхФайлов, Элемент, СтандартнаяОбработка,
	...
	ПараметрыДобавленияИзображения = РаботаСФайламиСлужебныйВызовСервера.ПараметрыДобавленияИзображения(
		ВладелецПрисоединенныхФайлов);
	Если Просмотр Или Не ПараметрыДобавленияИзображения.ПравоДобавления Тогда
...
Процедура ВыполнитьДействиеСФайлом(ПараметрыВыполнения, ОбработчикЗавершения)
	...
	Размещение = ЗначениеПараметраПрисоединенногоФайла(Форма, Число(ПараметрыВыполнения.НомерЭлемента),
		"ПутьКРеквизитуРазмещения");
	Если ЗначениеЗаполнено(Размещение) Тогда

		Если ПараметрыВыполнения.Действие = "ПросмотретьФайл" Тогда
			ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(Размещение, Неопределено,
...
Процедура ЗавершениеВыполненияКомандыСОповещением(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли (ДополнительныеПараметры.Действие = "ЗагрузитьФайл" Или ДополнительныеПараметры.Действие
		= "ЗавершитьПеретаскивание") И Результат <> Неопределено И Результат.ФайлДобавлен Тогда

		ОбновитьРеквизитХраненияПрисоединенногоФайла(ДополнительныеПараметры.Форма,
			ДополнительныеПараметры.НомерЭлемента, Результат.ФайлСсылка);

	ИначеЕсли ДополнительныеПараметры.Действие = "ВыбратьФайл" И Результат <> Неопределено Тогда

		ОбновитьРеквизитХраненияПрисоединенногоФайла(ДополнительныеПараметры.Форма,
			ДополнительныеПараметры.НомерЭлемента, Результат);
	...
	ИначеЕсли ДополнительныеПараметры.Действие = "ПоместитьФайл" Или ДополнительныеПараметры.Действие
		= "ОтменитьРедактирование" Тогда
...
Процедура ИзменитьВидимостьДополнительныхКоманд(Форма)
	...
	Для ИндексЭлемента = 0 По Форма.ПараметрыРаботыСФайлами.ВГраница() Цикл
		...
		КнопкаКомандыВыбрать          = Форма.Элементы.Найти("УправлениеПрисоединеннымиФайламиВыбратьФайл"
			+ ИндексЭлемента);
		КнопкаКомандыЗагрузить        = Форма.Элементы.Найти("УправлениеПрисоединеннымиФайламиЗагрузитьФайл"
			+ ИндексЭлемента);
		КнопкаКомандыСканировать      = Форма.Элементы.Найти("УправлениеПрисоединеннымиФайламиСканировать"
			+ ИндексЭлемента);
		КнопкаКомандыСоздатьИзШаблона = Форма.Элементы.Найти("УправлениеПрисоединеннымиФайламиСоздатьПоШаблону"
			+ ИндексЭлемента);
