...
Процедура ЗначенияДоступаПриИзменении(Форма, Элемент) Экспорт
	...
	Если Элемент.ТекущиеДанные <> Неопределено И Элемент.ТекущиеДанные.ВидДоступа = Неопределено Тогда
...
Процедура ЗначенияДоступаПриНачалеРедактирования(Форма, Элемент, НоваяСтрока, Копирование) Экспорт
	...
	Если Элемент.ТекущиеДанные.ЗначениеДоступа = Неопределено Тогда
		Если Форма.ТекущиеТипыВыбираемыхЗначений.Количество() > 1 И Форма.ТекущийВидДоступа <> Форма.ВидДоступаВнешниеПользователи
	...
	Элементы.ЗначенияДоступаЗначениеДоступа.КнопкаОчистки
		= Форма.ТекущийТипВыбираемыхЗначений <> Неопределено И Форма.ТекущиеТипыВыбираемыхЗначений.Количество() > 1;
...
Процедура ЗначениеДоступаНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ТекущиеТипыВыбираемыхЗначений.Количество() > 0 Тогда
		...
		Форма.ТекущиеТипыВыбираемыхЗначений.ПоказатьВыборЭлемента(
			Новый ОписаниеОповещения("ЗначениеДоступаНачалоВыбораПродолжение", ЭтотОбъект, Форма), НСтр(
			"ru = 'Выбор типа данных'"), Форма.ТекущиеТипыВыбираемыхЗначений[0]);
	КонецЕсли;
...
Процедура ЗначениеДоступаОбработкаВыбора(Форма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
	...
	Если ВыбранноеЗначение = Тип("СправочникСсылка.Пользователи") Или ВыбранноеЗначение = Тип(
		"СправочникСсылка.ГруппыПользователей") Тогда
	...
	ИначеЕсли ВыбранноеЗначение = Тип("СправочникСсылка.ВнешниеПользователи") Или ВыбранноеЗначение = Тип(
...
Процедура ВидыДоступаВидДоступаПредставлениеОбработкаВыбора(Форма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
	...
	Если Строки.Количество() > 0 И Строки[0].ПолучитьИдентификатор() <> Форма.Элементы.ВидыДоступа.ТекущаяСтрока Тогда

		ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вид доступа ""%1"" уже выбран.
				 |Выберите другой.'"), ВыбранноеЗначение));
...
Процедура ОбработкаВыбораТабличногоДокумента(ФормаОтчета, Элемент, Область, СтандартнаяОбработка) Экспорт
	...
	Если ФормаОтчета.НастройкиОтчета.ПолноеИмя = "Отчет.ПраваДоступа" И ТипЗнч(Область) = Тип(
		"ОбластьЯчеекТабличногоДокумента") И Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
		И ТипЗнч(Область.Расшифровка) = Тип("Строка") И СтрНачинаетсяС(Область.Расшифровка, "ОткрытьФормуСписка: ") Тогда
	...
	Если ТипЗнч(Область) = Тип("ОбластьЯчеекТабличногоДокумента") И Область.ТипОбласти
		= ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И Область.Расшифровка <> Неопределено Тогда
...
Процедура ПриОбработкеРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ФормаОтчета.НастройкиОтчета.ПолноеИмя = "Отчет.АнализПравДоступа" Тогда
		ПриОбработкеРасшифровкиОтчетаАнализПравДоступа(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);

	ИначеЕсли ФормаОтчета.НастройкиОтчета.ПолноеИмя = "Отчет.ПраваРолей" Тогда
		ПриОбработкеРасшифровкиОтчетаПраваРолей(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ПриНачалеВыбораЗначений(ФормаОтчета, УсловияВыбора, ОповещениеОЗакрытии, СтандартнаяОбработка) Экспорт
	...
	Если ФормаОтчета.НастройкиОтчета.ПолноеИмя = "Отчет.АнализПравДоступа" Тогда
		ПриНачалеВыбораЗначенийОтчетаАнализПравДоступа(ФормаОтчета, УсловияВыбора, ОповещениеОЗакрытии,
			СтандартнаяОбработка);

	ИначеЕсли ФормаОтчета.НастройкиОтчета.ПолноеИмя = "Отчет.ПраваРолей" Тогда
		ПриНачалеВыбораЗначенийОтчетаПраваРолей(ФормаОтчета, УсловияВыбора, ОповещениеОЗакрытии, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ЗначениеДоступаНачалоВыбораЗавершение(Форма)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ЗначениеДоступа) И ТекущиеДанные.ЗначениеДоступа
		<> Форма.ТекущийТипВыбираемыхЗначений Тогда
...
Процедура СформироватьДанныеВыбораЗначенияДоступа(Форма, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Форма.ТекущийВидДоступа <> Форма.ВидДоступаВнешниеПользователи И Форма.ТекущийВидДоступа
		<> Форма.ВидДоступаПользователи Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеВыбора = УправлениеДоступомСлужебныйВызовСервера.СформироватьДанныеВыбораПользователя(Текст, Ложь,
		Форма.ТекущийВидДоступа = Форма.ВидДоступаВнешниеПользователи, Форма.ТекущийВидДоступа
		<> Форма.ВидДоступаПользователи);
...
Процедура ПриОбработкеРасшифровкиОтчетаАнализПравДоступа(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ПараметрыРасшифровки.ИмяПоляРасшифровки = "ЗначениеДоступа" Тогда
		...
		Если Не ЗначениеЗаполнено(ГруппаДоступа) Или ТипЗнч(ГруппаДоступа) <> Тип("СправочникСсылка.ГруппыДоступа") Тогда
			Возврат;
		КонецЕсли;
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Ключ", ГруппаДоступа);
		ПараметрыФормы.Вставить("ПерейтиКВидуДоступа", ПараметрыРасшифровки.СписокПолей.Получить("ВидДоступа"));
		ПараметрыФормы.Вставить("ПерейтиКЗначениюДоступа", ПараметрыРасшифровки.СписокПолей.Получить("ЗначениеДоступа"));
	...
	Для Каждого ЭлементОтбора Из ФормаОтчета.Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если Настройка.Использование Тогда
			Если ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Или ЭлементОтбора.ВидСравнения
	...
	Если ТекущийВариант = "ПраваПользователейНаТаблицыОтчета" И ПараметрыРасшифровки.ИмяПоляРасшифровки <> "Пользователь"
		Или ТекущийВариант = "ПраваПользователяНаТаблицыОтчета" Тогда

		Отбор.Удалить("Отчет");
	КонецЕсли;

	Если ПараметрыРасшифровки.ИмяПоляРасшифровки = "Право" Или ПараметрыРасшифровки.ИмяПоляРасшифровки = "ПравоПросмотр"
		Или ПараметрыРасшифровки.ИмяПоляРасшифровки = "ПравоРедактирование" Или ПараметрыРасшифровки.ИмяПоляРасшифровки
		= "ПравоИнтерактивноеДобавление" Тогда
		...
		Если КлючВарианта <> "ПраваПользователяНаТаблицыОтчета" Тогда
			...
			Если ТипЗнч(ЗначениеПрава) = Тип("Число") И ЗначениеПрава = 0 Или ТипЗнч(ЗначениеПрава) = Тип("Булево")
				И Не ЗначениеПрава Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ПараметрыРасшифровки.ИмяПоляРасшифровки = "ОбъектМетаданных" Тогда
		...
		Если ТекущийВариант = "ПраваПользователяНаТаблицы" Или ТекущийВариант = "ПраваПользователяНаТаблицыОтчета" Тогда
			КлючВарианта = "ПраваПользователяНаТаблицу";
		КонецЕсли;

	ИначеЕсли ПараметрыРасшифровки.ИмяПоляРасшифровки = "Пользователь" И ТекущийВариант <> "ПраваПользователяНаТаблицы"
		И ТекущийВариант <> "ПраваПользователяНаТаблицу" И ТекущийВариант <> "ПраваПользователяНаТаблицыОтчетов" Тогда
		...
		Если ТекущийВариант = "АнализПравДоступа" И ГруппировкаПоОтчетамВключена(ФормаОтчета.Отчет.КомпоновщикНастроек) Тогда
	...
	ИначеЕсли ПараметрыРасшифровки.ИмяПоляРасшифровки = "Отчет" Тогда
		...
		Если ТекущийВариант = "ПраваПользователяНаТаблицыОтчета" Или ТекущийВариант
			= "ПраваПользователейНаТаблицыОтчета" Тогда
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ЗначениеРасшифровки) Тогда
			...
			Если Пользователь = Неопределено И Отбор.Свойство("Пользователь") И ТипЗнч(Отбор.Пользователь) = Тип(
				"СправочникСсылка.Пользователи") Тогда

				Пользователь = Отбор.Пользователь;
			КонецЕсли;

			Если ЗначениеЗаполнено(Пользователь) И ТипЗнч(ЗначениеРасшифровки) = Тип(
				"СправочникСсылка.ПрофилиГруппДоступа") Тогда
	...
	Если Отбор.Свойство("ОбъектМетаданных") И Не ЗначениеЗаполнено(Отбор.ОбъектМетаданных) И (КлючВарианта = "ПраваПользователейНаТаблицу"
...
Процедура ПриОбработкеРасшифровкиОтчетаПраваРолей(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ПараметрыРасшифровки.ИмяПоляРасшифровки = "Профиль" Тогда
		...
		Если ТипЗнч(Профиль) = Тип("СправочникСсылка.ПрофилиГруппДоступа") И ЗначениеЗаполнено(Профиль) Тогда
	...
	Если ТекущийВариант = "ПраваРолей" И ПараметрыРасшифровки.ИмяПоляРасшифровки = "УровеньДоступа" Тогда
	...
	ИначеЕсли ТекущийВариант = "ПраваРолей" И ПараметрыРасшифровки.ИмяПоляРасшифровки = "ИмяРоли" Тогда
	...
	ИначеЕсли ТекущийВариант = "ПраваРолей" И ПараметрыРасшифровки.ИмяПоляРасшифровки = "Уровень" Тогда
	...
	ИначеЕсли ТекущийВариант = "ПраваРолейНаОбъектМетаданных" И (ПараметрыРасшифровки.ИмяПоляРасшифровки = "ЕстьОграничение"
	...
	ИначеЕсли ТекущийВариант = "ПраваРолиНаОбъектыМетаданных" Тогда
		...
		Если ПараметрыРасшифровки.ИмяПоляРасшифровки = "Уровень" Тогда
			Отбор.Вставить("ОбъектМетаданных", ПараметрыРасшифровки.СписокПолей["ПолноеИмяОбъекта"]);
		КонецЕсли;
		Если Не Отбор.Свойство("ОбъектМетаданных") Или Не ЗначениеЗаполнено(Отбор.ОбъектМетаданных) Тогда
			Возврат;
		КонецЕсли;
		Если ПараметрыРасшифровки.ИмяПоляРасшифровки = "УровеньДоступа" Тогда
	...
	Если Отбор.Свойство("ОбъектМетаданных") И (КлючВарианта = "ПодробныеПраваРолиНаОбъектМетаданных" Или КлючВарианта
		= "ПраваРолейНаОбъектМетаданных") Тогда
...
Функция ЗначениеПараметра(КомпоновщикНастроек, ИмяПараметра, ИспользуетсяВсегда = Ложь)
	...
	Настройка = КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(
		Параметр.ИдентификаторПользовательскойНастройки);

	Если Настройка <> Неопределено И (ИспользуетсяВсегда Или Настройка.Использование) Тогда

		Возврат Настройка.Значение;
	КонецЕсли;

	Если Параметр <> Неопределено И (ИспользуетсяВсегда Или Параметр.Использование) Тогда
...
Функция ЗначениеНачальногоОтбора(ФормаОтчета, ИмяПараметра)
	...
	Иначе
		...
		Если ТипЗнч(Отбор) <> Тип("Структура") Или Не Отбор.Свойство(ИмяПараметра) Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура УточнитьКлючНазначенияИспользования(Ключ, Отбор, ИмяСвойства)
	...
	ТипСсылки = Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных,
...
Процедура СократитьКлючНазначенияИспользования(КлючНазначенияИспользования)
	...
	КлючНазначенияИспользования = УправлениеДоступомСлужебныйВызовСервера.СокращенныйКлючНазначенияИспользования(
...
Функция НайтиЭлементГруппировкиПоИмени(КоллекцияЭлементов, Имя)
	...
	Для Каждого Элемент Из КоллекцияЭлементов Цикл
		Если (ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных") Или ТипЗнч(Элемент) = Тип(
			"ГруппировкаТаблицыКомпоновкиДанных")) И Элемент.Имя = Имя Тогда
			Результат = Элемент;
...
Процедура ПриНачалеВыбораЗначенийОтчетаАнализПравДоступа(ФормаОтчета, УсловияВыбора, ОповещениеОЗакрытии,
	СтандартнаяОбработка)
...
Процедура ПриНачалеВыбораЗначенийОтчетаПраваРолей(ФормаОтчета, УсловияВыбора, ОповещениеОЗакрытии, СтандартнаяОбработка)
	...
	Если УсловияВыбора.ИмяПоля <> "Роль" И УсловияВыбора.ИмяПоля <> "ОбъектМетаданных" Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПослеВыбораОбъектовМетаданных(ВыбранныеЗначения, Контекст) Экспорт
	...
	Если Контекст.УсловияВыбора.ИмяПоля = "Роль" И ЗначениеЗаполнено(ВыбранныеЗначения) Тогда
