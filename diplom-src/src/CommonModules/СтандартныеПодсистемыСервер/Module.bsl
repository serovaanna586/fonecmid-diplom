...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	Если ИменаПараметровСеанса <> Неопределено И ИменаПараметровСеанса.Найти("ПараметрыКлиентаНаСервере")
		<> Неопределено Тогда
	...
	Если ИменаПараметровСеанса = Неопределено Тогда
		Если ПараметрыСеанса.ПараметрыКлиентаНаСервере.Количество() = 0 Тогда
			...
			НезаполненныеПараметрыКлиента.Вставить("ПервыйСерверныйВызовВыполнен", ?(ТекущийРежимЗапуска()
...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ПолноеИмяРеквизита = "Список.Дата", ИмяЭлемента = "Дата") Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		"СтандартныеПодсистемыСервер.УстановитьУсловноеОформлениеПоляДата", "ЭтотОбъект", Форма, Тип(
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ТекущийРежимЗапуска() = Неопределено И ПараметрыКлиента.Получить("ПервыйСерверныйВызовВыполнен") = Неопределено
	...
	Если ПриЗапуске Тогда
		КомментарийДляЖурналаРегистрацииБезСтекаВызовов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ОбщегоНазначения.КодОсновногоЯзыка()), "СтандартныеПодсистемыКлиент.ПередНачаломРаботыСистемы");
	Иначе
		КомментарийДляЖурналаРегистрацииБезСтекаВызовов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое обращение к неинициализированным параметрам клиента на сервере.
				 |Вероятно, вызван выполнен после некорректной очистки параметров сеанса без использования %2.'",
			ОбщегоНазначения.КодОсновногоЯзыка()), "ОбщегоНазначения.ОчиститьПараметрыСеанса");
	КонецЕсли;
	...
	ИмяСобытия = НСтр("ru = 'Не заполнены параметры клиента на сервере'", ОбщегоНазначения.КодОсновногоЯзыка());

	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , , КомментарийСоСтекомВызовов);

	Если Не ПриЗапуске Тогда
		ТекстОшибки = НСтр("ru = 'Не инициализированы параметры клиента на сервере.
...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияРасширенийПрограммы() Экспорт
	...
	Если ЭтоРазделенныйРежимСеансаБезРазделителей() Тогда
		ТекстОшибки = НСтр("ru = 'Для выполнения требуемых действий следует
...
Функция ЭтоОшибкаТребованияПерезапускаСеанса(ИнформацияОбОшибке) Экспорт
	...
	Возврат ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") И ЗначениеЗаполнено(ТекстОшибки) И СтрНачинаетсяС(
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке), ТекстОшибки);
...
Процедура ОбновитьПараметрРаботыПрограммы(ИмяПараметра, Значение, ЕстьИзменения = Ложь, СтароеЗначение = Неопределено) Экспорт
	...
	РегистрыСведений.ПараметрыРаботыПрограммы.ОбновитьПараметрРаботыПрограммы(ИмяПараметра, Значение, ЕстьИзменения,
		СтароеЗначение);
...
Функция ЭтоПредопределенныеДанные(Знач Элемент, ИмяРеквизита = "", ЗначениеРеквизита = "") Экспорт
	...
	Если (ЗначенияРеквизитов.ИмяПредопределенныхДанных = "" И ЗначенияРеквизитов.ИмяПредопределенногоНабора = ""
...
Процедура УстановитьПараметрРаботыРасширения(ИмяПараметра, Значение, БезУчетаВерсииРасширений = Ложь) Экспорт
	...
	РегистрыСведений.ПараметрыРаботыВерсийРасширений.УстановитьПараметрРаботыРасширения(ИмяПараметра, Значение,
		БезУчетаВерсииРасширений);
...
// 
Процедура ПриОтправкеДанныхПодчиненному(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза,
	Знач Получатель) Экспорт
	...
	ИгнорироватьОтправкуДанныхОбработанныхВЦентральномУзлеРИБПриОбновленииИБ(ЭлементДанных, СозданиеНачальногоОбраза,
		Получатель);
	Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Игнорировать Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПодсистемаОбменДаннымиСуществует Тогда
		...
		МодульОбменДаннымиСобытия.ПриОтправкеДанныхКорреспонденту(ЭлементДанных, ОтправкаЭлемента,
			СозданиеНачальногоОбраза, Получатель, Ложь);
	...
	ИнтеграцияПодсистемБСП.ПриОтправкеДанныхПодчиненному(ЭлементДанных, ОтправкаЭлемента, СозданиеНачальногоОбраза,
		Получатель);
	Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Игнорировать Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ОбменДаннымиВМоделиСервиса") Тогда
		...
		МодульОбменДаннымиВМоделиСервиса.ПриОтправкеДанныхПодчиненному(ЭлементДанных, ОтправкаЭлемента,
...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Знач Отправитель) Экспорт
	...
	ИнтеграцияПодсистемБСП.ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад,
		Отправитель);
	Если ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать Тогда
		Возврат;
	КонецЕсли;
	...
	ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад,
		Отправитель);
...
Процедура ПриПолученииДанныхОтГлавного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель = Неопределено) Экспорт
	...
	Если ПодсистемаОбменДаннымиСуществует Тогда
		МодульОбменДаннымиСобытия = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСобытия");
		МодульОбменДаннымиСобытия.ПриПолученииДанныхОтГлавногоВНачале(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад,
			Отправитель);
	...
	// Вызов переопределяемого обработчика для выполнения прикладной логики обмена РИБ.
	ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного(Отправитель, ЭлементДанных, ПолучениеЭлемента,
		ОтправкаНазад);
	
	// Вставка кода от подсистемы обмена данными должна быть последней, влияющей на логику получения.
	Если ПодсистемаОбменДаннымиСуществует И Не СозданиеНачальногоОбраза(ЭлементДанных) Тогда
...
Процедура СброситьРазмерыИПоложениеОкна(Форма) Экспорт
	...
	Пока Истина Цикл
		...
		Если Не ЕстьСледующей Тогда
			Прервать;
		КонецЕсли;
		Если Не СтрНачинаетсяС(Выборка.КлючОбъекта, ИмяФормы) Или Выборка.КлючНастроек <> "" Или Выборка.КлючОбъекта
			= КлючОбъектаХранения Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрНайти(ПоследняяЧастьКлюча, КлючПоискаРусский) > 0 Или СтрНайти(ПоследняяЧастьКлюча,
			КлючПоискаАнглийский) > 0 Тогда
...
Процедура УстановитьПустуюФормуНаПустойРабочийСтол() Экспорт
	...
	Если ТекущийСоставФорм.ЛеваяКолонка.Количество() = 0 И ТекущийСоставФорм.ПраваяКолонка.Количество() = 0 Тогда
...
Функция ЭтоТаблицаРегистра(ИмяТаблицы) Экспорт
	...
	Если СтрНачинаетсяС(ВРегИмяТаблицы, ВРег("РегистрСведений")) Или СтрНачинаетсяС(ВРегИмяТаблицы, ВРег(
		"РегистрНакопления")) Или СтрНачинаетсяС(ВРегИмяТаблицы, ВРег("РегистрБухгалтерии")) Или СтрНачинаетсяС(
		ВРегИмяТаблицы, ВРег("РегистрРасчета")) Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ПриОпределенииСправочниковДляЗагрузкиДанных(ЗагружаемыеСправочники) Экспорт
	...
	СтрокаТаблицы = ЗагружаемыеСправочники.Найти(Метаданные.Справочники.ИдентификаторыОбъектовМетаданных.ПолноеИмя(),
		"ПолноеИмя");
	...
	СтрокаТаблицы = ЗагружаемыеСправочники.Найти(Метаданные.Справочники.ИдентификаторыОбъектовРасширений.ПолноеИмя(),
		"ПолноеИмя");
	Если СтрокаТаблицы <> Неопределено Тогда
		ЗагружаемыеСправочники.Удалить(СтрокаТаблицы);
	КонецЕсли;
...
Процедура ПриОпределенииОбъектовСРедактируемымиРеквизитами(Объекты) Экспорт
	...
	Объекты.Вставить(Метаданные.Справочники.ИдентификаторыОбъектовМетаданных.ПолноеИмя(),
		"РеквизитыРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.Справочники.ИдентификаторыОбъектовРасширений.ПолноеИмя(),
		"РеквизитыРедактируемыеВГрупповойОбработке");
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	...
	ИсключенияПоискаСсылок.Добавить(
		Метаданные.РегистрыСведений.БезопасноеХранилищеДанныхОбластейДанных.Измерения.Владелец);
...
Функция ТекстСообщенияПриДинамическомОбновлении(ДинамическиеИзмененияКонфигурации, УточняющаяСсылка = "") Экспорт
	...
	Если ДинамическиеИзмененияКонфигурации.КонфигурацияБазыДанныхИзмененаДинамически Тогда
		ТекстСообщенияКонфигурация = НСтр(
	...
	Сообщения.Добавить(НСтр(
		"ru = 'Для того чтобы применить, рекомендуется перезапустить программу и продолжить работу.'"));
	Сообщения.Добавить(" ");
...
Функция КомпонентаПоследнейВерсии(Идентификатор, Местоположение, ВнешняяКомпонента = Неопределено) Экспорт
	...
	Если ВнешняяКомпонента <> Неопределено И ВнешняяКомпонента.Доступна Тогда
		ПоследняяВерсияВнешнейКомпоненты = Новый Структура("Версия, Местоположение", ВнешняяКомпонента.Версия,
			ВнешняяКомпонента.Местоположение);
	Иначе
...
Функция ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ТребуетсяПоказРекомендацииПоОбъемуОперативнойПамяти",
		ТребуетсяПоказРекомендацииПоОбъемуОперативнойПамяти() И Не ОбщегоНазначения.ПодсистемаСуществует(
	...
	Если ЭтоВызовПередНачаломРаботыСистемы И Не Параметры.ПолученныеПараметрыКлиента.Свойство(
		"ПоказатьНерекомендуемуюВерсиюПлатформы") И ПоказатьНерекомендуемуюВерсиюПлатформы(Параметры) Тогда
	...
	Если ЭтоВызовПередНачаломРаботыСистемы И Не Параметры.ПолученныеПараметрыКлиента.Свойство(
		"ВосстановитьСвязьСГлавнымУзлом") И Не ОбщегоНазначения.РазделениеВключено() Тогда

		УстановитьПривилегированныйРежим(Истина);
		ВосстановитьСвязьСГлавнымУзлом = ПланыОбмена.ГлавныйУзел() = Неопределено И ЗначениеЗаполнено(
			Константы.ГлавныйУзел.Получить());
		УстановитьПривилегированныйРежим(Ложь);
	...
	Если ЭтоВызовПередНачаломРаботыСистемы И Не Параметры.ПолученныеПараметрыКлиента.Свойство(
		"ВыбратьНачальныеРегиональныеНастройкиИБ") И ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы() Тогда

		Параметры.Вставить("ВыбратьНачальныеРегиональныеНастройкиИБ", Пользователи.ЭтоПолноправныйПользователь( ,
			Истина, Ложь));
		СтандартныеПодсистемыВызовСервера.СкрытьРабочийСтолПриНачалеРаботыСистемы();
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не Параметры.РазделениеВключено И ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
	...
	Если ЭтоВызовПередНачаломРаботыСистемы И Не Параметры.ПолученныеПараметрыКлиента.Свойство(
		"ПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском") И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ()
	...
	Если ЭтоВызовПередНачаломРаботыСистемы И Не Параметры.ПолученныеПараметрыКлиента.Свойство(
		"НеобходимоОбновлениеПараметровРаботыПрограммы") Тогда
	...
	Если Не Параметры.РазделениеВключено И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
...
Процедура УстановитьПустуюФормуНаРабочийСтол() Экспорт
	...
	Если ТипЗнч(ТекущиеНастройки) = Тип("НастройкиНачальнойСтраницы") Тогда
		ТекущийСоставФорм = ТекущиеНастройки.ПолучитьСоставФорм();
		Если ТекущийСоставФорм.ПраваяКолонка.Количество() = 0 И ТекущийСоставФорм.ЛеваяКолонка.Количество() = 1
...
Функция ПоказыватьПредупреждениеОбУстановленныхОбновленияхПрограммыДляПользователя(ИмяПользователя = Неопределено)
	...
	Результат = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"ОбщиеНастройкиПользователя", "ПоказыватьПредупреждениеОбУстановленныхОбновленияхПрограммы", , ,
		ИмяПользователя);
...
Процедура УстановитьКонстантуНеИспользоватьРазделениеПоОбластямДанных(Параметры) Экспорт
	...
	ЭтоОбменДаннымиВМоделиСервиса = ОбщегоНазначения.ПодсистемаСуществует(
		"СтандартныеПодсистемы.РаботаВМоделиСервиса.ОбменДаннымиВМоделиСервиса");
	Для Каждого НовоеЗначение Из НовыеЗначения Цикл

		Если ЭтоОбменДаннымиВМоделиСервиса И НовоеЗначение.Ключ = "СтандартныеПодсистемыВАвтономномРежиме" Тогда
		...
		Если ПредыдущееЗначение <> НовоеЗначение.Значение Тогда
			...
			Если ЭтоОбменДаннымиВМоделиСервиса И НовоеЗначение.Ключ = "СтандартныеПодсистемыВАвтономномРежиме"
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимо создавать предопределенный элемент, помеченный на удаление.'");
	КонецЕсли;

	СтарыеСвойства = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Источник.Ссылка, "ПометкаУдаления, ИмяПредопределенныхДанных"
	...
	Если (СтарыеСвойства.ИмяПредопределенныхДанных <> "" Или ИмяРеквизита <> "" И СтарыеСвойства[ИмяРеквизита] <> "")
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимо помечать на удаление предопределенный элемент:
				 |""%1"".'"), Строка(Источник.Ссылка));
	ИначеЕсли (ЗначениеРеквизита <> "" И СтарыеСвойства[ИмяРеквизита] = "" Или СтарыеСвойства.ИмяПредопределенныхДанных
		= "") И СтарыеСвойства.ПометкаУдаления = Истина Тогда
...
Процедура ПриОтправкеДанныхПодчиненномуСобытие(Источник, ЭлементДанных, ОтправкаЭлемента, СозданиеНачальногоОбраза) Экспорт
	...
	Если ОтправкаЭлемента <> ОтправкаЭлементаДанных.Игнорировать Тогда
		// Вызов переопределяемого обработчика для выполнения прикладной логики обмена РИБ.
		ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхПодчиненному(Источник, ЭлементДанных, ОтправкаЭлемента,
...
Процедура ПриПолученииДанныхОтПодчиненногоСобытие(Источник, ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад) Экспорт
	...
	Если ПолучениеЭлемента <> ПолучениеЭлементаДанных.Игнорировать Тогда
		// Вызов переопределяемого обработчика для выполнения прикладной логики обмена РИБ.
		ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного(Источник, ЭлементДанных, ПолучениеЭлемента,
			ОтправкаНазад);
	КонецЕсли;
...
Процедура ПриПолученииДанныхОтГлавногоСобытие(Источник, ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад) Экспорт
	...
	Если ПолучениеЭлемента <> ПолучениеЭлементаДанных.Игнорировать Тогда
		// Вызов переопределяемого обработчика для выполнения прикладной логики обмена РИБ.
		ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного(Источник, ЭлементДанных, ПолучениеЭлемента,
			ОтправкаНазад);
	КонецЕсли;
...
Процедура ПослеПолученияДанных(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Или Источник.НомерПринятого = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник.Ссылка,
		"НомерПринятого") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Коллекция Из КоллекцииМетаданных Цикл
		Для Каждого ОбъектМетаданных Из Коллекция Цикл
			...
			Если РазделениеВключено Тогда
				...
				Если (ЭтоРазделенныйСеанс И Не ЭтоРазделенныйОбъектМетаданных) Или (Не ЭтоРазделенныйСеанс
					И ЭтоРазделенныйОбъектМетаданных) Тогда
					Продолжить;
				КонецЕсли;
...
Процедура СоздатьНедостающиеПредопределенныеДанные(ОбъектыМетаданных)
	...
	Для Каждого ОбъектМетаданных Из ОбъектыМетаданных Цикл
		...
		Если Метаданные.ПланыСчетов.Содержит(ОбъектМетаданных) Или Метаданные.ПланыВидовРасчета.Содержит(
			ОбъектМетаданных) Или Не ОбъектМетаданных.Иерархический Тогда
		...
		СохранитьСуществующиеПредопределенныеОбъектыПередСозданиемНедостающих(ОбъектМетаданных, ПолноеИмя, ТаблицаИмен,
			Имена, Запрос, ОписанияСохраненных, ТаблицыБезСохраненных);
	КонецЦикла;
		...
		Если Счет.ВидыСубконто.Количество() > Индекс Тогда
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					...
					УровеньЖурналаРегистрации.Ошибка, Счет.Метаданные(), Счет, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр(
					"ru = 'У счета ""%1"" субконто №%2 ""%3"" не совпадает с предопределенным субконто ""%4"".'"),
					Строка(Счет), Индекс + 1, Строка(Счет.ВидыСубконто[Индекс].ВидСубконто), Строка(Строка.ВидСубконто)),
...
Процедура СохранитьСуществующиеПредопределенныеОбъектыПередСозданиемНедостающих(ОбъектМетаданных, ПолноеИмя,
	ТаблицаИмен, Имена, Запрос, ОписанияСохраненных, ТаблицыБезСохраненных)
...
Процедура ПередЗапускомПрограммы()
	...
	Если ТекущийРежимЗапуска() = Неопределено И ДлительныеОперации.ЭтоЗапускБезВызоваПередЗапускомПрограммы() Тогда
		Возврат;
	КонецЕсли;
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> ТекущийЯзыкВстроенногоЯзыка Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вариант встроенного языка конфигурации ""%1"" не поддерживается.
				 |Используйте вариант языка ""%2"".'"), Метаданные.ВариантВстроенногоЯзыка,
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы)
		< 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ТекущийРежим = Режимы.НеИспользовать Тогда
		ПоддерживаемыеВерсииПлатформы = СтрРазделить("8.3.14, 8.3.15, 8.3.16, 8.3.17, 8.3.18, 8.3.19, 8.3.20, 8.3.21",
			", ", Ложь);
	...
	Иначе
		...
		Если СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_14") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_15")
			Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_16") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_17")
			Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_18") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_19")
			Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_20") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_21") Тогда
	...
	Если ЗначениеЗаполнено(НеПоддерживаемаяВерсияПлатформы) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается.
				 ...
				 |(или ""Версия %2"" при разработке на более старших версиях).'"), НеПоддерживаемаяВерсияПлатформы,
			ПоддерживаемаяВерсияПлатформы);
	КонецЕсли;
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".
					 |Правильный формат, например: ""1.2.3.45"".'"), Метаданные.Версия);
		КонецПопытки;
		Если НулеваяВерсия Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".
					 |Версия не может быть нулевой.'"), Метаданные.Версия);
		КонецЕсли;
	КонецЕсли;

	ТекущийПользовательИБ = ПользователиИнформационнойБазы.ТекущийПользователь();
	Если Не ТекущийПользовательИБ.АутентификацияСтандартная И Не ТекущийПользовательИБ.АутентификацияОС
	...
	Если Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.АдминистраторСистемы)
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В конфигурации в свойстве %1 не указаны стандартные роли %2 и %3.'"), "ОсновныеРоли",
			Метаданные.Роли.АдминистраторСистемы.Имя, Метаданные.Роли.ПолныеПрава.Имя);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПользователиИнформационнойБазы.ТекущийПользователь().Имя)
		И (Не ОбщегоНазначения.РазделениеВключено() Или Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных())
...
Процедура СкорректироватьНачальнуюСтраницуНеразделенногоПользователя()
	...
	Если ТекущийРежимЗапуска() = Неопределено Или Не ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(ПредыдущийСоставФормВМетаданных) <> Тип("Структура") Или Не ПредыдущийСоставФормВМетаданных.Свойство(
		"ЛеваяКолонка") Или ТипЗнч(ПредыдущийСоставФормВМетаданных.ЛеваяКолонка) <> Тип("Массив")
		Или Не ПредыдущийСоставФормВМетаданных.Свойство("ПраваяКолонка") Или ТипЗнч(
		ПредыдущийСоставФормВМетаданных.ПраваяКолонка) <> Тип("Массив") Тогда
...
Процедура КомпенсироватьИзмененияСоставаФормВМетаданныхНачальнойСтраницы(ПредыдущийСоставФормВМетаданных)
	...
	УдалитьНовыеФормыНачальнойСтраницы(СоставФорм.ЛеваяКолонка, ПредыдущийСоставФормВМетаданных.ЛеваяКолонка,
		СоставФормВМетаданных.ЛеваяКолонка);

	УдалитьНовыеФормыНачальнойСтраницы(СоставФорм.ПраваяКолонка, ПредыдущийСоставФормВМетаданных.ПраваяКолонка,
		СоставФормВМетаданных.ПраваяКолонка);
...
Процедура ИгнорироватьОтправкуИдентификаторовОбъектовМетаданных(ЭлементДанных, ОтправкаЭлемента,
	Знач СозданиеНачальногоОбраза = Ложь)

	Если Не СозданиеНачальногоОбраза И ОбъектМетаданных(ЭлементДанных)
		= Метаданные.Справочники.ИдентификаторыОбъектовМетаданных Тогда
...
Функция ОбъектМетаданных(Знач ЭлементДанных)
	...
	Возврат ?(ТипЗнч(ЭлементДанных) = Тип("УдалениеОбъекта"), ЭлементДанных.Ссылка.Метаданные(),
...
Функция СозданиеНачальногоОбраза(Знач ЭлементДанных)
	...
	Возврат ?(ТипЗнч(ЭлементДанных) = Тип("УдалениеОбъекта"), Ложь, ЭлементДанных.ДополнительныеСвойства.Свойство(
		"СозданиеНачальногоОбраза"));
...
Процедура ПрочитатьПараметрыИзСтрокиСоединения(СтруктураПараметровАдминистрирования)
	...
	СтруктураПараметровАдминистрирования.ИмяВКластере = СтроковыеФункцииКлиентСервер.СократитьДвойныеКавычки(Сред(
		ПодстрокиСтрокиСоединения[1], 6));
...
Процедура УстановитьКлючНазначенияИспользованияФормы(Форма, Ключ, УстановитьНастройки)
	...
	Если ХранилищеСистемныхНастроек.Загрузить(Ключ, "КлючНазначенияИспользованияФормы") <> Истина И ПравоДоступа(
		"СохранениеДанныхПользователя", Метаданные) Тогда
...
Процедура УстановитьКлючСохраненияПоложенияОкнаФормы(Форма, Ключ, УстановитьНастройки)
	...
	Если ХранилищеСистемныхНастроек.Загрузить(Ключ, "КлючСохраненияПоложенияОкнаФормы") <> Истина И ПравоДоступа(
		"СохранениеДанныхПользователя", Метаданные) Тогда
...
Процедура ПриОтправкеСерверногоОповещенияИзмененаКонфигурацияИлиРасширения(ИмяОповещения, ВариантыПараметров)
	...
	Для Каждого ВариантПараметров Из ВариантыПараметров Цикл
		...
		Если Не КонфигурацияБазыДанныхИзмененаДинамически И Не ЗначениеЗаполнено(ДинамическиеИзменения.Расширения)
		...
		Для Каждого Адресат Из ВариантПараметров.Адресаты Цикл
			...
			Если ТипЗнч(ДатаНапомнитьЗавтра) = Тип("Дата") И ТекущаяДатаСеанса() < ДатаНапомнитьЗавтра Тогда
				Продолжить;
			КонецЕсли;
			Если ДинамическиеИзменения.Исправления <> Неопределено И ДинамическиеИзменения.Расширения = Неопределено
				...
				И ДинамическиеИзменения.Исправления.Добавлено <> 0 И ДинамическиеИзменения.Исправления.Удалено = 0 Тогда
				...
				Если ТипЗнч(РасписаниеОповещения) = Тип("Структура") И РасписаниеОповещения.Свойство("Расписание")
...
Функция ТребуетсяПоказРекомендацииПоОбъемуОперативнойПамяти()
	...
	Если ОбщегоНазначения.ЭтоВебКлиент() Или Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Возврат ОперативнаяПамять < РекомендуемыйОбъем И (Рекомендация.Показывать Или (ТекущаяДатаСеанса()
		- Рекомендация.ДатаПредыдущегоПоказа) > 60 * 60 * 24 * 60)

КонецФункции

Процедура ИгнорироватьОтправкуДанныхОбработанныхВЦентральномУзлеРИБПриОбновленииИБ(ЭлементДанных,
	СозданиеНачальногоОбраза, Получатель)

	Если Получатель <> Неопределено И Не СозданиеНачальногоОбраза И ТипЗнч(ЭлементДанных) = Тип(
		"РегистрСведенийНаборЗаписей.ДанныеОбработанныеВЦентральномУзлеРИБ") Тогда
