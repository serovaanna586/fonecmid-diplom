...
Функция ЗафиксироватьДлительностьКлючевыхОпераций(ЗамерыДляЗаписи) Экспорт
	...
	Для Каждого Замер Из Замеры Цикл
		...
		Для Каждого ВложенныйЗамер Из ВложенныеЗамеры Цикл
			...
			КлючеваяОперацияВложенногоШагаСсылка = ОценкаПроизводительностиПовтИсп.ПолучитьКлючевуюОперациюПоИмени(
				КлючеваяОперацияВложенногоШага, Истина);
		...
		Если ВложенныеЗамеры.Количество() > 0 Тогда
			...
			КлючеваяОперацияУдельнаяСсылка = ОценкаПроизводительностиПовтИсп.ПолучитьКлючевуюОперациюПоИмени(
				КлючеваяОперацияУдельная, Истина);
			НоваяЗапись = НаборЗаписей.Добавить();
