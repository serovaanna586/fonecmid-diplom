...
//
Функция ЗанятьФайл(ДанныеФайла, СтрокаОшибки = "", Знач УникальныйИдентификатор = Неопределено,
...
//     * Расширение - Строка
//
Функция ДанныеФайла(ФайлСсылка, ВерсияСсылка = Неопределено, ИдентификаторФормы = Неопределено,
	Знач ВызыватьИсключение = Истина) Экспорт
	...
	Если ВерсияСсылка = Неопределено Тогда
		ДанныеФайла.Вставить("СсылкаНаДвоичныеДанныеФайла", ПоместитьВоВременноеХранилище(
			РаботаСФайлами.ДвоичныеДанныеФайла(ФайлСсылка, ВызыватьИсключение), ИдентификаторФормы));
		...
		ДанныеФайла.Вставить("Кодировка", РегистрыСведений.КодировкиФайлов.ОпределитьКодировкуФайла(ФайлСсылка,
			ФайлОбъект.Расширение));
	Иначе
		ДанныеФайла.Вставить("СсылкаНаДвоичныеДанныеФайла", ПоместитьВоВременноеХранилище(
			РаботаСФайлами.ДвоичныеДанныеФайла(ВерсияСсылка, ВызыватьИсключение), ИдентификаторФормы));
...
Функция ДанныеФайлаДляОткрытия(ФайлСсылка, ВерсияСсылка, ИдентификаторФормы = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ВерсияСсылка) И ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия",
		Метаданные.НайтиПоТипу(ТипЗнч(ФайлСсылка))) И ЗначениеЗаполнено(ФайлСсылка.ТекущаяВерсия) Тогда
...
Функция ЗанятьФайлДляРедактирования(ФайлСсылка, УникальныйИдентификатор = Неопределено,
	...
	Если Не ЗначениеЗаполнено(Результат.ДанныеФайла.Редактирует) И Не ЗанятьФайл(Результат.ДанныеФайла,
		Результат.ТекстСообщения, УникальныйИдентификатор) Тогда
		Возврат Результат;
	КонецЕсли;
...
Процедура СохранитьРабочийКаталогПапкиИЗаменитьПутиВРегистре(СсылкаПапки, РабочийКаталогПапки,
	...
	Для Каждого СтруктураЗаписи Из СписокДляЗамены Цикл
		...
		ЗаписатьСтруктуруЗаписиВРегистр(
			СтруктураДляЗаписиВРегистр.Файл, СтруктураДляЗаписиВРегистр.Путь, СтруктураДляЗаписиВРегистр.Размер,
			СтруктураДляЗаписиВРегистр.ДатаПомещенияВРабочийКаталог, СтруктураДляЗаписиВРегистр.НаЧтение,
...
Функция ДанныеФайлаИНавигационнаяСсылкаВсехВерсийФайла(ФайлСсылка, ИдентификаторФормы) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДанныеФайла.ПометкаУдаления = Ложь И ПолучитьИзВременногоХранилища(
			ДанныеФайла.СсылкаНаДвоичныеДанныеФайла) = Неопределено Тогда
...
Функция РезультатУдаленияДанных(ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлИлиВерсия, "Автор") <> АвторизованныйПользователь Тогда
		...
		Результат.ТекстПредупреждения = ?(ЭтоВерсияФайла, НСтр("ru = 'Только автор может удалить версию файла.'"), НСтр(
			"ru = 'Только автор может удалить файл.'"));
...
Процедура ПроверитьПодписи(ИсходныеДанные, ДанныеСтрок) Экспорт
	...
	Для Каждого СтрокаПодписи Из ДанныеСтрок Цикл
		ОписаниеОшибки = "";
		ПодписьВерна = МодульЭлектроннаяПодпись.ПроверитьПодпись(Неопределено, ИсходныеДанные,
			СтрокаПодписи.АдресПодписи, ОписаниеОшибки, СтрокаПодписи.ДатаПодписи);
