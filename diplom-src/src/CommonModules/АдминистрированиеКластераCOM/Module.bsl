...
Функция БлокировкаСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	...
	СоединениеСАгентомСервера = СоединениеСАгентомСервера(COMСоединитель,
		ПараметрыАдминистрированияКластера.АдресАгентаСервера, ПараметрыАдминистрированияКластера.ПортАгентаСервера);

	Кластер = ПолучитьКластер(СоединениеСАгентомСервера, ПараметрыАдминистрированияКластера.ПортКластера,
	...
	ИБ = ПолучитьИБ(СоединенияСРабочимиПроцессами, Кластер, ПараметрыАдминистрированияИБ.ИмяВКластере,
	...
	Результат = ОписаниеОбъектаОбъектнойМоделиCOMАдминистратора(
		ИБ.ИнформационнаяБаза, СловарьСвойствБлокировкиСеансовИРегламентныхЗаданий());
...
Процедура УстановитьБлокировкуСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	...
	ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(ИБ.ИнформационнаяБаза, УстанавливаемаяБлокировка,
...
Процедура ПроверитьПараметрыАдминистрирования(Знач ПараметрыАдминистрированияКластера,
	...
	Если ПроверятьПараметрыАдминистрированияКластера Или ПроверятьПараметрыАдминистрированияИнформационнойБазы Тогда
		...
		Исключение
			...
			Иначе
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключиться с компьютера %1 к кластеру серверов по причине:
						 |%2.'"), ИмяКомпьютера(), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецЕсли;
	...
	Если ПроверятьПараметрыАдминистрированияИнформационнойБазы Тогда
		...
		СоединенияСРабочимиПроцессами = СоединенияСРабочимиПроцессами(COMСоединитель, СоединениеСАгентомСервера,
			Кластер);

		ПолучитьИБ(СоединенияСРабочимиПроцессами, Кластер, ПараметрыАдминистрированияИБ.ИмяВКластере,
			ПараметрыАдминистрированияИБ.ИмяАдминистратораИнформационнойБазы,
			ПараметрыАдминистрированияИБ.ПарольАдминистратораИнформационнойБазы);
...
Процедура УстановитьБлокировкуРегламентныхЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	...
	СоединениеСАгентомСервера = СоединениеСАгентомСервера(
		COMСоединитель, ПараметрыАдминистрированияКластера.АдресАгентаСервера,
		ПараметрыАдминистрированияКластера.ПортАгентаСервера);
...
Функция СеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ,
	...
	Кластер = ПолучитьКластер(
		СоединениеСАгентомСервера, ПараметрыАдминистрированияКластера.ПортКластера,
		ПараметрыАдминистрированияКластера.ИмяАдминистратораКластера,
		ПараметрыАдминистрированияКластера.ПарольАдминистратораКластера);

	ОписаниеИнформационнойБазы = ПолучитьОписаниеИБ(
		СоединениеСАгентомСервера, Кластер, ПараметрыАдминистрированияИБ.ИмяВКластере);
...
Функция СоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ,
	...
	Возврат ПолучитьСоединения(
		COMСоединитель, СоединениеСАгентомСервера, Кластер, ПараметрыАдминистрированияИБ, Фильтр, Истина);
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера,
	...
	Для ТекущаяПопытка = 0 По КоличествоПопыток Цикл
		...
		Соединения = ПолучитьСоединения(
			COMСоединитель, СоединениеСАгентомСервера, Кластер, ПараметрыАдминистрированияИБ, Фильтр, Ложь);
...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	Результат = ОписаниеОбъектаОбъектнойМоделиCOMАдминистратора(
		ПрофильБезопасности, СловарьСвойствПрофиляБезопасности());
	
	// Виртуальный каталоги
	Результат.Вставить("ВиртуальныеКаталоги", ОписанияОбъектовОбъектнойМоделиCOMАдминистратора(
	...
	Результат.Вставить("COMКлассы", ОписанияОбъектовОбъектнойМоделиCOMАдминистратора(
			ПолучитьCOMКлассы(СоединениеСАгентомСервера, Кластер, ИмяПрофиля), СловарьСвойствCOMКласса()));
	
	// Внешние компоненты
	Результат.Вставить("ВнешниеКомпоненты", ОписанияОбъектовОбъектнойМоделиCOMАдминистратора(
			ПолучитьВнешниеКомпоненты(СоединениеСАгентомСервера, Кластер, ИмяПрофиля),
		СловарьСвойствВнешнейКомпоненты()));
	
	// Внешние модули
	Результат.Вставить("ВнешниеМодули", ОписанияОбъектовОбъектнойМоделиCOMАдминистратора(
			ПолучитьВнешниеМодули(СоединениеСАгентомСервера, Кластер, ИмяПрофиля), СловарьСвойствВнешнегоМодуля()));
	
	// Приложения ОС
	Результат.Вставить("ПриложенияОС", ОписанияОбъектовОбъектнойМоделиCOMАдминистратора(
			ПолучитьПриложенияОС(СоединениеСАгентомСервера, Кластер, ИмяПрофиля), СловарьСвойствПриложенияОС()));
	
	// Интернет-ресурсы
	Результат.Вставить("ИнтернетРесурсы", ОписанияОбъектовОбъектнойМоделиCOMАдминистратора(
			ПолучитьИнтернетРесурсы(СоединениеСАгентомСервера, Кластер, ИмяПрофиля), СловарьСвойствИнтернетРесурса()));
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	ПрименитьИзмененияСвойствПрофиляБезопасности(СоединениеСАгентомСервера, Кластер, ПрофильБезопасности,
...
Процедура УстановитьСвойстваПрофиляБезопасности(Знач ПараметрыАдминистрированияКластера,
	...
	ПрофильБезопасности = ПолучитьПрофильБезопасности(
		СоединениеСАгентомСервера, Кластер, СвойстваПрофиляБезопасности.Имя);
...
Процедура УдалитьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	ПолучитьПрофильБезопасности(
		СоединениеСАгентомСервера, Кластер, ИмяПрофиля);
...
Функция ПолучитьКластер(СоединениеСАгентомСервера, Знач ПортКластера, Знач ИмяАдминистратораКластера,
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'На рабочем сервере %1 не существует кластер %2'"), СоединениеСАгентомСервера.ConnectionString,
		ПортКластера);
...
Функция СоединенияСРабочимиПроцессами(COMСоединитель, СоединениеСАгентомСервера, Кластер)
	...
	Если Результат.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'В кластере серверов %1:%2 не существует активных рабочих процессов.'"), Кластер.HostName, Формат(
			Кластер.MainPort, "ЧГ=0"));
	КонецЕсли;
	Возврат Результат;
...
Функция ПолучитьОписаниеИБ(СоединениеСАгентомСервера, Кластер, Знач ИмяВКластере)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'В кластере серверов %1:%2 не существует информационная база ""%3""'"), Кластер.HostName, Формат(
		Кластер.MainPort, "ЧГ=0"), ИмяВКластере);

КонецФункции

Функция ПолучитьИБ(СоединенияСРабочимиПроцессами, Кластер, Знач ИмяВКластере, Знач ИмяАдминистратораИБ,
	...
	Если ИнформационнаяБазаНайдена Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
			"ru = 'Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").'"),
			ИмяВКластере, Кластер.HostName, Кластер.MainPort, ИмяАдминистратораИБ);
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В кластере серверов %1:%2 не существует информационная база ""%3""'"), Кластер.HostName, Формат(
			Кластер.MainPort, "ЧГ=0"), ИмяВКластере);
	КонецЕсли;

КонецФункции

Функция ПолучитьСеансы(СоединениеСАгентомСервера, Кластер, ИнформационнаяБаза, Знач Фильтр = Неопределено,
	Знач Описания = Ложь)
	...
	Для Каждого Блокировка Из СоединениеСАгентомСервера.GetInfoBaseLocks(Кластер, ИнформационнаяБаза) Цикл
		...
		Если Блокировка.Session <> Неопределено Тогда
			...
			АдминистрированиеКластера.ДанныеСеансаИзБлокировки(
				БлокировкиСеансов, Блокировка.LockDescr, Блокировка.Session.SessionID, ИнформационнаяБаза.Name);
	...
	Для Каждого Сеанс Из СоединениеСАгентомСервера.GetInfoBaseSessions(Кластер, ИнформационнаяБаза) Цикл
		...
		ОписаниеСеанса.Вставить("РежимБлокировкиБД", ?(БлокировкиСеансов[ОписаниеСеанса.Номер] <> Неопределено,
			БлокировкиСеансов[ОписаниеСеанса.Номер].РежимБлокировкиБД, ""));
		ОписаниеСеанса.Вставить("Разделитель", ?(БлокировкиСеансов[ОписаниеСеанса.Номер] <> Неопределено,
			БлокировкиСеансов[ОписаниеСеанса.Номер].Разделитель, ""));
...
Функция ПолучитьСоединения(COMСоединитель, СоединениеСАгентомСервера, Кластер, ПараметрыАдминистрированияИБ,
	Знач Фильтр = Неопределено, Знач Описания = Ложь)
			...
			Если НРег(ИнформационнаяБаза.Name) = НРег(ИмяВКластере) Тогда
				...
				Для Каждого Соединение Из СоединениеСРабочимПроцессом.GetInfoBaseConnections(ИнформационнаяБаза) Цикл
					...
					Если АдминистрированиеКластера.ПроверитьУсловияФильтра(ОписаниеСоединения, Фильтр) Тогда
						...
						Иначе
							Соединения.Добавить(Новый Структура("СоединениеСРабочимПроцессом, Соединение",
								СоединениеСРабочимПроцессом, Соединение));
						КонецЕсли;
					КонецЕсли;
...
Функция ПолучитьПрофильБезопасности(СоединениеСАгентомСервера, Кластер, ИмяПрофиля)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'В кластере серверов %1:%2 не существует профиль безопасности ""%3""'"), Кластер.HostName, Формат(
		Кластер.MainPort, "ЧГ=0"), ИмяПрофиля);
...
Функция ПолучитьВиртуальныеКаталоги(СоединениеСАгентомСервера, Кластер, ИмяПрофиля)
	...
	Для Каждого ВиртуальныйКаталог Из СоединениеСАгентомСервера.GetSecurityProfileVirtualDirectories(Кластер,
		ИмяПрофиля) Цикл
...
Процедура ПрименитьИзмененияСвойствПрофиляБезопасности(СоединениеСАгентомСервера, Кластер, ПрофильБезопасности,
	СвойстваПрофиляБезопасности)

	ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
		ПрофильБезопасности, СвойстваПрофиляБезопасности, СловарьСвойствПрофиляБезопасности());
	...
	Для Каждого УдаляемыйВиртуальныйКаталог Из УдаляемыеВиртуальныеКаталоги Цикл
		СоединениеСАгентомСервера.UnregSecurityProfileVirtualDirectory(
			Кластер, ИмяПрофиля, УдаляемыйВиртуальныйКаталог.Alias);
	КонецЦикла;
	...
	Для Каждого СоздаваемыйВиртуальныйКаталог Из СоздаваемыеВиртуальныеКаталоги Цикл
		...
		ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
			ВиртуальныйКаталог, СоздаваемыйВиртуальныйКаталог, СловарьСвойствВиртуальногоКаталога());
	...
	Для Каждого УдаляемыйCOMКласс Из УдаляемыеCOMКлассы Цикл
		СоединениеСАгентомСервера.UnregSecurityProfileCOMClass(
			Кластер, ИмяПрофиля, УдаляемыйCOMКласс.Name);
	КонецЦикла;
	...
	Для Каждого СоздаваемыйCOMКласс Из СоздаваемыеCOMКлассы Цикл
		...
		ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
			COMКласс, СоздаваемыйCOMКласс, СловарьСвойствCOMКласса());
	...
	Для Каждого УдаляемыйВнешнийКомпонент Из УдаляемыеВнешниеКомпоненты Цикл
		СоединениеСАгентомСервера.UnregSecurityProfileAddIn(
			Кластер, ИмяПрофиля, УдаляемыйВнешнийКомпонент.Name);
	КонецЦикла;
	...
	Для Каждого СоздаваемыйВнешнийКомпонент Из СоздаваемыеВнешниеКомпоненты Цикл
		...
		ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
			ВнешнийКомпонент, СоздаваемыйВнешнийКомпонент, СловарьСвойствВнешнейКомпоненты());
	...
	Для Каждого УдаляемыйВнешнийМодуль Из УдаляемыеВнешниеМодули Цикл
		СоединениеСАгентомСервера.UnregSecurityProfileUnSafeExternalModule(
			Кластер, ИмяПрофиля, УдаляемыйВнешнийМодуль.Name);
	КонецЦикла;
	...
	Для Каждого СоздаваемыйВнешнийМодуль Из СоздаваемыеВнешниеМодули Цикл
		...
		ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
			ВнешнийМодуль, СоздаваемыйВнешнийМодуль, СловарьСвойствВнешнегоМодуля());
	...
	Для Каждого УдаляемоеПриложениеОС Из УдаляемыеПриложенияОС Цикл
		СоединениеСАгентомСервера.UnregSecurityProfileApplication(
			Кластер, ИмяПрофиля, УдаляемоеПриложениеОС.Name);
	КонецЦикла;
	...
	Для Каждого СоздаваемоеПриложениеОС Из СоздаваемыеПриложенияОС Цикл
		...
		ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
			ПриложениеОС, СоздаваемоеПриложениеОС, СловарьСвойствПриложенияОС());
	...
	Для Каждого УдаляемыйИнтернетРесурс Из УдаляемыеИнтернетРесурсы Цикл
		СоединениеСАгентомСервера.UnregSecurityProfileInternetResource(
			Кластер, ИмяПрофиля, УдаляемыйИнтернетРесурс.Name);
	КонецЦикла;
	...
	Для Каждого СоздаваемыйИнтернетРесурс Из СоздаваемыеИнтернетРесурсы Цикл
		...
		ЗаполнитьСвойстваОбъектаОбъектнойМоделиCOMАдминистратораПоОписанию(
			ИнтернетРесурс, СоздаваемыйИнтернетРесурс, СловарьСвойствИнтернетРесурса());
...
Функция ОписаниеОбъектаОбъектнойМоделиCOMАдминистратора(Знач Объект, Знач Словарь)
	...
	Для Каждого ФрагментСловаря Из Словарь Цикл
		...
		ИначеЕсли ТипЗнч(ФрагментСловаря.Значение) = Тип("ФиксированнаяСтруктура") Тогда
			...
			Иначе
				Свойство = ОписаниеОбъектаОбъектнойМоделиCOMАдминистратора(ПодчиненныйОбъект,
