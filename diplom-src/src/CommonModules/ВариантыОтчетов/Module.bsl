...
Функция ОтчетыТекущегоПользователя() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		МодульДополнительныеОтчетыИОбработки.ПриДобавленииДополнительныхОтчетовДоступныхТекущемуПользователю(
			ДоступныеОтчеты);
	КонецЕсли;
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 в процедуре %2.
				 ...
				 |%3'"), "КлючВарианта", "ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера", ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 в функции %2:
				 |вариант ""%3"" отсутствует в отчете ""%4"".'"), "КлючВарианта", "ВариантыОтчетов.ОписаниеВарианта",
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(Форма, Настройки) Экспорт
	...
	ПользовательскиеНастройки = ПользовательскиеНастройкиВариантаОтчета(РеквизитыФормы.ВариантСсылка,
		ПользовательСсылка);
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		"ВариантыОтчетов.СброситьПользовательскиеНастройки", "Ключ", Ключ,
	...
	Если Ключ = "*" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие",
			"ТипОтчета = ЗНАЧЕНИЕ(Перечисление.ТипыОтчетов.Внутренний)");
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, НСтр(
					"ru = 'В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:'")
			...
			Если ЕстьИзменения Тогда
				ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(
					ВыборкаХранилища.КлючОбъекта, ВыборкаХранилища.КлючНастроек, ПользовательскиеНастройкиКД, ,
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибокЧтенияПодряд = ОшибокЧтенияПодряд + 1;
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, НСтр(
				"ru = 'В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:'") + Символы.ПС
...
Процедура ЗагрузитьПользовательские(ПользовательскиеВарианты = Неопределено) Экспорт
	...
	Если ТипЗнч(ПользовательскиеВарианты) <> Тип("ТаблицаЗначений") Или ПользовательскиеВарианты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Пока ОписаниеВарианта.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(ХранилищеВарианта, ОписаниеВарианта,
			"Наименование, Отчет, ТипОтчета, КлючВарианта, Автор");
		...
		НайденныеПодсистемы = ПодсистемыОтчетов.НайтиСтроки(Новый Структура("ОтчетПолноеИмя",
			ОписаниеВарианта.ОтчетПолноеИмя));
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
	...
	Если ТипЗнч(Результат.НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") И Результат.КлючВарианта = Неопределено
		И Результат.Объект = Неопределено И ТипЗнч(Результат.СсылкаВарианта) = ТипСсылкиДополнительногоОтчета() Тогда
	...
	Если НадоПрочитатьСсылкуОтчета Или НадоПрочитатьНастройки Тогда
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов") Или Не ЗначениеЗаполнено(
			Результат.СсылкаВарианта) Тогда
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В методе ""%1"" не указаны параметры ""%2"".'"), "ПодключитьОтчетИЗагрузитьНастройки",
		...
		Если ЗначениеЗаполнено(СвойстваВарианта.Родитель) Тогда
			Если ТипЗнч(СвойстваВарианта.РодительКлючВарианта) = Тип("Строка") И ЗначениеЗаполнено(
				СвойстваВарианта.РодительКлючВарианта) Тогда
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вариант ""%1"" (ключ ""%2"") не существует в схеме отчета ""%3"".'"), Строка(
				Результат.СсылкаВарианта), Результат.КлючВарианта, Строка(Результат.СсылкаОтчета));
			Возврат Результат;
		КонецЕсли;

		Если ВариантыНастроекКД.Количество() = 1 Или ВариантыНастроекКД.Индекс(ВариантНастроекКД) = 0 Тогда
	...
	// Инициализация схемы.
	АдресСхемыЗаполнен = (ТипЗнч(Результат.АдресСхемы) = Тип("Строка") И ЭтоАдресВременногоХранилища(
		Результат.АдресСхемы));
	...
	ОтчетыСервер.ЗаполнитьДополнительныеСвойства(ОтчетОбъект, Результат.НастройкиКД, Результат.КлючВарианта,
		КлючПредопределенногоВарианта);
	...
	Если Результат.НастройкиФормы.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		...
		ОтчетОбъект.ПередЗагрузкойНастроекВКомпоновщик(
			Результат, КлючСхемы, Результат.КлючВарианта, Результат.НастройкиКД, Результат.ПользовательскиеНастройкиКД);
	КонецЕсли;
	...
	ОтчетыКлиентСервер.ЗагрузитьНастройки(КомпоновщикНастроекКД, Результат.НастройкиКД,
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если ВнешнийОбъект = Неопределено Или Не ТекущийОбъект.ИспользуетХранилищеВариантов
...
//  СтандартнаяОбработка - Булево - если Истина - вариант отчета сохранен в стандартном хранилище.
//
Процедура ПользовательскиеВариантыОтчетов(ПолноеИмяОтчета, ПользовательИнформационнойБазы, ТаблицаВариантовОтчетов,
	...
	Если (МетаданныеОтчета <> Неопределено И МетаданныеОтчета.ХранилищеВариантов <> Неопределено) Или ТипЗнч(
		ХранилищеВариантовОтчетов) <> Тип("СтандартноеХранилищеНастроекМенеджер") Тогда
...
//
Процедура УдалитьПользовательскийВариантОтчета(СведенияОВариантеОтчета, ПользовательИнформационнойБазы,
	СтандартнаяОбработка) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	УдалитьВариантОтчета(СведенияОВариантеОтчета.КлючОбъекта, СведенияОВариантеОтчета.КлючВарианта,
		ПользовательИнформационнойБазы);
...
Процедура ПриПодключенииОтчета(ПараметрыОткрытия) Экспорт
		...
		Если ТипЗнч(МетаданныеОтчета) <> Тип("ОбъектМетаданных") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось открыть отчет ""%1"".
					 |Возможно, было отключено расширение конфигурации с этим отчетом.'"), ПараметрыОткрытия.Отчет);
		КонецЕсли;
	...
	ИначеЕсли ПараметрыОткрытия.ТипОтчета = "Расширение" Тогда
		Если Метаданные.Отчеты.Найти(ПараметрыОткрытия.ИмяОтчета) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось открыть отчет ""%1"".
					 |Возможно, было отключено расширение конфигурации с этим отчетом.'"), ПараметрыОткрытия.ИмяОтчета);
		КонецЕсли;
...
Функция Обновить(Знач Настройки = Неопределено) Экспорт
	...
	Если Настройки.Отложенное Тогда
		...
		Если Настройки.РазделенныеДанные Тогда
			...
			Если Настройки.Конфигурация Тогда
				ПромежуточныйРезультат = ОбновитьИндексПоиска("РазделенныеДанныеКонфигурации",
					Настройки.ИндексироватьСхему);
			...
			Если Настройки.Расширения Тогда
				ПромежуточныйРезультат = ОбновитьИндексПоиска("РазделенныеДанныеРасширений",
					Настройки.ИндексироватьСхему);
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	ДопСвойства.Вставить("ИспользуемыеТаблицы", ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры,
	...
	ВыполнятьЗамеры = ТипЗнч(ИмяКлючевойОперации) = Тип("Строка") И Не ПустаяСтрока(ИмяКлючевойОперации)
		И ВыполнятьЗамеры();
	Если ВыполнятьЗамеры Тогда
		КомментарийКлючевойОперации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры,
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка, Результат.Метаданные, Результат.СсылкаВарианта,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ВыполнятьЗамеры Тогда
		МодульОценкаПроизводительности.ЗакончитьЗамерВремениТехнологический(
			ИмяКлючевойОперации, ВремяНачала, 1, КомментарийКлючевойОперации);
	КонецЕсли;
	...
	Модифицированы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДопСвойства,
	...
	Если ПолучатьФлажокПустой Тогда
		...
		ИначеЕсли Результат.ТабличныйДокумент.ВысотаТаблицы = 0 И Результат.ТабличныйДокумент.ШиринаТаблицы = 0 Тогда
			Пустой = Истина;
		Иначе
	...
	Если СсылкиВариантов.Количество() > 0 Тогда
		...
		Для Каждого Ссылка Из СсылкиВариантов Цикл
			...
			Иначе
				...
				СтрокаТаблицы.ТипОтчетаСтрокой = ВариантыОтчетовКлиентСервер.ТипОтчетаСтрокой(Неопределено,
					СтрокаТаблицы.Отчет);
				Если СтрокаТаблицы.ТипОтчетаСтрокой = "Внутренний" Тогда
					СсылкиОтчетовКонфигурации.Добавить(СтрокаТаблицы.Отчет);
				ИначеЕсли СтрокаТаблицы.ТипОтчетаСтрокой = "Расширение" Тогда
...
Функция ИнформацияОбОтчете(Знач ОтчетПолноеИмя, Знач ВызыватьИсключение = Ложь) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недостаточно прав доступа к отчету ""%1"".'"), ОтчетПолноеИмя);
		КонецЕсли;
	ИначеЕсли ВРег(Префикс) = "ВНЕШНИЙОТЧЕТ" Тогда
		// Получение метаданных и проверок не требуется.
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неизвестный тип отчета ""%1"" (ожидался ""%2"" или ""%3"").'"), "Отчет", "ВнешнийОтчет",
			ОтчетПолноеИмя);
...
Функция ПредопределенныеВариантыОтчетов(ТипОтчетов = "Внутренний", ПодключенныеКХранилищу = Истина) Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если ПодключенныеКХранилищу И Не ОтчетПодключенКХранилищу(ОтчетМетаданные, КэшФлажкаХранилища) Тогда
			Продолжить;
		КонецЕсли;
		...
		ОписаниеОтчета = ОписаниеОтчетаПоУмолчанию(Результат, ОтчетМетаданные, ОтчетСсылка, ОтчетТип,
			ГруппироватьПоОтчетам);
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если ПустаяСтрока(ОписаниеВарианта.Наименование) Тогда // если конфигурация частично локализована
						ОписаниеВарианта.Наименование = ?(ОписаниеВарианта.КлючВарианта <> "Основной",
							ОписаниеВарианта.КлючВарианта,  // не локализуется
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Не удалось прочитать настройки варианта ""%1"":
									 |%2'"), ОписаниеВарианта.КлючВарианта, ПодробноеПредставлениеОшибки(
								ИнформацияОбОшибке()));
							ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, ТекстОшибки, ОтчетМетаданные);
						КонецПопытки;
					КонецЕсли;
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	...
	ИсключенияПоискаСсылок.Добавить(
		Метаданные.Справочники.ВариантыОтчетов.ТабличныеЧасти.Размещение.Реквизиты.Подсистема);
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Итог) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(
		Итог, "2.3.3.3", "Роль.ИспользованиеВариантовОтчетов", "Роль.ДобавлениеИзменениеЛичныхВариантовОтчетов",
		Библиотека);
...
Процедура ПриОпределенииСправочниковДляЗагрузкиДанных(ЗагружаемыеСправочники) Экспорт
	...
	СтрокаТаблицы = ЗагружаемыеСправочники.Найти(Метаданные.Справочники.ПользовательскиеНастройкиОтчетов.ПолноеИмя(),
		"ПолноеИмя");
...
Процедура ПриОпределенииОбъектовСРедактируемымиРеквизитами(Объекты) Экспорт
	...
	Объекты.Вставить(Метаданные.Справочники.ПользовательскиеНастройкиОтчетов.ПолноеИмя(),
		"РеквизитыРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.Справочники.ПредопределенныеВариантыОтчетов.ПолноеИмя(),
		"РеквизитыРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.Справочники.ПредопределенныеВариантыОтчетовРасширений.ПолноеИмя(),
		"РеквизитыРедактируемыеВГрупповойОбработке");
КонецПроцедуры
	...
	Для Каждого КомандаОтчетов Из КомандыОтчетов Цикл
		...
		Если НайденнаяКоманда <> Неопределено И ЗначениеЗаполнено(НайденнаяКоманда.ТипПараметра) Тогда
			Если ЗначениеЗаполнено(КомандаОтчетов.ТипПараметра) Тогда
				НайденнаяКоманда.ТипПараметра = Новый ОписаниеТипов(НайденнаяКоманда.ТипПараметра,
					КомандаОтчетов.ТипПараметра.Типы());
			Иначе
...
Процедура ПриСбореПоказателейСтатистикиКонфигурации() Экспорт
	...
	МодульЦентрМониторинга.ЗаписатьСтатистикуОбъектаКонфигурации("Справочник.ВариантыОтчетов.Пользовательский",
		Выборка.Количество());
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В методе ""%1"" не указан параметр ""%2"".'"), "ПодключитьОтчетОбъект", "СсылкаОтчета");
		Возврат Результат;
	Иначе
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из программы'"), Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;

	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Или ТипЗнч(
		Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		...
		Если ТипЗнч(Результат.Метаданные) <> Тип("ОбъектМетаданных") Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отчет ""%1"" не существует в программе'"), Результат.Имя);
			Возврат Результат;
		КонецЕсли;
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Не удалось подключить отчет %1:'"), Результат.Метаданные);
			ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
			МодульДополнительныеОтчетыИОбработки = ОбщегоНазначения.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
			МодульДополнительныеОтчетыИОбработки.ПриПодключенииДопОтчета(Результат.Ссылка, Результат, Результат.Успех,
				ПолучатьМетаданные);
		КонецЕсли;
	КонецЕсли;
...
Функция ИзмененияКлючей()
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Найденные = Изменения.НайтиСтроки(Новый Структура("ОтчетМетаданные, СтароеИмяВарианта",
			Изменение.ОтчетМетаданные, Изменение.АктуальноеИмяВарианта));
		Если Найденные.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка регистрации изменений имени варианта отчета ""%1"":
					 ...
					 |также числится как старое имя ""%4"" (актуальное имя ""%5"").'"), Строка(Изменение.Отчет),
				Изменение.АктуальноеИмяВарианта, Изменение.СтароеИмяВарианта, Конфликт.СтароеИмяВарианта,
				Конфликт.АктуальноеИмяВарианта);
		КонецЕсли;
		Найденные = Изменения.НайтиСтроки(Новый Структура("ОтчетМетаданные, СтароеИмяВарианта",
			Изменение.ОтчетМетаданные, Изменение.СтароеИмяВарианта));
		Если Найденные.Количество() > 2 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка регистрации изменений имени варианта отчета ""%1"":
					 ...
					 |варианта отчета ""%4"" (актуальное имя ""%5"").'"), Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта, Изменение.АктуальноеИмяВарианта, Строка(
				Конфликт.ОтчетМетаданные.Представление()), Конфликт.АктуальноеИмяВарианта);
		КонецЕсли;
	КонецЦикла;
...
Функция СброситьНастройкиВариантаОтчета(ВариантОбъект) Экспорт
	Если ВариантОбъект.Пользовательский Или (ВариантОбъект.ТипОтчета <> Перечисления.ТипыОтчетов.Внутренний
		И ВариантОбъект.ТипОтчета <> Перечисления.ТипыОтчетов.Расширение) Или Не ЗначениеЗаполнено(
		ВариантОбъект.ПредопределенныйВариант) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ОбщиеНастройкиПанели() Экспорт
	ОбщиеНастройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы(), "ПанельОтчетов");
...
Функция СохранитьОбщиеНастройкиПанели(ОбщиеНастройки) Экспорт
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы(), "ПанельОтчетов", ОбщиеНастройки);
	Возврат ОбщиеНастройки;
КонецФункции
...
Функция ВыполнятьЗамеры()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		МодульОценкаПроизводительностиВызовСервераПовтИсп = ОбщегоНазначения.ОбщийМодуль(
...
Процедура ЗаписатьВЖурнал(Уровень, Сообщение, ВариантОтчета = Неопределено) Экспорт
	...
	КонецЕсли;
	ЗаписьЖурналаРегистрации(НаименованиеПодсистемы(Неопределено), Уровень, ОбъектМетаданных, Данные, Сообщение);
КонецПроцедуры
...
Процедура ЗаписатьВЖурналЗапускПроцедуры(ИмяПроцедуры)
	...
	ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
		УровеньЖурналаРегистрации.Информация, , , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Запуск процедуры ""%1"".'"), ИмяПроцедуры));
...
Процедура ЗаписатьВЖурналЗавершениеПроцедуры(ИмяПроцедуры, ИзмененоОбъектов = Неопределено)
	...
	Если ИзмененоОбъектов <> Неопределено Тогда
		Текст = Текст + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Изменено %1 объектов.'"), ИзмененоОбъектов);
	КонецЕсли;
	ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
...
Процедура ПриУдаленииПользователя(ПользовательОбъект, Отказ) Экспорт
	Если ПользовательОбъект.ЭтоНовый() Или ПользовательОбъект.ОбменДанными.Загрузка Или Отказ
...
Процедура ОперативноеОбновлениеСлужебногоПользователя() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Пользователи, , ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
...
Функция ОбновитьИндексПоиска(Режим, ИндексироватьСхему)
	...
	Уточнение = НРег(ПредставлениеРежима(Режим)) + ", " + ?(ИндексироватьСхему, НСтр("ru = 'полное'"), НСтр(
		"ru = 'по изменениям'"));

	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Обновление индекса поиска (%1)'"), Уточнение);
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Если ОбщиеДанные Тогда
		...
		ИначеЕсли Режим = "ОбщиеДанныеРасширений" Тогда
			...
			Запрос.УстановитьПараметр("ПустаяСсылка",
				Справочники.ПредопределенныеВариантыОтчетовРасширений.ПустаяСсылка());
		КонецЕсли;
	Иначе
		Запрос.Текст =
		"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ОбщиеДанные Тогда
				...
				Если Найденные.Количество() = 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Вариант ""%1"" не существует для отчета ""%2""'"), ВариантОбъект.КлючВарианта,
						ВариантОбъект.Отчет);
				...
				СведенияОбОтчете.НастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
					ОписаниеВарианта.СистемнаяИнформация, "НастройкиКД");
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр(
					"ru = 'Не удалось перестроить индекс поиска для варианта ""%1"" отчета ""%2"". Возможно, отчет неисправен.'"),
...
Процедура АктуализироватьКлючиПредопределенных(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление ключей вариантов отчетов (%1)'"), ?(Режим = "ОбщиеДанныеКонфигурации", НСтр(
		"ru = 'метаданные конфигурации'"), НСтр("ru = 'метаданные расширений'")));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Если Режим = "ОбщиеДанныеРасширений" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ".ПредопределенныеВариантыОтчетов",
...
Процедура ПометитьУдаленныеПредопределенные(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление настроек предопределенных (%1)'"), ?(Режим = "ОбщиеДанныеКонфигурации", НСтр(
		"ru = 'метаданные конфигурации'"), НСтр("ru = 'метаданные расширений'")));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого ВариантИзБазы Из ПредопределенныеВариантыОтчетов Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			ОписаниеОтчета = Результат.ВариантыОтчетов.НайтиСтроки(Новый Структура("Отчет, ЭтоВариант",
				ВариантИзБазы.Отчет, Ложь))[0];
			ЗаполнитьОписаниеСтрокиВарианта(ОписаниеВарианта, ОписаниеОтчета);
...
Процедура СформироватьТаблицуФункциональныхОпций(Режим, Результат)
	...
	ТаблицаФункциональныхОпций.Колонки.Добавить("ПредопределенныйВариант",
		РеквизитыВариантов.ПредопределенныйВариант.Тип);
	...
	Для Каждого ОписаниеВарианта Из Результат.ВариантыОтчетов Цикл
		...
		ОписаниеОтчета = Результат.ВариантыОтчетов.НайтиСтроки(Новый Структура("Отчет, ЭтоВариант",
			ОписаниеВарианта.Отчет, Ложь))[0];
		ЗаполнитьОписаниеСтрокиВарианта(ОписаниеВарианта, ОписаниеОтчета);
		Если ПустаяСтрока(ОписаниеОтчета.ОсновнойВариант) Или ОписаниеВарианта.КлючВарианта
			= ОписаниеОтчета.ОсновнойВариант Тогда
...
Функция ОбновитьПредопределенныйВариантОтчета(Режим, ОписаниеВарианта, Результат)
	...
	Попытка
		...
		Если Результат.ОбновлятьЗамеры Тогда
			...
			ЗарегистрироватьЗамерыВариантаКОбновлению(Ключ, ОписаниеВарианта.КлючЗамеров,
				ОписаниеВарианта.Наименование, Результат);
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			...
			Если МодульМультиязычностьСервер.КодПервогоДополнительногоЯзыкаИнформационнойБазы()
				= ТекущийЯзык().КодЯзыка Тогда
				ВариантОбъект.НаименованиеЯзык1 = ?(ЗначениеЗаполнено(ОписаниеВарианта.Наименование),
					ОписаниеВарианта.Наименование, ВариантОбъект.Наименование);
				ВариантОбъект.ОписаниеЯзык1     = ?(ЗначениеЗаполнено(ОписаниеВарианта.Описание),
					ОписаниеВарианта.Описание, ВариантОбъект.Описание);
			КонецЕсли;

			Если МодульМультиязычностьСервер.КодВторогоДополнительногоЯзыкаИнформационнойБазы()
				= ТекущийЯзык().КодЯзыка Тогда
				ВариантОбъект.НаименованиеЯзык2 = ?(ЗначениеЗаполнено(ОписаниеВарианта.Наименование),
					ОписаниеВарианта.Наименование, ВариантОбъект.Наименование);
				ВариантОбъект.ОписаниеЯзык2     = ?(ЗначениеЗаполнено(ОписаниеВарианта.Описание),
					ОписаниеВарианта.Описание, ВариантОбъект.Описание);
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого РазмещениеОтчета Из ОписаниеВарианта.Размещение Цикл
			...
			ПолноеИмя = ?(ТипЗнч(РазмещениеОтчета.Ключ) = Тип("Строка"), РазмещениеОтчета.Ключ,
				РазмещениеОтчета.Ключ.ПолноеИмя());
...
Функция ИзменилисьКлючевыеНастройкиПредопределенного(ОписаниеВарианта, ВариантИзБазы)
	Возврат (ВариантИзБазы.Наименование <> ОписаниеВарианта.Наименование Или ВариантИзБазы.Родитель <> ОписаниеВарианта.ВариантРодитель
...
Функция ИзменилисьВторостепенныеНастройкиПредопределенного(ОписаниеВарианта, ВариантИзБазы)
	// Шапка
	Если ВариантИзБазы.Включен <> ОписаниеВарианта.Включен Или ВариантИзБазы.Описание <> ОписаниеВарианта.Описание
		Или ВариантИзБазы.КлючЗамеров <> ОписаниеВарианта.КлючЗамеров Или ВариантИзБазы.ГруппироватьПоОтчету
		<> ОписаниеВарианта.ГруппироватьПоОтчету Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ОписаниеВарианта.Размещение Цикл
		...
		Если СтрокаРазмещения = Неопределено Или СтрокаРазмещения.Важный <> (НРег(КлючИЗначение.Значение) = НРег(
			"Важный")) Или СтрокаРазмещения.СмТакже <> (НРег(КлючИЗначение.Значение) = НРег("СмТакже")) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ИзменилисьНастройкиПоиска(ВариантИзБазы, СтарыеСведения)
	Возврат ВариантИзБазы.ХешНастроек <> СтарыеСведения.ХешНастроек Или ВариантИзБазы.НаименованияПолей
		<> СтарыеСведения.НаименованияПолей Или ВариантИзБазы.НаименованияПараметровИОтборов <> СтарыеСведения.НаименованияПараметровИОтборов
...
Процедура АктуализироватьВариантыОтчетовПоПредопределенным(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		"ru = 'Обновление вариантов отчетов (%1)'"), НРег(ПредставлениеРежима(Режим)));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	ИначеЕсли Режим = "РазделенныеДанныеРасширений" Тогда
		...
		Запрос.УстановитьПараметр("ПустаяСсылкаВарианта",
			Справочники.ПредопределенныеВариантыОтчетовРасширений.ПустаяСсылка());
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ".ПредопределенныеВариантыОтчетов",
			".ПредопределенныеВариантыОтчетовРасширений");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		Если ЗначениеЗаполнено(СуффиксЯзыка) Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПредопределенныеКонфигурации.Наименование КАК Наименование,", "ПредопределенныеКонфигурации.Наименование КАК Наименование,
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВариантыОтчетов.Наименование КАК Наименование,", "ВариантыОтчетов.Наименование КАК Наименование,
																									   |ВариантыОтчетов.НаименованиеЯзык1 КАК НаименованиеЯзык1,
																									   |ВариантыОтчетов.НаименованиеЯзык2 КАК НаименованиеЯзык2,");

			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "втВариантыОтчетов.Наименование КАК РеквизитНаименование,", "втВариантыОтчетов.Наименование КАК РеквизитНаименование,
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "втПредопределенные.Наименование КАК Наименование,", "втПредопределенные.Наименование КАК Наименование,
	...
	Для Каждого СтрокаТаблицы Из Найденные Цикл
		...
		Если Не СтрокаТаблицы.УстановитьПометкуУдаления И СтрокаТаблицы.ГруппироватьПоОтчету
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удаление вариантов удаленных отчетов (%1)'"), НРег(ПредставлениеРежима(Режим)));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаменитьКлючиПользовательскихНастроек(СтарыйВариант, АктуальныйВариант)
	Если СтарыйВариант.КлючВарианта = АктуальныйВариант.КлючВарианта Или Не ЗначениеЗаполнено(СтарыйВариант.КлючВарианта)
		Или Не ЗначениеЗаполнено(АктуальныйВариант.КлючВарианта) Или ТипЗнч(АктуальныйВариант.Отчет) <> Тип(
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, НСтр(
				"ru = 'В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:'") + Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), СтарыйВариант.Ссылка);
		КонецПопытки;
		...
		ОписаниеНастроек = ХранилищеПользовательскихНастроекОтчетов.ПолучитьОписание(
			ВыборкаХранилища.КлючОбъекта, ВыборкаХранилища.КлючНастроек, ВыборкаХранилища.Пользователь);
		
		// Запись настроек в хранилище по новому ключу.
		ХранилищеПользовательскихНастроекОтчетов.Сохранить(
			НовыйКлючОбъекта, ВыборкаХранилища.КлючНастроек, ВыборкаХранилища.Настройки, ОписаниеНастроек,
			ВыборкаХранилища.Пользователь);
	КонецЦикла;
...
//   Результат - см. РезультатОбновленияОбщихДанных
//
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование,
	Результат)
...
Функция ДеревоПодсистемСформировать(Форма, ВариантОснование) Экспорт
	...
	Если ТипЗнч(ВариантОснование.ПредопределенныйВариант) = Тип(
		"СправочникСсылка.ПредопределенныеВариантыОтчетовРасширений") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПредопределенныеВариантыОтчетов",
			"ПредопределенныеВариантыОтчетовРасширений");
	КонецЕсли;
...
Процедура ОпределитьПоведениеСпискаПользователейВариантаОтчета(Форма) Экспорт
	...
	НастройкаДоступностиРазрешена = Не Элементы.Доступен.ТолькоПросмотр И ПравоДоступа("Чтение",
...
Функция ПользовательскиеНастройкиВариантаОтчета(ВариантОтчета, Пользователь, КлючНастройки = Неопределено)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка, , ПредставлениеВариантОтчета, ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
...
Функция ЗаполнитьПоляДляПоиска(ВариантОбъект, СведенияОбОтчете = Неопределено) Экспорт
	...
	Если НастройкиДляПоиска <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(НастройкиДляПоиска.НаименованияПараметровИОтборов) Тогда
			ПоляДляПоиска.НаименованияПараметровИОтборов = "#" + СокрЛП(
				НастройкиДляПоиска.НаименованияПараметровИОтборов);
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для варианта ""%1"" отчета ""%2"" не заполнены настройки поиска:
				 |Наименования полей, параметров и отборов.'"), ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Подробнее см. процедуру %1.'"), "ВариантыОтчетовПереопределяемый.НастроитьВариантыОтчетов");
		КонецЕсли;
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") И ТипЗнч(ВариантОбъект) = Тип(
		"СправочникОбъект.ВариантыОтчетов") Тогда
		Попытка
	...
	// Последняя проверка.
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов") Или ТипЗнч(ВариантОбъект)
			= Тип("СправочникОбъект.ПредопределенныеВариантыОтчетовРасширений") Тогда
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось прочитать настройки предопределенного варианта отчета ""%1"".'"),
				ВариантОбъект.КлючЗамеров), ВариантОбъект.Ссылка);
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
Функция ПоляДляПоиска(ВариантОтчета)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) Тогда
			...
			Результат.НаименованияПараметровИОтборов = ВариантОтчета["НаименованияПараметровИОтборов"
...
Функция СформироватьПредставленияПараметровИОтборов(КомпоновщикНастроек)
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройки.Элементы Цикл
		...
		Если ПользовательскаяНастройка.РежимОтображения = Режимы.Авто И ОбщаяНастройка.РежимОтображения
			<> Режимы.БыстрыйДоступ Тогда
			Продолжить;
		КонецЕсли;
...
Функция НастройкиОтчета(ОтчетСсылка, КлючВарианта, ОтчетОбъект)
	...
	Если ОтчетыСНастройками.Найти(ОтчетСсылка) = Неопределено И (ОтчетОбъект = Неопределено
		Или Метаданные.Отчеты.Содержит(ОтчетОбъект.Метаданные())) Тогда
...
Функция ЭтоВнешнийОтчет(ОтчетОбъект)
	...
	Если Метаданные.Отчеты.Найти(ИмяОтчета) <> Неопределено И ТипЗнч(ОтчетОбъект) = Тип("ОтчетОбъект." + ИмяОтчета) Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
КонецФункции
...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	ТолькоНепомеченныеДляУдаления = ?(ПараметрыПоиска.Свойство("ПометкаУдаления"), ПараметрыПоиска.ПометкаУдаления,
		Истина);
	...
	Если ПараметрыПоиска.Свойство("ТолькоЛичные") Тогда
		ПоказыватьЛичныеВариантыОтчетовДругихАвторов = ПоказыватьЛичныеВариантыОтчетовДругихАвторов
			И Не ПараметрыПоиска.ТолькоЛичные;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ПоказыватьЛичныеВариантыОтчетовДругихАвторов",
	...
	Иначе
		...
		Если ПараметрыПоиска.Свойство("КлючВариантаБезУсловий") И ЗначениеЗаполнено(
			ПараметрыПоиска.КлючВариантаБезУсловий) Тогда
...
Функция ПодготовитьУсловиеПоискаПоСтроке(Знач Запрос, ТекстЗапроса, Знач ЕстьСтрокаПоиска, Знач ПараметрыПоиска,
	Знач ПравоЧтенияАвторов)
	...
	Если ЕстьСтрокаПоиска Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВариантыИПодсистемыПоСтрокеПоиска",
			ВариантыИПодсистемыПоСтрокеПоиска);

		ПользовательскиеНастройкиПоСтрокеПоиска = "(" + СтрЗаменить(ШаблонПоиска, "<ИмяТаблицы.ИмяПоля>",
			"Пользовательские.Наименование") + ")";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПользовательскиеНастройкиПоСтрокеПоиска",
			ПользовательскиеНастройкиПоСтрокеПоиска);
	...
	Иначе
		...
		НачалоВыборкиИзТаблицы = ("ОБЪЕДИНИТЬ ВСЕ
								  |
								  |ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура СформироватьРезультатыПоиска(Знач МассивСлов, Знач ИсходнаяТаблица, Результат)
	...
	ШаблонОбластиПоиска = Новый ФиксированнаяСтруктура("Значение, КоличествоНайденныхСлов, ПодсветкаСлов", "", 0,
	...
	Для Индекс = 1 По Количество Цикл
		...
		Если Не ЗначениеЗаполнено(Вариант.НаименованиеВарианта.Значение) И ЗначениеЗаполнено(
			СтрокаТаблицы.НаименованиеВарианта) Тогда
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Вариант.НаименованияПолей.Значение) И ЗначениеЗаполнено(
			СтрокаТаблицы.НаименованияПолей) Тогда
			Вариант.НаименованияПолей.Значение = СтрокаТаблицы.НаименованияПолей;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Вариант.НаименованияПараметровИОтборов.Значение) И ЗначениеЗаполнено(
			СтрокаТаблицы.НаименованияПараметровИОтборов) Тогда
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Вариант.ПредставлениеАвтора.Значение) И ЗначениеЗаполнено(
			СтрокаТаблицы.АвторПредставление) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ПредставлениеПользовательскойНастройки) Тогда
			...
			Иначе
				Вариант.НаименованияПользовательскихНастроек.Значение = Вариант.НаименованияПользовательскихНастроек.Значение
					+ РазделительПредставления + СтрокаТаблицы.ПредставлениеПользовательскойНастройки;
			КонецЕсли;
		КонецЕсли;

		Если ЗначениеЗаполнено(СтрокаТаблицы.НаименованиеПодсистемы) И Вариант.Подсистемы.Найти(
			СтрокаТаблицы.Подсистема) = Неопределено Тогда
			...
			Вариант.НаименованияПодсистем.Значение = ?(ПустаяСтрока(НаименованияПодсистем),
				СтрокаТаблицы.НаименованиеПодсистемы, НаименованияПодсистем + РазделительПредставления
		...
		Если Индекс = Количество Или СтрокаТаблицы.Ссылка <> Вариант.Ссылка Тогда
			...
			Для Каждого Слово Из МассивСлов Цикл
				СловоНайдено = ОтметитьСлово(Вариант.НаименованиеВарианта, Слово) Или ОтметитьСлово(Вариант.Описание,
					Слово) Или ОтметитьСлово(Вариант.НаименованияПолей, Слово, Истина) Или ОтметитьСлово(
					Вариант.ПредставлениеАвтора, Слово, Истина) Или ОтметитьСлово(
					Вариант.НаименованияПараметровИОтборов, Слово, Истина) Или ОтметитьСлово(Вариант.КлючевыеСлова,
					Слово, Истина) Или ОтметитьСлово(Вариант.НаименованияПользовательскихНастроек, Слово, Истина);
		...
		Если ИспользоватьРазделитель Тогда
			...
			Пока Позиция > 10 Цикл
				...
				Если ПозицияРазделителя < Позиция Тогда
					...
					СтруктураГде.Значение = (Сред(СтруктураГде.Значение, ПозицияРазделителя + ДлинаРазделителя) + РазделительХранения
						+ Лев(СтруктураГде.Значение, ПозицияРазделителя - 1));
					ОстатокВРег = (Сред(ОстатокВРег, ПозицияРазделителя + ДлинаРазделителя) + РазделительХранения + Лев(
						ОстатокВРег, ПозицияРазделителя - 1));
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения, АдресРезультата) Экспорт
	...
	ТолькоТекущийРаздел = ПараметрыЗаполнения.РежимНастройки Или Не ПоискПоСтроке
		Или ПараметрыЗаполнения.ИскатьВоВсехРазделах = 0;
	...
	Запрос.УстановитьПараметр("ОтключенныеВариантыПрограммы", ПараметрыПоиска.ОтключенныеВариантыПрограммы);
	Запрос.УстановитьПараметр("НетОтбораПоПодсистемамИОтчетам", Не ПоискПоСтроке
		И ПараметрыПоиска.Подсистемы.Количество() = 0);
	...
	Если ПоискПоСтроке Тогда
		...
		Для Каждого РодительСсылка Из РезультатПоиска.РодителиСвязанныеСВариантами Цикл
			...
			Для Каждого СтрокаТаблицы Из МестаВывода Цикл
				Найденные = ТаблицаРезультат.НайтиСтроки(Новый Структура("Подсистема, Родитель",
					СтрокаТаблицы.Подсистема, РодительСсылка));
				Если Найденные.Количество() = 0 Тогда
					ТаблицаРезультат.Удалить(СтрокаТаблицы);
				КонецЕсли;
			КонецЦикла;
	...
	МассивСлов = ?(ПоискПоСтроке, ВариантыОтчетовКлиентСервер.РазложитьСтрокуПоискаВМассивСлов(ВРег(СокрЛП(
		ПараметрыЗаполнения.СтрокаПоиска))), Неопределено);
	...
	Если СуффиксТекущегоЯзыка <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
				...
				МодульМультиязычностьСервер.ИзменитьПолеЗапросаПодТекущийЯзык(ТекстЗапросаПредварительный,
...
Процедура ЗаполнитьИменаОтчетов(ТаблицаРезультат)
	...
	Для Каждого ОтчетДляВывода Из ТаблицаРезультат Цикл
		Если ОтчетДляВывода.ТипОтчета = Перечисления.ТипыОтчетов.Внутренний Или ОтчетДляВывода.ТипОтчета
			= Перечисления.ТипыОтчетов.Расширение Тогда
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр(
		"ru = 'Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, обратитесь к администратору.'");
КонецФункции
	...
	Для Каждого Команда Из ДобавленныеКоманды Цикл
		...
		Если Не ЗначениеЗаполнено(Команда.ТипПараметра) Тогда
			Если ТипЗнч(СведенияОбОбъекте.ТипСсылкиДанных) = Тип("Тип") Тогда
				Команда.ТипПараметра = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
					СведенияОбОбъекте.ТипСсылкиДанных));
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не НайденВПрограмме Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" не существует в программе>'"), ИмяОтчета);
			ИначеЕсли Не ДоступенПоПравам Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Недостаточно прав для работы с отчетом ""%1"">'"), ИмяОтчета);
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" отключен в настройках программы>'"), ИмяОтчета);
			Иначе
...
Функция ЭтоПредопределенныйВариантОтчета(ВариантОтчета) Экспорт
	...
	Если ТипВариантаОтчета = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов") Или ТипВариантаОтчета = Тип(
		"СправочникОбъект.ПредопределенныеВариантыОтчетовРасширений") Или ТипВариантаОтчета = Тип(
		"СправочникСсылка.ПредопределенныеВариантыОтчетов") Или ТипВариантаОтчета = Тип(
		"СправочникСсылка.ПредопределенныеВариантыОтчетовРасширений") Тогда
	...
	Возврат СвойстваВариантаОтчета.Пользовательский <> Истина И ЗначениеЗаполнено(
		СвойстваВариантаОтчета.ПредопределенныйВариант);
...
Процедура ДополнитьОтборыИзСтруктуры(Отбор, Структура, РежимОтображения = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из Структура Цикл
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			Отбор, ИмяПоля, Значение, Условие, , Истина, РежимОтображения);
	КонецЦикла;
КонецПроцедуры
...
Функция ОписаниеВариантаОтчета(ОписаниеФайла)
	...
	ИмяКаталога = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(
		ФайловаяСистема.СоздатьВременныйКаталог());
...
Функция ВариантОтчетаПоОписанию(ОписаниеВариантаОтчета, ВариантОтчетаОснование)
	...
	Попытка
		...
		ВариантОтчета = ПользовательскийВариантОтчета(ИнформацияОбОтчете.Отчет, ОписаниеВариантаОтчета.КлючВарианта,
			Автор);
...
Процедура СохранитьПользовательскиеНастройкиИзФайла(ВариантОтчета, ОписаниеВариантаОтчета)
	...
	Для Каждого ЭлементСписка Из ОписаниеВариантаОтчета.ПользовательскиеНастройки Цикл
		...
		ХранилищеПользовательскихНастроекОтчетов.Сохранить(
			ОписаниеНастроек.КлючОбъекта, ОписаниеНастроек.КлючНастроек, Настройки, ОписаниеНастроек,
...
Процедура ПоделитьсяПользовательскимиНастройками(ВыбранныеПользователи, ШаблонОписанияНастроек) Экспорт
	...
	Для Каждого СвойстваПользователя Из СвойстваПользователей.Действительных Цикл
		...
		ХранилищеПользовательскихНастроекОтчетов.Сохранить(
			ОписаниеНастроек.КлючОбъекта, ОписаниеНастроек.КлючНастроек, ШаблонОписанияНастроек.Настройки,
...
#КонецОбласти
	...
	Пока Выборка.Следующий() Цикл
		...
		СвойстваПользователя = СвойстваПользователя(Выборка.Ссылка, ПользовательИБ.Имя,
			Выборка.ИдентификаторПользователяИБ);
...
Процедура НормализоватьШаблонОписанияНастроек(ШаблонОписанияНастроек, ТекущийПользователь)
	...
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(
		ШаблонОписанияНастроек.КлючОбъекта + "/ТекущиеПользовательскиеНастройки", "", ШаблонОписанияНастроек.Настройки);
...
#Область ГенерацияОповещенияНовыхПользователейНастроек
	...
	Сообщение.Действия.Добавить(
		ВариантыОтчетовКлиентСервер.ИмяДействияПрименитьПереданныеНастройки(), НСтр("ru = 'Применить настройки?'"));

	Попытка
		МодульОбсуждения.ОтправитьСообщение(
			Пользователи.ТекущийПользователь(), Получатели, Сообщение, ОписаниеНастроек.ВариантОтчета);
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка, , ПредставлениеВариантаОтчета, ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
...
Функция КоллекцияПредопределенныхВариантовОтчетов()
	...
	ОписаниеВариантаОтчета = Новый ОписаниеТипов("СправочникСсылка.ПредопределенныеВариантыОтчетов, 
												 |СправочникСсылка.ПредопределенныеВариантыОтчетовРасширений");
...
//
Функция ОписаниеОтчетаПоУмолчанию(ОписанияОтчетов, МетаданныеОтчета, СсылкаОтчета, ТипОтчета = Неопределено,
	ГруппироватьПоОтчетам = Неопределено)
...
Функция РезультатОбновленияОбщихДанных(Знач Режим, Знач РазделенныеОбработчики)
	...
	Результат.Вставить("ЕстьИзменения", Ложь);
	Результат.Вставить("ЕстьВажныеИзменения", Ложь);
	Результат.Вставить("ВариантыОтчетов", ПредопределенныеВариантыОтчетов(?(Результат.ОбновлятьКонфигурацию,
		"Внутренний", "Расширение")));
	Результат.Вставить("ОбновлятьЗамеры", ОбщегоНазначения.ПодсистемаСуществует(
