...
Функция ОписаниеПоля(ПутьКДанным, СпискиПолей) Экспорт
	...
	Для Каждого НастройкиСписка Из СпискиПолей Цикл
		...
		Реквизит = НайтиПоле(ПутьКДанным, ПодчиненныеЭлементы(КоллекцияПолей), Ложь, ИсточникиДоступныхПолей,
			НастройкиСписка);
		Если Реквизит <> Неопределено Тогда
			...
			Если Родитель <> Неопределено Тогда
				Для Каждого ИсточникДоступныхПолей Из ИсточникиДоступныхПолей(ИсточникиДоступныхПолей, НастройкиСписка,
					Родитель) Цикл
	...
	Для Индекс = 1 По СтрДлина(Формула) Цикл
		...
		Если ЭтоРазделитель И ОткрывающихСкобок = 0 Тогда
			Если ЗначениеЗаполнено(Операнд) Тогда
				ЭтоФункция = Ложь;
				Если Символ = "(" И СтрНайти(Операнд, ".") = 0 Или ЛогическиеОператоры.Найти(ВРег(Операнд))
					<> Неопределено Тогда
					ЭтоФункция = Истина;
				КонецЕсли;
		...
		Если РаспознанныеЭлементы[ОписаниеЭлемента.Ключ] = Неопределено Тогда
			Если ЭтоФункция Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Неизвестная функция в выражении - ""%1""'"), Операнд);
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Поле %1 не найдено в списке доступных полей'"), Операнд);
			КонецЕсли;
...
Процедура ДобавитьСписокПолейНаФорму(Форма, Параметры) Экспорт
	...
	Если ПодключенныеСпискиПолей = Неопределено Тогда
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизитаПодключенныеСпискиПолей,
			Новый ОписаниеТипов("ТаблицаЗначений")));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяСпискаПолей", Новый ОписаниеТипов("Строка"),
			ИмяРеквизитаПодключенныеСпискиПолей));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяСпискаИсточников", Новый ОписаниеТипов("Строка"),
			ИмяРеквизитаПодключенныеСпискиПолей));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("СкобкиИдентификаторов", Новый ОписаниеТипов("Булево"),
			ИмяРеквизитаПодключенныеСпискиПолей));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("СкобкиПредставлений", Новый ОписаниеТипов("Булево"),
			ИмяРеквизитаПодключенныеСпискиПолей));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИспользоватьИдентификаторыДляФормул",
			Новый ОписаниеТипов("Булево"), ИмяРеквизитаПодключенныеСпискиПолей));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ПриОпределенииИсточниковДоступныхПолей",
			Новый ОписаниеТипов("Строка"), ИмяРеквизитаПодключенныеСпискиПолей));
	КонецЕсли;
	...
	СтрокаПоиска.УстановитьДействие("ИзменениеТекстаРедактирования",
		"Подключаемый_СтрокаПоискаИзменениеТекстаРедактирования");
	...
	СписокПолей = Форма.Элементы.Добавить(ИмяСпискаПолей, Тип("ТаблицаФормы"),
		ПараметрыДобавления.МестоРазмещенияСписка);
	...
	ПредставлениеПоля = Форма.Элементы.Добавить(ИмяКолонкиПредставление(ИмяСпискаПолей), Тип("ПолеФормы"),
		ГруппаКолонок);
	...
	ПредставлениеПоля = Форма.Элементы.Добавить(ИмяСпискаПолей + "ПредставлениеПутиКДанным", Тип("ПолеФормы"),
		ГруппаКолонок);
...
Процедура ЗаполнитьСписокДоступныхРеквизитов(Знач ТекущийРеквизит, ИсточникиДоступныхПолей, Знач Отбор = "",
	...
	Для Каждого КоллекцияДоступныхПолей Из КоллекцииДоступныхПолей Цикл
		Для Каждого ОписаниеПоля Из КоллекцияДоступныхПолей.Элементы Цикл
			Если ОписаниеПоля.Таблица И (Строка(ОписаниеПоля.Поле) = "ДополнительныеРеквизиты" Или СтрЗаканчиваетсяНа(
				ОписаниеПоля.Поле, ".ДополнительныеРеквизиты")) Тогда
				Продолжить;
			КонецЕсли;
			Если ОписаниеПоля.Таблица И (Строка(ОписаниеПоля.Поле) = "КонтактнаяИнформация" Или СтрЗаканчиваетсяНа(
				ОписаниеПоля.Поле, ".КонтактнаяИнформация")) Тогда
				Продолжить;
			КонецЕсли;
			Если ОписаниеПоля.Таблица И (Строка(ОписаниеПоля.Поле) = "Представления" Или СтрЗаканчиваетсяНа(
				ОписаниеПоля.Поле, ".Представления")) Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ОписаниеПоля.Родитель <> Неопределено И СтрНачинаетсяС(Реквизит.Заголовок,
				ОписаниеПоля.Родитель.Заголовок) Тогда
			...
			Если ТипЗнч(ОписаниеПоля) = Тип("ДоступноеПолеОтбораКомпоновкиДанных") И ЗначениеЗаполнено(
				ОписаниеПоля.Маска) И СтрНачинаетсяС(ОписаниеПоля.Маска, "{") И СтрЗаканчиваетсяНа(ОписаниеПоля.Маска,
				"}") Тогда
			...
			Если Не ЗначениеЗаполнено(Реквизит.Картинка) Тогда
				...
				ИначеЕсли СтрНачинаетсяС(Реквизит.Имя, "Печать") Или СтрНачинаетсяС(Реквизит.Имя, "Подпись")
					Или СтрНачинаетсяС(Реквизит.Имя, "Факсимиле") Или СтрНачинаетсяС(Реквизит.Имя, "Картинка")
					Или Реквизит.Тип.Типы().Количество() = 1 И МодульРаботаСФайламиСлужебный <> Неопределено
	...
	Для Каждого ДоступныйРеквизит Из ДоступныеРеквизиты Цикл
		...
		Если ДоступныйРеквизит.ЕстьПодчиненные Или Реквизит.Тип.Типы().Количество() = 1 И Не ОбщегоНазначения.ЭтоСсылка(
			Реквизит.Тип.Типы()[0]) Тогда
...
Функция ИсточникиДоступныхПолей(ИсточникиДоступныхПолей, НастройкиСписка, Реквизит = Неопределено)
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Если Не ИсточникНайден И НастройкиСписка <> Неопределено И ЗначениеЗаполнено(
			НастройкиСписка.ПриОпределенииИсточниковДоступныхПолей) Тогда
...
Функция КоллекцияДопРеквизитов(ИмяОбъектаМетаданных)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(СписокСвойств,
		"Заголовок,ИдентификаторДляФормул,ТипЗначения");
...
Функция УстановитьОтбор(Знач Форма, Знач ИмяСписка, Знач Отбор, Знач КоллекцияРеквизитов = Неопределено,
	Знач Уровень = 0)
	...
	Для Каждого Реквизит Из КоллекцияРеквизитов.ПолучитьЭлементы() Цикл
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		Если Не Реквизит.СоответствуетОтбору И (Уровень < 2 Или Реквизит.Поле <> Неопределено И СтрНачинаетсяС(Отбор,
			Реквизит.ПредставлениеПутиКДанным)) Тогда
...
Функция НайтиТекстВСтроке(Знач Строка, Знач Текст)
	...
	Для Каждого Подстрока Из СтрРазделить(Текст, " ", Ложь) Цикл
		...
		ФорматированныеСтроки.Добавить(Новый ФорматированнаяСтрока(ПодстрокаВхождения, ШрифтыСтиля.ВажнаяНадписьШрифт,
...
Процедура ЗаменитьЭлементыФормулы(Выражение, ЭлементыФормулы, Значения)
	...
	Для Каждого ОписаниеЭлемента Из ЭлементыФормулы.ОперандыИФункции Цикл
		...
		Если Значение = Неопределено Тогда
			Если ЭтоФункция И Не СтрЗаканчиваетсяНа(Операнд, СуффиксОтключеннойФункции()) Тогда
...
Функция НайтиПоле(ПутьКДанным, КоллекцияРеквизитов, ИскатьПоПредставлению, ИсточникиДоступныхПолей, НастройкиСписка)
	...
	Если Владелец <> Неопределено Тогда
		...
		Возврат НайтиПоле(ПутьКДанным, ПодчиненныеЭлементы(Владелец), ИскатьПоПредставлению, ИсточникиДоступныхПолей,
			НастройкиСписка);
	КонецЕсли;

	Для Каждого Папка Из Папки Цикл
		РазвернутьПоле(Папка, ИсточникиДоступныхПолей, НастройкиСписка);
		Реквизит = НайтиПоле(ПутьКДанным, ПодчиненныеЭлементы(Папка), ИскатьПоПредставлению, ИсточникиДоступныхПолей,
			НастройкиСписка);
...
Функция ФорматироватьЧисла(Строка, РазделительДробнойЧасти = ".")
	...
	Для Индекс = 1 По ДлинаСтроки Цикл
		...
		ПредыдущийСимволЭтоРазделитель = ПредыдущийСимвол = "" Или СтрНайти("()[]/*-+%=<>, " + Символы.Таб
			+ Символы.ПС, ПредыдущийСимвол) > 0;

		Если ЭтоЦифра(Символ) И (ПредыдущийСимволЭтоРазделитель Или ЭтоЦифра(ПредыдущийСимвол) И ЗначениеЗаполнено(
			Число)) Тогда
			Число = Число + Символ;
		ИначеЕсли Не ЕстьРазделительВЧисле И (Символ = "," Или Символ = ".") И ЭтоЦифра(СледующийСимвол) И (ЭтоЦифра(
			ПредыдущийСимвол) Или ПредыдущийСимволЭтоРазделитель) И ЗначениеЗаполнено(Число) Тогда
