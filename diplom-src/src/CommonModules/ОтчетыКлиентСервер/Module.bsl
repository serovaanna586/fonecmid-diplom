...
Функция НастройкиОтчетаПоУмолчанию() Экспорт
	...
	Настройки.Вставить("ВариантПредставленияПериода", ПредопределенноеЗначение(
		"Перечисление.ВариантыПредставленияПериода.Стандартный"));
...
//
Функция ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки, Идентификатор, Иерархия = Неопределено,
	...
	Если ТипНастройки <> Тип("НастройкиКомпоновкиДанных") Тогда
		...
		ИначеЕсли ТипНастройки = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипНастройки = Тип(
			"КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Или ТипНастройки = Тип(
			"КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных") Тогда

			Для Каждого ВложенныйЭлемент Из Настройки Цикл
				РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(ВложенныйЭлемент, Идентификатор,
					Иерархия);
...
Функция ЭлементыНастроекОтобранные(Настройки, Отбор, ЭлементыНастроек = Неопределено, Результат = Неопределено) Экспорт
	...
	Для Каждого Элемент Из ЭлементыНастроек Цикл
		...
		Если ТипЗнч(ЭлементАнализируемый) = Тип("ЭлементОтбораКомпоновкиДанных") И ЭлементАнализируемый.Использование
			= Отбор.Использование И ЭлементАнализируемый.ПравоеЗначение = Отбор.Значение Тогда
		...
		ИначеЕсли ТипЗнч(ЭлементАнализируемый) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
			И ЭлементАнализируемый.Использование = Отбор.Использование И ЭлементАнализируемый.Значение = Отбор.Значение Тогда

			Результат.Добавить(ЭлементАнализируемый);

		ИначеЕсли ТипЗнч(ЭлементАнализируемый) = Тип("ОтборКомпоновкиДанных") Или ТипЗнч(ЭлементАнализируемый) = Тип(
			"ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
...
Функция НайтиДоступноеПолеКД(НастройкиКД, ПолеКД)
	...
	Если ТипЗнч(НастройкиКД) = Тип("ГруппировкаКомпоновкиДанных") Или ТипЗнч(НастройкиКД) = Тип(
		"ГруппировкаТаблицыКомпоновкиДанных") Или ТипЗнч(НастройкиКД) = Тип("ГруппировкаДиаграммыКомпоновкиДанных") Тогда
		...
		Для Каждого ЭлементСтруктурыКД Из СтруктураКД Цикл
			...
			Если ТипЗнч(ЭлементСтруктурыКД) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
				...
				Если ЭлементСтруктурыКД.Настройки.ПараметрыДанных.ДоступныеПараметры <> Неопределено Тогда
					// Настройки, которым принадлежат параметры данных, связаны с источником доступных настроек.
					ДоступнаяНастройка = ЭлементСтруктурыКД.Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(
						ПараметрКД);
					Если ДоступнаяНастройка <> Неопределено Тогда
						Возврат ДоступнаяНастройка;
					КонецЕсли;
...
Функция ЗначениеТипаГруппыИЭлементы(ИсходноеЗначение, Условие = Неопределено) Экспорт
	Если Условие <> Неопределено Тогда
		Если Условие = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Или Условие
			...
			КонецЕсли;
		ИначеЕсли Условие = ВидСравненияКомпоновкиДанных.ВИерархии Или Условие
			= ВидСравненияКомпоновкиДанных.НеВИерархии Тогда
			Возврат ГруппыИЭлементы.Группы;
		КонецЕсли;
	КонецЕсли;
...
//
Функция ЗагрузитьНастройки(КомпоновщикНастроек, Настройки, ПользовательскиеНастройки = Неопределено,
	ФиксированныеНастройки = Неопределено) Экспорт
	НастройкиЗагружены = (ТипЗнч(Настройки) = Тип("НастройкиКомпоновкиДанных") И Настройки
	...
	Если ТипЗнч(ФиксированныеНастройки) = Тип("НастройкиКомпоновкиДанных") И ФиксированныеНастройки
		<> КомпоновщикНастроек.ФиксированныеНастройки Тогда
...
Функция НайтиЭлементНастройки(ЭлементНастройки, ИдентификаторПользовательскойНастройки)
	...
	Пока Индекс < Группы.Количество() Цикл
		...
		Для Каждого ПодчиненныйЭлемент Из КоллекцияЭлементов Цикл
			...
			ИначеЕсли ПодчиненныйЭлемент.ИдентификаторПользовательскойНастройки
				= ИдентификаторПользовательскойНастройки Тогда
...
Функция ЗначениеПараметраВыбора(Настройки, ПользовательскиеНастройки, Поле, РежимИзмененияВарианта)
	...
	Если Значение = Неопределено Тогда
		...
		НайтиЗначенияПоляЭлементовОтбора(Поле, ЭлементыОтбора, ПользовательскиеНастройки, Значение,
			РежимИзмененияВарианта);
	КонецЕсли;
...
Функция ЗначениеПараметраДанныхПоПолю(Настройки, ПользовательскиеНастройки, Поле, РежимИзмененияВарианта)
	...
	Для Каждого Элемент Из ЭлементыНастроек Цикл
		...
		ЭлементАнализируемый = ?(РежимИзмененияВарианта Или ЭлементПользовательский = Неопределено, Элемент,
			ЭлементПользовательский);
		...
		Если ЭлементАнализируемый.Использование И (Поля[0] = Поле Или Поля[1] = Поле) И ЗначениеЗаполнено(
			ЭлементАнализируемый.Значение) И ТипЗнч(ЭлементАнализируемый.Значение) <> Тип("СписокЗначений") Тогда
...
Процедура НайтиЗначенияПоляЭлементовОтбора(Поле, ЭлементыОтбора, ПользовательскиеНастройки, Значение,
	РежимИзмененияВарианта)
	...
	Для Каждого Элемент Из ЭлементыОтбора Цикл
		...
		Если ТипЗнч(ЭлементАнализируемый) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			НайтиЗначенияПоляЭлементовОтбора(Поле, Элемент.Элементы, ПользовательскиеНастройки, Значение,
				РежимИзмененияВарианта);
		Иначе
			Если Элемент.ЛевоеЗначение = Поле И ЭлементАнализируемый.Использование И ЭлементАнализируемый.ВидСравнения
				= ВидСравненияКомпоновкиДанных.Равно И ЗначениеЗаполнено(ЭлементАнализируемый.ПравоеЗначение) Тогда
...
Функция ПредставлениеЭлементаУсловногоОформления(ЭлементКД, НастройкаВариантаКД, Состояние) Экспорт
	...
	Если ПредставлениеОтбора = "" И ДоступныСведенияИзВарианта Тогда
		ПредставлениеОтбора = ПредставлениеОтбора(НастройкаВариантаКД.Отбор, НастройкаВариантаКД.Отбор.Элементы,
			Состояние);
	КонецЕсли;
...
Функция ПредставлениеОформления(ОформлениеКД)
	...
	Для Каждого ЭлементКД Из ОформлениеКД.Элементы Цикл
		Если ЭлементКД.Использование Тогда
			...
			Представление = Представление + ?(Представление = "", "", ", ") + ПредставлениеКлюча + ?(
...
Функция ПредставлениеЦвета(Цвет)
	...
	ИначеЕсли Цвет.Вид = ВидЦвета.WebЦвет Или Цвет.Вид = ВидЦвета.WindowsЦвет Тогда
...
Функция ИмяВПредставление(Знач ИсходнаяСтрока)
	...
	Для НомерСимвола = 1 По СтрДлина(ИсходнаяСтрока) Цикл
		...
		Если ЭтоПервыйСимвол Тогда
			...
			КонецЕсли;
		Иначе
			Если (КодСимвола >= 65 И КодСимвола <= 90) Или (КодСимвола >= 1040 И КодСимвола <= 1071) Тогда
				Символ = " " + НРег(Символ);
...
Функция СкопироватьРекурсивно(Знач Узел, Знач ЧтоКопировать, Знач КудаВставить, Знач Индекс,
	Соответствие = Неопределено, БезСтруктуры = Ложь) Экспорт
...
Функция ПараметрыКопирования(ТипЭлемента, Коллекция, БезСтруктуры = Ложь)
	...
	Если ТипЭлемента = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Или ТипЭлемента = Тип(
		"ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("ВыбранноеПолеКомпоновкиДанных") Или ТипЭлемента = Тип(
		"АвтоВыбранноеПолеКомпоновкиДанных") Или ТипЭлемента = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("ПолеГруппировкиКомпоновкиДанных") Или ТипЭлемента = Тип(
		"АвтоПолеГруппировкиКомпоновкиДанных") Или ТипЭлемента = Тип("ЭлементПорядкаКомпоновкиДанных") Или ТипЭлемента
		= Тип("АвтоЭлементПорядкаКомпоновкиДанных") Тогда
...
//   ЭлементУсловногоОформленияКомпоновкиДанных - элемент настройки, который заполняется.
//
Функция ЗаполнитьСвойстваРекурсивно(Узел, ЧтоЗаполнить, ЧемЗаполнить, Соответствие = Неопределено,
	ПараметрыКопирования = Неопределено, БезСтруктуры = Ложь) Экспорт
	Если Соответствие = Неопределено Тогда
		Соответствие = Новый Соответствие;
	КонецЕсли;
	...
	Если ЭтоДанныеФормыЭлементДерева Тогда
		...
		Если КоллекцияВложенных.Количество() > 0 Тогда
			НоваяКоллекцияВложенных = ?(ЭтоДанныеФормыЭлементДерева, ЧтоЗаполнить.ПолучитьЭлементы(),
				ЧтоЗаполнить.Строки);
	...
	Если ПараметрыКопирования.ЕстьОтбор Тогда
		...
		Если КоллекцияВложенных.Количество() > 0 Тогда
			...
			Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
				СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено,
					Новый Соответствие);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыКопирования.ЕстьУсловноеОформление Тогда
		...
		ЗаполнитьЗначенияСвойств(ЧтоЗаполнить.УсловноеОформление, ЧемЗаполнить.УсловноеОформление, ,
			"ДоступныеПоляОтбора, ДоступныеПоляПолей, Элементы");
	...
	Если ПараметрыКопирования.ЕстьЗначенияВложенныхПараметров Тогда
		...
		Для Каждого ПодчиненнаяСтрока Из ЧемЗаполнить.ЗначенияВложенныхПараметров Цикл
			СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, ЧтоЗаполнить.ЗначенияВложенныхПараметров, Неопределено,
				Соответствие);
		КонецЦикла;
	КонецЕсли;
...
Функция ДобавитьУникальноеЗначениеВСписок(Список, Значение, Представление, Использование) Экспорт
	Если ТипЗнч(Список) <> Тип("СписокЗначений") Или (Не ЗначениеЗаполнено(Значение) И Не ЗначениеЗаполнено(
		Представление)) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Для Каждого ЭлементИсточник Из СписокИсточник Цикл
		...
		Иначе
			Если ЗаменятьСуществующие Тогда
				...
				ЗаполнитьЗначенияСвойств(ЭлементПриемник, ЭлементИсточник, , ?(ЗаменятьПредставление, "",
					"Представление"));
			Иначе
				Результат.Пропущено = Результат.Пропущено + 1;
			КонецЕсли;
