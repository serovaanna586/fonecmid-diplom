...
#Если ВебКлиент Тогда
	...
	ОписанияФайлов.Добавить(Новый ОписаниеПередаваемогоФайла(ИмяВременногоФайла, ПоместитьВоВременноеХранилище(
		ДвоичныеДанныеШаблона)));
	...
	КонецЕсли;
	ИмяВременногоФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ВременныйКаталог)
		+ ИмяВременногоФайла;
#Иначе
...
#КонецЕсли
	...
	РежимЗапуска = СвойствоЗначение(ServiceManager, "MacroExecutionMode", 0); // const short NEVER_EXECUTE = 0
	ПараметрыДокумента.SetValue(0, РежимЗапуска);

	Document = Desktop.LoadComponentFromURL("file:///" + СтрЗаменить(ИмяВременногоФайла, "\", "/"), "_blank", 0,
		ПараметрыДокумента);
...
//
Процедура ДобавитьВерхнийКолонтитул(Знач ПечатнаяФорма, Знач Область) Экспорт
...
//
Процедура ДобавитьНижнийКолонтитул(Знач ПечатнаяФорма, Знач Область) Экспорт
...
//
Процедура ПрисоединитьОбласть(Знач ПечатнаяФормаHandler, Знач ОбластьHandler, Знач ПереходНаСледСтроку = Истина,
...
#Если ВебКлиент Тогда
				ИмяВременногоФайла = УправлениеПечатьюСлужебныйКлиент.СоздатьВременныйКаталог("OOWriter") + Строка(
					Новый УникальныйИдентификатор) + ".tmp";
#Иначе
...
//
Процедура ПрисоединитьИЗаполнитьКоллекцию(Знач ПечатнаяФормаHandler, Знач ОбластьHandler, Знач Данные,
	Знач ЭтоСтрокаТаблицы = Ложь, Знач ПереходНаСледСтроку = Истина) Экспорт
...
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось сформировать печатную форму: %1. 
			 |%2'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке), ТекстУточнения);
