...
Процедура ПодключитьСхему(Отчет, Контекст, Схема, КлючСхемы) Экспорт
	...
	Иначе
		АдресСхемыЗаполнен = (ТипЗнч(Контекст.АдресСхемы) = Тип("Строка") И ЭтоАдресВременногоХранилища(
			Контекст.АдресСхемы));
...
//
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикНастроек, Схема, Отчет = Неопределено,
	ВариантОтчета = Неопределено) Экспорт
	Попытка
...
Процедура ВывестиКоманду(ФормаОтчета, КомандаИлиКоманды, ТипГруппы, ВНачалоГруппы = Ложь, ТолькоВоВсехДействиях = Ложь,
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			"ОтчетыСервер.ВывестиКоманду", "ТипГруппы");
	КонецЕсли;
	...
	Если СуффиксПодгруппы <> "" Тогда
		...
		Если Подгруппа = Неопределено Тогда
			Подгруппа = ФормаОтчета.Элементы.Вставить(Группа.Имя + СуффиксПодгруппы, Тип("ГруппаФормы"), Группа,
				ПередЧемВставить);
...
Функция ОтчетПустой(ОтчетОбъект, ПроцессорКД = Неопределено) Экспорт
	Если ПроцессорКД = Неопределено Тогда
		...
		МакетКД = КомпоновщикМакетаКД.Выполнить(ОтчетОбъект.СхемаКомпоновкиДанных,
...
#Область ЗаполнениеСтруктуры
	...
	Для Каждого ЭлементПользовательскойНастройки Из ПользовательскиеНастройки.Элементы Цикл
		...
		Если ЭлементНастройки = Неопределено Или НедоступныеЭлементыСтруктуры.Получить(ТипЗнч(ЭлементНастройки)) = ТипФормы
		...
		Если ТипФормы = ТипФормыОтчета.Настройка И ТипЗнч(ЭлементНастройки) = Тип(
			"ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
...
Функция ДоступныеНастройки(ПараметрыЗагрузки, НастройкиОтчета) Экспорт
	...
	Возврат Новый Структура("Настройки, ПользовательскиеНастройки, ФиксированныеНастройки", Настройки,
		ПользовательскиеНастройки, ФиксированныеНастройки);
КонецФункции
...
Процедура УстановитьДоступныеЗначения(Отчет, Форма) Экспорт
	...
	Для Каждого КоллекцияНастроек Из КоллекцииНастроек Цикл
		...
		Для Каждого ЭлементНастройки Из КоллекцияНастроек.Элементы Цикл
			Если ТипЗнч(ЭлементНастройки) <> Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И ТипЗнч(
				ЭлементНастройки) <> Тип("ЭлементОтбораКомпоновкиДанных") Тогда
				Продолжить;
			КонецЕсли;

			Если Не ЭтоПользовательскиеНастройки И ЗначениеЗаполнено(
			...
			Если ЭтоПользовательскиеНастройки Тогда
				...
				ЭлементОсновнойНастройки = ОтчетыКлиентСервер.ПолучитьОбъектПоПользовательскомуИдентификатору(
					...
					ЭлементПользовательскойНастройки.ИдентификаторПользовательскойНастройки, , КоллекцияНастроек);
			Иначе
				ЭлементПользовательскойНастройки = ЭлементНастройки;
...
Функция ЭтоТехническийОбъект(Знач ПолноеИмяОбъекта) Экспорт
	...
	Возврат ПолноеИмяОбъекта = ВРег("Справочник.ПредопределенныеВариантыОтчетовРасширений") Или ПолноеИмяОбъекта = ВРег(
		"Справочник.ПредопределенныеВариантыОтчетов");
...
Функция ТребуетсяСброситьПредопределенныеПараметрыВывода(ПараметрыЗагрузки) Экспорт
	...
	Если ПараметрыЗагрузки.Свойство("ИмяСобытия") И ПараметрыЗагрузки.ИмяСобытия
		= ОтчетыКлиентСервер.ИмяСобытияНастройкиПоУмолчанию() Тогда
...
Функция СтандартныеСвойстваПредопределенныхПараметровВывода(Настройки)
	...
	Возврат ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Настройки.ДополнительныеСвойства, КлючСтандартныхСвойствПредопределенныхПараметровВывода());
...
Процедура УстановитьФиксированныеОтборы(СтруктураОтборов, НастройкиКД, НастройкиОтчета) Экспорт
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Или СтруктураОтборов = Неопределено
...
Процедура ЗаполнитьДополнительныеСвойства(ОтчетОбъект, НовыеНастройки, Знач КлючВарианта,
	Знач КлючПредопределенногоВарианта, Знач КонтекстВарианта = "", Знач ФормаПараметрыОтбор = Неопределено) Экспорт
...
Функция КоличествоДоступныхНастроек(КомпоновщикНастроек) Экспорт
	...
	Для Каждого ЭлементПользовательскойНастройки Из ПользовательскиеНастройки.Элементы Цикл
		...
		РежимОтображения = ?(ЭлементНастройки = Неопределено, ЭлементПользовательскойНастройки.РежимОтображения,
			ЭлементНастройки.РежимОтображения);

		Если РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Авто Или РежимОтображения
			= РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ Тогда
...
Функция СвойстваЭлементаПользовательскихНастроек(КомпоновщикНастроек, ЭлементПользовательскойНастройки,
	ЭлементНастройки, ОписаниеНастройки)
...
Процедура ПолучитьСведенияОГруппировке(Группировка, Сведения, ДополнительныеСвойства)
	...
	Если ТипГруппировки <> Тип("НастройкиКомпоновкиДанных") И ТипГруппировки <> Тип("ГруппировкаКомпоновкиДанных")
		И ТипГруппировки <> Тип("ГруппировкаТаблицыКомпоновкиДанных") И ТипГруппировки <> Тип(
		"ГруппировкаДиаграммыКомпоновкиДанных") Тогда
		Возврат;
	КонецЕсли;

	Если ТипГруппировки <> Тип("НастройкиКомпоновкиДанных") И ЗначениеЗаполнено(
		Группировка.ИдентификаторПользовательскойНастройки) Тогда
...
Процедура ПолучитьСведенияОКоллекции(ЭлементНастроек, Коллекция, Сведения, ДополнительныеСвойства)
	...
	Если ТипКоллекции <> Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") И ТипКоллекции <> Тип(
		"КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") И ТипКоллекции <> Тип(
...
Процедура ПолучитьСведенияОСвойствахЭлементаНастроек(ЭлементНастроек, Сведения, ДополнительныеСвойства)
	...
	Если ТипЭлементаНастроек <> Тип("ТаблицаКомпоновкиДанных") И ТипЭлементаНастроек <> Тип(
	...
	Для Каждого Идентификатор Из ИдентификаторыСвойств Цикл
		...
		Если ДоступныеСвойства.Свойство(Идентификатор) И ЗначениеЗаполнено(
			Свойство.ИдентификаторПользовательскойНастройки) Тогда
...
Процедура ПолучитьСведенияОЭлементахСвойстваНастроек(Настройки, Свойство, ИдентификаторСвойства, Сведения,
	...
	Для Каждого Элемент Из Свойство.Элементы Цикл
		...
		Если ЗначениеЗаполнено(Элемент.ИдентификаторПользовательскойНастройки) Тогда
			...
			ИначеЕсли ТипЭлемента = Тип("ЗначениеПараметраКомпоновкиДанных") Или ТипЭлемента = Тип(
				"ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
...
Процедура ПолучитьСведенияОЗначенияхВложенныхПараметров(Настройки, ЗначенияПараметров, ИдентификаторСвойства, Сведения,
	...
	Для Каждого ЗначениеПараметра Из ЗначенияПараметров Цикл
		Если ЗначениеЗаполнено(ЗначениеПараметра.ИдентификаторПользовательскойНастройки) Тогда
			...
			ВидыСведений.ОписаниеНастройки = Настройки[ИдентификаторСвойства].ДоступныеПараметры.НайтиПараметр(
...
Функция ИдентификаторыСвойствНастроек(ДополнительныеСвойства)
	...
	ИдентификаторыСвойств = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДополнительныеСвойства, "ИдентификаторыСвойствНастроек", ИдентификаторыСвойствПоУмолчанию);
...
Функция ИменаРеквизитовЭлементовНастроек(Форма, ВидыЭлементов)
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		Для Каждого ВидЭлемента Из ВидыЭлементов Цикл
			Если СтрНачинаетсяС(Реквизит.Имя, ВидЭлемента) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(
				СтрЗаменить(Реквизит.Имя, ВидЭлемента, "")) Тогда
...
Процедура ПодготовитьФормуКПерегруппировкеЭлементов(Форма, УзелИерархииЭлементов, ИменаРеквизитов,
	ВидыСтилизованныхЭлементов)
	Элементы = Форма.Элементы;
...
Процедура ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов,
	ВидыСтилизованныхЭлементов)
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл
		...
		Если Не ЗначениеЗаполнено(ИдентификаторГруппы) Или Не СвойстваЭлементов.Группы.Свойство(ИдентификаторГруппы,
			СвойстваГруппы) Тогда
	...
	ВывестиСтилизованныеЭлементыФормыНастроек(Форма, ЭлементыНастроек, ОписаниеНастроек, ИменаРеквизитов,
		ВидыСтилизованныхЭлементов);
...
Функция ПоведениеГруппУзлаИерархииЭлементовФормыНастроекПереопределено(УзелИерархииЭлементов)
	...
	Для Каждого Группа Из УзелИерархииЭлементов.ПодчиненныеЭлементы Цикл
		...
		Если ЗначениеЗаполнено(Группа.Подсказка) Или ЗначениеЗаполнено(Группа.Высота) Или ЗначениеЗаполнено(
			Группа.Ширина) Или Группа.РастягиватьПоВертикали <> Неопределено Или Группа.РастягиватьПоГоризонтали <> Неопределено
...
Процедура ДобавитьОтступКолонок(Форма, ЭлементыНастроек, УзелИерархииЭлементов)
	...
	Для НомерКолонки = 2 По КоличествоКолонок Цикл
		...
		ГруппаПолейКолонки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Поля колонки %1'"), НомерКолонки);
...
Функция ЭлементыФормыНастроек(Форма, ОписаниеНастроек, ИменаРеквизитов)
	...
	Для Каждого Запись Из НеполныеЭлементы Цикл
		...
		Если Элементы.Найти(ИмяДополнительногоЭлемента) <> Неопределено Или ГруппаЭлемента.ПодчиненныеЭлементы.Найти(
			ИмяДополнительногоЭлемента) <> Неопределено Тогда
	...
	Для Каждого Элемент Из ГруппаЭлементов.ПодчиненныеЭлементы Цикл
		...
		ИначеЕсли ТипЗнч(Элемент) = Тип("ПолеФормы") Тогда
			...
			ИначеЕсли ТипЗнч(ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
				...
				Запись.ЭтоФлажок = ЗначениеЗаполнено(ЭлементНастройки.Представление) Или ЭлементНастройки.ВидСравнения
					= ВидСравненияКомпоновкиДанных.Заполнено Или ЭлементНастройки.ВидСравнения
					= ВидСравненияКомпоновкиДанных.НеЗаполнено;

				ЭлементПользовательскойНастройки = ПользовательскиеНастройки.Найти(
					ОписаниеНастройкиЭлемента.ИдентификаторНастройки);

				Запись.ЭтоСписок = Не Запись.ЭтоФлажок И ОтчетыКлиентСервер.ЭтоВидСравненияСписка(
					ЭлементПользовательскойНастройки.ВидСравнения);
			ИначеЕсли ТипЗнч(ЭлементНастройки) = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
				Запись.ЭтоФлажок = ЗначениеЗаполнено(ЭлементНастройки.Представление) Или ЗначениеЗаполнено(
...
Процедура ПодготовитьЭлементыФормыНастроекКРаспределению(ЭлементыНастроек, Группировка, ЭтоОсновнаяФорма,
	ДополнительныеСвойства)
...
#Область УстановкаНомеровГрупп
	...
	Для Каждого Поиск Из ВариантыПоиска Цикл
		...
		Для Каждого Элемент Из НайденныеЭлементы Цикл
			...
			Если ((Элемент.ЭтоФлажок Или Элемент.ЭтоСписок) И Индекс <> ПредыдущийИндекс) Или (Не Элемент.ЭтоФлажок
				И Не Элемент.ЭтоСписок И Индекс > ПредыдущийИндекс + 1) Тогда
				НомерГруппы = НомерГруппы + 1;
			КонецЕсли;
...
Процедура РаспределитьЭлементыФормыНастроек(Форма, Знач Группа, ЭлементыНастроек)
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл
		...
		ТолькоПоляВвода = ПризнакиЭлементов.Найти(Истина, "ЭтоФлажок") = Неопределено И ПризнакиЭлементов.Найти(Истина,
			"ЭтоСписок") = Неопределено;
		...
		Для Каждого Признаки Из ПризнакиЭлементов Цикл
			...
			РаспределитьЭлементыФормыНастроекПоСвойствам(Форма, Родитель, ЭлементыНастроек, НомерКолонки,
				Признаки.НомерГруппы);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

Процедура РаспределитьЭлементыФормыНастроекПоСвойствам(Форма, Родитель, ЭлементыНастроек, НомерКолонки,
...
Процедура ВывестиСтилизованныеЭлементыФормыНастроек(Форма, ЭлементыНастроек, ОписаниеНастроек, ИменаРеквизитов,
	ВидыЭлементов)
	// Изменение реквизитов.
	ПутьКДаннымЭлементов = Новый Структура("ПоИмени, ПоИндексу", Новый Соответствие, Новый Соответствие);

	РеквизитыДобавляемые = РеквизитыЭлементовНастроекДобавляемые(ЭлементыНастроек, ВидыЭлементов, ИменаРеквизитов,
		ПутьКДаннымЭлементов);
	...
	Для Каждого ВидЭлемента Из ВидыЭлементов Цикл
		...
		Для Каждого Элемент Из НайденныеЭлементы Цикл
			Если ГраницаПредопределенных >= НайденныеЭлементы.Индекс(Элемент) Тогда
				...
				ПутьКДаннымЭлементов.ПоИндексу.Вставить(Элемент.ИндексНастройки,
					Предопределенные[ИндексПредопределенных]);
				Продолжить;
			КонецЕсли;
...
Процедура ВывестиПериодыНастроек(Форма, ЭлементыНастроек, ИменаРеквизитов)
	НайденныеЭлементы = ЭлементыНастроек.НайтиСтроки(Новый Структура("ЭтоПериод, СвойствоНастройки", Истина,
		"Значение"));
	Если НайденныеЭлементы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл
		...
		Если ИменаРеквизитовПредопределенных.Найти(ИмяРеквизита) <> Неопределено Тогда
			...
			Для Каждого ЭлементПериода Из НайденныйЭлемент.ПодчиненныеЭлементы Цикл
				...
				Если ЭтоСтандартноеПредставление Тогда
					ЭлементПериода.Видимость = ТипЭлементаПериода <> Тип("КнопкаФормы") Или (ТипЭлементаПериода = Тип(
						"КнопкаФормы") И СтрНачинаетсяС(ЭлементПериода.ИмяКоманды, "ВыбратьПериод"));
				Иначе
					ЭлементПериода.Видимость = (ТипЭлементаПериода = Тип("КнопкаФормы") Или ТипЭлементаПериода = Тип(
						"ГруппаФормы"));
				КонецЕсли;
		...
		ДобавитьПолеПериода(Элементы, Группа, ШаблонИмениЭлемента, "ДатаНачала", Поле.Заголовок,
		...
		ДобавитьПолеПериода(Элементы, Группа, ШаблонИмениЭлемента, "ДатаОкончания", Поле.Заголовок,
			ЭтоСтандартноеПредставление);
...
Процедура ДобавитьПолеПериода(Элементы, Группа, ШаблонИмени, Свойство, ЗаголовокЭлементаНастройки,
	ЭтоСтандартноеПредставление)
...
Функция ЗаголовокЭлементаПериода(Свойство, ЗаголовокЭлементаНастройки)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 (дата %2)'"), ЗаголовокЭлементаНастройки, ?(СтрЗаканчиваетсяНа(НРег(Свойство), НРег(
		"ДатаНачала")), НСтр("ru = 'начала'"), НСтр("ru = 'окончания'")));
...
Процедура УстановитьСвойстваКнопкиВыбораПериода(ЭлементПериода, Период, ЭтоСтандартноеПредставление)
	Если ТипЗнч(ЭлементПериода) <> Тип("ГруппаФормы") Или ЭлементПериода.ПодчиненныеЭлементы.Количество() = 0
...
Процедура ВывестиСпискиНастроек(Форма, ЭлементыНастроек, ОписаниеНастроек, ИменаРеквизитов)
	...
	СвязанныеЭлементы = ЭлементыНастроек.Скопировать(Новый Структура("ЭтоСписок, СвойствоНастройки", Истина,
		"Использование"));
...
Процедура ДобавитьЭлементыСписка(Форма, ЭлементНастройки, ОписаниеЭлементаНастройки, ИмяСписка, ИменаРеквизитов)
	...
	ПолеЗначения.ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(
		ОписаниеЭлементаНастройки.Настройки, Форма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы,
		ОписаниеЭлементаНастройки.ЭлементНастройки);

	ИнициализироватьСписок(Форма, ЭлементНастройки.ИндексНастройки, ПолеЗначения,
		ОписаниеЭлементаНастройки.ЭлементНастройки);
...
Процедура ДобавитьКомандыСписка(Форма, ЭлементНастройки, ЭлементыНастроек, ИмяСписка)
	...
	ДобавитьКомандуСписка(Форма, ГруппаЗаголовка, ИмяКоманды, ЗаголовокКоманды, "Подключаемый_Список_ВставитьИзБуфера",
		БиблиотекаКартинок.ВставитьИзБуфераОбмена);
КонецПроцедуры
...
Функция ЗначенияДляВыбора(ПараметрыНастройки, ТипИлиТипы = Неопределено) Экспорт
	...
	Для Каждого Тип Из Типы Цикл
		...
		Для Каждого ЭлементСписка Из СписокВыбора Цикл
			...
			Если ТипЗнч(ЗначениеДляВыбора.Значение) = Тип("Структура") И ЗначениеДляВыбора.Значение.Свойство(
				"Значение", ЗначениеПеречисления) Тогда
...
Процедура УстановитьУсловияОтборов(ПараметрыЗагрузки, КомпоновщикНастроек) Экспорт
	...
	Для Каждого Условие Из УсловияОтборов Цикл
		...
		Если ОтчетыКлиентСервер.ЭтоВидСравненияСписка(ЭлементПользовательскойНастройки.ВидСравнения) И ТипЗнч(
			ЭлементПользовательскойНастройки.ПравоеЗначение) <> Тип("СписокЗначений") Тогда
		...
		ЭлементНастройки = ОтчетыКлиентСервер.ПолучитьОбъектПоПользовательскомуИдентификатору(
			Настройки, ЭлементПользовательскойНастройки.ИдентификаторПользовательскойНастройки, ,
			ПользовательскиеНастройки);
