...
Процедура ИнициализироватьЗаголовкиОтчета(Форма) Экспорт
	...
	Если Не ВариантыОтчетовСлужебныйКлиентСервер.РежимВариантаОтчета(Форма.КлючТекущегоВарианта) Или Не НастройкиОтчета.РазрешеноИзменятьВарианты
	...
	ДополнитьСвойстваЗаголовковОтчета(Форма, РезультатОтчета, Заголовки, РазрезСвойствЗаголовков,
		ИндексСтруктурыОтчета, ГраницыРазделов);
...
Функция ПредставлениеЭлементаСтруктуры(Элемент) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ПараметрЗаголовка.Значение) И (ПараметрЗаголовка.Использование
			Или ПараметрИспользованияЗаголовка.Использование И ПараметрИспользованияЗаголовка.Значение
			<> ТипВыводаТекстаКомпоновкиДанных.НеВыводить) Тогда
		...
		ИначеЕсли ТипЭлемента = Тип("ГруппировкаКомпоновкиДанных") Или ТипЭлемента = Тип(
			"ГруппировкаДиаграммыКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
	...
	Для Каждого ИндексРаздела Из ИндексРазделовОтчетов Цикл
		...
		Если ТипЗнч(Раздел) = Тип("ТаблицаКомпоновкиДанных") Тогда
			...
			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Раздел.Строки, ПорядокГруппировки, ПорядокПоля,
				ПоляСортировки);

			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Раздел.Колонки, ПорядокГруппировки, ПорядокПоля,
				ПоляСортировки);
		...
		Иначе
			...
			ИндексироватьПоляГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Раздел,
				ПорядокПоля, ПоляСортировки);
			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Раздел.Структура, ПорядокГруппировки, ПорядокПоля,
				ПоляСортировки);
...
Процедура ИндексироватьГруппировкиРазделаОтчета(ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Группировки,
	ПорядокГруппировки, ПорядокПоля, ПоляСортировки = Неопределено)
	...
	Для Каждого Элемент Из Группировки Цикл
		...
		Если ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
			...
			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Элемент.Настройки, Элемент.Настройки.Структура,
				ПорядокГруппировки, ПорядокПоля, ПоляСортировки);

		ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда

			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Элемент.Строки, ПорядокГруппировки, ПорядокПоля,
				ПоляСортировки);

			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Элемент.Колонки, ПорядокГруппировки, ПорядокПоля,
				ПоляСортировки);
		...
		ИначеЕсли ТипЭлемента <> Тип("ДиаграммаКомпоновкиДанных") Тогда
			...
			ИндексироватьПоляГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Элемент,
				ПорядокПоля, ПоляСортировки);
			ИндексироватьГруппировкиРазделаОтчета(
				ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Элемент.Структура, ПорядокГруппировки, ПорядокПоля,
				ПоляСортировки);
...
Функция ПоляСортировки(Группировка, Настройки, ПоляСортировки = Неопределено, Замещать = Истина)
	...
	Для Каждого Элемент Из ЭлементыСортировки Цикл
		...
		Если ТипЗнч(Элемент) = Тип("АвтоЭлементПорядкаКомпоновкиДанных") И ТипГруппировки <> Тип(
			"НастройкиКомпоновкиДанных") Тогда

			ПоляСортировки(Настройки, Настройки, ПоляСортировки, Ложь);

		ИначеЕсли ТипЗнч(Элемент) <> Тип("АвтоЭлементПорядкаКомпоновкиДанных") И (Замещать
...
Функция ИндексГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Группировка, ПорядокГруппировки)
...
Функция СодержитДочерниеГруппировки(Группировка)
	...
	Для Каждого Элемент Из Группировка.Структура Цикл
		...
		Если ТипЭлемента <> Тип("ГруппировкаКомпоновкиДанных") И ТипЭлемента <> Тип(
...
Процедура ИндексироватьПоляГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Группировка,
	ПорядокПоля, ПоляСортировки)
	...
	Для Каждого Элемент Из ПоляГруппировки.Элементы Цикл
		...
		Если ИспользуетсяАвтоПоле И Не ВариантыОтчетовСлужебныйКлиентСервер.ПолеСодержитсяВГруппировкеОтчета(
			Группировка.Выбор, Элемент.Поле) И Не ВариантыОтчетовСлужебныйКлиентСервер.ПолеСодержитсяВГруппировкеОтчета(
			ИспользуемыеНастройки.Выбор, Элемент.Поле) Тогда

			ДобавитьПолеОтчетаВИндекс(ИндексСтруктурыОтчета, ИндексГруппировки, ПоляГруппировки, Элемент, ПорядокПоля,
				ПоляСортировки, ИспользуемыеПоляГруппировки);

		КонецЕсли;

	КонецЦикла;

	ИндексироватьВыбранныеПоляГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Группировка,
		ПорядокПоля, ПоляСортировки, ИспользуемыеПоляГруппировки);
...
Функция ИспользуетсяАвтоПоле(Поля)
	...
	Для Каждого Элемент Из Поля.Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Или ТипЗнч(Элемент) = Тип(
			"АвтоПолеГруппировкиКомпоновкиДанных") Тогда
...
Процедура ИндексироватьВыбранныеПоляГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Группировка,
	ПорядокПоля, ПоляСортировки, ИспользуемыеПоляГруппировки, Поля = Неопределено, Родитель = Неопределено) // АПК:134 реализация сложной логики для формирования индекса структуры отчета.
	...
	Для Каждого Элемент Из Поля.Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Тогда
			...
			ИндексироватьВыбранныеПоляНастроекОтчета(
				ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Группировка, ПорядокПоля, ПоляСортировки,
		...
		ИначеЕсли РазрешеноИспользоватьПолеВГруппировке(Группировка, Элемент.Поле) Тогда
			...
			ДобавитьПолеОтчетаВИндекс(
				ИндексСтруктурыОтчета, ИндексГруппировки, Родитель, Элемент, ПорядокПоля, ПоляСортировки,
				ИспользуемыеПоляГруппировки);
...
Процедура ИндексироватьВыбранныеПоляНастроекОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Группировка,
	ПорядокПоля, ПоляСортировки, ИспользуемыеПоляГруппировки, Поля = Неопределено, Родитель = Неопределено) // АПК:134 реализация сложной логики для формирования индекса структуры отчета.
	...
	Для Каждого Элемент Из Поля.Элементы Цикл
		...
		Если ТипЭлемента = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			...
			ИндексироватьВыбранныеПоляНастроекОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки, Группировка,
				ПорядокПоля, ПоляСортировки, ИспользуемыеПоляГруппировки, Элемент, Родитель);

		ИначеЕсли ТипЭлемента <> Тип("АвтоВыбранноеПолеКомпоновкиДанных") И РазрешеноИспользоватьПолеВГруппировке(
			Группировка, Элемент.Поле) И Не ВариантыОтчетовСлужебныйКлиентСервер.ПолеСодержитсяВГруппировкеОтчета(
			Группировка.Выбор, Элемент.Поле, Ложь) Тогда

			ДобавитьПолеОтчетаВИндекс(ИндексСтруктурыОтчета, ИндексГруппировки, Родитель, Элемент, ПорядокПоля,
				ПоляСортировки, ИспользуемыеПоляГруппировки);
...
Функция РазрешеноИспользоватьПолеВГруппировке(Группировка, Поле)
	...
	Если ЭтоГруппировкаДетальныхЗаписей И (ВариантыОтчетовСлужебныйКлиентСервер.ПолеСодержитсяВГруппировкеОтчета(
		Группировка.Выбор, Поле)
		Или Не ВариантыОтчетовСлужебныйКлиентСервер.ПолеИспользуетсяВРодительскихГруппировкахОтчета(
		Группировка.Родитель, Поле)) Тогда
	...
	Для Каждого Элемент Из Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("ПолеГруппировкиКомпоновкиДанных") И Элемент.Использование И (Элемент.Поле = Поле
			Или СтрНайти(Строка(Элемент.Поле), Строка(Поле)) > 0 Или СтрНайти(Строка(Поле), Строка(Элемент.Поле)) > 0) Тогда
...
Процедура ДобавитьПолеОтчетаВИндекс(ИндексСтруктурыОтчета, ИндексГруппировки, Родитель, Поле, ПорядокПоля,
	ПоляСортировки, ИспользуемыеПоляГруппировки)
...
Функция ОписаниеПоляОтчета(Родитель, Поле, ИспользуемыеПоляГруппировки)
	...
	ТипГруппировки = ?(ПолеГруппировки = Неопределено, ТипГруппировкиКомпоновкиДанных.Элементы,
...
Функция РолиПолейОтчета(АдресСхемы)
	...
	Для Каждого НаборДанных Из Схема.НаборыДанных Цикл
		...
		Для Каждого Поле Из НаборДанных.Поля Цикл
			...
			Если ТипЗнч(Поле) = Тип("ПапкаПолейНабораДанныхСхемыКомпоновкиДанных") Или ТипЗнч(Поле) = Тип(
				"ВложенныйНаборДанныхСхемыКомпоновкиДанных") Тогда
...
Процедура ОпределитьДоступныеДействияПолейРазделаОтчета(ИндексСтруктурыОтчета, ПорядокРаздела, ЭтоРесурс = Ложь)
	...
	Для Каждого ПорядокГруппировки Из ПорядокГруппировок Цикл
		...
		Для Каждого ПолеГруппировки Из ПоляГруппировки Цикл
			...
			Индекс.ПереместитьПолеВыше = Не ЭтоПолеГруппировкиКолонки И ПорядокРаздела = 1 И Не ЭтоРесурс И Индекс.СодержитРодительскиеГруппировки
				И ПоляГруппировкиВыше.Найти(Не ПолеГруппировки.Период, "Период") = Неопределено;

			Индекс.ПереместитьПолеНиже = Не ЭтоПолеГруппировкиКолонки И ПорядокРаздела = 1 И Не ЭтоРесурс И Индекс.СодержитДочерниеГруппировки
			...
			Если Не ЭтоПолеГруппировкиКолонки И ПоляГруппировки.Индекс(ПолеГруппировки) < ПоляГруппировки.Количество()
				- 1 Тогда
...
Функция МакетыКомпоновкиДанныхОписаны(АдресСхемы)
	...
	Возврат Схема.Макеты.Количество() > 0 Или Схема.МакетыЗаголовковГруппировок.Количество() > 0
		Или Схема.МакетыГруппировок.Количество() > 0 Или Схема.МакетыПолей.Количество() > 0
...
Процедура ДополнитьСвойстваЗаголовковОтчета(Форма, РезультатОтчета, Заголовки, РазрезСвойствЗаголовков,
	ИндексСтруктурыОтчета, ГраницыРазделов)
...
Функция ИндексПоляПоСвойствамЗаголовка(Форма, СвойстваЗаголовка, ИндексСтруктурыОтчета, ПорядокРаздела, ОбработанныеПоля)
	...
	Если ИндексПоля = Неопределено Тогда
		ИндексПоля = ИндексПоляПоРасшифровке(Форма, СвойстваЗаголовка.Расшифровка, ИндексСтруктурыОтчета, Поиск,
			ОбработанныеПоля);
	КонецЕсли;
...
Функция ИндексПоляПоПредставлению(ИндексСтруктурыОтчета, Поиск, ОбработанныеПоля)
	...
	Для Каждого ИндексПоля Из НайденныеПоля Цикл
		...
		Если ОбластьВладельцаСРеквизитами И КоличествоСовпадений = ОписаниеПредставления.Количество() Или Не ОбластьВладельцаСРеквизитами
...
Функция ПрерватьСканированиеРезультатаОтчета(Ячейка, ИндексСтруктурыОтчета, ГраницаЗаголовкаРаздела)
	...
	Если ГраницаЗаголовкаРаздела = Неопределено И Не ЭтоЯчейкаЗаголовкаОтчета(Ячейка) Тогда

		Возврат Ложь;
	КонецЕсли;

	Если ГраницаЗаголовкаРаздела = Неопределено Или ЭтоЯчейкаЗаголовкаОтчета(Ячейка) И Ячейка.Верх
		- ГраницаЗаголовкаРаздела = 1 Тогда
...
Функция ДанныеЭлементаРасшифровки(Форма, Расшифровка) Экспорт
	...
	Если Не ВариантыОтчетовСлужебныйКлиентСервер.РежимВариантаОтчета(Форма.КлючТекущегоВарианта)
		Или Не Форма.НастройкиОтчета.РазрешеноИзменятьВарианты Или Не ТипЗнч(Расшифровка) = Тип(
		"ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
	...
	Если ТипЗнч(ЭлементРасшифровки) = Тип("ЭлементРасшифровкиКомпоновкиДанныхГруппировка") Или ТипЗнч(Родитель) <> Тип(
		"ЭлементРасшифровкиКомпоновкиДанныхГруппировка") Тогда
	...
	Иначе
		...
		ДанныеЭлементаРасшифровки.Вставить("Значение", ?(СвойстваПоля.Значение = Null, Неопределено,
			СвойстваПоля.Значение));
	...
	ДанныеЭлементаРасшифровки.Вставить("ЭтоСсылка", ОбщегоНазначения.ЭтоСсылка(ТипЗнч(
		ДанныеЭлементаРасшифровки.Значение)));
...
Функция ОтборыВыделенныхЭлементовРасшифровки(Документ, ВыделенныеОбластиДокумента, Данные)
	...
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбластиДокумента Цикл
		...
		Для НомерСтроки = ВыделеннаяОбласть.Верх По ВыделеннаяОбласть.Низ Цикл
			...
			Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И ТипЗнч(Область.Расшифровка)
...
Функция ЗаполнитьЗначенияФильтра(ПараметрыЗаполнения) Экспорт
	...
	Иначе
		КоличествоЧитаемыхСтрок = Мин(КоличествоПервыхЧитаемыхСтрок + СвойстваЗаголовка.Низ,
	...
	Для НомерСтроки = СвойстваЗаголовка.Низ + 1 По Документ.ВысотаТаблицы Цикл
		...
		Если Заголовки[Ячейка.Имя] <> Неопределено Или Ячейка.ТипОбласти <> ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
...
Процедура УдалитьНеактуальныеРазделаОтчета(Настройки, СвойстваЗаголовка)
	...
	Пока ИндексРаздела >= 0 Цикл
		...
		Если Не Раздел.Использование Или ТипЗнч(Раздел) <> Тип("ГруппировкаКомпоновкиДанных") И ТипЗнч(Раздел) <> Тип(
			"ТаблицаКомпоновкиДанных") Тогда
