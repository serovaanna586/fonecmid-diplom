...
Процедура ОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении = Неопределено,
	...
	Если ДлительнаяОперация.Свойство("АдресДополнительногоРезультата") Тогда
		РасширенныеПараметры.Вставить("АдресДополнительногоРезультата",
			ДлительнаяОперация.АдресДополнительногоРезультата);
	КонецЕсли;
	...
	Если РасширенныеПараметры.ВыводитьОкноОжидания Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ДлительнаяОперация", РасширенныеПараметры, ?(ПараметрыОжидания <> Неопределено,
	...
	Иначе
		...
		РасширенныеПараметры.Вставить("ОповещениеОЗавершении", ОповещениеОЗавершении);
		РасширенныеПараметры.Вставить("ТекущийИнтервал", ?(РасширенныеПараметры.Интервал <> 0,
			РасширенныеПараметры.Интервал, 1));
...
Функция ОткрытьФормуДлительнойОперации(Знач ВладелецФормы, Знач ИдентификаторЗадания) Экспорт
	...
	Возврат ОткрытьФорму("ОбщаяФорма.ДлительнаяОперация", Новый Структура("ИдентификаторЗадания", ИдентификаторЗадания),
		ВладелецФормы);
...
Функция ПараметрыПроверкиДлительныхОпераций(ОбсужденияАктивны, Интервал)
	...
	Для Каждого ДлительнаяОперация Из АктивныеДлительныеОперации Цикл
		...
		Иначе
			...
			ДатаКонтроля = ДлительнаяОперация.Контроль + ?(Не ОбсужденияАктивны Или ДлительнаяОперация.ТекущийИнтервал
				> ИнтервалКонтроляОбсуждений, 0, ИнтервалКонтроляОбсуждений - ДлительнаяОперация.ТекущийИнтервал);
	...
	Если Не ЗначениеЗаполнено(ЗаданияДляПроверки) И Не ЗначениеЗаполнено(ЗаданияДляОтмены) Тогда
...
Функция ДлительнаяОперацияОтменена(ДлительнаяОперация)
	...
	Возврат ДлительнаяОперация.ФормаВладелец <> Неопределено И Не ДлительнаяОперация.ФормаВладелец.Открыта();
...
Процедура ПриПолученииСерверногоОповещения(ИмяОповещения, Результат) Экспорт
	...
	Если Операция = Неопределено Или ДлительнаяОперацияОтменена(Операция) Тогда
		Возврат;
	КонецЕсли;
...
Функция ОбработатьРезультатАктивнойОперации(ДлительнаяОперация, Результат)
	...
	Если Результат.Статус <> "Выполняется" Тогда
		Если Результат.Статус <> "Выполнено" Или Результат.Свойство("ФоновоеЗаданиеЗавершено")
			Или Не (ДлительнаяОперация.Свойство("АдресРезультата") И ЗначениеЗаполнено(
			ДлительнаяОперация.АдресРезультата) Или ДлительнаяОперация.Свойство("АдресДополнительногоРезультата")
...
Процедура ПроверитьПараметрыОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении,
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ДлительныеОперацииКлиент.ОжидатьЗавершение", "ДлительнаяОперация",
		ДлительнаяОперация, Тип("Структура"));
	...
	Если ПараметрыОжидания <> Неопределено Тогда
		...
		СообщениеПроверки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			"ПараметрыОжидания.ВыводитьОкноОжидания", "Истина", "ПараметрыОжидания.ОповещениеОПрогрессеВыполнения");

		ОбщегоНазначенияКлиентСервер.Проверить(Не (ПараметрыОжидания.ОповещениеОПрогрессеВыполнения <> Неопределено
			И ПараметрыОжидания.ВыводитьОкноОжидания), СообщениеПроверки, "ДлительныеОперацииКлиент.ОжидатьЗавершение");
...
Процедура ПоказатьОповещение(ОповещениеПользователя, ВладелецФормы = Неопределено) Экспорт
	...
	Если ТипЗнч(Оповещение.Важное) = Тип("Булево") Тогда
		СтатусОповещения = ?(Оповещение.Важное, СтатусОповещенияПользователя.Важное,
			СтатусОповещенияПользователя.Информация);
	КонецЕсли;

	ПоказатьОповещениеПользователя(?(Оповещение.Текст <> Неопределено, Оповещение.Текст, НСтр(
		"ru = 'Действие выполнено'")), НавигационнаяСсылкаОповещения, ПояснениеОповещения, Оповещение.Картинка,
		СтатусОповещения);
