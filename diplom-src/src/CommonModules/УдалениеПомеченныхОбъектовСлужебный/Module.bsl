...
Процедура УдалениеПомеченныхПоРасписанию() Экспорт
	...
	УдалитьПомеченныеОбъектыСлужебный(
		УдаляемыеОбъекты, "Стандартный", Неопределено, Истина);
		...
		Для Каждого ВыборкаДетальныеЗаписи Из РезультатЗапроса Цикл
			...
			Попытка
				Если ВыборкаДетальныеЗаписи.ВремяБлокировки <= ВремяСнятияБлокировки
					Или ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ВыборкаДетальныеЗаписи.ИдентификаторСеанса)
					= Неопределено Тогда
...
Функция ВыполнитьОбработкуПричинНеудаления(ТаблицаДействий) Экспорт
	...
	Если ОчередьПометкиКУдалению.Количество() > 0 Тогда
		Результат = ОбъединениеТаблиц(
			Результат, ОбработатьОчередьПометокНаУдаление(ОчередьПометкиКУдалению));
	КонецЕсли;
...
Функция УдалитьПомеченныеОбъекты(Знач ИсточникУдаляемыхОбъектов, РежимУдаления, НастройкиДополнительныхРеквизитов,
	...
	ИначеЕсли ИсточникУдаляемыхОбъектов = Неопределено Тогда
		ИсточникУдаляемыхОбъектов = ДеревоПомеченныхНаУдаление(УдалениеПомеченныхОбъектов.ПомеченныеНаУдаление( ,
			Истина), НастройкиДополнительныхРеквизитов, Новый Массив);
	КонецЕсли;
	...
	УдаляемыеОбъекты = ОбъединениеМассивов(
		УдаляемыеОбъектыИзРезультатаДополнительнойОбработки(РезультатПредыдущегоШага), УдаляемыеОбъектыИзДанныхФормы(
		...
		Если Запрос.Выполнить().Пустой() Тогда
			...
			Для Каждого Задание Из ЗаданиеПроверки Цикл
				РегламентныеЗаданияСервер.ИзменитьЗадание(
					Задание.УникальныйИдентификатор, Новый Структура("Использование", Ложь));
			КонецЦикла;
		КонецЕсли;
...
Процедура УстановитьБлокировкуИспользованияУдаляемыхОбъектов(Пакет, Идентификатор) Экспорт
	...
	Попытка
		...
		Для Каждого Задание Из ЗаданиеПроверки Цикл
			РегламентныеЗаданияСервер.ИзменитьЗадание(
				Задание.УникальныйИдентификатор, Новый Структура("Использование", Истина));
		КонецЦикла;
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
...
Функция ДеревоПомеченныхНаУдаление(УдаляемыеОбъекты, Настройки, Отмеченные)
	...
	Для Каждого УдаляемыйСсылка Из УдаляемыеОбъекты Цикл
		...
		УзелУдаляемого.НомерКартинки = НомерКартинки(УдаляемыйСсылка, Истина, УдаляемыйИнформация.Вид, "Удален");
...
Функция ДеревоПомеченныхНаУдалениеБезУдаленных(ПомеченныеНаУдаление, Удаленные)
	...
	Для Каждого Элемент Из УдаляемыеОбъекты Цикл
		Если Элемент.Родитель <> Неопределено И (Удаленные.Найти(Элемент.УдаляемыйСсылка) <> Неопределено
			Или Не Элемент.ЭтоОписаниеОбъектаМетаданных И ИнформацияОТипе(ТипЗнч(Элемент.УдаляемыйСсылка),
			Кеш).Технический) Тогда
...
Процедура ДобавитьСтрокуСвязейНеудаленных(ТаблицаСвязиНеУдаленных, Причина, КешИнформацииОТипах)
	...
	СтрокаПрепятствующегоУдалению.ОбнаруженныйСсылка = ?(СтрокаПрепятствующегоУдалению.ЭтоОшибка, Причина.ОписаниеОшибки,
	...
	СтрокаПрепятствующегоУдалению.НомерКартинки = ?(НомерКартинки <> 0, НомерКартинки, НомерКартинки(
		СтрокаПрепятствующегоУдалению.ОбнаруженныйСсылка, СтрокаПрепятствующегоУдалению.СсылочногоТипа, Вид,
		ОбнаруженныйСтатус));
