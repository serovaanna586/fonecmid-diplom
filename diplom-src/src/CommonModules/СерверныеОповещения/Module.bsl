...
//
Процедура ОтправитьСерверноеОповещениеСИдентификаторомГруппы(ИмяОповещения, Результат, Адресаты, ОтправитьСразу,
	ИдентификаторГруппы = Неопределено) Экспорт
	...
	Если ОтправитьСразу И СерверныеОповещенияСлужебныйПовтИсп.ЭтоСеансОтправкиСерверныхОповещенийКлиентам() Тогда

		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре %1 недопустимо указывать значение %2 для параметра %3
				 |при вызове из процедур %4.'"), "СерверныеОповещения.ОтправитьСерверноеОповещение", "Истина",
			"ОтправитьСразу", "ПриОтправкеСерверногоОповещения");
		ВызватьИсключение ТекстОшибки;

	ИначеЕсли ОтправитьСразу И ТекущийПользовательЗарегистрированВСистемеВзаимодействия() Тогда
...
Функция НедоставленныеСерверныеОповещенияСеанса(Знач Параметры) Экспорт
	...
	Если Не СеансАдминистратораСервиса Тогда
		...
		Если ТекущаяДатаСеанса() > ДатаСледующейПроверки И ОбщегоНазначения.ПодсистемаСуществует(
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Содержимое.ИмяОповещения) Тогда
			Если ТипЗнч(Содержимое.Адресаты) = Тип("Соответствие") Тогда
				...
				Если ТипЗнч(КлючиСеансов) <> Тип("Массив") Или КлючиСеансов.Найти(КлючЭтогоСеанса) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			Данные = НовыеДанныеСообщения();
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При вызове процедуры ""%1"" возникла ошибка:
					 |%2'"), Оповещение.ИмяМодуляОтправки + ".ПриОтправкеСерверногоОповещения",
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Серверные оповещения.Ошибка работы фонового задания'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОтправитьПодготовленныеСерверныеОповещения(СостояниеОтправки, МинимальныйПериодПоПользователям)
			...
			Если ТипЗнч(Содержимое.Адресаты) <> Тип("Соответствие") Тогда
				Если Не СообщениеУжеОтправлено(СостояниеОтправки, Выборка, "ВсеПользователи") Тогда
					...
					Если ОтправитьСообщение(Данные, ИдентификаторОбщегоОбсуждения) Тогда
						Контекст.ДатыУспешнойОтправкиОповещенийПоПользователям.Вставить("ВсеПользователи",
							ТекущаяДатаСеанса());
					Иначе
						Если Контекст.ДатыОповещенийСОшибкамиПоПользователям.Получить("ВсеПользователи") = Неопределено Тогда
							Контекст.ДатыОповещенийСОшибкамиПоПользователям.Вставить("ВсеПользователи",
	...
	Если ЗначениеЗаполнено(ДатаОповещенияСОшибкой) И ДатаОповещенияСОшибкой < ДатаПоследнегоОповещения
		- МаксимальныйПериодПовторенияДоставки() Тогда
	...
	ОбновитьСостояниеОтправки(СостояниеОтправки, "ДатаОповещенияСОшибкой,
	...
	Если Адресаты.Количество() > 20 Тогда
		...
		Если ОтправитьСообщение(Данные, Контекст.ИдентификаторОбщегоОбсуждения) Тогда
			Для Каждого ОписаниеАдресата Из Адресаты Цикл
				Контекст.ДатыУспешнойОтправкиОповещенийПоПользователям.Вставить(ОписаниеАдресата.Ключ,
					ТекущаяДатаСеанса());
			КонецЦикла;
		Иначе
			Для Каждого ОписаниеАдресата Из Адресаты Цикл
				Если Контекст.ДатыОповещенийСОшибкамиПоПользователям.Получить(ОписаниеАдресата.Ключ) = Неопределено Тогда
					Контекст.ДатыОповещенийСОшибкамиПоПользователям.Вставить(ОписаниеАдресата.Ключ,
						Данные.ДатаДобавления);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	Иначе
		Для Каждого ОписаниеАдресата Из Адресаты Цикл
			...
			Если ОтправитьСообщение(Данные, ИдентификаторОбсуждения) Тогда
				Контекст.ДатыУспешнойОтправкиОповещенийПоПользователям.Вставить(ИдентификаторПользователяИБ,
					ТекущаяДатаСеанса());
			Иначе
				Если Контекст.ДатыОповещенийСОшибкамиПоПользователям.Получить(ИдентификаторПользователяИБ)
					= Неопределено Тогда
					Контекст.ДатыОповещенийСОшибкамиПоПользователям.Вставить(ИдентификаторПользователяИБ,
						Данные.ДатаДобавления);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОповеститьОбАктивности(СостояниеОтправки, МинимальныйПериодПоПользователям, Контекст)
	...
	Для Каждого ОписаниеПериода Из МинимальныйПериодПоПользователям Цикл
		...
		Если ДатаОтправки = Неопределено Тогда
			ДатаОтправки = СостояниеОтправки.ДатыУспешнойОтправкиОповещенийПоПользователям.Получить(
				ОписаниеПериода.Ключ);
			ОбщаяДатаОтправки = СостояниеОтправки.ДатыУспешнойОтправкиОповещенийПоПользователям.Получить(
				"ВсеПользователи");
			Если ДатаОтправки = Неопределено Или ОбщаяДатаОтправки <> Неопределено И ДатаОтправки < ОбщаяДатаОтправки Тогда
				ДатаОтправки = ОбщаяДатаОтправки;
			КонецЕсли;
...
Функция НовыеСерверныеОповещения(ДатаПоследнегоОповещения)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Отбор", ?(ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных(),
		"ИСТИНА", "ОтправленныеСерверныеОповещения.ОбластьДанныхВспомогательныеДанные = 0"));
...
Функция СостояниеОтправкиСерверныхОповещений()
	...
	Состояние.Вставить("ИдентификаторФоновогоЗадания", ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить константу %1 по причине:
				 |%2'"), "СостояниеОтправкиСерверныхОповещений", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Серверные оповещения.Ошибка работы фонового задания'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
	КонецПопытки;
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить константу %1 по причине:
				 |%2'"), "СостояниеОтправкиСерверныхОповещений", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Серверные оповещения.Ошибка запуска фонового задания'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
	КонецПопытки;
...
Функция ОтправкаСерверныхОповещенийКлиентамУжеВыполняется(СостояниеОтправки)
	...
	Возврат ИсполняющееФоновоеЗадание <> Неопределено И ИсполняющееФоновоеЗадание.Состояние
...
Процедура УдалитьУстаревшиеОповещения()
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Отбор", ?(ДоступноИспользованиеРазделенныхДанных, "ИСТИНА",
	...
	Для Каждого КлючИЗначение Из Оповещения Цикл
		...
		Если Не ЗначениеЗаполнено(Оповещение.Имя) Или КлючИЗначение.Ключ <> Оповещение.Имя Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре ""%1""
					 ...
					 |%2 = ""%3""
					 |%4 = ""%5"".'"), "ОбщегоНазначенияПереопределяемый.ПриДобавленииСерверныхОповещений", "Ключ",
				КлючИЗначение.Ключ, "Оповещение.Имя", Оповещение.Имя);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Оповещение.ИмяМодуляПолучения) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ОбщегоНазначенияПереопределяемый.ПриДобавленииСерверныхОповещений", Оповещение.Имя,
				"ИмяМодуляПолучения");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		Если Метаданные.ОбщиеМодули.Найти(Оповещение.ИмяМодуляПолучения) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре ""%1""
					 ...
					 |""%4"".'"), "ОбщегоНазначенияПереопределяемый.ПриДобавленииСерверныхОповещений", Оповещение.Имя,
				"ИмяМодуляПолучения", Оповещение.ИмяМодуляПолучения);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		Если Метаданные.ОбщиеМодули.Найти(Оповещение.ИмяМодуляОтправки) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре ""%1""
					 ...
					 |в свойстве ""%3"" указан несуществующий общий модуль
					 |""%4"".'"), "ОбщегоНазначенияПереопределяемый.ПриДобавленииСерверныхОповещений", Оповещение.Имя,
				"ИмяМодуляОтправки", Оповещение.ИмяМодуляОтправки);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось настроить регламентное задание
				 |""%1"" по причине:
				 |%2'"), Метаданные.РегламентныеЗадания.ОтправкаСерверныхОповещенийКлиентам.Имя,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(
...
Процедура НастроитьЗаданиеОтправкаСерверныхОповещенийКлиентамБезПопытки(Включить, ПериодПовтора, ПриЗапуске)
	...
	ИмяПользователя = "";
	Если ЗначениеЗаполнено(ИмяПользователя()) Или Не ПриЗапуске
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать служебного пользователя ""%1"" по причине:
					 |%2'"), ИмяСлужебногоПользователя(), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ВызватьИсключение ТекстОшибки;
		КонецПопытки;
		Если ПользовательИБ <> Неопределено Тогда
	...
	Попытка
		...
		НайденныеЗадания = РегламентныеЗаданияСервер.НайтиЗадания(Отбор);
		Если НайденныеЗадания.Количество() = 0 Или НайденныеЗадания[0].УникальныйИдентификатор
			<> Задание.УникальныйИдентификатор Тогда
			НастроитьЗаданиеОтправкаСерверныхОповещенийКлиентамБезПопытки(Включить, ПериодПовтора, ПриЗапуске);
	...
	Для Каждого КлючИЗначение Из ПараметрыЗадания Цикл
		...
		Если КлючИЗначение.Ключ = "Расписание" Тогда
			Если ПриЗапуске Тогда
				Если ПараметрыЗадания.Расписание.ПериодПовтораВТечениеДня < Задание.Расписание.ПериодПовтораВТечениеДня Тогда
					Возврат Ложь;
				КонецЕсли;
...
Функция ТекущийПользовательЗарегистрированВСистемеВзаимодействия()
	...
	Если Не ЗначениеЗаполнено(ПользовательИБ.Имя) Или Не ОбщегоНазначения.ПодсистемаСуществует(
		"СтандартныеПодсистемы.Обсуждения") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось зарегистрировать текущего пользователя
				 ...
				 |%3'"), ПользовательИБ.Имя, НРег(ПользовательИБ.УникальныйИдентификатор), ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Серверные оповещения.Ошибка регистрации пользователя в системе взаимодействия'",
			ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрацииОшибкиСВ(ИнформацияОбОшибке), , ,
			ТекстОшибки);
		Возврат Ложь;
	КонецПопытки;
...
Функция СлужебныйПользовательИБ()
	...
	РольДляЗапуска = ?(ОбщегоНазначения.РазделениеВключено(), Метаданные.Роли.БазовыеПраваБСП,
		Метаданные.Роли.Администрирование);
	Если ПользовательИБ <> Неопределено Тогда
	...
	ОписаниеПользователяИБ.Вставить("Роли", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РольДляЗапуска));
...
Функция ИдентификаторЛичногоОбсуждения(ИдентификаторПользователяИБ = Неопределено)
	...
	Попытка
		...
		Иначе
			Попытка
				ИдентификаторТекущегоПользователя = СистемаВзаимодействия.ПолучитьИдентификаторПользователя(
		...
		Пока Истина Цикл
			...
			КонецПопытки;
		КонецЦикла;
		Если Не Создано И (Обсуждение.Отображаемое Или ОбновлятьУчастниковОбсуждения
			И (Обсуждение.Участники.Количество() <> 1 Или Не Обсуждение.Участники.Содержит(
			ИдентификаторТекущегоПользователя))) Тогда
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось создать личное обсуждение для пользователя ""%1 (%2)"" по причине:
				 |%3'"), ПользователиИнформационнойБазы.ТекущийПользователь().Имя, НРег(ИдентификаторПользователяИБ),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Серверные оповещения.Ошибка создания личного обсуждения'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ИдентификаторОбщегоОбсуждения()
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Серверные оповещения.Ошибка создания общего обсуждения'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрацииОшибкиСВ(ИнформацияОбОшибке), , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		Возврат Неопределено;
	КонецПопытки;
	УстановитьПривилегированныйРежим(Ложь);
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось отправить сообщение в обсуждение %1 по причине:
				 |%2'"), НРег(ИдентификаторОбсуждения), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Серверные оповещения.Ошибка отправки сообщения'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрацииОшибкиСВ(ИнформацияОбОшибке), , , ТекстОшибки);
		Возврат Ложь;
	КонецПопытки;
