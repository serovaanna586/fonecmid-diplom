...
Процедура ДополнитьДублиСвязаннымиПодчиненнымиОбъектами(ПарыЗамен, ПараметрыЗамены) Экспорт
	...
	Пока Позиция <= ИспользуемыеСвязи.Количество() - 1 Цикл
		...
		ЧастиПакета.Добавить(ТекстЗапросаОбъектовДляЗамены(
			 ОписаниеПодчиненногоОбъекта, СвязиПодчиненногоОбъекта));
	...
	Для Каждого Замена Из ТаблицыНайденныхДублей Цикл
		...
		Если ЗначениеЗаполнено(ОписаниеПодчиненногоОбъекта.ИмяМодуляМетодаПоиска) И НеподобранныеЗамены.Количество()
			> 0 Тогда

			ВыполнитьПоискПоПрикладнымПравилам(
				ИспользуемыеСвязи, НеподобранныеЗамены, ОписаниеПодчиненногоОбъекта, ПарыЗамен);
...
Функция ПроверитьВозможностьЗаменыЭлементов(ПарыЗамен, ПараметрыЗамены) Экспорт
	...
	Если СведенияОбОбъекте <> Неопределено И (СведенияОбОбъекте = "" Или СтрНайти(СведенияОбОбъекте,
		"ВозможностьЗаменыЭлементов") > 0) Тогда
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Для Каждого ПодключенныйОбъект Из ПодключенныеОбъекты Цикл
		...
		Если ПравоДоступа("Изменение", ПодключенныйОбъект) И Источники.Строки.Найти(ПодключенныйОбъект, "Метаданные")
			<> Неопределено Тогда
...
Функция НастройкиОбъектовМетаданных() Экспорт
	...
	ЗарегистрироватьКоллекциюМетаданных(Настройки, СписокОбъектов, Метаданные.ПланыВидовХарактеристик,
...
Процедура ДобавитьСвязиПодчиненныхОбъектов(СвязиПодчиненныхОбъектов, СтрокаСвязи)
	...
	КлючевыеРеквизиты = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаСвязи.ПоляСвязей, ",", ,
		Истина);
	...
	Для Каждого ИмяРеквизита Из КлючевыеРеквизиты Цикл
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не существует поле связи %1 в объекте метаданных %2'"), ИмяРеквизита, ИмяПодчиненногоОбъекта);
...
Функция НовыйПараметрыЗамены(НайденныйДубль, ОписаниеПодчиненногоОбъекта, ИспользуемыеСвязи)
	...
	ПараметрыЗамены.Вставить("ИспользуемыеСвязи", ИспользуемыеСвязи.Скопировать(Новый Структура("Ключ",
		ОписаниеПодчиненногоОбъекта.Ключ)));
...
Функция ТекстЗапросаОбъектовДляЗамены(ОписаниеПодчиненногоОбъекта, Связи)
	...
	Для Каждого КлючевойРеквизит Из ОписаниеПодчиненногоОбъекта.КлючевыеРеквизиты Цикл
		...
		Если ИзменяемыеРеквизитыМассив.Найти(КлючевойРеквизит) <> Неопределено Тогда
			...
			Значение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ВЫБОР КОГДА Замена%1.Оригинал IS NULL ТОГДА Таб.%1 ИНАЧЕ Замена%1.Дубль КОНЕЦ КАК %1",
				КлючевойРеквизит);
			ЗначениеИзменяемыеРеквизиты = ЗначениеИзменяемыеРеквизиты + ?(ПустаяСтрока(ЗначениеИзменяемыеРеквизиты),
				Значение, "," + Значение);
			;
		
			Значение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(" ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаЗамен КАК Замена%1
																			   |ПО Таб.%1 = Замена%1.Оригинал",
				КлючевойРеквизит);
		...
		Иначе
			...
			Значение =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Таб.%1 КАК %1", КлючевойРеквизит);
			НеИзменяемыеРеквизиты = НеИзменяемыеРеквизиты + ?(ПустаяСтрока(НеИзменяемыеРеквизиты), Значение, ","
				+ Значение);
	...
	ВставитьВТекстЗапроса(ТекстЗапроса, "ЛЕВОЕ СОЕДИНЕНИЕ #СоединениеЗначениеЗаменяемыхРеквизитов ПО ИСТИНА",
		СоединениеЗначениеЗаменяемыхРеквизитов);
	ВставитьВТекстЗапроса(ТекстЗапроса, "&СвязиПоКлючу", СвязиПоКлючу);
	ВставитьВТекстЗапроса(ТекстЗапроса, "&НеИзменяемыеРеквизиты", ?(ПустаяСтрока(НеИзменяемыеРеквизиты),
		"НЕОПРЕДЕЛЕНО", НеИзменяемыеРеквизиты));
	ВставитьВТекстЗапроса(ТекстЗапроса, "&ИзменяемыеРеквизиты", ?(ПустаяСтрока(ИзменяемыеРеквизиты), "НЕОПРЕДЕЛЕНО",
		ИзменяемыеРеквизиты));
...
Процедура ПоместитьТаблицыЗаменВЗапрос(МВТ, ТаблицыЗамен)
	...
	Для Каждого ТаблицаЗначения Из ТаблицыЗамен Цикл
		...
		ТекстЗапроса = ТекстЗапроса + Разделитель + СтрЗаменить(ШаблонЗапросаОбъединения, "ИмяТаблицы",
			ТаблицаЗначения.Ключ);
...
Процедура ВыполнитьПоискПоПрикладнымПравилам(Знач ИспользуемыеСвязи, Знач НеподобранныеЗамены,
	Знач ОписаниеПодчиненногоОбъекта, Знач ПарыЗамен)
	...
	Для Сч = 0 По НеподобранныеЗамены.Количество() - 1 Цикл
		...
		НеподобранныеЗамены[Сч] = НовыйПараметрыЗамены(
			НеподобранныеЗамены[Сч], ОписаниеПодчиненногоОбъекта, ИспользуемыеСвязи);
	...
	ОбщегоНазначения.ВыполнитьМетодКонфигурации(
		ОписаниеПодчиненногоОбъекта.ИмяМодуляМетодаПоиска + ".ПриПоискеЗаменыСсылок", ПараметрыМетодаПоиска);
...
Функция ОбъектыСКомандамиОбъединенияДублей()
	...
	ПоискИУдалениеДублейПереопределяемый.ПриОпределенииОбъектовСКомандамиОбъединенияДублейЗаменыСсылок(
		ОбъектыСКомандами);
