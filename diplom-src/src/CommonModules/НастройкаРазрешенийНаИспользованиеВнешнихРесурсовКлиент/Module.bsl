...
//
Процедура НачатьИнициализациюЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Знач Идентификаторы, Знач ФормаВладелец,
	Знач ОповещениеОЗакрытии, Знач РежимВключения = Ложь, Знач РежимОтключения = Ложь, Знач РежимВосстановления = Ложь) Экспорт
	...
	Если РежимВключения Или ОтображатьПомощникНастройкиРазрешений() Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеИнициализацииЗапросаРазрешенийНаИспользованиеВнешнихРесурсов",
			НастройкаРазрешенийНаИспользованиеВнешнихРесурсовКлиент, Состояние);

		ОткрытьФорму(
			"Обработка.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.Форма.ИнициализацияЗапросаРазрешений",
			ПараметрыФормы, ФормаВладелец, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ПослеИнициализацииЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.КодВозврата = КодВозвратаДиалога.ОК Тогда
		...
		Если СостояниеИнициализации.ТребуетсяПрименениеРазрешений Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПослеНастройкиРазрешенийНаИспользованиеВнешнихРесурсов",
				НастройкаРазрешенийНаИспользованиеВнешнихРесурсовКлиент, Состояние);

			ОткрытьФорму(
				"Обработка.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.Форма.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов",
				ПараметрыФормы, Состояние.ФормаВладелец, , , , ОписаниеОповещения,
...
Процедура ПослеНастройкиРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Или Результат = КодВозвратаДиалога.Пропустить Тогда
		...
		ЗапланироватьПроверкуПримененияРазрешенийПослеЗакрытияФормыВладельца(
			Состояние.ФормаВладелец, Состояние.ИдентификаторыЗапросов);
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов",
			НастройкаРазрешенийНаИспользованиеВнешнихРесурсовКлиент, Состояние);
...
Процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Настройка разрешений'"), , НСтр(
			"ru = 'Внесены изменения в настройки профилей безопасности в кластере серверов.'"));
...
Процедура ЗапланироватьПроверкуПримененияРазрешенийПослеЗакрытияФормыВладельца(ВладелецФормы, ИдентификаторыЗапросов)
	...
	Если ТипЗнч(ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ОписаниеОповещенияПроверкиПримененияРазрешений = Новый ОписаниеОповещения("ПроверкаПримененияРазрешенийПослеЗакрытияФормыВладельца",
			НастройкаРазрешенийНаИспользованиеВнешнихРесурсовКлиент, Состояние);
