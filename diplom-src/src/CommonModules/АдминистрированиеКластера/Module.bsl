...
Процедура ПроверитьПараметрыАдминистрирования(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено, ПроверятьПараметрыАдминистрированияКластера = Истина,
	...
	МенеджерАдминистрирования.ПроверитьПараметрыАдминистрирования(ПараметрыАдминистрированияКластера,
		ПараметрыАдминистрированияИБ, ПроверятьПараметрыАдминистрированияИнформационнойБазы,
		ПроверятьПараметрыАдминистрированияКластера);
...
Функция БлокировкаСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено) Экспорт
...
//   ПараметрыАдминистрированияИБ - см. АдминистрированиеКластера.ПараметрыАдминистрированияИнформационнойБазыКластера
//
Процедура СнятьБлокировкуСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено) Экспорт
...
//
Функция СеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено, Знач Фильтр = Неопределено) Экспорт
...
//          - см. АдминистрированиеКластера.ФильтрСеансов
//
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено, Знач Фильтр = Неопределено) Экспорт
...
//
Функция СоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено, Знач Фильтр = Неопределено) Экспорт
...
Процедура УстановитьПрофильБезопасностиИнформационнойБазы(Знач ПараметрыАдминистрированияКластера,
	Знач ПараметрыАдминистрированияИБ = Неопределено, Знач ИмяПрофиля = "") Экспорт
	...
	МенеджерАдминистрирования.УстановитьПрофильБезопасностиИнформационнойБазы(
		ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ, ИмяПрофиля);
...
Функция СвойстваСеансов(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	...
	Возврат МенеджерАдминистрирования.СвойстваСеансов(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИдентификаторИнформационнойБазы, Фильтр,
		ИспользоватьСловарь);
...
Функция СвойстваСоединений(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера,
	...
	Возврат МенеджерАдминистрирования.СвойстваСоединений(
		ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИдентификаторИнформационнойБазы,
		ПараметрыАдминистрированияИнформационнойБазы, Фильтр, ИспользоватьСловарь);
...
Процедура ДобавитьУсловиеФильтра(Фильтр, Знач Свойство, Знач ВидСравненияЗначений, Знач Значение) Экспорт
	...
	Если Фильтр = Неопределено Тогда
		...
		Иначе
			...
			НовыйЭлементФильтра = Новый Структура("Свойство, ВидСравнения, Значение", Свойство, ВидСравненияЗначений,
				Значение);
	...
	ИначеЕсли ТипЗнч(Фильтр) = Тип("Структура") Тогда
		...
		СуществующийЭлементФильтра = Новый Структура("Свойство, ВидСравнения, Значение", Фильтр.Ключ,
			ВидСравнения.Равно, Фильтр.Значение);
		НовыйЭлементФильтра = Новый Структура("Свойство, ВидСравнения, Значение", Свойство, ВидСравненияЗначений,
			Значение);
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверный тип значения параметра %1, ожидалось %2 или %3.'"), "Фильтр", "Структура",
				"КлючИЗначение");
		КонецЕсли;
...
Процедура ДанныеСеансаИзБлокировки(ДанныеСеансов, Знач ТекстБлокировки, Знач КлючСеанса, Знач ИмяИнформационнойБазы) Экспорт
	...
	Если Лев(ТекстНРег, 9) = "бд(сеанс," Тогда
		ЗначенияБлокировкиСтрокой = Сред(ТекстНРег, СтрНайти(ТекстНРег, "(") + 1, СтрНайти(ТекстНРег, ")") - СтрНайти(
			ТекстНРег, "(") - 1);
		ЗначенияБлокировки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ЗначенияБлокировкиСтрокой, ",");
		Если ЗначенияБлокировки.Количество() >= 3 И ЗначенияБлокировки[0] = "сеанс" И ЗначенияБлокировки[1] = НРег(
			ИмяИнформационнойБазы) Тогда
			...
			ДанныеСеансов[КлючСеанса] = Новый Структура("РежимБлокировкиБД, Разделитель", ЗначенияБлокировки[2],
				ЗначениеРазделителя);
