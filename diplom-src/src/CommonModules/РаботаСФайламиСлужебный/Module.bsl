...
Процедура ДобавитьНастройкиРаботыСФайлами(ОбщиеНастройки, ПерсональныеНастройки) Экспорт
	...
	ПерсональныеНастройки.Вставить("ЭтоПолноправныйПользователь", Пользователи.ЭтоПолноправныйПользователь( , , Ложь));
...
Процедура ПриВыполненииДоставкиВПапку(ПараметрыДоставки, Вложения) Экспорт
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Рассылка отчетов ''%1'' от %2'"), ПараметрыДоставки.Рассылка, Формат(
		ПараметрыДоставки.ДатаВыполнения, "ДЛФ=DT"));
	...
	Для Каждого Вложение Из ТаблицаВложений Цикл
		...
		СведенияОФайле.АдресВременногоХранилищаФайла = ПоместитьВоВременноеХранилище(
			Новый ДвоичныеДанные(Вложение.ПолныйПутьКФайлу));
		...
		Иначе
			Вложение.ФайлСсылка = РаботаСФайламиСлужебныйВызовСервера.СоздатьФайлСВерсией(ПараметрыДоставки.Папка,
				СведенияОФайле);
		КонецЕсли;
		...
		Если ПараметрыДоставки.ДобавлятьСсылки <> "" Тогда
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя = СтрЗаменить(
				ПараметрыДоставки.ПредставлениеОтчетовПолучателя, Вложение.ПолныйПутьКФайлу,
				ПолучитьНавигационнуюСсылкуИнформационнойБазы() + "#" + ПолучитьНавигационнуюСсылку(
				Вложение.ФайлСсылка));
		КонецЕсли;
...
Функция ИменаСправочниковХраненияФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь) Экспорт
	...
	Если МетаданныеВладельца <> Неопределено Тогда
		ИмяСтандартногоОсновногоСправочника = МетаданныеВладельца.Имя + ?(СтрЗаканчиваетсяНа(МетаданныеВладельца.Имя,
	...
	Для Каждого КлючИЗначение Из ИменаСправочников Цикл
		...
		Если Метаданные.Справочники.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У владельца файлов типа ""%1""
					 |указан несуществующий справочник ""%2"".'"), Строка(ТипВладельцаФайлов), Строка(
				КлючИЗначение.Ключ)));
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ПрисоединенныеФайлы") И Не КлючИЗначение.Ключ = "Файлы" Тогда
			...
			Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У владельца файлов типа ""%1""
					 ...
					 |без обязательного постфикса ""%3"".'"), Строка(ТипВладельцаФайлов), Строка(КлючИЗначение.Ключ),
				"ПрисоединенныеФайлы"));
		...
		ИначеЕсли КлючИЗначение.Значение = Истина Тогда
			Если ОсновнойСправочникУказан Тогда
				Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У владельца файлов типа ""%1""
						 |основной справочник указан более одного раза.'"), Строка(ТипВладельцаФайлов), Строка(
					КлючИЗначение.Ключ)));
			КонецЕсли;
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У владельца файлов типа ""%1""
				 |не имеется справочников для хранения файлов.'"), Строка(ТипВладельцаФайлов)));
...
Функция ФайлОбъект(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВладелецФайла", МетаданныеОбъектаФайла) Тогда
		...
		Если ЕстьТекущаяВерсия Тогда // С возможностью хранить версии
			...
			ФайлОбъект.Вставить("ВладелецФайла", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрисоединенныйФайл,
				"ВладелецФайла"));
	...
	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("РодительскаяВерсия", МетаданныеОбъектаФайла) Тогда
		...
		ФайлОбъект.Вставить("ВладелецФайла", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлОбъект.Владелец,
			"ВладелецФайла"));
	...
	Для Каждого Справочник Из МетаданныеСправочники Цикл
		Если Справочник.Реквизиты.Найти("ВладелецФайла") <> Неопределено Тогда
			...
			Если ЕстьВозможностьХранитьВерсии Тогда
				СправочникВерсийФайлов = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
					Справочник.Реквизиты.ТекущаяВерсия.Тип.Типы()[0]);
			...
			Иначе
				ТекстЗапроса = ТекстЗапроса + ?(ПустаяСтрока(ТекстЗапроса), "", " ОБЪЕДИНИТЬ ВСЕ") + "
																									 ...
																									 |ВЫБРАТЬ
																									 |	ТИПЗНАЧЕНИЯ(Файлы.ВладелецФайла) "
					+ ?(ДобавитьПсевдонимПоля, "КАК ВладелецФайла,", ",") + "
																			|	Файлы.Размер / 1024 / 1024 " + ?(
					ДобавитьПсевдонимПоля, "КАК ОбъемВсего", "") + "
																   |ИЗ
...
Функция ЭтоСправочникФайловИлиВерсийФайлов(ПолноеИмя) Экспорт
	...
	Если ВРег(ЧастиИмени[0]) <> ВРег("Справочник") И ВРег(ЧастиИмени[0]) <> ВРег("Catalog") Тогда
		Возврат Ложь;
	КонецЕсли;

	Если СтрЗаканчиваетсяНа(ВРег(ЧастиИмени[1]), ВРег("ПрисоединенныеФайлы")) Или ВРег(ЧастиИмени[1]) = ВРег("Файлы")
...
Процедура ПеренестиРезультатыПроверкиПодписей(ПодписиВФорме, ПодписанныйФайл) Экспорт
	...
	Если Свойства.ДатаПроверкиПодписи = Null Или Свойства.ПодписьВерна = Null Тогда
	...
	Для Каждого Строка Из ПодписиВФорме Цикл
		...
		Если Строка.ДатаПодписи <> СтрокаВОбъекте.ДатаПодписи Или Строка.Комментарий <> СтрокаВОбъекте.Комментарий
			Или Строка.КомуВыданСертификат <> СтрокаВОбъекте.КомуВыданСертификат И ЗначениеЗаполнено(
			СтрокаВОбъекте.КомуВыданСертификат) Или Строка.Отпечаток <> СтрокаВОбъекте.Отпечаток И ЗначениеЗаполнено(
			СтрокаВОбъекте.Отпечаток) Или Строка.УстановившийПодпись <> СтрокаВОбъекте.УстановившийПодпись Тогда
			Возврат; // Если объект был изменен, результаты проверки не переносятся.
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаписатьИнформациюОШифровании(ФайлСсылка, Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор,
	...
	Попытка
		...
		Для Каждого ДанныеДляЗаписиНаСервере Из МассивДанныхДляЗанесенияВБазу Цикл
			...
			ПолноеИмяФайлаВРабочемКаталоге = РаботаСФайламиСлужебныйВызовСервера.ПолноеИмяФайлаВРабочемКаталоге(
				ВерсияСсылка, ИмяРабочегоКаталога, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
			...
			Если Не ПустаяСтрока(ДанныеДляЗаписиНаСервере.АдресФайла) И ЭтоАдресВременногоХранилища(
				ДанныеДляЗаписиНаСервере.АдресФайла) Тогда
		...
		Если Не ФайлОбъект.ХранитьВерсии Или (ВозможностьХранитьВерсии И Не ЗначениеЗаполнено(
			ФайлОбъект.ТекущаяВерсия)) Тогда
...
Процедура ПроверитьФайлОбработан(ФайлСсылка, ИмяПроцедуры) Экспорт
	...
	ОбновлениеИнформационнойБазы.ПроверитьОбъектОбработан(ФайлСсылка, ,
		"РаботаСФайлами.ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений", ИмяПроцедуры);
	...
	Для Каждого ФайлБезТекста Из ФайлыДляИзвлеченияТекста Цикл
		...
		Исключение
			Если ФайлЗаблокирован Тогда
				ПоляФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ФайлБезТекста.Ссылка, "Наименование, Расширение");
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Извлечение текста'", ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка, , , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось выполнить регламентное извлечение текста из файла
							 |""%1""
							 |по причине:
							 |""%2"".'"), ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ПоляФайла.Наименование,
					ПоляФайла.Расширение), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
			КонецЕсли;
		КонецПопытки;
...
Функция ПодготовитьДанныеДляСозданияФайловогоНачальногоОбраза(СтруктураПараметров) Экспорт
			...
			Если ЕстьФайлыВТомах И Не ПустаяСтрока(ПутьКАрхивуСФайламиТомов) И (Лев(ПутьКАрхивуСФайламиТомов, 2) <> "\\"
				...
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Путь к архиву с файлами томов должен быть задан
						 |в формате UNC (\\servername\resource)'"), , "ПутьКАрхивуСФайламиТомовWindows");
				Результат.ДанныеПодготовлены = Ложь;
			КонецЕсли;
			Если Не ПустаяСтрока(ПолноеИмяФайловойБазы) И (Лев(ПолноеИмяФайловойБазы, 2) <> "\\" Или СтрНайти(
				ПолноеИмяФайловойБазы, ":") <> 0) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Путь к файловой информационной базе должен быть задан
						 |в формате UNC (\\servername\resource)'"), , "ПолноеИмяФайловойБазыWindows");
				Результат.ДанныеПодготовлены = Ложь;
			КонецЕсли;
		КонецЕсли;
	Иначе
	...
	Если ПустаяСтрока(ПолноеИмяФайловойБазы) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Укажите полное имя файловой базы (файл 1cv8.1cd)'"), , "ПолноеИмяФайловойБазыWindows");
		Результат.ДанныеПодготовлены = Ложь;
			...
			Иначе
				...
				Если Файл.Существует() И ФайлБазы.Существует() Тогда
					Результат.ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Файлы ""%1"" и ""%2"" уже существуют.
						...
						ПутьКАрхивуСФайламиТомов, ПолноеИмяФайловойБазы);
...
Процедура СоздатьФайловыйНачальныйОбразНаСервере(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		СтрокаСоединения = "File=""" + Параметры.ИмяКаталога + """;" + "Locale=""" + Параметры.Язык + """;";
...
Функция ПодготовитьДанныеДляСозданияСерверногоНачальногоОбраза(СтруктураПараметров) Экспорт
	...
	Если ОбщегоНазначения.ЭтоWindowsСервер() Тогда
		...
		Если ЕстьФайлыВТомах Тогда
			Если Не ПустаяСтрока(ПутьКАрхивуСФайламиТомов) И (Лев(ПутьКАрхивуСФайламиТомов, 2) <> "\\" Или СтрНайти(
				ПутьКАрхивуСФайламиТомов, ":") <> 0) Тогда
...
Функция ТекстЗапросаДляИзвлеченияТекста(ПолучитьВсеФайлы = Ложь, ДополнительныеПоля = Ложь) Экспорт
	...
	Для Каждого ИмяСправочника Из ВсеИменаСправочников Цикл
		...
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаДляФайловСНеизвлеченнымТекстом(ИмяСправочника, ЧислоФайловВВыборке,
...
Процедура ОчиститьНенужныеФайлы(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	Если ОчищатьНенужныеФайлы = Перечисления.РежимыОчисткиФайлов.ОчищатьУдаленныеИНенужные Тогда
		...
		НастройкиОчисткиФайлов = НастройкиОчистки.НайтиСтроки(Новый Структура("ЭтоНастройкаДляЭлементаСправочника",
			Ложь));
		Для Каждого Настройка Из НастройкиОчисткиФайлов Цикл

			МассивИсключений = Новый Массив;
			ДетализированныеНастройки = НастройкиОчистки.НайтиСтроки(
				Новый Структура("ИдентификаторВладельца, ЭтоНастройкаДляЭлементаСправочника", Настройка.ВладелецФайла,
				Истина));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.Файлы) Или МодульТекущиеДелаСервер.ДелоОтключено(
		"РедактируемыеФайлы") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТипВладельца Из ТипыВладельцевФайлов Цикл
		...
		Описание = Описание + "
							  |Справочник.ВерсииФайлов.Чтение.Объект." + ПолноеИмяВладельца + "
																							  |Справочник.ВерсииФайлов.Изменение.Объект."
			+ ПолноеИмяВладельца + "
								   |Справочник.Файлы.Чтение.Объект." + ПолноеИмяВладельца + "
																							|Справочник.Файлы.Изменение.Объект."
			+ ПолноеИмяВладельца + "
								   |";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ПодсистемаЭПСуществует Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Перенос электронных подписей и сертификатов шифрования
		...
		Обработчик.ИзменяемыеОбъекты    = ИзменяемыеОбъектыПриПереносеЭлектронныхПодписейИСертификатовШифрования()
			+ "," + СтрСоединить(ПолныеИменаСправочниковПрисоединенныхФайлов(), ", ");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.4.1.1";
	Обработчик.Комментарий = НСтр("ru = 'Перенос информации о наличии файлов в регистр сведений Наличие файлов.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление универсальной даты и типа хранения элементов справочника Файлы.'");
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Параметры.Вставить("ПерсональныеНастройкиРаботыСФайлами",
		Новый ФиксированнаяСтруктура(НастройкиРаботыСФайлами.ПерсональныеНастройки));

	Параметры.Вставить("ОбщиеНастройкиРаботыСФайлами",
		Новый ФиксированнаяСтруктура(НастройкиРаботыСФайлами.ОбщиеНастройки));
...
Процедура ПриОпределенииНаследованияПодсистем(Выгрузка, НаследующиеПодсистемы) Экспорт
	...
	Для Каждого Удаленный Из Удаленные Цикл
		Если СтрНайти(Удаленный.ПолноеИмя, "Подсистема.СтандартныеПодсистемы.Подсистема.ПрисоединенныеФайлы") Тогда
			СтрокаРаботаСФайлами = Выгрузка.Найти("Подсистема.СтандартныеПодсистемы.Подсистема.РаботаСФайлами",
				"ПолноеИмя");
...
Процедура ПослеДобавленияИзмененияПользователяИлиГруппы(Ссылка, ЭтоНовый) Экспорт
	...
	ЭтоПользователь = ТипЗнч = Тип("СправочникСсылка.Пользователи") Или ТипЗнч = Тип(
...
Функция ТекстЗапросаДляОчисткиФайлов(ВладелецФайла, Настройка, МассивИсключений, ЭлементИсключение)
	...
	Если ЕстьВозможностьХранитьВерсии Тогда
		СправочникВерсийФайлов = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
			МетаданныеОбъектаФайлов.Реквизиты.ТекущаяВерсия.Тип.Типы()[0]);
		ПолноеИмяСправочникаВерсийФайлов = СправочникВерсийФайлов.ПолноеИмя;
		...
		Если Настройка.ПериодОчистки <> Перечисления.ПериодОчисткиФайлов.ПоПравилу Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ПолноеИмяСправочникаВерсийФайлов",
				ПолноеИмяСправочникаВерсийФайлов);
		Иначе
	...
	Иначе // Не ЕстьВозможностьХранитьВерсии
		...
		Если Настройка.ПериодОчистки <> Перечисления.ПериодОчисткиФайлов.ПоПравилу Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ТипВладельцаФайла", "Справочник."
				+ Настройка.ТипВладельцаФайла.Имя);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#СправочникВладелецФайла", ВладелецФайла.ПолноеИмя);
		Иначе
...
Функция РежимОчисткиФайлов() Экспорт
	...
	Если Не ЗначениеЗаполнено(Режим) Тогда
		...
		Если Задание.Количество() > 0 Тогда
			Задание = Задание[0];
			Режим = ?(Задание.Использование, Перечисления.РежимыОчисткиФайлов.ОчищатьУдаленныеИНенужные,
				Перечисления.РежимыОчисткиФайлов.НеОчищать);
		Иначе
...
Процедура ЗаписатьВЖурналСобытийОчисткиФайлов(Знач ТекстСообщения, Знач Уровень = Неопределено,
	...
	КонецЕсли;
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОчисткаФайлов(), Уровень, ?(ПрисоединенныйФайл <> Неопределено,
		ПрисоединенныйФайл.Метаданные(), Неопределено), ПрисоединенныйФайл, ТекстСообщения);
...
Функция НавигационнойСсылкиФайла(ФайлСсылка, УникальныйИдентификатор) Экспорт
	...
	Если ЭтоЭлементРаботаСФайлами(ФайлСсылка) Тогда
		Возврат РаботаСФайламиСлужебныйВызовСервера.ПолучитьНавигационнуюСсылкуДляОткрытия(ФайлСсылка,
...
//
Процедура ПриОтправкеФайла(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза = Ложь,
	...
	Если СозданиеНачальногоОбраза И Получатель <> Неопределено И Не ЭтоУзелРаспределеннойИнформационнойБазы(
		Получатель.Ссылка) Тогда
...
Процедура ПриОтправкеФайлаСозданиеНачальногоОбраза(ЭлементДанных, ОтправкаЭлемента, Получатель)
	...
	ИначеЕсли ЭтоЭлементРаботаСФайлами И ТипФайла <> Тип("СправочникОбъект.ИдентификаторыОбъектовМетаданных") Тогда
		...
		Иначе
			...
			Исключение
				СообщениеОбОшибке = НСтр("ru='Не удалось скопировать данные файла во временный каталог.'") + Символы.ПС
					+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());

				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииДляОбмена(), УровеньЖурналаРегистрации.Предупреждение,
					ЭлементДанных.Ссылка.Метаданные(), ЭлементДанных.Ссылка, СообщениеОбОшибке);
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОтправкеФайлаСообщениеОбмена(ЭлементДанных, ОтправкаЭлемента, Получатель)
	...
	ИначеЕсли ЭтоЭлементРаботаСФайлами(ЭлементДанных) И ТипЗнч(ЭлементДанных) <> Тип(
		"СправочникОбъект.ИдентификаторыОбъектовМетаданных") Тогда
...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента, Отправитель = Неопределено)
	...
	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.ВерсииФайлов") Или (ЭтоЭлементРаботаСФайлами(ЭлементДанных)
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если Не ЭлементДанных.ЭтоНовый() Тогда
			ТипХраненияПредыдущейВерсииФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
													ЭлементДанных.Ссылка, "ТипХраненияФайла");
			...
			Если ДвоичныеДанные = Неопределено Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не удалось добавить файл ни в один из томов, т.к. он не существует.
								 ...
								 |%1.'"), ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(
					ЭлементДанных.Наименование, ЭлементДанных.Расширение));

				ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Добавление файла в том'",
					ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, ТипМетаданных,
					ЭлементДанных.Ссылка, ТекстОшибки);
...
Процедура ОбновитьСостояниеОчередиИзвлеченияТекста(ИсточникТекста, СостояниеИзвлеченияТекста) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.РаботаСФайламиВМоделиСервиса")
		И ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда

		МодульРаботаСФайламиСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"РаботаСФайламиСлужебныйВМоделиСервиса");
		МодульРаботаСФайламиСлужебныйВМоделиСервиса.ОбновитьСостояниеОчередиИзвлеченияТекста(ИсточникТекста,
			СостояниеИзвлеченияТекста);
...
//
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "", ЗаголовокОшибки = Неопределено,
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
												|нет справочников для хранения файлов.'"), Строка(ВладелецФайлов),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
												|нет справочника ""%3"" для хранения файлов.'"), Строка(ВладелецФайлов),
			Строка(ТипЗнч(ВладелецФайлов)), Строка(ИмяСправочника));
	КонецЕсли;
...
Функция ИменаСправочниковХраненияВерсийФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь)
	...
	Для Каждого КлючИЗначение Из ИменаСправочников Цикл
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ВерсииПрисоединенныхФайлов") И Не КлючИЗначение.Ключ
			= "ВерсииФайлов" Тогда
		...
		ИначеЕсли КлючИЗначение.Значение = Истина Тогда
			Если ОсновнойСправочникУказан Тогда
				Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У владельца файлов типа ""%1""
						 |основной справочник версий указан более одного раза.'"), Строка(ТипВладельцаФайлов), Строка(
					КлючИЗначение.Ключ)));
			КонецЕсли;
			ОсновнойСправочникУказан = Истина;
		КонецЕсли;
...
//  Строка - имя справочника
//
Функция ИмяСправочникаХраненияВерсийФайлов(ВладелецФайлов, ИмяСправочника = "", ЗаголовокОшибки = Неопределено,
	ОкончаниеОшибки = Неопределено) Экспорт
	...
	ШаблонПричиныОшибки = НСтр("ru = 'У владельца версий файлов ""%1""
...
Функция СравнитьЭлементыОтбора(Элемент1, Элемент2)
	...
	Если Элемент1.Использование = Элемент2.Использование И ТипЗнч(Элемент1) = ТипЗнч(Элемент2) Тогда

		Если ТипЗнч(Элемент1) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			Если Элемент1.ВидСравнения <> Элемент2.ВидСравнения Или Элемент1.ЛевоеЗначение <> Элемент2.ЛевоеЗначение
		...
		Иначе
			...
			Если Элемент1.ТипГруппы <> Элемент2.ТипГруппы Или ЧислоЭлементов <> Элемент2.Элементы.Количество() Тогда
				Возврат Ложь;
			КонецЕсли;
...
//
Функция ПолучитьНавигационнуюСсылкуВоВременномХранилище(ВерсияСсылка, ИдентификаторФормы = Неопределено,
	ВыбрасыватьИсключение = Истина) Экспорт
...
Функция ПолучитьПоказыватьКолонкуРазмер() Экспорт
	...
	ПоказыватьКолонкуРазмер = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиПрограммы",
		"ПоказыватьКолонкуРазмер");
	Если ПоказыватьКолонкуРазмер = Неопределено Тогда
		ПоказыватьКолонкуРазмер = Ложь;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиПрограммы", "ПоказыватьКолонкуРазмер",
			ПоказыватьКолонкуРазмер);
	КонецЕсли;
...
Функция ОшибкаФайлНеНайденВХранилищеФайлов(ФайлОбъект)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось открыть файл:
			 ...
			 |%2'"), ИмяФайла, ОбщегоНазначения.ПредметСтрокой(ФайлОбъект.ВладелецФайла));

	Если ФайлОбъект.ПометкаУдаления Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
			"ru = 'Файл помечен на удаление и очищен как ненужный.'");
	ИначеЕсли ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
			"ru = 'Данные файла недоступны, т.к. возможно файл был очищен как ненужный или удален антивирусной программой.'");
	Иначе
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр(
			"ru = 'Данные файла недоступны, т.к. возможно файл был очищен как ненужный.'");
	КонецЕсли;
...
Процедура СообщитьОбОшибкеФайлНеНайден(ФайлОбъект, ВызыватьИсключение) Экспорт
	...
	КритичностьОшибки = ?(ВызыватьИсключение, УровеньЖурналаРегистрации.Ошибка,
...
Процедура ДобавитьОтборыВСписокФайлов(Список) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Список.Отбор, "Служебный",
		ВидСравненияКомпоновкиДанных.НеРавно, Истина, "СкрыватьСлужебные", Истина);
...
Функция СпрашиватьРежимРедактированияПриОткрытииФайла()
	СпрашиватьРежимРедактированияПриОткрытииФайла = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиОткрытияФайлов", "СпрашиватьРежимРедактированияПриОткрытииФайла");
	Если СпрашиватьРежимРедактированияПриОткрытииФайла = Неопределено Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиОткрытияФайлов",
...
Функция ВариантОткрытияФайла()
	...
	ВариантОткрытияФайла = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиОткрытияФайлов",
		"ВариантОткрытияФайла");
	Если ВариантОткрытияФайла = Неопределено Тогда
		ВариантОткрытияФайла = "Открыть";
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиОткрытияФайлов", "ВариантОткрытияФайла",
			ВариантОткрытияФайла);
	КонецЕсли;
...
Функция ПолучениеФайлаЗапрещено(ЭлементДанных)
	...
	Возврат ЭлементДанных.ЭтоНовый() И Не ПроверитьРасширениеФайлаДляЗагрузки(ЭлементДанных.Расширение, Ложь);
...
Процедура ОбработатьОтправкуФайлаПоТипуХранения(ЭлементДанных)
	...
	Иначе
		...
		Если ЭлементДанных.Метаданные().Реквизиты.Найти("ТекущаяВерсия") <> Неопределено И ЗначениеЗаполнено(
			ЭлементДанных.ТекущаяВерсия) Тогда
		...
		Попытка
			...
			ЭлементДанных.ФайлХранилище = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(
				АдресВоВременномХранилище), Новый СжатиеДанных(9));
		Исключение
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	Знач АдресФайлаВоВременномХранилищеДвоичныеДанные, Знач РеквизитыЗначения = Неопределено)
...
Функция СоздатьВерсию(ФайлСсылка, СведенияОФайле, Контекст = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(СведенияОФайле.ВремяИзменения) Или УниверсальноеВремя(СведенияОФайле.ВремяИзменения)
		> СведенияОФайле.ВремяИзмененияУниверсальное Тогда
	...
	Версия.ИндексКартинки = РаботаСФайламиСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(
	...
	Если СведенияОФайле.СсылкаНаВерсиюИсточник <> Неопределено Тогда // создание Файла из шаблона
		...
		Если ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе И ТипХраненияФайлов
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И ТипХраненияФайлов
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе И ТипХраненияФайлов
			= Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
...
Процедура ОбновитьФайл(ИнформацияОФайле, ПрисоединенныйФайл) Экспорт
	...
	Если Не ИнформацияОФайле.Свойство("ДатаМодификацииУниверсальная") Или Не ЗначениеЗаполнено(
		ИнформацияОФайле.ДатаМодификацииУниверсальная) Или ИнформацияОФайле.ДатаМодификацииУниверсальная
...
Функция ОбновитьВерсиюФайла(ФайлСсылка, СведенияОФайле, ВерсияСсылка = Неопределено,
	УникальныйИдентификаторФормы = Неопределено, Пользователь = Неопределено) Экспорт
	...
	Контекст = КонтекстОбновленияФайла(Версия, СведенияОФайле.АдресВременногоХранилищаФайла, СсылкаНаВерсию,
		ТипХраненияФайла);
	...
	Попытка
		...
		Если СведенияОФайле.ХранитьВерсии Тогда
			ОбновитьВерсиюВФайле(ФайлСсылка, Версия.Ссылка, СведенияОФайле.АдресВременногоХранилищаТекста,
...
Процедура ОбновитьВерсиюВФайле(ФайлСсылка, Версия, Знач АдресВременногоХранилищаТекста,
	...
	Попытка
		...
		Если АдресВременногоХранилищаТекста <> Неопределено И МетаданныеСправочника.ПолнотекстовыйПоиск
	...
	Если Размер > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Размер файла ""%1"" (%2 Мб)
				 |превышает максимально допустимый размер файла (%3 Мб).'"), Имя,
...
//     * Объект - ОпределяемыйТип.ПрисоединенныйФайлОбъект
//
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр,
	НастраиватьОбъектФормы = Ложь) Экспорт
	...
	Если РасширениеФайлаВСписке Тогда
		Если ВозможностьСоздаватьВерсииФайлов И Контекст.Объект.Свойство("ТекущаяВерсия") И ЗначениеЗаполнено(
			Контекст.Объект.ТекущаяВерсия) Тогда
	...
	Если ТолькоПросмотр Или Не ПравоДоступа("Изменение", Контекст.Объект.Ссылка.Метаданные()) Тогда
		УстановитьКнопкиИзмененияНевидимыми(Контекст.Элементы);
	КонецЕсли;

	Если Не ТолькоПросмотр И Не Контекст.Объект.Ссылка.Пустая() И НастраиватьОбъектФормы Тогда
...
Функция ЗаполнитьДанныеФайлаПоШаблону(Контекст, Параметры)
	...
	ЗаполнитьЗначенияСвойств(ОбъектЗначение, КопируемыйОбъект, "Наименование,
															   |Зашифрован,
															   |Описание,
															   |ПодписанЭП,
															   |Размер,
...
Функция СписокПодписейДляОтправки(Источник, УникальныйИдентификатор, ИмяФайла)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		МодульЭлектроннаяПодписьСлужебныйКлиентСервер = ОбщегоНазначения.ОбщийМодуль(
		...
		Для Каждого ЭлектроннаяПодписьФайла Из ЭлектронныеПодписи Цикл
			...
			ИмяФайлаПодписи = МодульЭлектроннаяПодписьСлужебныйКлиентСервер.ИмяФайлаПодписи(
				СоставИмениФайлаДанных.ИмяБезРасширения, Строка(ЭлектроннаяПодписьФайла.КомуВыданСертификат),
				РасширениеДляФайловПодписи);
			ЭлектроннаяПодписьФайла.Вставить("ИмяФайла", ИмяФайлаПодписи);

			ДанныеПоСертификату = МодульЭлектроннаяПодписьСлужебный.ДанныеПоСертификату(ЭлектроннаяПодписьФайла,
				УникальныйИдентификатор);
			ЭлектроннаяПодписьФайла.Вставить("АдресСертификата", ДанныеПоСертификату.АдресСертификата);

			ИмяФайлаСертификата = МодульЭлектроннаяПодписьСлужебныйКлиентСервер.ИмяФайлаСертификата(
				СоставИмениФайлаДанных.ИмяБезРасширения, Строка(ЭлектроннаяПодписьФайла.КомуВыданСертификат),
	...
	Если ФайлВложение.ПодписанЭП Тогда
		...
		Для Каждого ЭлектроннаяПодписьФайла Из СписокПодписей Цикл
			...
			Если ЗначениеЗаполнено(ЭлектроннаяПодписьФайла.АдресСертификата) Тогда
				ОписаниеФайла = ОписаниеФайла(ЭлектроннаяПодписьФайла.ИмяФайлаСертификата,
					ЭлектроннаяПодписьФайла.АдресСертификата);
				Результат.Добавить(ОписаниеФайла);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПодготовитьФайлКУдалению(ПрисоединенныйФайл)
	...
	Попытка
		...
		Если ФайлОбъект = Неопределено Тогда // Уже удален
			...
			Возврат Результат;
		КонецЕсли;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Редактирует", ФайлОбъект.Метаданные()) И ЗначениеЗаполнено(
			ФайлОбъект.Редактирует) Тогда
			ЗаписатьВЖурналСобытийОчисткиФайлов(НСтр(
				"ru = 'Файл открыт на редактирование, поэтому не может быть удален.'"), , ПрисоединенныйФайл);
			ЗафиксироватьТранзакцию();
			Возврат Результат;
		КонецЕсли;
...
Процедура УдалитьНенужныеФайлы(ОписаниеПрисоединенногоФайла)
	...
	Для Каждого ПрисоединенныйФайл Из УдаляемыеПрисоединенныеФайлы Цикл
		...
		Исключение
			Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И УдаляемыеФайлы.Количество() > 0 Тогда
...
Функция УдалитьНенужныйФайл(ПрисоединенныйФайл)
	...
	Попытка
		...
		Если ПрисоединенныйФайлОбъект = Неопределено Тогда // Уже удален
			...
			Возврат Ложь;
		КонецЕсли;
		Если ТипЗнч(ПрисоединенныйФайл) <> Тип("СправочникСсылка.ВерсииФайлов") И ЗначениеЗаполнено(
			ПрисоединенныйФайлОбъект.Редактирует) Тогда
			ЗафиксироватьТранзакцию();
			Возврат Ложь;
		КонецЕсли;
...
Функция ВыбратьДанныеПоПравилу(НастройкаОчистки, МассивИсключений, ПроцессорВывода) Экспорт
	...
	СхемаКомпоновкиДанных.НаборыДанных[0].Запрос = ТекстЗапросаДляОчисткиФайлов(ВладелецФайла, НастройкаОчистки,
		МассивИсключений, ЭлементИсключение);
	...
	МакетКомпоновкиДанных = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанных, КомпоновщикНастроек.Настройки, , , Тип(
	...
	ИначеЕсли ВсеДокументы.СодержитТип(ТипЗнч(ВладелецФайла.ЗначениеПустойСсылки)) Тогда
		...
		Для Каждого Реквизит Из Документ.Реквизиты Цикл
			Если Реквизит.Тип = Новый ОписаниеТипов("Дата") Тогда
				ФрагментЗапроса = СтрЗаменить(ШаблонЗапроса, "&ИмяРеквизитаДата", "СправочникВладелецФайла."
					+ Реквизит.Имя);
				ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "ДнейДоУдаленияОтДаты", "ДнейДоУдаленияОт"
					+ Реквизит.Имя);
...
Функция ТекстЗапросаДляСинхронизацииФайлов(ВладелецФайла, НастройкаСинхронизации, МассивИсключений, ЭлементИсключение)
	...
	Если ТипЗнч(ВладелецФайла) <> Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		СправочникПапок = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(
			НастройкаСинхронизации.ИдентификаторВладельца, Ложь);
	Иначе
	...
	ТекстЗапроса = ТекстЗапроса + "
								  ...
								  |	" + ?(ЕстьВозможностьХранитьВерсии, "ИСТИНА", "ЛОЖЬ") + " КАК ЭтоФайл
																							   |ИЗ
																							   |	Справочник."
		+ СправочникФайлов.Имя + " КАК СправочникФайлов
	...
	ТекстЗапроса = ТекстЗапроса + "
								  ...
								  |	" + ?(ПредставлениеТипаВладельца = "Документ", "СправочникПапок.Представление",
		"СправочникПапок.Наименование") + ",
										  |	СправочникПапок.ПометкаУдаления,";

	Если ОбщегоНазначения.ЭтоСправочник(СправочникПапок) И СправочникПапок.Иерархический Тогда
		ТекстЗапроса = ТекстЗапроса + "
									  |	ВЫБОР
									  |		КОГДА СправочникПапок.Родитель = ЗНАЧЕНИЕ(Справочник."
			+ СправочникПапок.Имя + ".ПустаяСсылка)
									|			ТОГДА НЕОПРЕДЕЛЕНО
...
Функция ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	...
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
			Ответ.КодСостояния, РаскодироватьСтроку(АдресСервера, СпособКодированияСтроки.URLВКодировкеURL),
			ИнформацияОбОшибке);
...
Функция ВыполнитьМетодWebdav(ИмяМетода, АдресФайлаHRef, СоответствиеЗаголовков, СтруктураОбмена, XMLЗапрос = "",
	...
	Если ТекстПротокола <> Неопределено Тогда
		ТекстПротокола = ТекстПротокола + ?(ПустаяСтрока(ТекстПротокола), "", Символы.ПС) + ИмяМетода + " "
			+ АдресФайлаHRef + Символы.ПС + Символы.ПС + XMLЗапрос + Символы.ПС;
	КонецЕсли;
...
Процедура ВызватьМетодHTTP(СтруктураОбмена, Соединение, ИмяМетода, HTTPЗапросWebdav, ТекущаяПопытка = 1)
	...
	Если СтруктураОбмена.Ответ.КодСостояния = 429 И ТекущаяПопытка <= 5 Тогда
...
Функция ПолучитьUID1C(АдресФайлаHRef, ПараметрыСинхронизации)
	...
	Результат = ВыполнитьМетодWebdav("PROPFIND", АдресФайлаHRef, ЗаголовкиHTTP, ПараметрыСинхронизации, "<?xml version=""1.0"" encoding=""utf-8""?>
	...
	Если Результат.Успешно Тогда
		...
		НайденныеEtag = ВычислитьXPath(
			"//*[local-name()='propstat'][contains(./*[local-name()='status'],'200 OK')]/*[local-name()='prop']/*[local-name()='UID1C']",
...
Функция ПолучитьEtag(АдресФайлаHRef, ПараметрыСинхронизации)
	...
	Если Результат.Успешно Тогда
		...
		НайденныеEtag = ВычислитьXPath(
			"//*[local-name()='propstat'][contains(./*[local-name()='status'],'200 OK')]/*[local-name()='prop']/*[local-name()='getetag']",
			КонтекстXml).ПолучитьСледующий();
...
Функция СоздатьHTTPСоединениеWebdav(СтруктураHref, ПараметрыСинхронизации, Таймаут)
	...
	Если Не ЗначениеЗаполнено(СтруктураHref.Порт) Тогда
		Результат = Новый HTTPСоединение(СтруктураHref.Хост, , ПараметрыСинхронизации.Логин,
			ПараметрыСинхронизации.Пароль, ИнтернетПрокси, Таймаут, ЗащищенноеСоединение);
	Иначе
		Результат = Новый HTTPСоединение(СтруктураHref.Хост, СтруктураHref.Порт, ПараметрыСинхронизации.Логин,
			ПараметрыСинхронизации.Пароль, ИнтернетПрокси, Таймаут, ЗащищенноеСоединение);
	КонецЕсли;
...
Функция ВызватьМетодGET(АдресФайлаHRef, ИдентификаторEtag, ПараметрыСинхронизации, ДатаМодификацииФайла = Неопределено,
	ДлинаФайла = Неопределено)
...
Функция ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	РезультатЗагрузки = ВызватьМетодGET(АдресФайла, ИдентификаторEtag, ПараметрыСинхронизации, ДатаМодификацииФайла,
		ДлинаФайла);
	...
	Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
		...
		Если СсылкаСуществующегоФайла = Неопределено Тогда
			...
			ПараметрыДобавляемогоФайла.РасширениеБезТочки = ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(
		...
		Иначе
			...
			РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияФайла(СсылкаСуществующегоФайла, СведенияОФайле,
				Истина, "", "", Ложь);
		...
		ЗапомнитьСерверныеДанныеСсылки(НовыйФайл.Ссылка, АдресФайла, ИдентификаторEtag, ЭтоФайл, ОбъектВладелец, Ложь,
	...
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
		ИмяФайла), ПараметрыСинхронизации.УчетнаяЗапись, СтатусДляЖурналаРегистрации);

	Возврат НовыйФайл;

КонецФункции

Процедура ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСообщения, УчетнаяЗапись,
	УстанавливаемыйУровеньЖурналаРегистрации = Неопределено)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииСинхронизация(), УстанавливаемыйУровеньЖурналаРегистрации, ,
		УчетнаяЗапись, ТекстСообщения);
...
Процедура ПрочитатьПараметрыКаталога(РезультатПроверки, HttpАдрес, СтруктураОбмена)
	...
	Попытка
		...
		Результат = ВыполнитьМетодWebdav("PROPFIND", АдресСервера, ЗаголовкиHTTP, СтруктураОбмена, "<?xml version=""1.0"" encoding=""utf-8""?>
																								   |<D:propfind xmlns:D=""DAV:"" xmlns:U=""tsov.pro""><D:prop>
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			НайденныйPropstat = ВычислитьXPath(
				"./*[local-name()='propstat'][contains(./*[local-name()='status'],'200 OK')]/*[local-name()='prop']",
			...
			Если НайденныйPropstat <> Неопределено Тогда
				Для Каждого ДочернийУзелPropstat Из НайденныйPropstat.ДочерниеУзлы Цикл
					Если ДочернийУзелPropstat.ЛокальноеИмя = "quota-available-bytes" Тогда
						...
						РезультатПроверки.РезультатТекст = РезультатПроверки.РезультатТекст + ?(ПустаяСтрока(
							РезультатПроверки.РезультатТекст), "", Символы.ПС)
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИнформацияОСвободномМесте,
							РазмерВМегабайтах);
					ИначеЕсли ДочернийУзелPropstat.ЛокальноеИмя = "quota-used-bytes" Тогда
						...
						РезультатПроверки.РезультатТекст = РезультатПроверки.РезультатТекст + ?(ПустаяСтрока(
							РезультатПроверки.РезультатТекст), "", Символы.ПС)
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИнформацияОЗанятомМесте,
							РазмерВМегабайтах);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Исключение
		...
		РезультатПроверки.РезультатТекст = РезультатПроверки.РезультатТекст + ?(ПустаяСтрока(
			РезультатПроверки.РезультатТекст), "", Символы.ПС) + ОписаниеОшибки;
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(ОписаниеОшибки, СтруктураОбмена.УчетнаяЗапись,
			УровеньЖурналаРегистрации.Ошибка);
		РезультатПроверки.Отказ = Истина;
	КонецПопытки;
...
Функция СтруктураURIРаскодированная(Знач СтрокаURI)
	...
	Результат.Вставить("ПутьНаСервере", РаскодироватьСтроку(ЗакончитьБезСлеша(ПутьНаСервере),
...
Функция КодироватьURIПоСтруктуре(Знач СтруктураURI, ВключаяПутьНаСервере = Истина)
	...
	Если ЗначениеЗаполнено(СтруктураURI.Порт) Тогда
		Результат = Результат + ":" + ?(ТипЗнч(СтруктураURI.Порт) = Тип("Число"), Формат(СтруктураURI.Порт, "ЧГ=0"),
			СтруктураURI.Порт);
	КонецЕсли;
...
Процедура ЗапомнитьСерверныеДанныеСсылки(Ссылка, АдресФайлаHRef, ИдентификаторEtag, ЭтоФайл, ВладелецФайла, ЭтоПапка,
...
Функция ВычислитьXPath(Выражение, Контекст, УзелКонтекста = Неопределено)
	...
	Возврат Контекст.ДокументDOM.ВычислитьВыражениеXPath(Выражение, ?(УзелКонтекста = Неопределено,
		Контекст.ДокументDOM, УзелКонтекста), Контекст.РазыменовательDOM);
...
Процедура СкачатьДеревоФайловРекурсивно(ТекущиеСтрокиДереваФайлов, HttpАдрес, ПараметрыСинхронизации, Отказ = Ложь)
	...
	Попытка
		...
		Результат = ВыполнитьМетодWebdav("PROPFIND", АдресСервера, ЗаголовкиHTTP, ПараметрыСинхронизации, "<?xml version=""1.0"" encoding=""utf-8""?>
																										  |<D:propfind xmlns:D=""DAV:"" xmlns:U=""tsov.pro""><D:prop>
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			НайденныйHref = ВычислитьXPath("./*[local-name()='href']", КонтекстДокументаXML,
				НайденныйResponse).ПолучитьСледующий();
			...
			Если НайденныйPropstat <> Неопределено Тогда
				Для Каждого ДочернийУзелPropstat Из НайденныйPropstat.ДочерниеУзлы Цикл
					Если ДочернийУзелPropstat.ЛокальноеИмя = "resourcetype" Тогда
						НоваяСтрокаДереваФайлов.ЭтоПапка = ВычислитьXPath("./*[local-name()='collection']",
							КонтекстДокументаXML, ДочернийУзелPropstat).ПолучитьСледующий() <> Неопределено;
					...
					ИначеЕсли ДочернийУзелPropstat.ЛокальноеИмя = "getlastmodified" Тогда
						НоваяСтрокаДереваФайлов.ДатаМодификации = ОбщегоНазначенияКлиентСервер.ДатаRFC1123(
							ДочернийУзелPropstat.ТекстовоеСодержимое);//UTC
	...
	Для Каждого СтрокаДереваФайлов Из ТекущиеСтрокиДереваФайлов Цикл
		Если СтрокаДереваФайлов.ЭтоПапка = Истина Тогда
			СкачатьДеревоФайловРекурсивно(СтрокаДереваФайлов.Строки, СтрокаДереваФайлов.Href, ПараметрыСинхронизации,
				Отказ);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗагрузитьНовыеПрисоединенныеФайлы(СтрокиДереваФайлов, ТаблицаФайлов, ПараметрыСинхронизации,
	...
	Для Каждого СтрокаДереваФайлов Из СтрокиДереваФайлов Цикл
		...
		Если СтрокаДереваФайлов.ЭтоПапка Тогда
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href")
				= Неопределено) Тогда
				...
				Если ОбъектВладелец = Неопределено Или ТипЗнч(ОбъектВладелец) = Тип(
				...
				Если Не ПроверитьВозможностьUID1C(СтрокаДереваФайлов.Href, Строка(Новый УникальныйИдентификатор),
					ПараметрыСинхронизации) Тогда
					ТекстСобытия = НСтр(
						"ru = 'Невозможно сохранение дополнительных свойств файла, он не будет загружен: %1'");
					ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ТекстСобытия, СтрокаДереваФайлов.ИмяФайла), ПараметрыСинхронизации.УчетнаяЗапись);
					Продолжить;
				КонецЕсли;

				Попытка

					ТекущаяПапкаФайлов = РаботаСФайламиСлужебныйВызовСервера.СоздатьПапкуФайлов(
						СтрокаДереваФайлов.ИмяФайла, ОбъектВладелец, ПараметрыСинхронизации.АвторФайлов);
					...
					ЗапомнитьСерверныеДанныеСсылки(
						НоваяСтрокаТаблицыФайлов.ФайлСсылка, НоваяСтрокаТаблицыФайлов.ToHref,
						НоваяСтрокаТаблицыФайлов.ToEtag, НоваяСтрокаТаблицыФайлов.ЭтоФайл,
						НоваяСтрокаТаблицыФайлов.Родитель, НоваяСтрокаТаблицыФайлов.ЭтоПапка,
						ПараметрыСинхронизации.УчетнаяЗапись);
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(ТекущаяПапкаФайлов) Тогда
					...
					Если СтараяСтрокаТаблицыФайлов = Неопределено Тогда
						...
						ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							ТекстСобытия, РаскодироватьСтроку(СтрокаДереваФайлов.Href,
							СпособКодированияСтроки.URLВКодировкеURL)), ПараметрыСинхронизации.УчетнаяЗапись,
							УровеньЖурналаРегистрации.Ошибка);
					...
					Если Не ЗначениеЗаполнено(ТекущаяПапкаФайлов) Тогда
						...
						ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							ТекстСобытия, РаскодироватьСтроку(СтрокаДереваФайлов.Href,
							СпособКодированияСтроки.URLВКодировкеURL), СтараяСтрокаТаблицыФайлов.UID1C),
							ПараметрыСинхронизации.УчетнаяЗапись, УровеньЖурналаРегистрации.Ошибка);
				...
				Если СтараяСтрокаТаблицыФайлов <> Неопределено Тогда
					...
					СтараяСтрокаТаблицыФайлов.ИзмененНаСервере   = Не ЭтоОдинаковыеПутиURI(
						СтараяСтрокаТаблицыФайлов.ToHref, СтараяСтрокаТаблицыФайлов.Href);
				КонецЕсли;
			КонецЕсли;
			
			// Для подчиненных строк это теперь родитель.
			ЗагрузитьНовыеПрисоединенныеФайлы(СтрокаДереваФайлов.Строки, ТаблицаФайлов, ПараметрыСинхронизации,
		...
		Иначе 
			...
			Если (ТекущийФайл = Неопределено) Или (ТаблицаФайлов.Найти(ТекущийФайл.ФайлСсылка, "ФайлСсылка")
				= Неопределено) Тогда
	...
	Для Каждого СтрокаДереваФайлов Из СтрокиДереваФайлов Цикл
		...
		Если СтрокаДереваФайлов.ЭтоПапка = Истина Тогда //папки
			...
			Если Не ПустаяСтрока(СтрокаДереваФайлов.UID1C) Тогда
				...
				Запрос.УстановитьПараметр("УникальныйИдентификатор1С",
					Новый УникальныйИдентификатор(СтрокаДереваФайлов.UID1C));
			...
			Если ТекущаяПапкаФайлов = Неопределено Или ТаблицаФайлов.Найти(ТекущаяПапкаФайлов.Ссылка, "ФайлСсылка")
				= Неопределено Тогда
				Продолжить;
			КонецЕсли;
		...
		Иначе 
			...
			Если (ТекущийФайл <> Неопределено) И (ТаблицаФайлов.Найти(ТекущийФайл.ФайлСсылка, "ФайлСсылка")
				<> Неопределено) Тогда
				// Обновим ToHref
				СтараяСтрокаТаблицыФайлов = ТаблицаФайлов.Найти(ТекущийФайл.ФайлСсылка, "ФайлСсылка");
				СтараяСтрокаТаблицыФайлов.ToHref = СтрокаДереваФайлов.Href;
...
Процедура ВыполнитьСинхронизациюФайловОблачнымСервисом(ПараметрыСинхронизации)
	...
	Если ТаблицаФайлов = Неопределено Тогда
		ТекстСобытия = НСтр(
			"ru = 'Не удалось получить таблицу файлов из информационной базы, синхронизация не выполнена.'");
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись,
			УровеньЖурналаРегистрации.Ошибка);
		Возврат;
	КонецЕсли;
...
Функция СинхронизироватьФайлы(ТаблицаФайлов, ПараметрыСинхронизации, АдресСервера, ЭтоУдалениеФайлов = Ложь)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		СозданНовыйВБазе            = (Не ЗначениеЗаполнено(СтрокаТаблицы.Href)) И (Не ЗначениеЗаполнено(
			СтрокаТаблицы.ToHref));

		ИзмененВБазе                = ЗначениеЗаполнено(СтрокаТаблицы.Изменения); // что-то поменялось
		ИзмененоСодержимоеНаСервере = ЗначениеЗаполнено(СтрокаТаблицы.ToEtag) И (СтрокаТаблицы.Etag
			<> СтрокаТаблицы.ToEtag); // поменялось содержимое
		...
		Попытка
			Если ЭтоСсылкаНаФайл(СтрокаТаблицы.ФайлСсылка) Тогда
				БлокировкаДанных = Новый БлокировкаДанных;
				ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Метаданные.НайтиПоТипу(ТипЗнч(
					СтрокаТаблицы.ФайлСсылка)).ПолноеИмя());
			...
			Если ЭтоУдалениеФайлов Тогда
				Если УдаленНаСервере И Не УдаленВБазе Тогда
					ОбновитьСтатусСинхронизацииФайла = УдалитьФайлВОблачномСервисе(ПараметрыСинхронизации,
						СтрокаТаблицы);
				КонецЕсли;
			Иначе
				...
				Если СозданНовыйВБазе И Не УдаленВБазе Тогда
					...
					ОбновитьСтатусСинхронизацииФайла = СоздатьФайлВОблачномСервисе(АдресСервера,
						ПараметрыСинхронизации, СтрокаТаблицы, ТаблицаФайлов);
				...
				ИначеЕсли (ИзмененВБазе Или ИзмененНаСервере) И Не (УдаленВБазе Или УдаленНаСервере) Тогда
					...
					Если ИзмененНаСервере И Не ИзмененВБазе Тогда
						ОбновитьСтатусСинхронизацииФайла = ИзменитьФайлВОблачномСервисе(ИзмененоСодержимоеНаСервере,
							ОбновитьСтатусСинхронизацииФайла, ПараметрыСинхронизации, СтрокаТаблицы);
					КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Не удалось синхронизировать файл ""%1"" по причине:'"), Строка(СтрокаТаблицы.ФайлСсылка))
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстОшибки, ПараметрыСинхронизации.УчетнаяЗапись,
				УровеньЖурналаРегистрации.Ошибка);
		КонецПопытки;
...
Функция ИзменитьФайлВОблачномСервисе(Знач ИзмененоСодержимоеНаСервере, ОбновитьСтатусСинхронизацииФайла,
	Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия,
		СтрокаТаблицы.Наименование), ПараметрыСинхронизации.УчетнаяЗапись);
...
Функция СоздатьФайлВОблачномСервисе(Знач АдресСервера, Знач ПараметрыСинхронизации, Знач СтрокаТаблицы,
	Знач ТаблицаФайлов)
	
	// отправим новый на сервер
	СтрокаТаблицы.Наименование = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
		СтрокаТаблицы.Наименование, "-");
	СтрокаТаблицы.ToHref       = ЗакончитьБезСлеша(АдресСервера) + НачатьСлешем(ЗакончитьБезСлеша(РассчитатьHref(
		СтрокаТаблицы, ТаблицаФайлов)));
	...
	ИначеЕсли СтрокаТаблицы.ЭтоПапка Тогда
		ВызватьМетодMKCOL(СтрокаТаблицы.ToHref, ПараметрыСинхронизации);
	Иначе
		СтрокаТаблицы.ToEtag = ВызватьМетодPUT(СтрокаТаблицы.ToHref, СтрокаТаблицы.ФайлСсылка, ПараметрыСинхронизации,
	...
	Если Не СтрокаТаблицы.ЭтоФайл И Не СтрокаТаблицы.ЭтоПапка И Не ОбъектЯвляетсяГруппой Тогда
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.ФайлСсылка, "Редактирует")
			<> ПараметрыСинхронизации.АвторФайлов Тогда
...
Функция ВыбратьДанныеПоПравилам(УчетнаяЗапись, Синхронизировать = Истина)
	...
	Для Каждого Настройка Из НастройкиСинхронизации Цикл
		...
		Для Каждого СтрокаФайлов Из ДеревоФайлов.Строки Цикл
			...
			Если ЗначениеЗаполнено(СтрокаФайлов.ФайлСсылка) Тогда
				...
				Если Метаданные.Справочники.Содержит(МетаданныеФайла) И МетаданныеФайла.Иерархический Тогда
	...
	Для Каждого Строка Из ТаблицаФайлов Цикл
		...
		Если (Синхронизировать И Строка.Синхронизировать > 0) Или (Не Синхронизировать И Строка.Синхронизировать = 0) Тогда
		...
		Если Не Синхронизировать И НесинхронизируемыеВладельцы.Найти(Строка.Родитель) = Неопределено Тогда
...
Процедура УдалитьНесинхронизируемыеФайлыУчетнойЗаписи(УчетнаяЗапись = Неопределено)
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			ТекстОшибки = НСтр(
				"ru = 'При загрузке структуры файлов с сервера произошла ошибка, синхронизация не выполнена.'");
				...
				Попытка
					...
					Если ЗначениеЗаполнено(СтрокаТаблицы.Href) Тогда
						...
						ЗаписатьВЖурналСобытийСинхронизацииФайлов(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия,
							СтрокаТаблицы.НаименованиеСервер), ПараметрыСинхронизации.УчетнаяЗапись);
					КонецЕсли;
...
Функция ТекстЗапросаДляФайловСНеизвлеченнымТекстом(ИмяСправочника, ЧислоФайловВВыборке, ПолучитьВсеФайлы,
	ДополнительныеПоля)
	...
	Попытка
		...
		Если ФайлОбъект <> Неопределено Тогда
			...
			Если ПустаяСтрока(ИмяФайлаСДвоичнымиДанными) Тогда
				...
				ФайлЗаблокирован = Ложь;
			Иначе
				РезультатИзвлеченияТекста = ИзвлечьТекстИзФайлаНаДиске(ИмяФайлаСДвоичнымиДанными,
					ФайлБезТекста.Кодировка);
				ФайлОбъект.СтатусИзвлеченияТекста = РезультатИзвлеченияТекста.СтатусИзвлеченияТекста;
				ФайлОбъект.ТекстХранилище = РезультатИзвлеченияТекста.ТекстХранилище;
...
Процедура ВерсииФайловПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ПереименованиеФайла") Или Источник.ДополнительныеСвойства.Свойство(
		"КонвертацияФайлов") Тогда
	...
	Если Не ТекущаяВерсия.Ссылка.Пустая() Тогда
		...
		Если РеквизитыФайла.Размер <> ТекущаяВерсия.Размер Или РеквизитыФайла.ДатаСоздания <> ТекущаяВерсия.ДатаСоздания
			Или РеквизитыФайла.Расширение <> ТекущаяВерсия.Расширение Или РеквизитыФайла.Том <> ТекущаяВерсия.Том
			Или РеквизитыФайла.ПутьКФайлу <> ТекущаяВерсия.ПутьКФайлу Или РеквизитыФайла.ИндексКартинки
			<> ТекущаяВерсия.ИндексКартинки Или РеквизитыФайла.ДатаМодификацииУниверсальная
			<> ТекущаяВерсия.ДатаМодификацииФайла Тогда
			НачатьТранзакцию();
			Попытка
				БлокировкаДанных = Новый БлокировкаДанных;
				ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Метаданные.НайтиПоТипу(ТипЗнч(
					ФайлСсылка)).ПолноеИмя());
				ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", ФайлСсылка);
				БлокировкаДанных.Заблокировать();
				ФайлОбъект = ФайлСсылка.ПолучитьОбъект();
...
Процедура ПередУдалениемПрисоединенногоФайлаСервер(Знач Ссылка, Знач ВладелецФайлов, Знач Том, Знач ТипХраненияФайла,
...
Процедура ОбработкаПроверкиЗаполненияПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ОтложеннаяЗапись") И РаботаСФайлами.ДвоичныеДанныеФайла(
		Источник.Ссылка, Ложь) = Неопределено Тогда
...
Процедура ЗарегистрироватьОбъектыДляПереносаЭлектронныхПодписейИСертификатовШифрования(Параметры) Экспорт
	...
	Для Каждого ПолноеИмя Из ПолныеИменаСправочников Цикл
		...
		ЕстьТЧУдалитьСертификатыШифрования = ОбъектМетаданных.ТабличныеЧасти.Найти("УдалитьСертификатыШифрования")
			<> Неопределено;

		ЕстьТЧУдалитьЭлектронныеПодписи = ОбъектМетаданных.ТабличныеЧасти.Найти("УдалитьЭлектронныеПодписи")
			<> Неопределено;
		...
		ТекущийТекстЗапроса = СтрЗаменить(ТекущийТекстЗапроса, "ТабличнаяЧастьУдалитьСертификатыШифрования", ПолноеИмя
			+ ".УдалитьСертификатыШифрования");

		ТекущийТекстЗапроса = СтрЗаменить(ТекущийТекстЗапроса, "ТабличнаяЧастьУдалитьЭлектронныеПодписи", ПолноеИмя
...
Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений(Параметры) Экспорт
	...
	Для Каждого ПолноеИмяСправочника Из ПолныеИменаСправочников Цикл
		ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведенийДляТаблицы(Параметры, ПолноеИмяСправочника,
			ОбработкаЗавершена);
	КонецЦикла;
...
Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведенийДляТаблицы(ПараметрыОбновления,
	...
	ВыборкаСсылок = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(ПараметрыОбновления.Очередь,
	...
	Попытка
		...
		Если ЕстьТабличнаяЧастьЭП Тогда
			ПеренестиДанныеЭлектроннойПодписиВРегистрСведений(МассивСсылок, ПолноеИмяОбъектаМетаданных,
				ОбъектМетаданных);
		КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать объект %1 по причине:
				 |%2'"), ОбъектМетаданных, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Предупреждение, ОбъектМетаданных, , ТекстСообщения);
	КонецПопытки;

	Если Не ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(ПараметрыОбновления.Очередь,
		ПолноеИмяОбъектаМетаданных) Тогда
		ОбработкаЗавершена = Ложь;
	КонецЕсли;

	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать некоторые объекты (пропущены): %1'"), ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация, ОбъектМетаданных, , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработана очередная порция объектов: %1'"), ОбъектовОбработано));
	КонецЕсли;
...
Процедура ПеренестиДанныеЭлектроннойПодписиВРегистрСведений(МассивОбъектов, ПолноеИмяОбъектаМетаданных, ОбъектМетаданных)
	...
	Попытка
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УдалитьЭлектронныеПодписи", ПолноеИмяОбъектаМетаданных
			+ ".УдалитьЭлектронныеПодписи");
		Если ОбъектМетаданных = Метаданные.Справочники.ВерсииФайлов Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТЧЭлектронныеПодписи.Ссылка КАК ПодписанныйОбъект",
...
Процедура ПеренестиДанныеСертификатовВРегистрСведений(МассивОбъектов, ПолноеИмяОбъектаМетаданных)
	...
	Попытка
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УдалитьСертификатыШифрования", ПолноеИмяОбъектаМетаданных
			+ ".УдалитьСертификатыШифрования");
		Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
