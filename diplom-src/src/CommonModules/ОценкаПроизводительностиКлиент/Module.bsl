...
//
Функция НачатьЗамерДлительнойОперации(КлючеваяОперация, ФиксироватьСОшибкой = Ложь, АвтоЗавершение = Ложь,
	ИмяПоследнегоШага = "ПоследнийШаг") Экспорт
		...
		Иначе
			...
			Если ПараметрыПриложенияСтандартныеПодсистемы.Свойство("ОценкаПроизводительности") Тогда
				...
				ПараметрыПриложения[ИмяПараметра].Вставить("СмещениеДатыКлиента",
					ПараметрыПриложенияСтандартныеПодсистемы["СмещениеДатыКлиента"]);
			Иначе
				ПараметрыОценкиПроизводительности = ОценкаПроизводительностиВызовСервера.ПолучитьПараметрыНаСервере();
				ТекущийПериодЗаписи = ПараметрыОценкиПроизводительности.ПериодЗаписи;
				ДатаИВремяНаСервере = ПараметрыОценкиПроизводительности.ДатаИВремяНаСервере;
				...
				// Получаем дату в UTC
				ДатаИВремяНаКлиенте = ТекущаяУниверсальнаяДатаВМиллисекундах();
				ПараметрыПриложения[ИмяПараметра].Вставить("СмещениеДатыКлиента", ДатаИВремяНаСервере
					- ДатаИВремяНаКлиенте);
			КонецЕсли;
		КонецЕсли;
...
#КонецЕсли
	...
	Если АвтоЗавершение Тогда
		Если Не ПараметрыПриложения[ИмяПараметра]["ЕстьОбработчик"] Тогда
			...
			ПараметрыПриложения[ИмяПараметра]["ВремяПодключенияОбработчика"] = ТекущаяУниверсальнаяДатаВМиллисекундах()
				+ ПараметрыПриложения[ИмяПараметра]["СмещениеДатыКлиента"];
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗавершитьЗамерВремениНаКлиентеАвто() Экспорт
	...
	Если ОценкаПроизводительностиЗамерВремени <> Неопределено Тогда
		...
		Для Каждого Замер Из Замеры Цикл
			...
			Если ЗамерЗначение["АвтоЗавершение"] Тогда
				Если ЗамерЗначение["ВремяНачала"] <= ВремяПодключенияОбработчика И ЗамерЗначение["ВремяОкончания"]
					= Неопределено Тогда
	...
	Иначе
		...
		ОценкаПроизводительностиЗамерВремени["ВремяПодключенияОбработчика"] = ТекущаяУниверсальнаяДатаВМиллисекундах()
			+ ОценкаПроизводительностиЗамерВремени["СмещениеДатыКлиента"];
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьРезультатыАвтоНеГлобальный(ПередЗавершением = Ложь) Экспорт
	...
	Если ОценкаПроизводительностиЗамерВремени <> Неопределено Тогда
		...
		НовыйПериодЗаписи = ОценкаПроизводительностиВызовСервера.ЗафиксироватьДлительностьКлючевыхОпераций(
			ЗамерыДляЗаписи);
