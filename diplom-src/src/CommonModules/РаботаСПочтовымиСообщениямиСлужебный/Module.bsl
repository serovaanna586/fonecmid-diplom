...
Функция ИнтернетПочтовыйПрофиль(УчетнаяЗапись, ДляПолучения = Ложь) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.АвторизацияСПомощьюПочтовогоСервиса Тогда
			Если ЗначениеЗаполнено(Пароли.СрокДействияТокенаДоступа) И Пароли.СрокДействияТокенаДоступа - 60
...
Функция УчетнаяЗаписьЭтойБазыПоДаннымУчетнойЗаписиКорреспондента(УчетнаяЗаписьКорреспондента) Экспорт
	...
	Попытка
		...
		Если УчетнаяЗаписьЭтойБазы <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(УчетнаяЗаписьЭтойБазы, УчетнаяЗаписьКорреспондента, ,
				"ИмяПредопределенныхДанных, Родитель, Владелец, Ссылка");
		Иначе
...
Процедура ОбновитьНастройкиСервераВосстановленияПочты(Ссылка) Экспорт
	...
	Если Не НастройкиУчетнойЗаписи.Используется Или НастройкиУчетнойЗаписи.УчетнаяЗаписьПочты <> Ссылка Тогда
		Возврат;
	КонецЕсли;
	...
	ВнешнийПочтовыйСервер = ПользователиСлужебный.СпособВосстановленияПароляПользователяИнформационнойБазы(
...
Процедура ПриОпределенииОбъектовСРедактируемымиРеквизитами(Объекты) Экспорт
	Объекты.Вставить(Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты.ПолноеИмя(),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия") Тогда
		...
		МодульВзаимодействия.ПриПолученииЧитаемыхОбъектовОбработчикаОбновленияУчетныхЗаписейЭлектроннойПочты(
			ЧитаемыеОбъекты);
		МодульВзаимодействия.ПриПолученииИзменяемыхОбъектовОбработчикаОбновленияУчетныхЗаписейЭлектроннойПочты(
			ИзменяемыеОбъекты);
	КонецЕсли;
...
Процедура ОтправитьСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки) Экспорт
	...
	РеквизитыОтправителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись,
		"ИмяПользователя,АдресЭлектроннойПочты,ОтправлятьСкрытыеКопииПисемНаЭтотАдрес");
...
Процедура ОпределитьПапкуОтправленные(Соединение)
	...
	Для Каждого ПочтовыйЯщик Из ПочтовыеЯщики Цикл
		Если НРег(ПочтовыйЯщик) = "отправленные" Или НРег(ПочтовыйЯщик) = "inbox.sent" Или НРег(ПочтовыйЯщик) = "sent" Тогда
...
Функция ИспользоватьIMAPПриОтправкеПисем(Профиль)
	...
	Возврат Не ПочтовыйСерверХранитПисьмаОтправленныеПоSMTP(Профиль) И ЗначениеЗаполнено(Профиль.АдресСервераIMAP)
		И ЗначениеЗаполнено(Профиль.ПортIMAP) И ЗначениеЗаполнено(Профиль.ПользовательIMAP) И Профиль.ПарольIMAP <> "";
...
Функция ЗагрузитьСообщения(Знач УчетнаяЗапись, Знач ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли Не РежимТестирования Тогда
			НастройкиТранспорта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись,
				"ПротоколВходящейПочты,ОставлятьКопииСообщенийНаСервере,ПериодХраненияСообщенийНаСервере");
			...
			Если НастройкиТранспорта.ОставлятьКопииСообщенийНаСервере Тогда
				Если ЗаголовкиИдентификаторы.Количество() = 0 И НастройкиТранспорта.ПериодХраненияСообщенийНаСервере
					> 0 Тогда
		...
		Для Каждого НаименованиеКолонки Из Колонки Цикл
			...
			ИначеЕсли ТипЗнч(ПолеПисьма) = Тип("ИнтернетТекстыПочтовогоСообщения") Тогда
				...
				Для Каждого ОчереднойТекст Из ПолеПисьма Цикл
					...
					ОписаниеТекста.Вставить("Текст", ОбщегоНазначенияКлиентСервер.УдалитьНедопустимыеСимволыXML(
						ОчереднойТекст.Текст));
					ОписаниеТекста.Вставить("ТипТекста", Строка(ОчереднойТекст.ТипТекста));
					Тексты.Добавить(ОписаниеТекста);
				КонецЦикла;
				ПолеПисьма = Тексты;
			ИначеЕсли ТипЗнч(ПолеПисьма) = Тип("ВажностьИнтернетПочтовогоСообщения") Или ТипЗнч(ПолеПисьма) = Тип(
				"СпособКодированияНеASCIIСимволовИнтернетПочтовогоСообщения") Тогда
...
Процедура ЗаполнитьВложенияПоПисьму(Вложения, Письмо)
	...
	Для Каждого Текст Из Письмо.Тексты Цикл
		...
		Пока ИмяТекстаВложения = "" Или Вложения.Получить(ИмяТекстаВложения) <> Неопределено Цикл
			Индекс = Индекс + 1;
			ИмяТекстаВложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 - (%2).%3",
				ПредставлениеПисьма, Индекс, Расширение);
		КонецЦикла;
...
//
Процедура ПроверитьВозможностьОтправкиИПолученияЭлектроннойПочты(УчетнаяЗапись, СообщениеОбОшибке,
	ДополнительноеСообщение) Экспорт
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляОтправки Тогда
		ТекстОшибки = Справочники.УчетныеЗаписиЭлектроннойПочты.ПроверитьВозможностьПодключенияКПочтовомуСерверу(
			УчетнаяЗапись, Ложь);
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Не удалось подключиться к серверу SMTP: %1'") + Символы.ПС, ТекстОшибки);
		КонецЕсли;
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляПолучения Или НастройкиУчетнойЗаписи.ИспользоватьДляОтправки
		И НастройкиУчетнойЗаписи.ПротоколВходящейПочты = "IMAP" Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			СообщениеОбОшибке = СообщениеОбОшибке + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось подключиться к серверу %1:
																												 |%2'"),
				НастройкиУчетнойЗаписи.ПротоколВходящейПочты, ТекстОшибки);
		КонецЕсли;
...
Процедура ПриПолученииДанных(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель)
	...
	Если ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.УчетныеЗаписиЭлектроннойПочты") Тогда
		...
		Иначе
			ЭлементДанных.ИспользоватьДляПолучения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементДанных.Ссылка,
				"ИспользоватьДляПолучения");
			ЭлементДанных.ИспользоватьДляОтправки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементДанных.Ссылка,
	...
	Для Индекс = -Вложения.ВГраница() По 0 Цикл
		...
		Для Каждого ВыбранныйФормат Из ВыбранныеФорматыСохранения Цикл
			...
			ПолноеИмяФайла = ФайловаяСистема.УникальноеИмяФайла(
				ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ИмяВременногоКаталога) + ИмяФайла);
...
Функция ПредставлениеПисьма(ТемаПисьма, ДатаПисьма)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления, ?(ПустаяСтрока(ТемаПисьма),
		НСтр("ru = '<Без темы>'"), ТемаПисьма), Формат(ДатаПисьма, "ДЛФ=D"));
...
Функция ОписанияВложений(КоллекцияВложений) Экспорт
	...
	Для Каждого Вложение Из КоллекцияВложений Цикл
		...
		КонецЕсли;
		ОписаниеВложения.АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные,
...
Функция ПочтовыйСерверХранитПисьмаОтправленныеПоSMTP(ИнтернетПочтовыйПрофиль)
	...
	Возврат НРег(ИнтернетПочтовыйПрофиль.АдресСервераSMTP) = "smtp.gmail.com" Или СтрЗаканчиваетсяНа(НРег(
		ИнтернетПочтовыйПрофиль.АдресСервераSMTP), ".outlook.com") > 0;
	...
	Попытка
		Для Каждого Письмо Из Письма Цикл
			...
			Если ОшибочныеПолучатели.Количество() > 0 Тогда
				...
				ЗаписьЖурналаРегистрации(ИмяСобытияОтправкаПочты(), УровеньЖурналаРегистрации.Ошибка, , УчетнаяЗапись,
					ТекстОшибки);
			КонецЕсли;
	...
	Если ТипТекста = Неопределено Тогда
		Если ПараметрыПисьма.Свойство("ТипТекста", ТипТекста) Тогда
			...
			ИначеЕсли ТипЗнч(ТипТекста) = Тип("ПеречислениеСсылка.ТипыТекстовЭлектронныхПисем") Тогда
				Если ТипТекста = Перечисления.ТипыТекстовЭлектронныхПисем.HTML Или ТипТекста
					= Перечисления.ТипыТекстовЭлектронныхПисем.HTMLСКартинками Тогда
					Текст.ТипТекста = ТипТекстаПочтовогоСообщения.HTML;
...
Функция ПолучательПисьмаОтклоненСервером(Знач ТекстОшибки)
	...
	Возврат СтрНайти(ТекстОшибки, "invalid mailbox") > 0 Или СтрНайти(ТекстОшибки, "user not found") > 0;
...
Функция РасширенноеПредставлениеОшибки(ИнформацияОбОшибке, КодЯзыка, ВключитьПодробноеПредставлениеОшибки = Истина) Экспорт
	...
	Если ВключитьПодробноеПредставлениеОшибки Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ТекстОшибки,
	...
	Иначе
		...
		Для Каждого Параметр Из ПараметрыЗапроса Цикл
			...
			Для Каждого Значение Из Значения Цикл
				СписокПараметров.Добавить(Параметр.Ключ + "=" + КодироватьСтроку(Значение,
...
Функция ВыполнитьЗапрос(АдресСервера, АдресРесурса, ПараметрыЗапроса, ПоместитьПараметрыВТелоЗапроса = Истина) Экспорт
	...
	Попытка
		Соединение = Новый HTTPСоединение(АдресСервера, , , , Прокси, 60,
		...
		КонецЕсли;
	Исключение
		ЗаписьЖурналаРегистрации(ИмяСобытияАвторизацияПоПротоколуOAuth(), УровеньЖурналаРегистрации.Ошибка, , ,
	...
	Если HTTPОтвет <> Неопределено Тогда
		Если HTTPОтвет.КодСостояния <> 200 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Запрос ""%1"" не выполнен. Код состояния: %2.'"), АдресРесурса, HTTPОтвет.КодСостояния)
				+ Символы.ПС + HTTPОтвет.ПолучитьТелоКакСтроку();
			ЗаписьЖурналаРегистрации(ИмяСобытияАвторизацияПоПротоколуOAuth(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстОшибки);
		КонецЕсли;
...
Функция ОбновитьТокенДоступа(Знач УчетнаяЗапись, Знач ТокенОбновления)
	...
	Если Не ЗначениеЗаполнено(НастройкиАвторизации.ИдентификаторПриложения) Тогда
		ТекстОшибки = НСтр(
			"ru = 'Настройки авторизации Интернет-сервиса ""%1"" не найдены для домена ""%2"". Необходимо перенастроить почту.'");
		ЗаписьЖурналаРегистрации(ИмяСобытияАвторизацияПоПротоколуOAuth(), УровеньЖурналаРегистрации.Ошибка, ,
			УчетнаяЗапись, ТекстОшибки);
		Возврат "";
	КонецЕсли;
	...
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(УчетнаяЗапись, СрокДействияТокенаДоступа,
		"СрокДействияТокенаДоступа");
