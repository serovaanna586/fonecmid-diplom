...
Функция ДвоичныеДанныеФайла(Знач ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1'"), "ПрисоединенныйФайл"),
		"РаботаСФайлами.ДвоичныеДанныеФайла");
	ОбщегоНазначенияКлиентСервер.Проверить(Не ФайлОбъект.ЭтоГруппа, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 (папка файлов ""%2"")'"), "ПрисоединенныйФайл",
...
Функция ДанныеФайла(Знач ПрисоединенныйФайл, Знач ДополнительныеПараметры = Неопределено,
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		ДляРедактирования = ?(ДополнительныеПараметры.Свойство("ДляРедактирования"),
			ДополнительныеПараметры.ДляРедактирования, Ложь);
		ИдентификаторФормы = ?(ДополнительныеПараметры.Свойство("ИдентификаторФормы"),
			ДополнительныеПараметры.ИдентификаторФормы, Неопределено);
		ВызыватьИсключение = ?(ДополнительныеПараметры.Свойство("ВызыватьИсключение"),
	...
	Если ВызыватьИсключение Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено,
			...
			"ru = 'Не найден присоединенный файл ""%1"" (%2)'"), Строка(ПрисоединенныйФайл),
	...
	ИспользуетсяВерсионированиеФайлов = ПоддерживаетсяХранениеВерсий И ПрисоединенныйФайл.ХранитьВерсии
	...
	Результат.Вставить("ИмяФайла", ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ФайлОбъект.Наименование,
	...
	Если ЕстьВозможностьХранитьВерсии И ЗначениеЗаполнено(ПрисоединенныйФайл.ТекущаяВерсия) Тогда
		РаботаСФайламиСлужебный.ЗаполнитьДополнительныеДанныеФайла(Результат, ПрисоединенныйФайл,
			ПрисоединенныйФайл.ТекущаяВерсия);
	Иначе
	...
	Результат.Вставить("ФайлРедактируетТекущийПользователь", ?(Результат.ФайлРедактируется, ФайлОбъект.Редактирует
		= Пользователи.АвторизованныйПользователь(), Ложь));

	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
		Если ФайлОбъект.Зашифрован Тогда
			Результат.Вставить("МассивСертификатовШифрования", МодульЭлектроннаяПодпись.СертификатыШифрования(
				ПрисоединенныйФайл));
		КонецЕсли;
	КонецЕсли;

	Файл = ?(ИспользуетсяВерсионированиеФайлов, ПрисоединенныйФайл.ТекущаяВерсия, ПрисоединенныйФайл);
	Результат.Вставить("Кодировка", РегистрыСведений.КодировкиФайлов.ОпределитьКодировкуФайла(Файл,
		ФайлОбъект.Расширение));
...
Процедура ЗаполнитьПрисоединенныеФайлыКОбъекту(Знач ВладелецФайла, Знач Файлы) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Или ТипЗнч(ВладелецФайла) = Тип(
...
Функция КОбъектуМожноПрисоединятьФайлы(ВладелецФайлов, ИмяСправочника = "") Экспорт
	...
	Возврат СправочникПрисоединенныеФайлы <> Неопределено И ПравоДоступа("Добавление", СправочникПрисоединенныеФайлы)
		И ТипыХранимыхФайлов.СодержитТип(Тип("СправочникСсылка." + ИмяСправочника)) И Не ТипыХранимыхФайлов.СодержитТип(
...
Функция ДобавитьФайлСДиска(ВладелецФайлов, ПутьКФайлуНаДиске) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Не задано значение параметра %1 в %2.'"), "ВладелецФайлов", "РаботаСФайлами.ДобавитьФайлСДиска");
	КонецЕсли;
...
Функция ДобавитьФайл(ПараметрыФайла, Знач АдресФайлаВоВременномХранилище, Знач АдресВременногоХранилищаТекста = "",
	Знач Описание = "", Знач НоваяСсылкаНаФайл = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не задано значение параметра %1 в %2.'"), "ПараметрыФайла.ВладелецФайлов",
			"РаботаСФайлами.ДобавитьФайл");
	КонецЕсли;

	ДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресФайлаВоВременномХранилище); // ДвоичныеДанные
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("РаботаСФайлами.ДобавитьФайл", "АдресФайлаВоВременномХранилище",
		ДвоичныеДанные, Тип("ДвоичныеДанные"));

	ГруппаФайлов = Неопределено;
	Если ПараметрыФайла.Свойство("ГруппаФайлов") И ЗначениеЗаполнено(ПараметрыФайла.ГруппаФайлов)
	...
	Иначе
		...
		Если Не Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(НоваяСсылкаНаФайл)) Или Не ЗначениеЗаполнено(
			НоваяСсылкаНаФайл) Тогда
	...
	Попытка
		...
		Если ХранитьВерсии Тогда
			...
			РаботаСФайламиСлужебный.ОбновитьВерсиюВФайле(ПрисоединенныйФайл.Ссылка, Версия,
				АдресВременногоХранилищаТекста);
		Иначе
	...
	Исключение
		...
		КомментарийЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения, ИмяБезРасширения + "." + РасширениеБезТочки, ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке));

		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Файлы.Добавление присоединенного файла'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , КомментарийЖурналаРегистрации);

		МенеджерФайла.ПослеОбновленияДанныхФайла(Контекст, Ложь);

		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения, ИмяБезРасширения + "." + РасширениеБезТочки, КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке));
...
Функция МаксимальныйРазмерФайла() Экспорт
	...
	ИмяКонстанты = ?(РазделениеВключеноИДоступноИспользование, "МаксимальныйРазмерФайлаОбластиДанных",
	...
	Для Каждого Настройка Из НастройкиРаботыСФайлами Цикл
		...
		Если КлючОбъектаНастройки <> Неопределено Тогда
			Если СтрНачинаетсяС(КлючОбъектаНастройки, "НастройкиОткрытияФайлов\") Тогда
				...
				ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъектаНастройки, СтрЗаменить(Настройка.Ключ,
					ТипФайловНастройки, ""), Настройка.Значение);
			Иначе
				ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъектаНастройки, Настройка.Ключ,
					Настройка.Значение);
			КонецЕсли;
...
Функция СконвертироватьФайлыВПрисоединенные(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не задано значение параметра %1 в %2.'"), "ВладелецФайлов",
			"РаботаСФайлами.СконвертироватьФайлыВПрисоединенные");
	КонецЕсли;
				...
				Если ЗначениеЗаполнено(ФайлИсточникОбъект.ТекущаяВерсия) Тогда
					...
					Если ТекущаяВерсияОбъект <> Неопределено Тогда
						...
						ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(
							Метаданные.Справочники.ВерсииФайлов.ПолноеИмя());
...
Процедура ПриСозданииНаСервере(Форма, ДобавляемыеЭлементы = Неопределено) Экспорт
	...
	Для Индекс = 1 По КоличествоЭлементов Цикл
		...
		Если ДобавляемыйЭлемент.Свойство("Видимость") И Не ДобавляемыйЭлемент.Видимость Тогда
		...
		Если ДобавляемыйЭлемент.Свойство("ПутьКДанным") Тогда
			...
			Если РеквизитРазмещения <> Неопределено Тогда
				...
				Для ИндексРеквизита = 1 По ПолныйПутьКДанным.ВГраница() Цикл
					...
					РеквизитРазмещения   = РеквизитФормыПоИмени(ПодчиненныеРеквизиты,
						ПолныйПутьКДанным[ИндексРеквизита]);
					Если РеквизитРазмещения = Неопределено Тогда
						Прервать;
					КонецЕсли;
			...
			Если РеквизитРазмещения = Неопределено Тогда
				...
				РеквизитРазмещения = Новый РеквизитФормы("ПолеПрисоединенногоФайла" + НомерЭлемента,
					ТипПрисоединенныеФайлы);
			...
			Если ДобавляемыйЭлемент.Свойство("ПоказыватьПредпросмотр") И ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
	...
	Для Индекс = 0 По ДобавляемыеЭлементы.ВГраница() Цикл
		...
		Для ИндексРеквизита = 1 По ПолныйПутьКДаннымВладельца.ВГраница() Цикл
			...
			ПодчиненныеРеквизиты         = Форма.ПолучитьРеквизиты(ПутьКРеквизиту);
			ВладелецПрисоединенныхФайлов = РеквизитФормыПоИмени(ПодчиненныеРеквизиты,
				ПолныйПутьКДаннымВладельца[ИндексРеквизита]);
			Если ВладелецПрисоединенныхФайлов = Неопределено Тогда
				Прервать;
			КонецЕсли;
		...
		ДоступноДобавление = Не ФайлыРедактируютсяВОблачномСервисе И ПравоДоступа("ИнтерактивноеДобавление",
			МетаданныеСправочникаСФайлами);
		ДоступноИзменение = Не ФайлыРедактируютсяВОблачномСервисе И ПравоДоступа("Изменение",
			МетаданныеСправочникаСФайлами);
...
Процедура ДобавитьПодписьКФайлу(ПрисоединенныйФайл, СвойстваПодписи, ИдентификаторФормы = Неопределено) Экспорт
	...
	Если СтруктураРеквизитов.Зашифрован Тогда
		ВызватьИсключение РаботаСФайламиСлужебныйКлиентСервер.СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(
			ПрисоединенныйФайлСсылка);
	КонецЕсли;
...
Процедура ИзменитьСправочникХраненияФайлов(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не задано значение параметра %1 в %2.'"), "ВладелецФайлов",
			"РаботаСФайлами.ИзменитьСправочникХраненияФайлов");
	КонецЕсли;
	...
	Для Каждого ФайлИсточник Из ФайлыИсточник Цикл
		...
		Попытка
			...
			Если ПрисоединенныйФайл.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
				...
				МенеджерЗаписи.Файл = СсылкаНового;
				МенеджерЗаписи.ДвоичныеДанныеФайла = Новый ХранилищеЗначения(ХранилищеФайла.Получить(),
			...
			Если ТекущаяВерсияОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
				...
				Если ЗначениеЗаполнено(ФайлИсточникОбъект.ТекущаяВерсия) Тогда
					РаботаСФайламиСлужебный.ПометитьНаУдалениеВерсииФайла(ФайлИсточникОбъект.Ссылка,
						ТекущаяВерсияОбъект.Ссылка);
				КонецЕсли;
			КонецЕсли;
...
//
Функция СоздатьПрисоединенныйФайлНаОснованииФайла(ВладелецФайлов, МенеджерПрисоединенныхФайлов, ТекущаяВерсияОбъект,
	ФайлИсточникОбъект)
...
Функция КлючиОбъектовНастроекРаботыСФайлами()
	...
	КлючиОбъектовНастроекРаботыСФайлами.Вставить("СпрашиватьРежимРедактированияПриОткрытииФайла",
	...
	КлючиОбъектовНастроекРаботыСФайлами.Вставить("ТекстовыеФайлыСпособОткрытия",
		"НастройкиОткрытияФайлов\ТекстовыеФайлы");
	КлючиОбъектовНастроекРаботыСФайлами.Вставить("ГрафическиеСхемыРасширение",
		"НастройкиОткрытияФайлов\ГрафическиеСхемы");
	КлючиОбъектовНастроекРаботыСФайлами.Вставить("ГрафическиеСхемыСпособОткрытия",
		"НастройкиОткрытияФайлов\ГрафическиеСхемы");
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при пометке на удаление присоединенных файлов.'") + Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
			...
			Если Источник.ПометкаУдаления И ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '""%1"" не может быть удален,
						 ...
						 |занятый для редактирования.'"), Строка(Источник.Ссылка), Строка(Выборка.Ссылка));
			КонецЕсли;
...
Процедура ВыполнитьДействияПередЗаписьюПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		...
		Если Не РаботаСФайламиСлужебный.ЕстьПраво("ДобавлениеФайлов", Источник.ВладелецФайла) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недостаточно прав для добавления файлов в папку ""%1"".'"), Строка(Источник.ВладелецФайла));
		КонецЕсли;
	Иначе
		...
		Если ИзмененаПометкаУдаления Тогда
			...
			Если Не РаботаСФайламиСлужебный.ЕстьПраво("ПометкаУдаленияФайлов", Источник.ВладелецФайла) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недостаточно прав для пометки файлов на удаление в папке ""%1"".'"), Строка(
		...
		Если ЗаписьПодписанногоОбъекта <> Истина Тогда
			...
			Если Не Источник.ЭтоГруппа И Источник.Зашифрован И СсылкаЗашифрован И Источник.ПодписанЭП
				И Не СсылкаПодписан Тогда
		...
		Если Не Источник.ЭтоГруппа И СправочникПоддерживаетВозможностьХранитьВерсии И ЗначениеЗаполнено(
			Источник.ТекущаяВерсия) Тогда
			...
			Если РеквизитыТекущейВерсии.Наименование <> Источник.Наименование И ЗначениеЗаполнено(
				Источник.ТекущаяВерсия) Тогда
	...
	Если Не ЗначениеЗаполнено(Источник.ВладелецФайла) Тогда
		...
		Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка записи файла при обновлении ИБ'",
				ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , Источник.Ссылка,
				ОписаниеОшибки);
		Иначе
	...
	Иначе
		Источник.ИндексКартинки = РаботаСФайламиСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(
			Источник.Расширение);
	КонецЕсли;
...
Процедура ВыполнитьДействияПередУдалениемПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	РаботаСФайламиСлужебный.ПередУдалениемПрисоединенногоФайлаСервер(
		Источник.Ссылка, Источник.ВладелецФайла, Источник.Том, Источник.ТипХраненияФайла, Источник.ПутьКФайлу);
...
Процедура ПроверитьИзменениеАвтораФайла(Знач ПрежниеЗначения, Знач Источник)
	...
	Если Источник.Редактирует <> ПрежниеЗначения.Редактирует И (НедопустимыйАвтор(Источник.Редактирует,
		ТекущийПользователь) Или НедопустимыйАвтор(ПрежниеЗначения.Редактирует, ТекущийПользователь)) Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для редактирования файла.'");
	КонецЕсли;

	Если Источник.Изменил <> ПрежниеЗначения.Изменил И (НедопустимыйАвтор(Источник.Изменил, ТекущийПользователь)
...
Функция НедопустимыйАвтор(АвторСсылка, ТекущийПользователь)
	...
	Возврат АвторСсылка <> Неопределено И Не АвторСсылка.Пустая() И ТипЗнч(АвторСсылка) <> Тип(
		"СправочникСсылка.УчетныеЗаписиСинхронизацииФайлов") И АвторСсылка <> ТекущийПользователь;
...
Процедура СоздатьГиперссылкуФайлов(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыГиперссылки)
	...
	Если ЭлементРазмещения <> Неопределено И ТипЗнч(ЭлементРазмещения) = Тип("ГруппаФормы") Тогда

		ЭтоГруппаКнопок = ЭлементРазмещения.Вид = ВидГруппыФормы.ГруппаКнопок Или ЭлементРазмещения.Вид
			= ВидГруппыФормы.КоманднаяПанель;
		ВидГруппы = ?(ЭтоГруппаКнопок, ВидГруппыФормы.ГруппаКнопок, ВидГруппы);
		Если ЭлементРазмещения.Вид = ВидГруппыФормы.ОбычнаяГруппа Или ЭлементРазмещения.Вид = ВидГруппыФормы.Страница
	...
	ГруппаРазмещенияНаФорме = Форма.Элементы.Вставить(ИмяГруппы, Тип("ГруппаФормы"), ЭлементРодитель, ЭлементРазмещения);
	ГруппаРазмещенияНаФорме.Вид = ВидГруппы;

	ПодменюДобавления = Неопределено;
	Если ДобавляемыйЭлемент.ДобавлятьФайлы И ДоступноДобавление Тогда
		...
		КнопкаЗагрузки = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиЗагрузитьФайл" + НомерЭлемента,
			ГруппаРазмещенияНаФорме, ЗагрузитьФайл.Имя);
		...
		КнопкаЗагрузкиИзПодменю = ДобавитьКнопкуНаФорму(Форма, "ЗагрузитьФайлИзПодменю" + НомерЭлемента,
			ПодменюДобавления, ЗагрузитьФайл.Имя);
	...
	Если ДобавляемыйЭлемент.ОтображатьЗаголовокСправа Или Не ДобавляемыйЭлемент.ДобавлятьФайлы Тогда
		ГиперссылкаПерехода = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиОткрытьСписок"
			+ НомерЭлемента, ГруппаРазмещенияНаФорме, КомандаОткрытияСписка.Имя);
	Иначе
	...
	Если ДобавляемыйЭлемент.ОтображатьКоличество Тогда
		...
		Владелец = ?(ЗначениеЗаполнено(Форма.Параметры.ЗначениеКопирования), Форма.Параметры.ЗначениеКопирования,
			ВладелецПрисоединенныхФайлов);
		...
		Если КоличествоПрисоединенныхФайлов > 0 Тогда
			ГиперссылкаПерехода.Заголовок = ГиперссылкаПерехода.Заголовок + " (" + Формат(
				КоличествоПрисоединенныхФайлов, "ЧГ=") + ")";
		КонецЕсли;
...
Процедура СоздатьПолеФайла(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыПоляФайла)
	...
	Если ЭлементРазмещения <> Неопределено И ТипЗнч(ЭлементРазмещения) = Тип("ГруппаФормы") И (ЭлементРазмещения.Вид
		= ВидГруппыФормы.ОбычнаяГруппа Или ЭлементРазмещения.Вид = ВидГруппыФормы.Страница) Тогда
	...
	ГруппаШапка = Форма.Элементы.Добавить(
		"ГруппаУправлениеПрисоединеннымиФайламиШапка" + НомерЭлемента, Тип("ГруппаФормы"), ГруппаРазмещенияНаФорме);
	...
	Если ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
		...
		ЭлементПредпросмотра = Форма.Элементы.Добавить("ПолеКартинкиПрисоединенногоФайла" + НомерЭлемента, Тип(
			"ПолеФормы"), ГруппаРазмещенияНаФорме); // РасширениеПоляФормыДляПоляВвода
		...
		ГруппаДобавленияКонтекстногоМеню = Форма.Элементы.Добавить("ГруппаДобавленияФайлаКонтекстноеМеню"
			+ НомерЭлемента, Тип("ГруппаФормы"), КонтекстноеМенюПредпросмотра);

		ГруппаДобавленияКонтекстногоМеню.Вид = ВидГруппыФормы.ГруппаКнопок;

		Если ЗначениеЗаполнено(РеквизитРазмещения) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(РеквизитРазмещения)) Тогда
	...
	Если Не ПустаяСтрока(ДобавляемыйЭлемент.Заголовок) Тогда
		...
		ДекорацияЗаголовок = Форма.Элементы.Добавить("ЗаголовокУправленияПрисоединеннымиФайлами" + НомерЭлемента, Тип(
			"ДекорацияФормы"), ГруппаШапка);
	...
	Если ДобавляемыйЭлемент.ВыводитьЗаголовокФайла Тогда
		...
		ГиперссылкаЗаголовка = Форма.Элементы.Добавить("ЗаголовокПрисоединенногоФайла" + НомерЭлемента, Тип(
			"КнопкаФормы"), ГруппаШапка);
		...
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(РеквизитРазмещения)) Тогда
			...
			РеквизитыПрисоединенногоФайла  = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитРазмещения,
				"Наименование, Расширение");
			ГиперссылкаЗаголовка.Заголовок = РеквизитыПрисоединенногоФайла.Наименование + ?(СтрНачинаетсяС(
				РеквизитыПрисоединенногоФайла.Расширение, "."), "", ".") + РеквизитыПрисоединенногоФайла.Расширение;
		КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
		...
		ПанельКоманд = Форма.Элементы.Добавить("ПанельКомандУправленияПрисоединеннымиФайлами" + НомерЭлемента, Тип(
			"ГруппаФормы"), ГруппаШапка);
		...
		ГруппаПодменю = Форма.Элементы.Добавить("ГруппаДобавленияФайла" + НомерЭлемента, Тип("ГруппаФормы"),
			ПодменюДобавления);
	...
	Если ДобавляемыйЭлемент.ДобавлятьФайлы Тогда
		...
		ЗагрузитьФайл = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиЗагрузитьФайл_" + ТолькоОдинФайлТекст
			+ НомерЭлемента);
		ЗагрузитьФайл.Действие  = "Подключаемый_КомандаПанелиПрисоединенныхФайлов";
		ЗагрузитьФайл.Подсказка = НСтр("ru = 'Загрузить файл с диска'");
		...
		Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
			...
			КнопкаЗагрузки = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиЗагрузитьФайл"
				+ НомерЭлемента, ПанельКоманд, ЗагрузитьФайл.Имя);
			...
			КнопкаЗагрузкиИзПодменю = ДобавитьКнопкуНаФорму(Форма,
				"УправлениеПрисоединеннымиФайламиЗагрузитьФайлИзПодменю" + НомерЭлемента, ГруппаПодменю,
				ЗагрузитьФайл.Имя);
		...
		Если Не ДобавляемыйЭлемент.ТолькоОдинФайл Тогда
			...
			СоздатьПоШаблону = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиСоздатьПоШаблону_"
				+ НомерЭлемента);
			СоздатьПоШаблону.Заголовок = НСтр("ru = 'Создать по шаблону...'");
			ЗаполнитьЗначенияСвойств(СоздатьПоШаблону, СвойстваКомандыФормы);
			...
			Если ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
				ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиСоздатьПоШаблонуИзКонтекстногоМеню"
					+ НомерЭлемента, ГруппаДобавленияКонтекстногоМеню, СоздатьПоШаблону.Имя);
			КонецЕсли;

			Сканировать = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиСканировать_" + НомерЭлемента);
			Сканировать.Заголовок = ?(ОбщегоНазначения.ЭтоМобильныйКлиент(), НСтр("ru = 'Сфотографировать...'"), НСтр(
				"ru = 'Сканировать...'"));
			ЗаполнитьЗначенияСвойств(Сканировать, СвойстваКомандыФормы);
			...
			Если ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
				ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиСканироватьИзКонтекстногоМеню"
					+ НомерЭлемента, ГруппаДобавленияКонтекстногоМеню, Сканировать.Имя);
			КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ВыбиратьФайл И ДоступноИзменение Тогда
		...
		Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
			...
			КнопкаВыбораФайла = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиВыбратьФайл"
				+ НомерЭлемента, ПодменюДобавления, ВыбратьФайл.Имя);
		...
		Если ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
			...
			КнопкаВыбораИзКонтекстногоМеню = ДобавитьКнопкуНаФорму(Форма, "ВыбратьФайлИзКонтекстногоМеню"
				+ НомерЭлемента, КонтекстноеМенюПредпросмотра, ВыбратьФайл.Имя);
	...
	Если ДобавляемыйЭлемент.ОчищатьФайл И ДоступноИзменение Тогда
		...
		Если ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
			ДобавитьКнопкуНаФорму(Форма, "ОчиститьИзКонтекстногоМеню" + НомерЭлемента, КонтекстноеМенюПредпросмотра,
				ОчиститьФайл.Имя);
		КонецЕсли;
	...
	Если ДобавляемыйЭлемент.РедактироватьФайл = "ВФорме" Тогда
		...
		РедактироватьФайл           = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиОткрытьФорму_"
			+ НомерЭлемента);
	...
	ИначеЕсли ДобавляемыйЭлемент.РедактироватьФайл = "Непосредственно" И ДоступноИзменение Тогда

		РедактироватьФайл           = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиРедактироватьФайл_"
			+ НомерЭлемента);
		...
		ПоместитьФайл           = Форма.Команды.Добавить("УправлениеПрисоединеннымиФайламиПоместитьФайл_"
			+ НомерЭлемента);
		...
		Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
			...
			ГруппаНепосредственногоРедактирования = Форма.Элементы.Добавить(
				"ГруппаНепосредственногоРедактирования" + НомерЭлемента, Тип("ГруппаФормы"), ПанельКоманд);
			...
			КнопкаРедактирования = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиРедактироватьФайл"
				+ НомерЭлемента, ГруппаНепосредственногоРедактирования, РедактироватьФайл.Имя);

			КнопкаПомещения = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиПоместитьФайл"
				+ НомерЭлемента, ГруппаНепосредственногоРедактирования, ПоместитьФайл.Имя);

			КнопкаОтмены = ДобавитьКнопкуНаФорму(Форма, "УправлениеПрисоединеннымиФайламиОтменитьРедактирование"
				+ НомерЭлемента, ГруппаНепосредственногоРедактирования, ОтменитьРедактирование.Имя);

			УстановитьДоступностьРедактирования(ДанныеФайлаРазмещения, КнопкаРедактирования, КнопкаОтмены,
				КнопкаПомещения);
		...
		Если ДобавляемыйЭлемент.ПоказыватьПредпросмотр Тогда
			...
			ГруппаРедактированияВМеню = Форма.Элементы.Добавить(
				"ГруппаРедактированияВМеню" + НомерЭлемента, Тип("ГруппаФормы"), КонтекстноеМенюПредпросмотра);
