...
//                                      которые были сформированы в фоновом задании.
//
Функция ВыполнитьФункциюВНесколькоПотоков(ИмяФункции, Знач ПараметрыВыполнения,
	Знач НаборПараметровФункции = Неопределено) Экспорт
	...
	ПараметрыВыполненияДочернихПотоков.АдресРезультата = ПоместитьВоВременноеХранилище(Неопределено,
	...
	ПараметрыМногопоточнойОперации.Вставить("ПрерватьВыполнениеЕслиОшибка",
		ПараметрыВыполнения.ПрерватьВыполнениеЕслиОшибка);
	ПараметрыМногопоточнойОперации.Вставить("АдресРезультатов", АдресРезультатов);

	Возврат ВыполнитьФункцию(ПараметрыВыполненияДочернихПотоков, ИмяМетодаМногопоточногоПроцесса(),
		ПараметрыМногопоточнойОперации);
...
//                                      которые были сформированы в фоновом задании.
//
Функция ВыполнитьПроцедуруВНесколькоПотоков(ИмяПроцедуры, Знач ПараметрыВыполнения,
	Знач НаборПараметровПроцедуры = Неопределено) Экспорт
	...
	Результат = ВыполнитьФункцию(ПараметрыВыполненияДочернихПотоков, ИмяМетодаМногопоточногоПроцесса(),
		ПараметрыМногопоточнойОперации);
...
Функция ВыполнитьВФоне(Знач ИмяПроцедуры, Знач ПараметрыПроцедуры, Знач ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ЗапуститьНеВФоне И ПараметрыВыполнения.ЗапуститьВФоне Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Параметры ""%1"" и ""%2""
			|не могут одновременно принимать значение %3 в %4.'"), "ЗапуститьНеВФоне", "ЗапуститьВФоне", "Истина",
	...
	Если ПараметрыВыполнения.БезРасширений И ПараметрыВыполнения.СРасширениямиБазыДанных Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Параметры ""%1"" и ""%2""
			|не могут одновременно принимать значение %3 в %4.'"), "БезРасширений", "СРасширениямиБазыДанных",
			"Истина", "ДлительныеОперации.ВыполнитьВФоне");
	КонецЕсли;
#Если ВнешнееСоединение Тогда
	...
	Если ПараметрыВыполнения.БезРасширений И ИнформационнаяБазаФайловая Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			...
			|в файловой информационной базе в %2.'"), "БезРасширений", "ДлительныеОперации.ВыполнитьВФоне");
	ИначеЕсли ПараметрыВыполнения.СРасширениямиБазыДанных И ИнформационнаяБазаФайловая Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Фоновое задание не может быть запущено с параметром ""%1""
			|в файловой информационной базе в %2.'"), "СРасширениямиБазыДанных", "ДлительныеОперации.ВыполнитьВФоне");
	КонецЕсли;
#КонецЕсли
	...
	Если ПараметрыВыполнения.Свойство("АдресРезультата") Тогда
		...
		ИначеЕсли Не ЭтоАдресВременногоХранилища(ПараметрыВыполнения.АдресРезультата) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				...
				|в %2.'"), "ПараметрыВыполнения.АдресРезультата", "ДлительныеОперации.ВыполнитьВФоне");
		КонецЕсли;
...
#Если ВнешнееСоединение Тогда
	ВыполнитьБезФоновогоЗадания = ИнформационнаяБазаФайловая Или ОбщегоНазначения.РежимОтладки()
		Или ПараметрыВыполнения.ЗапуститьНеВФоне Или (ЕстьФоновыеЗаданияВФайловойИБ()
		И Не ПараметрыВыполнения.ЗапуститьВФоне) Или Не ВозможноВыполнитьВФоне(ИмяПроцедуры);
#Иначе
		ВыполнитьБезФоновогоЗадания = Не ПараметрыВыполнения.БезРасширений И Не ПараметрыВыполнения.СРасширениямиБазыДанных
...
#КонецЕсли
	...
	Попытка
		Задание = ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяПроцедуры, ПараметрыВыполнения,
			ПараметрыЭкспортнойПроцедуры, БезопасныйРежим, ПараметрыВыполнения.ОжидатьЗавершение <> Неопределено);
	Исключение
		Результат.Статус = "Ошибка";
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Отмена выполнения фонового задания'",
			ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация, , , КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()));
	КонецПопытки;
	...
	ИначеЕсли Задание = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
				 ...
				 |%2
				 |
				 |См. также журнал регистрации.'"), Результат.КраткоеПредставлениеОшибки,
			Результат.ПодробноеПредставлениеОшибки);
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
				 ...
				 |%4
				 |
				 |См. также журнал регистрации.'"), Результат.КраткоеПредставлениеОшибки, Задание.ИмяМетода, Строка(
			ИдентификаторЗадания), Результат.ПодробноеПредставлениеОшибки);
	КонецЕсли;
...
Функция ЗапуститьВыполнениеВФоне(Знач ИдентификаторФормы, Знач ИмяЭкспортнойПроцедуры, Знач Параметры,
	...
	Если ОбщегоНазначения.РежимОтладки() Или ЗапущеноЗаданий > 0 Тогда
	...
	Иначе
		...
		БезопасныйРежим = БезопасныйРежим();
		УстановитьОтключениеБезопасногоРежима(Истина);
		Задание = ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяЭкспортнойПроцедуры, ПараметрыВыполнения,
			ПараметрыЭкспортнойПроцедуры, БезопасныйРежим);
		УстановитьОтключениеБезопасногоРежима(Ложь);
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Задание = Неопределено) Экспорт
	...
	Если Задание = Неопределено Тогда
		РезультатИзОповещения = ПолучитьИзОповещений(Ложь, ИдентификаторЗадания, "ДлительнаяОперацияЗавершена");
		Если РезультатИзОповещения <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(Результат, РезультатИзОповещения);
			Возврат Результат;
		КонецЕсли;
		Результат.КраткоеПредставлениеОшибки = НСтр(
			"ru = 'Операция не выполнена из-за аварийного завершения фонового задания.'");
		Результат.ПодробноеПредставлениеОшибки = Результат.КраткоеПредставлениеОшибки + Символы.ПС + НСтр(
			"ru = 'Фоновое задание не существует'") + ": " + Строка(ИдентификаторЗадания);
...
Процедура ВыполнитьПроцедуруМодуляОбъектаОбработки(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.ЭтоВнешняяОбработка Тогда
		...
		Если ЗначениеЗаполнено(Ссылка) И ОбщегоНазначения.ПодсистемаСуществует(
			"СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
...
Функция ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяПроцедуры, ПараметрыВыполнения, ПараметрыПроцедуры = Неопределено,
	БезопасныйРежим = Ложь, ОтправлятьОповещения = Ложь) Экспорт
	...
	Возврат ВыполнитьФоновоеЗадание(ПараметрыВыполнения, ИмяПроцедурыФоновогоЗадания, ПараметрыПроцедурыФоновогоЗадания,
...
Функция ЭтоЗапускБезВызоваПередЗапускомПрограммы() Экспорт
	...
	Если ФоновоеЗадание = Неопределено Или ФоновоеЗадание.ИмяМетода
		<> ИмяПроцедурыФоновогоЗаданияБезВызоваПередЗапускомПрограммы() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ВыполнитьСКонтекстомКлиента(ВсеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыКлиента.Получить("КлючСеансаРодителя")) И Не ЗначениеЗаполнено(
		ПараметрыКлиента.Получить("ИдентификаторЗаданияМногопоточногоПроцесса")) Тогда

		ФоновоеЗадание = ПолучитьТекущийСеансИнформационнойБазы().ПолучитьФоновоеЗадание();
		Если ФоновоеЗадание <> Неопределено И ВсеПараметры.ИмяПроцедуры = ИмяМетодаМногопоточногоПроцесса() Тогда
			ПараметрыКлиента = Новый Соответствие(ПараметрыКлиента);
	...
	Попытка
		Если ВсеПараметры.ПараметрыВыполнения.Свойство("ЭтоФункция") И ВсеПараметры.ПараметрыВыполнения.ЭтоФункция Тогда
			ВызватьФункцию(ВсеПараметры.ИмяПроцедуры, ВсеПараметры.ПараметрыПроцедуры,
				ВсеПараметры.ПараметрыВыполнения.АдресРезультата);
		Иначе
...
Функция ПолучитьИзОповещений(ПропускатьПолученные, ИдентификаторЗадания, ВидОповещений)
	...
	Для Каждого Оповещение Из Оповещения Цикл
		...
		Если ТипЗнч(Параметры) <> Тип("Структура") Или Не Параметры.Свойство("Результат") Или ТипЗнч(
			Параметры.Результат) <> Тип("Структура") Тогда
			Продолжить;
		КонецЕсли;
		Если ВидОповещений = "ДлительнаяОперацияЗавершена" И Параметры.Свойство("ВидОповещения")
			И Параметры.ВидОповещения = ВидОповещений Тогда
			Возврат Параметры.Результат;
		КонецЕсли;
		Если Не Параметры.Результат.Свойство(ВидОповещений) Или ВидОповещений = "Сообщения" И ТипЗнч(
			Параметры.Результат[ВидОповещений]) <> Тип("ФиксированныйМассив") Или ВидОповещений = "Прогресс" И ТипЗнч(
			Параметры.Результат[ВидОповещений]) <> Тип("Структура") Тогда
			Продолжить;
		КонецЕсли;
		Если ВидОповещений = "Сообщения" Тогда
	...
	Если ПропускатьПолученные И ЗначениеЗаполнено(Оповещение) Тогда
		...
		Для Каждого КлючИЗначение Из ПолученныеОповещения Цикл
			ПолученноеОповещение = ?(КлючИЗначение.Значение.Сообщения = Неопределено, КлючИЗначение.Значение.Прогресс,
				КлючИЗначение.Значение.Сообщения);
...
Функция ПропуститьОповещение(Данные) Экспорт
	...
	Возврат Данные.ИмяОповещения = ИмяОповещения() И Данные.Результат.ВидОповещения = "ДлительнаяОперацияЗавершена"
...
Процедура ОтправитьОповещениеКлиенту(ВидОповещения, ПередаваемоеЗначение, ФоновоеЗадание = Неопределено) Экспорт
	...
	Иначе
		...
		Если ФоновоеЗадание = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ИдентификаторЗаданияМногопоточногоПроцесса = ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить(
	...
	Если ВидОповещения = "СообщениеПользователю" Тогда
		Результат.Сообщения = Новый ФиксированныйМассив(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
	...
	ПараметрыОповещения = Новый Структура("ВидОповещения, ИдентификаторЗадания, Результат", ВидОповещения,
		ИдентификаторОсновногоЗадания, Результат);
	...
	СерверныеОповещения.ОтправитьСерверноеОповещениеСИдентификаторомГруппы(ИмяОповещения(), ПараметрыОповещения,
		Адресаты, Не ЗаписатьСообщенияПользователю, ИдентификаторОсновногоЗадания);
...
Функция ВыполнитьФоновоеЗадание(ПараметрыВыполнения, ИмяМетода, Параметры, Ключ, Наименование)
	...
	Если ТекущийРежимЗапуска() = Неопределено И ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
	...
	ИначеЕсли ПараметрыВыполнения.СРасширениямиБазыДанных Тогда
		Возврат РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(ИмяМетода, Параметры, Ключ,
			Наименование);
	Иначе
...
Функция СписокПараметров(Знач Параметр1, Знач Параметр2, Знач Параметр3, Знач Параметр4, Знач Параметр5,
	Знач Параметр6, Знач Параметр7)
...
Функция ВыполнитьМногопоточныйПроцесс(ПараметрыОперации) Экспорт
	...
	Для Каждого КлючЗначение Из Порции Цикл
		Результаты.Вставить(КлючЗначение.Ключ,
	...
	Попытка
		...
		Для Каждого КлючЗначение Из Порции Цикл
			...
			Пока Не ПорцияОбработана Цикл
				...
				Поток = ДобавитьПоток(ПараметрыОперации, ПотокиПроцесса, ИдентификаторФормы, КлючЗначение.Ключ,
					КлючЗначение.Значение);

				Если ТипЗнч(Поток) = Тип("СтрокаТаблицыЗначений") Тогда
					Результат = ВыполнитьПоток(ПараметрыОперации, Поток, Результаты[КлючЗначение.Ключ],
						ИдентификаторФормы);
				...
				СтатусОжидания = ОжидатьСвободныйПоток(ИдентификаторФормы, Результаты, Поток.ИдентификаторЗадания,
					ПараметрыОперации.ПрерватьВыполнениеЕслиОшибка);
		...
		Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
			...
			Исключение
				...
				Если КлючПотока <> Неопределено Тогда
					...
					Результаты[КлючПотока].ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(
						ИнформацияОбОшибке);
...
Функция ДопустимоеКоличествоПотоков()
	...
	Если ОбщегоНазначения.РазделениеВключено() Или ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Возврат 1;
	КонецЕсли;
