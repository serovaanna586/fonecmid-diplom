...
Функция ТипЗначенияОграниченныйСвязьюПоТипу(Настройки, ПользовательскиеНастройки, ЭлементНастройки,
	ОписаниеЭлементаНастройки, ТипЗначения = Неопределено) Экспорт
	...
	Если ТипЗнч(СвязанныйЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") И (Не СвязанныйЭлементНастройки.Использование
		Или ДопустимыеВидыСравнения.Найти(СвязанныйЭлементНастройки.ВидСравнения) = Неопределено) Тогда
		Возврат ТипЗначения;
	КонецЕсли;

	ОписаниеСвязанногоЭлементаНастройки = ОтчетыКлиентСервер.НайтиДоступнуюНастройку(Настройки,
		СвязанныйЭлементНастройки);
...
Функция СведенияОЭлементеНастройки(КомпоновщикНастроек, Идентификатор) Экспорт
	...
	Элемент = ОтчетыКлиентСервер.ПолучитьОбъектПоПользовательскомуИдентификатору(
		Настройки, ЭлементПользовательскойНастройки.ИдентификаторПользовательскойНастройки, ИерархияНастроек,
...
Процедура ВыбратьПериод(Форма, ИмяКоманды, ВариантПериода = Неопределено) Экспорт
	...
	Если ВариантПериода = ПредопределенноеЗначение("Перечисление.ВариантыПериода.Финансовый") Тогда
		...
		ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборФинансовогоПериода", Контекст, Форма, , , ,
			Обработчик);
...
Процедура СдвинутьПериод(Форма, ИмяКоманды) Экспорт
	...
	ИначеЕсли Период.ДатаНачала = НачалоГода(Период.ДатаОкончания) И КонецГода(Период.ДатаНачала)
	...
	ИначеЕсли Период.ДатаНачала = НачалоГода(Период.ДатаОкончания) И (Месяц(Период.ДатаОкончания) = 6 Или Месяц(
		Период.ДатаОкончания) = 9) Тогда
...
Функция ПриДобавленииВКоллекциюНужноУказыватьТипЭлемента(ТипКоллекции) Экспорт
	Возврат ТипКоллекции <> Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") И ТипКоллекции <> Тип(
...
Функция ВыборПереопределен(ФормаОтчета, Знач Обработчик, ЭлементКомпоновки, ДоступныеТипы, ОтмеченныеЗначения,
	...
	ИнтеграцияПодсистемБСПКлиент.ПриНачалеВыбораЗначений(ФормаОтчета, УсловияВыбора, Обработчик,
		ОткрытьСтандартнуюФорму);
	ОтчетыКлиентПереопределяемый.ПриНачалеВыбораЗначений(ФормаОтчета, УсловияВыбора, Обработчик,
		ОткрытьСтандартнуюФорму);
...
Функция ЭтоВыборПользователей(Форма, ЭлементФормы, ДоступныеТипы, Знач ОтмеченныеЗначения, ПараметрыВыбора, Обработчик) Экспорт
	...
	Если КоличествоТипов = 1 Тогда
		ЭтоВыборПользователей = ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.Пользователи"))
			Или ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи"));

	ИначеЕсли КоличествоТипов = 2 Тогда
		ЭтоВыборПользователей = ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.Пользователи"))
			И ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи")) Или ДоступныеТипы.СодержитТип(Тип(
			"СправочникСсылка.Пользователи")) И ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ГруппыПользователей"))
			Или ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи")) И ДоступныеТипы.СодержитТип(Тип(
			"СправочникСсылка.ГруппыВнешнихПользователей"));

	ИначеЕсли КоличествоТипов = 4 Тогда
		ЭтоВыборПользователей = ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.Пользователи"))
			И ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ГруппыПользователей")) И ДоступныеТипы.СодержитТип(Тип(
			"СправочникСсылка.ВнешниеПользователи")) И ДоступныеТипы.СодержитТип(Тип(
			"СправочникСсылка.ГруппыВнешнихПользователей"));
	Иначе
	...
	Если ЭтоВыборПользователей Тогда
		ПроверитьОтмеченныеЗначения(ОтмеченныеЗначения, ДоступныеТипы);
		ВыбратьТип = КоличествоТипов = 4 Или КоличествоТипов = 2 И ДоступныеТипы.СодержитТип(Тип(
			"СправочникСсылка.Пользователи")) И ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи"));
		...
		Иначе
			...
			Форма.ПоказатьВыборИзМеню(
				...
				СписокТиповПользователей, ЭлементФормы);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьОтмеченныеЗначения(ОтмеченныеЗначения, ДоступныеТипы)
	...
	Пока Индекс >= 0 Цикл
		...
		Если Не ДоступныеТипы.СодержитТип(ТипЗнч(Элемент.Значение)) Или Не ЗначениеЗаполнено(Элемент.Значение) Тогда
