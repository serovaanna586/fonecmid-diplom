...
//
Процедура ДобавитьФайл(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть, ДатаДляРазмещенияВТоме = Неопределено,
	ЗаполнятьСлужебныйРеквизитХранилище = Ложь, ТомДляРазмещения = Неопределено) Экспорт

	ЗаполнитьСведенияОФайле(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть, ДатаДляРазмещенияВТоме,
		ЗаполнятьСлужебныйРеквизитХранилище);
...
//                                     иначе - том будет выбран автоматически.
//
Процедура ЗаполнитьСведенияОФайле(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть, ДатаДляРазмещенияВТоме = Неопределено,
	ЗаполнятьСлужебныйРеквизитХранилище = Ложь) Экспорт

	ОжидаемыеТипы = Новый Массив;
	ОжидаемыеТипы.Добавить(Тип("ДвоичныеДанные"));
	ОжидаемыеТипы.Добавить(Тип("Строка"));
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("РаботаСФайламиСлужебный.ДобавитьФайлВТом", "ДвоичныеДанныеИлиПуть",
		ДвоичныеДанныеИлиПуть, Новый ОписаниеТипов(ОжидаемыеТипы));

	Если ТипЗнч(ПрисоединенныйФайл) = Тип("Структура") И Не ЗначениеЗаполнено(ПрисоединенныйФайл.Ссылка) Тогда

		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заполнено значение свойства %1 в параметре %2 (Структура) в процедуре %3.'"), "Ссылка",
	...
	Если ТипЗнч(ДвоичныеДанныеИлиПуть) = Тип("Строка") Тогда
		...
		Иначе
			...
			НСтр("ru = 'Не удалось добавить файл ""%1"" ни в один из томов, т.к. он отсутствует.
				 ...
				 |Обратитесь к администратору.'"), ПрисоединенныйФайл.Наименование + "."
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		ДанныеФайла = ?(ТипЗнч(ДвоичныеДанныеИлиПуть) = Тип("Строка"), Новый ДвоичныеДанные(ДвоичныеДанныеИлиПуть),
...
//
Процедура ПереименоватьФайл(ПрисоединенныйФайл, Знач НовоеИмя, Знач СтароеИмя = "",
	...
	Попытка
		...
		НовыйПутьКФайлу = ФайлНаДиске.Путь + РаботаСФайламиСлужебныйКлиентСервер.УникальноеИмяСПутем(ФайлНаДиске.Путь,
			НовоеИмяФайла);
...
Функция СвойстваФайлаВТоме(Файл = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Файл) Тогда
		...
		Если РеквизитыФайла.Свойство("Владелец") И ЗначениеЗаполнено(РеквизитыФайла.Владелец) Тогда
			СвойстваФайла.ВладелецФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыФайла.Владелец,
				"ВладелецФайла");
		КонецЕсли;
...
Функция ПолноеИмяФайлаВТоме(СвойстваФайла, ДатаДляРазмещенияВТоме = Неопределено) Экспорт
	...
	ИмяФайла = СвойстваФайла.Наименование + ?(ЗначениеЗаполнено(СвойстваФайла.НомерВерсии), "."
		+ СвойстваФайла.НомерВерсии, "") + ?(СтрНайти(СвойстваФайла.Расширение, ".") > 0, СвойстваФайла.Расширение, "."
		+ СвойстваФайла.Расширение);

	Попытка
		Возврат КорневойКаталог + РаботаСФайламиСлужебныйКлиентСервер.УникальноеИмяСПутем(КорневойКаталог, ИмяФайла);
	Исключение
...
Функция ПолныйПутьТома(Том) Экспорт
	...
	КорневойКаталог = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Том, ?(ОбщегоНазначения.ЭтоWindowsСервер(),
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ПутьКТомуБезУчетаРегиональныхНастроек() Тогда
			ЗначениеРазделителя = ?(МодульРаботаВМоделиСервиса.ИспользованиеРазделителяСеанса(), Формат(
				МодульРаботаВМоделиСервиса.ЗначениеРазделителяСеанса(), "ЧГ=;"), "");
		Иначе
...
Функция ОбъемТома(Том) Экспорт
	...
	Для Каждого Тип Из ТипыПрисоединенныхФайлов Цикл
		...
		Если Тип = Тип("СправочникСсылка.ВерсииФайлов") Или Тип = Тип(
...
Процедура ПроверкаСсылокНаНесуществующиеФайлыВТоме(Проверка, ПараметрыПроверки) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Или Не ХранитьФайлыВТомахНаДиске() Тогда
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Добавление файла'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОписанияОшибки, ПрисоединенныйФайл.Наименование + "." + ПрисоединенныйФайл.Расширение, Строка(
			ПрисоединенныйФайл.Том), ПутьКТому, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));

		Если Пользователи.ЭтоПолноправныйПользователь() Тогда

			СтрокаИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОписанияОшибки, ПрисоединенныйФайл.Наименование + "." + ПрисоединенныйФайл.Расширение, Строка(
				ПрисоединенныйФайл.Том), ПутьКТому, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Иначе
			...
			СтрокаИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось добавить файл:
					 |""%1.%2"".
					 |
					 |Обратитесь к администратору.'"), ПрисоединенныйФайл.Наименование, ПрисоединенныйФайл.Расширение);
...
Процедура ДобавитьТом(КаталогПоискаУдаляемыхФайлов, Том)
	...
	Если ОбщегоНазначения.РазделениеВключено() И (ЗначениеЗаполнено(Том.ПолныйПутьLinux) И СтрНайти(
		Том.ПолныйПутьLinux, "%z") = 0 Или ЗначениеЗаполнено(Том.ПолныйПутьWindows) И СтрНайти(Том.ПолныйПутьWindows,
		"%z") = 0) Тогда
	...
	ВремяПоследнейОчисткиФайлов = ?(КаталогПоискаУдаляемыхФайлов.ВремяПоследнейОчисткиФайлов
		< Том.ВремяПоследнейОчисткиФайлов, Том.ВремяПоследнейОчисткиФайлов,
...
Процедура ОчиститьУдаленныеФайлыВТоме(Каталог)
	...
	Для Каждого Файл Из УдаленныеФайлы(Каталог.Тома, КорневойКаталог, ФайлыДляУдаления) Цикл
		ЗаписьЖурналаРегистрации(НСтр("ru='Работа с файлами.Очистка файлов'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ТомаХраненияФайлов, , НСтр(
			"ru='Удален файл:'") + Символы.НПП + Файл);
		УдалитьФайл(Файл);
	КонецЦикла;
	...
	Исключение
		...
		Ошибка = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru='Работа с файлами.Очистка файлов'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ТомаХраненияФайлов, Том.Ссылка,
...
Функция ЗапросПроверкиСуществованияФайлов(Знач ТаблицаФайлы)
	...
	Для Каждого Тип Из ТипыПрисоединенныхФайлов Цикл
		...
		ТекстЗапроса = СтрЗаменить(ШаблонЗапроса, "&СправочникПрисоединенныхФайлов",
			МетаданныеПрисоединенныеФайлы.ПолноеИмя());
	...
	Для Каждого Файл Из НайтиФайлы(Каталог, ПолучитьМаскуВсеФайлы(), Ложь) Цикл
		Если Файл.ЭтоКаталог() Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
				ФайлыДляУдаления, ФайлыДляУдаления(Файл.ПолноеИмя, ВремяПоследнейОчистки));
		Иначе
...
Процедура ПослеОбновленияДанныхФайла(Контекст, Успешно) Экспорт
	...
	Иначе
		...
		ОсновнойФайл = ?(ЭтоВерсия, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрисоединенныйФайл, "Владелец"),
			ПрисоединенныйФайл);
...
Функция СведенияОФайлеИзИмени(ИмяФайла)
	...
	Если ЧастиИмени.Количество() = 2 Тогда
		...
		СведенияОФайле.ИмяСправочника = ?(Метаданные.Справочники.Найти(ИмяСправочника) = Неопределено, "",
			ИмяСправочника);
		СведенияОФайле.УникальныйИдентификатор = Новый УникальныйИдентификатор(ЧастиИмени[1]);
		СведенияОФайле.Полные = ЗначениеЗаполнено(СведенияОФайле.ИмяСправочника) И ЗначениеЗаполнено(
			СведенияОФайле.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ЗаполнитьЛишниеФайлы(ТаблицаФайловНаДиске, Том) Экспорт
	...
	Для Каждого СправочникФайлов Из ТипыФайлов Цикл
		...
		ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "&ВладелецФайла", "СправочникПрисоединенныеФайлы." + ?(
			ЭтоСправочникВерсий, "Владелец.ВладелецФайла", "ВладелецФайла"));
		ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "&Отредактировал", "СправочникПрисоединенныеФайлы." + ?(
			ЭтоСправочникВерсий, "Автор", "Изменил"));
		ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "&СправочникПрисоединенныеФайлы", "Справочник."
			+ МетаданныеСправочника.Имя);

		Если Не ЭтоСправочникВерсий И ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия", МетаданныеСправочника) Тогда

			СправочникВерсийФайлов = Метаданные.НайтиПоТипу(
				МетаданныеСправочника.Реквизиты.ТекущаяВерсия.Тип.Типы()[0]);
			ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "&УсловиеТекущаяВерсия", "СправочникПрисоединенныеФайлы.ТекущаяВерсия = ЗНАЧЕНИЕ(Справочник."
...
Функция ТомДоступен(Том, ПредставлениеТома, Путь, ПараметрыПроверки)
	...
	Если ПустаяСтрока(Путь) Тогда
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
			"ru = 'У тома хранения файлов ""%1"" не задан путь к сетевому каталогу. Сохранение файлов в него невозможно.'"),
			ПредставлениеТома);
	...
	Исключение
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Том хранения файлов ""%1"" недоступен по причине: 
				 ...
				 |Невозможна работа со всеми файлами, хранящимися в этом томе.'"), Путь, КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()));
...
Процедура ПоискСсылокНаНесуществующиеФайлыВТомах(ОбъектМетаданных, ПараметрыПроверки, ДоступныеТома)
	...
	// @query-part-2
	ПолеВладельца = ?(ПолноеИмя = "Справочник.ВерсииФайлов", "ПРЕДСТАВЛЕНИЕССЫЛКИ(ОбъектМетаданных.Владелец) ",
		"Неопределено ");
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			СвойстваФайлаВТоме = Новый Структура("Том, ПутьКФайлу", СтрокаРезультата.ТомСсылка,
				СтрокаРезультата.ПутьКФайлу);
			...
			Если СтрокаРезультата.Владелец <> Неопределено Тогда
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Версия ""%1"" файла ""%2"" не существует в томе ""%3"".'"), СтрокаРезультата.Файл,
					СтрокаРезультата.Владелец, СтрокаРезультата.Том);
			Иначе
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Файл ""%1"" не существует в томе ""%2"".'"), СтрокаРезультата.Файл, СтрокаРезультата.Том);
			КонецЕсли;
			...
			Если ОбъектМетаданных.Реквизиты.Найти("Ответственный") <> Неопределено Тогда
				Проблема.Вставить("Ответственный", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаОбъект,
					"Ответственный"));
			КонецЕсли;
...
Функция ПроверитьОбъектПрисоединенныхФайлов(ОбъектМетаданных)
	...
	Если СтрЗаканчиваетсяНа(ОбъектМетаданных.Имя, "ПрисоединенныеФайлы") Или ОбъектМетаданных.ПолноеИмя()
		= "Справочник.ВерсииФайлов" Тогда

		Возврат ОбъектМетаданных.Реквизиты.Найти("ПутьКФайлу") <> Неопределено И ОбъектМетаданных.Реквизиты.Найти(
			"Том") <> Неопределено;
...
Процедура ЗаполнитьНастройкиХраненияФайлов() Экспорт
	...
	Константы.СпособХраненияФайлов.Установить(
		?(Константы.ХранитьФайлыВТомахНаДиске.Получить(), "ВТомахНаДиске", "ВИнформационнойБазе"));
